...
/**
 * The three title orders name the same concepts differently, and a consumer who
 * filters an order by the field names it collects silently loses the brackets
 * whose names it does not recognise. Nothing throws: the bracket was never in
 * the order, so a composer that reports what it dropped correctly reports
 * nothing.
 *
 * That shipped twice on one implementation. TCG titles lost every parallel
 * because `product_finish` did not match `print_finish`, and Lot titles lost
 * the product because `manufacturer_product_set` was read as `manufacturer`
 * alone -- it is one combined bracket, not an alias for the first third of one.
 * Both survived a test asserting the rendered order is a subsequence of the
 * canonical order, which a missing bracket satisfies trivially.
 *
 * So CSM states the mapping rather than leaving each consumer to infer it.
 * Translate with `semCanonicalBracket` before matching names; the canonical
 * vocabulary is the Standard grammar's.
 *
 * See COS-39.
 */
export const semBracketAliases = Object.freeze({
...
/** A grammar's order, translated into the canonical vocabulary. */
export function semCanonicalTitleOrder(grammar) {
  const order = grammar === "tcg" ? semTcgTitleOrder
    : grammar === "lot" ? semLotTitleOrder
      : semStandardTitleOrder;
  return Object.freeze([...new Set(order.map((name) => semCanonicalBracket(grammar, name)))]);
}
...
export function isSemCardNumberText(value = "", {
  ...
  // TCG subsets number themselves with a lettered prefix on both sides of the
  // slash: Trainer Gallery TG22/TG30, Galarian Gallery GG01/GG70, Shiny Vault
  // SV01/SV122. The predicate rejected all of them while accepting 086/070 --
  // all digits, via the rarity branch -- and OP01-120, letters with a hyphen.
  // A reviewed title in the evaluation set carries TG22/TG30 verbatim as the
  // card number, so the contract's predicate was rejecting a value the
  // contract's own ground truth treats as one.
  //
  // Gated on TCG grammar so Standard validation keeps its rule that a checklist
  // code never contains a slash -- that is what separates a checklist code from
  // a print run there, and loosening it would let 17/50 read as a card number.
  if (cleanUpper(grammar) === "TCG" && /^[A-Z]{1,4}\d{1,5}\/[A-Z]{1,4}\d{1,5}$/i.test(text)) return true;
