...
as $$
  select
    ...
    ci.fields,
    ci.retrieval_status,
    coalesce(cri.reference_status, cri.metadata->>'reference_status') as reference_status,
    jsonb_strip_nulls(
      coalesce(cri.metadata, '{}'::jsonb)
      || jsonb_build_object(
        'retrieval_status', ci.retrieval_status,
        'identity_reference_status', ci.reference_status,
        'reference_status', coalesce(cri.reference_status, cri.metadata->>'reference_status')
      )
...
comment on function public.match_card_image_embeddings(
  ...
  'Returns top-K visually similar card identity candidates by cosine distance, including identity/reference review status so runtime can keep candidate-only visual neighbors out of GPT prompts.';
