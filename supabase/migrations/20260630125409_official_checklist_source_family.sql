...
as $$
  ...
  scored as (
    select
      ...
      (
        ...
        case when coalesce(s.source_type, '') like '%\_OFFICIAL_CHECKLIST' escape '\' then 0.04 else 0 end +
    ...
    where c.review_status in ('REVIEW_REQUIRED', 'REVIEWED_INTERNAL')
      and c.source_status in ('AUTO_PARSED_FROM_VERIFIED_TITLE', 'TOPPS_OFFICIAL_RAW', 'OFFICIAL_CHECKLIST_RAW', 'REVIEWED_INTERNAL')
      and (
  ...
  select
    ...
    case
      ...
      when scored.source_status in ('TOPPS_OFFICIAL_RAW', 'OFFICIAL_CHECKLIST_RAW') then 'registry'
...
comment on function public.search_catalog_candidates(text, text, text, text, text, text, text, integer)
  is 'Catalog-first identity candidate lookup. Official checklist source types share one scoring path; current-query hidden labels must not be sent to model prompts.';
