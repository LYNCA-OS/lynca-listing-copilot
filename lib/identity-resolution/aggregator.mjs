...
function rawSourceType(candidate = {}) {
  return normalizeText(candidate.source || candidate.source_type || "MARKETPLACE").toUpperCase();
}

function retrievalCandidateEvidenceEligible(candidate = {}) {
  const source = rawSourceType(candidate);
  if (source === "VISUAL_VECTOR") {
    return candidate.identity_evidence_eligible === true || candidate.selected === true;
  }
  return true;
}
...
function evidenceItemsFromRetrievalCandidates(candidates = []) {
  return (Array.isArray(candidates) ? candidates : [])
    .filter(retrievalCandidateEvidenceEligible)
    .flatMap((candidate) => {
...
function canonicalEvidenceItem(item = {}, index = 0) {
  const canonicalValue = canonicalValueKey(item.field, item.value);
  const source = normalizeSource(item.source);
  const originalSource = item.metadata?.original_source || item.source || null;

  return {
    evidence_id: `ev_${String(index + 1).padStart(4, "0")}`,
    field: item.field,
    value: item.value,
    canonical_value: canonicalValue,
    canonical_key: [item.field, canonicalValue, source, item.image_id || "", item.region || item.metadata?.region || ""].join("::"),
    source,
    original_source: originalSource,
    source_rank: sourceRank(source),
    confidence: item.confidence,
    image_id: item.image_id || null,
    capture_role: item.metadata?.capture_role || null,
    region: item.region || item.metadata?.region || null,
    metadata: item.metadata || {}
  };
}

function sourceAliasSummary(items = []) {
  const aliases = {};
  items.forEach((item) => {
    const original = item.metadata?.original_source || item.source;
    if (!original) return;
    aliases[original] = normalizeSource(item.source);
  });
  return aliases;
}

function fieldSourceMatrix(items = []) {
  const matrix = {};
  items.forEach((item) => {
    matrix[item.field] ||= {};
    matrix[item.field][item.source] = (matrix[item.field][item.source] || 0) + 1;
  });
  return matrix;
}
...
export function canonicalizeIdentityEvidence(input = {}) {
  ...
  return {
    ...
    canonical_items: evidenceItems.map(canonicalEvidenceItem),
    ...
    source_aliases: sourceAliasSummary(evidenceItems),
    field_source_matrix: fieldSourceMatrix(evidenceItems),
    normalization_policy: {
      field_aliases: "legacy provider fields normalize into identityFieldNames",
      source_priority: "SLAB_LABEL > CARD_BACK_PRINTED_TEXT > CARD_FRONT_PRINTED_TEXT > INTERNAL_APPROVED_HISTORY > OFFICIAL_CHECKLIST > STRUCTURED_DATABASE > VECTOR_APPROVED_REFERENCE > MODEL_INFERENCE > PRIMARY_FAST_VISION > OCR_ONLY > MARKETPLACE > VISUAL_GUESS",
