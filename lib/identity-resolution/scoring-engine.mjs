...
export function scoreCandidate(candidate = {}, {
  ...
  const positive =
    ...
    + weights.focusedVisualSupport * component(candidate, "focused_visual_support")
    + weights.taxonomySerialSupport * component(candidate, "taxonomy_serial_support")
  ...
  return {
    ...
    score_formula: "w1*OCR_confidence + w2*cross_view_agreement + w3*registry_match + w4*slab_match + w5*retrieval_support + w6*focused_visual_support + w7*taxonomy_serial_support + w8*structural_validity - w9*conflict_penalty - w10*glare_penalty - w11*marketplace_overreliance_penalty"
  };
}
