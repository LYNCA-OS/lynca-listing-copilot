...
export function focusedVisualParallelEvidence(item = {}, field = "") {
  ...
  return ["MODEL_INFERENCE", "PRIMARY_FAST_VISION"].includes(normalizeSource(item.source))
    && captureRole === "focused_reread"
    && region.includes("CROP_AND_READ_PARALLEL")
    && statusAllowsPublish;
}

// The finish family -- Refractor, Prizm, Holo, Mojo -- is a coarse visual
// property of the same kind as the colour, and the first-pass vision read is
// trusted for the colour at this confidence already. Requiring a focused
// crop-and-read for the family alone is the difference between "Orange" and
// "Orange Refractor" on a card where the model read both from the same image
// and was right about both: across the reviewed benchmark parallel_family was
// dropped as RESOLVER_NOT_PRESERVED on every card that carried one, costing
// refractor six times and the finish word most of the rest.
//
// The fine-grained parallel_exact ("Orange Lava Refractor") stays gated, so the
// specific claim still needs real backing; only the family-level one rides on
// the same evidence as the colour it sits next to.
const primaryVisionCoarseParallelFields = new Set(["surface_color", "parallel_family"]);

function primaryVisionSurfaceColorEvidence(item = {}, field = "") {
  if (!primaryVisionCoarseParallelFields.has(field)) return false;
  if (normalizeSource(item.source) !== "PRIMARY_FAST_VISION") return false;
  const metadata = item.metadata || {};
  const unresolvedReason = normalizeText(metadata.field_unresolved_reason).toLowerCase();
  if (/review|operator|manual|uncertain|unresolved|conflict/.test(unresolvedReason)) return false;
  if (metadata.color_corrected !== undefined
    && !["1", "true", "yes", "on"].includes(String(metadata.color_corrected).trim().toLowerCase())) {
    return false;
  }
  return Number(item.confidence || 0) >= 0.78;
}
...
function candidateHasGroundedIdentitySource(candidate = {}, field = "", context = {}) {
  return (candidate.evidence_items || []).some((item) => {
    ...
    return sourceIsSlab(item.source)
      ...
      || primaryVisionSurfaceColorEvidence(item, field)
