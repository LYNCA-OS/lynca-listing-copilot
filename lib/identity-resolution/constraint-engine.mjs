...
export function validateCandidate(candidate = {}, context = {}) {
  ...
  if (field === "players") {
    const hasAllowedSource = (candidate.evidence_items || []).some((item) => sourceIsOcr(item.source) || sourceIsRegistry(item.source) || sourceIsRetrieval(item.source));
    if (!hasAllowedSource) {
      violations.push(violation(field, "player_without_grounded_source", "player must come from OCR, registry, or retrieval evidence"));
    }
  }
