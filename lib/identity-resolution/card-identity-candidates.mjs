...
function evidenceSnapshot(aggregation = {}) {
  return {
    ...
    official_card_type: aggregationValues(aggregation, "official_card_type"),
    observable_components: aggregationValues(aggregation, "observable_components").flatMap((value) => Array.isArray(value) ? value : [value]),
...
function scoreWholeCardCandidate(candidate = {}, {
  ...
  const evidenceOfficialCardType = evidence.official_card_type.length ? evidence.official_card_type : evidence.card_type;
  if (evidenceOfficialCardType.length && (fields.official_card_type || fields.card_type)) {
    if (evidenceOfficialCardType.some((value) => compatibleText(value, fields.official_card_type || fields.card_type))) {
      score = addSignal(score, signals, "official_card_type", 0.06);
    } else {
      addConflict(conflicts, "official_card_type", "candidate official_card_type conflicts with card evidence", "MEDIUM");
    }
  }
