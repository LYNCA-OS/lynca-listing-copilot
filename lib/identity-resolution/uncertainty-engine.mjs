...
function conflictIntensity(conflicts = []) {
  ...
  const unresolvedConflicts = conflicts.filter((conflict) => conflict.resolved !== true);
  if (!unresolvedConflicts.length) return 0;
  const maxSeverity = Math.max(...unresolvedConflicts.map((conflict) => severityWeight(conflict.severity)));
  const volume = Math.min(0.2, unresolvedConflicts.length * 0.05);
  return clamp01(maxSeverity + 0.2 + volume);
}
