...
function draftFieldPolicy(fieldState = {}, {
  ...
  if (!hasSelected) {
    ...
    displayPolicy = sourceSummaryOnly(fieldState, ["AGNES_INFERENCE", "VISUAL_GUESS", "MARKETPLACE"])
      || Math.max(Number(fieldState.resolution_confidence || 0), maxResolvedEvidenceConfidence(fieldState)) < 0.74
...
function governorRootCause(policy = {}) {
  ...
  if (sources.has("VISUAL_GUESS") || sources.has("PRIMARY_FAST_VISION") || sources.has("AGNES_INFERENCE")) return "SINGLE_MODEL_PERCEPTION_ONLY";
