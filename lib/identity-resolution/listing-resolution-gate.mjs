...
import { finalizeRetrievalApplicationOutcome } from "../listing/candidates/retrieval-application-layer.mjs";
import { collapseRelatedSubjectIdentities } from "../listing/pipeline/subject-identity.mjs";
import { resolveIdentity, resolveIdentityWithConvergence } from "./solver.mjs";
import { identityStatuses } from "./types.mjs";

export const identityResolverPolicyVersion = "listing-identity-resolver-v1";
...
function mergeSubjectValues(...groups) {
  return collapseRelatedSubjectIdentities(groups.flatMap(normalizedTextArray));
}
  ...
  Object.entries(evidence).forEach(([fieldName, field]) => {
    ...
    candidates.forEach((candidate) => {
      evidenceSources(field, candidate).forEach((source) => {
        if (typeof sourceFilter === "function" && !sourceFilter(source, {
          field: fieldName,
          candidate
        })) {
          if (typeof onSourceFiltered === "function") {
            onSourceFiltered(source, { field: fieldName, candidate });
          }
          return;
        }
        items.push({
...
const retrievalCandidateSourceTypes = new Set([
  "APPROVED_HISTORY",
  "APPROVED_MEMORY",
  "CATALOG",
  "INTERNAL_APPROVED_HISTORY",
...
function isRetrievalCandidateSource(source = {}) {
  const types = [
    source.source_type,
    source.source,
  ...
  if (types.some((type) => retrievalCandidateSourceTypes.has(type))) return true;
  if (source.candidate_id || source.candidate_identity_id) return true;
  if (/^(?:supabase|catalog|registry|vector):\/\//i.test(normalizeText(source.source_url))) return true;
  return /catalog|registry|approved[_ -]?(?:reference|history)|retrieval[_ -]?candidate/i.test(
    [source.domain, source.evidence_kind, source.retrieval_status, source.reference_status]
      .map(normalizeText)
      .filter(Boolean)
      .join(" ")
  );
...
function independentNonWeakSourceCount(fieldState = {}) {
  const weak = new Set(["MARKETPLACE", "VISUAL_GUESS", "MODEL_INFERENCE", "MODEL_WORLD_KNOWLEDGE", "PRIMARY_FAST_VISION"]);
...
function draftSelectedValue(fieldState = {}, publishability) {
  const field = fieldState.field;
  const currentImageOverride = primaryFastVisionDraftOverride(fieldState);
  if (currentImageOverride) return currentImageOverride.value;
...
const primaryFastVisionDraftFields = Object.freeze(new Set([
  "year",
  "manufacturer",
  "brand",
  "product",
...
function candidateHasSource(candidate = {}, sources = []) {
  const allowed = new Set(sources);
  return (candidate.evidence_items || []).some((item) => allowed.has(item?.source));
}

function candidateHasAppliedRetrievalEvidence(candidate = {}) {
  return (candidate.evidence_items || []).some((item) => {
    const metadata = item?.metadata || {};
    return metadata.retrieval_application_decision === "APPLY"
      && metadata.retrieval_application_reason === "selected_candidate_safe_field_application";
  });
}

function draftFieldValuesEqual(field, left, right) {
  if (field === "players") {
    const leftPlayers = normalizedTextArray(left);
    const rightPlayers = normalizedTextArray(right);
    return leftPlayers.length === rightPlayers.length
      && subjectContainsAll(leftPlayers, rightPlayers)
      && subjectContainsAll(rightPlayers, leftPlayers);
  }
  return canonicalText(Array.isArray(left) ? left.join(" / ") : left)
    === canonicalText(Array.isArray(right) ? right.join(" / ") : right);
}

function primaryFastVisionDraftOverride(fieldState = {}) {
  const field = fieldState.field;
  if (!primaryFastVisionDraftFields.has(field)) return null;
  const candidates = fieldState.candidates || [];
  if (candidates.some((candidate) => candidateHasSource(candidate, [
    "SLAB_LABEL",
    "CARD_BACK_PRINTED_TEXT",
    "CARD_FRONT_PRINTED_TEXT",
    "OCR_ONLY"
  ...
  // A trusted candidate is allowed to replace a draft field only after the
  // retrieval application layer has selected it against the current card's
  // anchors. Do not let the lower-confidence raw vision snapshot silently
  // undo that explicit APPLY decision in the writer-draft renderer. Nearby or
  // unselected catalog candidates still cannot override the current image.
  const selected = fieldState.resolved_value;
  const selectedHasAppliedRetrievalEvidence = candidates.some((candidate) => (
    candidateHasAppliedRetrievalEvidence(candidate)
      && draftFieldValuesEqual(field, candidate.value, selected)
  ));
  if (selectedHasAppliedRetrievalEvidence) return null;

  const candidate = candidates
    .filter((item) => candidateHasSource(item, ["PRIMARY_FAST_VISION"]))
    .sort((left, right) => {
      const snapshotDelta = Number(right.draft_current_image_snapshot === true)
        - Number(left.draft_current_image_snapshot === true);
      return snapshotDelta || Number(right.score || right.confidence || 0) - Number(left.score || left.confidence || 0);
    })[0];
  const primaryVisionConfidence = Math.max(
    0,
    ...((candidate?.evidence_items || [])
      .filter((item) => item?.source === "PRIMARY_FAST_VISION")
      .map((item) => Number(item.confidence || 0)))
  );
  if (!candidate || primaryVisionConfidence < 0.45) return null;
  if (field === "surface_color" && !new RegExp(`^(?:${colorTokens.join("|")})$`, "i").test(normalizeText(candidate.value))) return null;

  const conflictsWithSelected = hasValue(selected, field)
    && !draftFieldValuesEqual(field, selected, candidate.value);
  const unresolvedOrDropped = !hasValue(selected, field) || fieldState.decision_route !== "USE";
  return conflictsWithSelected || unresolvedOrDropped ? candidate : null;
}
...
function draftFieldPolicy(fieldState = {}, {
  ...
  const field = fieldState.field;
  const currentImageOverride = primaryFastVisionDraftOverride(fieldState);
  let publishability = publishabilityByField[field] || fieldPublishability(fieldState, { criticalFields });
  if (currentImageOverride) publishability = fieldPublishabilityStatuses.REVIEW_REQUIRED;
  ...
  const governor = currentImageOverride
    ? {
        action: accuracyGovernorActions.KEEP,
        display_policy: draftDisplayPolicies.INCLUDE_HIGHLIGHTED,
        reason: "current_image_core_fact_preserved_for_writer_review"
      }
    : accuracyGovernorDecision({
        fieldState,
        selectedValue,
        displayPolicy,
        publishability,
  ...
  return {
    ...
    sources: currentImageOverride
      ? unique((currentImageOverride.evidence_items || []).map((item) => item.source))
      : fieldSources(fieldState),
    draft_source_override: currentImageOverride ? "PRIMARY_FAST_VISION_CONFLICT_REVIEW" : null,
...
function governorRootCause(policy = {}) {
  ...
  if (sources.has("VISUAL_GUESS") || sources.has("PRIMARY_FAST_VISION") || sources.has("MODEL_INFERENCE") || sources.has("MODEL_WORLD_KNOWLEDGE")) return "SINGLE_MODEL_PERCEPTION_ONLY";
...
function writerDraftPresentation({
  ...
  const identity = lotDetected
    ? normalizeResolvedFields({
      ...(identityResolution.identity || identityResolution.resolved_identity || {}),
      multi_card: true
    })
    : identityForWriterDraft(identityResolution, { draftGate });
  ...
  // renderListingPresentation reads three states, not two: true means the
  // current image confirmed the numerator, false means it rejected it, and null
  // means nothing looked. Only false triggers the denominator-only fallback
  // that prints "#/25" in place of "12/25".
  //
  // An `||` expression cannot express the third state -- it collapses "no draft
  // state for serial_number" into false, which the renderer then reads as an
  // active rejection. On the reviewed benchmark that suppressed the numerator
  // on 23 of 27 cards where the pipeline already held the correct value.
  //
  // This is the same mistake as the evidence-level one fixed in the renderer:
  // absence of a signal is not a signal against. Reject only when the draft
  // gate actually says to omit the field.
  const serialDraftConfirms = serialDraftState?.display_policy === "INCLUDE_NORMAL"
    && serialDraftState?.selected_value
    && serialDraftState.selected_value === identity.serial_number;
  const serialDraftRejects = /^OMIT/.test(String(serialDraftState?.display_policy || ""));
  const serialVerifiedForPresentation = serialNumeratorVerified === true || serialDraftConfirms
    ? true
    : serialNumeratorVerified === false || serialDraftRejects
      ? false
      : null;
...
function buildPublicationGate({
  ...
  const writerReviewReady = (modelPublishRecommended || writerDraft.can_render === true) && !titleLanguageBlocked;
  ...
  const workflowRoute = titleLanguageBlocked
    ? publicationStatuses.RESCAN_REQUIRED
    : lotDetected
      ? writerReviewReady ? publicationStatuses.DEEP_REVIEW : publicationStatuses.RESCAN_REQUIRED
    : !writerReviewReady
  ...
  return {
    ...
    blocked_reasons: unique([
      lotDetected ? "MULTI_CARD_LOT_REQUIRES_WRITER_REVIEW" : null,
...
function applyMultiCardLotGuard(identityResolution = {}, lotGuard = {}) {
  ...
  const conflict = {
    field: "multi_card",
    conflict_type: "MULTI_CARD_LOT_REQUIRES_WRITER_REVIEW",
    conflicting_values: [lotGuard.card_count ? `${lotGuard.card_count} cards` : "multiple cards"],
    severity: "MEDIUM",
    reason: "multiple-card input must use Lot grammar and remain writer-reviewed",
    resolved: true,
    resolution: "route_to_lot_writer_draft"
  };
  const trace = {
    field: "multi_card",
    step: "lot_guard",
    ...
    output: {
      ...
      route: "LOT_WRITER_REVIEW"
    },
    decision: "multi_card_lot_routed_to_writer_review",
...
function applyWeakVisualParallelDrop(identityResolution = {}, {
  ...
  const drops = (identityResolution.field_states || []).filter((fieldState) => {
    return ["parallel", "variation"].includes(fieldState.field)
      && hasValue(fieldState.resolved_value, fieldState.field)
      && Number(fieldState.resolution_confidence || 0) < minConfidence
      && sourceSummaryOnly(fieldState, ["MODEL_INFERENCE", "MODEL_WORLD_KNOWLEDGE", "PRIMARY_FAST_VISION", "VISUAL_GUESS"]);
  });
  if (!drops.length) return identityResolution;
...
function descriptorHasStrongConfirmation(fieldState = {}) {
  ...
  const groundedSourceCount = [...sourceSet].filter((source) => {
    return !["MODEL_INFERENCE", "MODEL_WORLD_KNOWLEDGE", "PRIMARY_FAST_VISION", "VISUAL_GUESS", "MARKETPLACE"].includes(source);
  }).length;
...
function identityResolutionGateInput(result = {}, {
  ...
  const retrievalApplication = result.retrieval_application;
  const applicationOwnsCandidates = retrievalApplication?.owns_candidate_application === true;
  const applicationEvidenceItems = applicationOwnsCandidates
    && retrievalApplication.enabled === true
    && Array.isArray(retrievalApplication.identity_evidence_items)
    ? retrievalApplication.identity_evidence_items
    : [];
  const blockedWorldKnowledgeEvidenceItems = Array.isArray(result.world_knowledge?.identity_evidence_items)
    ? result.world_knowledge.identity_evidence_items
    : [];
  const blockedCandidateEvidence = [];
  const directEvidenceItems = evidenceDocumentToIdentityEvidenceItems(evidenceDocument, {
    providerId,
    sourceFilter: applicationOwnsCandidates
      ? (source) => !isRetrievalCandidateSource(source)
      : null,
    onSourceFiltered: applicationOwnsCandidates
      ? (source, context) => blockedCandidateEvidence.push({
          field: context.field,
          value: context.candidate?.value ?? null,
          source_type: source.source_type || source.source || source.original_source_type || null,
          candidate_id: source.candidate_id || null,
          source_url: source.source_url || null
        })
      : null
  });
  const evidenceItems = [...directEvidenceItems, ...applicationEvidenceItems];
  ...
  return {
    ...
    evidenceItems,
    criticalFields,
    retrievalEvidenceIsolation: {
      enabled: applicationOwnsCandidates,
      policy: "raw_retrieval_candidate_evidence_blocked; application_field_evidence_only; world_knowledge_shadow_never_identity_evidence",
      direct_evidence_item_count: directEvidenceItems.length,
      application_evidence_item_count: applicationEvidenceItems.length,
      world_knowledge_evidence_item_count: 0,
      blocked_world_knowledge_evidence_item_count: blockedWorldKnowledgeEvidenceItems.length,
      blocked_raw_candidate_evidence_count: blockedCandidateEvidence.length,
      blocked_fields: unique(blockedCandidateEvidence.map((item) => item.field)),
      blocked_candidate_ids: unique(blockedCandidateEvidence.map((item) => item.candidate_id))
    },
    resolvedHint: evidenceDocument.resolved,
    retrievalCandidates: applicationOwnsCandidates ? [] : retrievalCandidates,
    registryRecords,
    productSchemas,
    options: {
      includeResolvedHint: directEvidenceItems.length === 0,
      criticalFields
    }
  };
}
...
function observedSubjectCandidates(result = {}, evidenceDocument = {}) {
  return [
    result.candidate_observation_snapshot?.players,
    result.candidate_observation_snapshot?.player,
...
const currentImageDraftProviderPattern = /(?:^|_)(?:openai|primary_fast_vision|cascade_fast)(?:_|$)/i;
const rawCurrentImageDraftFields = Object.freeze([
  "year",
  "product",
  "set",
  "players",
...
function currentImageDraftCandidate(field, value) {
  return {
    field,
    value,
    score: 0.55,
    confidence: 0.55,
    sources: ["PRIMARY_FAST_VISION"],
    best_source: "PRIMARY_FAST_VISION",
    marketplace_only: false,
    draft_current_image_snapshot: true,
    evidence_items: [{
      field,
      value,
      ...
      metadata: {
        ...
        original_source: "RAW_PROVIDER_FIELDS",
        draft_only: true
      }
    }]
  };
...
function currentImageDraftFieldState(field, value) {
  const candidate = currentImageDraftCandidate(field, value);
  return {
    field,
    ...
    resolved_value: null,
    resolution_confidence: 0,
    resolution_reason: "current_image_fact_preserved_for_writer_draft",
    ambiguity: false,
    decision_route: "DROP",
    supporting_sources: [],
    conflicting_sources: [],
...
function currentImageDraftFacts(result = {}, evidenceDocument = {}, providerId = "") {
  const provider = normalizeText(providerId || result.provider || result.source || "");
  if (!currentImageDraftProviderPattern.test(provider)) return {};
  const raw = [
    result.raw_observed_fields,
    result.raw_provider_fields,
    // Candidate selection freezes the last pre-retrieval current-image
    // snapshot. It can retain a safe fact after normalization turns the same
    // field into null because of a downstream conflict. Keep it last so null
    // normalized fields cannot erase a literal year/basic color/subject draft.
    result.candidate_observation_snapshot
  ].filter((item) => item && typeof item === "object" && !Array.isArray(item));
  if (!raw.length) return {};
  // raw_observed_fields may intentionally null a disputed value. For the small
  // draft-only safe-field allowlist, preserve the current provider observation
  // as the final snapshot and route conflicts to writer review. High-risk
  // serial, grade, exact parallel, and boolean claims remain excluded below.
  return normalizeResolvedFields(Object.assign({}, ...raw));
}

function withCurrentImageDraftFacts(identityResolution = {}, result = {}, evidenceDocument = {}, providerId = "") {
  const observed = currentImageDraftFacts(result, evidenceDocument, providerId);
  const observedEntries = rawCurrentImageDraftFields
    .map((field) => [field, observed[field]])
    .filter(([field, value]) => hasValue(value, field));
  if (!observedEntries.length) return identityResolution;

  const observedByField = new Map(observedEntries);
  const seen = new Set();
  const fieldStates = (identityResolution.field_states || []).map((fieldState) => {
    const field = fieldState.field;
    if (!observedByField.has(field)) return fieldState;
    seen.add(field);
    const value = observedByField.get(field);
    const candidates = fieldState.candidates || [];
    const representedIndex = candidates.findIndex((candidate) => draftFieldValuesEqual(field, candidate.value, value));
    if (representedIndex >= 0) {
      if (candidateHasSource(candidates[representedIndex], ["PRIMARY_FAST_VISION"])) return fieldState;
      const snapshot = currentImageDraftCandidate(field, value);
      return {
        ...fieldState,
        candidates: candidates.map((candidate, index) => index === representedIndex
          ? {
              ...candidate,
              sources: unique([...(candidate.sources || []), "PRIMARY_FAST_VISION"]),
              draft_current_image_snapshot: true,
              evidence_items: [...(candidate.evidence_items || []), ...snapshot.evidence_items]
            }
          : candidate)
      };
    }
    return {
      ...fieldState,
      candidates: [currentImageDraftCandidate(field, value), ...candidates]
    };
  });

  observedEntries.forEach(([field, value]) => {
    if (!seen.has(field)) fieldStates.push(currentImageDraftFieldState(field, value));
  });
  return {
    ...identityResolution,
...
function preserveObservedMultiSubjectIdentity(identity = {}, result = {}, evidenceDocument = {}) {
  ...
  const originalCurrent = normalizedTextArray(identity.players);
  const current = mergeSubjectValues(originalCurrent);
  const currentWasCanonicalized = current.length !== originalCurrent.length
    || current.some((value, index) => value !== originalCurrent[index]);
  if (!observed.length) return currentWasCanonicalized
    ? normalizeResolvedFields({ ...identity, players: current })
    : identity;
  if (current.length >= observed.length) return currentWasCanonicalized
    ? normalizeResolvedFields({ ...identity, players: current })
    : identity;
  if (current.length && !subjectContainsAll(observed, current)) return identity;
  ...
  const nextFieldStates = fieldStates.map((fieldState) => {
    ...
    return {
      ...
      candidates: [
        ...
        currentImageDraftCandidate("players", identity.players)
      ]
    };
  });

  if (!touched && !fieldStates.some((fieldState) => fieldState.field === "players")) {
    const candidate = currentImageDraftCandidate("players", identity.players);
    nextFieldStates.push({
      ...currentImageDraftFieldState("players", identity.players),
      candidates: [candidate],
      resolved_value: identity.players,
      resolution_confidence: 0.55,
      resolution_reason: "observed_multi_subject_preserved",
      decision_route: "USE",
      supporting_sources: [{ source: "PRIMARY_FAST_VISION", confidence: 0.55 }],
      source_summary: [{ source: "PRIMARY_FAST_VISION", count: 1 }]
    });
    touched = true;
  }
...
function finishIdentityResolutionGate(result = {}, {
  ...
  criticalFields = [],
  identityResolution = {},
  retrievalEvidenceIsolation = null
} = {}) {
  ...
  const draftIdentityResolution = withCurrentImageDraftFacts(
  ...
  //
  // Three states, not two. Downstream, false means "the current image was asked
  // about this numerator and would not confirm it" and prints the denominator
  // alone ("#/50"); null means nobody looked, and the resolved value stands.
  // Collapsing the two costs the numerator on cards where it was never in
  // doubt, so a rejection is only claimed when verification was actually asked
  ...
  // Two ways of having asked: verification was demanded up front, or a focused
  // crop-and-read ran. Either one that did not come back with the numerator is
  // a refusal by the current image. A trace entry carries the attempt in its
  // action name even when it produced no focused_vision summary to fold in.
  // Rollback without a deploy. Set LISTING_TRUST_UNCONTRADICTED_SERIAL_REREAD=0
  // to restore the previous behaviour, where any requested re-read that did not
  // update the field suppressed the numerator.
  //
  // Default-on because the measured cost of the old behaviour is 16 of 25
  // numbered cards losing a correct token, and the risk it guards against --
  // publishing a numerator the card does not carry -- is still guarded: an
  // actual conflict from the re-read still suppresses, and every title is
  // confirmed by a writer before listing.
  const trustUncontradictedSerialReread = !["0", "false", "no", "off"].includes(
    String(process.env.LISTING_TRUST_UNCONTRADICTED_SERIAL_REREAD ?? "1").trim().toLowerCase()
  );
  const serialReadAsked = (
    Array.isArray(result.fast_vision_policy?.secondary_verification_required_fields)
      ? result.fast_vision_policy.secondary_verification_required_fields
      : []
  ).includes("serial_number")
    || serialTrace.some((entry) => /serial/i.test(String(entry?.action || "")));
  // A focused re-read that changed nothing is not a rejection.
  //
  // This was `serialReadAsked ? false : null`: ask for a second look, get no
  // update, conclude the numerator is unverified. But "no update" is the
  // ordinary outcome when the first read was already right, so the most common
  // success was being recorded as a failure. Downstream that made
  // `serialNumeratorVerified === false`, which fails both routes that keep a
  // numerator in the title, and the renderer fell back to `#/D`.
  //
  // Measured on the 50-card sealed set: of 25 cards whose reviewed title
  // carries a serial, the pipeline rendered exactly 1 correctly and dropped the
  // numerator on 16 -- while `print_run_numerator` sat in the resolved fields,
  // correct, on nearly all of them. The bare model got 12 right on the same
  // cards precisely because nothing was gating it.
  //
  // This is the same fallacy the comment in listing-renderer.mjs warns about
  // one layer up -- absence of evidence treated as evidence against -- so the
  // three outcomes are now distinct, as the contract requires: an update
  // confirms, a recorded conflict rejects, and silence is UNKNOWN. Only a
  // conflict may suppress.
  // Narrow on purpose: this only relaxes the case that was measured.
  //
  // When the provider did supply print-run evidence, a requested re-read that
  // did not update the field still suppresses the numerator and asks the writer
  // — that policy is deliberate and pinned by tests, and evidence that exists
  // but was not confirmed is real doubt.
  //
  // The measured failure is the other case: no print-run evidence entry at all.
  // There, "the re-read changed nothing" carries no information whatsoever, and
  // treating it as a rejection threw away a correct numerator on 16 of the 25
  // numbered cards in the sealed 50.
  const printRunEvidenceEntry = result.evidence?.print_run_number
    || result.evidence?.numerical_rarity
    || result.evidence?.serial_number;
  const serialConflicted = focusedConflictingFields(serialTrace).has("serial_number");
  const serialNumeratorVerified = focusedUpdatedFields(serialTrace).has("serial_number")
    ? true
    : serialConflicted
      ? false
      : serialReadAsked
        && !(trustUncontradictedSerialReread && !printRunEvidenceEntry)
        ? false
        : null;
  ...
  const draftGate = buildDraftGate(draftIdentityResolution, {
    criticalFields,
    reviewItems,
    publishabilityByField
  });
  ...
  const writerDraft = writerDraftPresentation({
    identityResolution: draftIdentityResolution,
    evidence: result.evidence || {},
    maxLength,
    titleLanguageBlocked,
  ...
  const unresolved = unique([
    ...
    ...(lotGuard.detected ? ["multi-card lot requires writer review"] : []),
    ...(!canRenderFinalTitle
  ...
  const renderedTitle = activePresentation?.rendered_title || presentation.rendered_title || "";
  ...
  const retrievalApplicationOutcome = finalizeRetrievalApplicationOutcome({
    result,
    resolvedAfter: finalIdentity,
    titleAfter: finalTitle
  });
  const activeModules = activePresentation?.modules || result.modules;
  const activeModuleOrder = activePresentation?.module_order || result.module_order;
  const renderedLegacyFields = publicLegacyFields(activeIdentity, {});
  const renderedFields = {
    ...(result.rendered_fields && typeof result.rendered_fields === "object" && !Array.isArray(result.rendered_fields)
    ...
    module_order: activeModuleOrder,
    title_render_source: titleRenderSource,
    fields: activeIdentity
  };

  return {
    ...result,
    ...retrievalApplicationOutcome.result_patch,
    retrieval_evidence_isolation: retrievalEvidenceIsolation,
    title: finalTitle,
...
export function applyIdentityResolutionGate(result = {}, {
  ...
  return finishIdentityResolutionGate(result, {
    ...
    retrievalEvidenceIsolation: input.retrievalEvidenceIsolation,
    identityResolution
  });
}
...
export async function applyIdentityResolutionGateWithConvergence(result = {}, {
  ...
  const applicationOwnsCandidates = result.retrieval_application?.owns_candidate_application === true;
  const input = identityResolutionGateInput(result, {
    providerId,
    retrievalCandidates,
    registryRecords,
  ...
  const identityResolution = await resolveIdentityWithConvergence({
    ...
    registryRecords: input.registryRecords,
    productSchemas: input.productSchemas,
    retrieveEvidence: applicationOwnsCandidates ? null : retrieveEvidence,
    options: {
      ...input.options,
