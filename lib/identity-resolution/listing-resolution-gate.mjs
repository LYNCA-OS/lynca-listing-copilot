...
function independentNonWeakSourceCount(fieldState = {}) {
  const weak = new Set(["MARKETPLACE", "VISUAL_GUESS", "AGNES_INFERENCE"]);
...
function draftSelectedValue(fieldState = {}, publishability) {
  ...
  if (field === "serial_number"
    ...
    && (fieldState.ambiguity === true || fieldStateHasUnresolvedConflict(fieldState))) {
    return ambiguousSerialPlaceholder(fieldState) || value;
  }
...
function draftFieldPolicy(fieldState = {}, {
  ...
  if (!hasSelected) {
    ...
    displayPolicy = priority === reviewPriorities.CRITICAL && fieldHasBlockingConflict(fieldState)
...
function finishIdentityResolutionGate(result = {}, {
  ...
  const reviewItems = writerReviewItems(gatedIdentityResolution, criticalFields, [
    ...publishabilityReviewItems(gatedIdentityResolution, criticalFields),
    ...secondaryReviewItems,
    ...exactTaxonomyReviewItems(gatedIdentityResolution),
    ...yearAutoPublishReviewItems(gatedIdentityResolution)
  ]);
  const publishabilityByField = fieldPublishabilityMap(gatedIdentityResolution, {
    criticalFields,
    reviewItems
  });
  const draftGate = buildDraftGate(gatedIdentityResolution, {
    criticalFields,
    reviewItems,
    publishabilityByField
  });
  ...
  return {
    ...
    confidence: canRenderFinalTitle
      ? confidenceForIdentityStatus(gatedIdentityResolution.status, {
          existingConfidence: result.confidence,
          reason: result.reason,
          unresolved
        })
      : canRenderWriterDraft ? confidenceForWriterDraft(result.confidence, reviewItems) : "LOW",
