...
function primaryFastVisionDraftOverride(fieldState = {}) {
  ...
  const contradictoryDirectText = candidates.some((directCandidate) => (
    candidateHasSource(directCandidate, [
      "SLAB_LABEL",
      "CARD_BACK_PRINTED_TEXT",
      "CARD_FRONT_PRINTED_TEXT",
      "OCR_ONLY"
    ])
      && hasValue(directCandidate.value, field)
      && !draftFieldValuesEqual(field, directCandidate.value, candidate.value)
  ));
  if (contradictoryDirectText) return null;
...
export const __listingResolutionGateTestHooks = Object.freeze({
  primaryFastVisionDraftOverride
});
