...
function primaryFastVisionDraftOverride(fieldState = {}) {
  ...
  const candidate = candidates
    ...
    .sort((left, right) => Number(right.score || right.confidence || 0) - Number(left.score || left.confidence || 0))[0];
  ...
  const conflictsWithSelected = hasValue(selected, field)
    && canonicalText(Array.isArray(selected) ? selected.join(" / ") : selected)
      !== canonicalText(Array.isArray(candidate.value) ? candidate.value.join(" / ") : candidate.value);
  const unresolvedOrDropped = !hasValue(selected, field) || fieldState.decision_route !== "USE";
  return (fieldState.conflicts === true && conflictsWithSelected) || unresolvedOrDropped ? candidate : null;
}
