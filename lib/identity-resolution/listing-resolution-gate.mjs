...
function normalizedSubjectArray(value) {
  return normalizedTextArray(value)
    .flatMap((entry) => entry.split(/\s*;\s*|\s*\n\s*/))
    .map(normalizeText)
    .filter(Boolean);
}
...
function serialNumeratorVerificationState(result = {}, trace = []) {
  if (focusedConflictingFields(trace).has("serial_number")) return false;
  ...
  const upstream = hasCanonical
    ? result.serial_numerator_verified
    : hasLegacyAlias
      ? result.serialNumeratorVerified
      : null;
  if (upstream === false) return false;
  if (upstream === true) return true;
  if (focusedUpdatedFields(trace).has("serial_number")) return true;
  return null;
}
...
function observedSubjectCandidates(result = {}, evidenceDocument = {}) {
  ...
  ].flatMap(normalizedSubjectArray).filter(Boolean);
}

function rawProviderSubjectCandidates(result = {}) {
  return [
    result.raw_provider_fields?.players,
    result.raw_provider_fields?.player,
    result.raw_provider_fields?.subject
  ].flatMap(normalizedSubjectArray).filter(Boolean);
}
...
function preserveObservedMultiSubjectIdentity(identity = {}, result = {}, evidenceDocument = {}) {
  ...
  const directlyObserved = mergeSubjectValues(rawProviderSubjectCandidates(result));
  ...
  // An entirely empty resolved subject may only be recovered from the current
  // provider's direct observation. Retrieval and catalog candidates are not
  // allowed to manufacture the card's subject set.
  if (!current.length && directlyObserved.length < 2) return identity;
  const recovered = current.length
    ? mergeSubjectValues(current, observed)
    : directlyObserved;
...
function finishIdentityResolutionGate(result = {}, {
  ...
  // A focused-crop re-read counts as direct current-instance provenance, while
  // pending or inconclusive verification remains unknown.
  const serialVerificationTrace = [
  ...
  const serialNumeratorVerified = serialNumeratorVerificationState(result, serialVerificationTrace);
