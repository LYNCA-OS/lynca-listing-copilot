...
function buildPublicationGate({
  ...
  const writerReviewReady = autoPublishAllowed || writerDraft.can_render === true;
  const status = autoPublishAllowed
    ? publicationStatuses.AUTO_PUBLISH_READY
    : writerReviewReady
  ...
  return {
    ...
    partial_writer_draft: !autoPublishAllowed && writerDraft.can_render === true,
    upload_blocked_until_writer_approval: autoPublishAllowed !== true,
...
function finishIdentityResolutionGate(result = {}, {
  ...
  const gatedIdentityResolution = releasePrimaryFastVisionAbstain(lotGuardedIdentityResolution, {
    result,
    criticalFields
  });
  ...
  const reviewItems = writerReviewItems(gatedIdentityResolution, criticalFields, secondaryReviewItems);
  ...
  const publicationGate = buildPublicationGate({
    autoPublishAllowed: canRenderFinalTitle,
    writerDraft,
    reviewItems,
    titleLanguageBlocked,
    lotDetected: lotGuard.detected
