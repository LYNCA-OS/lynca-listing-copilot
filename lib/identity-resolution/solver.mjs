...
export function resolveIdentity({
  ...
  const trace = [];
  const canonicalEvidence = canonicalizeIdentityEvidence({
    evidenceItems,
    resolvedHint,
    retrievalCandidates,
    registryRecords,
  ...
  const allEvidenceItems = canonicalEvidence.evidence_items;
...
function buildCoreConvergenceReport(convergence = {}) {
  ...
  return {
    ...
    phase_sequence: trace.length
      ? ["detect_conflict", "retrieve", "re_evaluate", "converge"]
      : [],
