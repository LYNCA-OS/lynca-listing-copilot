...
function selectFieldCandidate({
  ...
  // Candidate selection already checked trust, anchor agreement, field
  // permissions, and current-card conflicts before emitting APPLY. Preserve
  // that narrow semantic-identity decision through identity resolution;
  // otherwise short printed text such as `Topps Chrome` or a checklist code
  // can silently replace `Topps Chrome UFC` or the official insert name.
  // Triple-conflict and physical-instance guards remain upstream.
  const appliedRetrieval = valid.find(candidateHasAppliedRetrievalEvidence);
  if (appliedRetrieval) {
    return {
      selected: appliedRetrieval,
      reason: "selected_retrieval_application_preserved",
      ambiguity: false,
