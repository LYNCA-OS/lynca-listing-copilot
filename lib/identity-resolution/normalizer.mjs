...
const arrayFields = new Set(["players", "attributes"]);
...
const sourceAliases = Object.freeze({
  ...
  AGNES: "AGNES_INFERENCE",
  AGNES_INFERENCE: "AGNES_INFERENCE",
  VISION_MODEL: "AGNES_INFERENCE",
  OCR: "OCR_ONLY",
  OCR_ONLY: "OCR_ONLY",
...
export function normalizeGradeCompany(value) {
  const text = normalizeText(value);
  if (!text) return null;
  if (/^(beckett|bgs)$/i.test(text)) return "BGS";
  if (/^psa$/i.test(text)) return "PSA";
  if (/^sgc$/i.test(text)) return "SGC";
  if (/^cgc$/i.test(text)) return "CGC";
  return text;
}
...
function normalizeBoolean(value) {
  ...
  return /^(true|yes|y|1|rc|rc logo|rookie|rookie card|rookie logo|rookie ticket|rated rookie|1st bowman|first bowman|auto|autograph|ssp|case hit|patch|relic|sketch|redemption|1\/1)$/i.test(normalizeText(value));
}
...
export function sourceIsGroundTruthEligible(source) {
  ...
  return !["MARKETPLACE", "VISUAL_GUESS", "AGNES_INFERENCE"].includes(normalized);
}
