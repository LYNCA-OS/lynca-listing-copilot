...
function aggregationCanonicalValuesForField(aggregation = {}, field) {
  return unique(aggregationValuesForField(aggregation, field).map((value) => canonicalValueKey(field, value)).filter(Boolean));
}

function fieldHasSingleNonConflictingValue(aggregation = {}, field) {
  return aggregationCanonicalValuesForField(aggregation, field).length === 1;
}

function fieldHasConflictingValues(aggregation = {}, field) {
  return aggregationCanonicalValuesForField(aggregation, field).length > 1;
}

function taxonomyExactPrerequisitesMet(aggregation = {}, identity = {}) {
  if (!normalizeText(identity.product) || !normalizeText(identity.collector_number)) return false;
  if (!fieldHasSingleNonConflictingValue(aggregation, "product")) return false;
  if (!fieldHasSingleNonConflictingValue(aggregation, "collector_number")) return false;
  if (fieldHasConflictingValues(aggregation, "checklist_code")) return false;
  if (fieldHasConflictingValues(aggregation, "serial_number")) return false;
  if (fieldHasConflictingValues(aggregation, "surface_color")) return false;
  if (fieldHasConflictingValues(aggregation, "parallel_family")) return false;
  return true;
}

function explicitParallelConflictPresent(aggregation = {}, selectedName = "") {
  const selected = comparable(selectedName);
  if (!selected) return false;
  const explicitValues = [
    ...aggregationValuesForField(aggregation, "parallel_exact"),
    ...aggregationValuesForField(aggregation, "parallel"),
    ...aggregationValuesForField(aggregation, "variation")
  ].map(normalizeText).filter(Boolean);
  if (!explicitValues.length) return false;
  const uniqueExplicit = unique(explicitValues.map(comparable));
  if (uniqueExplicit.length > 1) return true;
  return uniqueExplicit[0] !== selected;
}
...
export function deriveParallelExactEvidenceFromTaxonomy(aggregation = {}, {
  ...
  const identity = identityContextFromAggregation(aggregation);
  if (!taxonomyExactPrerequisitesMet(aggregation, identity)) return [];
  ...
  if (explicitParallelConflictPresent(aggregation, uniqueNames[0])) return [];
