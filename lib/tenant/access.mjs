...
function legacyBreakGlassConfigured(env = process.env) {
  return Boolean(
    cleanText(env.METAVERSE_USERNAME, 320)
    && String(env.METAVERSE_PASSWORD || "")
    && String(env.METAVERSE_AUTH_SECRET || "")
  );
}

function legacyBreakGlassIdentity({ env = process.env, fallbackEmail = "" } = {}) {
  const email = cleanText(env.METAVERSE_EMAIL || fallbackEmail || env.METAVERSE_USERNAME, 320).toLowerCase();
  return Object.freeze({
...
function buildLegacyBreakGlassContext({ requestId, env = process.env, fallbackEmail = "" } = {}) {
  const identity = legacyBreakGlassIdentity({ env, fallbackEmail });
  return Object.freeze({
    requestId,
    actorType: ACTOR_TYPES.USER,
    ...identity,
    ...
    user: Object.freeze({ id: LEGACY_USER_ID, email: identity.email }),
    breakGlass: true
  });
}
...
export async function resolveTenantIdentityForPrincipal({ tenantId, userId } = {}, options = {}) {
  ...
  if (options.allowLegacyBreakGlass === true
      && normalizedTenantId === LEGACY_TENANT_ID
      && normalizedUserId === LEGACY_USER_ID
      && legacyBreakGlassConfigured(options.env || process.env)) {
    return legacyBreakGlassIdentity({
...
export async function requireTenantAccess(req, {
  ...
  let context = tenantId === LEGACY_TENANT_ID
      && userId === LEGACY_USER_ID
      && Number(session.session_version) === LISTING_SESSION_VERSION
      && legacyBreakGlassConfigured(env)
    ? buildLegacyBreakGlassContext({ requestId, env, fallbackEmail: session.email })
    : null;
  let rows = context ? null : membershipCacheGet(cacheKey, env);
  if (!context && !rows) {
    try {
      rows = await fetchMembershipRows({
