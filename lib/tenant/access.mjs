...
// Short positive-only cache for the requireTenantAccess membership lookup.
// Every API request re-queried tenant_members; under sustained load (batch
// evals, busy tenants) transient Supabase REST failures surfaced as 503
// "Authentication is temporarily unavailable" on ~half the requests. A hit
// skips only the network read — validatedMembership (status, tenant/user
// match, session_version) still runs on every request, so a rotated session
// version is rejected immediately. Revocation of a membership row itself is
// delayed by at most the TTL; negative results and errors are never cached,
// preserving fail-closed semantics.
const membershipCacheMaxEntries = 1000;
const membershipCache = new Map();

function membershipCacheTtlMs(env = process.env) {
  const parsed = Number(env?.TENANT_MEMBERSHIP_CACHE_TTL_MS);
  if (!Number.isFinite(parsed)) return 45_000;
  return Math.max(0, Math.min(300_000, Math.trunc(parsed)));
}

function membershipCacheGet(key, env) {
  const ttl = membershipCacheTtlMs(env);
...
function membershipCacheSet(key, rows, env) {
  if (membershipCacheTtlMs(env) <= 0) return;
  membershipCache.delete(key);
  membershipCache.set(key, { rows, storedAt: Date.now() });
...
export function __clearTenantMembershipCacheForTests() {
...
async function fetchMembershipRows(filters, {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    if (!retryable || attempt === maxAttempts - 1) {
      console.warn("[tenant_membership_auth_unavailable]", JSON.stringify({
        attempts: attempt + 1,
        http_status: status || null,
        transport_error: cleanText(
          transportError?.cause?.code || transportError?.code || transportError?.name,
          80
        ) || null,
        response_shape_invalid: response?.ok === true
      }));
...
function buildUserContext(membership, requestId, fallbackEmail = "") {
  const { user, tenant, role, tenantId, userId, sessionVersion } = membership;
  const email = cleanText(user.email || fallbackEmail, 320).toLowerCase();
  return Object.freeze({
    ...
    role,
    sessionVersion,
    authorizationSource: "PERSISTED_MEMBERSHIP",
...
function sessionAuthorizationTtlMs(env = process.env) {
  const configured = Number(env?.TENANT_SESSION_AUTHZ_TTL_MS);
  const requested = Number.isFinite(configured)
    ? Math.max(0, Math.min(300_000, Math.trunc(configured)))
    : 45_000;
  return Math.min(requested, membershipCacheTtlMs(env));
}

function buildSessionAuthorizationContext(session, requestId, env, now = Date.now()) {
  const ttl = sessionAuthorizationTtlMs(env);
  const checkedAt = Number(session?.membership_checked_at);
  const role = normalizeTenantRole(session?.role);
  if (ttl <= 0 || !role || !Number.isSafeInteger(checkedAt)) return null;
  ...
  const email = cleanText(session?.email, 320).toLowerCase();
  const sessionVersion = Number(session?.session_version);
  if (!tenantId || !userId || !email || !Number.isSafeInteger(sessionVersion) || sessionVersion < 1) return null;
  return Object.freeze({
    requestId,
    actorType: ACTOR_TYPES.USER,
    tenantId,
    ...
    role,
    sessionVersion,
    authorizationSource: "SIGNED_RECENT_MEMBERSHIP",
    tenant: Object.freeze({
      id: tenantId,
      name: cleanText(session?.tenant_name, 320),
      plan: cleanText(session?.tenant_plan, 80)
    }),
...
export async function resolveTenantIdentityForAuthUser({ authUserId, tenantId } = {}, options = {}) {
  ...
  return Object.freeze({
    ...
    tenantName: cleanText(membership.tenant.name, 320),
    tenantPlan: cleanText(membership.tenant.plan, 80)
  });
}
...
export async function requireTenantAccess(req, {
  ...
  let context = buildSessionAuthorizationContext(session, requestId, env, now);
  if (!context) {
    const cacheKey = JSON.stringify([tenantId, userId]);
    let rows = membershipCacheGet(cacheKey, env);
    if (!rows) {
      try {
      ...
      // Positive-only: a single well-formed membership row is cacheable; empty
      // or ambiguous results always re-read so a revoked or missing membership
      // is never served from memory.
      if (rows.length === 1) membershipCacheSet(cacheKey, rows, env);
    }
    ...
    context = buildUserContext(membership, requestId, session.email);
  }
