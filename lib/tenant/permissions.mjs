...
export const TENANT_PERMISSIONS = Object.freeze({
  ...
  VIEW_COST: "VIEW_COST",
  // COS-42 (founder, 2026-08-04): "Field-level semantic approval/correction is
  // a separate trusted reviewer/admin workflow; it is not the default writer
  // workflow." A writer's title edit is cleaned commercial feedback and must
  // never become semantic truth on its own, so approving canonical FIELDS is a
  // different act from editing a title and needs its own permission rather than
  // borrowing EDIT_TITLE, which every writer holds.
  REVIEW_SEMANTIC_FIELDS: "REVIEW_SEMANTIC_FIELDS"
});
...
export const ROLE_PERMISSION_MATRIX = Object.freeze({
  [TENANT_ROLES.OWNER]: Object.freeze({
    ...
    [TENANT_PERMISSIONS.REVIEW_SEMANTIC_FIELDS]: T
  }),
  [TENANT_ROLES.MANAGER]: Object.freeze({
    ...
    [TENANT_PERMISSIONS.VIEW_TEAM]: T,
    [TENANT_PERMISSIONS.RETRY_JOB]: T,
    // Managers may finish work they own, but cannot edit or submit feedback
    // for another writer's assignment. This keeps management access from
    // becoming a tenant-wide writer override while allowing an upload owner to
    // complete the same card journey.
  ...
  [TENANT_ROLES.WRITER]: Object.freeze({
    ...
    [TENANT_PERMISSIONS.EXPORT_DATA]: N,
    [TENANT_PERMISSIONS.VIEW_COST]: N,
    // Deliberately N, and this is the whole point of the permission. A writer
    // holds EDIT_TITLE and SUBMIT_FEEDBACK; neither may become a route to
    // rewriting canonical fields.
    [TENANT_PERMISSIONS.REVIEW_SEMANTIC_FIELDS]: N
  })
});
