...
export function bindProductionRequestContext(res, context = {}) {
  const authorizationSource = safeIdentifier(
    context?.authorizationSource || context?.authorization_source
  );
  if (authorizationSource) {
    res?.setHeader?.("x-lynca-authz-source", authorizationSource);
  }
