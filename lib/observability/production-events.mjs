...
import { writeSupabaseRow, writeSupabaseRows } from "../supabase-rest.mjs";
...
function sanitizedMetadata(value, depth = 0) {
  ...
  return Object.fromEntries(Object.entries(value).flatMap(([key, nested]) => {
    ...
    const safeTimingNumber = /_ms$/.test(normalizedKey) && typeof nested === "number";
    if (!normalizedKey || (sensitivePattern.test(normalizedKey) && !safeTimingNumber)) return [];
...
/**
 * Stage timings, reduced to plain bounded numbers.
 *
 * The route already measures every stage and returns them on the response, but
 * only a FAILING request recorded them -- the provider-failure receipt carried
 * `latency_stages_ms` into `error_logs` while a success threw them away. The
 * requests whose latency we actually want to manage are the successful ones,
 * and for those the breakdown existed for the length of one HTTP response.
 *
 * They ride the request-log write that already happens at `res.end`, so this
 * costs no extra round trip. That timing also matters for completeness: the
 * session patch runs BEFORE persistence finishes, so the receipt stored there
 * can never contain `csm_persistence_ms` or `request_total_ms`. Both are known
 * by the time the response body is serialised.
 */
export function safeLatencyStages(stages) {
  ...
  for (const [key, value] of Object.entries(stages)) {
    ...
    // The cap is per KIND, not one number for everything. An hour is the right
    // ceiling for a duration and a nonsense one for a byte count: a two-image
    // upload of 11,238,422 bytes was silently clamped to 3,600,000 and stored
    // as if that were the size. A ceiling that rewrites a value it does not
    // understand is worse than no ceiling, because the result still looks like
    // a measurement.
    const ceiling = key.endsWith("_bytes") ? 2 ** 40 : 3_600_000;
    out[key] = Math.min(Math.round(number), ceiling);
  }
  return Object.keys(out).length ? out : null;
}

/**
 * The client's own stage timings, shaped before they reach a column.
 *
 * These arrive from the browser, so they are capped and filtered: `client_`
 * keys only, finite non-negative numbers only, and a bounded count. They
 * describe the client's work and are never read back as truth about the server.
 *
 * Shared rather than copied. It began as a private function inside the ingest
 * endpoint, which meant the OTHER title endpoint -- the one the writer flow
 * actually calls -- had no way to accept the same data, so six production cards
 * recorded ten server stages and not one client stage. The ~13s between the
 * server's 6-9.6s and the 23s a writer sees stayed unattributable.
 *
 * The per-kind ceiling is `safeLatencyStages`'s, for its reason: `client_upload_bytes`
 * is a byte count, and an hour-in-milliseconds ceiling silently rewrote an 11MB
 * upload to 3,600,000 and stored it as if that were the size.
 */
export function safeClientTiming(timing) {
...
export function buildRequestLogRow({
  ...
  const stages = safeLatencyStages(latencyStages);
  ...
  return {
    ...
    metadata: {
      ...
      ...(stages ? { latency_stages_ms: stages } : {})
    },
    timestamp: createdAt
  };
}
...
export function buildErrorLogRow({
  ...
  return {
    ...
    metadata: {
      error_fingerprint: operationalErrorFingerprint(error, normalizedType),
      ...sanitizedMetadata({
        provider_request_id: error?.provider_request_id,
        provider_client_request_id: error?.provider_client_request_id,
        provider_status: error?.provider_status,
        provider_error_code: error?.provider_error_code,
        provider_error_type: error?.provider_error_type,
        provider_error_param: error?.provider_error_param,
        provider_ms: error?.provider_ms,
        latency_stages_ms: error?.latency_stages_ms
      })
    },
    created_at: createdAt
  };
...
export function createRequestTelemetry(req, res, { api = "", now = () => Date.now() } = {}) {
  ...
  const telemetry = {
    ...
    async finish({ statusCode = res?.statusCode || 200, latencyStages = null, env, fetchImpl } = {}) {
    ...
    async fail(error, { statusCode = res?.statusCode || 500, recoverable = false, errorType = "", sessionId, jobId, env, fetchImpl } = {}) {
      const [requestLog, errorLog] = await Promise.all([
        telemetry.finish({ statusCode, latencyStages: error?.latency_stages_ms || null, env, fetchImpl }),
...
export function finishProductionRequest(res, statusCode, payload = {}) {
  ...
  const providerFailureReceipt = payload?.provider_failure_receipt
    && typeof payload.provider_failure_receipt === "object"
    ? payload.provider_failure_receipt
    : null;
  const task = statusCode >= 500 || providerFailureReceipt
    ? telemetry.fail(Object.assign(
      new Error(String(payload?.message || payload?.error || "request_failed").slice(0, 240)),
      {
        ...
        provider_request_id: providerFailureReceipt?.provider_request_id,
        provider_client_request_id: providerFailureReceipt?.provider_client_request_id,
        provider_status: providerFailureReceipt?.http_status,
        provider_error_code: providerFailureReceipt?.provider_error_code,
        provider_error_type: providerFailureReceipt?.provider_error_type,
        provider_error_param: providerFailureReceipt?.provider_error_param,
        provider_ms: providerFailureReceipt?.provider_ms,
    ...
    // A success carries its own breakdown on the response body. Reading it here
    // is what closes the gap: the stages that only exist after the packet is
    // persisted -- `csm_persistence_ms`, `request_total_ms` -- are present by
    // the time this runs, and were recorded nowhere before.
    : telemetry.finish({ statusCode, latencyStages: payload?.latency_stages_ms || null });
  try {
    waitUntil(task);
  } catch {
