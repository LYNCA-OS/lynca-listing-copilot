...
async function persistOperationalWrite(kind, write, options = {}) {
  ...
  if (result?.saved) {
    const slowWriteMs = Math.max(1, Math.min(
      30_000,
      Number.parseInt(String(env.PRODUCTION_OBSERVABILITY_SLOW_WRITE_MS || "1000"), 10) || 1_000
    ));
    operationalWriteCircuitOpenUntil = finishedAt - now >= slowWriteMs
      ? finishedAt + circuitMs
      : 0;
    return result;
  }
  operationalWriteCircuitOpenUntil = finishedAt + circuitMs;
  return result;
}
...
export function finishProductionRequest(res, statusCode, payload = {}) {
  ...
  if (statusCode >= 500 && operationalBackendFailureShouldSkipDb(payload)) {
    console.warn("[production_observability_backend_failure_skipped]", JSON.stringify({
      error_code: String(payload?.error_code || "BACKEND_UNAVAILABLE").slice(0, 120),
...
export function operationalBackendFailureShouldSkipDb(payload = {}) {
  const code = String(payload?.error_code || payload?.error_type || "").trim().toUpperCase();
  return payload?.retryable === true && (
    code === "V4_JOB_STATUS_BACKEND_UNAVAILABLE"
    || code === "V4_SESSION_STATUS_BACKEND_UNAVAILABLE"
    || code === "V4_QUEUE_BACKEND_UNAVAILABLE"
  );
}
