...
export function normalizeListingSessionClaims(session, env = process.env) {
  ...
  const role = validIdentityToken(session.role, 40).toUpperCase();
  const membershipCheckedAt = Number(session.membership_checked_at ?? session.membershipCheckedAt);
  const authorizationSnapshotPresent = Boolean(
    session.role
    || session.membership_checked_at !== undefined
    || session.membershipCheckedAt !== undefined
  );
  ...
  if (authorizationSnapshotPresent && (!role || !Number.isSafeInteger(membershipCheckedAt) || membershipCheckedAt < 1)) {
    return null;
  }
  ...
  return {
    ...
    ...(authorizationSnapshotPresent ? {
      role,
      membership_checked_at: membershipCheckedAt,
      tenant_name: cleanText(session.tenant_name ?? session.tenantName, 320),
      tenant_plan: cleanText(session.tenant_plan ?? session.tenantPlan, 80)
    } : {}),
    sid: session.sid,
    iat: session.iat,
    exp: session.exp
...
export function createListingSessionToken(identity, secret = process.env.METAVERSE_AUTH_SECRET, {
  ...
  const legacyUser = identityUserId === LEGACY_USER_ID && identityTenantId === LEGACY_TENANT_ID
    ...
    : "";
  const role = validIdentityToken(identity?.role, 40).toUpperCase();
  const session = normalizeListingSessionClaims({
    ...
    ...(role ? {
      role,
      membership_checked_at: Number(identity?.membership_checked_at ?? identity?.membershipCheckedAt) || now,
      tenant_name: identity?.tenant?.name ?? identity?.tenant_name ?? identity?.tenantName ?? "",
      tenant_plan: identity?.tenant?.plan ?? identity?.tenant_plan ?? identity?.tenantPlan ?? ""
    } : {}),
