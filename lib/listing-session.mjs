...
export function listingSessionCookieIsSecure(req, env = process.env) {
  // Vercel terminates TLS before invoking the function. Force Secure there so
  // a spoofed forwarding header can never downgrade a production cookie.
  if (env.VERCEL === "1" || ["production", "preview"].includes(String(env.VERCEL_ENV || "").toLowerCase())) {
    return true;
  }
