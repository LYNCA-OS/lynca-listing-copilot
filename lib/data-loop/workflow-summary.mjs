import { labelsForCsmFields } from "../listing/csm/field-labels.mjs";
...
function writerActionText(status, fields = []) {
  ...
  if (fields.length) return `请优先确认黄色模块：${labelsForCsmFields(fields).slice(0, 6).join(", ")}。`;
...
function operatorNextActions({
  ...
  if (status === "LOW_TOUCH_REVIEW") {
    ...
    addAction(actions, `先处理黄色模块：${labelsForCsmFields(reviewFields).slice(0, 6).join(", ")}。`, "review");
  } else {
