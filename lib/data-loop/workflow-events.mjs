...
const sidecarKeys = Object.freeze([
  "paddle_ocr",
  "splink",
  "cleanlab",
  "label_studio",
...
export function buildRecognitionWorkflowEvent({
  ...
  return {
    ...
    // payload tenant is trusted by the V4 API/worker boundary; provider output
    // must never be allowed to choose ownership for sidecar persistence.
    tenant_id: cleanText(payload.tenant_id || payload.tenantId || result.tenant_id || result.tenantId) || null,
...
export function summarizeWorkflowTriggers(event = {}) {
  ...
  return {
    ...
    lightgbm: catalogCandidates.length > 0 || vectorCandidates.length > 0,
    phoenix: true
  };
}
