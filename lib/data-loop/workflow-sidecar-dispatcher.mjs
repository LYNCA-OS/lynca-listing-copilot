...
async function dispatchCatalogGapQueue({ result = {}, config, payload = {}, fetchImpl = globalThis.fetch }) {
  try {
    if (result.catalog_gap_queue_candidate !== true) {
    ...
    const existing = await supabaseSelectRows({
      ...
      query: `?asset_id=eq.${encodeURIComponent(assetId)}&status=eq.PENDING&select=gap_id&limit=1`,
      fetchImpl
    });
    if (existing.length) {
    ...
    for (const field of ["category", "year", "manufacturer", "brand", "product", "set", "subset", "insert", "players", "collector_number", "checklist_code", "official_card_type", "surface_color"]) {
      if (resolved[field] !== undefined && resolved[field] !== null && resolved[field] !== "") identityFields[field] = resolved[field];
    }
    const instanceFields = {};
    for (const field of ["serial_number", "numerical_rarity", "grade_company", "card_grade", "auto_grade", "grade_type"]) {
      if (resolved[field] !== undefined && resolved[field] !== null && resolved[field] !== "") instanceFields[field] = resolved[field];
    }
    const imageIds = (Array.isArray(payload.images) ? payload.images : [])
      .map((image) => cleanText(image?.image_id || image?.id || image?.name))
      .filter(Boolean);
    const internalCandidates = (Array.isArray(result.catalog_candidates) ? result.catalog_candidates : [])
      .slice(0, 5)
      .map((candidate) => ({
        candidate_id: candidate.candidate_id || null,
        candidate_identity_id: candidate.candidate_identity_id || null,
        reference_title: candidate.reference_title || null,
        conflicting_fields: candidate.conflicting_fields || [],
    ...
    const [row] = await supabaseInsertRows({
      ...
      rows: [{
        asset_id: assetId,
        gap_reason: cleanText(result.open_set_status || result.cold_start_status || "no_prompt_safe_catalog_candidate") || "no_prompt_safe_catalog_candidate",
        status: "PENDING",
        ...
        observed_fields: identityFields,
