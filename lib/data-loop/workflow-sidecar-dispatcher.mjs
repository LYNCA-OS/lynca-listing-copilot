...
import { trustedInternalServiceOrigin } from "../listing/v4/jobs/internal-service-origin.mjs";
...
function sidecarAllowedOrigins(env = process.env) {
  const configured = cleanText(env.DATA_LOOP_SIDECAR_ALLOWED_ORIGINS)
    .split(/[\s,]+/)
  ...
  const internal = trustedInternalServiceOrigin(env);
  return new Set([...configured, ...(internal ? [internal] : [])]);
}

function approvedSidecarUrl(value, env = process.env) {
  ...
  return sidecarAllowedOrigins(env).has(parsed.origin) ? raw : "";
}

function sidecarToken({ url = "", explicitToken = "", env = process.env } = {}) {
  const token = cleanText(explicitToken);
  if (token) return token;
  const internalOrigin = trustedInternalServiceOrigin(env);
  if (!url || !internalOrigin) return "";
  try {
    return new URL(url).origin === internalOrigin
      ? cleanText(env.DATA_LOOP_INTERNAL_SIDECAR_TOKEN)
      : "";
  } catch {
    return "";
  }
...
function configFromEnv(env = process.env) {
  const splinkBatchUrl = approvedSidecarUrl(env.DATA_LOOP_SPLINK_BATCH_URL || env.SPLINK_BATCH_URL, env);
  const cleanlabScoreUrl = approvedSidecarUrl(env.DATA_LOOP_CLEANLAB_SCORE_URL || env.CLEANLAB_SCORE_URL, env);
  const fiftyOneSyncUrl = approvedSidecarUrl(env.DATA_LOOP_FIFTYONE_SYNC_URL || env.FIFTYONE_SYNC_URL, env);
  const labelStudioUrl = approvedSidecarUrl(env.LABEL_STUDIO_URL, env);
  const cvatUrl = approvedSidecarUrl(env.CVAT_URL, env);
  const lightGbmUrl = approvedSidecarUrl(env.DATA_LOOP_LIGHTGBM_RERANKER_URL || env.LIGHTGBM_RERANKER_URL, env);
  const phoenixEndpoint = approvedSidecarUrl(env.PHOENIX_COLLECTOR_ENDPOINT || env.PHOENIX_ENDPOINT || env.DATA_LOOP_PHOENIX_ENDPOINT, env);
  return {
    ...
    splinkBatchUrl,
    splinkBatchToken: sidecarToken({ url: splinkBatchUrl, explicitToken: env.DATA_LOOP_SPLINK_BATCH_TOKEN || env.SPLINK_BATCH_TOKEN, env }),
    cleanlabScoreUrl,
    cleanlabScoreToken: sidecarToken({ url: cleanlabScoreUrl, explicitToken: env.DATA_LOOP_CLEANLAB_SCORE_TOKEN || env.CLEANLAB_SCORE_TOKEN, env }),
    ...
    fiftyOneSyncUrl,
    fiftyOneSyncToken: sidecarToken({ url: fiftyOneSyncUrl, explicitToken: env.DATA_LOOP_FIFTYONE_SYNC_TOKEN || env.FIFTYONE_SYNC_TOKEN, env }),
    ...
    lightGbmUrl,
    lightGbmToken: sidecarToken({ url: lightGbmUrl, explicitToken: env.DATA_LOOP_LIGHTGBM_RERANKER_TOKEN || env.LIGHTGBM_RERANKER_TOKEN, env }),
    phoenixEndpoint,
    phoenixApiKey: sidecarToken({ url: phoenixEndpoint, explicitToken: env.PHOENIX_API_KEY || env.DATA_LOOP_PHOENIX_API_KEY, env })
  };
}
...
async function postJson({
  ...
  if (response.status >= 300 && response.status < 400) {
    throw new Error(`HTTP ${response.status} redirect_not_allowed`);
  }
...
export function previewWorkflowSidecars({
  ...
  if (!cleanText(event.tenant_id)) {
    return defaultWorkflowSidecars(workflowSidecarStatuses.FAILED, "tenant_context_required");
  }
...
async function dispatchCatalogGapQueue({ event = {}, result = {}, config, payload = {}, fetchImpl = globalThis.fetch }) {
  try {
    ...
    const tenantId = cleanText(event.tenant_id || payload.tenant_id || payload.tenantId);
    if (!tenantId) return { status: workflowSidecarStatuses.NOT_TRIGGERED, reason: "tenant_context_required" };
...
export async function dispatchWorkflowSidecars({
  ...
  const [paddleOcr, splink, cleanlab, labelStudio, cvat, fiftyone, lightgbm, phoenix, catalogGapQueue] = await Promise.all([
    ...
    dispatchCatalogGapQueue({ event, result, config, payload, fetchImpl })
  ]);
