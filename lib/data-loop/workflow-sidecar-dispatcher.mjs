...
export async function dispatchWorkflowSidecars({
  ...
  const [paddleOcr, splink, cleanlab, labelStudio, cvat, fiftyone, lightgbm, phoenix] = await Promise.all([
...
export async function attachWorkflowSidecarsToListingResult({
  ...
  try {
    ...
    if (!config.awaitDispatch) {
      ...
      const dispatchPromise = dispatchWorkflowSidecars({ event, env, fetchImpl, payload }).catch(() => null);
    ...
    const sidecars = await dispatchWorkflowSidecars({ event, env, fetchImpl, payload });
    const merged = mergeWorkflowSidecars(result, sidecars);
    return {
      ...merged,
      workflow_summary: buildWorkflowSummary({ result: merged, event, actionPlan, sidecars })
