...
import { assertTenantListingAssetObjectPath } from "./listing/storage/storage-verification-store.mjs";
...
function requiredPrincipalId(value, label) {
  const normalized = String(value || "").trim();
  if (!/^[a-zA-Z0-9][a-zA-Z0-9._:-]{0,159}$/.test(normalized)) {
    throw new Error(`Feedback ${label} is required.`);
  }
  return normalized;
}

function assertFeedbackImageScope(asset, tenantId) {
  const assetId = asset?.id;
  const paths = [asset?.front_object_path, asset?.back_object_path]
    .concat((Array.isArray(asset?.additional_image_paths) ? asset.additional_image_paths : [])
      .map((image) => image?.object_path || image?.objectPath));
  for (const objectPath of paths.filter(Boolean)) {
    assertTenantListingAssetObjectPath({ tenantId, assetId, objectPath });
  }
}
...
async function writeSupabaseRow({
  ...
  if (upsert) endpoint.searchParams.set("on_conflict", onConflict || "id");
...
export async function createTitleFeedbackRecord({
  ...
  const normalizedTenantId = requiredPrincipalId(tenantId, "tenant_id");
  const normalizedUserId = requiredPrincipalId(userId, "user_id");
  if (normalizedTenantId !== LEGACY_TENANT_ID) {
    throw new Error("Legacy title feedback storage is restricted to tenant_legacy.");
  }
...
export async function createListingReviewRecord({
  ...
  const normalizedTenantId = requiredPrincipalId(tenantId, "tenant_id");
  const normalizedUserId = requiredPrincipalId(userId, "user_id");
  const normalizedOperatorId = requiredPrincipalId(operatorId, "operator_id");
  if (normalizedOperatorId !== normalizedUserId) {
    throw new Error("Feedback operator_id must match the signed user_id.");
  }
  const records = buildListingReviewRecords({ payload, operatorId: normalizedUserId });
  records.asset.tenant_id = normalizedTenantId;
  records.analysisRun.tenant_id = normalizedTenantId;
  records.review.tenant_id = normalizedTenantId;
  records.review.operator_id = normalizedUserId;
  assertFeedbackImageScope(records.asset, normalizedTenantId);
  ...
  const legacyFeedback = records.legacyFeedback && normalizedTenantId === LEGACY_TENANT_ID
    ? await createTitleFeedbackRecord({
      ...records.legacyFeedback,
      tenantId: normalizedTenantId,
