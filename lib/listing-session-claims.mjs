...
function validAuthorizationSnapshot(session = {}) {
  ...
  const checkedAt = Number(session.membership_checked_at ?? session.membershipCheckedAt);
  const issuedAt = Number(session.iat);
  return validIdentityToken(role, 40)
    && Number.isSafeInteger(checkedAt)
    && checkedAt >= 1
    && checkedAt <= issuedAt + allowedClockSkewMs;
}
...
export function validListingSessionClaims(session, {
  ...
  if (legacyUser) {
    return legacyUser.length <= 254
      && !/[\u0000-\u001f\u007f]/.test(legacyUser)
      && validAuthorizationSnapshot(session);
  }
