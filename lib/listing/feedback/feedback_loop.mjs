...
export function buildFieldLevelGroundTruth({
  ...
  return fieldGraphKeysForTraining().map((field) => ({
    ...
    label_source: stableTrainingSample ? "writer_reviewed_title" : "writer_rejected_or_unstable",
...
export function buildHardNegativeSamples({
  ...
  return candidateRows
    ...
    .map((row) => ({
      ...
      training_eligible: reviewOutcome !== "REJECTED",
...
export function buildFeedbackLoopEvent({
  ...
  const fieldGroundTruthRows = buildFieldLevelGroundTruth({
    queryCardId,
    correctedFieldGraph,
    reviewOutcome,
  ...
  return {
    ...
    training_ready: stableTrainingSample,
