...
function sanitizeStorageBucket(value) {
  const bucket = String(value || "").trim();
  if (!bucket || bucket.length > 100) return "";
  return /^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(bucket) ? bucket : "";
}

function imageBucket(image = {}) {
  if (!isPlainObject(image)) return "";
  return sanitizeStorageBucket(image.bucket || image.storageBucket || image.storage_bucket);
}
...
function additionalImagePathRecord(image = {}, index = 0) {
  ...
  return {
    ...
    bucket: imageBucket(image) || null,
    object_path: objectPath,
    content_sha256: rawImageContentSha256(image) || null,
    image_id: image.id || image.image_id || null,
    derived: image.derived === true,
    source_image_id: image.sourceImageId || image.source_image_id || image.cropMetadata?.source_image_id || image.crop_metadata?.source_image_id || null,
    source_region: image.sourceRegion || image.source_region || null,
    crop_metadata: image.cropMetadata || image.crop_metadata || null
  };
}

function pushAdditionalImagePath(paths, imageRecord) {
  if (!imageRecord?.object_path) return;
  const key = `${imageRecord.bucket || ""}\u0000${imageRecord.object_path}`;
  const frontKey = `${paths.front_bucket || ""}\u0000${paths.front_object_path || ""}`;
  const backKey = `${paths.back_bucket || ""}\u0000${paths.back_object_path || ""}`;
  if (key === frontKey || key === backKey) return;
  if (paths.additional_image_paths.some((existing) => (
    `${existing.bucket || ""}\u0000${existing.object_path}` === key
  ))) return;
  paths.additional_image_paths.push(imageRecord);
}

function removeAdditionalImagePath(paths, objectPath, bucket = "") {
  if (!objectPath) return;
  const key = `${bucket || ""}\u0000${objectPath}`;
  paths.additional_image_paths = paths.additional_image_paths.filter((image) => (
    `${image.bucket || ""}\u0000${image.object_path}` !== key
  ));
}

export function extractAssetImagePaths(images = [], directPaths = {}) {
  const paths = {
    front_bucket: sanitizeStorageBucket(directPaths.front_bucket || directPaths.frontBucket) || null,
    front_object_path: sanitizeStorageObjectPath(directPaths.front_object_path || directPaths.frontObjectPath) || null,
    back_bucket: sanitizeStorageBucket(directPaths.back_bucket || directPaths.backBucket) || null,
  ...
  images.forEach((image, index) => {
    const objectPath = imageObjectPath(image);
    if (!objectPath) return;
    const bucket = imageBucket(image) || null;
    const role = imageRole(image, index);

    if (role === "front_original" && paths.front_object_path === objectPath) {
      paths.front_bucket = paths.front_bucket || bucket;
      paths.front_content_sha256 = paths.front_content_sha256 || rawImageContentSha256(image) || null;
      removeAdditionalImagePath(paths, objectPath, paths.front_bucket);
      return;
    }

    if (role === "back_original" && paths.back_object_path === objectPath) {
      paths.back_bucket = paths.back_bucket || bucket;
      paths.back_content_sha256 = paths.back_content_sha256 || rawImageContentSha256(image) || null;
      removeAdditionalImagePath(paths, objectPath, paths.back_bucket);
      return;
    }

    if (role === "front_original" && !paths.front_object_path) {
      paths.front_bucket = bucket;
      paths.front_object_path = objectPath;
      paths.front_content_sha256 = rawImageContentSha256(image) || paths.front_content_sha256;
      removeAdditionalImagePath(paths, objectPath, bucket);
      return;
    }

    if (role === "back_original" && !paths.back_object_path) {
      paths.back_bucket = bucket;
...
export function buildAssetFingerprint(imagePaths = {}) {
  const reference = (bucket, objectPath) => objectPath
    ? `${String(bucket || "").trim()}\u0000${objectPath}`
    : null;
  const parts = [
    imagePaths.front_content_sha256 || reference(imagePaths.front_bucket, imagePaths.front_object_path),
    imagePaths.back_content_sha256 || reference(imagePaths.back_bucket, imagePaths.back_object_path),
    ...(Array.isArray(imagePaths.additional_image_paths)
      ? imagePaths.additional_image_paths.map((image) => (
        image.content_sha256 || reference(image.bucket, image.object_path)
      ))
      : [])
  ].filter(Boolean);
