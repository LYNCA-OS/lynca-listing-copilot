...
const absentCardNameNarration = /^\(?\s*(?:no\s+(?:(?:separate|separately|distinct|printed)\s+){0,3}(?:printed\s+)?card\s+(?:name|title)(?:\s+(?:is\s+)?(?:present|printed|visible|readable))?|without\s+(?:a\s+)?(?:(?:separate|distinct|printed)\s+){0,2}card\s+(?:name|title)|(?:not|isn['’]?t)\s+(?:a\s+)?(?:(?:separate|distinct|printed)\s+){0,2}card\s+(?:name|title)|card\s+(?:name|title)\s+(?:is\s+)?not\s+(?:present|printed|visible|readable|shown))\s*\)?[.!]?$/i;
