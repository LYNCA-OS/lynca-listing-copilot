...
export function compactV4L2RecognitionPrompt(payload, maxTitleLength) {
  ...
  return [
    ...
    "Return compact valid JSON only. Deterministic code expands the sparse transport and renders the final English one-line title.",
    ...
    "Card number/code: PAU, NB-TYG, S-P, #256, 201/165, TAEV-EN006 are identity card numbers/codes. Include them when visible and title space allows; they are lower priority than core identity and numerical rarity for Standard cards, high priority for TCG. Every emitted collector_number/checklist_code/card_number/tcg_card_number must also have a directly_observed field_evidence entry with the shortest literal visible_text; otherwise add that field to unresolved.",
    ...
    "Color/parallel safety: visual Gold/Purple/Red/Blue/Green/Silver/Black/Orange may be surface_color. Printed/slab/catalog-supported exact optical wording may be normal parallel_exact evidence. Strong current-image evidence with compatible product context and confidence >=0.80 may be returned only as a VISION_ONLY writer-review hypothesis in parallel_exact, with review_required true and directly_observed true; below 0.80 leave it unresolved.",
    ...
    "Sparse transport: return only non-empty string/list fields, true boolean fields, and present numeric fields. Omitted fields are unknown/default and are expanded locally; omission must never cause a visible fact to disappear.",
    "Field evidence: keep field_evidence short and only for non-empty high-risk/review-sensitive fields. Do not transcribe boilerplate or long OCR text. visible_text must be the shortest literal text that supports the value.",
