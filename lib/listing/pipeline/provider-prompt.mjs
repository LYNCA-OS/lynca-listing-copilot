...
export function providerReadOnlyOutputShape() {
  return {
...
export function readOnlyV4RecognitionPrompt(payload, maxTitleLength) {
  return [
    ...
    "A manufacturer logo or wordmark printed on the physical card is directly observed manufacturer evidence even when it is a stylized emblem; do not require plain prose. Never treat a grading-company logo as the card manufacturer.",
    "Preserve each visible identity fact in its own field: set/subset/insert is the named line, players is only the subject, and card_name must not repeat the subject or manufacturer. A slab label is a literal identity map: capture its year, named line, subject, code, grading company, card grade, and separate autograph grade.",
    "Before returning, do one omission scan for the basic surface colour, Auto/Autograph/Signatures text or visible ink, Patch/Relic/Jersey material, RC marker, printed code, and limited numbering. Uncertainty in one fact must not erase another visible fact.",
    ...
    "Boolean rows are true-only. Never emit false. Omit card_count for one card; emit card_count only for a proven visible lot of two or more physical cards.",
    ...
    "Required JSON shape:",
    JSON.stringify(providerReadOnlyOutputShape()),
    "Current asset:",
    JSON.stringify({
      assetId: payload.assetId || payload.asset_id || null,
      imageCount: Array.isArray(payload.images) ? payload.images.length : 0,
