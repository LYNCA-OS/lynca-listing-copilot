...
export function compactV4L2RecognitionPrompt(payload, maxTitleLength) {
  ...
  return [
    ...
    "Core completeness: when the same card/slab visibly provides year, product, set or subject, return all of them. An uncertain optional finish, code or rarity must never erase readable core facts. Preserve season strings such as 2025-26; copyright-only or statistics years are not issued years.",
    ...
    "Front/back complement: take the subject and visible attributes from the front, then use the back for printed product/set and identity code. Do not turn a checklist code into product prose, a team into the subject, or legal/copyright text into card identity.",
    ...
    "Named identity segments: preserve literal names such as Downtown, Rookie Ticket, Flashback, Raindrops Signatures, Lucky Hyper, Ringside and similar printed or prompt-safe catalog-supported segments in card_name/insert/set. Do not replace a specific named segment with generic Base, Card or Autographed Card.",
    "Core evidence contract: every emitted year, product and surface_color must have one short matching current-image field_evidence row. Use literal visible_text and the narrowest honest source_type; evidence notes such as 'back printed text', 'visible on backs', uncertainty commentary, or parenthetical explanations belong only in field_evidence/unresolved, never inside a field value.",
    ...
    "Base color is required when visible: map only the basic color to surface_color (Gold, White, Red, Blue, Green, Purple, Silver, Black, Orange). Do not turn sparkle, shine, crystal, wave, shimmer, holo, refractor, or prizm appearance into an exact finish. parallel_exact requires printed/slab/prompt-safe catalog support; otherwise leave it unresolved.",
    "Color discipline: inspect the full border/background before omitting a basic color, but never default a reflective card to Silver. A printed or agreeing catalog exact finish may be kept separately from surface_color.",
    "Lot: two or more separate physical card/slab rectangles visible in one image => multi_card true and card_count equal to that visible count; keep up to three subjects and shared product facts, and emit one directly observed multi_card field_evidence row quoting the plural rectangle observation. If several extracted subjects belong to separate visible rectangles, this rule is mandatory. One physical card, multiple uploaded views, or multiple subjects printed on one card are not a lot; then omit card_count and multi_card.",
    ...
    "Return only non-empty fields, true booleans, and present numbers. Add short field_evidence only for high-risk values; visible_text must be the shortest supporting text. If uncertain, omit the value and name the field in unresolved. Do not guess.",
