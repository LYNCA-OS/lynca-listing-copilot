...
import { serverCaptureQualityForPayload } from "../image-quality/quality-gate.mjs";
...
export function captureQualityForPayload(payload = {}) {
  return serverCaptureQualityForPayload(payload);
}

// The authoritative capture-quality object carries cache and policy provenance
// that deterministic code needs, but those version fields are not visual
// evidence and must not inflate or steer the provider prompt. Keep the prompt
// projection bounded to server-derived facts that can affect observation.
export function captureQualityPromptSummary(payload = {}) {
  const quality = captureQualityForPayload(payload);
  const images = (Array.isArray(quality?.images) ? quality.images : [])
    .filter((image) => Number.isFinite(image?.width) && Number.isFinite(image?.height))
    .slice(0, 8)
    .map((image) => ({
      width: image.width,
      height: image.height,
      resolution_sufficient: typeof image.resolution_sufficient === "boolean"
        ? image.resolution_sufficient
        : null,
      crop_complete: typeof image.crop_complete === "boolean" ? image.crop_complete : null,
      image_quality_degraded: typeof image.image_quality_degraded === "boolean"
        ? image.image_quality_degraded
        : null,
      critical_region_occlusion: image.critical_region_occlusion || {}
    }));
  return {
    capture_profile_id: quality?.capture_profile_id || null,
    image_count: Number(quality?.image_count || 0),
    image_quality_degraded: typeof quality?.image_quality_degraded === "boolean"
      ? quality.image_quality_degraded
      : null,
    route: quality?.route || null,
    glare_route: quality?.glare_route || null,
    recovered_regions: Array.isArray(quality?.recovered_regions) ? quality.recovered_regions : [],
    unresolved_regions: Array.isArray(quality?.unresolved_regions) ? quality.unresolved_regions : [],
    critical_region_occlusion: quality?.critical_region_occlusion || {},
    images
  };
}
...
export function compactV4L2RecognitionPrompt(payload, maxTitleLength) {
  ...
  return [
    ...
    JSON.stringify({
      ...
      fileNames: (payload.images || []).map((image) => image.name).filter(Boolean).slice(0, 2),
      captureQuality: captureQualityPromptSummary(payload)
    }),
    l1FastScoutHintPromptSection(payload),
...
export async function buildListingPrompt(payload, maxTitleLength) {
  ...
  return [
    ...
    "Capture quality:",
    JSON.stringify(captureQualityPromptSummary(payload)),
    l1FastScoutHintPromptSection(payload),
    "Required JSON shape:",
    JSON.stringify(providerMinimalOutputShape({ includeVectorDecision: Boolean(vectorPacket) })),
    vectorCandidatePromptSection(vectorPacket),
