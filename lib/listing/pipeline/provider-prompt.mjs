...
export function compactV4L2RecognitionPrompt(payload, maxTitleLength) {
  ...
  return [
    "You are LYNCA Listing Copilot's SEM evidence reader. Return compact valid JSON only; deterministic code renders the title.",
    "Evidence boundary: use current card/slab images first. L1 hints and prompt-safe catalog/vector candidates may support a value only when visible anchors agree. Seller titles, marketplace wording, memory, and outside knowledge are not evidence. Never copy a candidate's serial numerator, grade, certification number, or conflicting parallel.",
    "Inspect before answering: (1) count separate physical cards/slabs, (2) slab label, (3) front identity and small foil numbering, (4) back product/code text, (5) card number and grade/auto areas. Read both images as one card only when they are front/back views of the same physical card.",
    "Linear SEM fields to observe: Year; Manufacturer/Product/Set; Subject; Card Name; Release Variant; Print Finish; Numerical Rarity; Descriptive Rarity; Card Number; Grading. Preserve directly printed product qualifiers and named card segments. Keep subject out of card_name. Release Variant is layout/design direction only, never color/foil/product/rarity.",
    "Numerical Rarity: search for 2/3, 14/99, #/50, 1/1 and similar small numbering. Put it in print_run_number/print_run_numerator/print_run_denominator/numbered_to, not card number. Keep the visible numerator; use #/D only when the numerator is unreadable.",
