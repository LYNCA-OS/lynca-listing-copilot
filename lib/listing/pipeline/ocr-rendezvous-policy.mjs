...
// Whether the OCR workers are currently unable to answer at all.
//
// Deliberately narrow: only a job that failed *because the worker could not be
// reached* counts. A job that failed on the image, or on parsing, says nothing
// about whether waiting is worthwhile -- the next crop might still succeed.
// Widening this would turn a targeted refusal into a general opt-out.
...
export function ocrWorkersUnavailable(latestOcrState = null) {
  ...
  const reasons = Array.isArray(latestOcrState.failed_reasons) ? latestOcrState.failed_reasons : [];
  if (!reasons.length) return false;
  const unavailable = reasons.filter((reason) => {
    const text = String(reason ?? "").toLowerCase();
    return workerUnavailableReasons.some((needle) => text.includes(needle));
  });
  if (!unavailable.length) return false;
  // Every recorded failure has to be an unavailability, otherwise some jobs are
  // reaching a worker and a wait can still pay off.
  return unavailable.length === reasons.length;
}
...
export function criticalOcrRendezvousDecision({
  ...
  // Waiting for a worker that cannot answer is not a small cost, it is the
  // whole cost. Measured per day on production:
  //
  //   2026-07-29  299 sessions  619ms avg wait   0 patched   84 worker_unavailable
  //   2026-07-28  144           164ms            0           96
  //   2026-07-27  206         1,136ms            0          112
  //   2026-07-26  211           444ms            0           19
  //   2026-07-25  268           718ms            0          134
  //
  // Five consecutive days, 1,128 sessions, **zero** patches applied, and still
  // hundreds of milliseconds spent waiting on each one. The 4.6% patch rate the
  // ceiling above was sized against is a seven-day average carried entirely by
  // 07-22 and 07-24; the current rate is zero, because the OCR workers sit on a
  // Cloud Run project whose billing account is closed.
  //
  // So when the state we already hold says the workers are failing for that
  // reason, the wait is zero rather than capped. This is not a new policy about
  // whether OCR is worthwhile -- background jobs still run and still land on the
  // next read -- it is a refusal to hold a listener for an answer that provably
  // is not coming.
