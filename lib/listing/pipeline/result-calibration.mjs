...
export function suppressReviewOnlyParallelFields(fields, reason, unresolved = []) {
  ...
  const narrowSurfaceColor = narrowSurfaceColorFromOpenSetParallel(fields);
  [
    "parallel_family",
    "parallel_exact",
    "parallel",
  ...
  if (suppressed.surface_color !== undefined || narrowSurfaceColor) {
    suppressed.surface_color = narrowSurfaceColor || null;
  }
  return suppressed;
}

export function unresolvedWithSuppressedParallelReview(fields = {}, presentationFields = {}, unresolved = []) {
  const canonicalFields = [];
  const reviewFieldBySourceField = {
    surface_color: "surface_color",
    parallel_family: "parallel_family",
    parallel_exact: "parallel_exact",
    parallel: "parallel_exact",
    variation: "variation"
  };

  for (const [sourceField, reviewField] of Object.entries(reviewFieldBySourceField)) {
    const originalValue = fields[sourceField];
    const presentationValue = presentationFields[sourceField];
    const wasPresent = originalValue !== null
      && originalValue !== undefined
      && String(originalValue).trim() !== "";
    const remainsPresent = presentationValue !== null
      && presentationValue !== undefined
      && String(presentationValue).trim() !== "";
    if (wasPresent && !remainsPresent) canonicalFields.push(reviewField);
  }

  return [...new Set([
    ...canonicalFields,
    ...normalizeUnresolved(unresolved, fields)
  ])].slice(0, 12);
}
