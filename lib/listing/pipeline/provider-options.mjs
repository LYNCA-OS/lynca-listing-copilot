...
export function defaultProviderOptionsFromEnv(env = process.env) {
  ...
  return {
    ...
    enable_retrieval_application: envFlag(env, "ENABLE_RETRIEVAL_APPLICATION", true),
    // Retrieval is applied after the provider observation. Keeping candidate
    // packets out of the vision prompt prevents a plausible near-neighbour
    // from anchoring the model before it has read the current card.
    enable_retrieval_prompt_context: envFlag(env, "ENABLE_RETRIEVAL_PROMPT_CONTEXT", false),
    ...
    v4_ultra_fast_image_detail: String(env.V4_ULTRA_FAST_IMAGE_DETAIL || "high").trim().toLowerCase(),
    v4_ultra_fast_text_verbosity: String(env.V4_ULTRA_FAST_TEXT_VERBOSITY || "medium").trim().toLowerCase(),
...
export function postObservationExactAnchorCatalogBudgetMs(env = process.env, providerOptions = {}) {
  ...
  const ultraFast = optionFlag(providerOptions, "v4_ultra_fast_l2", envFlag(env, "ENABLE_V4_ULTRA_FAST_L2", false));
  const fallback = ultraFast ? 5_000 : 1_800;
...
export function ultraFastImageDetail(providerOptions = {}) {
  ...
  return ["low", "auto", "high"].includes(requested) ? requested : "high";
}

export function ultraFastTextVerbosity(providerOptions = {}) {
  const requested = String(
    providerOptions.v4_ultra_fast_text_verbosity
    ?? providerOptions.v4UltraFastTextVerbosity
    ?? "medium"
  ).trim().toLowerCase();
...
export function retrievalApplicationEnabled(env = process.env, options = {}) {
  return optionFlag(options, "enable_retrieval_application", envFlag(env, "ENABLE_RETRIEVAL_APPLICATION", true));
}

export function retrievalPromptContextEnabled(env = process.env, options = {}) {
  // This is a production master switch. A request payload must never be able
  // to re-enable candidate anchoring when the deployment has disabled it.
  if (!envFlag(env, "ENABLE_RETRIEVAL_PROMPT_CONTEXT", false)) return false;
  return optionFlag(options, "enable_retrieval_prompt_context", true);
}
