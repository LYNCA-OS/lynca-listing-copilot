...
function printedCodeEvidenceIsDirect(fieldName, value, patch = {}) {
  ...
  if (/^\d{1,4}$/.test(code)) {
    ...
    const verifiedBackFullImage = String(patch.provenance?.source_side || "").toLowerCase() === "back"
      && ["unique_back_numeric", "positioned_full_image_numeric"].includes(
        patch.provenance?.direct_extraction_method
      )
      && Number(patch.confidence || 0) >= 0.92;
    if (!explicitMarker && !isolated && !verifiedBackFullImage) return false;
    if (/\b(?:PASSING|RECORD|TEAM|ATT|COMP|POINTS?|ASSISTS?|REB|AVG|STATS?)\b/i.test(normalizedCorpus)
      && !explicitMarker
      && !verifiedBackFullImage) {
      return false;
    }
  }
