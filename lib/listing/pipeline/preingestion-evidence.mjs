...
export async function applyPreIngestionBundleToPayload(payload = {}, {
  ...
  const effectiveCaptureQuality = bundle.quality_summary?.capture_quality;
  if (effectiveCaptureQuality?.authority === "SERVER_CANONICAL_METADATA") {
    payload.effective_capture_quality = effectiveCaptureQuality;
  }
  if (bundle.quality_summary?.client_capture_quality) {
    payload.client_capture_quality = bundle.quality_summary.client_capture_quality;
  }
  // Legacy bundles stored unauthenticated browser observations under this
  // name. They remain auditable but no longer become runtime decision input.
  delete payload.capture_quality;
  delete payload.captureQuality;
