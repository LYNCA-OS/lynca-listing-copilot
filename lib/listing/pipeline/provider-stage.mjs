...
import { addTiming, nowMs, recordProviderAttempt, timeAsync } from "./timing.mjs";
...
export function openAiRequestContextFromPayload(payload = {}, {
  ...
  const benchmarkProfile = String(providerOptions.recognition_benchmark_profile || "").trim();
  const singleHttpRequest = benchmarkProfile === "cold_second_look_shadow_benchmark";
  return {
    ...
    recognition_benchmark_profile: benchmarkProfile || null,
    provider_http_request_budget: singleHttpRequest ? 1 : null,
    provider_http_retry_policy: singleHttpRequest ? "FORBIDDEN" : "PRODUCTION_DEFAULT",
...
export async function runTimedProviderCall(providerId, timingContext, work, {
  ...
  let queued;
  try {
    queued = await runWithProviderConcurrency({
      providerId,
      failIfBusy,
      work: async () => {
        startedAtMs = nowMs();
        try {
          return await timeAsync(timingContext, "provider_total_ms", work);
        } finally {
          completedAtMs = nowMs();
        }
      }
    ...
    recordProviderAttempt(timingContext, {
      ...
      completedAtMs,
      status: "FAILED",
      errorCode: error?.code || error?.name || "PROVIDER_CALL_FAILED"
    });
    throw error;
  }
  recordProviderAttempt(timingContext, {
    queuedAtMs: queued.queued_at_ms,
    startedAtMs: queued.started_at_ms,
    completedAtMs: queued.completed_at_ms,
    status: "COMPLETED"
  });
