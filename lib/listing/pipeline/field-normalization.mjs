...
function recoverPrintedCodeFromCardName(value, context = {}) {
  const text = normalizeStringOrNull(value);
  if (!text || /[a-z]/.test(text)) return null;
  if (!/^(?=[A-Z0-9-]{3,24}$)(?=.*\d)(?=.*-)[A-Z0-9]+(?:-[A-Z0-9]+)+$/.test(text)) return null;
  return normalizePrintedCardCodeForFields(text, context);
}
...
export function normalizeFields(fields = {}) {
  ...
  const product = narratedProduct && /^(?:Topps|Bowman|Panini|Upper\s+Deck|Donruss|Leaf)$/i.test(explicitProduct || "")
    ...
    : explicitProduct;
  const normalized = {
    year: normalizeCardYearValue(normalizeIdentityTextOrNull(fields.year)),
  ...
  if (!normalized.card_number && !normalized.collector_number && !normalized.checklist_code) {
    const recoveredCardCode = recoverPrintedCodeFromCardName(normalized.card_name, normalized);
    if (recoveredCardCode) {
      normalized.collector_number = recoveredCardCode;
      normalized.card_name = null;
    }
  }
