...
export function normalizeRookieMarker(value) {
  ...
  return /^(?:RC|Rookie|Rookie Card)$/i.test(normalized) ? "RC" : normalized;
}
...
export function normalizeFields(fields = {}) {
  ...
  // colored parallels on refractor stock; the card itself rarely says the
  // word, so providers report only the color. Append the finish for a colored
  // parallel unless a finish naming it (or a Sapphire-line product) already
  // covers it. Base cards carry no surface_color and are untouched.
  ...
  if (chromeStockFamily && !chromeFinishAlreadyNamed && String(normalized.surface_color || "").trim()) {
