...
function providerTokenMetrics(result = {}) {
  ...
  return {
    ...
    reasoning_tokens: finiteNumber(diagnostics.reasoning_tokens, null),
    visible_output_tokens: finiteNumber(diagnostics.visible_output_tokens, null),
...
function safeOcrMetrics(ocr = {}) {
  const jobObservability = compactArray(ocr.job_observability);
  return {
    ...
    job_observability_count: jobObservability.length,
    job_observability_truncated: jobObservability.length > 32,
    job_observability: jobObservability.slice(0, 32).map((job) => ({
      job_id: cleanText(job?.job_id) || null,
      crop_role: cleanText(job?.crop_role) || null,
      source_image_id: cleanText(job?.source_image_id) || null,
      source_side: cleanText(job?.source_side).toLowerCase() || null,
      source_region: cleanText(job?.source_region) || null,
