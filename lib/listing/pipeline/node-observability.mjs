...
function scalarEvidenceValues(value) {
  if (Array.isArray(value)) return value.flatMap(scalarEvidenceValues);
  if (value === null || value === undefined || typeof value === "object") return [];
  return nonEmpty(value) ? [value] : [];
}

function compactGroupValues(fields = {}, aliases = []) {
  return [...new Set(aliases
    .flatMap((field) => scalarEvidenceValues(fields[field]))
    .map(cleanText)
    .filter(Boolean))]
    .slice(0, 8);
}

function printedCodeGuardReport(raw = {}, aliases = []) {
  const candidates = aliases.flatMap((field) => scalarEvidenceValues(raw[field]));
  ...
  const acceptedCount = candidates.filter((value) => (
    normalizePrintedCardCodeForFields(value, raw) !== null
  )).length;
  return {
    rejected: acceptedCount === 0,
    candidate_count: candidates.length,
    accepted_count: acceptedCount,
    source_fields: aliases.filter((field) => scalarEvidenceValues(raw[field]).length > 0)
  };
}
...
function oracleSensorEvidence(result = {}, payload = {}) {
  const options = payload.provider_options || payload.providerOptions || {};
  const evaluationProfile = options.evaluation_profile || result.evaluation_profile || "";
  if (evaluationProfile !== "v4_accuracy_ceiling_oracle_v1") return undefined;
  const observations = [];
  const providerFields = fieldContainer(result.raw_provider_fields || result.raw_observed_fields || {});
  if (Object.values(providerFields).some(nonEmpty)) {
    observations.push({ source: "GPT_5_MINI_OBSERVATION", fields: providerFields });
  }
  const bySource = new Map();
  for (const patch of compactArray(payload.preingestion_evidence_patches)) {
    const field = cleanText(patch?.field);
    if (!field || !nonEmpty(patch?.value)) continue;
    const model = cleanText(patch?.provenance?.model_id || patch?.model_id || "OCR").toUpperCase();
    const source = model.includes("GOOGLE") ? "GOOGLE_VISION_OCR"
      : model.includes("PP-OCR") || model.includes("PPOCR") ? "PP_OCR_SHADOW"
        : "PREINGESTION_OCR";
    const fields = bySource.get(source) || {};
    fields[field] = patch.value;
    bySource.set(source, fields);
  }
  for (const [source, fields] of bySource) observations.push({ source, fields });
  return observations;
}
...
function safeOcrMetrics(ocr = {}) {
  return {
    ...
    state_read_error_count: finiteNumber(ocr.state_read_error_count, 0),
    last_state_read_error: cleanText(ocr.last_state_read_error).slice(0, 180) || null,
    ...
    job_observability: compactArray(ocr.job_observability).slice(0, 32).map((job) => ({
      ...
      request_count: finiteNumber(job?.request_count, null),
      full_image_fallback_used: job?.full_image_fallback_used === true,
      full_image_fallback_kind: cleanText(job?.full_image_fallback_kind) || null,
      text_candidate_count: finiteNumber(job?.text_candidate_count, null),
...
function fieldReviewSet(result = {}) {
  ...
  for (const field of compactArray(result.field_normalization_guard?.explicit_uncertainty_suppressed_fields)) {
    values.add(cleanText(field));
  }
...
function fieldFlow(result = {}) {
  ...
  const rows = Object.entries(fieldGroups).map(([group, aliases]) => {
    ...
    const normalizationGuard = group === "collector_number" && rawPresent && !resolvedPresent
      ? printedCodeGuardReport(raw, aliases)
      : { rejected: false, candidate_count: 0, accepted_count: 0, source_fields: [] };
    ...
    else if (normalizationGuard.rejected) disposition = "INTENTIONALLY_REJECTED_BY_NORMALIZATION_GUARD";
    ...
    return {
      ...
      normalization_guard_rejected: normalizationGuard.rejected,
      normalization_guard_candidate_count: normalizationGuard.candidate_count,
      normalization_guard_accepted_count: normalizationGuard.accepted_count,
      normalization_guard_source_fields: normalizationGuard.source_fields,
      composite_destination_fields: compositeRetention.destination_fields,
      raw_values: compactGroupValues(raw, aliases),
      resolved_values: compactGroupValues(resolved, aliases),
      rendered_values: compactGroupValues(rendered, aliases),
      disposition
    };
  });
  return {
    ...
    normalization_guard_rejection_count: rows
      .filter((row) => row.disposition === "INTENTIONALLY_REJECTED_BY_NORMALIZATION_GUARD").length,
    normalization_guard_rejection_fields: rows
      .filter((row) => row.disposition === "INTENTIONALLY_REJECTED_BY_NORMALIZATION_GUARD")
      .map((row) => row.field_group),
...
function reconciliationChecks({ result, payload, catalog, vector, ocr, tokens, fieldFlowReport, title, catalogCapacity, vectorCapacity }) {
  ...
  const identityStatus = cleanText(result.identity_resolution_status || result.identity_resolution?.status || result.ambiguity_status).toUpperCase();
  const semanticReviewRequired = !title
    && !result.provider_error_type
    && !result.provider_error_code
    && (
      identityStatus === "ABSTAIN"
      || cleanText(result.workflow_route || result.route).toUpperCase() === "WRITER_REVIEW_REQUIRED"
      || cleanText(result.title_render_source).toLowerCase() === "identity_resolution_abstain"
    );
  const failedResult = !semanticReviewRequired
    && Boolean(result.provider_error_type || result.provider_error_code || cleanText(result.confidence).toUpperCase() === "FAILED");
  checks.push(check("writer_title_matches_result_status", failedResult || semanticReviewRequired ? !title : Boolean(title), {
    expected: failedResult
      ? "no writer title"
      : semanticReviewRequired
        ? "no fabricated title; writer review required"
        : "writer title present",
...
export function buildPipelineNodeLedger({ result = {}, timingContext = null, payload = {} } = {}) {
  ...
  const preProviderRescueShadow = result.pre_provider_rescue_shadow && typeof result.pre_provider_rescue_shadow === "object"
    ? result.pre_provider_rescue_shadow
    : {};
  ...
  const visualFeatureSource = cleanText(result.visual_feature_summary?.source).toLowerCase();
  const visualFeatureCount = finiteNumber(result.visual_feature_summary?.feature_count, 0);
  const onlineVectorEmbeddingUsed = visualFeatureSource === "vector_worker"
    || finiteNumber(timing.vector_embedding_ms, 0) > 0
    || finiteNumber(timing.vector_embedding_overlap_ms, 0) > 0;
  const reusableVisualFeaturesUsed = visualFeatureCount > 0 && !onlineVectorEmbeddingUsed;
  ...
  const definitions = [
    ...
    {
      id: "client_image_upload",
      category: "client",
      keys: ["client_upload_ms"],
      expected: finiteNumber(clientTiming.client_upload_ms, 0) > 0,
      input: imageCount,
      output: imageCount,
      skip: finiteNumber(clientTiming.client_upload_ms, 0) > 0 ? "" : "storage_objects_reused_or_timing_not_reported",
      metrics: {
        image_count: finiteNumber(clientTiming.client_storage_image_count, null),
        signing_ms: finiteNumber(clientTiming.client_storage_sign_ms, null),
        upload_ms: finiteNumber(clientTiming.client_storage_put_ms, null),
        verification_ms: finiteNumber(clientTiming.client_storage_verify_ms, null),
        signing_attempts: finiteNumber(clientTiming.client_storage_sign_attempts, null),
        upload_attempts: finiteNumber(clientTiming.client_storage_put_attempts, null),
        verification_attempts: finiteNumber(clientTiming.client_storage_verify_attempts, null),
        retry_count: finiteNumber(clientTiming.client_network_retry_count, null),
        recovered_upload_count: finiteNumber(clientTiming.client_storage_recovered_upload_count, null),
        last_error_stage: cleanText(clientTiming.client_network_error_stage) || null,
        last_error_code: cleanText(clientTiming.client_network_error_code) || null
      }
    },
    {
      ...
      metrics: {
        ...
        request_ms: finiteNumber(clientTiming.client_preingestion_request_ms, null),
        request_attempts: finiteNumber(clientTiming.client_preingestion_request_attempts, null),
    ...
    {
      id: "stored_visual_feature_lookup",
      category: "retrieval",
      keys: ["stored_visual_feature_lookup_ms"],
      expected: visualFeatureSource === "supabase_stored_visual_embedding",
      input: imageCount,
      output: visualFeatureSource === "supabase_stored_visual_embedding" ? visualFeatureCount : null,
      skip: visualFeatureSource === "supabase_stored_visual_embedding" ? "" : "optional_precomputed_feature_path"
    },
    {
    ...
    },
    {
      id: "vector_embedding",
      category: "retrieval",
      keys: ["vector_embedding_ms", "vector_embedding_overlap_ms"],
      expected: hasVectorTrace && onlineVectorEmbeddingUsed,
      input: imageCount,
      output: onlineVectorEmbeddingUsed
        ? finiteNumber(result.candidate_context?.vector?.worker_feature_count, visualFeatureCount || null)
        : null,
      skip: reusableVisualFeaturesUsed
        ? "precomputed_visual_features_reused"
        : hasVectorTrace ? "vector_embedding_not_required" : "vector_retrieval_not_used"
    },
    {
    ...
    },
    {
      ...
      metrics: {
        ...
        exact_anchor_budget_used_count: finiteNumber(timing.post_observation_exact_anchor_budget_used_count, 0),
        exact_anchor_budget_ms: finiteNumber(timing.post_observation_exact_anchor_budget_ms, 0),
        structured_anchor_budget_used_count: finiteNumber(timing.post_observation_structured_anchor_budget_used_count, 0),
        structured_anchor_budget_ms: finiteNumber(timing.post_observation_structured_anchor_budget_ms, 0)
      }
    },
    ...
    {
      id: "pre_provider_rescue_shadow",
      category: "decision_observability",
      keys: ["pre_provider_rescue_shadow_ms"],
      expected: preProviderRescueShadow.enabled === true,
      input: preProviderRescueShadow.enabled === true ? 1 : 0,
      output: preProviderRescueShadow.rescue_recommended === true ? 1 : 0,
      skip: preProviderRescueShadow.enabled === true ? "" : "pre_provider_rescue_shadow_disabled",
      metrics: {
        mode: preProviderRescueShadow.mode || null,
        strategy_mutation_allowed: preProviderRescueShadow.strategy_mutation_allowed === true,
        critical_path_budget_ms: finiteNumber(preProviderRescueShadow.critical_path_budget_ms, 0),
        rescue_recommended: preProviderRescueShadow.rescue_recommended === true,
        risk_score: finiteNumber(preProviderRescueShadow.risk_score, null),
        reasons: compactArray(preProviderRescueShadow.reasons).map(cleanText).filter(Boolean).slice(0, 12),
        recommended_lanes: compactArray(preProviderRescueShadow.recommended_lanes).map(cleanText).filter(Boolean).slice(0, 12)
      }
    },
  ...
  const sensorEvidence = oracleSensorEvidence(result, payload);
  ...
  return {
    ...
    request_context: {
      ...
      evaluation_profile: cleanText(
        payload.provider_options?.evaluation_profile
        || payload.providerOptions?.evaluation_profile
