...
function defaultBaseWasIntentionallySuppressed(group, raw = {}, aliases = []) {
  if (group !== "card_type") return false;
  const values = compactGroupValues(raw, aliases).map(comparableFieldText).filter(Boolean);
  return values.length > 0 && values.every((value) => value === "base" || value === "base card");
}
...
function fieldFlow(result = {}) {
  ...
  const rows = Object.entries(fieldGroups).map(([group, aliases]) => {
    ...
    const policySuppressed = rawPresent
      && !resolvedPresent
      && defaultBaseWasIntentionallySuppressed(group, raw, aliases);
    ...
    else if (policySuppressed) disposition = "POLICY_SUPPRESSED_DEFAULT_BASE";
  ...
  return {
    ...
    normalization_guard_rejection_fields: rows
      ...
      .map((row) => row.field_group),
    policy_suppression_count: rows
      .filter((row) => row.disposition === "POLICY_SUPPRESSED_DEFAULT_BASE").length,
    policy_suppression_fields: rows
      .filter((row) => row.disposition === "POLICY_SUPPRESSED_DEFAULT_BASE")
      .map((row) => row.field_group),
