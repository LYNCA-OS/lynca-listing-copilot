...
function funnelMetrics(funnel = {}) {
  return {
    raw_candidate_count: finiteNumber(funnel.raw_candidate_count, 0),
    approved_candidate_count: finiteNumber(funnel.approved_candidate_count, 0),
...
function fieldReviewSet(result = {}) {
  ...
  for (const [field, state] of Object.entries(result.field_states || {})) {
...
function reconciliationChecks({ result, catalog, vector, ocr, tokens, fieldFlowReport, title }) {
  const checks = [];
  ...
  for (const [kind, funnel] of [["catalog", catalog], ["vector", vector]]) {
    ...
    if (funnel.raw_candidate_count > 0 && funnel.approved_candidate_count === 0 && funnel.conflict_blocked_count === 0) {
      checks.push(check(`${kind}_candidate_drop_has_explanation`, false, {
        severity: "WARNING",
        expected: "approved candidate or conflict-blocked reason",
        actual: `${funnel.raw_candidate_count} raw candidates with no classified outcome`
      }));
    }
  }
...
export function buildPipelineNodeLedger({ result = {}, timingContext = null, payload = {} } = {}) {
  ...
  const definitions = [
    ...
    { id: "preingestion_bundle_load", category: "preingestion", keys: ["preingestion_bundle_load_ms"], expected: bundleUsed, input: bundleUsed ? 1 : null, skip: bundleUsed ? "" : "no_preingestion_bundle" },
    ...
    { id: "preingestion_evidence_refresh", category: "evidence", keys: ["preingestion_evidence_refresh_ms"], expected: Number(ocr.patch_count || 0) > 0, output: finiteNumber(result.preingestion_evidence_refresh?.added_patch_count, null), skip: Number(ocr.patch_count || 0) > 0 ? "" : "no_current_ocr_patches" },
  ...
  const checks = reconciliationChecks({ result, catalog, vector, ocr, tokens, fieldFlowReport: flow, title });
