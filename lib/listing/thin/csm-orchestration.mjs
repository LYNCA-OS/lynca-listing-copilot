...
export function resolveCanonicalObservation(observation, {
  ...
  const grammarContextReceipts = observation?.tcg_field_source_authority_receipt
    && observation?.tcg_grammar_context_claim_receipt
    ? {
        tcg_field_source_authority_receipt:
          observation.tcg_field_source_authority_receipt,
        tcg_grammar_context_claim_receipt:
          observation.tcg_grammar_context_claim_receipt
      }
    : {};
  if (observation?.tcg_grammar_context_claim_receipt?.status === "APPLIED") {
    return {
      ...observation,
      ...
      resolution_contract: TCG_GRAMMAR_CONTEXT_RESOLUTION_CONTRACT
    };
  }
  const verifiedOriginalReleaseId = writer.verified_original_observation_overlay;
  const externalIdentityReleaseId = writer.external_identity.registry_release_id;
  const externalIdentityContract =
    externalIdentityResolutionContractForRegistryRelease(externalIdentityReleaseId);
  const verifiedOriginal = verifiedOriginalReleaseId == null ? null
    : resolveVerifiedOriginalObservation(
        observation?.fields || {},
        externalIdentityContext,
        { releaseId: verifiedOriginalReleaseId }
      );
  const resolution = resolveExternalIdentitySupportForRelease(
    verifiedOriginal?.fields || observation?.fields || {}, {
    registryReleaseId: externalIdentityReleaseId,
    externalIdentityContext
  });
  ...
  if (verifiedOriginal) {
    const combinedContract =
      combinedPostObservationResolutionContractForReleases({
        verifiedOriginalObservationReleaseId: verifiedOriginalReleaseId,
        externalIdentityRegistryReleaseId: externalIdentityReleaseId
      });
    ...
    return {
      ...
      observed_fields: observation.observed_fields || observation.fields,
      ...grammarContextReceipts,
      verified_original_observation_support: verifiedOriginal.receipt,
      external_identity_support: resolution.receipt,
      resolution_contract_sha256: combinedContract.contract_sha256,
  ...
  if (resolution.status !== "APPLIED") {
    return {
      ...observation,
      external_identity_support: resolution.receipt,
      resolution_contract_sha256: externalIdentityContract.contract_sha256,
      resolution_contract: externalIdentityContract
    };
  }
...
function csmSessionPatch({ rows, result, receipt }) {
  let writer;
  try {
    writer = writerProjectionContractForPreparedResult({
      ...result,
      ...
      historicalReplay: result?.csm_persistence_checkpoint != null
    });
  } catch (error) {
    throw invalidPreparedResult(error?.detail || "writer_contract_binding_invalid");
  }
  const accuracyLossLedger = result.accuracy_loss_ledger
    ? validateAccuracyLossLedger(result.accuracy_loss_ledger, {
        result,
  ...
  const tcgGrammarContextV4 = contractVersion
    === CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION;
  const sourceOperationPayloadSha256 = String(
    rows.output?.structured_output?.tcg_field_source_authority_receipt
      ?.operation_payload_sha256 || ""
  ).trim().toLowerCase();
  const checkpointOperationPayloadSha256 = result?.csm_persistence_checkpoint
    ?.payload_sha256 == null ? null : String(
      result.csm_persistence_checkpoint.payload_sha256
    ).trim().toLowerCase();
  if (tcgGrammarContextV4
      && (!/^[0-9a-f]{64}$/.test(sourceOperationPayloadSha256)
        || (checkpointOperationPayloadSha256 !== null
          && checkpointOperationPayloadSha256 !== sourceOperationPayloadSha256))) {
    throw invalidPreparedResult("tcg_operation_payload_sha256_invalid");
  }
  ...
  const pipelineIdentity = {
    ...
    ...(writer.pipeline_fingerprint_semantics === "CURRENT" ? {
  ...
  const pipelineFingerprint = createHash("sha256")
    .update(JSON.stringify(pipelineIdentity)).digest("hex");
  return {
    ...
    csm_owner_versions: sealCsmOwnerExecutionReceipt({
      ...
      ...(tcgGrammarContextV4 ? {
        operation_payload_sha256: sourceOperationPayloadSha256
      } : {}),
      ...(writer.owner_optional_receipt_policy === "SEAL" ? {
...
export async function prepareCanonicalListingPath({
  ...
  const result = await runCanonicalListingPath({
    ...
    sourceAuthorityContext: sourceAuthorityContext == null ? null : {
  ...
  const rows = buildCsmStageRows({
    ...
    tcgFieldSourceAuthorityReceipt:
      result.tcg_field_source_authority_receipt,
    tcgGrammarContextClaimReceipt:
      result.tcg_grammar_context_claim_receipt,
    ...
    founderBetaWebReceipt: writer.durable_founder_beta_web_receipt
      ? result.founder_beta_web_receipt : null,
    setCardNameRelationReceipt: writer.durable_set_card_name_relation_receipt
      ? result.set_card_name_relation_receipt : null,
    title: result.title,
    registryReleaseId: result.tcg_grammar_context_claim_receipt?.status === "APPLIED"
      ? TCG_GRAMMAR_CONTEXT_RESOLUTION_CONTRACT.registry_release_id
      : result.external_identity_support?.status === "APPLIED"
      ? result.external_identity_support.registry_release_id
      : registryReleaseId,
    createdAt,
