...
import { durableReasoningEffortReceipt } from "./provider-response-attestation.mjs";
...
function csmSessionPatch({ rows, result, model, effort, imageDetail, promptVersion }) {
  ...
  const composerVersion = rows.output?.composer_version || THIN_COMPOSER_VERSION;
  const marketplaceProfileVersion = rows.output?.marketplace_profile_version || EBAY_PROFILE_VERSION;
  const executedModel = result.model || model;
  const requestedEffort = result.requested_effort || effort;
  const effortReceipt = durableReasoningEffortReceipt(result);
  const executedImageDetail = result.image_detail || imageDetail;
  const executedPromptVersion = Object.prototype.hasOwnProperty.call(result, "prompt_version")
    ? result.prompt_version
    : promptVersion;
  const pipelineFingerprint = createHash("sha256").update(JSON.stringify({
    contract: contractVersion, model: executedModel, effort: requestedEffort,
