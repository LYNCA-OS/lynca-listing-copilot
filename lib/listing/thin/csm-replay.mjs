...
export function replayFromRows(rows) {
  ...
  const composed = durableReceipt?.lot_terminal ? {
    ...recomposed,
    lot_terminal_durable: true,
    lot_quantity_unresolved: durableReceipt.lot_terminal.lot_quantity_unresolved,
    lot_single_card: durableReceipt.lot_terminal.lot_single_card,
    lot_unshared_attributes: [...durableReceipt.lot_terminal.lot_unshared_attributes],
    lot_publishable: durableReceipt.lot_terminal.publishable,
    lot_publication_failure_code: durableReceipt.lot_terminal.failure_code ?? null,
    publication_coverage_durable: true
  } : durableReceipt ? { ...recomposed, publication_coverage_durable: true } : recomposed;
  if (rows?.output?.marketplace_profile_version === LYNCA_STANDARD_PROFILE_VERSION_V3) {
    const problems = canonicalNamingTraceProblems(rows, { title: composed.title, composed });
    if (problems.length) throw new CsmReplayError(
      "canonical_naming_v03_trace_invalid",
      problems[0].kind
    );
  }
