...
import {
  ...
  assertDurableProjectionReplayed,
...
export const CSM_REPLAY_FIELD_SHAPE = Object.freeze({
  CURRENT_COMPLETE: "CURRENT_COMPLETE",
  CAPTURED_E1AE: "CAPTURED_E1AE"
});

function isDurableProjectionContractVersion(version) {
  return version === CSM_DURABLE_PROJECTION_CONTRACT_VERSION
    || version === CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION;
}

function isCapturedE1aeStandardOutputTuple(output, resolution) {
  return output?.contract_version === CSM_STAGE_LEGACY_CONTRACT_VERSION
    && resolution?.grammar === "NON_TCG"
    && output?.composer_version === THIN_COMPOSER_VERSION
    && [LYNCA_STANDARD_PROFILE_VERSION_V1, LYNCA_STANDARD_PROFILE_VERSION_V2]
      .includes(output?.marketplace_profile_version)
    && output?.structured_output?.composition_grammar === "standard";
}

export function isCapturedE1aeStandardTuple(output, resolution) {
  return isCapturedE1aeStandardOutputTuple(output, resolution)
    && isCapturedE1aeReplayTuple(output, resolution);
}

export function capturedE1aeCompositionGrammar(output, resolution) {
  let grammar = String(output?.structured_output?.composition_grammar || "")
    .trim().toLowerCase();
  if (!grammar && [THIN_COMPOSER_VERSION_V1, THIN_COMPOSER_VERSION_V2]
    .includes(output?.composer_version)) {
    const brackets = output?.included_brackets;
    if (resolution?.grammar === "TCG") grammar = "tcg";
    else if (resolution?.grammar === "NON_TCG"
        && Array.isArray(brackets) && brackets.length > 0) {
      grammar = brackets.includes("lot") ? "lot" : "standard";
    }
  }
  if ((grammar === "tcg" && resolution?.grammar === "TCG")
      || (["standard", "lot"].includes(grammar) && resolution?.grammar === "NON_TCG")) {
    return grammar;
  }
  return null;
}

// These are the finite stage-v2 tuples the captured e1ae reader published.
// Fresh writers are gated separately by the active closed writer contract;
// this selector exists only so immutable paid checkpoints and rows keep their
// original executable meaning after activation moves forward.
const CAPTURED_E1AE_RESOLUTION_BASE = Object.freeze({
  contract_version: CSM_STAGE_LEGACY_CONTRACT_VERSION,
  revision: 1,
  resolution_status: "COMPLETE"
});
const CAPTURED_E1AE_ORDINARY_RESOLUTION = Object.freeze({
  ...CAPTURED_E1AE_RESOLUTION_BASE,
...
const CAPTURED_E1AE_VERIFIED_RESOLUTION = Object.freeze({
  ...CAPTURED_E1AE_RESOLUTION_BASE,
  grammar: "NON_TCG",
  registry_release_id: "registry_thin_sem_v25",
...
function matchesStoredDescriptor(value, descriptor) {
  return Object.entries(descriptor).every(([field, expected]) => value?.[field] === expected);
}

export function isCapturedE1aeReplayTuple(output, resolution) {
  if (output?.contract_version !== CSM_STAGE_LEGACY_CONTRACT_VERSION
      || !matchesStoredDescriptor(resolution, CAPTURED_E1AE_RESOLUTION_BASE)
      || output?.marketplace !== "EBAY") return false;
  const external = output?.structured_output?.external_identity_support;
  const verified = output?.structured_output?.verified_original_observation_support;
  if (external != null) {
    if (verified != null || capturedE1aeCompositionGrammar(output, resolution) !== "standard"
        || resolution?.grammar !== "NON_TCG") return false;
    ...
    ].includes(release?.receipt?.registry_release_id)
      && Object.entries(release.receipt).every(([field, value]) => external[field] === value)
      && Object.entries(release.output).every(([field, value]) => output?.[field] === value)
      && Object.entries(release.resolution).every(
        ([field, value]) => resolution?.[field] === value
      );
  }
  if (verified != null) {
    const composer = verifiedOriginalObservationComposerContractForReceipt(verified);
    return verified?.release_id === VERIFIED_ORIGINAL_OBSERVATION_LEGACY_RELEASE_ID
      && validateVerifiedOriginalObservationReceiptShape(verified)
      && matchesStoredDescriptor(resolution, CAPTURED_E1AE_VERIFIED_RESOLUTION)
      && output?.composer_version === composer?.composer_version
      && output?.marketplace_profile_version === composer?.marketplace_profile_version
      && output?.structured_output?.composition_grammar === "standard";
  }
  if (!matchesStoredDescriptor(resolution, CAPTURED_E1AE_ORDINARY_RESOLUTION)) return false;
  if (isCapturedE1aeStandardOutputTuple(output, resolution)) return true;
  const grammar = capturedE1aeCompositionGrammar(output, resolution);
  if (!grammar) return false;
  if ([THIN_COMPOSER_VERSION_V1, THIN_COMPOSER_VERSION_V2]
    .includes(output?.composer_version)) {
    return output?.marketplace_profile_version === EBAY_PROFILE_VERSION;
  }
...
// The earliest published eBay Lot writer rendered `LotxN`, while the e1ae
// reader intentionally replayed the same rows with the later `Lot*N` spelling
// (and omitted a single-card marker). Keep that known split finite: integrity
// binds the authentic writer bytes, while public read/review stays byte-stable
// with the e1ae reader.
export function isCapturedE1aeComposerV1LotTuple(output, resolution) {
  return isCapturedE1aeReplayTuple(output, resolution)
    && output?.composer_version === THIN_COMPOSER_VERSION_V1
    && output?.marketplace_profile_version === EBAY_PROFILE_VERSION
    && capturedE1aeCompositionGrammar(output, resolution) === "lot";
}
...
function composeLyncaStandardReplay(fields, marketplaceProfileVersion, output) {
  ...
  return composeLyncaStandardNameForProfile(fields, {
    ...
    publicationCoverage: isDurableProjectionContractVersion(output?.contract_version)
  });
}
...
const COMPOSER_DISPATCH = Object.freeze({
  [THIN_COMPOSER_VERSION_V1]: Object.freeze({
    [EBAY_PROFILE_VERSION]: (fields, output) => composeFromCanonicalFields(fields, {
      ...
      features: {
        ...
        durable_lot_terminal_shared_only: isDurableProjectionContractVersion(output?.contract_version),
        publication_coverage: isDurableProjectionContractVersion(output?.contract_version)
      }
    })
  }),
  ...
  [THIN_COMPOSER_VERSION]: Object.freeze({
    ...
    [LYNCA_STANDARD_PROFILE_VERSION_V3]: (fields, output) => composeLyncaStandardReplay(
      fields, LYNCA_STANDARD_PROFILE_VERSION_V3, output
    )
  }),
...
function storedWriterTitleForIntegrity(rows, replayed) {
  const output = rows?.output;
  if (!isCapturedE1aeComposerV1LotTuple(output, rows?.resolution)) {
    return replayed?.title;
  }
  return composeFromCanonicalFields(replayed.fields, {
    profile: MARKETPLACE_PROFILES.ebay,
    features: {
...
export function composeCanonicalFieldsForStoredOutput(fields, output) {
  if (output?.marketplace_profile_version === LYNCA_STANDARD_PROFILE_VERSION_V3
      && !isDurableProjectionContractVersion(output?.contract_version)) {
    throw new CsmReplayError("canonical_naming_v03_stage_contract_mismatch");
  }
...
function replayTcgGrammarContextCompositionGrammar(rows, durableReceipt) {
  const structured = rows?.output?.structured_output;
  const claim = durableReceipt?.tcg_grammar_context_claim_receipt;
  const observedGrammar = String(structured?.observed_composition_grammar || "")
    .trim().toLowerCase();
  const resolvedGrammar = String(structured?.composition_grammar || "")
    .trim().toLowerCase();
  if (!claim || claim.raw_grammar !== observedGrammar
      || claim.resolved_grammar !== resolvedGrammar
      || !COMPOSITION_GRAMMARS.includes(resolvedGrammar)
      || csmIdentityGrammarForComposition(resolvedGrammar) !== rows?.resolution?.grammar) {
    throw new CsmReplayError("tcg_grammar_context_replay_receipt_invalid");
  }
  if (claim.status === "APPLIED" && (
    observedGrammar !== "standard"
      || resolvedGrammar !== "tcg"
      || rows?.resolution?.grammar !== "TCG"
      || rows?.output?.composer_version !== THIN_COMPOSER_VERSION_V2
      || rows?.output?.marketplace_profile_version !== EBAY_PROFILE_VERSION
      || rows?.output?.marketplace !== "EBAY"
  )) {
    throw new CsmReplayError("tcg_grammar_context_replay_tuple_invalid");
  }
  return resolvedGrammar;
}

function replayCompositionGrammarForContract(rows, durableReceipt) {
  return rows?.output?.contract_version
    === CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION
    ? replayTcgGrammarContextCompositionGrammar(rows, durableReceipt)
    : replayCompositionGrammar(rows);
}
...
export function replayFromRows(rows, {
  allowUnsealedMutation = false,
  fieldShape = CSM_REPLAY_FIELD_SHAPE.CURRENT_COMPLETE
} = {}) {
  if (!Object.values(CSM_REPLAY_FIELD_SHAPE).includes(fieldShape)) {
    throw new CsmReplayError("csm_replay_field_shape_unsupported");
  }
  const compose = (fields) => composeCanonicalFieldsForStoredOutput(fields, rows?.output);
  const durableStage = isDurableProjectionContractVersion(
    rows?.output?.contract_version
  );
  let durableReceipt;
  try { durableReceipt = readDurableProjectionReceipt(rows); }
  catch (error) { throw new CsmReplayError(error.message); }
  const compositionGrammar = replayCompositionGrammarForContract(rows, durableReceipt);
  ...
  if (durableStage && compositionGrammar === "lot" && lotTerminal == null
  ...
  if (durableStage) {
    ...
    const normalizedStoredCoverage = normalizeCoverageForReplay(storedPublicationCoverage);
    const normalizedRecomposedCoverage = normalizeCoverageForReplay(
      recomposed.publication_coverage
    );
    if (!allowUnsealedMutation
        && JSON.stringify(canonicalJsonValue(normalizedStoredCoverage))
        !== JSON.stringify(canonicalJsonValue(normalizedRecomposedCoverage))) {
      throw new CsmReplayError("publication_coverage_replay_mismatch");
    }
    if (!allowUnsealedMutation) {
      try {
        assertDurableProjectionReplayed(durableReceipt, { fields, composed: recomposed }, {
          normalizedPublicationCoverage: {
            stored: normalizedStoredCoverage,
            recomposed: normalizedRecomposedCoverage
          }
        });
      } catch (error) {
        throw new CsmReplayError(error.message);
      }
    }
  }
  ...
  if (!allowUnsealedMutation
      && rows?.output?.marketplace_profile_version === LYNCA_STANDARD_PROFILE_VERSION_V3) {
    const problems = canonicalNamingTraceProblems(rows, { title: composed.title, composed });
    if (problems.length) throw new CsmReplayError(problems[0].kind);
  }
  const replayed = { fields, title: composed.title, grammar: composed.grammar, composed };
  if (!allowUnsealedMutation) {
    const evidenceProblems = durableEvidenceClassificationProblems(rows);
    if (evidenceProblems.length) {
      throw new CsmReplayError(
        evidenceProblems[0].kind,
        evidenceProblems[0].field || evidenceProblems[0].bracket || ""
      );
    }
    const externalProblems = externalIdentityProblems(rows);
    if (externalProblems.length) {
      throw new CsmReplayError(
        externalProblems[0].kind,
        externalProblems[0].field || externalProblems[0].bracket || ""
      );
    }
    const verifiedProblems = verifiedOriginalProblems(rows, replayed);
    if (verifiedProblems.length) {
      throw new CsmReplayError(
        verifiedProblems[0].kind,
        verifiedProblems[0].field || verifiedProblems[0].bracket || ""
      );
    }
    const relationProblems = setCardNameRelationProblems(rows, replayed);
    if (relationProblems.length) {
      throw new CsmReplayError(
        relationProblems[0].kind,
        relationProblems[0].detail || relationProblems[0].field || ""
      );
    }
  }
  return replayed;
...
function hasExternalIdentitySignal(rows) {
  const releases = Object.values(EXTERNAL_IDENTITY_REPLAY_COMPATIBILITY_REGISTRY.releases);
  return rows?.output?.structured_output?.external_identity_support != null
    || releases.some((release) => Object.entries(release.output).some(
      ([field, value]) => rows?.output?.[field] === value
    ))
    || releases.some((release) => Object.entries(release.resolution).some(
      ([field, value]) => rows?.resolution?.[field] === value
    ))
    || (rows?.evidence || []).some((row) => (
      row?.source_ref?.support_type === "EXACT_EXTERNAL_IDENTITY"
    ))
    || (rows?.candidates || []).some((row) => row?.source_trust === "REVIEWED_REGISTRY_EXACT")
    || (rows?.resolved || []).some((row) => (
      Array.isArray(row?.rationale_codes)
        && row.rationale_codes.includes("EXACT_EXTERNAL_IDENTITY_SUPPORT")
    ));
}
...
function externalIdentityProblems(rows) {
  ...
  if (!hasExternalIdentitySignal(rows)) return problems;
...
function durableEvidenceClassificationProblems(rows) {
  if (!isDurableProjectionContractVersion(rows?.output?.contract_version)) return [];
  const sessionId = rows.output.recognition_session_id;
  const external = rows.output.structured_output?.external_identity_support != null;
  const verified = rows.output.structured_output?.verified_original_observation_support != null;
  for (const row of rows?.evidence || []) {
    if (row?.modality === "WHOLE_CARD_VISUAL") {
      ...
      if (!CSM_BRACKETS.includes(row.bracket)
          || !valuePresent(row.raw_value)
          || !sameCanonicalValue(row.raw_value, row.normalized_value)
          ...
          || JSON.stringify(Object.keys(row.source_ref).sort()) !== JSON.stringify(["images"])
          || row.source_ref.images !== sessionId) {
        return [{ kind: "durable_visual_evidence_invalid", bracket: row?.bracket || null }];
      }
      continue;
    }
    if (row?.modality === "REGISTRY") {
      const supportType = row?.source_ref?.support_type;
      if (supportType === "EXACT_EXTERNAL_IDENTITY") {
        if (!external) return [{ kind: "external_identity_receipt_missing_or_unexpected" }];
        continue;
      }
      if (supportType === "EXACT_VERIFIED_ORIGINAL_CLOSED_PROJECTION") {
        if (!verified) return [{ kind: "verified_original_receipt_missing_or_unexpected" }];
        continue;
      }
      return [{ kind: "durable_registry_evidence_unclaimed", bracket: row?.bracket || null }];
    }
    return [{ kind: "durable_evidence_modality_invalid", bracket: row?.bracket || null }];
  }
  return [];
}

function hasVerifiedOriginalSignal(rows) {
  return rows?.output?.structured_output?.verified_original_observation_support != null
    ...
    || (rows?.candidates || []).some((row) => (
      row?.source_trust === "REVIEWED_CLOSED_PROJECTION_EXACT"
    ))
    || (rows?.resolved || []).some((row) => (
      Array.isArray(row?.rationale_codes)
        && row.rationale_codes.includes("EXACT_VERIFIED_ORIGINAL_CLOSED_PROJECTION")
    ));
}
...
function verifiedOriginalProblems(rows, replayed = null) {
  ...
  if (!hasVerifiedOriginalSignal(rows)) return problems;
  if (!metadata || typeof metadata !== "object" || Array.isArray(metadata)) {
  ...
  const composerContract = verifiedOriginalObservationComposerContractForReceipt(metadata);
  if (!composerContract) {
    problems.push({ kind: "verified_original_composer_contract_missing" });
    return problems;
  }
...
function durableExternalIdentityPrivateReceipt(rows) {
  const metadata = rows?.output?.structured_output?.external_identity_support;
  if (!metadata) return null;
  const sources = new Map();
  for (const row of rows?.evidence || []) {
    if (row?.source_ref?.support_type !== "EXACT_EXTERNAL_IDENTITY") continue;
    for (const source of row.source_ref.sources || []) {
...
function setCardNameRelationProblems(rows, replayed) {
  const output = rows?.output;
  if (!isDurableProjectionContractVersion(output?.contract_version)) return [];
  const outputSem = output?.structured_output?.sem;
  if (!outputSem || typeof outputSem !== "object" || Array.isArray(outputSem)) {
    return [{ kind: "set_card_name_resolved_projection_invalid", field: "sem" }];
  }
  for (const field of ["set", "card_name"]) {
    const resolvedValue = replayed?.fields?.[field];
    const resolvedRows = (rows?.resolved || []).filter((row) => row?.bracket === field);
    if (resolvedRows.length !== 1) {
      return [{ kind: "set_card_name_resolved_cardinality_invalid", field }];
    }
    const row = resolvedRows[0];
    if (resolvedValue) {
      if (row.selected_kind !== "VALUE"
          || typeof row.canonical_value !== "string"
          || row.canonical_value !== resolvedValue
          || row.empty_reason != null
          || !Object.hasOwn(outputSem, field)
          || typeof outputSem[field] !== "string"
          || outputSem[field] !== resolvedValue) {
        return [{ kind: "set_card_name_resolved_projection_invalid", field }];
      }
    } else if (row.selected_kind !== "EMPTY"
        || row.canonical_value !== null
        || !["ABSENT", "INSUFFICIENT_EVIDENCE"].includes(row.empty_reason)
        || (outputSem && typeof outputSem === "object" && Object.hasOwn(outputSem, field))) {
      return [{ kind: "set_card_name_resolved_projection_invalid", field }];
    }
  }
  const observedRelationFields = {};
  for (const field of ["set", "card_name"]) {
    const visual = (rows?.evidence || []).filter((row) => (
      row?.modality === "WHOLE_CARD_VISUAL" && row?.bracket === field
    ));
    if (visual.length > 1) {
      return [{ kind: "set_card_name_observed_evidence_cardinality_invalid", field }];
    }
    if (visual.length === 1 && (
      !valuePresent(visual[0].raw_value)
        || !sameCanonicalValue(visual[0].raw_value, visual[0].normalized_value)
        || !visual[0].source_ref || typeof visual[0].source_ref !== "object"
        || Array.isArray(visual[0].source_ref)
        || JSON.stringify(Object.keys(visual[0].source_ref).sort())
          !== JSON.stringify(["images"])
        || visual[0].source_ref.images !== output?.recognition_session_id
    )) {
      return [{ kind: "set_card_name_observed_evidence_invalid", field }];
    }
    observedRelationFields[field] = visual[0]?.raw_value ?? "";
  }
  const externalIdentitySupport = durableExternalIdentityPrivateReceipt(rows);
  const verifiedOriginalObservationSupport =
    output?.structured_output?.verified_original_observation_support;
  let observedFields = observedRelationFields;
  if (externalIdentitySupport) {
    observedFields = { ...replayed?.fields };
    for (const [field, decision] of Object.entries(
      externalIdentitySupport.field_decisions || {}
    )) {
      observedFields[field] = decision.observed_value;
    }
    // Set/Card Name are independently reconstructed from their exact visual
    // rows. A resolver decision may bind the rest of the observed tuple, but
    // it cannot hide an added or removed relation-bearing observation.
    for (const field of ["set", "card_name"]) {
      observedFields[field] = observedRelationFields[field];
    }
  } else if (verifiedOriginalObservationSupport) {
    observedFields = verifiedOriginalObservationSupport.observed_fields;
  }
  try {
    validateResolvedSetCardNameRelationReceipt({
      receipt: output?.structured_output?.set_card_name_relation_receipt,
      observedFields,
      resolvedFields: replayed?.fields,
      externalIdentitySupport,
      verifiedOriginalObservationSupport,
      verifiedOriginalResolvedProjection: verifiedOriginalObservationSupport
        ? verifiedOriginalObservationReplayProjection(replayed?.fields)
        : null
    });
  } catch (error) {
    return [{
      kind: "set_card_name_relation_transition_invalid",
      detail: String(error?.message || "invalid")
    }];
  }
  return [];
}
...
export function verifyReplay(rows, originalTitle) {
  ...
  if ((rows?.output?.contract_version === CSM_STAGE_LEGACY_CONTRACT_VERSION
      || rows?.resolution?.contract_version === CSM_STAGE_LEGACY_CONTRACT_VERSION)
      && !isCapturedE1aeReplayTuple(rows?.output, rows?.resolution)) {
    problems.push({ kind: "unsupported_stage_v2_replay_tuple" });
  }
  ...
  const integrityTitle = storedWriterTitleForIntegrity(rows, replayed);
  if (integrityTitle !== originalTitle) {
    problems.push({
      kind: "title_mismatch",
      stored: rows.output?.title ?? null,
      original: originalTitle,
      replayed: integrityTitle
    });
  }
