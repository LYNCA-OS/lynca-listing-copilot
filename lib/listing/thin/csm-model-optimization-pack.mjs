...
import { CSM_STAGED_TRANSPORT_PROFILE } from "./staged-recognition-input.mjs";
...
const REQUEST_EXTENSION_KEYS = Object.freeze([
  "prompt_cache",
  "text_verbosity",
  "reasoning_context"
]);
 ...
 * added here. Reserved request extensions are explicit nulls so a later Luna
 * experiment can be enabled by replacing this pack without silently changing
 * today's provider bytes.
 */
export function validateCsmModelOptimizationPack(value) {
  ...
  if (!exactPlainObject(value.request_extensions, REQUEST_EXTENSION_KEYS)) {
    throw new TypeError("model_optimization_pack_request_extensions_invalid");
  }
  ...
  const requestExtensions = {
    prompt_cache: value.request_extensions.prompt_cache,
    text_verbosity: value.request_extensions.text_verbosity,
    reasoning_context: value.request_extensions.reasoning_context
  };
  if (Object.values(requestExtensions).some((extension) => extension !== null)) {
    throw new TypeError("model_optimization_pack_request_extension_unreleased");
  }
  ...
  return deepFreeze({
    ...
    transport_profile_id: requiredText(
      value.transport_profile_id,
      "optimization_transport_profile_id"
    )
  });
}
...
export const CSM_LUNA_OPTIMIZATION_PACK = validateCsmModelOptimizationPack({
  ...
  // This is a reference to the model-neutral bounded derived transport. The
  // pack may select it; the transport implementation never imports Luna.
  transport_profile_id: CSM_STAGED_TRANSPORT_PROFILE.id
});
