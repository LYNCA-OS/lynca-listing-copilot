...
import {
  ...
  externalIdentityResolutionContractForRegistryRelease
...
const LEGACY_RELEASE_ID = "verified_original_closed_projection_subset_a_v1";
const RELEASE_ID = "verified_original_closed_projection_subset_a_v2";
...
export const VERIFIED_ORIGINAL_OBSERVATION_LEGACY_RELEASE_ID = LEGACY_RELEASE_ID;
...
const resolutionContractPayload = (marketplaceProfileVersion, contractId) => ({
...
const LEGACY_RESOLUTION_CONTRACT_PAYLOAD = resolutionContractPayload(
  "lynca-standard-name-v0.2",
  "lynca.csm.post-observation.verified-original-closed-projection.v1"
);
const LEGACY_RESOLUTION_CONTRACT = deepFreeze({
  ...LEGACY_RESOLUTION_CONTRACT_PAYLOAD,
  contract_sha256: sha256(LEGACY_RESOLUTION_CONTRACT_PAYLOAD)
});
export const VERIFIED_ORIGINAL_OBSERVATION_RESOLUTION_CONTRACT_V1 =
  LEGACY_RESOLUTION_CONTRACT;
const RESOLUTION_CONTRACT_PAYLOAD = resolutionContractPayload(
  "lynca-standard-name-v0.3",
  "lynca.csm.post-observation.verified-original-closed-projection.v2"
);
...
const LEGACY_COMBINED_RESOLUTION_CONTRACT_PAYLOAD = {
  ...COMBINED_RESOLUTION_CONTRACT_PAYLOAD,
  verified_original_observation_contract_sha256:
    LEGACY_RESOLUTION_CONTRACT.contract_sha256
};
export const COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT_V1 = deepFreeze({
  ...LEGACY_COMBINED_RESOLUTION_CONTRACT_PAYLOAD,
  contract_sha256: sha256(LEGACY_COMBINED_RESOLUTION_CONTRACT_PAYLOAD)
});

const FORWARD_COMBINED_RESOLUTION_CONTRACT_PAYLOAD = {
  ...COMBINED_RESOLUTION_CONTRACT_PAYLOAD,
  external_identity_contract_sha256:
    EXTERNAL_IDENTITY_RESOLUTION_CONTRACT_V3.contract_sha256
};
export const COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT_EXTERNAL_V3 = deepFreeze({
  ...FORWARD_COMBINED_RESOLUTION_CONTRACT_PAYLOAD,
  contract_sha256: sha256(FORWARD_COMBINED_RESOLUTION_CONTRACT_PAYLOAD)
});

const COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT_REGISTRY = deepFreeze([
  {
    verified_original_observation_release_id: LEGACY_RELEASE_ID,
    external_identity_registry_release_id:
      EXTERNAL_IDENTITY_RESOLUTION_CONTRACT.registry_release_id,
    contract: COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT_V1
  },
  {
    verified_original_observation_release_id: RELEASE_ID,
    external_identity_registry_release_id:
      EXTERNAL_IDENTITY_RESOLUTION_CONTRACT.registry_release_id,
    contract: COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT
  },
  {
    verified_original_observation_release_id: RELEASE_ID,
    external_identity_registry_release_id:
      EXTERNAL_IDENTITY_RESOLUTION_CONTRACT_V3.registry_release_id,
    contract: COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT_EXTERNAL_V3
  }
]);
...
// projection router.
const verifiedOriginalObservationHealthReceipt = ({
  releaseId,
  resolutionContract,
  combinedContract
}) => deepFreeze({
  ...
  post_observation_contract_sha256: combinedContract.contract_sha256,
  composer_version: resolutionContract.composer_contract.composer_version,
  marketplace_profile_version:
    resolutionContract.composer_contract.marketplace_profile_version
});
export const VERIFIED_ORIGINAL_OBSERVATION_LEGACY_HEALTH_RECEIPT =
  verifiedOriginalObservationHealthReceipt({
    releaseId: LEGACY_RELEASE_ID,
    resolutionContract: LEGACY_RESOLUTION_CONTRACT,
    combinedContract: COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT_V1
  });
export const VERIFIED_ORIGINAL_OBSERVATION_HEALTH_RECEIPT =
  verifiedOriginalObservationHealthReceipt({
    releaseId: RELEASE_ID,
    resolutionContract: VERIFIED_ORIGINAL_OBSERVATION_RESOLUTION_CONTRACT,
    combinedContract: COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT
  });
export const VERIFIED_ORIGINAL_OBSERVATION_EXTERNAL_V3_HEALTH_RECEIPT =
  verifiedOriginalObservationHealthReceipt({
    releaseId: RELEASE_ID,
    resolutionContract: VERIFIED_ORIGINAL_OBSERVATION_RESOLUTION_CONTRACT,
...
export function verifiedOriginalObservationHealthReceiptForReleases({
  verifiedOriginalObservationReleaseId,
  externalIdentityRegistryReleaseId
} = {}) {
  const combinedContract = combinedPostObservationResolutionContractForReleases({
    verifiedOriginalObservationReleaseId,
    externalIdentityRegistryReleaseId
  });
  if (combinedContract === COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT_V1) {
    return VERIFIED_ORIGINAL_OBSERVATION_LEGACY_HEALTH_RECEIPT;
  }
  if (combinedContract === COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT) {
    return VERIFIED_ORIGINAL_OBSERVATION_HEALTH_RECEIPT;
  }
  if (combinedContract === COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT_EXTERNAL_V3) {
    return VERIFIED_ORIGINAL_OBSERVATION_EXTERNAL_V3_HEALTH_RECEIPT;
  }
  throw new TypeError("verified_original_observation_health_pair_unknown");
}

export function verifiedOriginalObservationHealthReceiptForRelease(releaseId) {
  if (![LEGACY_RELEASE_ID, RELEASE_ID].includes(releaseId)) {
  ...
  return verifiedOriginalObservationHealthReceiptForReleases({
    verifiedOriginalObservationReleaseId: releaseId,
    externalIdentityRegistryReleaseId:
      EXTERNAL_IDENTITY_RESOLUTION_CONTRACT.registry_release_id
  });
}

const releaseReceipt = ({ releaseId, resolutionContract }) => deepFreeze({
  ...
  resolution_contract_sha256:
    resolutionContract.contract_sha256,
  resolver_version: VERIFIED_ORIGINAL_OBSERVATION_RESOLVER_VERSION,
  conflict_policy_version: VERIFIED_ORIGINAL_OBSERVATION_CONFLICT_POLICY_VERSION
});
const LEGACY_RELEASE_RECEIPT = releaseReceipt({
  releaseId: LEGACY_RELEASE_ID,
  resolutionContract: LEGACY_RESOLUTION_CONTRACT
});
const RELEASE_RECEIPT = releaseReceipt({
  releaseId: RELEASE_ID,
  resolutionContract: VERIFIED_ORIGINAL_OBSERVATION_RESOLUTION_CONTRACT
});
...
export const VERIFIED_ORIGINAL_OBSERVATION_REPLAY_COMPATIBILITY_REGISTRY = deepFreeze({
  ...
  releases: {
    [LEGACY_RELEASE_ID]: {
      receipt: LEGACY_RELEASE_RECEIPT,
      composer_contract: LEGACY_RESOLUTION_CONTRACT.composer_contract
    },
    [RELEASE_ID]: {
...
export function resolveVerifiedOriginalObservation(observedFields, context = null, {
  ...
  const release = VERIFIED_ORIGINAL_OBSERVATION_REPLAY_COMPATIBILITY_REGISTRY
    .releases[releaseId];
  if (!release) throw new TypeError("verified_original_observation_active_release_unknown");
...
export function verifiedOriginalObservationComposerContractForReceipt(receipt) {
  return releaseForReceipt(receipt)?.composer_contract || null;
}

export function combinedPostObservationResolutionContractForVerifiedOriginalRelease(
  releaseId
) {
  if (![LEGACY_RELEASE_ID, RELEASE_ID].includes(releaseId)) {
    throw new TypeError("verified_original_observation_active_release_unknown");
  }
  return combinedPostObservationResolutionContractForReleases({
    verifiedOriginalObservationReleaseId: releaseId,
    externalIdentityRegistryReleaseId:
      EXTERNAL_IDENTITY_RESOLUTION_CONTRACT.registry_release_id
  });
...
export function combinedPostObservationResolutionContractForReleases({
  verifiedOriginalObservationReleaseId,
  externalIdentityRegistryReleaseId
} = {}) {
  const match = COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT_REGISTRY.find((entry) => (
    entry.verified_original_observation_release_id
      === verifiedOriginalObservationReleaseId
      && entry.external_identity_registry_release_id
        === externalIdentityRegistryReleaseId
  ));
  if (!match) throw new TypeError("post_observation_resolution_pair_unknown");
  return match.contract;
}

export function externalIdentityResolutionContractForPostObservationSha256(value) {
  const digest = String(value || "").trim().toLowerCase();
  for (const contract of [
    EXTERNAL_IDENTITY_RESOLUTION_CONTRACT,
    EXTERNAL_IDENTITY_RESOLUTION_CONTRACT_V3
  ]) {
    if (digest === contract.contract_sha256) return contract;
  }
  const combined = COMBINED_POST_OBSERVATION_RESOLUTION_CONTRACT_REGISTRY.find(
    ({ contract }) => contract.contract_sha256 === digest
  );
  return combined ? externalIdentityResolutionContractForRegistryRelease(
    combined.external_identity_registry_release_id
  ) : null;
}
...
export function postObservationResolutionContractForVerifiedOriginals({
  ...
  if (activeReleaseId != null && ![LEGACY_RELEASE_ID, RELEASE_ID].includes(activeReleaseId)) {
    throw new TypeError("verified_original_observation_active_release_unknown");
  }
  const combinedContract = activeReleaseId == null ? null
    : combinedPostObservationResolutionContractForReleases({
        verifiedOriginalObservationReleaseId: activeReleaseId,
        externalIdentityRegistryReleaseId
      });
  const match = activeReleaseId != null
