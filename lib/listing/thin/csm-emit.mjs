 ...
 * only at parse time.
 */
export function checkNumberBrackets(fields = {}) {
  const problems = [];
  ...
  if (fields.card_number
    ...
    problems.push(`card_number "${fields.card_number}" is not a CSM card number`);
  }
  return problems;
}
...
export function unknownFieldNames(fields = {}) {
  const known = new Set(Object.keys(semImplementationTermMap));
