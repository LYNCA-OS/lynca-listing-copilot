 ...
 * `print_run_number`, `collector_number` and `rc`/`auto`/`patch`/`relic`
 * booleans, none of which are what this schema calls them. A field this map
 * misses is silently dropped from the canonical object -- which is the same
 * class of bug as the TCG parallel bracket that vanished because the grammar
...
export function toResolvedFields(fields = {}) {
  ...
  return {
    ...
    // Both are CSM canonical fields -- `semCanonicalEditableFields` has named
    // them all along. Leaving them out of the resolved record would persist a
    // card whose title says "1st Edition" while the stored identity does not,
    // which is the projection outranking the canonical record that
    // `60 CSM Rebuild Contract` forbids.
    special_stamp: fields.special_stamp || undefined,
    ...
    grade_company: fields.grading_info?.company || undefined,
    card_grade: fields.grading_info?.card_grade || undefined,
    auto_grade: fields.grading_info?.auto_grade || undefined,
    grade_type: fields.grading_info?.grade_type || undefined,
    // Backwards compatibility for replaying pre-structured checkpoints.
    grade: fields.grading_info ? undefined : (fields.grade || undefined)
  };
}
...
export function unknownFieldNames(fields = {}) {
  ...
  const aliases = new Set(["subjects", "grade", "components", "ip", "grammar", "lot_count",
    ...
    // The finish admission layer's bookkeeping. These are not canonical fields
    // and must never become brackets: they preserve what was OBSERVED next to
    // what was RESOLVED, so a term denied promotion stays in the evidence
    // record and the rejection is reversible by a registry that knows better.
