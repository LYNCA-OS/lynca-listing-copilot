...
function buildFieldLedger(parsedValue, result, { capturedE1ae = false } = {}) {
  const admittedSem = resolvedFieldsToSemSuggestion(toResolvedFields(
    result?.fields || {}, { capturedE1ae }
  ));
...
export function buildAccuracyLossLedger({
  rawProviderOutput,
  result,
  semantics = "CURRENT"
} = {}) {
  if (!["CURRENT", "CAPTURED_E1AE_V1"].includes(semantics)) {
    throw new TypeError("accuracy_loss_ledger_semantics_unsupported");
  }
  ...
  const fieldLedger = buildFieldLedger(parsed.value, result, {
    capturedE1ae: semantics === "CAPTURED_E1AE_V1"
  });
...
function validateAccuracyLossLedgerV1(value, {
  result = null,
  semantics = "CURRENT"
} = {}) {
  if (!["CURRENT", "CAPTURED_E1AE_V1"].includes(semantics)) {
  ...
  if (result) {
    ...
    const admittedSem = resolvedFieldsToSemSuggestion(toResolvedFields(
      result.fields || {}, { capturedE1ae: semantics === "CAPTURED_E1AE_V1" }
    ));
