...
function compileCsmModelExecutionArtifacts({
  ...
  const expectedSchema = writer.web_search_tools_enabled
    ? CANONICAL_FIELDS_SCHEMA
    : CAPTURED_E1AE_CANONICAL_FIELDS_SCHEMA;
  const requestSchema = schema || expectedSchema;
  if (requireActiveAdapter
      && sha256ExecutionContractValue(requestSchema)
        !== sha256ExecutionContractValue(expectedSchema)) {
    throw new TypeError("active_writer_schema_override_mismatch");
  }
  const providerRequest = resolvedAdapter.compileRequest({
    ...
    webSearchToolsEnabled: writer.web_search_tools_enabled,
  ...
  if (resolvedAdapter.provider !== profileReceipt.provider
      ...
      || requestBuilderVersion !== writer.canonical_fields.request_builder_version
      || parserVersion !== writer.canonical_fields.response_parser_version) {
