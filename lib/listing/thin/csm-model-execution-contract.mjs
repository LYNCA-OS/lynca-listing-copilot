...
export const CSM_MODEL_EXECUTION_CONTRACT_VERSION = "csm-model-execution-v1";
...
export function validateCsmModelExecutionContract(value, {
  ...
  if (!exactPlainObject(value, CSM_MODEL_EXECUTION_CONTRACT_KEYS)) {
    throw new TypeError("execution_contract_shape_invalid");
  }
  const contract = {
    contract_version: canonicalText(value.contract_version, "contract_version"),
  ...
  if (contract.contract_version !== CSM_MODEL_EXECUTION_CONTRACT_VERSION) {
    throw new TypeError("execution_contract_version_unsupported");
  }
...
function compileCsmModelExecutionArtifacts({
  ...
  transportProfileId = profile?.transport_profile_id,
  imageUrls = [],
  ...
  const providerRequest = resolvedAdapter.compileRequest({
    ...
    prompt: requiredContent(renderedPrompt, "rendered_prompt"),
    schema,
    samplingParameters: optimizationPack?.request_defaults.sampling_parameters || "omit",
    requestExtensions: optimizationPack?.request_extensions || null
  });
  ...
  const profileReceipt = {
    ...
    prompt_style_version: requiredText(promptStyleVersion, "prompt_style_version"),
    ...
    optimization_pack_sha256: optimizationPack
      ...
      : null,
    transport_profile_id: requiredText(transportProfileId, "transport_profile_id"),
  ...
  const executionContract = validateCsmModelExecutionContract({
    ...
    transport_profile_id: profileReceipt.transport_profile_id,
    ...
    semantic_prompt_version: requiredText(semanticPromptVersion, "semantic_prompt_version"),
