...
export async function checkCsmProviderAdmissionReadiness({
  ...
  const authority = await probe(CSM_PROVIDER_AUTHORITY_RPCS.lookup, {
  ...
  const pacer = await probe(CSM_PROVIDER_AUTHORITY_RPCS.pacerReadiness, {
  ...
  const receipt = pacer.result;
  const pacerReady = receipt?.ok === true
