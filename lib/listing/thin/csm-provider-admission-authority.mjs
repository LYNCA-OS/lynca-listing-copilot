...
export const CSM_PROVIDER_AUTHORITY_LIMITS = Object.freeze({
  ...
  // 440k wall remains an 83-attempt last-resort bound at 5,300 tokens each.
...
// Expand/contract rollout bridge only. The running Production contract and
// health receipt remain at 65,200 while the database moves to 66,000. Accept
// exactly the two rollout endpoints here; do not turn this into a range. Once
// no old runtime can observe 65,200, contract this list to the new runtime
// value in the release that updates CSM_PROVIDER_AUTHORITY_LIMITS.
const CSM_PROVIDER_PACER_READINESS_BURST_TOKENS = Object.freeze([
...
export function csmProviderPacerReadinessMatches(receipt) {
  return receipt?.ok === true
    ...
    && CSM_PROVIDER_PACER_READINESS_BURST_TOKENS.includes(
...
export async function checkCsmProviderAdmissionReadiness({
  ...
  // weakening the fail-closed contract: both receipts still have to match
  // their exact production limits before a paid attempt is admitted.
  const [authority, pacer] = await Promise.all([
  ...
  async function runAttempt({ queuedAttempt, execute } = {}) {
    ...
    try {
      ...
      try {
        ...
        try {
          const failureResult = providerErrorResult(error, ambiguous);
          await settleAttempt(
            lease,
        ...
        throw executionError(error, ambiguous);
      }

      let authoritySettleMs = null;
      try {
        const settleStartedAt = now();
        await settleAttempt(lease, "SUCCEEDED", result);
