...
} from "./luna-direct-dispatcher.mjs";
...
export const CSM_THIN_RUNTIME_CONTRACT = Object.freeze({
  ...
  // One successful semantic result remains the absolute contract. A second
  // physical attempt is admitted only after the authority durably settles an
  // exact, complete, definitive HTTP 502 with no output or token receipt.
  maximumAttempts: 2,
  semanticResultLimit: 1,
  automaticTransportRetryLimit: 1,
  automaticTransportRetryPolicy: LUNA_DEFINITIVE_502_TRANSPORT_RETRY_POLICY,
  claimPollMs: 1_000,
