...
import {
  reconcileSetCardNameRelationReceipt
...
export const CANONICAL_NAMING_TCG_GRAMMAR_AUTHORITY_MISSING =
...
export function finishCanonicalFields(fields, {
  ...
  fieldDefects = [],
  writerContract = activeWriterProjectionContract()
} = {}) {
  const writer = validateCsmWriterProjectionContract(writerContract);
  ...
  const futureComposition = writer.composition_features.publication_coverage;
  const lotFailureCode = futureComposition ? lotPublicationFailureCode({
    quantityUnresolved: composed.lot_quantity_unresolved,
    singleCard: composed.lot_single_card
  }) : null;
  // CNL already protects Standard. Apply the same single-card invariant to the
  // shared result so a withheld Web-only subject cannot produce product-only TCG.
  const mandatorySubjectMissing = writer.composition_features.enforce_global_mandatory_subject
    && cleaned.grammar !== "lot" && cleaned.subjects.length === 0;
  const tcgGrammarAuthorityMissing =
    writer.canonical_fields.parser_semantics
      === CANONICAL_FIELDS_PARSER_SEMANTICS.WEB_V3_TCG_CONTEXT
    && cleaned.grammar === "standard"
    && fieldDefects.includes(CANONICAL_TCG_GRAMMAR_AUTHORITY_MISSING_DEFECT);
  const publicationBlocked = mandatorySubjectMissing || tcgGrammarAuthorityMissing;
  return {
    title: publicationBlocked ? "" : composed.title,
    fields: cleaned,
    ...
    canonical_naming_publishable: !publicationBlocked
      && composed.canonical_naming_publishable !== false,
    canonical_naming_failure_code: tcgGrammarAuthorityMissing
      ? CANONICAL_NAMING_TCG_GRAMMAR_AUTHORITY_MISSING
      : mandatorySubjectMissing
        ? "canonical_naming_mandatory_subject_identity_missing"
        : composed.canonical_naming_failure_code || null,
    ...(futureComposition ? {
    ...
    length: publicationBlocked ? 0 : composed.length
  };
}
...
export function composeActiveCanonicalFields(fields, {
  ...
  if (standard) {
    ...
    if (canonicalContract) {
      return composeLyncaStandardNameForProfile(fields, {
        ...
        publicationCoverage: writer.composition_features.publication_coverage,
  ...
  if (writer.composition_features.durable_lot_terminal_shared_only
      || writer.composition_features.publication_coverage) {
    composeOptions.features = {
      durable_lot_terminal_shared_only:
        writer.composition_features.durable_lot_terminal_shared_only,
      publication_coverage: writer.composition_features.publication_coverage
    };
  }
...
export function finishCanonicalTitle(payload, options = {}) {
  const writer = validateCsmWriterProjectionContract(
    options.writerContract || activeWriterProjectionContract()
  );
  const { fields, defects, observed_fields: observedFields } = parseCanonicalFields(payload, {
    semantics: writer.canonical_fields.parser_semantics,
    finishAdmissionSemantics:
      writer.canonical_fields.finish_admission_semantics,
    tcgFieldSourceAuthorityReceipt: options.tcgFieldSourceAuthorityReceipt,
    tcgGrammarContextClaimReceipt: options.tcgGrammarContextClaimReceipt
  });
  return {
    ...finishCanonicalFields(fields, {
      ...options,
      writerContract: writer,
    ...
    ...(writer.canonical_fields.parser_semantics
      === CANONICAL_FIELDS_PARSER_SEMANTICS.WEB_V3_TCG_CONTEXT ? {
        ...
        tcg_field_source_authority_receipt:
          options.tcgFieldSourceAuthorityReceipt,
        tcg_grammar_context_claim_receipt:
          options.tcgGrammarContextClaimReceipt
      } : {})
  };
}
...
function canonicalProviderFailure(response, body, {
  ...
  providerMs = null,
  retryPolicy = "DEFINITIVE_502_ONLY_V1"
} = {}) {
  ...
  error.safe_to_retry = retryPolicy === "CAPTURED_E1AE_GENERIC_TRANSIENT_V1"
    ? status === 429 || [500, 502, 503, 504].includes(status)
    : !error.provider_business_failure
      && (status === 429 || [500, 502, 503, 504].includes(status));
...
export async function runCanonicalListingPath({
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw canonicalProviderFailure(response, {}, {
        providerClientRequestId,
        providerMs: Date.now() - startedAt,
        retryPolicy: writer.provider_dispatch.retry_policy
      });
    }
  ...
  const observation = finishCanonicalTitle(rawProviderOutput, {
    ...
    tcgFieldSourceAuthorityReceipt:
      parsedProviderResponse.receipt?.tcg_field_source_authority_receipt,
    tcgGrammarContextClaimReceipt:
      parsedProviderResponse.receipt?.tcg_grammar_context_claim_receipt
  });
  const providerRelationReceipt = parsedProviderResponse.receipt?.set_card_name_relation_receipt;
  ...
  let finalRelationReceipt = null;
  if (writer.reconcile_set_card_name_relations) {
    try {
      finalRelationReceipt = reconcileSetCardNameRelationReceipt({
        providerReceipt: providerRelationReceipt,
        observedFields: observation.fields,
        resolvedFields: finished.fields,
        externalIdentitySupport: finished.external_identity_support,
        verifiedOriginalObservationSupport: finished.verified_original_observation_support
      });
    } catch (error) {
      throw canonicalProviderContractFailure(response, "post_observation_relation_failed", {
        providerClientRequestId,
        providerMs: Date.now() - startedAt,
        cause: error
      });
  ...
  return {
    ...
    // This is the final post-resolution contract. Captured Production did not
    // emit the relation key at all, including as null.
    ...(writer.reconcile_set_card_name_relations ? {
      set_card_name_relation_receipt: finalRelationReceipt
    } : {}),
