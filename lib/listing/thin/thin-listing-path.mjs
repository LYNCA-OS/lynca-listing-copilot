...
import { composeLyncaStandardName } from "./canonical-naming-adapter.mjs";
import {
...
export function composeActiveCanonicalFields(fields, {
  ...
  if (!verifiedExternalIdentity && String(fields?.grammar || "standard").toLowerCase() === "standard") {
    return composeLyncaStandardName(fields, limit === undefined ? {} : { limit });
  }
...
export async function runCanonicalListingPath({
  ...
  const parsedProviderResponse = providerAdapter.parseResponse(body);
