...
function canonicalProviderFailure(response, body) {
  ...
  const error = new Error(
    `canonical_path_provider_failed: ${body?.error?.message || (status ? `provider_status_${status}` : "invalid_response")}`
  );
...
export async function runCanonicalListingPath({
  imageUrls, model, effort = "none", imageDetail = "high", providerClientRequestId = null,
  callProvider
}) {
