  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let probe = null;
        try {
          probe = await rpcResponse.json();
        } catch (error) {
          if (requestTimedOut(error)) throw error;
        }
        if (probe?.code !== "missing_csm_stage_row_identity") {
          return { ready: false, reason: "atomic_rpc_probe_contract_mismatch" };
        }

        phase = "product_projection_probe";
        ...
        if (!projectionResponse.ok) {
          return { ready: false, reason: `product_projection_probe_${projectionResponse.status}` };
        }
        let projection = null;
        try {
        ...
        return projection?.ok === true
          && projection?.code === "csm_product_projection_ready"
          && projection?.version === CSM_PRODUCT_PROJECTION_VERSION
          ? { ready: true, reason: null }
          : { ready: false, reason: "product_projection_probe_contract_mismatch" };
      }
    } catch (error) {
