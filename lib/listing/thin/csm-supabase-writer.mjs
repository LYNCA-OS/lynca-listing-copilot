...
import { validateExternalIdentityReplayPacket } from "./csm-replay.mjs";
import {
...
export async function readCsmResolutionRecord({
  ...
  if (replayDescriptor
    ...
    // These private values bind the saved receipt to both the original visual
    // observation and the selected Registry value. They are consumed only by
    // the replay validator below and never copied into the public projection.
    // Do not filter by modality: a drifted exact-support row must stay visible
    // so the release-scoped validator can reject it.
    evidenceEndpoint.searchParams.set(
      "select", "bracket,raw_value,normalized_value,modality,source_ref"
    );
