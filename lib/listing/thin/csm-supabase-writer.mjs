...
export async function readCsmResolutionRecord({
  ...
  if (row.resolution_id) {
    ...
    resolutionEndpoint.searchParams.set("select", "id,resolver_version");
    ...
    try {
      resolverVersion = (await readRows(resolutionEndpoint, "csm_resolution_version_read"))[0]?.resolver_version || "";
    } catch { resolverVersion = ""; }
  }
