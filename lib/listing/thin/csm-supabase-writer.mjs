...
import { readDurableProjectionReceipt } from "./csm-forward-reader-bridge.mjs";
...
import {
  ...
  validateCapturedE1aeResolutionReviewIntegrity,
...
import {
  ...
  tcgGrammarContextSessionIdentitySha256,
...
export const THIN_REGISTRY_PAYLOAD_CONTRACT = Object.freeze({
  mode: "local_sem_and_composer_only",
...
const EXTERNAL_IDENTITY_FORWARD_REGISTRY_RELEASE_ID =
  "registry_thin_external_identity_high_risers_v3";
const EXTERNAL_IDENTITY_FORWARD_REPLAY_RELEASE = externalIdentityReplayReleaseForReceipt({
  registry_release_id: EXTERNAL_IDENTITY_FORWARD_REGISTRY_RELEASE_ID
});
export const THIN_EXTERNAL_IDENTITY_FORWARD_REGISTRY_RELEASE_CONTRACT = Object.freeze({
  id: EXTERNAL_IDENTITY_FORWARD_REGISTRY_RELEASE_ID,
  registry_version: "thin-path-external-identity-high-risers-v3",
  content_sha256: EXTERNAL_IDENTITY_FORWARD_REPLAY_RELEASE.receipt.pack_sha256,
  sem_standard_version: "linear-cos-10-23-v25"
});
export const THIN_EXTERNAL_IDENTITY_FORWARD_REGISTRY_PAYLOAD_CONTRACT = Object.freeze({
  mode: "post_observation_exact_external_identity",
  external_catalog: true,
  pack_id: EXTERNAL_IDENTITY_FORWARD_REPLAY_RELEASE.receipt.pack_id,
  pack_version: EXTERNAL_IDENTITY_FORWARD_REPLAY_RELEASE.receipt.pack_version,
  index_id: EXTERNAL_IDENTITY_FORWARD_REPLAY_RELEASE.receipt.index_id,
  pack_sha256: EXTERNAL_IDENTITY_FORWARD_REPLAY_RELEASE.receipt.pack_sha256,
  index_sha256: EXTERNAL_IDENTITY_FORWARD_REPLAY_RELEASE.receipt.index_sha256,
  resolution_contract_sha256:
    EXTERNAL_IDENTITY_FORWARD_REPLAY_RELEASE.receipt.resolution_contract_sha256,
  provider_calls_added: 0
});
export const THIN_TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE_CONTRACT = Object.freeze({
  id: TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE.release_id,
  registry_version: "tcg-grammar-context-trainer-gallery-v1",
  content_sha256: TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE.content_sha256,
  sem_standard_version: "linear-cos-10-23-v25"
});
export const THIN_TCG_GRAMMAR_CONTEXT_REGISTRY_PAYLOAD_CONTRACT = Object.freeze({
  mode: "grammar_context_closed_transition",
  external_catalog: false,
  registry_schema_version: TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE.schema_version,
  registry_content_sha256: TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE.content_sha256,
  policy_version: TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE.policy_version,
  normalization_version: TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE.normalization_version,
  decision_document_sha256:
    TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE.decision_document_sha256,
  resolution_contract_schema_version:
    TCG_GRAMMAR_CONTEXT_RESOLUTION_CONTRACT.schema_version,
  resolution_contract_sha256:
...
const TCG_GRAMMAR_CONTEXT_STRUCTURED_KEYS = Object.freeze([
  "observed_composition_grammar",
  "tcg_field_source_authority_receipt",
  "tcg_grammar_context_claim_receipt"
]);

function stageProjectionContractVersion(rows) {
  const outputVersion = safeText(rows?.output?.contract_version);
  const resolutionVersion = safeText(rows?.resolution?.contract_version);
  if (!outputVersion || outputVersion !== resolutionVersion) {
    throw new Error("durable_projection_row_family_mismatch");
  }
  if (![CSM_STAGE_LEGACY_CONTRACT_VERSION,
    CSM_DURABLE_PROJECTION_CONTRACT_VERSION,
    CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION]
    .includes(outputVersion)) {
    throw new Error("csm_stage_contract_version_unsupported");
  }
  const structured = rows?.output?.structured_output || {};
  if (outputVersion !== CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION
      && TCG_GRAMMAR_CONTEXT_STRUCTURED_KEYS.some((key) => (
        Object.hasOwn(structured, key)
      ))) {
    throw new Error("tcg_grammar_context_receipt_outside_contract");
  }
  if (outputVersion === CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION) {
    readDurableProjectionReceipt(rows);
  }
  return outputVersion;
}

function stageRowsIdentity(rows) {
  stageProjectionContractVersion(rows);
...
function ownerStageContractVersion(owner) {
  let ownerReceipt;
  try { ownerReceipt = projectCsmOwnerExecutionReceipt(owner); }
  catch { return "INVALID"; }
  if (ownerReceipt == null) return null;
  const requestBuilder = safeText(owner?.request_builder_version);
  const responseParser = safeText(owner?.response_parser_version);
  if (requestBuilder === TCG_GRAMMAR_CONTEXT_CANONICAL_REQUEST_BUILDER_VERSION
      || responseParser === TCG_GRAMMAR_CONTEXT_CANONICAL_RESPONSE_PARSER_VERSION) {
    return requestBuilder === TCG_GRAMMAR_CONTEXT_CANONICAL_REQUEST_BUILDER_VERSION
        && responseParser === TCG_GRAMMAR_CONTEXT_CANONICAL_RESPONSE_PARSER_VERSION
      ? CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION : "INVALID";
  }
  if (requestBuilder === FUTURE_CANONICAL_REQUEST_BUILDER_VERSION
      || responseParser === FUTURE_CANONICAL_RESPONSE_PARSER_VERSION) {
    return requestBuilder === FUTURE_CANONICAL_REQUEST_BUILDER_VERSION
        && responseParser === FUTURE_CANONICAL_RESPONSE_PARSER_VERSION
      ? CSM_DURABLE_PROJECTION_CONTRACT_VERSION : "INVALID";
  }
  return null;
}

function validTcgGrammarContextOwnerVersions(owner, rows) {
  let ownerReceipt;
  let sourceReceipt;
  try {
    ownerReceipt = projectCsmOwnerExecutionReceipt(owner);
    sourceReceipt = validateTcgFieldSourceAuthorityReceipt(
  ...
  const tenantId = safeText(rows?.resolution?.tenant_id);
  const recognitionSessionId = safeText(rows?.resolution?.recognition_session_id);
  let sessionIdentitySha256;
  try {
    sessionIdentitySha256 = tcgGrammarContextSessionIdentitySha256({
      tenantId,
      recognitionSessionId
    });
  } catch { return false; }
  return ownerReceipt != null
    && owner.resolver === rows?.resolution?.resolver_version
    && owner.composer === rows?.output?.composer_version
    && owner.marketplace_profile === rows?.output?.marketplace_profile_version
    && owner.request_builder_version
      === TCG_GRAMMAR_CONTEXT_CANONICAL_REQUEST_BUILDER_VERSION
    && owner.response_parser_version
      === TCG_GRAMMAR_CONTEXT_CANONICAL_RESPONSE_PARSER_VERSION
    && /^[0-9a-f]{64}$/.test(safeText(owner.operation_payload_sha256))
    && sourceReceipt.operation_payload_sha256 === owner.operation_payload_sha256
    && sourceReceipt.provider_client_request_id_sha256
      === sha256Text(owner.provider_client_request_id)
    && sourceReceipt.provider_response_id_sha256
      === sha256Text(owner.provider_response_id)
    && sourceReceipt.session_identity_sha256 === sessionIdentitySha256;
}

function sessionTcgImageFingerprints(session) {
  const snapshot = plainRecord(session?.identity_snapshot)
    ? session.identity_snapshot : null;
  const requestSummary = plainRecord(session?.request_summary)
    ? session.request_summary : null;
  const imageReferences = Array.isArray(snapshot?.image_references)
    ? snapshot.image_references : null;
  const recognitionInput = Array.isArray(requestSummary?.recognition_input)
    ? requestSummary.recognition_input : null;
  const expectedOriginalCount = Number(snapshot?.expected_original_count);
  if (!imageReferences || !recognitionInput
      || !Number.isInteger(expectedOriginalCount)
      || expectedOriginalCount < 1 || expectedOriginalCount > 2
      || recognitionInput.length !== expectedOriginalCount) return null;

  const expectedRoles = ["front_original", "back_original"]
    .slice(0, expectedOriginalCount);
  const originalReferences = expectedRoles.map((role) => imageReferences.filter(
    (reference) => reference?.derived !== true
      && safeText(reference?.image_role) === role
  ));
  if (originalReferences.some((matches) => matches.length !== 1)
      || imageReferences.filter((reference) => reference?.derived !== true).length
        !== expectedOriginalCount) return null;

  const originalHashes = originalReferences.map(([reference]) =>
    safeText(reference.content_sha256).toLowerCase());
  if (originalHashes.some((hash) => !/^[0-9a-f]{64}$/.test(hash))) return null;

  const recognitionHashes = [];
  for (let index = 0; index < recognitionInput.length; index += 1) {
    const entry = recognitionInput[index];
    const original = originalReferences[index][0];
    const originalHash = originalHashes[index];
    const read = safeText(entry?.read);
    const explicitContentHash = safeText(entry?.content_sha256).toLowerCase();
    const explicitOriginalHash = safeText(entry?.original_content_sha256).toLowerCase();
    if (!plainRecord(entry)
        || safeText(entry.image_role) !== expectedRoles[index]
        || !["original", "readability_derived"].includes(read)
        || (explicitOriginalHash && explicitOriginalHash !== originalHash)
        || (safeText(entry.source_image_id)
          && safeText(entry.source_image_id) !== safeText(original.image_id))) return null;

    if (read === "original") {
      if (explicitContentHash && explicitContentHash !== originalHash) return null;
      recognitionHashes.push(originalHash);
      continue;
    }

    const derivedReferences = imageReferences.filter((reference) => (
      reference?.derived === true
        && safeText(reference?.image_role) === "readability_derived"
        && safeText(reference?.source_image_id) === safeText(original.image_id)
    ));
    if (derivedReferences.length > 1) return null;
    const storedDerivedHash = derivedReferences.length === 1
      ? safeText(derivedReferences[0].content_sha256).toLowerCase() : "";
    if (storedDerivedHash && !/^[0-9a-f]{64}$/.test(storedDerivedHash)) return null;
    if (explicitContentHash && !/^[0-9a-f]{64}$/.test(explicitContentHash)) return null;
    if (explicitContentHash && storedDerivedHash
        && explicitContentHash !== storedDerivedHash) return null;
    const recognitionHash = explicitContentHash || storedDerivedHash;
    if (!recognitionHash) return null;
    recognitionHashes.push(recognitionHash);
  }
  return {
    originalImageFingerprints: originalHashes.map((hash) => `sha256:${hash}`),
    recognitionImageFingerprints: recognitionHashes.map((hash) => `sha256:${hash}`)
  };
}

// This establishes internal consistency between durable session-owned facts
// and the sealed CSM packet. A row-only replay still cannot attest that the
// provider execution occurred; it may claim only that its stored bytes agree.
function validTcgGrammarContextSessionAuthority(session, rows) {
  if (!validTcgGrammarContextOwnerVersions(session?.csm_owner_versions, rows)
      || safeText(session?.tenant_id) !== safeText(rows?.resolution?.tenant_id)
      || safeText(session?.id) !== safeText(rows?.resolution?.recognition_session_id)) {
    return false;
  }
  const fingerprints = sessionTcgImageFingerprints(session);
  if (!fingerprints) return false;
  const structured = rows?.output?.structured_output || {};
  const claim = structured.tcg_grammar_context_claim_receipt;
  try {
    validateTcgFieldSourceAuthorityReceipt(
      structured.tcg_field_source_authority_receipt,
      {
        founderBetaWebReceipt: structured.founder_beta_web_receipt,
        fields: {
          set: claim?.normalized_set,
          card_number: claim?.normalized_card_number,
          grammar: structured.observed_composition_grammar
        },
        sourceExecution: {
          operationPayloadSha256:
            session.csm_owner_versions.operation_payload_sha256,
          ...fingerprints,
          providerClientRequestId:
            session.csm_owner_versions.provider_client_request_id,
          providerResponseId: session.csm_owner_versions.provider_response_id,
...
function validAtomicSessionPatch(rows, sessionPatch, hashes) {
  if (!plainRecord(sessionPatch)
      || SESSION_HASH_KEYS.some((key) => safeText(sessionPatch[key]) !== hashes[key])) {
    return false;
  }
  const outputContractVersion = safeText(rows?.output?.contract_version);
  const ownerContractVersion = ownerStageContractVersion(
    sessionPatch.csm_owner_versions
  );
  if (ownerContractVersion === "INVALID"
      || (ownerContractVersion !== null
        && outputContractVersion !== ownerContractVersion)) return false;
  if (outputContractVersion
      !== CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION) return true;
  if (sessionPatch.csm_contract_version !== rows.output.contract_version
      || sessionPatch.csm_registry_release_id !== rows.resolution.registry_release_id
      || sessionPatch.csm_grammar !== rows.resolution.grammar
      || !SESSION_STATUS_KEYS.every((key) => sessionPatch[key] === "COMPLETE")
      || !/^[0-9a-f]{64}$/.test(safeText(
        sessionPatch.recognition_pipeline_fingerprint
      ))) return false;
  return validTcgGrammarContextOwnerVersions(
    sessionPatch.csm_owner_versions, rows
  );
}
...
export async function checkCsmPersistenceReadiness({
  ...
  endpoint.searchParams.set("id", `in.(${THIN_REGISTRY_RELEASE_ID},${EXTERNAL_IDENTITY_REGISTRY_RELEASE_ID},${EXTERNAL_IDENTITY_FORWARD_REGISTRY_RELEASE_ID},${TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE.release_id})`);
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const externalForwardRelease = releases.get(EXTERNAL_IDENTITY_FORWARD_REGISTRY_RELEASE_ID)
          || null;
        const tcgGrammarContextRelease = releases.get(
          TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE.release_id
        ) || null;
        if (!release || !externalRelease || !externalForwardRelease
            || !tcgGrammarContextRelease) {
          return { ready: false, reason: "registry_release_missing" };
        }
        if (!Object.entries(THIN_REGISTRY_RELEASE_CONTRACT)
          .every(([name, value]) => release[name] === value)
          || !exactRecordContract(release.registry_payload, THIN_REGISTRY_PAYLOAD_CONTRACT)
          ...
          || !Object.entries(THIN_EXTERNAL_IDENTITY_FORWARD_REGISTRY_RELEASE_CONTRACT)
            .every(([name, value]) => externalForwardRelease[name] === value)
          || !exactRecordContract(
            externalForwardRelease.registry_payload,
            THIN_EXTERNAL_IDENTITY_FORWARD_REGISTRY_PAYLOAD_CONTRACT
          )
          || !Object.entries(THIN_TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE_CONTRACT)
            .every(([name, value]) => tcgGrammarContextRelease[name] === value)
          || !exactRecordContract(
            tcgGrammarContextRelease.registry_payload,
...
export async function writeCsmStagePacketAtomically(rows, {
  ...
  if (!validAtomicSessionPatch(rows, sessionPatch, hashes)) {
...
export async function writeCsmStageRows(rows, {
  ...
  if (rows.output.contract_version
      === CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION) {
    return writerFailure("csm_v4_requires_atomic_session_patch", {
      statusCode: 400,
      written
    });
  }
...
export async function readCsmResolutionRecord({
  ...
  sessionEndpoint.searchParams.set("select", [
    "id", "tenant_id", "asset_id", "created_at", "csm_owner_versions",
    "identity_snapshot", "request_summary",
    "csm_contract_version", "csm_registry_release_id", "csm_grammar",
  ...
  };
  {
    ...
    if ([evidence, candidates, links].some((rows) => rows.length >= 100)) {
      throw new Error("csm_replay_readback_truncated");
    }
    replayRows = {
      output: row,
      resolution,
  ...
  const storedContractVersions = [
    row.contract_version,
    resolution?.contract_version,
    session.csm_contract_version
  ];
  const v4Readback = storedContractVersions.some((version) => (
    version === CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION
  ));
  // Preserve the owner-receipt error boundary. A malformed sealed owner is
  // different from a valid owner whose stage family was cross-spliced.
  const ownerExecutionReceipt = projectCsmOwnerExecutionReceipt(
    session.csm_owner_versions
  );
  const ownerContractVersion = ownerStageContractVersion(
    session.csm_owner_versions
  );
  if (ownerContractVersion === "INVALID"
      || (ownerContractVersion !== null
        && storedContractVersions.some((version) => (
          version !== ownerContractVersion
        )))) {
    throw new Error("csm_tcg_grammar_context_readback_invalid");
  }
  if (v4Readback) {
    if (resolution?.contract_version !== row.contract_version
        || session.csm_contract_version !== row.contract_version
        || session.csm_registry_release_id !== resolution?.registry_release_id
        || session.csm_grammar !== resolution?.grammar
        || !validTcgGrammarContextSessionAuthority(session, replayRows)
        || !verifyReplay(replayRows, row.title).ok) {
      throw new Error("csm_tcg_grammar_context_readback_invalid");
    }
  }
  const tcgGrammarContextSourceExecutionVerified = v4Readback;
  ...
  return {
    ...
    ...(tcgGrammarContextSourceExecutionVerified ? {
...
export async function appendCsmResolutionReview({
  ...
  if (review?.tenant_id !== normalizedTenantId) {
    throw new Error("csm_review_tenant_mismatch");
  }
  const body = { ...review, tenant_id: normalizedTenantId };
  if (body.schema_version === CAPTURED_E1AE_RESOLUTION_REVIEW_VERSION) {
    validateCapturedE1aeResolutionReviewIntegrity(body);
  } else {
