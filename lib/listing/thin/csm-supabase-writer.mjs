...
const EXTERNAL_IDENTITY_PUBLIC_ACTIONS = new Set(["FILL", "CORROBORATE", "NORMALIZE_ALIAS"]);
...
function publicExternalIdentitySource(source, field) {
 ...
 * HTTPS sources.
 */
function projectExternalIdentityReadback({ output, resolution, registryRelease, evidenceRows }) {
  const stored = output?.structured_output?.external_identity_support;
  if (!plainRecord(stored)
      || resolution?.registry_release_id !== EXTERNAL_IDENTITY_REGISTRY_RELEASE_ID
      || registryRelease?.id !== resolution.registry_release_id) return null;

  const releasePayload = registryRelease.registry_payload;
  if (!plainRecord(releasePayload)) return null;
  const packSha256 = safeSha256(stored.pack_sha256);
  const indexSha256 = safeSha256(stored.index_sha256);
  const resolutionContractSha256 = safeSha256(releasePayload.resolution_contract_sha256);
  if (!packSha256 || !indexSha256 || !resolutionContractSha256
      || safeSha256(registryRelease.content_sha256) !== packSha256
      || safeSha256(releasePayload.pack_sha256) !== packSha256
      || safeSha256(releasePayload.index_sha256) !== indexSha256
      || safeVersionText(releasePayload.pack_id) !== safeVersionText(stored.pack_id)
      || safeVersionText(releasePayload.index_id) !== safeVersionText(stored.index_id)) return null;

  const recordId = safeVersionText(stored.record_id);
  const indexVersion = safeVersionText(stored.index_version);
  ...
  if (!recordId || !indexVersion
      || !["EXACT_FOUR_ANCHOR", "VERIFIED_ORIGINAL_SET"].includes(matchBasis)
  ...
  for (const row of Array.isArray(evidenceRows) ? evidenceRows : []) {
    ...
    if (!plainRecord(sourceRef)
        || sourceRef.support_type !== "EXACT_EXTERNAL_IDENTITY"
        || !EXTERNAL_IDENTITY_PUBLIC_FIELDS.includes(field)
        || safeVersionText(sourceRef.pack_id) !== safeVersionText(stored.pack_id)
        || safeSha256(sourceRef.pack_sha256) !== packSha256
        || safeVersionText(sourceRef.index_id) !== safeVersionText(stored.index_id)
        || safeVersionText(sourceRef.index_version) !== indexVersion
        || safeSha256(sourceRef.index_sha256) !== indexSha256
        || safeVersionText(sourceRef.record_id) !== recordId
        || safeVersionText(sourceRef.match_mode) !== matchBasis
        || (sourceRef.original_set_sha256 == null
          ? ""
          : safeSha256(sourceRef.original_set_sha256)) !== originalSetSha256) continue;
    for (const source of Array.isArray(sourceRef.sources) ? sourceRef.sources : []) {
      const projected = publicExternalIdentitySource(source, field);
  ...
  for (const field of EXTERNAL_IDENTITY_PUBLIC_FIELDS) {
    ...
    const sourceIds = [...new Set((Array.isArray(decision.source_ids) ? decision.source_ids : [])
      ...
      .filter((sourceId) => allowedSourceIds.has(sourceId)))].sort();
  ...
  return {
    ...
    pack: {
      id: safeVersionText(stored.pack_id),
      version: safeVersionText(releasePayload.pack_version),
      sha256: packSha256
    },
    index: {
      id: safeVersionText(stored.index_id),
...
export async function readCsmResolutionRecord({
  ...
  sessionEndpoint.searchParams.set("select", "id,asset_id,created_at,csm_owner_versions");
  ...
  // Baseline runs stop here. Only a row that was durably composed from an
  // APPLIED external identity receipt pays the two provenance reads below.
  // The Registry evidence table is the source of truth for URLs; the output's
  // structured summary deliberately does not carry them.
  let externalIdentitySupport = null;
  const storedExternal = row.structured_output?.external_identity_support;
  if (plainRecord(storedExternal)
      && resolution?.registry_release_id === EXTERNAL_IDENTITY_REGISTRY_RELEASE_ID) {
    ...
    // `source_ref` is the only JSONB column read. In particular, raw and
    // normalized card values never enter this product-visibility path.
    evidenceEndpoint.searchParams.set("select", "source_ref");
    ...
    evidenceEndpoint.searchParams.set("modality", "eq.REGISTRY");
    evidenceEndpoint.searchParams.set("limit", "100");

    try {
      const [registryRows, evidenceRows] = await Promise.all([
        readRows(registryEndpoint, "csm_external_identity_registry_read"),
        readRows(evidenceEndpoint, "csm_external_identity_evidence_read")
      ]);
