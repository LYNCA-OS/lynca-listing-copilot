...
export function buildLunaDirectPayloadHash(input) {
  ...
  return createHash("sha256")
    .update(JSON.stringify([
      ...
      ["image_urls", Array.from(task.image_urls || [], (value) => String(value))]
    ]))
    .digest("hex");
}
...
export function classifyLunaDirectFailure(error) {
  ...
  if (status !== null) {
    ...
    return {
      ...
      ambiguous: [502, 503, 504].includes(status) && !explicitlySafe,
      kind: "http",
      status
    };
  }
...
export function createLunaDirectDispatcher({
  ...
  async function lookupAmbiguous(record, error) {
    ...
    try {
      ...
      throw lookupUnavailable(lookupError);
    }
    ...
    throw lookupUnavailable(new Error("ambiguous_result_lookup_invalid"));
  }
  ...
  function enqueue(input, { manualRetry = false } = {}) {
    ...
    const record = {
      ...
      operationKey,
      state: "queued",
      attempts: manualRetry ? Number(existing?.attempts || 0) : 0,
      result: undefined,
      error: null,
      manualRetry,
