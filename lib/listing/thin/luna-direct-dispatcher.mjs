  ...
  if (task.operation_scope === "derived_checkpoint") {
    return createHash("sha256")
      .update(JSON.stringify([
        ...
        ["checkpoint_payload_contract", "derived-checkpoint-stable-v1"]
      ]))
      .digest("hex");
  }
...
// added to payload identity. Production history has exactly one such effort:
// low. Refuse every other effort so a future none/max deployment cannot adopt
// an old result merely because its user-operation key is the same.
export function buildLegacyLowLunaDirectPayloadHash(input) {
  const task = normalizeTask(input);
  if (task.reasoning_effort !== "low") {
    throw new LunaDirectDispatchError(
      "LUNA_DIRECT_LEGACY_PAYLOAD_INELIGIBLE",
      "legacy_payload_recovery_requires_low_effort"
    );
  }
  const stableImages = Array.from(task.image_fingerprints || [], (value) => String(value));
  const imageIdentity = stableImages.length > 0
...
export function classifyLunaDirectFailure(error) {
  ...
  if (status !== null) {
    ...
    return {
      ...
      ambiguous: [502, 503, 504].includes(status) && !explicitlySafe && !explicitlyDefinitive,
      kind: "http",
      status
    };
  }
...
export function createLunaDirectDispatcher({
  ...
  async function attempt(record) {
    try {
      ...
      if (failure.retryable && record.attempts < maxAttempts) {
