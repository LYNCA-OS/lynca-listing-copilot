...
import { buildFounderBetaWebReceipt } from "./csm-forward-reader-bridge.mjs";
import {
  CARD_NAME_PREDICATE,
...
export const CSM_OPENAI_RESPONSES_ADAPTER = Object.freeze({
  ...
  parseResponse(body = {}, { request = null } = {}) {
    ...
    const rawOutput = audited ? JSON.stringify(audited.payload) : "";
    const relationReceipt = audited ? {
      schema_version: SET_CARD_NAME_RELATION_CONTRACT_VERSION,
      set: audited.payload.set ? {
        predicate: audited.set_card_name_relations?.set,
        value: audited.payload.set
      } : null,
      card_name: audited.payload.card_name ? {
        predicate: audited.set_card_name_relations?.card_name,
        value: audited.payload.card_name
      } : null
    } : null;
    if (relationReceipt) validateSetCardNameRelationReceipt(relationReceipt, audited.payload);
    return {
      ...
      receipt: {
        ...
        ...(explicitIncomplete ? {} : {
          ...
          founder_beta_web_receipt: buildFounderBetaWebReceipt(body, {
            rawOutput,
            request,
