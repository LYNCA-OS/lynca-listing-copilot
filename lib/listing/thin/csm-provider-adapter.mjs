...
export const CSM_CANONICAL_REQUEST_BUILDER_VERSION = "canonical-fields-request-v1";
...
function compileOpenAiCanonicalRequest({
  samplingParameters = "omit",
  ...
  const extensionKeys = Object.keys(extensions).sort();
  if (extensionKeys.join(",") !== "prompt_cache,reasoning_context,text_verbosity"
      || Object.values(extensions).some((value) => value !== null)) {
    throw new TypeError("openai_request_extension_unreleased");
  }
