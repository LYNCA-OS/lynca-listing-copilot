...
export const TCG_GRAMMAR_CONTEXT_CANONICAL_RESPONSE_PARSER_VERSION =
  "canonical-output-v6-source-authority";
export const TCG_GRAMMAR_CONTEXT_CANONICAL_REQUEST_BUILDER_VERSION =
...
export const CSM_OPENAI_RESPONSES_TCG_GRAMMAR_CONTEXT_ADAPTER_CONTRACT = deepFreeze({
  ...CSM_OPENAI_RESPONSES_ADAPTER_CONTRACT,
...
// Append-only v6 parser. The v5 parser above remains the immutable stage-v3
// behavior: it strips the model-authored source ledger after the Founder audit
// and emits only the existing Web/relation receipts. V6 consumes the same
// audited ledger exactly once to seal the two private Grammar-context receipts;
// neither the ledger nor any source id crosses the adapter boundary.
function parseTcgGrammarContextResponse(body = {}, {
  request = null,
  ...
  if (relationReceipt) validateSetCardNameRelationReceipt(relationReceipt, canonicalPayload);
  const fieldSourceAuthorityReceipt = canonicalPayload
    ? buildTcgFieldSourceAuthorityReceipt({
        fieldSources: audited.field_sources,
        fields: canonicalPayload,
        originalImageCount: imageCount,
        semanticStateSha256: authorityAudit.receipt.semantic_state_sha256,
        founderBetaWebReceipt: authorityAudit.receipt,
        sourceExecution: {
    ...
    : null;
  const grammarContextClaimReceipt = canonicalPayload
    ? buildTcgGrammarContextClaimReceipt({
        fields: canonicalPayload,
  ...
  return {
    ...
    receipt: {
      ...
      ...(explicitIncomplete ? {} : {
        set_card_name_relation_receipt: relationReceipt,
        founder_beta_web_receipt: authorityAudit.receipt,
        tcg_field_source_authority_receipt: fieldSourceAuthorityReceipt,
        tcg_grammar_context_claim_receipt: grammarContextClaimReceipt
      })
    }
  };
}
...
/** Dormant v4 adapter; the active/captured and v5 writers do not select it. */
export const CSM_OPENAI_RESPONSES_TCG_GRAMMAR_CONTEXT_ADAPTER =
  openAiResponsesAdapter({
    contract: CSM_OPENAI_RESPONSES_TCG_GRAMMAR_CONTEXT_ADAPTER_CONTRACT,
    requestDefaults: {
      prompt: CANONICAL_FIELDS_PROMPT,
      schema: CANONICAL_FIELDS_SCHEMA,
      webSearchToolsEnabled: true
    },
    parseResponse: parseTcgGrammarContextResponse
  });
...
const providerAdapters = Object.freeze({
  openai: Object.freeze({
    ...
    [TCG_GRAMMAR_CONTEXT_CANONICAL_REQUEST_BUILDER_VERSION]:
      CSM_OPENAI_RESPONSES_TCG_GRAMMAR_CONTEXT_ADAPTER
  })
});
