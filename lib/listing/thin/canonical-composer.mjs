...
const THIN_FIELDS = new Set([
  "lot", "ip", "language", "year", "manufacturer", "manufacturer_product", "product", "set",
 ...
 */
function productHierarchyCandidates(value) {
  ...
  const displayFull = stripCategoryFiller(full, { semanticRole: "product" }).title;
...
// When both repeated Manufacturer and Product would yield under the budget,
// keep a single observed leaf from Product. This is a serialization recovery:
// the canonical value is unchanged and no token is invented.
...
function renderBracket(name, fields) {
  switch (name) {
    ...
    case "manufacturer_product":
      return [
        fields.manufacturer,
        stripCategoryFiller(fields.product, { semanticRole: "product" }).title
      ].filter(Boolean).join(" ");
    case "subject":
...
function assemble(fields, order, omitted) {
  ...
  for (const name of order) {
    ...
    if (name !== "lot" && name !== "manufacturer_product") {
      text = stripCategoryFiller(text, { semanticRole: name }).title;
    }
    if (!text) continue;
...
export function composeFromCanonicalFields(fields, {
  ...
  limit,
  productLeafRecovery = true
} = {}) {
  if (productLeafRecovery) {
    const baseline = composeFromCanonicalFields(fields, {
      profile, limit, productLeafRecovery: false
    });
    const leaf = singleLeafAfterManufacturer(fields?.product, fields?.manufacturer);
    if (baseline.dropped.includes("manufacturer")
      && baseline.dropped.includes("product")
      && leaf) {
      const recovered = composeFromCanonicalFields({ ...fields, product: leaf }, {
        profile, limit, productLeafRecovery: false
      });
      if (recovered.length <= (limit ?? profile.characterBudget)
        && !recovered.dropped.includes("product")) {
        return {
          ...recovered,
          normalization_reasons: [
            ...recovered.normalization_reasons,
  ...
  const manufacturerCategory = stripCategoryFiller(fields.manufacturer, { semanticRole: "manufacturer" });
  const productCategory = stripCategoryFiller(fields.product, { semanticRole: "product" });
  ...
  const normalizedSerial = normalizeSlashSpacing(fields.serial);
  const normalizedCardNumber = normalizeSlashSpacing(fields.card_number);
  ...
  working.components = compactObservableComponents(working, normalizationReasons);
  ...
  for (const name of DROP_ORDER[grammar]) {
    ...
    if (name === "manufacturer" && !omitted.has("product")) {
      const originalProduct = working.product;
      const candidates = productHierarchyCandidates(originalProduct).slice(1);
      let compacted = null;
