...
import {
  ...
  canonicalLotCountText
} from "./lot-terminal-contract.mjs";
import {
  PUBLICATION_BRACKET_SOURCE_FIELDS,
  PUBLICATION_DISPOSITION,
  createPublicationCoverage,
  rebasePublicationCoverage
...
// docs/csm/decision-proposals/verified-external-identity-marketplace-projection-v1.md
// Codex's useful naming answer is a marketplace projection, not a different
// semantic object: Card Number remains Card Number and Subject remains Subject.
// Once an exact external identity receipt has admitted both values, this
// detachable profile places the checklist identifier beside the release
// identity, before Subject, and lets Team close the title.  The baseline CSM
// order remains untouched for every unverified card.
export const VERIFIED_EXTERNAL_IDENTITY_STANDARD_ORDER = Object.freeze((() => {
  const withoutCardNumber = BRACKET_ORDER.standard.filter((name) => name !== "card_number");
  const subjectAt = withoutCardNumber.indexOf("subject");
  return Object.freeze([
    ...withoutCardNumber.slice(0, subjectAt),
    "card_number",
    ...withoutCardNumber.slice(subjectAt)
  ]);
})());
...
// Historical v2 external-identity replay owns a complete literal budget order.
// Do not derive this from DROP_ORDER.standard: changing the baseline profile in
// a later release must not rewrite an already-persisted v2 title.
export const VERIFIED_EXTERNAL_IDENTITY_DROP_ORDER_V2 = Object.freeze([
...
// Text lanes do not have legitimate unpadded fractions. Unlike Serial/Card
// Number/Grade, `2023/2024` and `Topps/Panini` are explicit competing Lot
// values and must not be flattened into a shared claim.
const LOT_TEXT_PER_CARD_SEPARATOR = /;\s*|\s*\|\s*|\//;

function exactColorTokenInsideParallel(fields = {}) {
  const color = String(fields.surface_color ?? "").replace(/\s+/g, " ").trim();
  const exact = String(fields.parallel_exact ?? "").replace(/\s+/g, " ").trim();
  if (!color || !exact) return null;
  const escaped = color.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return new RegExp(`(?:^|[^a-z0-9])${escaped}(?:$|[^a-z0-9])`, "i").test(exact)
    ? color
    : null;
}

function addedDrops(before = [], after = []) {
  const prior = new Set(before);
  return after.filter((name) => !prior.has(name));
}
...
function renderGradingInfo(info) {
  ...
  const cardGrade = String(info.card_grade || "").trim();
  const autoGrade = String(info.auto_grade || "").trim();
  if (info.grade_type === "AUTO_ONLY") {
    return [company, "Auto", autoGrade || cardGrade].filter(Boolean).join(" ");
  }
  if (info.grade_type === "AUTHENTIC_WITH_AUTO") {
    return `${[company, "Authentic"].filter(Boolean).join(" ")}${autoGrade ? `/${autoGrade}` : ""}`;
  }
  if (cardGrade && autoGrade) {
    return `${[company, cardGrade].filter(Boolean).join(" ")}/${autoGrade}`;
  }
...
function renderBracket(name, fields, {
  ...
  switch (name) {
    ...
    case "lot": {
      if (fields.__captured_composer_v1_lot_marker) {
        const count = String(fields.lot_count ?? "").trim();
        return /^[1-9]\d*$/.test(count) ? `Lotx${count}` : "";
      }
      if (fields.__durable_lot_terminal_shared_only) {
        const count = canonicalLotCountText(fields.lot_count);
        return count && Number(count) >= 2 ? `Lot*${count}` : "";
      }
      const count = String(fields.lot_count ?? "").trim();
    ...
    case "grading_info":
      return fields.grade || (fields.__publication_coverage
        ? renderGradingInfo(fields.grading_info) : "");
    case "card_number":
...
function assemble(fields, order, omitted, {
  ...
  for (const name of order) {
    ...
    if (name !== "subject" && name !== "grading_info") {
      const beforeDedupe = text;
      text = dropRepeatedPrefix(text, seen);
      if (beforeDedupe && !text) dedupedBrackets?.add(name);
    }
...
const coverageValues = (value, sourceField) => (Array.isArray(value) ? value : [value])
  .map((entry) => sourceField === "grading_info" ? renderGradingInfo(entry)
    : String(entry ?? "").replace(/\s+/g, " ").trim())
  .filter(Boolean);

function buildV2PublicationCoverage(fields, {
  order, brackets, dropped, suppressed, restored, truncated, title,
  trimmedSubjects, lotUnsharedAttributes, normalizationReasons,
  partiallySuppressed, dedupedBrackets
}) {
  const emitted = new Set(brackets.map((entry) => entry.bracket));
  const droppedSet = new Set(dropped);
  const suppressedSet = new Set(suppressed);
  const unsharedSet = new Set(lotUnsharedAttributes);
  const dedupedSet = new Set(dedupedBrackets);
  const componentDuplicates = new Set(normalizationReasons
    .filter((reason) => reason.startsWith("observable_components:") && reason.endsWith("_duplicate"))
    .map((reason) => reason.slice("observable_components:".length, -"_duplicate".length)));

  // The backstop cuts a prefix of the assembled title. Freeze which brackets
  // were wholly emitted before the cut; readers must not trust stale
  // bracket_text for the suffix.
  const fullyEmitted = new Set();
  let cursor = 0;
  for (const entry of brackets) {
    const start = cursor + (cursor ? 1 : 0);
    const end = start + entry.text.length;
    if (!truncated || end <= title.length) fullyEmitted.add(entry.bracket);
    cursor = end;
  }

  const atoms = [];
  for (const bracket of order) {
    const sourceFields = PUBLICATION_BRACKET_SOURCE_FIELDS[bracket] || [];
    const effectiveSourceFields = bracket === "grading_info"
      ? [fields.grading_info ? "grading_info" : "grade"] : sourceFields;
    for (const sourceField of effectiveSourceFields) {
      const values = coverageValues(fields[sourceField], sourceField);
      for (let sourceIndex = 0; sourceIndex < values.length; sourceIndex += 1) {
        const canonicalValue = values[sourceIndex];
        let disposition;
        if (unsharedSet.has(sourceField)) {
          disposition = PUBLICATION_DISPOSITION.WITHHELD_BY_CONTRACT;
        } else if (bracket === "subject" && trimmedSubjects && sourceIndex > 0) {
          disposition = PUBLICATION_DISPOSITION.DROPPED_FOR_BUDGET;
        } else if (bracket === "search_optimization" && suppressedSet.has(bracket)) {
          const retained = partiallySuppressed.get(bracket) || [];
          const componentKey = normalizedComponentToken(canonicalValue);
          if (sourceField === "components" && componentDuplicates.has(componentKey)) {
            disposition = PUBLICATION_DISPOSITION.DEDUPED_COVERED;
          } else if (sourceField === "components"
              && retained.includes(componentKey) && fullyEmitted.has(bracket)) {
            disposition = PUBLICATION_DISPOSITION.PUBLISHED;
          } else {
            disposition = PUBLICATION_DISPOSITION.SUPPRESSED_BY_PROFILE;
          }
        } else if (suppressedSet.has(bracket)) {
          disposition = PUBLICATION_DISPOSITION.SUPPRESSED_BY_PROFILE;
        } else if (droppedSet.has(bracket)) {
          disposition = PUBLICATION_DISPOSITION.DROPPED_FOR_BUDGET;
        } else if (truncated && emitted.has(bracket) && !fullyEmitted.has(bracket)) {
          disposition = PUBLICATION_DISPOSITION.TRUNCATED_LOSS;
        } else if (fullyEmitted.has(bracket)) {
          disposition = PUBLICATION_DISPOSITION.PUBLISHED;
        } else if (dedupedSet.has(bracket)
            || (bracket === "product" && fields.manufacturer
              && productsSemanticallyEquivalent(fields.manufacturer, fields.product))
            || (bracket === "manufacturer" && fields.ip
              && foldForRepetition(fields.manufacturer).includes(foldForRepetition(fields.ip)))) {
          disposition = PUBLICATION_DISPOSITION.DEDUPED_COVERED;
        } else {
          disposition = PUBLICATION_DISPOSITION.WITHHELD_BY_CONTRACT;
        }
        atoms.push({ bracket, source_field: sourceField, source_index: sourceIndex,
          canonical_value: canonicalValue, disposition });
      }
    }
  }
  return createPublicationCoverage(atoms);
}
...
export function composeFromCanonicalFields(fields, {
  ...
  const verifiedExternalIdentity = features.verified_external_identity_title === true;
  const verifiedExternalIdentityPriorityV2 = verifiedExternalIdentity
    && features.verified_external_identity_priority_v2 === true;
  const verifiedTitleRenderSource = verifiedExternalIdentityPriorityV2
    ? "csm_marketplace_composer_v4_verified_external_identity"
    : "csm_marketplace_composer_v3_verified_external_identity";
  // A full literal parallel can lose the entire Print Finish bracket under the
  // 80-character COS-8 budget even when its colour alone would fit. The colour
  // is not guessed here: it is eligible only when it is already a whole token
  // inside `parallel_exact`. First render the unmodified baseline, then accept
  // the display-only compaction only when it restores Print Finish without
  // displacing any other bracket or invoking fallback truncation.
  //
  // Canonical fields remain untouched. The compact value exists only in the
  // nested Composer working copy, and the feature switch keeps an exact
  // runtime baseline available to paired replays.
  if (features.exact_parallel_color_compaction !== false) {
    const baseline = composeFromCanonicalFields(fields, {
      profile,
      limit,
      features: { ...features, exact_parallel_color_compaction: false }
    });
    const color = exactColorTokenInsideParallel(fields);
    const asV2 = (result) => ({
      ...result,
      ...(result.publication_coverage ? {
        publication_coverage: rebasePublicationCoverage(
          result.publication_coverage,
          fields,
          BRACKET_ORDER[result.grammar] || BRACKET_ORDER.standard
        )
      } : {}),
      title_render_source: verifiedExternalIdentity
        ? verifiedTitleRenderSource
        : "csm_marketplace_composer_v2"
    });
    if (!color || !baseline.dropped.includes("print_finish")) return asV2(baseline);

    const candidate = composeFromCanonicalFields({
      ...fields,
      parallel_exact: color,
      print_finish: color
    }, {
      profile,
      limit,
    ...
    if (!candidate.brackets.includes("print_finish")
      || candidate.dropped.includes("print_finish")
      || addedDrops(baseline.dropped, candidate.dropped).length
      || candidate.truncated
      || candidate.length > budget) return asV2(baseline);

    return asV2({
      ...candidate,
      normalization_reasons: [...new Set([
        ...candidate.normalization_reasons,
        "print_finish:exact_parallel_color_compacted"
      ])]
    });
  }
  ...
  if (features.product_leaf_recovery !== false) {
    ...
    if (withoutLeafRecovery.dropped.includes("manufacturer")
      ...
      if (withLeaf.length <= (limit ?? profile.characterBudget)
        ...
        return {
          ...withLeaf,
          ...(withLeaf.publication_coverage ? {
            publication_coverage: rebasePublicationCoverage(
              withLeaf.publication_coverage,
              fields,
              BRACKET_ORDER[withLeaf.grammar] || BRACKET_ORDER.standard
            )
          } : {}),
  ...
  const order = verifiedExternalIdentity && grammar === "standard"
    ? VERIFIED_EXTERNAL_IDENTITY_STANDARD_ORDER
    : BRACKET_ORDER[grammar];
  const dropOrder = verifiedExternalIdentityPriorityV2 && grammar === "standard"
  ...
  const useDurableLotSharedOnly = features.durable_lot_terminal_shared_only === true;
  const usePublicationCoverage = features.publication_coverage === true;
  const useCapturedComposerV1LotMarker =
    features.captured_composer_v1_lot_marker === true;
  ...
  const working = {
    ...
    __captured_composer_v1_lot_marker: useCapturedComposerV1LotMarker,
    __durable_lot_terminal_shared_only: useDurableLotSharedOnly,
    __publication_coverage: usePublicationCoverage
  };
  ...
  // by every card. Production 2026-08-08 published six lots that broke it.
  //
  //   set:          "Stats Autograph; Jersey Numbers Auto"
  //   grading_info: card_grade "Authentic; 9"
  //   team:         "Bulls; Celtics"
  //
  // A scalar carrying a separator between two cards' answers is, by
  ...
  if (working.grammar === "lot") {
    const sharedOnlyFields = useDurableLotSharedOnly
      ? LOT_UNSHARED_ATTRIBUTE_FIELDS.filter((name) => (
        name !== "serial" && name !== "components"
      ))
      // Frozen Composer v2/eBay behaviour for rows written before the durable
      // terminal receipt became the feature marker. Replay may not silently
      // upgrade those stored titles to today's stronger shared-only contract.
      : ["set", "card_name", "print_finish", "parallel_exact",
        "parallel_family", "surface_color", "descriptive_rarity", "release_variant",
        "card_number", "grade", "team", "product"];
    for (const field of sharedOnlyFields) {
      const separator = useDurableLotSharedOnly
        && !["card_number", "grade"].includes(field)
        ? LOT_TEXT_PER_CARD_SEPARATOR
        : PER_CARD_SEPARATOR;
      if (separator.test(String(working[field] ?? ""))) {
        lotUnsharedAttributes.push(field);
        working[field] = "";
      }
    }
    // `components: ["Auto", "Patch"]` can mean that every card has both, or
    // that different cards contributed one each. The aggregate array carries
    // no per-card/shared proof, and the historical replay explicitly rejected
    // promoting it for that reason. Fail closed at serialization: keep the
    // upstream observation, but publish no Lot-wide component assertion.
    if (useDurableLotSharedOnly
        && Array.isArray(fields.components) && fields.components.length) {
      lotUnsharedAttributes.push("components");
      working.components = [];
      working.attributes = [];
    }
  ...
  if (verifiedExternalIdentity && grammar === "standard") {
    normalizationReasons.push("marketplace:verified_external_identity_projection");
  }
  ...
  let dedupedBrackets = new Set();
  let brackets = assemble(working, order, omitted, {
    preserveTypedIdentity, partiallySuppressed, dedupedBrackets
  });
  const render = () => {
    dedupedBrackets = new Set();
  ...
  const restored = [];
  for (const name of [...dropOrder].reverse()) {
  ...
  const dropped = [
    ...dropOrder.filter((name) => omitted.has(name) && !suppressed.includes(name)),
  ...
  const publicationCoverage = usePublicationCoverage
    ? buildV2PublicationCoverage(fields, {
      order, brackets, dropped, suppressed, restored, truncated, title,
      trimmedSubjects, lotUnsharedAttributes, normalizationReasons,
      partiallySuppressed, dedupedBrackets
    })
  ...
  return {
    title,
    // Explicitly disabling the v2 compaction is the executable v1 replay
    // contract. Normal calls are retagged by the outer v2 wrapper above.
    title_render_source: verifiedExternalIdentity
      ? verifiedTitleRenderSource
      : features.exact_parallel_color_compaction === false
        ? "csm_marketplace_composer_v1"
        : "csm_marketplace_composer_v2",
    ...
    lot_quantity_unresolved: working.grammar === "lot"
      && (useDurableLotSharedOnly
        ? !canonicalLotCountText(working.lot_count)
    ...
    lot_single_card: working.grammar === "lot"
      && (useDurableLotSharedOnly
        ? canonicalLotCountText(working.lot_count) === "1"
    ...
    lot_unshared_attributes: useDurableLotSharedOnly
      ? [...new Set(lotUnsharedAttributes)].sort()
      : [...new Set(lotUnsharedAttributes)],
    ...(publicationCoverage ? { publication_coverage: publicationCoverage } : {}),
