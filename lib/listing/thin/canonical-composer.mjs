...
function buildV2PublicationCoverage(fields, {
  order, brackets, dropped, suppressed, truncated, title,
    ...
    for (const sourceField of sources) {
      ...
      for (let sourceIndex = 0; sourceIndex < values.length; sourceIndex += 1) {
        ...
        if (unsharedSet.has(sourceField)) {
          ...
          const key = normalizedComponentToken(canonicalValue);
          if (sourceField === "components" && componentDuplicates.has(key)) {
            disposition = PUBLICATION_DISPOSITION.DEDUPED_COVERED;
          } else if (sourceField === "components" && retained.includes(key)
              && fullyEmitted.has(bracket)) {
            disposition = PUBLICATION_DISPOSITION.PUBLISHED;
          } else disposition = PUBLICATION_DISPOSITION.SUPPRESSED_BY_PROFILE;
...
export function composeFromCanonicalFields(fields, {
  ...
  if (working.grammar === "lot") {
    const sharedOnlyFields = useDurableLotSharedOnly
      ? LOT_UNSHARED_ATTRIBUTE_FIELDS.filter((name) => name !== "serial" && name !== "components")
    ...
    for (const field of sharedOnlyFields) {
      const separator = useDurableLotSharedOnly && !["card_number", "grade"].includes(field)
        ? LOT_TEXT_PER_CARD_SEPARATOR : PER_CARD_SEPARATOR;
      if (separator.test(String(working[field] ?? ""))) {
        lotUnsharedAttributes.push(field);
        working[field] = "";
      }
    }
    if (useDurableLotSharedOnly && Array.isArray(fields.components) && fields.components.length) {
