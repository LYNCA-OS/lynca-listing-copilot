...
export const BRACKET_ORDER = Object.freeze({
  ...
  tcg: Object.freeze(insertAfter(
    [...new Set(semTcgTitleOrder.map((name) => semCanonicalBracket("tcg", name)))].filter((name) => THIN_FIELDS.has(name)),
    "numerical_rarity", "observable_components"
  )),
...
function renderBracket(name, fields, { preserveTypedIdentity = true } = {}) {
  switch (name) {
    // "Lot*n" when the count was read, a bare "Lot" when it was not. The form
    // is the founder's, and it matches what the comment above this line always
    // claimed -- the implementation had been rendering "n Card Lot" instead,
    // which no writer in the confirmed library writes. Their own spellings are
    // `lot`, `lotx1`, `lotx4`, always at the end; the position stays where
    // CSM's lot grammar puts it, since `lot_quantity` is its opening bracket.
    //
    ...
    case "lot":
      return fields.lot_count ? `Lot*${fields.lot_count}` : "Lot";
...
function assemble(fields, order, omitted, { preserveTypedIdentity = true } = {}) {
  const seen = new Set();
  const brackets = [];
  for (const name of order) {
    if (omitted.has(name)) continue;
    let text = renderBracket(name, fields, { preserveTypedIdentity });
...
export function composeFromCanonicalFields(fields, {
  ...
  let brackets = assemble(working, order, omitted, { preserveTypedIdentity });
  const render = () => {
    brackets = assemble(working, order, omitted, { preserveTypedIdentity });
