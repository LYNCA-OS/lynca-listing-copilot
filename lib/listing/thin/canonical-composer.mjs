...
function renderBracket(name, fields, {
  ...
  switch (name) {
    ...
    //
    // Two, not one. `Lotx1` is a lot of a single card, which contradicts both
    ...
    case "lot": {
      ...
      return /^[1-9]\d*$/.test(count) && Number(count) >= 2 ? `Lotx${count}` : "";
    }
    ...
    case "manufacturer_product_set": {
      ...
      // approved example, which reads "Lotx3 2024 Topps Chrome Update ...".
