...
//   1. affiliation observations are allowed only when they are not known
//      teams, graders, or rights/union marks;
...
const NON_SET_AFFILIATION = /\b(?:nflpa|beckett|(?:golden state|new york|los angeles)\s+(?:warriors|knicks|lakers)|(?:warriors|knicks|lakers|76ers|celtics|bulls|spurs|mavericks|heat|suns|magic|thunder|nuggets|clippers|nets|pacers|raptors|bucks|rockets|cavaliers|timberwolves|pistons|kings|dolphins|cowboys|patriots|bills|raiders|packers|steelers|eagles|broncos|49ers|rams|vikings|lions|texans|titans|colts|chargers|saints|commanders|panthers|ravens|bears|jaguars|seahawks|falcons|jets|bengals|browns|giants|dodgers|yankees|mets|padres|phillies|cubs|orioles|reds|pirates|mariners|royals|tigers|rangers|angels|astros|braves))\b/i;
...
function candidateSetValue(facts, fields) {
  const ranked = facts
    ...
    .filter((fact) => !NON_SET_AFFILIATION.test(fact.value))
