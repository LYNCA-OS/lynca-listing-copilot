...
import {
  capturedE1aeFamilyClaiming,
  capturedE1aeFinishRecognitionForProduct,
  capturedE1aeProductFamilyFor
} from "./captured-production-e1ae-finish-taxonomy.mjs";
 ...
 *
 * `parallel_exact` is preserved unless a governed product claim proves the
 * printed term belongs to a foreign product family. Printed names otherwise
 * remain the strongest rung: they measured 0.750 token precision against
 * 0.313 for colour+family.
 *
...
export function admitFinishVocabulary(fields = {}, {
  taxonomyConfirmsColour = () => false,
  taxonomyProfile = "CURRENT"
} = {}) {
  if (!["CURRENT", "CAPTURED_E1AE_V1"].includes(taxonomyProfile)) {
    throw new TypeError("finish_taxonomy_profile_unsupported");
  }
  const capturedE1ae = taxonomyProfile === "CAPTURED_E1AE_V1";
  const recognize = capturedE1ae
    ? capturedE1aeFinishRecognitionForProduct : finishRecognitionForProduct;
  const claim = capturedE1ae ? capturedE1aeFamilyClaiming : familyClaiming;
  const productFamily = capturedE1ae ? capturedE1aeProductFamilyFor : productFamilyFor;
  ...
  const foreign = ["parallel_exact", "parallel_family"]
    .filter((layer) => recognize(fields[layer], fields) === "FOREIGN");
  if (foreign.length) {
    const denied = [];
    ...
    for (const layer of foreign) {
      denied.push({
        ...
        product_family: productFamily(fields),
        claimed_by: claim(cleaned[layer])
      });
      cleaned[layer] = "";
    }
    ...
    if (!capturedE1ae) cleaned.print_finish = derivedPrintFinish(cleaned);
