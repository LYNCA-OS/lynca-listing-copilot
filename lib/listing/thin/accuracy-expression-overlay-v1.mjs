...
function safeIdentityFacts(facts = []) {
  return (Array.isArray(facts) ? facts : []).filter((fact) => (
    fact?.source_kind === "printed_text" && fact?.source_confidence === "high"
  ) || (
    fact?.basis === "exact_text" && fact?.uncertainty === "none"
  ));
}

function replayAttestedInsert(fields, observations = []) {
  const next = clone(fields);
  if (String(next.card_name ?? "").trim()) return { fields: next, changed: false };
  const candidate = (Array.isArray(observations) ? observations : [])
    ...
    .map((observation) => ({
      ...observation,
      value: String(observation.evidence ?? "").replace(/\s+/g, " ").trim(),
      entry: resolveKnowledgeEntry(observation.evidence)
    }))
    .find((observation) => observation.entry && observation.value.length <= 48);
  if (!candidate) return { fields: next, changed: false };
  next.card_name = candidate.value;
  return {
    fields: next,
    changed: true,
    details: [{ field: "card_name", value: candidate.value, source: candidate }]
  };
}
...
export function applyAccuracyExpressionOverlayV1(canonicalFields = {}, {
  ...
  // A generic logo observation is not a Set candidate. The capture lane must
  // carry the source modality and confidence explicitly; medium/low or
  // sentence-like logo spans remain evidence-only.
  const identity = replayCandidateIdentityV3(fields, safeIdentityFacts(candidateFacts));
  fields = identity.fields;
  if (identity.changes.length) changes.push({ mechanism: "candidate_identity_v3", details: identity.changes });

  for (const mechanism of mechanisms) {
    const result = mechanism === "attested_insert"
      ? replayAttestedInsert(fields, observations)
      : applyAccuracyMechanismV2(mechanism, fields, {
        freeFields: expressionFields,
        freeTitle: expressionTitle,
        observations
      });
    if (result.blocked) rejected.push({ mechanism, reason: result.blocked });
    if (result.changed) changes.push({ mechanism, details: result.details || [] });
