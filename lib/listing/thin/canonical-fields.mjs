...
export const CANONICAL_FIELD_NAMES = Object.freeze([
  "year", "manufacturer", "product", "set", "subjects", "team", "card_name",
  "release_variant", "print_finish", "descriptive_rarity", "card_number",
  "serial", "attributes", "grade"
]);
...
export const CANONICAL_FIELDS_SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    print_finish: { type: "string", description: "Parallel and finish as printed, colour included: \"Gold Refractor\", \"Purple Shock Prizm\", \"Orange Sapphire\", \"Mojo\". Report it whenever the card visibly has one." },
    ...
    serial: {
      ...
      description: "Limited print run as stamped, WITH the numerator: \"17/50\", \"2/3\", \"1/1\". Empty if the card carries no stamped print run. This is NOT the checklist code."
    },
    attributes: {
      type: "array",
...
export const CANONICAL_FIELDS_PROMPT = [
  ...
  "Three fields are easy to confuse and must not be merged: `card_name` is the printed card-title segment (Rated Rookie, Next Stop Signatures); `release_variant` is a layout difference only (Horizontal, Variation); `print_finish` is the parallel and its colour (Gold Refractor, Purple Shock Prizm). Most cards have a print_finish and no release_variant.",
  "The stamped print run (17/50) and the checklist code (#221) are different things and must never be put in each other's field."
].join(" ");

export function buildCanonicalFieldsRequest({ imageUrls = [], model, effort = "none", maxOutputTokens = 4096 }) {
  return {
    model,
...
const EMPTY_FIELDS = Object.freeze({
  ...
  lot_count: "", unreadable: [], low_confidence: [], ip: "", components: []
});
...
export function parseCanonicalFields(raw) {
  ...
  for (const key of ["year", "manufacturer", "product", "set", "team", "card_name",
    "release_variant", "print_finish", "descriptive_rarity", "card_number", "serial", "grade"]) {
  ...
  fields.components = fields.attributes.filter((value) => !DESCRIPTIVE_RARITY.has(value));
  ...
  for (const key of ["manufacturer", "product", "card_name", "print_finish"]) {
