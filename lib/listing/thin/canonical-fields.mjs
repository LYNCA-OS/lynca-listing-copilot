...
export const CANONICAL_FIELDS_PROMPT_VERSION = "csm-canonical-fields-web-v1";
...
export const CANONICAL_FIELDS_PROMPT = [
  ...
  "Use the built-in Web Search only when the images do not settle the collectible identity. Web evidence may resolve year, manufacturer, product, set, card_name or subject, but it must never overwrite visible current-card copy such as card number, serial, grading, finish, surface, parallel or stamp. Keep a visible conflict unresolved instead of choosing the Web answer.",
  "At the same time, do not invent: current-card copy must be visible in these images; a Web result about what this card usually says is not current-card evidence.",
...
export function parseCanonicalFields(raw) {
  ...
  if (fields.grammar === "standard" && semGrammarForResolved(fields) === "TCG") {
  ...
  const lotCount = typeof parsed.lot_count === "string"
    ? cleanString(parsed.lot_count) : "";
  fields.lot_count = canonicalLotCountText(lotCount);
  if (parsed.lot_count != null && cleanString(parsed.lot_count) !== ""
  ...
  if (uniqueSubjects.length !== subjects.length) defects.push("duplicate_subjects");
  ...
  fields.attributes = [...new Set(attributes)];
  if (fields.attributes.length !== attributes.length) defects.push("duplicate_attributes");
