...
export const CANONICAL_FIELDS_SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    parallel_family: {
      ...
      description: "The BASE treatment family only: Refractor, Prizm, Holo or Foil. The other entries are patterns that sit on one of those -- a Raywave Refractor is a Refractor -- so choose a pattern only when no base family is present. Empty if you cannot tell which."
    },
...
export const CANONICAL_FIELDS_PROMPT = [
  ...
  // Three sources disagreed about this field and fixing them one at a time
  // failed twice: a schema rule alone scored 19-25, and a deliberation split
  // alone unstuck the silence (family filled 46 vs 37) without becoming score,
  // because this sentence went on offering Mojo as a peer of Refractor. Mojo is
  // a pattern that sits ON a family. Changed together with the schema wording
  // and the read-do-not-reconsider clause below, since each was cancelling the
  // others.
  "The parallel is asked for in three separate pieces and you should answer whichever ones you can: `surface_color` is just the basic colour (a shimmering gold card is simply Gold); `parallel_family` is the BASE finish treatment, of which there are only a few -- Refractor, Prizm, Holo, Foil -- while Mojo, Raywave, Geometric, Cracked Ice and the rest are patterns printed ON one of those, so a Raywave Refractor is a Refractor and a Mojo Prizm is a Prizm; `parallel_exact` only when the full name is literally printed. Answering the colour alone is a good answer -- the composer combines them.",
  // The finish is seen, not worked out. Deliberating over it was measured to
  // make the model quieter rather than righter: at low effort it filled
  // parallel_family on 35 cards against 50 at none, and its hit rate fell.
  "The colour and the finish family are things you see rather than things you work out. Read them off the card and answer; a second guess replaces a legible impression with a worse one. If the surface plainly has a treatment but you cannot name which family, give the colour and leave the family empty -- that is a complete answer, not a failure.",
