...
export const CANONICAL_FIELDS_SCHEMA = Object.freeze({
  ...
  properties: {
    year: { type: "string", description: "Season or copyright year as printed: \"2023\" or \"2023-24\". If there is a slab label, take it from there. If there is no slab, take it from the copyright line on the back, which is the small legal line naming the year, publisher and product together. A statistics year in a stat table on the back is not the issue year." },
...
export const CANONICAL_FIELDS_PROMPT = [
  ...
  "Inspect in this order before answering: (1) the slab label if there is one, (2) the card front for the subject, the parallel/finish wording and small foil limited numbering, (3) the card back for product, identity code, and the copyright line, (4) the grade and autograph areas. Do this even when the identity already seems obvious.",
  // A slab label is a third party's transcription of the card's identity, and
  // graded cards score far higher because of it: 0.8653 against 0.7443 on 150
  // stored observations, with year at 1.000 graded and 0.885 raw. A raw card
  // has an equivalent the model was never pointed at -- the copyright line
  // carries year, publisher and product in one printed string.
  "When there is no slab label, the copyright line on the back is the closest thing the card has to one: it prints the issue year, the publisher and the product together, and it is authoritative for the year. Read it before answering.",
