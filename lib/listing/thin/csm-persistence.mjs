...
const sameCanonicalValue = (left, right) => JSON.stringify(canonicalValue(left))
...
export function buildCsmStageRows({
  ...
  tcgFieldSourceAuthorityReceipt = null, tcgGrammarContextClaimReceipt = null,
  registryReleaseId = THIN_REGISTRY_RELEASE_ID, createdAt = null,
  contractVersion = CSM_DURABLE_PROJECTION_CONTRACT_VERSION
}) {
  if (![CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION,
    CSM_DURABLE_PROJECTION_CONTRACT_VERSION, CSM_STAGE_LEGACY_CONTRACT_VERSION]
    .includes(contractVersion)) {
    throw new TypeError("csm_stage_contract_version_unsupported");
  }
  const tcgGrammarContextContract =
    contractVersion === CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION;
  const durableProjectionContract = tcgGrammarContextContract
    || contractVersion === CSM_DURABLE_PROJECTION_CONTRACT_VERSION;
  const compositionGrammarName = compositionGrammar(composed?.grammar);
  const observedCompositionGrammarName = compositionGrammar(observedFields?.grammar);
  const capturedE1ae = contractVersion === CSM_STAGE_LEGACY_CONTRACT_VERSION;
  const sem = resolvedFieldsToSemSuggestion(toResolvedFields(fields, { capturedE1ae }));
  const observedSem = resolvedFieldsToSemSuggestion(
    toResolvedFields(observedFields, { capturedE1ae })
  );
  ...
  let tcgGrammarContextApplied = false;
  if (tcgGrammarContextContract) {
    if (tcgFieldSourceAuthorityReceipt == null || tcgGrammarContextClaimReceipt == null) {
      throw new TypeError("tcg_grammar_context_receipt_missing");
    }
    // The joint namespace is a standard-to-tcg transition question; a LOT is
    // outside it. An observed lot carries no registry claim, so the claim
    // validation must not run against re-derived fields (regression: the
    // LOT_SHARED_ONLY acceptance case 503'd with
    // tcg_grammar_context_receipt_invalid).
    if (observedCompositionGrammarName === "standard"
        || observedCompositionGrammarName === "tcg") {
      try {
        validateTcgFieldSourceAuthorityReceipt(tcgFieldSourceAuthorityReceipt, {
          founderBetaWebReceipt,
          fields: observedFields
        });
        validateTcgGrammarContextClaimReceipt(tcgGrammarContextClaimReceipt, {
          fields: observedFields,
          fieldSourceAuthorityReceipt: tcgFieldSourceAuthorityReceipt
        });
      } catch {
        throw new TypeError("tcg_grammar_context_receipt_invalid");
      }
    }
    tcgGrammarContextApplied = tcgGrammarContextClaimReceipt.status === "APPLIED";
    if (tcgGrammarContextApplied) {
      const expectedResolved = { ...observedFields, grammar: "tcg" };
      if (externalApplied || verifiedOriginalApplied
          || observedCompositionGrammarName !== "standard"
          || compositionGrammarName !== "tcg"
          || !sameCanonicalValue(fields, expectedResolved)
          || registryReleaseId !== TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE.release_id) {
        throw new TypeError("tcg_grammar_context_transition_invalid");
      }
    } else if (observedCompositionGrammarName !== compositionGrammarName) {
      throw new TypeError("tcg_grammar_context_transition_unproved");
    }
  } else if (tcgFieldSourceAuthorityReceipt != null
      || tcgGrammarContextClaimReceipt != null) {
    throw new TypeError("tcg_grammar_context_receipt_outside_contract");
  }
  if (!externalApplied && !verifiedOriginalApplied
      && !tcgGrammarContextApplied
      && !sameCanonicalValue(fields, observedFields)) {
  ...
  if (durableProjectionContract) {
    if (founderBetaWebReceipt != null) {
      try {
        validateFounderBetaWebReceiptAgainstFields(founderBetaWebReceipt, observedFields);
      }
  ...
  const resolution = {
    ...
    resolver_version: tcgGrammarContextApplied
      ? TCG_GRAMMAR_CONTEXT_RESOLUTION_CONTRACT.resolver_version
      : externalApplied
      ? externalReplayRelease.resolution.resolver_version
      : verifiedOriginalApplied
        ? VERIFIED_ORIGINAL_OBSERVATION_RESOLVER_VERSION
        : THIN_RESOLVER_VERSION,
    conflict_policy_version: tcgGrammarContextApplied
  ...
  const output = {
    ...
    structured_output: {
      ...
      ...(tcgGrammarContextContract ? {
        observed_composition_grammar: observedCompositionGrammarName,
        tcg_field_source_authority_receipt: structuredClone(
          tcgFieldSourceAuthorityReceipt
        ),
        tcg_grammar_context_claim_receipt: structuredClone(
 ...
 */
function schemaAliases(bracket, { durableProjectionContract = true } = {}) {
  if (bracket === "subject") return ["subjects"];
  if (durableProjectionContract && bracket === "numerical_rarity") return ["serial"];
  ...
  if (durableProjectionContract && bracket === "search_optimization") {
