...
export const THIN_COMPOSER_VERSION = "thin-marketplace-composer-v2";
...
export function buildCsmStageRows({
  ...
  externalIdentitySupport = null, composed, title,
  ...
  if (externalApplied) {
    ...
    if (externalIdentitySupport.pack_id !== EXTERNAL_IDENTITY_SUPPORT_PACK.pack_id
        || externalIdentitySupport.pack_version !== EXTERNAL_IDENTITY_SUPPORT_PACK.pack_version
        || externalIdentitySupport.pack_sha256 !== EXTERNAL_IDENTITY_SUPPORT_PACK.pack_sha256
        || externalIdentitySupport.index_id !== EXTERNAL_IDENTITY_SUPPORT_PACK.index_id
        || externalIdentitySupport.index_version !== EXTERNAL_IDENTITY_SUPPORT_PACK.index_version
        || externalIdentitySupport.index_sha256 !== EXTERNAL_IDENTITY_SUPPORT_PACK.index_sha256
        || externalIdentitySupport.registry_release_id !== EXTERNAL_IDENTITY_REGISTRY_RELEASE_ID
        || externalIdentitySupport.resolution_contract_sha256
  ...
  for (const bracket of CSM_BRACKETS) {
    ...
    if (present) {
      ...
      const candidateId = rowId(
        ...
        SEM_STANDARD_VERSION,
        contentDigest({ value: observedValue, value_kind: "VALUE", source_trust: "VISUAL_ONLY", observationConfidence })
      );
      candidates.push({
        id: candidateId,
        ...base,
        ...
        candidate_rank: externalSupported ? 2 : 1,
        ...stamp
      });
    ...
    if (externalSupported) {
      ...
      const registryEvidenceId = rowId(
        recognitionSessionId, bracket, "registry-obs", CSM_STAGE_CONTRACT_VERSION,
      ...
      const registryCandidateId = rowId(
        recognitionSessionId, bracket, "registry-cand", CSM_STAGE_CONTRACT_VERSION,
        externalIdentitySupport.index_version,
  ...
  const resolutionId = rowId(
    ...
    externalApplied ? EXTERNAL_IDENTITY_RESOLVER_VERSION : THIN_RESOLVER_VERSION,
  ...
  const resolution = {
    ...
    resolver_version: externalApplied ? EXTERNAL_IDENTITY_RESOLVER_VERSION : THIN_RESOLVER_VERSION,
  ...
  const output = {
    ...
    contract_version: CSM_STAGE_CONTRACT_VERSION,
    marketplace: "EBAY",
    composer_version: externalApplied ? EXTERNAL_IDENTITY_COMPOSER_VERSION : THIN_COMPOSER_VERSION,
