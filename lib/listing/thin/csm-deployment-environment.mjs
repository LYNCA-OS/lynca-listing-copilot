...
export const CSM_PRODUCTION_SUPABASE_PROJECT_REF = "irpgnhkslrsiucybkufc";
...
function supabaseProjectRef(value) {
  try {
    ...
    return url.protocol === "https:" && url.username === "" && url.password === ""
      && url.pathname === "/" && !url.search && !url.hash ? match?.[1] || null : null;
  } catch {
    return null;
  }
}
...
export function checkCsmDeploymentEnvironment(env = process.env) {
  ...
  if (!supabaseUrl) failures.push("SUPABASE_URL_missing");
  else if (!projectRef) failures.push("SUPABASE_URL_invalid_project_origin");
  else if (target === "production" && projectRef !== CSM_PRODUCTION_SUPABASE_PROJECT_REF) {
    failures.push("SUPABASE_URL_must_match_production_project");
  } else if (target === "preview" && projectRef === CSM_PRODUCTION_SUPABASE_PROJECT_REF) {
    failures.push("PREVIEW_SUPABASE_URL_must_not_match_production_project");
  }
