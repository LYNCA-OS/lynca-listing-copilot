 ...
 * and nobody looked.
 */
export function stripCategoryFiller(title) {
  const text = String(title ?? "");
  const stripped = text.replace(CATEGORY_FILLER, " ").replace(/\s{2,}/g, " ").trim();
  return { title: stripped, applied: stripped !== text };
}

/** "Autograph"/"Autographs" -> "Auto", named in the contract's compression list. */
export function compressAutograph(title) {
  const text = String(title ?? "");
  const compressed = text.replace(/\bAutographs?\b/gi, "Auto");
