...
export const CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION =
  "csm-stage-shadow-v4";
...
export const CSM_TCG_GRAMMAR_CONTEXT_FORWARD_READER_VERSION =
  "csm-durable-forward-reader-v2";
...
function exactResolvedBracketState(rows) {
  const state = {};
  for (const bracket of semCanonicalEditableFields) {
    const matching = (Array.isArray(rows?.resolved) ? rows.resolved : [])
      .filter((entry) => entry?.bracket === bracket);
    if (matching.length !== 1) {
      throw new TypeError("tcg_grammar_context_resolved_cardinality_invalid");
    }
    const row = matching[0];
    state[bracket] = row.selected_kind === "EMPTY" ? "" : row.canonical_value;
  }
  exactResolvedIdentity(rows, state, new Set(semCanonicalEditableFields));
  return Object.freeze(state);
}

function validateTcgGrammarContextRowFamily(rows) {
  const version = CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION;
  for (const collection of [rows?.evidence, rows?.candidates]) {
    if (!Array.isArray(collection)
        || collection.some((row) => row?.contract_version !== version)) {
      throw new TypeError("durable_projection_row_family_mismatch");
    }
  }
}

function rejectV4AuthorityOutsideTcgGrammarContextContract(rows) {
  const resolution = rows?.resolution || {};
  const v4OnlyAuthority = {
  ...
  if (Object.entries(v4OnlyAuthority).some(
    ([field, value]) => resolution[field] === value
  )) {
    throw new TypeError("tcg_grammar_context_authority_outside_contract");
  }
}

function validateTcgGrammarContextProjection(rows, {
  observedIdentity,
  webReceipt,
  externalReceipt,
  verifiedReceipt
  ...
  const fieldSourceReceipt = structured?.tcg_field_source_authority_receipt ?? null;
  const claimReceipt = structured?.tcg_grammar_context_claim_receipt ?? null;
  if (fieldSourceReceipt == null) {
    throw new TypeError("tcg_field_source_authority_receipt_missing");
  }
  if (claimReceipt == null) {
    throw new TypeError("tcg_grammar_context_claim_receipt_missing");
  }
  if (fieldSourceReceipt.semantic_state_sha256 !== webReceipt.semantic_state_sha256
      || fieldSourceReceipt.founder_beta_web_receipt_sha256
        !== canonicalSha256(webReceipt)) {
    throw new TypeError("tcg_field_source_authority_receipt_binding_invalid");
  }

  const observedGrammar = clean(structured?.observed_composition_grammar).toLowerCase();
  const resolvedGrammar = clean(structured?.composition_grammar).toLowerCase();
  // The joint namespace is a standard-to-tcg transition question; a LOT is
  // outside it. The claim-family binding (source authority, claim receipt,
  // observed claim identity, applied/unapplied transitions) must not run for
  // observed lot grammar: a lot carries no registry claim and its card_number
  // is a multi-card joined value with no per-card evidence row (observed: the
  // LOT_SHARED_ONLY acceptance case failed the row-family audit with
  // founder_beta_observed_identity_cardinality_invalid).
  if (observedGrammar === "standard" || observedGrammar === "tcg") {
    const observedClaimFields = {
      grammar: observedGrammar,
      set: clean(claimReceipt?.normalized_set),
      card_number: clean(claimReceipt?.normalized_card_number)
    };
    try {
      validateTcgFieldSourceAuthorityReceipt(fieldSourceReceipt, {
        founderBetaWebReceipt: webReceipt,
        fields: observedClaimFields
      });
    } catch {
      throw new TypeError("tcg_field_source_authority_receipt_invalid");
    }
    if (observedIdentity?.set !== observedClaimFields.set) {
      throw new TypeError("tcg_grammar_context_observed_set_mismatch");
    }
    exactObservedIdentity(rows, observedClaimFields, new Set(["set", "card_number"]));
    try {
    ...
    if (claimReceipt.field_source_authority_receipt_sha256
          !== fieldSourceReceipt.receipt_sha256
        || claimReceipt.raw_grammar !== observedGrammar
        || claimReceipt.resolved_grammar !== resolvedGrammar) {
      throw new TypeError("tcg_grammar_context_claim_receipt_binding_invalid");
    }
    if (claimReceipt.status === "APPLIED") {
      if (externalReceipt != null || verifiedReceipt != null
          || observedGrammar !== "standard" || resolvedGrammar !== "tcg"
          || rows?.resolution?.registry_release_id
            !== TCG_GRAMMAR_CONTEXT_REGISTRY_RELEASE.release_id
          || rows?.resolution?.resolver_version
            !== TCG_GRAMMAR_CONTEXT_RESOLUTION_CONTRACT.resolver_version
          || rows?.resolution?.conflict_policy_version
            !== TCG_GRAMMAR_CONTEXT_RESOLUTION_CONTRACT.conflict_policy_version) {
        throw new TypeError("tcg_grammar_context_applied_transition_invalid");
      }
      const resolvedState = exactResolvedBracketState(rows);
      exactObservedIdentity(rows, resolvedState, new Set(semCanonicalEditableFields));
      const resolvedSem = Object.fromEntries(Object.entries(resolvedState)
        .filter(([, value]) => valuePresent(value)));
      if (clean(resolvedState.set) !== observedClaimFields.set
          || clean(resolvedState.card_number) !== observedClaimFields.card_number
          || !sameValue(structured?.sem, resolvedSem)) {
        throw new TypeError("tcg_grammar_context_applied_fields_changed");
      }
    } else if (observedGrammar !== resolvedGrammar) {
      throw new TypeError("tcg_grammar_context_unapplied_transition_invalid");
    }
  }
  const expectedIdentityGrammar = resolvedGrammar === "tcg" ? "TCG" : "NON_TCG";
  if (rows?.resolution?.grammar !== expectedIdentityGrammar) {
    throw new TypeError("tcg_grammar_context_resolution_grammar_mismatch");
  }

  return Object.freeze({
    field_source_authority_receipt: fieldSourceReceipt,
    grammar_context_claim_receipt: claimReceipt
  });
}
...
export function readDurableProjectionReceipt(rows) {
  ...
  const tcgGrammarContextKeys = [
    "observed_composition_grammar", "tcg_field_source_authority_receipt",
    "tcg_grammar_context_claim_receipt"
  ];
  const hasStructuredKey = (key) => Object.hasOwn(
    output?.structured_output || {}, key
  );
  if (version === CSM_LEGACY_PROJECTION_CONTRACT_VERSION) {
    ...
    if (futureKeys.some((key) => output?.structured_output?.[key] != null)
        || tcgGrammarContextKeys.some(hasStructuredKey)) {
  ...
  if (version === CSM_DURABLE_PROJECTION_CONTRACT_VERSION
      && tcgGrammarContextKeys.some(hasStructuredKey)) {
    throw new TypeError("tcg_grammar_context_receipt_outside_contract");
  }
  if (version === CSM_DURABLE_PROJECTION_CONTRACT_VERSION) {
    // TCG is also a legal raw v3 grammar, so grammar alone is not a v4 marker.
    // Its Registry/resolver/policy values are v4-only authority and cannot be
    // made v3 merely by stripping the two receipts and recomputing hashes.
    rejectV4AuthorityOutsideTcgGrammarContextContract(rows);
  }
  if (![CSM_DURABLE_PROJECTION_CONTRACT_VERSION,
    CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION].includes(version)) {
    throw new TypeError("csm_stage_contract_version_unsupported");
  }
  if (version === CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION) {
    validateTcgGrammarContextRowFamily(rows);
  }
  ...
  if (version === CSM_TCG_GRAMMAR_CONTEXT_PROJECTION_CONTRACT_VERSION) {
    const grammarContext = validateTcgGrammarContextProjection(rows, {
      observedIdentity,
      webReceipt,
      externalReceipt,
      verifiedReceipt
    ...
    return Object.freeze({
      ...
      tcg_field_source_authority_receipt:
        grammarContext.field_source_authority_receipt,
      tcg_grammar_context_claim_receipt:
        grammarContext.grammar_context_claim_receipt
    });
  }
  return Object.freeze({
