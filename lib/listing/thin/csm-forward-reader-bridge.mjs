...
function founderBetaCanonicalAuthorityAudit(body, {
  ...
  if (urls.length > 20) throw new TypeError("founder_beta_web_source_budget_exceeded");
  ...
  for (const row of sourceRows) {
    ...
    const webUrls = sourceIds.filter((sourceId) => !allowedImages.has(sourceId))
      .map(sanitizeTraceUrl).sort();
    if (webIdentityFields.has(field)) {
      const governedUrls = webUrls.filter(governedIdentityAuthorityUrl);
      const unresolvedUrls = webUrls.filter((url) => !governedUrls.includes(url));
      const admitted = sourceIds.some((sourceId) => allowedImages.has(sourceId))
    ...
    } else if (webUrls.length) {
      fieldEvidence.push({
        field,
        support_urls: webUrls,
        conflict_urls: [],
        unresolved_urls: []
      });
    }
