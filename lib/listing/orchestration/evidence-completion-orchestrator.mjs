...
const preparedRetrievalReuseAction = "REUSE_PREPARED_RETRIEVAL";

function completionActionForPreparedQueryFamily(value = "") {
  const family = String(value || "").trim();
  if (family === retrievalQueryFamilies.INTERNAL_APPROVED_HISTORY) return completionActions.SEARCH_INTERNAL_APPROVED_HISTORY;
  if (family === retrievalQueryFamilies.INTERNAL_REGISTRY) return completionActions.SEARCH_INTERNAL_REGISTRY;
  if ([retrievalQueryFamilies.CATALOG_EXACT_CODE, retrievalQueryFamilies.EXACT_CHECKLIST_CODE].includes(family)) {
    return completionActions.SEARCH_EXACT_CHECKLIST_CODE;
  }
  if ([
    ...
    return completionActions.SEARCH_PLAYER_AND_COLLECTOR_NUMBER;
  }
  if ([
    ...
    return completionActions.SEARCH_PRODUCT_AND_SERIAL_DENOMINATOR;
  }
  if (family === retrievalQueryFamilies.OFFICIAL_SOURCES) return completionActions.SEARCH_OFFICIAL_SOURCES;
  if (family === retrievalQueryFamilies.BRAVE) return completionActions.SEARCH_BRAVE;
  if (family === retrievalQueryFamilies.EBAY) return completionActions.SEARCH_EBAY;
  if (family === retrievalQueryFamilies.OWS_FALLBACK) return completionActions.SEARCH_OWS_FALLBACK;
  return null;
}

function preparedRetrievalAttempts(retrieval = {}, attemptedActions = []) {
  const alreadyAttempted = new Set((Array.isArray(attemptedActions) ? attemptedActions : [])
    .map((attempt) => String(attempt?.action || "").trim())
    .filter(Boolean));
  const actions = [...new Set((Array.isArray(retrieval.queries) ? retrieval.queries : [])
    .map((query) => completionActionForPreparedQueryFamily(query?.family || query?.query_family))
    .filter(Boolean))];
  return actions
    .filter((action) => !alreadyAttempted.has(action))
    .map((action) => ({
      action,
      status: "reused_prepared_retrieval",
      reason: "The same retrieval family already ran before evidence completion."
    }));
}
...
function hasVisualEmbeddingPayload(input) {
  const items = Array.isArray(input)
    ? input
    : input && Array.isArray(input.features)
      ? input.features
      : [];
  return items.some((item) => Array.isArray(item?.embedding) && item.embedding.length);
}

function visualEmbeddingPayloadCount(input) {
  const items = Array.isArray(input)
    ? input
    : input && Array.isArray(input.features)
      ? input.features
      : [];
  return items.filter((item) => Array.isArray(item?.embedding) && item.embedding.length).length;
}
...
function hybridRetrievalEnabled(env = process.env) {
  return envFlag(env.ENABLE_ADVANCED_RETRIEVAL, false)
    || envFlag(env.ENABLE_HYBRID_RETRIEVAL, false)
    || envFlag(env.ENABLE_POSTGRES_HYBRID_RETRIEVAL, false);
}

function retrievalFamiliesForAction(action, visualEmbeddings = [], env = process.env) {
  const families = retrievalFamiliesByAction[action] || [];
  if (action === completionActions.SEARCH_INTERNAL_APPROVED_HISTORY && hasVisualEmbeddingPayload(visualEmbeddings)) {
    const vectorFamilies = hybridRetrievalEnabled(env)
      ? [
      ...
      : [retrievalQueryFamilies.VISUAL_VECTOR];
    return [...new Set([...families, ...vectorFamilies])];
  }
  return families;
}
...
function focusedVisionAttemptLimit(env = {}, remaining = {}) {
  ...
  return Math.max(1, Math.min(Number(remaining.rounds || 1), 1 + retries));
}
...
function requiredFocusedActionsFromEnv(env = process.env) {
  const fieldGroups = splitEnvList(env.REQUIRED_FOCUSED_FIELDS);
...
async function executeCompletionAction({
  ...
  if (actionIsRetrieval(action)) {
    ...
    let retrieval;
    const actionAllowedFamilies = retrievalFamiliesForAction(action, visualEmbeddings, env);
    const allowedFamilies = Array.isArray(allowedFamiliesOverride) && allowedFamiliesOverride.length
      ? allowedFamiliesOverride
      : actionAllowedFamilies;
    const visualQueryCount = allowedFamilies.includes(retrievalQueryFamilies.VISUAL_VECTOR)
      ? Math.max(1, visualEmbeddingPayloadCount(visualEmbeddings))
      : 0;
    const configuredMaxQueries = Number.isFinite(Number(maxQueriesOverride)) && Number(maxQueriesOverride) > 0
      ? Number(maxQueriesOverride)
      : null;
    const maxQueries = configuredMaxQueries || (usesExternalBudget
      ? Math.max(1, remaining.external_queries)
      : allowedFamilies.includes(retrievalQueryFamilies.VISUAL_VECTOR)
        ? Math.max(1, allowedFamilies.length + Math.max(0, visualQueryCount - 1))
        : 1);
    try {
  ...
  if (plannedVisionRecovery(action)) {
    ...
    if (typeof runFocusedVisionImpl === "function") {
      ...
      if (!lastFocusedError) {
        const endedAt = Date.now();
        const providerId = focusedVision?.provider_id || focusedVision?.provider || "focused_vision";
        const focusedDocument = focusedVision?.evidence_document || focusedVision?.document || focusedVision || {};
...
export async function completeEvidence({
  ...
  const preparedSummaries = (Array.isArray(initialRetrievalSummaries) ? initialRetrievalSummaries : [])
    .filter((summary) => summary && typeof summary === "object" && !Array.isArray(summary))
  let retrieval = preparedSummaries.reduce(
      (current, summary) => mergeRetrievalSummaries(current, summary),
      emptyRetrievalSummary(retrievalMode)
    );
  let trace = [];
  let attempts = Array.isArray(attemptedActions) ? [...attemptedActions] : [];
  const preparedAttempts = preparedRetrievalAttempts(retrieval, attempts);
  attempts.push(...preparedAttempts);
  const preparedRetrievalReuse = {
    summary_count: preparedSummaries.length,
    query_count: retrieval.queries.length,
    candidate_count: retrieval.sources.length,
    selected_candidate_present: Boolean(retrieval.selected_candidate),
    reused_query_families: preparedAttempts.map((attempt) => attempt.action)
  };
  ...
  if (retrieval.sources.length) {
    ...
    let reuseTrace = createCompletionTraceEntry({
      action: preparedRetrievalReuseAction,
      status: "executed",
      reason: "Reused catalog/vector candidates prepared earlier in the same recognition request.",
      input: {
        candidate_count: retrieval.sources.length,
        query_count: retrieval.queries.length
      },
    ...
    reuseTrace = appendCandidateVerification(reuseTrace, verification);
    reuseTrace = appendConvergenceToTrace(reuseTrace, {
      before: state,
      after: reevaluatedState
    });
    trace.push(reuseTrace);
    attempts.push(completionAttemptFromTrace(reuseTrace));
    state = reevaluatedState;
  }

  const hasVisualEmbeddings = hasVisualEmbeddingPayload(visualEmbeddings);

  if ((state.resolution_state !== completionResolutionStates.EVIDENCE_CLOSED || hasVisualEmbeddings)
    && !hasAttemptedAction(attempts, completionActions.SEARCH_INTERNAL_APPROVED_HISTORY)
    && !isResolutionBudgetExhausted(budget)) {
    const action = completionActions.SEARCH_INTERNAL_APPROVED_HISTORY;
    const actionDeadlineMs = Math.max(1500, remainingResolutionBudget(budget).resolution_time_ms);
    const execution = await raceCompletionActionDeadline(executeCompletionAction({
      action,
      state,
      resolved: currentResolved,
      evidence: currentEvidence,
      ...
      runFocusedVisionImpl,
      visualEmbeddings,
      allowedFamiliesOverride: allowedFamilies,
      maxQueriesOverride: maxQueries,
      excludeSourceFeedbackIds
    }), action, actionDeadlineMs);

    if (execution.deadline_exceeded) {
      const deadlineTrace = createCompletionTraceEntry({
        action,
        status: "deadline_exceeded",
        reason: `initial completion action exceeded the remaining resolution budget (${actionDeadlineMs}ms)`,
        input: {}
      });
      trace.push(deadlineTrace);
      attempts.push(completionAttemptFromTrace(deadlineTrace));
      budget = consumeResolutionBudget(budget, { rounds: 1, resolutionTimeMs: actionDeadlineMs });
    } else {
  ...
  while (!isResolutionBudgetExhausted(budget)) {
    ...
    const execution = await raceCompletionActionDeadline(executeCompletionAction({
      ...
      maxQueriesOverride: maxQueries,
      excludeSourceFeedbackIds
    }), next.action, actionDeadlineMs);
    if (execution.deadline_exceeded) {
      const deadlineTrace = createCompletionTraceEntry({
        action: next.action,
        status: "deadline_exceeded",
        reason: `completion action exceeded the remaining resolution budget (${actionDeadlineMs}ms)`,
        input: {}
      });
      trace.push(deadlineTrace);
      attempts.push(completionAttemptFromTrace(deadlineTrace));
  ...
  return {
    ...
    state,
    retrieval,
    prepared_retrieval_reuse: preparedRetrievalReuse,
...
function raceCompletionActionDeadline(promise, action, deadlineMs) {
  let timer;
  return Promise.race([
    promise,
    new Promise((resolve) => {
      timer = setTimeout(() => resolve({ deadline_exceeded: true, action }), Math.max(1, deadlineMs));
    })
  ]).finally(() => {
