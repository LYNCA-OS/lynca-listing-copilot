...
function retrievalFamiliesForAction(action, visualEmbeddings = []) {
  ...
  if (action === completionActions.SEARCH_INTERNAL_APPROVED_HISTORY && hasVisualEmbeddingPayload(visualEmbeddings)) {
    return [...new Set([...families, retrievalQueryFamilies.VISUAL_VECTOR])];
  }
  return families;
}
...
async function executeCompletionAction({
  ...
  if (actionIsRetrieval(action)) {
    ...
    let retrieval;
    const allowedFamilies = retrievalFamiliesForAction(action, visualEmbeddings);
