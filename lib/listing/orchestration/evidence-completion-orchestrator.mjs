...
function focusedFieldHasReviewBlocker(fieldName, focusedField = {}, focusedDocument = {}) {
  ...
  const fieldAliases = fieldName === "parallel" || fieldName === "variation"
...
function focusedPublishEvidenceFloor({
  ...
  if (!["parallel", "variation"].includes(fieldName)) return null;
  ...
  return {
    ...
    reason: "focused_parallel_reread_without_review_blocker"
  };
}
    ...
    if (!currentHasValue || !currentIsConfirmed) {
      ...
      nextEvidence[fieldName] = focusedField
        ...
        : createEvidenceField({
            ...
            confidence: publishFloor?.confidence ?? 0.78,
            sources: focusedSources
          });
      updatedFields.push(fieldName);
      continue;
