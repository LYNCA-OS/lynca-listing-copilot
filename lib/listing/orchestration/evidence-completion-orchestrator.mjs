...
const focusedFieldsByAction = Object.freeze({
  ...
  [completionActions.CROP_AND_READ_PARALLEL]: ["parallel", "variation", "ssp", "case_hit"],
...
function mergeAgreementField({
  ...
  const confidence = Math.max(Number(currentField.confidence || 0), Number(focusedField.confidence || 0), 0.5);
  ...
  return createEvidenceField({
    ...
    candidates: [
      ...
      { value: focusedValue, confidence: Number(focusedField.confidence || confidence), sources: focusedSources }
    ],
    sources,
    conflicts: [
    ...
    unresolvedReason: currentField.unresolved_reason || focusedField.unresolved_reason || null
  });
}
...
function mergeFocusedVisionEvidence({
  ...
  for (const fieldName of focusFields) {
    const rawFocusedValue = focusedRawValueForField(fieldName, focusedDocument);
    ...
    if (!currentHasValue || !currentIsConfirmed) {
      ...
      nextEvidence[fieldName] = focusedField
        ? {
            ...focusedField,
