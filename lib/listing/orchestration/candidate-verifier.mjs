...
function applyIndependentCandidateClosure({
  ...
  Object.keys(groups).forEach((fieldName) => {
    ...
    const matchingGroup = independentGroups.find((group) => valuesEqual(currentValue, group.value));
    const conflictingGroups = independentGroups.filter((group) => !valuesEqual(currentValue, group.value));
