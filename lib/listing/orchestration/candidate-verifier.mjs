...
const visualVectorExactMemoryFields = new Set([
  "serial_number",
  "grade_company",
  "card_grade",
  "auto_grade",
...
const physicalInstanceOnlyFields = new Set([
  "serial_number",
  "grade_company",
  "card_grade",
  "auto_grade",
...
const identityCatalogLockFields = new Set([
  "year",
  "manufacturer",
  "brand",
  "product",
...
const parallelCatalogLockFields = new Set([
  "surface_color",
  "parallel_family",
  "parallel_exact",
  "parallel",
...
const stylisticRendererFields = new Set([
  "team",
  "artist",
  "category",
  "standardness",
...
const catalogIdentitySourceTypes = new Set([
  "INTERNAL_APPROVED_HISTORY",
  "INTERNAL_REGISTRY",
  "OFFICIAL_CHECKLIST",
  "OFFICIAL_PRODUCT_PAGE",
...
function serialDenominator(value) {
  return String(value || "").match(/\/\s*0*(\d{1,4})\b/)?.[1] || "";
}
...
function moreSpecificValue(left, right, fieldName = "") {
  ...
  const rightKey = normalizeComparableField(fieldName, right);
  if (leftKey === rightKey) {
    const leftDisplay = normalizeText(Array.isArray(left) ? left.join(" / ") : left);
    const rightDisplay = normalizeText(Array.isArray(right) ? right.join(" / ") : right);
    return leftDisplay.length >= rightDisplay.length ? left : right;
  }
  return rightKey.length >= leftKey.length ? right : left;
}
...
function directEvidenceConflictFields(candidate = {}) {
  return [
...
function candidateFieldName(key = "") {
  if (key === "player" || key === "subject" || key === "subjects") return "players";
  if (key === "card_number") return "collector_number";
  if (key === "set_or_insert") return "insert";
  return key;
}

function isCatalogIdentityCandidate(candidate = {}) {
  if (candidate?.provider_id === "catalog") return true;
  if (String(candidate?.source_url || "").startsWith("supabase://catalog-cards/")) return true;
  if (candidate?.field_derivation?.source === "basketball_catalog_v0") return true;
  return catalogIdentitySourceTypes.has(String(candidate?.source_type || "").trim().toUpperCase());
}
...
function candidateHasExactIdentityAnchor(candidate = {}) {
  const matched = new Set(candidateMatchedFields(candidate));
  const exact = ["collector_number", "checklist_code", "card_number", "serial_number", "serial_denominator"].some((field) => matched.has(field));
  const identity = ["subjects", "players", "product", "set", "year", "brand", "manufacturer", "surface_color", "trigram"].some((field) => matched.has(field));
  if (exact && identity) return true;
  const fields = normalizeCandidateFields(candidate);
  return Boolean(
    (fieldHasValue(fields, "collector_number") || fieldHasValue(fields, "checklist_code") || candidateExpectedSerialDenominator(candidate))
    && (fieldHasValue(fields, "players") || fieldHasValue(fields, "product") || fieldHasValue(fields, "year") || fieldHasValue(fields, "brand"))
  );
}

function catalogHardConstraintEligible(candidate = {}) {
  if (!isCatalogIdentityCandidate(candidate)) return false;
  if (directEvidenceConflictFields(candidate).length) return false;
  if (!candidateIsStrong(candidate) && Number(candidate.match_score || candidate.candidate_selection_score || 0) < 0.62) return false;
  return candidateHasExactIdentityAnchor(candidate);
}

function candidateFieldDecisionPipeline(fieldName = "", candidate = {}) {
  if (physicalInstanceOnlyFields.has(fieldName)) return "factual_current_image_only";
  if (fieldName === "year") return "year_catalog_with_direct_evidence_guard";
  if (parallelCatalogLockFields.has(fieldName)) return "parallel_catalog_taxonomy";
  if (identityCatalogLockFields.has(fieldName)) return "identity_catalog";
  if (stylisticRendererFields.has(fieldName)) return "stylistic_renderer";
  return isCatalogIdentityCandidate(candidate) ? "identity_catalog" : "retrieval_evidence";
}

function candidateCanHardLockField(candidate = {}, fieldName = "") {
  if (!catalogHardConstraintEligible(candidate)) return false;
  if (physicalInstanceOnlyFields.has(fieldName)) return false;
  return identityCatalogLockFields.has(fieldName) || parallelCatalogLockFields.has(fieldName);
}

function addPipelineField(summary, pipeline, fieldName) {
  summary.decision_pipelines[pipeline] ||= [];
  if (!summary.decision_pipelines[pipeline].includes(fieldName)) {
    summary.decision_pipelines[pipeline].push(fieldName);
  }
}

function addFieldLock(summary, {
  fieldName,
  value,
  candidate,
  pipeline,
  lockType = "catalog_hard_constraint"
} = {}) {
  const key = `${fieldName}|${normalizeComparableField(fieldName, value)}|${candidate?.candidate_id || ""}`;
  if (summary.field_locks.some((lock) => lock.key === key)) return;
  summary.field_locks.push({
    key,
    field: fieldName,
    value,
...
function candidateExpectedSerialDenominator(candidate = {}) {
  const fromMetadata = serialDenominator(candidate.reference_metadata?.expected_serial_denominator)
  ...
  return serialDenominator(fields.expected_serial_denominator)
    || String(fields.expected_serial_denominator || "").replace(/[^0-9]/g, "")
    || serialDenominator(fields.serial_number);
}

function candidateCanSupplyResolvedField(candidate = {}, fieldName = "") {
  if (physicalInstanceOnlyFields.has(fieldName)) return false;
  if (!isCatalogIdentityCandidate(candidate)) return true;
  return true;
}
...
function normalizeCandidateFields(candidate = {}) {
  ...
  Object.entries(rawFields).forEach(([key, value]) => {
    ...
    const fieldName = candidateFieldName(key);
    if (!resolvedFieldNames.includes(fieldName)) return;
    if (!candidateCanSupplyResolvedField(candidate, fieldName)) return;
...
function visualVectorExactMemoryCanSupplyField(candidate = {}, fieldName = "", anchors = 0) {
  if (!visualVectorExactMemoryFields.has(fieldName)) return false;
  if (physicalInstanceOnlyFields.has(fieldName) && candidate.physical_instance_match !== true) return false;
  return visualSimilarity(candidate) >= 0.995
    && visualMargin(candidate) >= 0.12
    && anchors >= 1;
}

function applyExpectedSerialDenominatorEvidence({
  resolved,
  evidence,
  summary,
  candidate,
  ...
  const expectedDenominator = candidateExpectedSerialDenominator(candidate);
  if (!expectedDenominator || !fieldHasValue(resolved, "serial_number")) {
    return { evidence, changed: false };
  }

  const currentValue = resolved.serial_number;
  const currentDenominator = serialDenominator(currentValue);
  if (!currentDenominator) return { evidence, changed: false };
  const currentEvidence = evidence.serial_number || null;

  if (currentDenominator === expectedDenominator) {
    return {
      evidence: {
        ...evidence,
        serial_number: createEvidenceField({
          ...
          status: currentEvidence?.status === "MANUAL_CONFIRMED" ? "MANUAL_CONFIRMED" : currentEvidence?.status || "REVIEW",
          confidence: Math.max(Number(currentEvidence?.confidence || 0), candidateConfidence(candidate, candidateIsStrong(candidate)) - 0.04),
          candidates: currentEvidence?.candidates,
          sources: appendSource(currentEvidence, {
            ...source,
            evidence_kind: "catalog_expected_serial_denominator_support",
            expected_serial_denominator: expectedDenominator
          }),
          conflicts: markConflictsResolved(currentEvidence?.conflicts, {
            reason: "catalog_expected_denominator_matches_current_serial",
  ...
  return {
    evidence: {
      ...evidence,
      serial_number: withConflict(currentEvidence, {
        fieldName: "serial_number",
        existingValue: currentValue,
        candidateValue: `/${expectedDenominator}`,
        candidate,
        source: {
          ...source,
          evidence_kind: "catalog_expected_serial_denominator_conflict",
          expected_serial_denominator: expectedDenominator
        }
      })
    },
...
function groupedVisualFieldValues(candidates = [], resolved = {}) {
  ...
  candidates.forEach((candidate) => {
    ...
    Object.entries(fields).forEach(([fieldName, value]) => {
      ...
      if (visualVectorNeverCopyFields.has(fieldName) && !visualVectorExactMemoryCanSupplyField(candidate, fieldName, anchors)) return;
...
function currentFieldProtectedFromTrustedOverride(fieldName, existingField = {}) {
  ...
  if (fieldName === "year" && yearEvidenceHasStrongDirectSupport(existingField)) return true;
...
function catalogHardLockCanOverrideCurrent(fieldName, existingField = {}, candidate = {}) {
  if (!candidateCanHardLockField(candidate, fieldName)) return false;
...
function applyVisualVectorFieldEvidence({
  ...
  Object.entries(groupsByField).forEach(([fieldName, groups]) => {
    ...
    if (!hasCurrent) {
      if (!visualVectorConsensusOverrideFields.has(fieldName) && !visualVectorExactMemoryFields.has(fieldName)) {
        summary.visual_vector.ignored_fields.push({
          field: fieldName,
...
export function verifyRetrievalCandidates({
  ...
  const summary = {
    ...
    decision_pipelines: {
      identity_catalog: [],
      year_catalog_with_direct_evidence_guard: [],
      parallel_catalog_taxonomy: [],
      factual_current_image_only: [],
      stylistic_renderer: [],
      retrieval_evidence: []
    },
    ignored_candidates: [],
  ...
  if (selected && candidateIsGroundTruthEligible(selected)) {
    ...
    const selectedCatalogHardConstraint = catalogHardConstraintEligible(selected);
    const baseSource = sourceFromCandidate(selected);
    ...
    Object.entries(candidateFields).forEach(([fieldName, candidateValue]) => {
      ...
      const hasCurrent = fieldHasValue(nextResolved, fieldName);
      const currentEvidence = nextEvidence[fieldName] || null;
      const pipeline = candidateFieldDecisionPipeline(fieldName, selected);
      addPipelineField(summary, pipeline, fieldName);
      if (pipeline === "factual_current_image_only") {
        summary.ignored_candidates.push({
          candidate_id: selected.candidate_id || null,
          field: fieldName,
          reason: "catalog_candidate_cannot_lock_physical_instance_field",
          decision_pipeline: pipeline
        });
        return;
      }
      const hardLock = candidateCanHardLockField(selected, fieldName);
      const status = hardLock || strong ? "CONFIRMED" : "REVIEW";
      const source = {
        ...baseSource,
        evidence_kind: hardLock ? "catalog_identity_field_lock" : "trusted_retrieval_candidate_support",
        field_lock: hardLock,
        decision_pipeline: pipeline,
        hard_constraint_layer: selectedCatalogHardConstraint
      };
      ...
      if (!hasCurrent) {
        ...
        if (hardLock) addFieldLock(summary, { fieldName, value: candidateValue, candidate: selected, pipeline });
      ...
      if (valuesCompatible(currentValue, candidateValue, fieldName)) {
        ...
        summary[status === "CONFIRMED" ? "verified_fields" : "review_fields"].push(fieldName);
        if (hardLock) addFieldLock(summary, { fieldName, value: resolvedValue, candidate: selected, pipeline });
        changed = true;
        return;
      }

      if (catalogHardLockCanOverrideCurrent(fieldName, currentEvidence, selected) || trustedCandidateCanOverrideCurrent(fieldName, currentEvidence, selected)) {
        nextResolved = normalizeResolvedFields({
          ...nextResolved,
          [fieldName]: candidateValue
        });
        nextEvidence[fieldName] = createEvidenceField({
          ...
          conflicts: [
            ...(Array.isArray(currentEvidence?.conflicts) ? currentEvidence.conflicts : []),
            {
              ...
              reason: hardLock ? "catalog_hard_constraint_field_lock_override_model_inference" : "trusted_internal_history_override_model_inference",
              resolved: true
            }
          ]
        });
    ...
    const denominatorSupport = applyExpectedSerialDenominatorEvidence({
      resolved: nextResolved,
      evidence: nextEvidence,
      summary,
      candidate: selected,
      source: {
        ...
        evidence_kind: "catalog_expected_serial_denominator_support",
        field_lock: false,
        decision_pipeline: "factual_current_image_only",
        hard_constraint_layer: selectedCatalogHardConstraint
      }
    });
    nextEvidence = denominatorSupport.evidence;
    if (denominatorSupport.changed) {
      changed = true;
      addPipelineField(summary, "factual_current_image_only", "serial_number");
      if (denominatorSupport.conflict) summary.conflicting_fields.push("serial_number");
      else {
        summary.verified_fields.push("serial_number");
        addFieldLock(summary, {
          fieldName: "serial_number",
          value: nextResolved.serial_number,
          candidate: selected,
          pipeline: "factual_current_image_only",
          lockType: "catalog_expected_denominator_support_only"
        });
      }
    }
  }
  ...
  return {
    ...
    summary: {
      ...
      field_locks: summary.field_locks.map(({ key, ...lock }) => lock),
      decision_pipelines: Object.fromEntries(Object.entries(summary.decision_pipelines).map(([pipeline, fields]) => [
        pipeline,
        [...new Set(fields)]
      ]))
    }
