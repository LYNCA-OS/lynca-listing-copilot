...
function slabAutoGradeMissing(result = {}) {
  const resolved = normalizeResolvedFields(result.resolved || result.resolved_fields || result.fields || {});
  if (!resultHasSlabSignal(result)) return false;
  if (!hasValue(resolved.grade_company) || !hasValue(resolved.card_grade)) return false;
  if (hasValue(resolved.auto_grade)) return false;
  if (resolved.auto === true) return true;
  const autoEvidence = evidenceField(result, "auto");
  if (autoEvidence?.value === true) return true;
  const text = [
    resolved.card_name,
    resolved.card_type,
    resolved.official_card_type,
    resolved.insert,
    ...(Array.isArray(resolved.observable_components) ? resolved.observable_components : []),
    autoEvidence?.visible_text,
    autoEvidence?.raw_text,
    autoEvidence?.evidence_kind
  ].filter(Boolean).join(" ");
  return /\b(?:auto|autograph|signature|signed)\b/i.test(text);
}
...
function ocrTriggerReason(result = {}, task = {}) {
  ...
  if (task.task_id === "ocr_slab_label_verifier") {
    if (slabAutoGradeMissing(result)) return "slab_detected_auto_grade_missing_for_autographed_card";
