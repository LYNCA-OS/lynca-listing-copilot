...
const unresolvedFieldAliases = Object.freeze({
  ...
  "parallel family": "parallel_family",
  parallel_family: "parallel_family",
  "parallel exact": "parallel_exact",
  ...
  "numerical rarity": "numerical_rarity",
  "serial limit": "numerical_rarity",
...
function statusIsConflict(field) {
  const unresolvedConflicts = Array.isArray(field?.conflicts)
    ? field.conflicts.filter((conflict) => conflict?.resolved !== true)
    : [];
  return field?.status === "CONFLICT" || unresolvedConflicts.length > 0;
}
