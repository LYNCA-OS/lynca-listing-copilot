...
const defaultVerificationBatchSize = 50;
const maxDeleteBatchSize = 500;
const maxVerificationBatchSize = 100;
...
function retentionVerificationIdentity(objectPath) {
  const safePath = String(objectPath || "");
  const tenantScoped = safePath.match(
    /^tenants\/([a-zA-Z0-9_-]{1,128})\/listing-assets\/(\d{4}-\d{2}-\d{2})\/([a-zA-Z0-9][a-zA-Z0-9_-]{0,127})\/([a-zA-Z0-9][a-zA-Z0-9._-]{0,255})$/
  );
  if (tenantScoped) {
    return {
      tenant_id: tenantScoped[1],
  ...
  const legacy = safePath.match(
    /^listing-assets\/(\d{4}-\d{2}-\d{2})\/([a-zA-Z0-9][a-zA-Z0-9_-]{0,127})\/([a-zA-Z0-9][a-zA-Z0-9._-]{0,255})$/
  );
  if (legacy) {
    return {
      tenant_id: "tenant_legacy",
  ...
  throw new Error("Listing image retention encountered an out-of-scope object path.");
}

function postgrestInFilter(values) {
  // Retention identities accept only path segments made from alphanumerics,
  // dot, underscore, and dash. Commas and PostgREST quoting characters are
  // rejected by retentionVerificationIdentity before this filter is built.
  return `in.(${values.join(",")})`;
}

function verificationIdentityBatches(objectPaths, batchSize) {
  const byTenant = new Map();
  for (const objectPath of objectPaths) {
    const identity = retentionVerificationIdentity(objectPath);
    const identities = byTenant.get(identity.tenant_id) || [];
    identities.push(identity);
    byTenant.set(identity.tenant_id, identities);
  }

  const batches = [];
  for (const [tenantId, identities] of byTenant) {
    for (const identityBatch of chunk(identities, batchSize)) {
      batches.push({ tenant_id: tenantId, identities: identityBatch });
    }
  }
  return batches;
}

async function invalidateListingImageVerificationRecords({
  config,
  objectPaths,
  fetchImpl,
  ...
  const endpointBase = `${normalizeStorageUrl(config.url)}/rest/v1/${verificationTable}`;
  const invalidatedPaths = new Set();

  for (const batch of verificationIdentityBatches(objectPaths, verificationBatchSize)) {
    const expectedPaths = new Set(batch.identities.map((identity) => identity.object_path));
    const endpoint = new URL(endpointBase);
    endpoint.searchParams.set("tenant_id", `eq.${batch.tenant_id}`);
    endpoint.searchParams.set("bucket", `eq.${config.bucket}`);
    endpoint.searchParams.set("object_path", postgrestInFilter([...expectedPaths]));
    ...
    const payload = await readJsonResponse(response, "Supabase verification store");
    if (!response.ok) {
      throw new Error(`Supabase verification invalidation failed: ${response.status} ${String(payload.message || payload.error || "").slice(0, 180)}`);
    }
    if (!Array.isArray(payload)) {
      throw new Error("Supabase verification invalidation returned an unexpected response.");
    }

    for (const row of payload) {
      const returnedPath = String(row?.object_path || "");
      if (
        row?.object_verified !== false
        || row?.canonical_eligible !== false
        || String(row?.tenant_id || "") !== batch.tenant_id
        || String(row?.bucket || "") !== config.bucket
        || !expectedPaths.has(returnedPath)
      ) {
        throw new Error("Supabase verification invalidation could not be confirmed.");
      }
      invalidatedPaths.add(returnedPath);
    }
  }
...
async function deleteStorageObjects({
  ...
  for (const batch of chunk(objectPaths, deleteBatchSize)) {
    ...
    const expectedPaths = new Set(batch);
    const confirmedPaths = payload
      .map((row) => String(row?.name || row?.path || "").trim())
      .filter(Boolean);
    if (confirmedPaths.some((objectPath) => !expectedPaths.has(objectPath))) {
      throw new Error("Supabase storage retention delete confirmed an out-of-scope object.");
    }
    for (const objectPath of confirmedPaths) {
      if (deletedObjects.includes(objectPath)) continue;
      deletedObjects.push(objectPath);
      deletedCount += 1;
    }
  }
...
export async function runListingImageRetentionCleanup({
  ...
  const verificationBatchSize = positiveInteger(
    env.LISTING_IMAGE_RETENTION_VERIFICATION_BATCH_SIZE,
    defaultVerificationBatchSize,
    { min: 1, max: maxVerificationBatchSize }
  );
  ...
  if (dryRun) {
    return {
      ...
      verification_batch_size: verificationBatchSize,
      verification_invalidation_confirmed: false,
      verification_invalidated_count: 0,
      deleted_count: 0,
      deleted_objects: []
    };
  }
  ...
  const config = listingImageStorageReadiness(env);
  const invalidatedPaths = await invalidateListingImageVerificationRecords({
    config,
    objectPaths,
    fetchImpl,
    verificationBatchSize,
  ...
  let deletion;
  try {
    deletion = await deleteStorageObjects({
      config,
      objectPaths,
      fetchImpl,
      deleteBatchSize
    ...
    throw new Error(`${String(error?.message || "Supabase storage retention delete failed.")} Verification records were invalidated first; retry cleanup to finish deleting retained objects.`);
  }

  const finalInvalidatedPaths = await invalidateListingImageVerificationRecords({
    config,
    objectPaths,
    fetchImpl,
    verificationBatchSize,
  ...
  for (const objectPath of finalInvalidatedPaths) invalidatedPaths.add(objectPath);
  ...
  return {
    ...
    verification_batch_size: verificationBatchSize,
    verification_invalidation_confirmed: true,
    verification_invalidated_count: invalidatedPaths.size,
...
export function summarizeListingImageRetentionCleanup(result = {}) {
  return {
    ...
    verification_invalidation_confirmed: result.verification_invalidation_confirmed === true,
    verification_invalidated_count: Number(result.verification_invalidated_count || 0),
