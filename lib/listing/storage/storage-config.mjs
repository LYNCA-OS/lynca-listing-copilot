...
const hardMaxSignedUrlTtlSeconds = 2 * 60 * 60;
...
export function listingImageStorageReadiness(env = process.env) {
  ...
  const signedUrlTtlSeconds = boundedNumberFromEnv(
    env,
    "LISTING_IMAGE_SIGNED_URL_TTL_SECONDS",
    defaultSignedUrlTtlSeconds,
    hardMaxSignedUrlTtlSeconds
  );
  ...
  if (!serviceRoleKey) missing.push("SUPABASE_SECRET_KEY_OR_SERVICE_ROLE_KEY");
