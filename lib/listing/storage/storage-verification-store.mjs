...
export async function saveListingImageVerificationRecord({
  ...
  const readReusableRecord = () => readListingImageVerificationRecord({
    tenantId: normalizedTenantId,
    assetId,
    imageId,
    role,
  ...
  // INSERT triggers validate the parent generation before Postgres resolves an
  // ON CONFLICT. A finalized generation therefore must be read first: trying
  // the duplicate INSERT and relying on DO NOTHING is already too late.
  const reusable = await readReusableRecord();
  ...
  const response = await fetchImpl(endpoint, {
    ...
    headers: supabaseHeaders(serviceRoleKey, {
      // Verification rows are immutable provenance. Re-verifying an existing
      // object must therefore be a no-op, not an UPDATE that trips the
  ...
  if (!response.ok) {
    const message = await response.text();
    // Close the read/insert race without weakening identity checks. If a
    // concurrent request inserted the exact immutable record, it is reusable.
    const raced = await readReusableRecord();
    if (raced.verified && raced.durable) {
      return {
        saved: true,
        durable: true,
        reused: true,
  ...
  const rows = await readResponseJson(response);
  const savedRecord = Array.isArray(rows) ? rows[0] : rows;
  if (!savedRecord) {
    const existing = await readReusableRecord();
    if (!existing.verified || !existing.durable) {
      throw new Error(`Existing image verification record is not reusable: ${existing.reason || "unknown"}`);
    }
    return {
      saved: true,
      durable: true,
...
export async function readCanonicalListingImageVerificationByIdentity({
  tenantId,
  assetId,
  imageId,
  role,
  ...
  if (!isSupabaseConfigured(env) || typeof fetchImpl !== "function") {
    return { verified: false, durable: false, reason: "verification_store_unavailable" };
  }
  const safePath = assertTenantListingAssetObjectPath({ tenantId, assetId, objectPath });
  const { url, serviceRoleKey } = supabaseConfig(env);
  const endpoint = new URL(`${url}/rest/v1/${verificationTable}`);
  endpoint.searchParams.set("select", [
    "tenant_id", "asset_id", "image_id", "storage_role", "image_generation_id",
    "object_path", "bucket", "content_type", "size", "width", "height",
    "content_sha256", "object_verified", "content_hash_verified",
    "dimension_source", "canonical_eligible", "verified_at"
  ].join(","));
  endpoint.searchParams.set("tenant_id", `eq.${safeString(tenantId)}`);
  endpoint.searchParams.set("asset_id", `eq.${normalizeListingAssetId(assetId)}`);
  endpoint.searchParams.set("image_id", `eq.${boundedImageId(imageId)}`);
  endpoint.searchParams.set("storage_role", `eq.${normalizeListingImageStorageRole(role)}`);
  endpoint.searchParams.set("object_path", `eq.${safePath}`);
  endpoint.searchParams.set("object_verified", "eq.true");
  endpoint.searchParams.set("canonical_eligible", "eq.true");
  endpoint.searchParams.set("limit", "2");
  const response = await fetchReadWithDeadlineRetry(fetchImpl, endpoint, {
    ...
    timeoutMs: Math.max(250, Number(env.LISTING_VERIFICATION_READ_TIMEOUT_MS) || 8000),
    attempts: env.LISTING_VERIFICATION_READ_ATTEMPTS || 2
  });
  if (!response.ok) {
    const message = await response.text();
    throw new Error(`Canonical image verification read failed: ${response.status} ${message.slice(0, 180)}`);
  }
  const rows = await readResponseJson(response);
  const matches = (Array.isArray(rows) ? rows : []).filter((row) => (
    ...
    && sha256Pattern.test(safeString(row.content_sha256).toLowerCase())
    && safeString(row.dimension_source) === "object_bytes"
  ));
  if (matches.length !== 1) {
    return { verified: false, durable: true, reason: matches.length ? "verification_record_ambiguous" : "verification_record_missing" };
  }
  return { verified: true, durable: true, record: matches[0] };
}

export async function readCanonicalListingImageVerificationBySlotContent({
  tenantId,
  assetId,
  role,
  bucket,
  ...
  const normalizedAssetId = normalizeListingAssetId(assetId);
  const normalizedRole = normalizeListingImageStorageRole(role);
  const normalizedHash = safeString(contentSha256).toLowerCase();
  if (!sha256Pattern.test(normalizedHash)) {
    return { verified: false, durable: true, reason: "content_sha256_required" };
  }
  const { url, serviceRoleKey } = supabaseConfig(env);
  const endpoint = new URL(`${url}/rest/v1/${verificationTable}`);
  endpoint.searchParams.set("select", [
  ...
  endpoint.searchParams.set("asset_id", `eq.${normalizedAssetId}`);
  endpoint.searchParams.set("storage_role", `eq.${normalizedRole}`);
  endpoint.searchParams.set("content_sha256", `eq.${normalizedHash}`);
  endpoint.searchParams.set("object_verified", "eq.true");
  endpoint.searchParams.set("content_hash_verified", "eq.true");
  endpoint.searchParams.set("canonical_eligible", "eq.true");
  endpoint.searchParams.set("limit", "2");
  const response = await fetchReadWithDeadlineRetry(fetchImpl, endpoint, {
    headers: supabaseHeaders(serviceRoleKey, { prefer: "return=representation" })
  ...
  if (!response.ok) {
    const message = await response.text();
    throw new Error(`Canonical image verification reuse read failed: ${response.status} ${message.slice(0, 180)}`);
  }
  const rows = await readResponseJson(response);
  const matches = (Array.isArray(rows) ? rows : []).filter((row) => metadataMatches(row, {
    tenantId: normalizedTenantId,
    assetId: normalizedAssetId,
    role: normalizedRole,
    objectPath: row.object_path,
