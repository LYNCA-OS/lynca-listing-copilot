...
// COS-53's recognition input: a bounded downscale, canonical but not a crop.
const readabilityDerivedRole = "readability_derived";
const sha256Pattern = /^[0-9a-f]{64}$/;

function isSupabaseConfigured(env = process.env) {
  return listingImageStorageReadiness(env).configured;
}
...
function supabaseConfig(env = process.env) {
  ...
  return { url: readiness.url, serviceRoleKey: readiness.service_role_key };
}
...
function fetchWithoutServiceKeyRedirect(fetchImpl) {
...
export async function canonicalListingCropMetadataForVerification({
  ...
  // COS-53: a bounded DOWNSCALE is a canonical derived asset too, and it is not
  // a crop -- no region, no bounds. It earns canonical eligibility through the
  // same verified-source lookup, because the thing that makes a derived asset
  // trustworthy is naming which verified original it came from, and that is
  // identical for both kinds.
  const isReadabilityDownscale = normalizedRole === readabilityDerivedRole;
  if (!isCanonicalCrop && !isReadabilityDownscale) {
    return {
      crop_metadata: null,
      image_generation_id: generationId,
  ...
  if (sourceImageId === derivedImageId) {
    throw new Error("Derived source_image_id must reference a different original image.");
  }
  let sourceRegion = "";
  let normalizedBounds = null;
  let longEdge = 0;
  let transformVersion;
  if (isReadabilityDownscale) {
    longEdge = Number(input.long_edge);
    if (!Number.isInteger(longEdge) || longEdge <= 0 || longEdge > 8192) {
      throw new Error("Invalid readability downscale long_edge.");
    }
    if (safeString(input.derived_role) !== readabilityDerivedRole) {
      throw new Error("Readability downscale must declare its derived_role.");
    }
    transformVersion = safeString(input.transform_version || "readability-downscale-v1");
  } else {
    sourceRegion = safeString(input.source_region);
    ...
    normalizedBounds = normalizedUnitBounds(input.normalized_bounds);
    transformVersion = safeString(input.transform_version || "field-crop-v1");
  }
  if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,79}$/.test(transformVersion)) {
    throw new Error("Invalid derived transform_version.");
  }

  const { url, serviceRoleKey } = supabaseConfig(env);
  const serviceFetch = fetchWithoutServiceKeyRedirect(fetchImpl);
  ...
  endpoint.searchParams.set("select", [
    ...
    "width",
    "height",
    // COS-53's byte rule needs the source's byte count, not only its pixels.
    "size"
  ].join(","));
  ...
  const sourceObjectPathChecked = sourceObjectPath;
  const derivedObjectPath = assertTenantListingAssetObjectPath({
    tenantId,
    assetId: normalizedAssetId,
    objectPath
  });
  if (isReadabilityDownscale) {
    ...
    if (!Number.isFinite(sourceSize) || sourceSize <= 0) {
      throw new Error("Downscale source size is invalid.");
    }
    return {
      crop_metadata: {
        derived_id: derivedImageId,
        generation_id: generationId,
        asset_id: normalizedAssetId,
        source_image_id: sourceImageId,
        source_object_path: sourceObjectPathChecked,
