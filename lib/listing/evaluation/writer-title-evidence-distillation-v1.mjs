...
// Keep this weak-supervision report independent from stopped residual-schema
// experiments. It needs only the stable SEM-facing projection used to compare
// a writer-title parse with the same-call canonical fields.
function projectCurrentCanonicalFields(fields = {}) {
  invariant(record(fields), "writer_distillation_candidate_fields_invalid");
  const components = new Set(cleanArray(fields.components || fields.attributes));
  return {
    ...
    subject: cleanArray(fields.subjects),
    ...
    grading_info: record(fields.grading_info) ? canonicalObject(fields.grading_info) : null,
    description: clean(fields.description),
    search_optimization: ["RC", "Auto", "Patch", "Relic"]
...
function projectCandidateFields(fields) {
  const projected = projectCurrentCanonicalFields(fields);
