...
function routeFromAgnesResult(result) {
  ...
  if (!predictionTitleFromAgnesResult(result)) return "MANUAL_REQUIRED";
...
function predictionFromReviewedEvaluation({ result, groundTruthFields, criticalFields }) {
  const resolvedFields = result?.status === "evaluated" ? predictionFieldsFromAgnesResult(result) : {};
  const exact = result?.status === "evaluated"
    ? fieldsExactForCriticalFields(groundTruthFields, resolvedFields, criticalFields)
    : false;
  const route = routeFromAgnesResult(result);
  ...
  return {
    ...
    final_title: result?.status === "evaluated" ? predictionTitleFromAgnesResult(result) : "",
    corrected_title: "",
...
export function buildHeldOutCommercialItemsFromReviewedEvaluation({
  ...
  const resultsById = agnesResultsByReviewedId(agnesReport);
  const items = [];
  const rejected_rows = [];
  const warnings = [];
  ...
  reviewedItems.forEach((item, index) => {
    ...
    if (!result) warnings.push(`${assetId}: no matching Agnes evaluation result; counted as NOT_EVALUATED failure`);
    const route = result
      ? routeFromAgnesResult(result)
      : "NOT_EVALUATED";
