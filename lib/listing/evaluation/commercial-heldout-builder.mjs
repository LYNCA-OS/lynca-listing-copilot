...
function confirmedReviewedValue(field) {
  if (!isPlainObject(field) || normalizeText(field.status).toUpperCase() !== "CONFIRMED") {
    return undefined;
  }
  return field.value;
}

function commercialGroundTruthFromReviewedFields(fields = {}) {
  const ground = {};
  const year = confirmedReviewedValue(fields.year);
  if (valuePresent(year)) ground.year = normalizeText(year);

  const subject = confirmedReviewedValue(fields.subject);
  if (subject !== undefined) {
    const players = arrayFromAny(subject).map(normalizeText).filter(Boolean);
    if (players.length) ground.players = players;
  }

  const productOrSet = confirmedReviewedValue(fields.product_or_set);
  if (productOrSet !== undefined) {
    if (isPlainObject(productOrSet)) {
      if (normalizeText(productOrSet.product)) ground.product = normalizeText(productOrSet.product);
      if (normalizeText(productOrSet.set)) ground.set = normalizeText(productOrSet.set);
    } else if (normalizeText(productOrSet)) {
      ground.product = normalizeText(productOrSet);
    }
  }

  const cardType = confirmedReviewedValue(fields.card_type);
  if (valuePresent(cardType)) ground.card_type = normalizeText(cardType);

  const variant = confirmedReviewedValue(fields.variant_or_parallel);
  if (variant !== undefined) {
    const exact = isPlainObject(variant)
      ? normalizeText(variant.exact || variant.value || variant.parallel)
      : normalizeText(variant);
    if (exact) ground.parallel = exact;
  }

  const collector = confirmedReviewedValue(fields.collector_number);
  if (valuePresent(collector)) ground.collector_number = normalizeText(collector);

  const serial = confirmedReviewedValue(fields.serial_number);
  if (valuePresent(serial)) ground.serial_number = normalizeSerial(serial);

  const grade = confirmedReviewedValue(fields.grade);
  if (grade !== undefined) {
    if (isPlainObject(grade)) {
      if (normalizeText(grade.company || grade.grade_company)) {
        ground.grade_company = normalizeText(grade.company || grade.grade_company);
      }
      if (normalizeText(grade.card_grade)) ground.card_grade = normalizeText(grade.card_grade);
      if (normalizeText(grade.auto_grade)) ground.auto_grade = normalizeText(grade.auto_grade);
      if (normalizeText(grade.grade_type)) ground.grade_type = normalizeText(grade.grade_type);
    } else if (normalizeText(grade)) {
      ground.card_grade = normalizeText(grade);
    }
  }
...
function imagesFromReviewedGroundTruthItem(item = {}) {
  return arrayFromAny(item.image_inputs || item.images).map((image) => {
    if (typeof image === "string") return imageRecordFromReviewedImage({ object_path: image });
    return imageRecordFromReviewedImage(image);
  }).filter(Boolean);
}

export function reviewedGroundTruthToCommercialManifest(labels = {}, {
  allowDevelopmentLabels = false
} = {}) {
  const sourceItems = Array.isArray(labels.items) ? labels.items : [];
  const items = [];
  const rejected_rows = [];
  const warnings = [];

  sourceItems.forEach((item, index) => {
    const assetId = normalizeId(firstValue(item.asset_id, item.card_id, item.physical_card_id));
    const commercial = item.commercial_heldout === true || labels.commercial_heldout === true;
    const reasons = [];

    if (!commercial && !allowDevelopmentLabels) {
      reasons.push("not marked commercial_heldout; development labels are not commercial evidence");
    }
    if (!assetId) reasons.push("missing asset_id");

    const groundTruth = commercialGroundTruthFromReviewedFields(plainObject(item.fields));
    if (!Object.keys(groundTruth).length) reasons.push("no CONFIRMED reviewed ground_truth fields");

    const images = imagesFromReviewedGroundTruthItem(item);
    ...
    if (!commercial && allowDevelopmentLabels) {
      warnings.push(`${assetId}: development label admitted only for an engineering-path dry-run`);
    }

    items.push({
      asset_id: assetId,
      source_feedback_id: normalizeLookupId(firstValue(
        item.source_feedback_id,
        item.card_id,
        item.asset_id
      )),
      review_id: normalizeText(firstValue(item.review_id, item.source_review_id, item.source_feedback_id, item.card_id)),
      category: normalizeText(item.category) || "sports_card",
      images,
      ground_truth: groundTruth,
      critical_fields: criticalFieldsForGroundTruth(groundTruth),
      difficulty_tags: arrayFromAny(item.difficulty_tags),
      capture_tags: arrayFromAny(item.capture_tags)
    });
  });

  return {
    manifest: {
      schema_version: "reviewed-commercial-manifest-from-ground-truth-v1",
      items
    },
    rejected_rows,
    warnings
...
export function buildHeldOutCommercialItemsFromReviewedGroundTruth({
  labels = {},
  providerReport = {},
  allowDevelopmentLabels = false
} = {}) {
  const converted = reviewedGroundTruthToCommercialManifest(labels, { allowDevelopmentLabels });
  const built = buildHeldOutCommercialItemsFromReviewedEvaluation({
    reviewedManifest: converted.manifest,
    providerReport
  });
  return {
    items: built.items,
    rejected_rows: [...converted.rejected_rows, ...built.rejected_rows],
    warnings: [...converted.warnings, ...built.warnings]
  };
}
