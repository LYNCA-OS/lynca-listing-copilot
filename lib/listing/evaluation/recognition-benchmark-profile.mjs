export const recognitionBenchmarkProfileIds = Object.freeze({
  ...
  COLD_SECOND_LOOK_SHADOW: "cold_second_look_shadow_benchmark",
...
function optionsFor(profile, phase) {
  ...
  if (profile === recognitionBenchmarkProfileIds.COLD_SECOND_LOOK_SHADOW) {
    return {
      disable_identity_result_cache_read: true,
      disable_identity_result_cache_write: true,
      disable_approved_identity_memory: true,
      ...
      recognition_worker_preflight_shadow_only: true,
      trace_level: "evaluation",
      enable_targeted_visual_assist_candidate: false,
      enable_second_look_shadow_candidate: true,
      enable_world_knowledge_assist_candidate: false,
      second_look_variant: "CARD_CODE_ONLY"
    };
  }
...
export function assertColdSecondLookShadowBenchmarkResult(result = {}) {
  if (resultBenchmarkProfile(result) !== recognitionBenchmarkProfileIds.COLD_SECOND_LOOK_SHADOW) {
    throw new Error("cold_second_look_shadow_profile_mismatch");
  }
  assertNoWholeJobRetry(result, "cold_second_look_shadow");
  if (identityCacheHit(result)) throw new Error("cold_second_look_shadow_identity_cache_hit");
  if (providerCallSkipped(result)) throw new Error("cold_second_look_shadow_provider_call_skipped");
  // The normal cold observation remains authoritative and is still exactly one
  // call. The optional second look is accounted only by its isolated ledger.
  if (providerCalls(result) !== 1) {
    throw new Error(`cold_second_look_shadow_baseline_provider_calls_expected_1_received_${providerCalls(result)}`);
  }
  if (implicitProviderRetryAttempted(result)) {
    throw new Error("cold_second_look_shadow_implicit_provider_retry_forbidden");
  }
  const shadow = result.second_look_shadow;
  if (!shadow || typeof shadow !== "object") throw new Error("cold_second_look_shadow_trace_required");
  const ledger = Array.isArray(shadow.provider_call_ledger) ? shadow.provider_call_ledger : [];
  if (ledger.length !== 1) throw new Error("cold_second_look_shadow_call_ledger_expected_one_row");
  if (ledger.some((row) => (
    row?.logical_stage !== "TARGETED_SECOND_LOOK_CARD_CODE"
    || row?.attempt !== 1
    || ![0, 1].includes(Number(row?.provider_calls))
    || !["COMPLETED", "FAILED", "SKIPPED"].includes(String(row?.status || ""))
    || typeof row?.call_attempted !== "boolean"
    || row?.accounting_complete !== true
    || Number(row?.provider_calls) !== (row?.call_attempted ? 1 : 0)
    || !String(row?.started_at || "").trim()
    || !String(row?.completed_at || "").trim()
    || !Number.isFinite(Date.parse(row.started_at))
    || !Number.isFinite(Date.parse(row.completed_at))
    ...
    || row?.fallback === true
  ))) throw new Error("cold_second_look_shadow_call_ledger_invalid");
  if (Number(shadow.paid_provider_calls) !== Number(ledger[0].provider_calls)) {
    throw new Error("cold_second_look_shadow_paid_call_ledger_mismatch");
  }
  if (result.total_provider_calls !== null && result.total_provider_calls !== undefined
    && Number(result.total_provider_calls) !== 1 + Number(ledger[0].provider_calls)) {
    throw new Error("cold_second_look_shadow_total_provider_call_mismatch");
  }
  if (ledger.some((row) => String(row?.logical_stage || "") === "WORLD_KNOWLEDGE_ASSIST")) {
    throw new Error("cold_second_look_shadow_world_knowledge_call_forbidden");
  }
  if (shadow.full_provider_fallback_attempted === true || shadow.retry_attempted === true) {
    throw new Error("cold_second_look_shadow_retry_or_fallback_forbidden");
  }
  if (shadow.production_effect !== "NONE" || shadow.title_effect !== "NONE" || shadow.resolver_effect !== "PROPOSAL_ONLY") {
    throw new Error("cold_second_look_shadow_authority_boundary_invalid");
  }
  if (shadow.natural_language_model_response_persisted !== false) {
    throw new Error("cold_second_look_shadow_natural_language_persistence_forbidden");
  }
  if (shadow.baseline_unchanged !== true) throw new Error("cold_second_look_shadow_baseline_state_mutated");
  if (String(shadow.baseline_title || "") !== String(result.final_title || result.title || "")) {
    throw new Error("cold_second_look_shadow_baseline_title_mutated");
  }
  return true;
}
