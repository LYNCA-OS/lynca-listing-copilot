...
function directObservationProjection(result = {}, candidateControl = {}) {
  return withoutVolatileData({
    ...
    pre_application_evidence_snapshot:
      candidateControl.candidate_pre_application_evidence_snapshot || null,
    observation_status: candidateControl.candidate_observation_snapshot_status || "UNKNOWN",
    observation_reason: candidateControl.candidate_observation_snapshot_reason || null,
...
export async function buildRetrievalApplicationReplay({
  ...
  const candidateControl = buildCandidateSelectionPass({
    ...
    catalogContext,
    vectorContext,
    authoritativeCurrentImageContext: result.current_image_context || {}
  });
