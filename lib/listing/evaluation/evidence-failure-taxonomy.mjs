...
function ocrRendezvous(smoke = {}) {
  return smoke.preingestion_ocr_rendezvous
    || smoke.l2_status?.preingestion_ocr_rendezvous
    || {};
}
...
function bestSmokeForTrace(smokeRows = [], trace = {}) {
  ...
  const score = (row) => {
    ...
    // Evidence diagnosis must select the replay with the richest sensor trace.
    // Candidate/selection overlap is only a tie-breaker; otherwise an older
    // decision-rich replay can hide a newer completed crop trace.
    return (Array.isArray(rendezvous.raw_ocr_observations) ? rendezvous.raw_ocr_observations.length : 0) * 100_000_000
      + (Array.isArray(rendezvous.job_observability) ? rendezvous.job_observability.length : 0) * 1_000_000
      + (Array.isArray(rendezvous.patch_fields) ? rendezvous.patch_fields.length : 0) * 10_000
      + (row?.ok === true ? 1_000 : 0)
      + (selectedMatch ? 100 : 0)
      + overlap;
  };
...
function classifyMissing({ field, truth, trace, smoke }) {
  ...
  const jobObservability = rendezvous.job_observability;
  const rawOcrObservations = rendezvous.raw_ocr_observations;
  ...
  if (ocrReadableFields.has(field) && !Array.isArray(jobObservability) && !Array.isArray(rawOcrObservations)) {
    const rendezvousReason = clean(rendezvous.reason);
    const rendezvousStatus = clean(rendezvous.status).toUpperCase();
    return {
      category: "TRACE_MISSING",
      confidence: "HIGH",
...
export function buildEvidenceFailureTaxonomy({ dataset = {}, audit = {}, trace = {}, smoke = {}, generatedAt = new Date().toISOString() } = {}) {
  ...
  for (const card of rows(audit)) {
    ...
    for (const [field, result] of Object.entries(card.fields || {})) {
      ...
      failures.push({
        query_card_id: id,
        item_category: datasetById.get(id)?.category
          || datasetById.get(id)?.reviewed_ground_truth?.fields?.ip_sport?.reviewed_value
          || null,
        field,
        truth: result.truth,
        ...classification
