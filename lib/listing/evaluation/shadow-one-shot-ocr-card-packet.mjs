...
// Region observations are transport/evidence telemetry emitted alongside
// semantic field patches. They must remain available for audit, but they are
// neither a candidate field nor a role leak.
const nonSemanticTraceFields = new Set(["region_observation"]);
...
export async function executeShadowOneShotOcrCardPacket({
  ...
  for (let index = 0; index < packet.jobs.length; index += 1) {
    ...
    const semanticPatches = patches.filter((patch) => (
      !nonSemanticTraceFields.has(cleanText(patch.field).toLowerCase())
    ));
    tracePatches.push(...patches.filter((patch) => (
      nonSemanticTraceFields.has(cleanText(patch.field).toLowerCase())
    )));
    const legalPatches = semanticPatches.filter((patch) => allowed.has(cleanText(patch.field).toLowerCase()));
    const leakedFields = [...new Set(semanticPatches
