...
function prepareAnnotation(annotation, index, criticalFields) {
  ...
  const key = conceptId ? `${clean(annotation.field)}:${conceptId}` : claimKey(annotation);
  if (!clean(annotation.field) || !key) throw new Error("annotation_field_and_identity_required");
  return {
    ...annotation,
...
function preparePrediction(prediction, index) {
  ...
  const key = conceptId ? `${clean(prediction.field)}:${conceptId}` : claimKey(prediction);
  if (!clean(prediction.field) || !key) throw new Error("prediction_field_and_identity_required");
  return {
    ...prediction,
