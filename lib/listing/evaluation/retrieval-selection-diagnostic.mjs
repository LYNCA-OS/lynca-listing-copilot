...
function sourceGroup(candidate = {}) {
  ...
  const provenanceType = clean(candidate.catalog_provenance?.source_type).toUpperCase();
  const provenanceStatus = clean(candidate.catalog_provenance?.source_status).toUpperCase();
  ...
  if (/OFFICIAL/.test(provenanceType) || /OFFICIAL/.test(provenanceStatus)) return "official_catalog";
  if (/INTERNAL_CORRECTED_TITLE/.test(provenanceType) || provenanceStatus === "REVIEWED_INTERNAL") return "internal_reviewed_history";
  if (type === "CATALOG") return "community_catalog";
  return "unknown";
}
...
function isConfirmed(item, field) {
  return clean(
    item.reviewed_ground_truth?.field_statuses?.[field]
    || item.reviewed_ground_truth?.fields?.[field]?.reviewed_status
...
function cohortLabels(item = {}, smoke = {}) {
  const category = clean(
    item.category
    || item.reviewed_ground_truth?.fields?.ip_sport?.reviewed_value
  ).toLowerCase() || "unknown";
...
export function buildRetrievalSelectionDiagnostic({ dataset = {}, audit = {}, trace = {}, smoke = {}, generatedAt = new Date().toISOString() } = {}) {
  ...
  const cards = rows(dataset).flatMap((item) => {
    const id = idOf(item);
    if (item.retrieval_ground_truth?.retrieval_evaluable !== true) return [];
    const auditCard = auditById.get(id);
    if (!auditCard) return [];
    ...
    const sealedIds = new Set(item.retrieval_ground_truth?.sealed_source_candidate_ids || []);
    const eligibleCandidates = candidates.filter((candidate) => !sealedIds.has(identityOf(candidate)));
    const correct = eligibleCandidates.find((candidate) => clean(candidate.candidate_id) === clean(auditCard.correct_candidate_id)) || null;
    const selected = clean(cardTrace.selected_candidate_id);
    const selectedCorrect = auditCard.selected_candidate_correct === true;
    return [{
      query_card_id: id,
      ...
      selection_evaluable: auditCard.selected_candidate_correct !== null,
      ...
      candidates: eligibleCandidates.map((candidate) => ({
        ...
        correct: Boolean(correct && clean(candidate.candidate_id) === clean(correct.candidate_id))
      }))
    }];
  });
  ...
  const cohorts = Object.fromEntries(cohortNames.map((name) => {
    ...
    return [name, {
      ...
      selection_given_recall: metric(members.filter((card) => (
        card.selection_evaluable
        && card.correct_candidate_rank !== null
        && card.correct_candidate_rank <= 20
      )), (card) => card.selected_correct)
    }];
  }));
  const selectionOpportunities = cards.filter((card) => (
    card.selection_evaluable
    && card.correct_candidate_rank !== null
    && card.correct_candidate_rank <= 20
  ));
  ...
  for (const card of rows(audit)) {
    ...
    for (const [field, result] of Object.entries(card.fields || {})) {
      if (result.downstream_trace_eligible !== true || result.application_opportunity !== true) continue;
  ...
  return {
    ...
    selection: {
      ...
      excluded_contaminated_selection_count: cards.filter((card) => !card.selection_evaluable).length,
