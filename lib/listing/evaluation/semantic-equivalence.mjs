...
function foldOrthography(text) {
  return foldSeasonSpan(String(text ?? ""))
    .replace(/[‘’ʼ]/g, "'")
    .replace(/[“”]/g, '"')
    .replace(/[‐-―]/g, "-")
    .normalize("NFD")
 ...
 */
export function applyUnobtainableFacts(wanted, referenceText) {
  ...
  if (![...drop].some((t) => wanted.has(t))) return wanted;
  return new Set([...wanted].filter((t) => !drop.has(t)));
}
...
export const EQUIVALENCE_VERSION = `sem-equiv-1+${createHash("sha256")
  ...
  .update(JSON.stringify({
    ...
    folds: [foldSeasonSpan.toString(), foldOrthography.toString(), foldPlural.toString(),
...
export function scoreWithEquivalence(reference, title) {
  ...
  const wanted = applyUnobtainableFacts(equivalenceTokens(reference), reference);
  const got = applyLotFormatTolerance(wanted,
    applyHypernymRedundancy(wanted, equivalenceTokens(title)), reference);
  return {
