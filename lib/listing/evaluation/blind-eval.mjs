...
export function deterministicHashShuffle(values = [], seed = "", keyFor = (value) => value) {
  const normalizedSeed = normalizeText(seed);
  if (!normalizedSeed) return [...values];
  return [...values]
    .map((value, index) => ({
      value,
      index,
      rank: crypto.createHash("sha256")
        .update(`${normalizedSeed}:${normalizeText(keyFor(value))}`)
        .digest("hex")
    }))
...
export const maximumBlindEvalListings = 1000;

export function normalizeBlindEvalLimit(value, fallback = 2) {
  return Math.max(1, Math.min(maximumBlindEvalListings, Number(value) || fallback));
}

async function mapWithConcurrency(items = [], concurrency = 1, worker) {
  const results = new Array(items.length);
  let nextIndex = 0;
  const workerCount = Math.min(items.length, Math.max(1, Math.trunc(Number(concurrency) || 1)));
...
export async function loginSessionToCloud({
  baseUrl,
  username,
  password,
  env = process.env,
  ...
  const body = await parseJsonResponse(response, "Cloud login");
...
export async function loginToCloud(options = {}) {
  const session = await loginSessionToCloud(options);
...
export function assertSellerListing(listing = {}, expectedSeller = "dcsports87") {
  ...
  const normalizedExpectedSeller = normalizeText(expectedSeller).toLowerCase();
  if (normalizedExpectedSeller && seller !== normalizedExpectedSeller) {
    throw new Error(`Rejected non-${expectedSeller} listing: seller=${listing.seller || "(missing)"}`);
  }
  if (!normalizedExpectedSeller && !seller) throw new Error("Listing is missing a verifiable seller.");
...
const sealedProductTitlePatterns = Object.freeze([
  /\bfactory\s+sealed\b/i,
  /\bsealed\s+(?:hobby\s+)?(?:box|case|pack|display)\b/i,
  /\b(?:hobby|blaster|mega|booster|retail|display)\s+(?:box|case|pack)\b/i,
  /\b(?:box|case)\s+of\s+\d+\b/i,
  /\b\d+\s+(?:hobby\s+)?boxes?\b/i,
  /\b\d+\s+(?:box|pack)\s+case\b/i
]);

const unsuitableEvaluationListingPatterns = Object.freeze([
  ["buyer_choice_listing", /\b(?:you|u)\s+pick\b|\bpick\s+(?:your|any|from)\b|\bchoose\s+(?:your|any|from)\b/i],
  ["catalog_wide_choice_listing", /\bevery\s+card\s+ever\b|\bcomplete\s+your\s+set\b|\bbuy\s+more\s+(?:&|and)\s+save\b/i],
  ["title_card_range", /(?:^|\s)#?[a-z0-9]+\s*[-–—]\s*#[a-z0-9]+(?:\s|$)/i],
  ["multi_card_lot", /\b(?:lot\s+of|card\s+lot|bulk\s+cards?|team\s+set|complete\s+set|mystery\s+(?:pack|box|lot)|(?:card|sticker)\s+(?:pack|lot|bundle))\b|\bset\s+(?:of|de)\s+\d+\s+(?:cards?|stickers?)\b|\bpack\s+(?:of\s+)?\d+\s+(?:cards?|stickers?)\b|\b\d+\s+(?:cards?|stickers?)(?:\s+[a-z]+){0,3}\s+(?:pack|lot|bundle|set)\b|^\s*(?:lot\s*)?x\s*\d+\b/i],
  ["random_hit_pack", /\bguaranteed\b.{0,40}\b(?:auto(?:graph)?|numbered|parallel|hit|relic|patch)\b/i],
  ["card_supply", /\b(?:card\s+dividers?|team\s+labels?|top\s*loaders?|penny\s+sleeves?|card\s+holders?|storage\s+box|binders?|albums?)\b/i],
  ["break_or_randomized_listing", /\b(?:box|case|pack)\s+break\b|\brandom\s+(?:card|team|player)\b/i]
]);

export function isSealedProductListing(listing = {}) {
  const title = normalizeText(listing?.title);
  return Boolean(title && sealedProductTitlePatterns.some((pattern) => pattern.test(title)));
}

export function classifyBlindEvaluationListing(listing = {}) {
  const title = normalizeText(listing?.title);
  if (!title) return { eligible: false, reason: "missing_title" };
  if (normalizeText(listing?.item_group_href) || normalizeText(listing?.item_group_type)) {
    return { eligible: false, reason: "variation_group_listing" };
  }
  const match = unsuitableEvaluationListingPatterns.find(([, pattern]) => pattern.test(title));
  if (match) return { eligible: false, reason: match[0] };
  return { eligible: true, reason: "specific_card_listing" };
}

export function filterBlindEvaluationListings(listings = []) {
  const kept = [];
  const discarded = [];
  const discardReasons = {};
  for (const listing of listings) {
    const classification = classifyBlindEvaluationListing(listing);
    if (classification.eligible) {
      kept.push(listing);
      continue;
    }
    discarded.push({
      item_id: normalizeText(listing?.item_id),
...
export async function prepareBlindDataset({
  ...
  query = "",
  sportsOnly = false,
  excludeSealedProducts = false,
  requireSpecificCardListing = true,
  allowPartial = false,
  categoryIds = "",
  ...
  const safeLimit = normalizeBlindEvalLimit(limit, 2);
  ...
  const randomBlindSelection = normalizedSampleMode === "RANDOM_BLIND";
  const normalizedSampleSeed = normalizeText(sampleSeed)
    || normalizeText(runId)
    || crypto.randomUUID();
  const normalizedExpectedSeller = normalizeText(expectedSeller).toLowerCase();
  const normalizedListingsEndpoint = normalizeText(listingsEndpoint)
    || (!normalizedExpectedSeller
      ? "/api/ebay-card-listings"
      : normalizedExpectedSeller === "dcsports87"
      ? "/api/ebay-dcsports87-listings"
      : "/api/ebay-seller-listings");
  const exclusionSourceCount = excludeAnswerKeyPaths.map(normalizeText).filter(Boolean).length
    + (excludedItemIds.length ? 1 : 0);
  const cookie = await loginToCloud({ baseUrl: normalizedBaseUrl, username, password, env, requestTimeoutMs, fetchImpl });
  let queryList = normalizeQueryList(query, "card");
  // Specific search terms improve sample diversity, but a seller can have
  // valid cards whose titles do not contain any requested sport or TCG term.
  // Keep a broad final lookup; seller, novelty, and single-card filters still
  // fail closed before anything enters the blind bundle.
  if (!queryList.some((item) => item.toLowerCase() === "card")) queryList.push("card");
  if (randomBlindSelection) {
    queryList = deterministicHashShuffle(queryList, `${normalizedSampleSeed}:queries`, (item) => item);
  }
  const maximumFetchLimit = normalizedListingsEndpoint === "/api/ebay-card-listings" ? 200 : 50;
  const fetchLimit = Math.min(
    maximumFetchLimit,
    Math.max(safeLimit, Math.min(maximumFetchLimit, safeLimit + excluded.size + (sportsOnly ? 40 : 20)))
  );
  const targetListingPool = randomBlindSelection
    ? Math.max(safeLimit, Math.min(maximumBlindEvalListings * 2, safeLimit * 2))
    : safeLimit;
  const maxListingPagesPerQuery = Math.max(1, Math.min(50, Number(env.BLIND_EVAL_LISTING_PAGES || 3) || 3));
  const imageDownloadConcurrency = Math.max(1, Math.min(16, Number(env.BLIND_EVAL_IMAGE_DOWNLOAD_CONCURRENCY || 6) || 6));
  ...
  let sealedProductDiscardedCount = 0;
  ...
  for (const activeQuery of queryList) {
    ...
    while (listings.length < targetListingPool && pagesForQuery < maxListingPagesPerQuery && moreResultsAvailable) {
      ...
      if (normalizedListingsEndpoint === "/api/ebay-seller-listings" && normalizedExpectedSeller) {
        listingSearchParams.set("seller", normalizedExpectedSeller);
      }
      if (sportsOnly) listingSearchParams.set("sports_only", "1");
      if (categoryIds) listingSearchParams.set("category_ids", categoryIds);

      const listingsResponse = await fetchWithTimeout(fetchImpl, `${normalizedBaseUrl}${normalizedListingsEndpoint}?${listingSearchParams}`, {
        headers: {
          cookie,
          ...optionalProtectionHeaders(env)
        }
      }, requestTimeoutMs, `${normalizedExpectedSeller} listings`);
      listingsPayload = await parseJsonResponse(listingsResponse, `${normalizedExpectedSeller} listings`);
      listingPageCount += 1;
      pagesForQuery += 1;
      ...
      const cardListings = excludeSealedProducts
        ? candidateListings.filter((listing) => !isSealedProductListing(listing))
        : candidateListings;
      sealedProductDiscardedCount = candidateListings.length - cardListings.length;
      const evaluationListingFilter = requireSpecificCardListing
        ? filterBlindEvaluationListings(cardListings)
        : {
      ...
      sportsFilter = sportsOnly
        ? filterSportsCardListings(evaluationListingFilter.listings)
        : {
          listings: evaluationListingFilter.listings,
          discarded: [],
          discarded_count: 0,
          discard_reasons: {},
          filter_version: sportsCardFilterVersion
        };
      sportsFilter.evaluation_listing_filter = evaluationListingFilter;
    ...
    if (listings.length >= targetListingPool) break;
  }

  if (!listings.length) throw new Error(`${normalizedExpectedSeller} listings endpoint returned zero listings.`);
  if (listings.length < safeLimit && !allowPartial) {
  ...
  const selectedListingOrder = randomBlindSelection
    ? deterministicHashShuffle(listings, `${normalizedSampleSeed}:listings`, (listing) => listing.item_id)
    : listings;
  let listingCursor = 0;
  while (blindRows.length < safeLimit && listingCursor < selectedListingOrder.length) {
    const batch = selectedListingOrder.slice(listingCursor, listingCursor + imageDownloadConcurrency * 2);
    listingCursor += batch.length;
    const downloaded = await mapWithConcurrency(batch, imageDownloadConcurrency, async (listing) => {
      assertSellerListing(listing, normalizedExpectedSeller);
      ...
      try {
        ...
        const blindRow = { case_id: caseId, image_paths: imagePaths };
        assertBlindInputRow(blindRow, 0);
        return {
          blindRow,
          quality,
          answerRow: {
            case_id: caseId,
            seller: normalizedExpectedSeller || normalizeText(listing.seller).toLowerCase(),
            item_id: listing.item_id,
    ...
    for (const result of downloaded) {
      if (result.failure) {
        skippedImageDownloads.push(result.failure);
        continue;
      }
      if (blindRows.length >= safeLimit) break;
      blindRows.push(result.blindRow);
      answerRows.push(result.answerRow);
      downloadedImageQuality.push(...result.quality);
    }
  }

  if (blindRows.length < safeLimit && (!allowPartial || !blindRows.length)) {
  ...
  const manifest = {
    ...
    seller: normalizedExpectedSeller || null,
    listings_endpoint: normalizedListingsEndpoint,
    listing_count: blindRows.length,
    requested_listing_count: safeLimit,
    partial_dataset: blindRows.length < safeLimit,
    ...
    evaluation_sample_policy: buildEvaluationSamplePolicy({
      ...
      selectedItemIds: answerRows.map((row) => row.item_id),
      exclusionSourceCount,
      sampleSeed: normalizedSampleSeed,
      selectionStrategy: randomBlindSelection ? "deterministic_hash_shuffle" : ""
    }),
    sample_selection: {
      randomized: randomBlindSelection,
      strategy: randomBlindSelection ? "deterministic_hash_shuffle" : "source_order",
      seed: randomBlindSelection ? normalizedSampleSeed : null,
      eligible_candidate_count: listings.length,
      target_candidate_pool_size: targetListingPool
    },
    ...
    sealed_products_excluded: Boolean(excludeSealedProducts),
    sealed_product_discarded_count: sealedProductDiscardedCount,
    specific_card_listing_required: Boolean(requireSpecificCardListing),
    unsuitable_listing_discarded_count: sportsFilter.evaluation_listing_filter?.discarded_count || 0,
    unsuitable_listing_discard_reasons: sportsFilter.evaluation_listing_filter?.discard_reasons || {},
    ...
    image_download_concurrency: imageDownloadConcurrency,
    ...
    ground_truth_policy: {
      ...
      ebay_answer_key_use: "sealed_market_reference_for_post_run_manual_review_only",
      supabase_corrected_title_reviewed_ground_truth: false,
...
async function verifyExistingLocalImageOnCloud({
  ...
  return {
    ...
    object_path: verification.object_path,
    role,
    capture_angle: role.startsWith("image_") ? role.replace(/_original$/, "") : "detail",
    storageVerified: true,
    storage_verified: true,
    storageVerificationToken: verification.verification_token,
    storage_verification_token: verification.verification_token,
...
function blindProviderOptions(providerMode = "openai_vector", env = process.env) {
  const cGroup = normalizeText(providerMode || "openai_vector").toLowerCase() !== "openai_baseline";
  const forceVectorAssist = cGroup && (
    truthy(env.BLIND_EVAL_FORCE_VECTOR_ASSIST)
    || truthy(env.FORCE_VECTOR_ASSIST)
  );
  const disableVectorLazyMode = cGroup && (
    forceVectorAssist
    || truthy(env.BLIND_EVAL_DISABLE_VECTOR_LAZY_MODE)
    || truthy(env.DISABLE_VECTOR_LAZY_MODE)
  );
  const vectorIndexReady = cGroup && (
    truthy(env.BLIND_EVAL_VECTOR_INDEX_READY)
    || truthy(env.VECTOR_INDEX_READY)
  );
  return {
    ...
    enable_vector_lazy_mode: cGroup ? !disableVectorLazyMode : false,
    force_vector_assist: forceVectorAssist,
    vector_index_ready: vectorIndexReady ? true : undefined,
    vector_corrected_title_as_temporary_gt: false,
    vector_query_timeout_ms: cGroup ? Math.max(1, Number(env.CLOUD_LISTING_API_VECTOR_QUERY_TIMEOUT_MS || env.VECTOR_QUERY_TIMEOUT_MS || 120000)) : undefined,
...
function blindRecognitionErrorType(error = {}) {
  ...
  if (/abort|timeout|timed out|ETIMEDOUT/i.test(text)) return "network_timeout";
  if (/fetch failed|network|socket|ECONNRESET|EAI_AGAIN|ENOTFOUND|ECONNREFUSED/i.test(text)) return "network_fetch_failed";
  if (/login|unauthorized|forbidden|401|403/i.test(text)) return "cloud_auth_failed";
  return "cloud_request_failed";
}

function retryableBlindRecognitionError(error = {}) {
  const type = blindRecognitionErrorType(error);
  return type === "network_fetch_failed"
    || type === "network_timeout"
    || type === "cloud_request_failed";
}

function retryableExistingBlindPrediction(prediction = {}) {
  const status = normalizeText(prediction.recognition_status).toUpperCase();
  const errorType = normalizeText(prediction.error_type).toLowerCase();
  const httpStatus = Number(prediction.http_status || 0);
  if (status !== "FAILED" && !errorType) return false;
  if ([408, 409, 425, 429, 500, 502, 503, 504].includes(httpStatus)) return true;
  return /network_fetch_failed|network_timeout|cloud_request_failed|http_(?:408|409|425|429|5\d\d)/i.test(errorType);
}
...
function packetCandidateDebugRows(packet = {}, limit = 8) {
  const candidates = Array.isArray(packet?.vector_retrieval?.candidates)
  ...
  return candidates.slice(0, Math.max(1, Number(limit) || 8)).map((candidate) => ({
    ...
    candidate_identity_id: normalizeText(candidate.candidate_identity_id),
    source_trust: normalizeText(candidate.source_trust),
    reference_title: normalizeText(candidate.reference_title).slice(0, 180),
    ...
    conflicting_fields: Array.isArray(candidate.conflicting_fields) ? candidate.conflicting_fields : [],
    soft_conflicting_fields: Array.isArray(candidate.soft_conflicting_fields) ? candidate.soft_conflicting_fields : [],
    prompt_admitted: candidate.prompt_admitted === true
      || candidate.prompt_candidate === true
      || candidate.used_in_prompt === true
      || candidate.included_in_prompt === true,
    prompt_blocked: candidate.prompt_blocked === true
      || candidate.anchor_agreement?.prompt_hard_filter_pass === false
      || false,
    anchor_agreement: candidate.anchor_agreement && typeof candidate.anchor_agreement === "object" ? {
      agreed: Array.isArray(candidate.anchor_agreement.agreed) ? candidate.anchor_agreement.agreed : [],
      contradicted: Array.isArray(candidate.anchor_agreement.contradicted) ? candidate.anchor_agreement.contradicted : [],
      exact_code_match: candidate.anchor_agreement.exact_code_match === true,
      prompt_hard_filter_applicable: candidate.anchor_agreement.prompt_hard_filter_applicable === true,
      prompt_hard_filter_pass: candidate.anchor_agreement.prompt_hard_filter_pass === true
    } : null,
    fields: candidate.fields && typeof candidate.fields === "object" ? {
      year: candidate.fields.year || null,
      brand: candidate.fields.brand || candidate.fields.manufacturer || null,
      product: candidate.fields.product || null,
      set: candidate.fields.set || null,
      subjects: candidate.fields.subjects || candidate.fields.players || null,
      collector_number: candidate.fields.collector_number || candidate.fields.card_number || null,
      checklist_code: candidate.fields.checklist_code || null,
      expected_serial_denominator: candidate.fields.expected_serial_denominator || null,
      surface_color: candidate.fields.surface_color || null
    } : null
  }));
}
...
export async function runBlindRecognition({
  ...
  const allRows = await readJsonl(resolvedInputPath);
  assertBlindInputRows(allRows);
  const limitCount = Math.max(0, Math.trunc(Number(limit) || 0));
  const rows = limitCount > 0 ? allRows.slice(0, limitCount) : allRows;
  const normalizedBaseUrl = normalizeBaseUrl(baseUrl);
  const cGroup = normalizeText(providerMode || "openai_vector").toLowerCase() !== "openai_baseline";
  if (cGroup && truthy(env.BLIND_EVAL_REQUIRE_VECTOR_INDEX_READY) && !truthy(env.BLIND_EVAL_VECTOR_INDEX_READY) && !truthy(env.VECTOR_INDEX_READY)) {
    throw new Error("BLIND_EVAL_REQUIRE_VECTOR_INDEX_READY is enabled, but VECTOR_INDEX_READY/BLIND_EVAL_VECTOR_INDEX_READY is not true.");
  }
  const loginSession = await loginSessionToCloud({
  ...
  if (!loginSession.tenant_id) {
    throw new Error("Cloud login succeeded but no tenant identity was returned.");
  }
  ...
  const existingByCaseId = new Map(existingPredictions
    ...
    .filter((prediction) => !retryableExistingBlindPrediction(prediction))
    .map((prediction) => [normalizeText(prediction.case_id), prediction]));
  const predictions = Array.from({ length: rows.length });
  const maxRecognitionAttempts = Math.max(1, Number(env.BLIND_EVAL_RECOGNITION_ATTEMPTS || 2));
  const workerCount = Math.max(1, Math.min(Math.trunc(Number(concurrency) || 1), rows.length || 1));
  let cursor = 0;
  ...
  async function persistPredictions() {
    persistQueue = persistQueue.then(() => writeJsonl(outputPath, orderedPredictions())).catch(() => {});
    await persistQueue;
  }

  async function recognizeOne(row, index) {
    const existingPrediction = existingByCaseId.get(normalizeText(row.case_id));
    if (existingPrediction) {
      predictions[index] = existingPrediction;
      if (typeof onProgress === "function") {
        onProgress({
          ...
          total: rows.length,
          skipped: true,
          prediction_count: orderedPredictions().length
        });
      }
      return;
    }
    ...
    try {
      ...
      for (const [imageIndex, imagePath] of row.image_paths.entries()) {
        images.push(await uploadLocalImageToCloud({
          baseUrl: normalizedBaseUrl,
          cookie,
          tenantId: loginSession.tenant_id,
          caseId: row.case_id,
          imagePath,
          imageIndex,
          env,
      ...
      const payload = {
        ...
        provider_options: blindProviderOptions(providerMode, env),
      ...
      for (let attempt = 1; attempt <= maxRecognitionAttempts; attempt += 1) {
        ...
        try {
          ...
          if (!retryableBlindRecognitionError(error) || attempt >= maxRecognitionAttempts) throw error;
      ...
      predictions[index] = {
        ...
        c_group_diagnostics: {
          ...
          catalog_anchor_plan: data.catalog_anchor_plan || null,
          catalog_candidate_debug: packetCandidateDebugRows(data.catalog_candidate_packet),
          vector_assist_eligibility: data.vector_assist_eligibility || null,
          vector_candidate_debug: packetCandidateDebugRows(data.vector_candidate_packet),
      ...
      const errorType = blindRecognitionErrorType(error);
      predictions[index] = {
        case_id: row.case_id,
        recognition_output: recognitionOutputFromCloudData({}),
        model_confidence: "FAILED",
        model_notes: [normalizeText(error?.message || error).slice(0, 240)].filter(Boolean),
        provider,
        model_id: null,
        provider_mode: providerMode,
        ...
        timing: {
          ...
          blind_recognition_attempts: 0
        },
        usage: null,
        c_group_diagnostics: {
          catalog_prompt_assist_used: null,
          vector_prompt_assist_used: null,
          retrieval_title_assist_used: null,
          catalog_assist_eligibility: null,
          catalog_anchor_plan: null,
          catalog_candidate_debug: [],
          vector_assist_eligibility: null,
          vector_candidate_debug: [],
          catalog_retrieval_metrics: null,
          vector_retrieval_metrics: null,
          open_set_decision: null,
          retrieval_sources: []
        },
        used_inputs: {
          image_paths: row.image_paths
    ...
    if (typeof onProgress === "function") {
      ...
      onProgress({
        ...
        skipped: false,
        prediction_count: orderedPredictions().length,
        attempts: prediction.timing?.blind_recognition_attempts || 0,
        error_type: prediction.error_type || null,
        recognition_status: prediction.recognition_status || null
      });
    }
  }
  ...
  async function worker() {
    while (cursor < rows.length) {
      const index = cursor;
      cursor += 1;
      await recognizeOne(rows[index], index);
    }
  }

  await Promise.all(Array.from({ length: workerCount }, () => worker()));
  await persistQueue;
  await writeJsonl(outputPath, orderedPredictions());
  ...
  return {
    ...
    configured_limit: limitCount || null,
...
function policyAwareStatus(fieldComparison = {}, narrowComparison = {}, prediction = {}) {
  ...
  if (abstained) {
    return {
      ...
      notes: ["model abstained or produced no usable fields"]
    };
  }

  const criticalFields = ["player", "year", "rookie", "autograph", "relic", "grade"];
  const criticalErrors = [];
  for (const field of criticalFields) {
    if (fieldComparison[field] === "MISMATCH") criticalErrors.push(field);
  }
  if (narrowComparison.core_identity === "MISMATCH") criticalErrors.push("core_identity");
  if (narrowComparison.surface_color === "MISMATCH") criticalErrors.push("surface_color");
  if (narrowComparison.serial_denominator === "MISMATCH") criticalErrors.push("serial_denominator");

  const reviewReasons = [];
  for (const field of ["brand", "set", "parallel", "card_number", "serial_number", "grade"]) {
    const status = fieldComparison[field];
    if (status === "MISSING_MODEL" || status === "UNCERTAIN") reviewReasons.push(field);
  }
  for (const field of ["core_identity", "surface_color", "serial_denominator"]) {
    const status = narrowComparison[field];
    if (status === "MISSING_MODEL" || status === "UNCERTAIN") reviewReasons.push(field);
  }
  ...
  if (criticalErrors.length) {
    return {
      ...
      notes: criticalErrors.map((field) => `critical mismatch: ${field}`)
    };
  }
  if (narrowComparison.core_identity !== "MATCH" || reviewReasons.length) {
    return {
      status: "PARTIAL",
      critical_error_count: 0,
      review_reason_count: reviewReasons.length,
      notes: reviewReasons.map((field) => `needs review: ${field}`).slice(0, 8)
    };
  }
  return {
    status: "PASS",
...
export function comparePredictionToTitle(prediction = {}, answer = {}) {
  ...
  const policyStatus = policyAwareStatus(fieldComparison, narrowComparison, prediction);
  return {
    ...
    policy_diagnostics: policyStatus,
    notes: [
...
function usageNumber(value = {}, keys = []) {
  for (const key of keys) {
...
function sumUsage(predictions = []) {
  return predictions.reduce((totals, prediction) => {
    const usage = prediction.usage && typeof prediction.usage === "object" ? prediction.usage : {};
    totals.input_tokens += usageNumber(usage, ["input_tokens", "prompt_tokens"]);
    totals.output_tokens += usageNumber(usage, ["output_tokens", "completion_tokens"]);
    totals.total_tokens += usageNumber(usage, ["total_tokens"]);
    totals.provider_calls += usageNumber(usage, ["provider_calls"]);
    totals.retrieval_calls += usageNumber(usage, ["retrieval_calls"]);
    totals.estimated_cost_usd = Number((totals.estimated_cost_usd + usageNumber(usage, ["estimated_cost_usd"])).toFixed(6));
    return totals;
  }, {
    input_tokens: 0,
    output_tokens: 0,
...
function eligibilityFor(prediction = {}, source = "catalog") {
  const diagnostics = prediction.c_group_diagnostics && typeof prediction.c_group_diagnostics === "object"
    ? prediction.c_group_diagnostics
    : {};
  const key = source === "vector" ? "vector_assist_eligibility" : "catalog_assist_eligibility";
  return diagnostics[key] && typeof diagnostics[key] === "object" ? diagnostics[key] : {};
}

function debugRowsFor(prediction = {}, source = "catalog") {
  const diagnostics = prediction.c_group_diagnostics && typeof prediction.c_group_diagnostics === "object"
    ? prediction.c_group_diagnostics
    : {};
  const key = source === "vector" ? "vector_candidate_debug" : "catalog_candidate_debug";
  return Array.isArray(diagnostics[key]) ? diagnostics[key] : [];
}

function sumEligibility(predictions = [], source = "catalog", key = "raw_candidate_count") {
  return predictions.reduce((sum, prediction) => {
    const value = Number(eligibilityFor(prediction, source)[key]);
    return sum + (Number.isFinite(value) ? value : 0);
  }, 0);
}

function promptCandidateIds(predictions = [], source = "catalog") {
  return [...new Set(predictions.flatMap((prediction) => {
    const ids = eligibilityFor(prediction, source).prompt_candidate_ids;
    return Array.isArray(ids) ? ids.map(normalizeText).filter(Boolean) : [];
  }))];
}

function fieldSupportFields(predictions = [], source = "catalog") {
  return [...new Set(predictions.flatMap((prediction) => {
    const fields = eligibilityFor(prediction, source).field_support_fields;
    return Array.isArray(fields) ? fields.map(normalizeText).filter(Boolean) : [];
  }))];
}

function cGroupDiagnosticsSummary(predictions = []) {
  const latencyValues = predictions.map((prediction) => prediction.timing?.total_ms);
  const providerLatencyValues = predictions.map((prediction) => prediction.timing?.provider_total_ms);
  const catalogLatencyValues = predictions.map((prediction) => prediction.timing?.catalog_retrieval_ms);
  const evidenceLatencyValues = predictions.map((prediction) => prediction.timing?.evidence_completion_ms);
  const vectorLatencyValues = predictions.map((prediction) => prediction.timing?.vector_retrieval_ms);
  const catalogDebugCount = predictions.reduce((sum, prediction) => sum + debugRowsFor(prediction, "catalog").length, 0);
  const vectorDebugCount = predictions.reduce((sum, prediction) => sum + debugRowsFor(prediction, "vector").length, 0);
  return {
    provider_success_count: predictions.filter((prediction) => normalizeText(prediction.recognition_status).toUpperCase() !== "FAILED" && !normalizeText(prediction.error_type)).length,
    provider_failure_count: predictions.filter((prediction) => normalizeText(prediction.recognition_status).toUpperCase() === "FAILED" || normalizeText(prediction.error_type)).length,
    per_card_latency_ms: {
      p50: percentile(latencyValues, 0.5),
      p95: percentile(latencyValues, 0.95),
      max: numericValues(latencyValues).sort((a, b) => b - a)[0] ?? null
    },
    provider_latency_ms: {
      p50: percentile(providerLatencyValues, 0.5),
      p95: percentile(providerLatencyValues, 0.95)
    },
    catalog_retrieval_latency_ms: {
      p50: percentile(catalogLatencyValues, 0.5),
      p95: percentile(catalogLatencyValues, 0.95)
    },
    vector_retrieval_latency_ms: {
      p50: percentile(vectorLatencyValues, 0.5),
      p95: percentile(vectorLatencyValues, 0.95)
    },
    evidence_completion_latency_ms: {
      p50: percentile(evidenceLatencyValues, 0.5),
      p95: percentile(evidenceLatencyValues, 0.95)
    },
    usage_totals: sumUsage(predictions),
    catalog: {
      prompt_assist_used_count: predictions.filter((prediction) => prediction.c_group_diagnostics?.catalog_prompt_assist_used === true).length,
      raw_candidate_count: sumEligibility(predictions, "catalog", "raw_candidate_count"),
      approved_candidate_count: sumEligibility(predictions, "catalog", "approved_candidate_count"),
      trust_blocked_count: sumEligibility(predictions, "catalog", "trust_blocked_count"),
      conflict_blocked_count: sumEligibility(predictions, "catalog", "conflict_blocked_count"),
      prompt_candidate_count: sumEligibility(predictions, "catalog", "prompt_candidate_count"),
      field_support_count: sumEligibility(predictions, "catalog", "field_support_count"),
      field_support_fields: fieldSupportFields(predictions, "catalog"),
      prompt_candidate_ids: promptCandidateIds(predictions, "catalog"),
      debug_candidate_count: catalogDebugCount
    },
    vector: {
      prompt_assist_used_count: predictions.filter((prediction) => prediction.c_group_diagnostics?.vector_prompt_assist_used === true).length,
      raw_candidate_count: sumEligibility(predictions, "vector", "raw_candidate_count"),
      approved_candidate_count: sumEligibility(predictions, "vector", "approved_candidate_count"),
      trust_blocked_count: sumEligibility(predictions, "vector", "trust_blocked_count"),
      conflict_blocked_count: sumEligibility(predictions, "vector", "conflict_blocked_count"),
      prompt_candidate_count: sumEligibility(predictions, "vector", "prompt_candidate_count"),
      field_support_count: sumEligibility(predictions, "vector", "field_support_count"),
      field_support_fields: fieldSupportFields(predictions, "vector"),
      prompt_candidate_ids: promptCandidateIds(predictions, "vector"),
      debug_candidate_count: vectorDebugCount
    }
  };
}

function compactCandidateDebug(rows = []) {
  return rows.slice(0, 5).map((row) => ({
    candidate_id: row.candidate_id || row.candidate_identity_id || "",
    ...
    prompt_admitted: row.prompt_admitted === true,
    prompt_blocked: row.prompt_blocked === true
  }));
}

function perCardDecisionTrace(prediction = {}, scored = {}) {
  return {
    case_id: prediction.case_id,
    item_id: scored.item_id || "",
    predicted_title: prediction.recognition_output?.title || "",
    seller_title_weak_label: scored.title || "",
    overall_status: scored.overall_status,
    policy_status: scored.policy_status,
    field_comparison: scored.field_comparison || {},
    narrow_diagnostic_comparison: scored.narrow_diagnostic_comparison || {},
    latency_ms: prediction.timing?.total_ms ?? null,
    provider_latency_ms: prediction.timing?.provider_total_ms ?? null,
    usage: prediction.usage || null,
    catalog_assist_eligibility: eligibilityFor(prediction, "catalog"),
    vector_assist_eligibility: eligibilityFor(prediction, "vector"),
    catalog_candidate_debug: compactCandidateDebug(debugRowsFor(prediction, "catalog")),
    vector_candidate_debug: compactCandidateDebug(debugRowsFor(prediction, "vector")),
    notes: scored.notes || []
  };
}
...
export async function scoreBlindEval({
  ...
  const policyCounts = scored.reduce((acc, row) => {
  ...
  const summary = {
    ...
    c_group_diagnostics: cGroupDiagnosticsSummary(predictions),
    per_card_decision_trace: scored.map((row) => {
      const prediction = predictions.find((item) => item.case_id === row.case_id) || {};
      return perCardDecisionTrace(prediction, row);
    }),
    anti_leakage: {
