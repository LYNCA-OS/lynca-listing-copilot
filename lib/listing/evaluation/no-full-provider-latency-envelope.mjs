export const noFullProviderLatencyEnvelopeVersion = "no-full-provider-latency-envelope-v3";
...
// The current per-asset worker holds capacity one crop at a time. If detail
// OCR were enabled without a transport change, the three required crop
// latencies would add. Fenton-Wilkinson gives a deterministic approximation of
// that sum from the same retained marginal p50/p95 values. It is a model, not
// a card-level observation.
export function modelSerialFocusedOcrFromMarginals({ crops = [] } = {}) {
  if (!Array.isArray(crops) || crops.length === 0) fail("OCR_CROPS_REQUIRED", "crops");
  const z95 = inverseNormalCdf(0.95);
  const moments = crops.map((crop, index) => {
    const p50 = finiteNonNegative(crop.p50_ms, `crops[${index}].p50_ms`);
    const p95 = finiteNonNegative(crop.p95_ms, `crops[${index}].p95_ms`);
    if (!(p50 > 0 && p95 > p50)) fail("OCR_QUANTILES_INVALID", `crops[${index}]`);
    const mu = Math.log(p50);
    const sigma = (Math.log(p95) - mu) / z95;
    const mean = Math.exp(mu + ((sigma ** 2) / 2));
    const variance = (Math.exp(sigma ** 2) - 1) * Math.exp((2 * mu) + (sigma ** 2));
    return { mean, variance };
  });
  const mean = moments.reduce((sum, item) => sum + item.mean, 0);
  const variance = moments.reduce((sum, item) => sum + item.variance, 0);
  const sigmaSquared = Math.log(1 + (variance / (mean ** 2)));
  const mu = Math.log(mean) - (sigmaSquared / 2);
  const sigma = Math.sqrt(sigmaSquared);
  return deepFreeze({
    evidence_class: latencyEvidenceClasses.MODELLED_COMPONENT,
    observed: false,
    p50_ms: Math.round(Math.exp(mu)),
    p95_ms: Math.round(Math.exp(mu + (z95 * sigma))),
    assumptions: [
      "LOGNORMAL_MARGINALS_FIT_FROM_P50_P95",
      "FENTON_WILKINSON_SUM_APPROXIMATION",
      "ZERO_COVARIANCE_BETWEEN_CROP_LATENCIES",
      "CURRENT_PER_ASSET_CAPACITY_ONE_SERIALIZES_CROPS",
      "NOT_A_CARD_LEVEL_OBSERVATION",
      "NOT_A_TAIL_UPPER_BOUND_WHEN_SHARED_NETWORK_OR_COLD_START_CORRELATES_CALLS"
    ]
  });
}
...
const retainedFocusedOcrMarginals = deepFreeze([
  { id: "year_product", p50_ms: 655, p95_ms: 2277 },
  { id: "subject", p50_ms: 983, p95_ms: 2337 },
  { id: "card_code", p50_ms: 656, p95_ms: 2206 }
]);

// This is the most evidence-grounded planning proxy for the new one-request
// shape. It reuses the retained crop marginals and assumes the batch finishes
// when its slowest image finishes. It is neither a strict upper bound nor an
// observed batch distribution, so a real card-level packet must replace it.
export const noFullProviderOneShotOcrPlanningProxy = modelParallelFocusedOcrFromMarginals({
  crops: retainedFocusedOcrMarginals
});

export const noFullProviderCurrentSerialOcrModel = modelSerialFocusedOcrFromMarginals({
  crops: retainedFocusedOcrMarginals
});
...
export const noFullProviderReferenceStages = deepFreeze({
  ...
  focused_ocr: {
    evidence_class: latencyEvidenceClasses.MODELLED_COMPONENT,
    observed: false,
    basis: "retained three-crop marginals used as a one-request batch planning proxy; not yet observed card-level",
    assumptions: [
      ...noFullProviderOneShotOcrPlanningProxy.assumptions,
      "ONE_CLOUD_RUN_REQUEST",
      "ONE_GOOGLE_ANNOTATE_REQUEST",
      "THREE_VISION_UNITS",
      "ONE_OR_TWO_UNIQUE_SOURCE_DECODES",
      "PARALLEL_MAX_IS_A_PROXY_NOT_A_BATCH_UPPER_BOUND",
      "CURRENT_FOUR_MILLISECOND_COALESCER_DOES_NOT_PROVE_THIS_SHAPE",
      "CARD_LEVEL_BATCH_TIMING_REQUIRED_FOR_PROMOTION"
    ],
    quantiles_ms: {
      p50: {
        lower: noFullProviderOneShotOcrPlanningProxy.p50_ms,
        upper: noFullProviderOneShotOcrPlanningProxy.p50_ms
      },
      p95: {
        lower: noFullProviderOneShotOcrPlanningProxy.p95_ms,
        upper: noFullProviderOneShotOcrPlanningProxy.p95_ms
      }
    }
  },
...
// A deliberately separate stretch target. Keeping this out of the default
// reference envelope prevents an unobserved 1.0/1.85 second goal from being
// reported as the closest evidence-grounded speed.
export const noFullProviderStretchOneShotStages = deepFreeze({
  ...noFullProviderReferenceStages,
  focused_ocr: {
    evidence_class: latencyEvidenceClasses.DESIGN_BUDGET,
    observed: false,
    basis: "explicit three-view one-card Vision batch stretch acceptance target; not observed",
    assumptions: [
      "ONE_CLOUD_RUN_REQUEST",
      "ONE_GOOGLE_ANNOTATE_REQUEST",
      "THREE_VISION_UNITS",
