...
function titleParserSemSuggestion(title = "") {
  const parsed = parseReviewedTitleFields(title);
  const subject = parsed.players?.length ? parsed.players : parsed.character ? [parsed.character] : [];
  ...
  return compactObject({
    ...
    print_finish: parsed.print_finish || parsed.parallel_exact || parsed.parallel || parsed.surface_color,
...
export function buildGoldenSemReviewPacket(source = {}, {
  ...
  const items = sourceItems(source).map((item, index) => {
    ...
    const suggestion = titleParserSemSuggestion(title);
    return {
      item_id: itemId,
