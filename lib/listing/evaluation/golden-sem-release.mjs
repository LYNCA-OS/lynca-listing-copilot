...
export const goldenSemLaunchFields = formalReleaseLaunchFields;
...
function recognitionImages(item = {}) {
  const images = item.recognition_input?.images
    || item.images
    || item.image_inputs
    || [];
...
function evidenceSourceProvenance(value) {
  const row = plainObject(value);
  return {
    source_id: cleanText(row.source_id || row.id),
    source_sha256: cleanText(row.source_sha256 || row.content_sha256).toLowerCase()
  };
}

function validEvidenceSourceProvenance(value) {
  const row = evidenceSourceProvenance(value);
  return Boolean(row.source_id && /^[0-9a-f]{64}$/.test(row.source_sha256));
}
...
export function validateGoldenSemReviewPacket(packet = {}, {
  ...
  items.forEach((item, index) => {
    ...
    for (const field of goldenSemLaunchFields) {
      ...
      if (
        status === "CONFIRMED"
        && !asArray(fieldRecord.evidence_sources).every(validEvidenceSourceProvenance)
      ) {
        errors.push(`${itemId}.${field}: evidence_sources require source_id and source_sha256`);
      }
    }
  });
...
function assignGroups(items = [], seed = "lynca-golden-sem-v1") {
  ...
  const targets = targetCounts(items.length);
...
function frozenItem(item = {}) {
  ...
  for (const field of goldenSemLaunchFields) {
    ...
    evidenceSources[field] = asArray(record.evidence_sources)
      .map(evidenceSourceProvenance)
      .filter((source) => source.source_id || source.source_sha256);
  }
...
function partitionTruthPolicy(partition) {
  return {
    ...formalReleaseTruthPolicy,
    launch_gate_eligible: partition === "holdout"
  };
}
...
export function freezeGoldenSemReleaseSets(packet = {}, {
  ...
  const items = sourceItems(packet);
  const { assigned, targets, groupCount } = assignGroups(items, seed);
  const partitions = Object.fromEntries(splitNames.map((partition) => {
    const frozenItems = assigned[partition].map(frozenItem);
    const evaluationTruthPolicy = partitionTruthPolicy(partition);
    const frozenPartition = {
    ...
    frozenPartition.item_set_sha256 = releaseSetItemSetSha256(frozenItems, {
      split: partition,
      evaluationTruthPolicy
    });
    return [partition, frozenPartition];
  }));
  ...
  const holdoutTruthPolicy = partitionTruthPolicy("holdout");
  const holdoutReleaseSet = {
    ...
    frozen_at: frozenAt,
    item_set_digest_schema_version: releaseSetItemDigestSchemaVersion,
    evaluation_truth_policy: holdoutTruthPolicy,
    sem_standard_version: SEM_STANDARD_VERSION,
    leakage_policy: {
  ...
  holdoutReleaseSet.item_set_sha256 = releaseSetItemSetSha256(holdout.items, {
    split: holdoutReleaseSet.partition,
    evaluationTruthPolicy: holdoutTruthPolicy
  });
