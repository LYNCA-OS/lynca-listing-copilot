...
export const trustedCatalogSemPromotionSchemaVersion = "trusted-catalog-sem-promotion-v1";
  ...
  const items = (packet.items || []).map((item) => {
    ...
    return {
      ...
      retrieval_ground_truth: {
        accepted_candidate_ids: [...new Set([
          reviewedCatalogByFeedback.get(cleanText(item.source_feedback_id || item.item_id)),
          ...candidates.strong.map((candidate) => candidate.card.id)
        ].filter(Boolean))],
        accepted_identity_ids: item.card_identity_id ? [item.card_identity_id] : [],
        source: "SEALED_REVIEWED_INTERNAL_AND_TRUSTED_CATALOG_IDENTITIES"
      },
