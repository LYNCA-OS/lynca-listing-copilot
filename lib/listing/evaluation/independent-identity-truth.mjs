...
export const independentIdentityTruthSchemaVersion = "independent-identity-truth-v1";
...
export function validateIndependentIdentityLabel(label = {}, { item = {}, catalogById = new Map() } = {}) {
  ...
  const candidate = catalogById.get(label.source_candidate_id);
  ...
  if (candidate && canonicalIdentityId(catalogFields(candidate)) !== label.canonical_identity_id) {
    errors.push("CANONICAL_IDENTITY_MISMATCH");
  }
  if (candidate && sourceFeedbackId(candidate) === clean(item.source_feedback_id || item.item_id)) {
    errors.push("SAME_FEEDBACK_SELF_CORROBORATION");
  }
...
export function applyIndependentIdentityLabels(dataset = {}, reviewPacket = {}, catalog = {}) {
  ...
  const scopedItems = (dataset.items || []).flatMap((item) => {
    ...
    return [{
      ...
      retrieval_ground_truth: accepted ? {
        accepted_candidate_ids: [review.label.source_candidate_id],
        ...
        source: "INDEPENDENT_EXACT_IDENTITY_LABEL_V1",
  ...
  return {
    ...dataset,
    schema_version: "v4-oracle-independent-identity-dataset-v1",
