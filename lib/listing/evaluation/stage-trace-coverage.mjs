...
export function auditStageTraceCoverage({ dataset = {}, trace = {}, minimumCoverage = 0.99 } = {}) {
  const traceById = new Map(rows(trace).map((row) => [rowId(row), row]));
  const cards = rows(dataset).map((item) => {
    const id = rowId(item);
  ...
  const reasonCounts = {};
  const sourceContractViolations = cards.flatMap((card) => {
    const traceRow = traceById.get(card.query_card_id) || {};
    return traceRow.instrumentation?.pipeline_contract_violations || [];
  });
