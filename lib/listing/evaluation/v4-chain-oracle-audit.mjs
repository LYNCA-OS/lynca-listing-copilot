...
export function evaluateV4ChainOracleAudit({ dataset = {}, trace = {}, now = () => new Date() } = {}) {
  ...
  for (const item of rows(dataset)) {
    ...
    for (const [field, expected] of Object.entries(truth)) {
      const evidenceSources = evidence.filter((entry) => exact(field, expected, entry.fields[field])).map((entry) => entry.source);
  ...
  const datasetTruthClass = cleanText(dataset.evaluation_truth_policy?.field_ground_truth_class || "HUMAN_REVIEWED_FIELD_GROUND_TRUTH").toUpperCase();
  const formalTruth = datasetTruthClass === "HUMAN_REVIEWED_FIELD_GROUND_TRUTH";
  return {
