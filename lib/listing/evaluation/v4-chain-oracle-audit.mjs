...
export function evaluateV4ChainOracleAudit({ dataset = {}, trace = {}, now = () => new Date() } = {}) {
  ...
  for (const item of rows(dataset)) {
    ...
    const correctCandidate = retrievalEvaluable
      ? candidates.find((candidate) => candidateMatchesTruth(candidate, truthIds, expectedIdentity)) || null
      : null;
    ...
    cards.push({
      ...
      selected_candidate_correct: retrievalEvaluable && selectedCandidate
        ? candidateMatchesTruth(selectedCandidate, truthIds, expectedIdentity)
        : null,
  ...
  return {
    ...
    data_quality: {
      ...
      downstream_uncontaminated_card_count: rows(dataset).length - sealedSourceCandidateSelectedCount,
