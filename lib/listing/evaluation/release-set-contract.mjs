...
export const humanReviewedFieldGroundTruthClass = "HUMAN_REVIEWED_FIELD_GROUND_TRUTH";
export const minimumCardExactEvaluatedFields = 2;
export const formalLaunchCriticalFieldGroups = Object.freeze({
  subject: Object.freeze(["subject", "players", "player"]),
  year: Object.freeze(["year"]),
  product_or_set: Object.freeze(["product_or_set", "product", "set"]),
  variant_or_parallel: Object.freeze([
    "variant_or_parallel",
    "parallel",
    "release_variant",
    "print_finish"
  ]),
  numerical_rarity: Object.freeze(["numerical_rarity", "print_run_number", "serial_number"]),
  grade: Object.freeze(["grade", "grading_info", "card_grade"])
});

const setTypeValues = new Set(Object.values(releaseSetTypes));
const unknownValues = new Set(["", "UNKNOWN", "NOT_APPLICABLE", "N/A", "NULL"]);
const formalReviewStatuses = new Set(["CONFIRMED", "UNKNOWN", "NOT_APPLICABLE"]);
const nonHumanReviewerValues = new Set([
  "AUTOMATED",
  "PARSER_SUGGESTION",
  "TITLE_DERIVATION_ONLY",
  "UNREVIEWED"
]);
const nonHumanEvidenceValues = new Set([
  "PARSER_SUGGESTION",
  "WRITER_REVIEWED_TITLE_DERIVATION"
]);
...
function reviewFieldRecord(value) {
  const record = plainObject(value);
  return hasOwn(record, "reviewed_status")
    || hasOwn(record, "status")
    || hasOwn(record, "reviewed_value")
    || hasOwn(record, "evidence_sources")
    || hasOwn(record, "evidence");
}

function reviewFieldParts(review = {}, field) {
  const fields = plainObject(review.fields);
  const statuses = plainObject(review.field_statuses || review.statuses);
  const evidenceByField = plainObject(review.evidence_sources || review.evidence);
  const raw = fields[field];
  const record = reviewFieldRecord(raw) ? plainObject(raw) : {};
  return {
    status: cleanText(
      statuses[field]
    ...
    value: reviewFieldRecord(raw)
      ? (record.reviewed_value ?? record.value ?? "")
      : raw,
    evidence: reviewFieldRecord(raw)
      ? (record.evidence_sources ?? record.evidence ?? evidenceByField[field])
      : evidenceByField[field]
  };
}

function reviewValuePresent(value) {
...
function evidenceLabel(value) {
  if (value && typeof value === "object") {
    return cleanText(value.source || value.source_type || value.type || value.kind || value.id);
  }
  return cleanText(value);
}
...
export function validateFormalLaunchGroundTruth(dataset = {}) {
  const policy = plainObject(dataset.evaluation_truth_policy);
  const explicitTruthClass = hasOwn(policy, "field_ground_truth_class")
    && Boolean(cleanText(policy.field_ground_truth_class));
  const fieldGroundTruthClass = explicitTruthClass
    ? cleanText(policy.field_ground_truth_class).toUpperCase()
    : null;
  const humanReviewed = explicitTruthClass
    && fieldGroundTruthClass === humanReviewedFieldGroundTruthClass;
  const launchGateRequested = humanReviewed && policy.launch_gate_eligible !== false;
  const items = Array.isArray(dataset.items) ? dataset.items : [];
  const policyErrors = [];
  const metadataErrors = [];
  let validReviewedItemCount = 0;

  if (!humanReviewed) {
    policyErrors.push(
      `evaluation_truth_policy.field_ground_truth_class must explicitly equal ${humanReviewedFieldGroundTruthClass}`
    );
  } else if (!launchGateRequested) {
    policyErrors.push("evaluation_truth_policy.launch_gate_eligible must not be false");
  }

  if (humanReviewed && !items.length) {
    metadataErrors.push("items must not be empty for formal launch ground truth");
  }
  ...
  if (humanReviewed) {
    items.forEach((item, index) => {
      ...
      const fields = plainObject(review.fields);
      const statuses = plainObject(review.field_statuses || review.statuses);
      const fieldNames = [...new Set([...Object.keys(fields), ...Object.keys(statuses)])];
      const itemErrors = [];

      if (!reviewer) itemErrors.push(`${itemId}: reviewed_by is required`);
      else if (nonHumanReviewerValues.has(reviewer.toUpperCase())) {
        itemErrors.push(`${itemId}: reviewed_by must identify a human field reviewer`);
      }
      if (!reviewedAt) itemErrors.push(`${itemId}: reviewed_at is required`);
      else if (!validReviewedAt(reviewedAt)) itemErrors.push(`${itemId}: reviewed_at must be a valid date-time`);
      if (!fieldNames.length) itemErrors.push(`${itemId}: reviewed field ground truth is required`);

      for (const field of fieldNames) {
        const parts = reviewFieldParts(review, field);
        if (!parts.status) {
          itemErrors.push(`${itemId}.${field}: explicit reviewed field status is required`);
          continue;
        }
        if (!formalReviewStatuses.has(parts.status)) {
          itemErrors.push(`${itemId}.${field}: unsupported reviewed field status ${parts.status}`);
          continue;
        }
        if (parts.status !== "CONFIRMED") continue;
        if (!reviewValuePresent(parts.value)) {
          itemErrors.push(`${itemId}.${field}: CONFIRMED requires a reviewed value`);
        }
        const evidence = usableEvidence(parts.evidence);
        if (!evidence.length) {
          itemErrors.push(`${itemId}.${field}: CONFIRMED requires evidence_sources`);
        } else if (evidence.every((entry) => nonHumanEvidenceValues.has(evidenceLabel(entry).toUpperCase()))) {
          itemErrors.push(`${itemId}.${field}: title/parser derivation is not formal field evidence`);
        }
      }

      if (!itemErrors.length) validReviewedItemCount += 1;
      metadataErrors.push(...itemErrors);
    });
  }

  const reviewMetadataOk = humanReviewed && items.length > 0 && metadataErrors.length === 0;
  const errors = [...policyErrors, ...metadataErrors];
  return {
    ok: launchGateRequested && reviewMetadataOk,
    schema_version: "formal-launch-ground-truth-validation-v1",
    truth_policy: {
      explicit: explicitTruthClass,
      field_ground_truth_class: fieldGroundTruthClass,
      human_reviewed: humanReviewed,
      launch_gate_requested: launchGateRequested
    },
    review_metadata: {
      ok: reviewMetadataOk,
      item_count: items.length,
      valid_reviewed_item_count: validReviewedItemCount,
      invalid_reviewed_item_count: Math.max(0, items.length - validReviewedItemCount),
...
export function validateReleaseSetManifest(manifest = {}) {
  ...
  const formalGroundTruth = validateFormalLaunchGroundTruth(manifest);
  ...
  if (formalGroundTruth.truth_policy.human_reviewed) {
    errors.push(...formalGroundTruth.review_metadata.errors);
  }
  ...
  return {
    ...
    formal_launch_gate_eligible: formalGroundTruth.ok && errors.length === 0,
    formal_launch_ground_truth: formalGroundTruth,
    errors,
    warnings
  };
}
