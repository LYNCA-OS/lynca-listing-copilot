...
export const releaseSetItemDigestSchemaVersion = "release-set-content-digest-v2";
export const formalReleaseTruthPolicy = Object.freeze({
...
// One owner for the formal launch denominator. Diagnostic datasets may expose
// fewer fields, but a CORE_HOLDOUT row must explicitly classify every field so
// an omitted label cannot silently disappear from card-exact accuracy.
export const formalReleaseLaunchFields = Object.freeze([
  "year",
  "ip_sport",
  "language",
  "manufacturer",
...
export const formalReleaseCriticalCoverageGroups = Object.freeze([
  Object.freeze(["subject"]),
  Object.freeze(["product", "manufacturer"]),
  Object.freeze(["year", "card_number"])
]);
...
export function canonicalReleaseItemId(value) {
  return typeof value === "string" ? cleanText(value).toLowerCase() : "";
}

function stableItemIds(items = []) {
  return [...new Set(items.map((item) => canonicalReleaseItemId(item?.item_id || item?.query_card_id)).filter(Boolean))].sort();
}
...
function canonicalReleaseItems(items = []) {
  return items.map((item) => canonicalValue(item)).sort((left, right) => (
    canonicalReleaseItemId(left?.item_id || left?.query_card_id).localeCompare(
      canonicalReleaseItemId(right?.item_id || right?.query_card_id)
    )
  ));
}
...
export function releaseSetItemSetSha256(items = [], {
  ...
  const payload = canonicalValue({
    schema_version: releaseSetItemDigestSchemaVersion,
    split: cleanText(split).toLowerCase() || null,
    evaluation_truth_policy: plainObject(evaluationTruthPolicy),
    items: canonicalReleaseItems(Array.isArray(items) ? items : [])
  });
...
function canonicalTime(value) {
  ...
  return Number.isFinite(milliseconds) && new Date(milliseconds).toISOString() === text;
}

function formalTruthPolicyErrors(policy = {}) {
  const value = plainObject(policy);
  return Object.entries(formalReleaseTruthPolicy).flatMap(([key, expected]) => (
    value[key] === expected ? [] : [`evaluation_truth_policy.${key} must be ${JSON.stringify(expected)}`]
  ));
}

function evidenceSourcesForField(item = {}, field) {
  const sources = item.reviewed_ground_truth?.evidence_sources?.[field];
...
export function validateReleaseSetManifest(manifest = {}) {
  ...
  const split = cleanText(manifest.partition || manifest.split).toLowerCase();
  const truthPolicy = plainObject(manifest.evaluation_truth_policy);
  const formalCoreHoldout = setType === releaseSetTypes.CORE_HOLDOUT
    && split === "holdout"
    && truthPolicy.field_ground_truth_class === formalReleaseTruthPolicy.field_ground_truth_class
    && truthPolicy.formal_golden_sem === true
    && truthPolicy.launch_gate_eligible === true;
  ...
  if (!canonicalTime(manifest.frozen_at)) errors.push("frozen_at must be a canonical timestamp");
  if (manifest.item_set_digest_schema_version !== releaseSetItemDigestSchemaVersion) {
    errors.push(`item_set_digest_schema_version must be ${releaseSetItemDigestSchemaVersion}`);
  }
  if (!split) errors.push("partition is required");
  if ([releaseSetTypes.CORE_HOLDOUT, releaseSetTypes.COLD_START_HOLDOUT].includes(setType) && split !== "holdout") {
    errors.push("holdout set types require partition=holdout");
  }
  if (setType === releaseSetTypes.PRODUCTION_REPLAY && split !== "production_replay") {
    errors.push("production replay requires partition=production_replay");
  }
  errors.push(...formalTruthPolicyErrors(truthPolicy));
  if (!items.length) errors.push("items must not be empty");
  if (
    items.some((item) => !exactText(item?.item_id || item?.query_card_id))
    || stableItemIds(items).length !== items.length
  ) errors.push("item_id values must be strict and canonically unique");
  for (const item of items) {
    const itemId = canonicalReleaseItemId(item?.item_id);
    const queryCardId = canonicalReleaseItemId(item?.query_card_id);
    if (itemId && queryCardId && itemId !== queryCardId) {
      errors.push(`${itemId}: item_id and query_card_id must identify the same canonical item`);
    }
  }

  const identityGroupIds = items.map((item) => canonicalReleaseItemId(item?.identity_group_id));
  if (formalCoreHoldout && (
    identityGroupIds.some((value) => !value)
    || new Set(identityGroupIds).size !== items.length
  )) {
    errors.push("formal CORE_HOLDOUT identity_group_id values must be present and canonically unique");
  }

  const computedSha256 = releaseSetItemSetSha256(items, {
    split,
    evaluationTruthPolicy: truthPolicy
  });
  ...
  items.forEach((item, index) => {
    ...
    const statuses = plainObject(item?.reviewed_ground_truth?.field_statuses);
    ...
    if (!Array.isArray(item?.recognition_input?.images) || item.recognition_input.images.length < 1) {
      errors.push(`${itemId}: recognition_input.images is required`);
    } else {
      item.recognition_input.images.forEach((image, imageIndex) => {
        if (!exactSha256(image?.content_sha256)) {
          errors.push(`${itemId}: recognition_input.images[${imageIndex}].content_sha256 is required`);
        }
      });
    }
    if (!exactText(item?.reviewed_ground_truth?.reviewed_by)) {
      errors.push(`${itemId}: reviewed_ground_truth.reviewed_by is required`);
    }
    if (!canonicalTime(item?.reviewed_ground_truth?.reviewed_at)) {
      errors.push(`${itemId}: reviewed_ground_truth.reviewed_at must be a canonical timestamp`);
    }
    const fieldsToValidate = formalCoreHoldout ? formalReleaseLaunchFields : Object.keys(fields);
    if (formalCoreHoldout) {
      for (const field of formalReleaseLaunchFields) {
        if (!Object.hasOwn(fields, field)) errors.push(`${itemId}.${field}: formal field value is required`);
        if (!Object.hasOwn(statuses, field)) errors.push(`${itemId}.${field}: formal field status is required`);
      }
      const criticalCoverageComplete = formalReleaseCriticalCoverageGroups.every((group) => (
        group.some((field) => cleanText(statuses[field]).toUpperCase() === "CONFIRMED")
      ));
      if (!criticalCoverageComplete) {
        errors.push(`${itemId}: formal critical identity coverage requires subject, product/manufacturer, and year/card_number`);
      }
    }
    for (const field of fieldsToValidate) {
      const status = cleanText(statuses[field]).toUpperCase();
      if (!['CONFIRMED', 'UNKNOWN', 'NOT_APPLICABLE'].includes(status)) {
        errors.push(`${itemId}.${field}: explicit field status is required`);
        continue;
      }
      if (status !== "CONFIRMED") {
        if (cleanText(fields[field]).toUpperCase() !== status) {
          errors.push(`${itemId}.${field}: ${status} status requires the same explicit field marker`);
        }
        continue;
      }
      const sources = evidenceSourcesForField(item, field);
      if (!sources.length) {
        errors.push(`${itemId}.${field}: confirmed field evidence provenance is required`);
        continue;
      }
      sources.forEach((source, sourceIndex) => {
        if (!exactText(source?.source_id)) {
          errors.push(`${itemId}.${field}: evidence_sources[${sourceIndex}].source_id is required`);
        }
        if (!exactSha256(source?.source_sha256)) {
          errors.push(`${itemId}.${field}: evidence_sources[${sourceIndex}].source_sha256 is required`);
        }
      });
    }
  });
  ...
  return {
    ...
    partition: split || null,
    item_count: items.length,
    canonical_item_id_count: stableItemIds(items).length,
    independent_identity_group_count: new Set(identityGroupIds.filter(Boolean)).size,
    launch_field_contract_covered_count: formalCoreHoldout
      ? items.filter((item) => formalReleaseLaunchFields.every((field) => (
        Object.hasOwn(plainObject(item?.reviewed_ground_truth?.fields), field)
        && Object.hasOwn(plainObject(item?.reviewed_ground_truth?.field_statuses), field)
      ))).length
      : 0,
    critical_identity_covered_count: formalCoreHoldout
      ? items.filter((item) => {
        const statuses = plainObject(item?.reviewed_ground_truth?.field_statuses);
        return formalReleaseCriticalCoverageGroups.every((group) => (
          group.some((field) => cleanText(statuses[field]).toUpperCase() === "CONFIRMED")
        ));
      }).length
      : 0,
