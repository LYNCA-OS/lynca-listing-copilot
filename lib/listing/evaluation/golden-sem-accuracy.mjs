...
import { productProxyComparisonKey } from "../csm/product-semantics.mjs";
...
function evaluationTruthPolicy(dataset = {}, formalValidation = {}, releaseValidationOk = false) {
  const explicit = plainObject(dataset.evaluation_truth_policy);
  const truthPolicyExplicit = Object.prototype.hasOwnProperty.call(explicit, "field_ground_truth_class")
    && Boolean(cleanText(explicit.field_ground_truth_class));
  const fieldGroundTruthClass = truthPolicyExplicit
    ? cleanText(explicit.field_ground_truth_class).toUpperCase()
    : null;
  const formalGoldenSem = truthPolicyExplicit
    && fieldGroundTruthClass === humanReviewedFieldGroundTruthClass;
  const launchGateRequested = formalGoldenSem && explicit.launch_gate_eligible !== false;
  const formalLaunchGateEligible = launchGateRequested
    && formalValidation.ok === true
    && releaseValidationOk;
  return {
    truth_policy_explicit: truthPolicyExplicit,
    field_ground_truth_class: fieldGroundTruthClass,
    formal_golden_sem: formalGoldenSem,
    launch_gate_requested: launchGateRequested,
    launch_gate_eligible: formalLaunchGateEligible,
...
export function evaluateGoldenSemAccuracy({
  ...
  const formalGroundTruthValidation = validateFormalLaunchGroundTruth(dataset);
  const truthPolicy = evaluationTruthPolicy(dataset, formalGroundTruthValidation, validation.ok);
  const normalizeComparisonValue = (field, value) => (
    truthPolicy.writer_title_used_as_field_ground_truth && field === "product"
      ? productProxyComparisonKey(value)
      : normalizeGoldenSemValue(field, value)
  );
  ...
  let cardExactIneligibleCount = 0;
  let zeroFieldCardCount = 0;
  let singleFieldCardCount = 0;
  const criticalCoverageCounts = Object.fromEntries(
    Object.keys(formalLaunchCriticalFieldGroups).map((dimension) => [dimension, 0])
  );
  ...
  for (const item of items) {
    ...
    for (const field of goldenSemLaunchFields) {
      ...
      const expected = excluded ? null : normalizeComparisonValue(field, groundTruth);
      const actual = excluded ? null : normalizeComparisonValue(field, prediction[field]);
      ...
      fields[field] = {
        ...
        comparison_policy: truthPolicy.writer_title_used_as_field_ground_truth && field === "product"
          ? "TITLE_PROXY_PRODUCT_HIERARCHY"
          : "STRICT_SEM_FIELD",
    ...
    const cardExactEligible = evaluatedFields.length >= minimumCardExactEvaluatedFields;
    const cardExact = cardExactEligible ? errors.length === 0 : null;
    if (!cardExactEligible) {
      cardExactIneligibleCount += 1;
      if (evaluatedFields.length === 0) zeroFieldCardCount += 1;
      if (evaluatedFields.length === 1) singleFieldCardCount += 1;
    }
    if (cardExact !== null) {
    ...
    for (const [dimension, aliases] of Object.entries(formalLaunchCriticalFieldGroups)) {
      if (aliases.some((field) => fields[field] && !fields[field].excluded_from_denominator)) {
        criticalCoverageCounts[dimension] += 1;
      }
    }
    cards.push({
      ...
      card_exact_eligible: cardExactEligible,
  ...
  const evaluationCompleted = validation.ok && evaluatedCardCount > 0;
  const status = !evaluationCompleted
    || (truthPolicy.formal_golden_sem && formalGroundTruthValidation.review_metadata.ok !== true)
    ? "INCONCLUSIVE"
    : truthPolicy.writer_title_used_as_field_ground_truth
      ? "COMPLETED_PROXY"
      : truthPolicy.formal_golden_sem
        ? "COMPLETED"
        : "COMPLETED_DIAGNOSTIC";
  const criticalFieldCoverage = {
    required_dimensions: Object.keys(formalLaunchCriticalFieldGroups),
    total_cards: items.length,
    dimensions: Object.fromEntries(Object.entries(formalLaunchCriticalFieldGroups).map(([dimension, fields]) => [
      dimension,
      {
        accepted_field_names: fields,
        evaluated_cards: criticalCoverageCounts[dimension],
        total_cards: items.length,
        rate: rate(criticalCoverageCounts[dimension], items.length)
      }
    ]))
  };
  return {
    ...
    status,
    formal_launch_gate_eligible: truthPolicy.formal_launch_gate_eligible,
    source: {
    ...
    scope: {
      ...
      minimum_card_exact_evaluated_fields: minimumCardExactEvaluatedFields,
    ...
    summary: {
      ...
      card_exact_eligible_count: evaluatedCardCount,
      card_exact_ineligible_count: cardExactIneligibleCount,
    ...
    metrics: {
      ...
      card_exact_evaluable_coverage: {
        minimum_evaluated_fields_required: minimumCardExactEvaluatedFields,
        eligible_card_count: evaluatedCardCount,
        ineligible_card_count: cardExactIneligibleCount,
        zero_field_card_count: zeroFieldCardCount,
        single_field_card_count: singleFieldCardCount,
    ...
    validation: {
      ...validation,
      formal_launch_ground_truth: formalGroundTruthValidation
    },
    cards
  };
}
