...
function evaluationTruthPolicy(dataset = {}) {
  ...
  const fieldGroundTruthClass = cleanText(
    explicit.field_ground_truth_class || "HUMAN_REVIEWED_FIELD_GROUND_TRUTH"
  ).toUpperCase();
  const formalGoldenSem = fieldGroundTruthClass === "HUMAN_REVIEWED_FIELD_GROUND_TRUTH";
  return {
    field_ground_truth_class: fieldGroundTruthClass,
    formal_golden_sem: formalGoldenSem,
    launch_gate_eligible: formalGoldenSem && explicit.launch_gate_eligible !== false,
...
export function evaluateGoldenSemAccuracy({
  ...
  return {
    ...
    status: validation.ok && evaluatedCardCount > 0
