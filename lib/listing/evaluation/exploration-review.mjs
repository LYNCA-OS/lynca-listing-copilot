...
/** Cards available to evaluate on. Beyond this, "run more" is not an option. */
export const CORPUS_SIZE = 255;

/**
 * How many cards would it take to resolve THIS effect, and do we have them?
 *
 * The question a founder should not have to ask, and that I answered by
 * instinct once: an arm came back +0.0051 at 9W/7L and my recommendation was
 * "run 150". The arithmetic says otherwise, and the arithmetic is free.
 *
 * A sign test needs discordant pairs, not cards. At a win share of w among
 * discordant pairs, detecting it at alpha 0.05 with power 0.8 takes about
 * (1.96 + 0.84)^2 / (4 (w - 0.5)^2) of them, and ties are dead weight -- an arm
 * that changes nothing on 68% of cards spends 68% of every run learning
 * nothing.
 *
 * When the answer exceeds the corpus, "scale up" is not a plan. The treatment
 * has to change, or the effect has to be accepted as unresolvable here.
 */
export function requiredSampleSize({ wins, losses, ties }) {
  const discordant = wins + losses;
  const n = discordant + ties;
  if (!discordant || !n) return { resolvable: false, reason: "no discordant pairs to test" };
  const winShare = wins / discordant;
  const edge = Math.abs(winShare - 0.5);
  if (edge < 1e-9) return { resolvable: false, reason: "wins and losses are exactly balanced" };
  const discordantNeeded = Math.ceil((1.96 + 0.84) ** 2 / (4 * edge ** 2));
  const discordantRate = discordant / n;
  const cardsNeeded = Math.ceil(discordantNeeded / discordantRate);
  return {
    resolvable: cardsNeeded <= CORPUS_SIZE,
    winShare,
    discordantRate,
    discordantNeeded,
    cardsNeeded,
...
/**
 * What to do next, decided rather than discussed.
 *
 * Ordered by what the evidence supports, not by what is easiest to try. The
 * "tighten to the dimension that gained" branch exists because an arm that
 * bought one dimension too dear is not a failed idea -- it is an idea applied
 * too widely, and that is a different next step from abandoning it.
 */
export function recommendNext({ delta, wins, losses, ties, ledger = [], belowDrift }) {
  const power = requiredSampleSize({ wins, losses, ties });
  if (delta <= 0) {
    const gained = ledger.filter((d) => d.gained > 0);
    return gained.length
      ? { action: "NARROW", detail: `整体为负，但 ${gained.map((d) => d.name).join("、")} 有真实增益。把改动收窄到这些维度再测，而不是整体重试。` }
      : { action: "ABANDON", detail: "整体为负且无维度增益。记录否决，不要再试这个机制。" };
  }
  if (!belowDrift && wins >= 8 && losses === 0) {
    return { action: "SHIP", detail: "清盘且高于漂移。上线，并把测量写进模块注释。" };
  }
  if (power.resolvable) {
    return {
      action: "SCALE",
      detail: `方向为正但未判定。按当前胜率 ${(power.winShare * 100).toFixed(0)}%（不一致对占 ${(power.discordantRate * 100).toFixed(0)}%），需要约 ${power.cardsNeeded} 张才能判定，语料有 ${power.corpus} 张，够。加样本。`
    };
  }
  const bought = ledger.filter((d) => d.verdict === "BOUGHT_TOO_DEAR");
  return {
    action: bought.length ? "NARROW" : "ABANDON",
    detail: `方向为正但在本语料上无法判定：需要约 ${power.cardsNeeded} 张，只有 ${power.corpus} 张。加样本不是出路。`
      + (bought.length
        ? ` ${bought.map((d) => d.name).join("、")} 是"买贵了"——把改动收窄到只影响它，把噪声维度排除，再测。`
        : " 换机制，或接受它在此语料上不可判定。")
  };
}
...
export function review({
  ...
  // Which cards actually moved. Printed unconditionally: asking the founder
  // whether they would like to see them is the same failure as asking whether
  // to run the review at all.
  movers = [],
  ...
  return {
    ...
    blockers,
    next: recommendNext({
      delta, wins, losses, ties, ledger, belowDrift: Math.abs(delta) < MEASURED_DRIFT
    }),
...
export function formatReview(r) {
  ...
  if (r.movers?.length) {
    const won = r.movers.filter((m) => m.delta > 0).sort((a, b) => b.delta - a.delta);
    const lost = r.movers.filter((m) => m.delta < 0).sort((a, b) => a.delta - b.delta);
    const show = (label, rows) => {
      if (!rows.length) return;
      lines.push(`  ${label}（${rows.length} 张，列出前 ${Math.min(4, rows.length)}）:`);
      for (const m of rows.slice(0, 4)) {
        lines.push(`    ${m.delta > 0 ? "+" : ""}${m.delta.toFixed(3)}`);
        lines.push(`      对照: ${m.control_title}`);
        lines.push(`      处理: ${m.treatment_title}`);
        lines.push(`      参考: ${m.reference}`);
      }
    };
    show("赢的卡", won);
    show("输的卡", lost);
  }
  if (r.next) lines.push(`  下一步 [${r.next.action}]  ${r.next.detail}`);
