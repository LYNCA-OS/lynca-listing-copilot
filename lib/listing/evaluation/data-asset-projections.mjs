...
function safeImages(images = []) {
  return (Array.isArray(images) ? images : []).map((image = {}, index) => {
    ...
    return {
      ...
      object_path: sanitizeStorageObjectPath(image.object_path || image.path) || null,
      content_sha256: /^[0-9a-f]{64}$/.test(contentSha256) ? contentSha256 : null,
...
function verifiedContentPinned(image = {}) {
  return Boolean(
    image.bucket
    && image.object_path
...
function hasSupportingEvidence(evidenceSources = {}) {
  return Object.values(plainObject(evidenceSources)).some((source) => {
    const record = plainObject(source);
    return cleanText(record.status).toUpperCase() === "SUPPORTED"
      && Array.isArray(record.evidence_refs)
      && record.evidence_refs.some((reference) => cleanText(reference));
  });
}
...
export function buildGoldenTitleCandidate({ feedbackEvent = {}, semExtraction = null, images = [] } = {}) {
  ...
  if ([feedbackEvent.dataset_disposition, writerFeedback.dataset_disposition]
    .some((value) => cleanText(value).toUpperCase() === "ADMIN_TEST_ONLY")) return null;
  ...
  const imageContentPinned = imageReferenceAvailable
    && safeImageRecords.every(verifiedContentPinned);
  const freezeBlockers = imageReferenceAvailable
    ? imageContentPinned ? [] : ["IMAGE_STORAGE_VERIFICATION_REQUIRED"]
...
export function buildGoldenSemCandidate({
  ...
  const contentFixedImages = (titleCandidate?.images || []).filter(verifiedContentPinned);
  if (!titleCandidate
      || !titleCandidate.freeze_eligible
      || !contentFixedImages.length
      || !approved
      || review.semantic_truth !== true
      || review.golden_sem_candidate !== true
      || cleanText(review.sem_standard_version) !== SEM_STANDARD_VERSION
      || cleanText(review.parser_version) !== WRITER_TITLE_SEM_PARSER_VERSION
      || !hasSupportingEvidence(review.evidence_sources)
      || !reviewer
      || !reviewedAt
