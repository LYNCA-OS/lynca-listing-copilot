...
export const evaluationDecisionTraceSchemaVersion = "evaluation-decision-trace-packet-v9";
...
export function buildEvaluationDecisionTracePacket(result = {}, payload = {}) {
  ...
  const unobservableReviewedCandidates = sourceAuditCandidates.filter((candidate) => (
    /internal|reviewed|corrected|writer/i.test(cleanText(
  ...
  return Object.freeze({
    ...
    knowledge_first_route: result.knowledge_first_route_shadow
      ...
      : null,
    provider_aux_route: result.knowledge_first_route_shadow?.provider_aux_route_shadow
      ? replayValue(withObservedProviderAuxRoute(
          result.knowledge_first_route_shadow.provider_aux_route_shadow,
          {
    ...
    targeted_assist_execution: result.targeted_assist_execution
      ? replayValue({
          ...
          // Trace persists only typed counters and hashes. The bounded
          // executor never exposes the model's natural-language response.
          provider_call_ledger: array(result.provider_call_ledger
            || result.targeted_assist_execution.provider_call_ledger)
        })
      : null,
