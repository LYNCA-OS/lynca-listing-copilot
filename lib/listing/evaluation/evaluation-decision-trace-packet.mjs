...
export const evaluationDecisionTraceSchemaVersion = "evaluation-decision-trace-packet-v4";
export const evaluationReplaySnapshotSchemaVersion = "evaluation-replay-snapshot-v2";
...
function buildReplaySnapshot(result = {}, payload = {}) {
  ...
  return {
    ...
    provider_field_evidence: replayValue(result.raw_provider_field_evidence || result.field_evidence || []),
    ...
    derivation_provenance: replayValue(result.forward_enumeration_trace || result.derivation_provenance || []),
...
export function buildEvaluationDecisionTracePacket(result = {}, payload = {}) {
  ...
  return Object.freeze({
    ...
    knowledge_first_route: result.knowledge_first_route_shadow
      ...
      : null,
    world_knowledge: result.world_knowledge ? {
      schema_version: cleanText(result.world_knowledge.schema_version, 120) || null,
      enabled: result.world_knowledge.enabled === true,
      proposal_count: Number(result.world_knowledge.proposal_count || 0),
      accepted_count: Number(result.world_knowledge.accepted_count || 0),
      unchecked_count: Number(result.world_knowledge.unchecked_count || 0),
      refuted_count: Number(result.world_knowledge.refuted_count || 0),
      invalid_count: Number(result.world_knowledge.invalid_count || 0),
      decisions: array(result.world_knowledge.decisions).slice(0, 8).map((decision) => ({
        field: cleanText(decision?.field, 80) || null,
        value: cleanText(decision?.value, 160) || null,
        basis: cleanText(decision?.basis, 40) || null,
        disposition: cleanText(decision?.disposition, 40) || null,
        checked: decision?.checked === true,
        reason: cleanText(decision?.reason, 120) || null,
        allowed_values: array(decision?.allowed_values).map((value) => cleanText(value, 120)).filter(Boolean).slice(0, 12)
      }))
    } : null,
