...
export const fieldLineageLedgerSchemaVersion = "field-lineage-ledger-v1";
...
function buildFieldLineage(result = {}, candidates = []) {
  ...
  return Object.entries(canonicalFieldAliases).map(([field, aliases]) => {
    ...
    const finalSpanMatched = finalMatchedValues.length > 0;
    const providerStage = {
    ...
    const selectedDecisions = candidateDecisions.filter((decision) => decision.selected);
    const resolverStage = {
    ...
    // rendered_fields is diagnostic metadata, not the final authority. A value
    // that is present in the final title must not be reported as a renderer
    // drop merely because an older renderer module omitted its field snapshot.
    const rendererIncluded = renderedPresent || finalSpanMatched;
    const rendererStage = {
      owner: "DETERMINISTIC_RENDERER",
      version: cleanText(result.renderer_version, 120) || null,
      values: rendererValues,
      decision: resolvedPresent && !rendererIncluded ? "DROP" : rendererIncluded ? "INCLUDE" : "NO_VALUE",
      reason: resolvedPresent && !rendererIncluded
        ? "RENDERER_NOT_INCLUDED"
        : renderedPresent ? "RENDERER_MODULE_PRESENT"
          : finalSpanMatched ? "FINAL_TITLE_SPAN_CONFIRMS_INCLUDE"
            : "NO_RESOLVED_VALUE"
    };
    const finalTitleStage = {
      owner: "FINAL_TITLE_SPAN_MATCHER",
      version: "final-title-span-v1",
      matched: finalSpanMatched,
      matched_values: finalMatchedValues,
      decision: finalSpanMatched ? "MATCH" : "NO_MATCH",
      reason: finalSpanMatched ? "FIELD_VALUE_PRESENT_IN_FINAL_TITLE" : "FIELD_VALUE_NOT_FOUND_IN_FINAL_TITLE"
    };
    return {
      field,
      raw_provider: providerStage,
      normalized: normalizationStage,
      retrieval_supported: retrievalStage,
      candidate_selection: {
        owner: "CANDIDATE_SELECTION",
        version: retrievalStage.version,
        decisions: selectedDecisions,
        decision: selectedDecisions.length ? "SELECTED" : "NO_SELECTED_FIELD_SUPPORT",
        reason: selectedDecisions.length ? "SELECTED_CANDIDATE_FIELD_DECISION_PRESENT" : "NO_SELECTED_CANDIDATE_FIELD_DECISION"
      },
      candidate_application: {
        owner: "CANDIDATE_APPLICATION",
        version: retrievalStage.version,
        decisions: selectedDecisions,
        decision: selectedDecisions.some((entry) => entry.action === "APPLY")
          ? "APPLY"
          : selectedDecisions.some((entry) => entry.action === "SUPPORT") ? "SUPPORT" : selectedDecisions.length ? "BLOCK" : "NO_DECISION",
        reason: selectedDecisions[0]?.reason || "NO_SELECTED_CANDIDATE_FIELD_DECISION"
      },
      resolver_result: resolverStage,
      renderer_module: rendererStage,
      final_title_span: finalTitleStage,
      // Compatibility aliases. New audits must read the named v1 stages above.
...
export function buildFieldLineageLedger(result = {}, candidates = candidateRows(result)) {
  return Object.freeze({
    schema_version: fieldLineageLedgerSchemaVersion,
    owner: "EVALUATION_FIELD_LINEAGE",
    fields: buildFieldLineage(result, candidates)
  });
}
...
export function buildEvaluationDecisionTracePacket(result = {}, payload = {}) {
  ...
  const fieldLineageLedger = buildFieldLineageLedger(result, candidates);
  const retrieval = object(result.retrieval);
  ...
  return Object.freeze({
    ...
    field_lineage_ledger: fieldLineageLedger,
    field_lineage: fieldLineageLedger.fields,
