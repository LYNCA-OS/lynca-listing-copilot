...
async function vectorComponent(env = {}, fetchImpl = globalThis.fetch) {
  ...
  if (!config.enabled) {
    if (indexReady && workerConfigured && runtime.ready) {
      return component({
        id: "vector_retrieval",
        status: componentStatuses.READY,
        summary: "Vector index and worker are ready; online retrieval is opt-in per request and disabled by default.",
        details: {
          default_enabled: false,
          online_retrieval_default_enabled: false,
          ...
          prompt_influence_by_default: false,
    ...
    return component({
      id: "vector_retrieval",
      status: indexReady && workerConfigured ? componentStatuses.DEGRADED : componentStatuses.DISABLED,
      fail_closed: true,
      summary: indexReady && workerConfigured
        ? "Vector index exists, but the dedicated worker has not proved a preloaded runtime and request-level assist remains fail-closed."
        : indexReady
        ? "Vector index is marked ready, but the worker is incomplete and online retrieval remains disabled."
        : "Vector index is not marked ready and online retrieval is disabled.",
      details: {
        default_enabled: false,
        online_retrieval_default_enabled: false,
        default_mode: config.mode,
      ...
      next_action: "Deploy a dedicated preloaded vector worker, then enable request-level assist only after readiness and an embedding canary pass."
    });
  }
  ...
  return component({
    ...
    details: {
      ...
      preload_status: runtime.preload_status,
      runtime_reason: runtime.reason,
      request_override_supported: runtime.ready,
      prompt_influence_by_default: config.mode === vectorRetrievalModes.ASSIST,
      mode: config.mode,
      model_id: config.modelId,
      model_revision: config.modelRevision,
      preprocessing_version: config.preprocessingVersion,
    ...
    next_action: runtime.ready && config.mode === vectorRetrievalModes.ASSIST
      ? ""
      : runtime.ready
