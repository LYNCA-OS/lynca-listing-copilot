...
// "Rookie Card") states that component as fact, not as a visual guess. Only the
// explicit "<component> card" wording qualifies, so product prose such as
// "PRO CARDS" or an insert named "Mojo Prizm" never derives a component.
const declaredComponentPattern = /\b(relic|memorabilia|swatch|logoman|patch|jersey|auto|autograph|autographed|signature|rookie|sketch|redemption)\s+cards?\b/gi;

function componentsFromDeclaredName(value) {
  const text = normalizeText(value);
  if (!text) return [];
  const components = [];
  for (const match of text.matchAll(declaredComponentPattern)) {
    const component = canonicalComponent(match[1]);
...
export function observableComponentsFromResolved(resolved = {}) {
  ...
  [resolved.card_name, resolved.set].forEach((value) => {
    componentsFromDeclaredName(value).forEach((component) => {
      if (!parts.includes(component)) parts.push(component);
    });
  });
  return parts;
...
function normalizeOfficialCardType(value) {
  ...
  if (/^(?:patch)\s*\/\s*(?:auto|autograph|signature)$/i.test(text)) return "Patch Auto";
  if (/^(?:auto|autograph|signature)\s*\/\s*(?:patch)$/i.test(text)) return "Patch Auto";
...
export function cardTypeTextParts(resolved = {}, {
  ...
  if (hasAuto && hasPatch && !(insertHasAuto && insertHasRelic)) {
    pushUniquePhrase(parts, "Patch Auto");
