...
function normalizeNumericalRarityText(text) {
  const value = normalizeText(text)
    .replace(/^#\s*/, "")
    .replace(/\s*\/\s*/g, "/");
  const full = value.match(/^0*\d{1,5}\/0*\d{1,5}$/);
  if (full) return value;
  const denominatorOnly = value.match(/^\/0*\d{1,5}$/);
  if (denominatorOnly) return value;
  return "";
}
...
function applyNumericalRarityEdit(resolved, text) {
  ...
  return {
    ...resolved,
    numerical_rarity: numericalRarity || null,
    one_of_one: numericalRarity === "1/1" || resolved.one_of_one === true && Boolean(numericalRarity)
  };
}
...
function moduleFieldNames(moduleKey) {
  return {
    ...
    numerical_rarity: ["numerical_rarity", "one_of_one"],
