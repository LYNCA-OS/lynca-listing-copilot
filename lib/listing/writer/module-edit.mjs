...
function applyReleaseVariantEdit(resolved, text) {
  const next = {
    ...resolved,
    official_card_type: resolved.official_card_type || null,
    insert: null,
    variation: null,
    subset: null
  };
  ...
  if (/\b(?:horizontal|vertical|variation|image variation|photo variation|international)\b/i.test(text)) {
    next.variation = text;
  } else {
    next.insert = text;
  }
...
function removeColorFromFinish(text, color) {
  if (!color) return text;
  return normalizeText(text.replace(new RegExp(`\\b${color.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "ig"), " "));
}

function applyPrintFinishEdit(resolved, text) {
  const next = {
    ...resolved,
    surface_color: null,
  ...
  const family = removeColorFromFinish(text, color);
  next.surface_color = color || null;

  if (family) {
    next.parallel_family = family;
    next.parallel_exact = text;
    next.parallel = text;
  } else if (!color) {
    next.parallel_family = text;
    next.parallel = text;
  }
...
function applyRarityEdit(resolved, text) {
  const lower = text.toLowerCase();
  const printRun = expandPrintRunFields({ print_run_number: text });
  const oneOfOne = /\b1\s*\/\s*1\b|one\s+of\s+one/.test(lower);
  return {
    ...resolved,
    ...
    ...(oneOfOne ? printRun : {}),
    numerical_rarity: oneOfOne ? printRun.print_run_number || null : resolved.numerical_rarity || null,
    one_of_one: oneOfOne || printRun.one_of_one === true
  };
}
...
function extractNumberTokens(text) {
  const tokenPattern = /#?\d{1,4}\s*\/\s*\d{1,4}|#?\s*\/\s*\d{1,4}|#?\d{1,4}[A-Z]?|[A-Z]{1,10}[- ][A-Z0-9]{1,16}/gi;
...
function applyNumberingEdit(resolved, text) {
  ...
  extractNumberTokens(text).forEach((token) => {
    ...
    Object.entries(split).forEach(([field, value]) => {
      if (!value) return;
      if (field === "serial_number" && !next.print_run_number) {
        const printRun = expandPrintRunFields({ print_run_number: value, serial_number: value });
        Object.assign(next, printRun);
        next.numerical_rarity = printRun.print_run_number || null;
        return;
      }
...
function normalizeNumericalRarityText(text) {
  const printRun = expandPrintRunFields({ print_run_number: text });
  return printRun.print_run_number || "";
}

function applyNumericalRarityEdit(resolved, text) {
  const numericalRarity = normalizeNumericalRarityText(text);
  const printRun = expandPrintRunFields({ print_run_number: numericalRarity });
  return {
    ...resolved,
    print_run_number: null,
    print_run_numerator: null,
    ...
    print_run_review_required: false,
    ...printRun,
    numerical_rarity: printRun.print_run_number || null,
    one_of_one: printRun.one_of_one === true || resolved.one_of_one === true && Boolean(printRun.print_run_number)
  };
}
...
function applyModuleText(resolved, moduleKey, moduleText) {
  ...
  switch (moduleKey) {
    ...
    case "release_variant":
      return applyReleaseVariantEdit(resolved, text);
    case "print_finish":
      return applyPrintFinishEdit(resolved, text);
    case "number_serial_grade":
      return applyNumberSerialGradeEdit(resolved, text);
    case "numerical_rarity":
      return applyNumericalRarityEdit(resolved, text);
    case "card_number":
    case "numbering":
      return applyNumberingEdit(resolved, text);
    case "descriptive_rarity":
...
function moduleFieldNames(moduleKey) {
  return {
    ...
    variant_parallel_rarity: ["insert", "surface_color", "parallel_family", "parallel_exact", "parallel", "variation", "subset", "rc", "first_bowman", "ssp", "case_hit", "print_run_number", "print_run_numerator", "print_run_denominator", "numbered_to", "serial_number", "serial_denominator", "numerical_rarity", "one_of_one"],
    card_variant: ["card_type", "insert", "surface_color", "parallel_family", "parallel_exact", "parallel", "variation", "subset"],
    release_variant: ["official_card_type", "insert", "variation", "subset"],
    print_finish: ["surface_color", "parallel_family", "parallel_exact", "parallel"],
    numerical_rarity: ["print_run_number", "print_run_numerator", "print_run_denominator", "numbered_to", "serial_number", "serial_denominator", "numerical_rarity", "one_of_one", "suspicious_print_run", "print_run_review_required"],
    descriptive_rarity: ["ssp", "case_hit"],
    card_number: ["collector_number", "checklist_code"],
    search_optimization: ["rc", "first_bowman", "auto", "patch", "relic", "jersey", "sketch", "redemption", "team"],
    number_serial_grade: ["print_run_number", "print_run_numerator", "print_run_denominator", "numbered_to", "serial_number", "serial_denominator", "numerical_rarity", "suspicious_print_run", "print_run_review_required", "collector_number", "checklist_code", "grade_company", "card_grade", "auto_grade", "grade_type"],
    numbering: ["print_run_number", "print_run_numerator", "print_run_denominator", "numbered_to", "serial_number", "serial_denominator", "numerical_rarity", "suspicious_print_run", "print_run_review_required", "collector_number", "checklist_code"],
    attributes: ["rc", "first_bowman", "ssp", "case_hit", "auto", "patch", "relic", "sketch", "redemption", "print_run_number", "print_run_numerator", "print_run_denominator", "numbered_to", "serial_number", "serial_denominator", "numerical_rarity", "one_of_one"],
