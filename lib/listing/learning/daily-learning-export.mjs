...
function normalizedImagesByAsset(value = {}) {
  ...
  if (Array.isArray(value)) {
    for (const image of value) {
      const assetId = cleanText(image?.asset_id);
      if (!assetId) continue;
      grouped.set(assetId, [...(grouped.get(assetId) || []), image]);
    }
    return grouped;
  }
  for (const [assetId, images] of Object.entries(plainObject(value))) {
    grouped.set(cleanText(assetId), arrayValue(images));
  }
  return grouped;
}
...
function safeImagesFor({ assetId = "", imagesByAsset = new Map(), additionalImages = [] } = {}) {
  const images = [...arrayValue(imagesByAsset.get(cleanText(assetId))), ...arrayValue(additionalImages)];
  const unique = new Map();
  for (const image of images) {
    const key = [image?.bucket, image?.object_path || image?.path, image?.content_sha256].map(cleanText).join(":");
...
function exportFeedbackEvent(feedbackEvent = {}) {
  ...
  const value = {
    ...
    writer_final_title: cleanText(writerFeedback.final_title || feedbackEvent.writer_raw_title) || null,
...
function exportSemanticCandidate(learningEvent = {}, feedbackEvent = {}, validationEvent = {}) {
  ...
  const candidate = {
    ...
    title_truth: true,
    semantic_truth: recordedValidation.validation_status === "VALIDATED",
    dataset_disposition: FEEDBACK_DATASET_DISPOSITION,
    training_eligible: false
  };
...
export function buildDailyLearningExport(bundle = {}, { date, generatedAt } = {}) {
  ...
  for (const learningEvent of learningEvents) {
    ...
    const validationEvent = sortedRecords(
  ...
  for (const feedbackEvent of feedbackEvents) {
    const matchingLearning = sortedRecords(learningByFeedbackId.get(cleanText(feedbackEvent.id)) || []);
    const latestLearning = matchingLearning.at(-1) || {};
    const latestValidation = sortedRecords(
    ...
    const images = safeImagesFor({
      ...
      imagesByAsset,
      additionalImages: feedbackEvent.images
    });
  ...
  const datasets = {
    ...
    semantic: sortedRecords(semantic, "candidate_id"),
    errors: sortedRecords(errors, "candidate_id"),
    golden: sortedRecords(golden, "candidate_id")
  };
  ...
  const manifest = {
    ...
    privacy: {
      ...
      image_reference_policy: "bucket_object_path_and_content_sha256_only"
    }
  };
