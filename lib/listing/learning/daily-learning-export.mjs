...
function exportFeedbackEvent(feedbackEvent = {}) {
  ...
  const value = {
    ...
    dataset_disposition: cleanText(feedbackEvent.dataset_disposition) || FEEDBACK_DATASET_DISPOSITION,
    training_eligible: false
  };
  return recordWithHash(value);
}

function isAdminTestFeedback(feedbackEvent = {}, learningEvent = {}) {
  return [
    feedbackEvent.dataset_disposition,
    feedbackEvent.writer_feedback?.dataset_disposition,
    learningEvent.dataset_disposition,
    learningEvent.feedback_training_event?.dataset_disposition
  ]
    .some((value) => cleanText(value).toUpperCase() === "ADMIN_TEST_ONLY");
}
...
function exportSemanticCandidate(learningEvent = {}, feedbackEvent = {}, validationEvent = {}) {
  if (isAdminTestFeedback(feedbackEvent, learningEvent)) return null;
...
export function buildDailyLearningExport(bundle = {}, { date, generatedAt } = {}) {
  ...
  for (const feedbackEvent of latestFeedbackEvents(feedbackEvents)) {
    ...
    if (isAdminTestFeedback(feedbackEvent, latestLearning)) continue;
