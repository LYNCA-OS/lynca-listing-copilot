...
export const worldKnowledgeLayerVersion = "world-knowledge-layer-v3";
...
export function validateWorldKnowledgeProposal(proposal = {}, claim = {}, model = null) {
  ...
  if (basis === "OBSERVED") {
    // The proposal lane carries no image id, source region, or visible text.
    // A model self-label therefore cannot promote itself into direct visual
    // evidence. Keep it observable in Trace until the normal evidence contract
    // independently corroborates the same value.
    return {
      field,
      value,
      basis,
      ...
      checked: false,
      reason: "observed_proposal_lacks_direct_evidence"
    };
  }
  if (!model) {
...
export function attachWorldKnowledgeProposals(result = {}, model = null, { enabled = false } = {}) {
  ...
  // UNCHECKED proposals are useful for coverage analysis, but they have no
  // independent authority. Keep them in the decision trace and never let them
  // enter Identity Resolver as evidence. This prevents model memory or a
  // self-labelled visual claim from becoming self-validating identity.
  const evidenceEligible = decisions.filter((row) => row.disposition === "ACCEPTED");
  const identityEvidenceItems = enabled ? evidenceEligible.map((row) => ({
    field: row.field,
    value: row.value,
  ...
  return {
    ...
    world_knowledge: {
      ...
      trace_only_count: decisions.filter((row) => row.disposition === "UNCHECKED").length,
      evidence_eligible_count: evidenceEligible.length,
