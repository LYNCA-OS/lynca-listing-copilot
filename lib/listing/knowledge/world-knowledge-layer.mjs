...
import { norm, outcomes } from "../catalog/constraint-enumerator.mjs";

export const worldKnowledgeLayerVersion = "world-knowledge-layer-v1";
...
function playerForClaim(claim = {}) {
  return norm(Array.isArray(claim.players) && claim.players.length
    ? claim.players[0]
    : claim.player || claim.subject);
}

function setForClaim(claim = {}) {
  return norm(claim.set || claim.subset || claim.insert || claim.card_name);
}

function allowedTeamValues(claim = {}, model = {}) {
  const player = playerForClaim(claim);
  if (!player) return { checked: false, reason: "no_subject_read", values: [] };
  const career = model.player_teams?.[player];
  if (!Array.isArray(career) || !career.length) {
    return { checked: false, reason: "subject_not_in_model", values: [] };
  }
  const year = seasonStartYear(claim.year);
  const byYear = year ? model.player_team_years?.[player]?.[String(year)] : null;
  return {
    checked: true,
    reason: Array.isArray(byYear) && byYear.length ? "player_team_year_covered" : "player_career_team_covered",
    values: Array.isArray(byYear) && byYear.length ? byYear : career
  };
}

function productValue(entry = "") {
  return cleanText(entry).split("|").at(-1);
}

function allowedProductValues(claim = {}, model = {}) {
  const set = setForClaim(claim);
  if (!set) return { checked: false, reason: "no_set_name_read", values: [] };
  const entries = model.set_product_years?.[set];
  if (!Array.isArray(entries) || !entries.length) {
    return { checked: false, reason: "set_not_in_model", values: [] };
  }
  const year = seasonStartYear(claim.year);
  const yearEntries = year
    ? entries.filter((entry) => cleanText(entry).startsWith(`${year}|`))
    : [];
  const applicable = yearEntries.length ? yearEntries : entries;
  return {
    checked: true,
    reason: yearEntries.length ? "set_product_year_covered" : "set_product_covered",
    values: [...new Set(applicable.map(productValue).filter(Boolean))]
  };
}
