 ...
 * never infers physical-copy fields. A reviewed original-set match may correct
 * only release year/set when all four independent text anchors select the same
 * record; every other conflicting or non-unique input still abstains.
 */
export function resolveExternalIdentitySupport(fields = {}, {
  ...
  const correctedFields = new Set(matchMode === "VERIFIED_ORIGINAL_SET" && sameFourAnchorRecord
    ? conflictFields.filter((field) => field === "year" || field === "set")
    : []);
