...
const RECEIPT_SCHEMA = "csm-external-identity-support-receipt.v2";
...
const VERIFIED_ORIGINAL_SET_REQUIRED_ANCHORS = Object.freeze([
  "manufacturer",
  "subject",
  "card_number"
]);
...
export const EXTERNAL_IDENTITY_RESOLVER_VERSION = "thin-path-exact-external-identity-v3";
export const EXTERNAL_IDENTITY_CONFLICT_POLICY_VERSION =
  "verified-original-set-four-anchor-release-correction-v3";
export const EXTERNAL_IDENTITY_COMPOSER_VERSION =
  "thin-marketplace-composer-v4-verified-external-identity";
export const EXTERNAL_IDENTITY_MARKETPLACE_PROFILE_VERSION =
  "ebay-verified-external-identity-v2";
export const EXTERNAL_IDENTITY_REGISTRY_RELEASE_ID =
...
const RESOLUTION_CONTRACT_PAYLOAD = {
  ...
  visible_conflict_policy: "verified_original_set_plus_four_anchor_corrects_year_set_else_abstain",
...
// Forward-readable activation contract. It is published beside, not through,
// the active v2 alias above: bridge deployments can validate future v3 rows
// without changing a single fresh write or advertised runtime behavior.
export const EXTERNAL_IDENTITY_RESOLUTION_CONTRACT_V3 = deepFreeze({
  schema_version: "csm-post-observation-resolution-contract.v1",
  contract_id: "lynca.csm.post-observation.external-identity.v3",
  ...
  matching: "exact_unique_four_anchor_or_verified_original_set_three_anchor",
  visible_conflict_policy:
    "verified_original_set_plus_manufacturer_subject_card_number_corrects_product_year_set_else_abstain",
  physical_copy_fields: "immutable",
  provider_calls_added: 0,
  contract_sha256: "14a0c6dee064019e21840b19c419495e40cbdd4b6e8a97a57fdc7ba66c25e09e"
});

const EXTERNAL_IDENTITY_RESOLUTION_CONTRACTS_BY_RELEASE = deepFreeze({
...
export function externalIdentityResolutionContractForRegistryRelease(releaseId) {
  const contract = EXTERNAL_IDENTITY_RESOLUTION_CONTRACTS_BY_RELEASE[
    String(releaseId || "").trim()
  ];
  if (!contract) throw new TypeError("external_identity_resolution_release_unknown");
  return contract;
}
...
export const EXTERNAL_IDENTITY_RELEASE_CONTRACT_V3 = deepFreeze({
  schema_version: "csm-external-identity-release-contract.v1",
  support_pack: {
    id: EXTERNAL_IDENTITY_SUPPORT_PACK.pack_id,
    version: EXTERNAL_IDENTITY_SUPPORT_PACK.pack_version,
  ...
  resolution_contract: {
    id: EXTERNAL_IDENTITY_RESOLUTION_CONTRACT_V3.contract_id,
    sha256: EXTERNAL_IDENTITY_RESOLUTION_CONTRACT_V3.contract_sha256,
  ...
  registry_release: {
    id: EXTERNAL_IDENTITY_RESOLUTION_CONTRACT_V3.registry_release_id,
    content_sha256: EXTERNAL_IDENTITY_SUPPORT_PACK.pack_sha256
  }
});

export function externalIdentityReleaseContractForRegistryRelease(releaseId) {
  if (releaseId === EXTERNAL_IDENTITY_RESOLUTION_CONTRACT.registry_release_id) {
    return EXTERNAL_IDENTITY_RELEASE_CONTRACT;
  }
  if (releaseId === EXTERNAL_IDENTITY_RESOLUTION_CONTRACT_V3.registry_release_id) {
    return EXTERNAL_IDENTITY_RELEASE_CONTRACT_V3;
  }
  throw new TypeError("external_identity_release_contract_unknown");
}
...
export const EXTERNAL_IDENTITY_CAPTURED_ROLLBACK_RELEASE_IDS = deepFreeze([
  "registry_thin_external_identity_high_risers_v1",
  "registry_thin_external_identity_high_risers_v2"
]);
...
// reinterpreted as any release-scoped pack after a rollback.
...
const EXTERNAL_IDENTITY_FIELD_DECISION_POLICIES = deepFreeze({
  ...
  registry_thin_external_identity_high_risers_v3: {
    actions: ["FILL", "CORROBORATE", "NORMALIZE_ALIAS", "CORRECT_CONFLICT"],
    corrected_fields: ["year", "product", "set"],
    correction_match_modes: ["VERIFIED_ORIGINAL_SET"],
    ...
    correction_identity: {
      ...
      hard_anchors: ["manufacturer", "subjects", "card_number"]
    }
  }
});
...
/** Validate the private durable decision ledger for a released tuple. */
export function validateExternalIdentityFieldDecisions(receipt) {
  ...
  const verifiedOriginalHardAnchors = policy.decision.verified_original_hard_anchors;
  if (receipt.match_mode === "VERIFIED_ORIGINAL_SET"
      && Array.isArray(verifiedOriginalHardAnchors)
      && (!correction || recordId !== correction.record_id
        || !verifiedOriginalHardAnchors.every((field) => (
          decisionValuesEqual(
            field,
...
export function validateExternalIdentityPublicFieldDecisions(receipt) {
  ...
  const correctedFields = new Set(policy.decision.corrected_fields);
  const correctionMatchModes = new Set(policy.decision.correction_match_modes);
  const verifiedOriginalActions = receipt.match_mode === "VERIFIED_ORIGINAL_SET"
    ? policy.decision.verified_original_public_actions
    : null;
  return Object.entries(decisions).every(([field, decision]) => (
    decision && typeof decision === "object" && !Array.isArray(decision)
      && allowedActions.has(decision.action)
      && (!verifiedOriginalActions?.[field]
        || verifiedOriginalActions[field].includes(decision.action))
      && sameStringSet(decision.source_ids, expectedSourceIds(recordId, field))
...
function receiptBase(status, reason, resolutionContract = EXTERNAL_IDENTITY_RESOLUTION_CONTRACT) {
  return {
    schema_version: externalIdentityReplayReleaseForReceipt({
      registry_release_id: resolutionContract.registry_release_id
...
function abstain(fields, reason, details = {}, resolutionContract) {
  return {
    status: "ABSTAINED",
    reason,
    fields: { ...fields },
    support: null,
    receipt: {
      ...receiptBase("ABSTAINED", reason, resolutionContract),
      ...details
    }
  };
}

function optionalConflicts(fields, record, optionalFields = ["year", "set"]) {
  const conflicts = [];
...
function verifiedOptionalObservations(fields, record, optionalFields = ["year", "set"]) {
  const verified = {};
  for (const field of optionalFields) {
    ...
    verified[field] = {
      ...
      match: normalizeTextExact(observed) === normalizeTextExact(record[field])
        ? "CANONICAL"
        : "CORRECTED_CONFLICT"
    };
  }
...
function fieldDecision(fields, record, field, sourceIds, correctedFields = new Set()) {
...
/**
 * Exact, deterministic identity support for one explicit, closed release.
 * The active wrapper below remains pinned to v2; a forward reader selects v3
 * only through the writer contract bound to an already-paid checkpoint.
 */
export function resolveExternalIdentitySupportForRelease(fields = {}, {
  registryReleaseId,
  index = HIGH_RISERS_EXTERNAL_IDENTITY_INDEX,
  externalIdentityContext = null
} = {}) {
  const resolutionContract = externalIdentityResolutionContractForRegistryRelease(
    registryReleaseId
  );
  const forwardV3 = resolutionContract.registry_release_id
    === EXTERNAL_IDENTITY_RESOLUTION_CONTRACT_V3.registry_release_id;
  const releaseAbstain = (reason, details = {}) => (
    abstain(fields, reason, details, resolutionContract)
  );
  const anchors = anchorValues(fields);
  if (anchors.subject_count > 1) {
    return releaseAbstain("MULTIPLE_SUBJECTS", { subject_count: anchors.subject_count });
  }
  ...
  if (matches.length > 1) {
    return releaseAbstain("AMBIGUOUS_MATCH", {
  ...
  if (originalSetMatch.state === "BROKEN_MAPPING") {
    return releaseAbstain("ORIGINAL_SET_MAPPING_INVALID", {
  ...
  if (originalSetMatch.record && matches[0]
    ...
    return releaseAbstain("IDENTITY_MATCH_DISAGREEMENT", {
  ...
  if (!record) {
    if (missingAnchors.length) {
      return releaseAbstain("MISSING_REQUIRED_ANCHOR", {
        missing_anchors: missingAnchors,
        original_set_match: originalSetMatch.state
      });
    }
    return releaseAbstain("NO_EXACT_MATCH", {
  ...
  const conflictFields = matchMode === "VERIFIED_ORIGINAL_SET"
    ...
    : optionalConflicts(fields, record, forwardV3
      ? ["year", "product", "set"] : ["year", "set"]);
  const verifiedMissingAnchors = matchMode === "VERIFIED_ORIGINAL_SET" && forwardV3
    ? VERIFIED_ORIGINAL_SET_REQUIRED_ANCHORS.filter((name) => !anchors[name])
    : [];
  if (verifiedMissingAnchors.length) {
    return releaseAbstain("MISSING_REQUIRED_ANCHOR", {
      record_id: record.record_id,
      missing_anchors: verifiedMissingAnchors,
      match_mode: matchMode,
      original_set_sha256: originalSetMatch.digest
    });
  }
  const sameFourAnchorRecord = Boolean(matches[0]
    && matches[0].record_id === record.record_id);
  const correctionAuthorized = matchMode === "VERIFIED_ORIGINAL_SET"
    && (forwardV3 || sameFourAnchorRecord);
  const correctedFields = new Set(correctionAuthorized
    ? conflictFields.filter((field) => forwardV3
      ? ["year", "product", "set"].includes(field)
      : field === "year" || field === "set")
    : []);
  const blockingConflicts = conflictFields.filter((field) => !correctedFields.has(field));
  if (blockingConflicts.length) {
    return releaseAbstain("CONFLICTING_OBSERVATION", {
      record_id: record.record_id,
      conflict_fields: blockingConflicts,
      match_mode: matchMode,
  ...
  const fieldDecisions = Object.fromEntries(Object.keys(supportedFields).map((field) => [
    field,
    fieldDecision(fields, record, field, sourceFieldMap[field], correctedFields)
  ]));
  ...
  return {
    ...
    receipt: {
      ...receiptBase("APPLIED", null, resolutionContract),
      record_id: record.record_id,
      match_mode: matchMode,
      ...
      matched_anchors: Object.fromEntries(REQUIRED_ANCHORS.map((name) => [name, anchors[name]])),
      verified_optional_observations: verifiedOptionalObservations(
        fields,
        record,
        forwardV3 ? ["year", "product", "set"] : ["year", "set"]
      ),
      ...(correctedFields.size ? { corrected_fields: [...correctedFields].sort() } : {}),
      field_decisions: fieldDecisions,
...
export function resolveExternalIdentitySupport(fields = {}, options = {}) {
  return resolveExternalIdentitySupportForRelease(fields, {
    ...options,
