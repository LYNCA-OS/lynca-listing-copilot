...
function renderTitle(resolved = {}, result = {}, maxLength = 80) {
  ...
  const presentation = renderListingPresentation({
    ...
    maxLength,
    // Preserve UNKNOWN. `false` means current-image verification explicitly
    // rejected the numerator and is allowed to degrade 31/50 to #/50; absence
    // of a verification result must not silently claim that rejection.
    serialNumeratorVerified: result.serial_numerator_verified ?? result.serialNumeratorVerified ?? null
  });
...
function addsMeaningfulHierarchySpecificity(currentValue, candidateValue) {
  const current = new Set(comparableTokens(currentValue));
  const added = comparableTokens(candidateValue).filter((token) => !current.has(token));
  return added.some((token) => (
    !hierarchyOwnershipTokens.has(token)
...
function trustedHierarchyCoreSuperset(currentValue, candidateValue) {
  ...
  const currentCore = new Set(comparableTokens(currentValue)
    .filter((token) => !hierarchyOwnershipTokens.has(token)));
  const candidateCore = new Set(comparableTokens(candidateValue)
    .filter((token) => !hierarchyOwnershipTokens.has(token)));
  return currentCore.size > 0
    && candidateCore.size > currentCore.size
    && [...currentCore].every((token) => candidateCore.has(token));
}
...
function safeSelectedCandidateUpgradeReason(field, currentValue, candidateValue, applicationReason = "") {
  ...
  if (applicationReason === "trusted_reviewed_identity_year_fill" && field === "year") {
    return "trusted_reviewed_identity_year_replace";
  }
  if (applicationReason === "trusted_reviewed_identity_variant_fill"
    && ["card_name", "official_card_type", "surface_color", "parallel", "parallel_family", "parallel_exact", "variation"].includes(field)) {
    return "trusted_reviewed_identity_variant_replace";
  }
  ...
  if (hierarchyFields.has(field) && (
    genericIdentityScaffold(field, currentValue)
    || (
      strictTextSuperset(currentValue, candidateValue)
      && addsMeaningfulHierarchySpecificity(currentValue, candidateValue)
    )
    || (
      applicationReason === "trusted_product_hierarchy_fill"
      && trustedHierarchyCoreSuperset(currentValue, candidateValue)
    )
  )) {
    return "trusted_product_hierarchy_specificity_upgrade";
  }
  return "";
}
...
// Cross-family product correction. The selection pass only emits a
// ready_product_correction when a high-trust catalog candidate is anchored to
// the observation by subject + serial + year, the observed product has no
// catalog support, and the anchored candidates map to a single product family
// (see buildSelectedCandidateProductCorrection). Here we overwrite the
// mis-identified product hierarchy — this is a deliberate replace, not a
// fill-missing, because the whole point is to correct a wrong value.
function applyProductCorrection(base = {}, result = {}) {
  const correction = result.selected_candidate_product_correction;
  if (!correction || correction.status !== "ready_product_correction") {
    return { resolved: base, appliedFields: [], reasonPerField: {}, candidateId: "" };
  }
  const resolved = { ...base };
  const appliedFields = [];
  const reasonPerField = {};
  for (const [field, value] of Object.entries(correction.product_fields || {})) {
    if (!hierarchyFields.has(field) || !valuePresent(value)) continue;
    const current = normalizeStringOrNull(resolved[field])?.toLowerCase();
    if (current && current === normalizeStringOrNull(value)?.toLowerCase()) continue;
    resolved[field] = value;
    appliedFields.push(field);
    reasonPerField[field] = "trusted_official_checklist_product_correction_replace";
  }
  return {
    resolved,
    appliedFields,
    reasonPerField,
    candidateId: normalizedCandidateId(correction.candidate_id)
  };
}
...
export function applyCandidateDecisionStage({
  ...
  const productCorrection = applyProductCorrection(before, result);
  const correctionBase = productCorrection.resolved;
  const selected = selectedCandidateOverlay(result);
  const selectedApplied = applyMissingFields(correctionBase, selected.overlay, {
  ...
  const appliedFields = [...new Set([
    ...productCorrection.appliedFields,
    ...selectedApplied.appliedFields,
    ...lowMarginApplied.appliedFields
  ])];
  const reasonPerField = {
    ...productCorrection.reasonPerField,
    ...selectedApplied.reasonPerField,
    ...lowMarginApplied.reasonPerField
  };
  const selectedId = selectedApplied.appliedFields.length
    ? selected.candidateId
    : (lowMargin.candidateId || productCorrection.candidateId);
  ...
  const decisionRecord = {
    ...
    field_application: {
      ...
      product_correction_fields: productCorrection.appliedFields,
