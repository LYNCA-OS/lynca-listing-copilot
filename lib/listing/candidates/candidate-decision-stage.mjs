...
import {
  buildCandidateFieldContentGuard,
  candidateSelectionHeuristicVersion
} from "./candidate-selection-pass.mjs";
import {
  productsSemanticallyEquivalent,
  sharedDistinctiveProductTokens
} from "../csm/product-semantics.mjs";
...
function selectedCandidateOverlay(result = {}) {
  ...
  const contentGuard = buildCandidateFieldContentGuard(result.candidate_field_inventory);
  const contentPollutionBlockedFields = new Set();
  const overlay = {};
  for (const row of rows) {
    ...
    if (embeddedReferenceFields.has(field) && applicationReason !== "trusted_reviewed_identity_variant_fill") continue;
    if (contentGuard.reasonsFor(row).length) {
      contentPollutionBlockedFields.add(field);
      continue;
    }
...
function safeSelectedCandidateUpgradeReason(field, currentValue, candidateValue, applicationReason = "") {
  ...
  if (field === "product" && productsSemanticallyEquivalent(currentValue, candidateValue)) return "";

  const exactCodeIdentity = applicationReason === "trusted_exact_code_identity_fill";
  if (field === "product"
    && applicationReason === "trusted_reviewed_product_canonicalization"
    && !strictTextSuperset(candidateValue, currentValue)
    && sharedDistinctiveProductTokens(currentValue, candidateValue).length > 0) {
    return "trusted_reviewed_product_canonicalization_replace";
  }
  ...
  if (exactCodeIdentity && exactCodeAuthoritativeHierarchyFields.has(field)) {
    if (field === "product" && strictTextSuperset(candidateValue, currentValue)) return "";
