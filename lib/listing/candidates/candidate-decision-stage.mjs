...
function selectedCandidateOverlay(result = {}) {
  ...
  for (const row of rows) {
    ...
    if (embeddedReferenceFields.has(field)
      && ![
        "trusted_exact_code_identity_fill",
        "trusted_reviewed_identity_variant_fill",
        "trusted_reviewed_current_source_semantic_fill"
      ].includes(applicationReason)) continue;
...
function consensusProductHierarchyOverlay(result = {}) {
  const application = result.consensus_product_hierarchy_application;
  if (application?.status !== "ready_safe_refinement"
    || application.renderer_application_allowed !== true
    || !valuePresent(application.value)) {
    return { overlay: {}, reasons: {}, supportingCandidateIds: [] };
  }
  return {
    overlay: { product: application.value },
    reasons: { product: application.reason || "trusted_catalog_consensus_product_hierarchy_refinement" },
    supportingCandidateIds: application.supporting_candidate_ids || []
  };
}
...
function renderTitle(resolved = {}, result = {}, maxLength = 80) {
  ...
  const presentation = renderListingPresentation({
    ...
    maxLength,
    // `null` means OCR did not decide. Preserve that tri-state so confirmed
    // current-image evidence can still render its numerator; only an explicit
    // `false` is an OCR conflict veto.
...
function safeSelectedCandidateUpgradeReason(field, currentValue, candidateValue, applicationReason = "") {
  ...
  if (applicationReason === "trusted_catalog_consensus_product_hierarchy_refinement"
    && field === "product"
    && strictTextSuperset(currentValue, candidateValue)) {
...
export function applyCandidateDecisionStage({
  ...
  const consensus = consensusProductHierarchyOverlay(result);
  const consensusApplied = applyMissingFields(before, consensus.overlay, {
    source: "trusted_catalog_consensus",
    reasons: consensus.reasons,
    allowSafeUpgrade: true
  });
  const selected = selectedCandidateOverlay(result);
  const selectedApplied = applyMissingFields(consensusApplied.resolved, selected.overlay, {
  ...
  const appliedFields = [...new Set([
    ...consensusApplied.appliedFields,
    ...selectedApplied.appliedFields,
    ...lowMarginApplied.appliedFields
  ])];
  const reasonPerField = {
    ...consensusApplied.reasonPerField,
  ...
  const resultPatch = {
    ...
    consensus_product_hierarchy_application: result.consensus_product_hierarchy_application,
