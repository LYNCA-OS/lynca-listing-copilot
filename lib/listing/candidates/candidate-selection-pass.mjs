...
import { rebindCandidateToObservedFields } from "../retrieval/vector-candidate-packet.mjs";

export const candidateSelectionHeuristicV1 = Object.freeze({
  version: "candidate-heuristic-v2-prompt-safe-identity-group-20260714",
...
function applicationTraceRows({ candidates = [], catalogPromptIds = new Set(), vectorPromptIds = new Set() } = {}) {
  return candidates.map((candidate) => {
    ...
    const promptEligible = candidateIsPromptEligible(candidate, promptIds);
    ...
    return {
      ...
      shadow_only_reason: promptEligible ? "" : "not_in_prompt_safe_candidate_ids"
    };
  });
}
