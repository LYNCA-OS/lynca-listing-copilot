...
export const candidateSelectionHeuristicV1 = Object.freeze({
  version: "candidate-heuristic-v7-content-bound-evidence-snapshot-20260730",
...
export const candidateObservationEvidenceSnapshotSchemaVersion =
  "candidate-observation-evidence-snapshot-v3";
export const candidateObservationReplayManifestSchemaVersion =
  "candidate-observation-replay-manifest-v2-process-private-capability";
const candidateObservationReplayAuthorityScope =
  "PROCESS_PRIVATE_VERIFIED_REPLAY_CAPABILITY";

const liveCandidateSnapshotAuthorities = new WeakMap();
const liveCandidateReplayManifestAuthorities = new WeakMap();
...
function sourceFeedbackIdSha256(candidate = {}) {
  ...
  return sourceFeedbackId
    ? crypto.createHash("sha256").update(sourceFeedbackId).digest("hex")
    : null;
}
...
function snapshotContentPayload(value = {}) {
  const {
    snapshot_content_sha256: ignoredDigest,
    replay_manifest: ignoredReplayManifest,
    ...payload
  } = isRecord(value) ? value : {};
  return payload;
}

function snapshotContentSha256(value = {}) {
  return `sha256:${crypto.createHash("sha256")
    .update(JSON.stringify(canonicalSnapshotValue(snapshotContentPayload(value))))
    .digest("hex")}`;
}

function validPersistedObservationEvidenceSnapshot(value = {}) {
  const expectedDigest = snapshotContentSha256(value);
  return isRecord(value)
    && value.schema_version === candidateObservationEvidenceSnapshotSchemaVersion
    && value.status === "COMPLETE"
    && cleanText(value.tenant_id)
    && cleanText(value.asset_id)
    && cleanText(value.image_generation_id)
    && /^sha256:[0-9a-f]{64}$/.test(cleanText(value.current_image_set_fingerprint))
    && isRecord(value.current_image_context)
    && cleanText(value.evidence_schema_version)
    && isRecord(value.raw_observed_fields)
    && isRecord(value.raw_provider_fields)
    && isRecord(value.normalized_evidence)
    && Array.isArray(value.raw_provider_field_evidence)
    && /^sha256:[0-9a-f]{64}$/.test(cleanText(value.snapshot_content_sha256))
    && value.snapshot_content_sha256 === expectedDigest;
}

function deepFreezeSnapshot(value, seen = new WeakSet()) {
  if (!value || typeof value !== "object" || seen.has(value)) return value;
  seen.add(value);
  for (const child of Object.values(value)) deepFreezeSnapshot(child, seen);
...
export function buildCandidatePreApplicationEvidenceSnapshot(result = {}, observationContext = {}) {
  const currentImageContext = buildVerifiedCurrentImageManifest(
    Object.keys(isRecord(observationContext) ? observationContext : {}).length
      ? observationContext
      : isRecord(result.current_image_context) ? result.current_image_context : {}
  );
  if (currentImageContext.status !== "COMPLETE") {
    return Object.freeze({
      schema_version: candidateObservationEvidenceSnapshotSchemaVersion,
      status: "UNKNOWN",
      source: "LIVE_PRE_APPLICATION_CURRENT_IMAGE_EVIDENCE",
      reason_code: currentImageContext.reason_code || "CURRENT_IMAGE_CONTEXT_INVALID"
    });
  }
  const evidenceSchemaVersion = cleanText(result.evidence_schema_version);
  if (!evidenceSchemaVersion) {
    return Object.freeze({
      schema_version: candidateObservationEvidenceSnapshotSchemaVersion,
      status: "UNKNOWN",
      source: "LIVE_PRE_APPLICATION_CURRENT_IMAGE_EVIDENCE",
      reason_code: "PRE_APPLICATION_EVIDENCE_SCHEMA_VERSION_MISSING"
    });
  }
  const payload = {
    schema_version: candidateObservationEvidenceSnapshotSchemaVersion,
    ...
    reason_code: null,
    source: "LIVE_PRE_APPLICATION_CURRENT_IMAGE_EVIDENCE",
    tenant_id: currentImageContext.tenant_id,
    asset_id: currentImageContext.asset_id,
    image_generation_id: currentImageContext.image_generation_id,
    current_image_set_fingerprint: currentImageContext.image_set_fingerprint,
    current_image_context: currentImageContext,
    evidence_schema_version: evidenceSchemaVersion,
    raw_observed_fields: {
      ...(isRecord(result.raw_observed_fields) ? result.raw_observed_fields : {})
    },
    raw_provider_fields: {
      ...(isRecord(result.raw_provider_fields) ? result.raw_provider_fields : {})
    },
    normalized_evidence: {
      ...(isRecord(result.normalized_evidence)
        ? result.normalized_evidence
        : isRecord(result.evidence) ? result.evidence : {})
    },
    raw_provider_field_evidence: [
      ...(Array.isArray(result.raw_provider_field_evidence)
        ? result.raw_provider_field_evidence
        : [])
    ]
  };
  const snapshotContentDigest = snapshotContentSha256(payload);
  const snapshot = immutableSnapshot({
    ...payload,
    snapshot_content_sha256: snapshotContentDigest
  });
  liveCandidateSnapshotAuthorities.set(snapshot, Object.freeze({
    snapshot_content_sha256: snapshotContentDigest,
    tenant_id: currentImageContext.tenant_id,
    asset_id: currentImageContext.asset_id,
    image_generation_id: currentImageContext.image_generation_id,
...
export function candidatePreApplicationEvidenceSnapshotMatches(
  snapshot = {},
  authoritativeCurrentImageContext = {},
  { trustedReplayManifest = null } = {}
) {
  if (!validPersistedObservationEvidenceSnapshot(snapshot)) {
    return { valid: false, reason_code: "PRE_APPLICATION_EVIDENCE_SNAPSHOT_CONTENT_INVALID" };
  }
  const contextMatch = currentImageManifestMatches(
    snapshot.current_image_context,
    authoritativeCurrentImageContext
  );
  if (!contextMatch.valid
    || snapshot.tenant_id !== contextMatch.manifest?.tenant_id
    || snapshot.asset_id !== contextMatch.manifest?.asset_id
    || snapshot.image_generation_id !== contextMatch.manifest?.image_generation_id
    || snapshot.current_image_set_fingerprint !== contextMatch.manifest?.image_set_fingerprint) {
    return {
      valid: false,
      reason_code: contextMatch.reason_code || "PRE_APPLICATION_EVIDENCE_CONTEXT_BINDING_INVALID"
    };
  }
  const liveAuthority = liveCandidateSnapshotAuthorities.get(snapshot);
  // A replay manifest is authority only when a trusted caller supplies the
  // exact object produced from a live authoritative snapshot. Persisted JSON
  // can carry the manifest for audit, but a co-persisted clone must never
  // authenticate itself by changing both digests together.
  const registeredReplayAuthority = isRecord(trustedReplayManifest)
    ? liveCandidateReplayManifestAuthorities.get(trustedReplayManifest)
    : null;
  const replayAuthority = registeredReplayAuthority
    && trustedReplayManifest.schema_version === candidateObservationReplayManifestSchemaVersion
    && trustedReplayManifest.authority_scope === candidateObservationReplayAuthorityScope
    && registeredReplayAuthority.snapshot_content_sha256 === trustedReplayManifest.snapshot_content_sha256
    ? trustedReplayManifest
    : null;
  const authority = liveAuthority || replayAuthority;
  if (!authority
    || authority.snapshot_content_sha256 !== snapshot.snapshot_content_sha256
    || authority.tenant_id !== snapshot.tenant_id
    || authority.asset_id !== snapshot.asset_id
    || authority.image_generation_id !== snapshot.image_generation_id
    || authority.current_image_set_fingerprint !== snapshot.current_image_set_fingerprint
    || authority.evidence_schema_version !== snapshot.evidence_schema_version) {
    return { valid: false, reason_code: "PRE_APPLICATION_EVIDENCE_AUTHORITY_UNAVAILABLE" };
  }
  return {
    valid: true,
    reason_code: null,
    snapshot_content_sha256: snapshot.snapshot_content_sha256,
    manifest: contextMatch.manifest
  };
}

export function buildCandidatePreApplicationReplayManifest(
  snapshot = {},
  authoritativeCurrentImageContext = {}
) {
  const validation = candidatePreApplicationEvidenceSnapshotMatches(
    snapshot,
    authoritativeCurrentImageContext
  );
  if (!validation.valid) return null;
  // This manifest is intentionally separate from the snapshot. A replay
  // caller may trust it only after authenticating the enclosing trace/artifact;
  // embedding the same fields back into the candidate snapshot would let the
  // snapshot certify itself.
  const manifest = Object.freeze({
    schema_version: candidateObservationReplayManifestSchemaVersion,
    authority_scope: candidateObservationReplayAuthorityScope,
    snapshot_content_sha256: snapshot.snapshot_content_sha256,
    tenant_id: snapshot.tenant_id,
    asset_id: snapshot.asset_id,
    image_generation_id: snapshot.image_generation_id,
    current_image_set_fingerprint: snapshot.current_image_set_fingerprint,
    evidence_schema_version: snapshot.evidence_schema_version
  });
  liveCandidateReplayManifestAuthorities.set(manifest, Object.freeze({
    snapshot_content_sha256: snapshot.snapshot_content_sha256
  }));
  return manifest;
}

function observationEvidenceSnapshot(
  result = {},
  authoritativeCurrentImageContext = {},
  trustedReplayManifest = null
) {
  const persisted = result.candidate_pre_application_evidence_snapshot;
  if (validPersistedObservationEvidenceSnapshot(persisted)) {
    const currentContext = Object.keys(isRecord(authoritativeCurrentImageContext)
      ? authoritativeCurrentImageContext
      : {}).length
      ? authoritativeCurrentImageContext
      : isRecord(result.current_image_context) ? result.current_image_context : {};
    const contextMatch = candidatePreApplicationEvidenceSnapshotMatches(persisted, currentContext, {
      trustedReplayManifest
    });
    if (contextMatch.valid) {
      return Object.isFrozen(persisted) ? persisted : immutableSnapshot(persisted);
    }
    return Object.freeze({
      schema_version: candidateObservationEvidenceSnapshotSchemaVersion,
      status: "UNKNOWN",
  ...
  if (isRecord(persisted)
    && persisted.schema_version === candidateObservationEvidenceSnapshotSchemaVersion
  ...
  return Object.freeze({
    ...
    reason_code: persisted
      ? persisted.schema_version === candidateObservationEvidenceSnapshotSchemaVersion
        ...
        : "PRE_APPLICATION_EVIDENCE_SNAPSHOT_INVALID"
      : "PRE_APPLICATION_EVIDENCE_SNAPSHOT_MISSING"
  });
}

function observedFieldsFromSnapshot(snapshot = {}) {
  const output = {};
  ...
  if (snapshot.status !== "COMPLETE") return output;

  // Candidate selection consumes only this pre-application snapshot. Terminal
  // field containers are absent so a catalog row cannot prove itself correct.
  assignMissingScalars(snapshot.raw_observed_fields);
  assignMissingScalars(snapshot.raw_provider_fields);
...
function currentImageSupportedFields(candidate = {}, result = {}) {
  const observed = observedFieldsFromSnapshot(observationEvidenceSnapshot(result));
  const fields = candidateFields(candidate);
  const supported = [];
  for (const [field, value] of Object.entries(fields)) {
...
const productCorrectionFields = new Set([
  "manufacturer",
  "brand",
  "product"
]);
...
function normalizedProductKey(fields = {}) {
  const manufacturer = cleanComparable(fields.manufacturer || fields.brand);
  const product = cleanComparable(fields.product || fields.set);
  if (!manufacturer) return product;
  if (!product || product === manufacturer || product.startsWith(`${manufacturer} `)) return product || manufacturer;
...
function candidateProductHierarchyOverlay(candidate = {}) {
  const fields = candidateFields(candidate);
  const overlay = {};
  for (const field of productCorrectionFields) {
    if (cleanText(fields[field]) !== "") overlay[field] = fields[field];
  }
  return overlay;
}

// Cross-family product correction. A vision model can mislabel the product
// ("Chrome Black" for Star Wars Smugglers Outpost) while reading stronger
// identity anchors correctly. Anchor agreement alone CANNOT authorise a
// cross-family product override: two genuinely different products can share the
// same subject + year + serial. This pass adds two bounded safety signals only
// available with the full candidate set in view:
...
// subject + year + serial-denominator anchor lock, before a non-empty product
// may be overwritten. Exact code remains retrieval/support evidence, but does
// not independently authorise replacement of conflicting current-image Product.
function buildSelectedCandidateProductCorrection({
  decision = {},
  candidates = [],
  traces = [],
  observedFields = {}
  ...
  const selectedCandidateId = cleanText(decision.selected_candidate_id);
  if (!observedKey || !selectedCandidateId) return notApplicable;

  // Product correction is a field-application path, so it must consume the
  // exact same fail-closed eligibility decision as Selection/Application.
  // Never resurrect a catalog row that was retained only for shadow trace.
  const eligibleCandidates = candidates.filter((candidate, index) => (
    traces[index]?.decision_eligible === true
    && candidateCanSupportEvidence(candidate)
  ));
  const anchored = eligibleCandidates.filter((candidate) => {
  ...
  const observedProductSupported = eligibleCandidates.some((candidate) => {
  ...
  const chosen = anchored.find((candidate) => candidateId(candidate) === selectedCandidateId);
  if (!chosen) return notApplicable;
...
function applicationTraceRows({ candidates = [], catalogPromptIds = new Set(), vectorPromptIds = new Set() } = {}) {
  return candidates.map((candidate) => {
    ...
    return {
      ...
      // Evaluation may prove that a reviewed title did not retrieve itself,
      // but the raw internal feedback id must never enter the trace packet.
      source_feedback_id_sha256: sourceFeedbackIdSha256(candidate),
...
export function buildCandidateSelectionPass({
  ...
  vectorContext = {},
  authoritativeCurrentImageContext = {},
  trustedCandidateReplayManifest = null
} = {}) {
  ...
  const preApplicationEvidenceSnapshot = observationEvidenceSnapshot(
    result,
    authoritativeCurrentImageContext,
    trustedCandidateReplayManifest
  );
  const observationSnapshotReady = preApplicationEvidenceSnapshot.status === "COMPLETE";
  const observedFields = observedFieldsFromSnapshot(preApplicationEvidenceSnapshot);
  ...
  const rawTraces = applicationTraceRows({ candidates, catalogPromptIds, vectorPromptIds });
  const traces = observationSnapshotReady
    ? rawTraces
    : rawTraces.map((trace) => ({
        ...trace,
        participation_level: participationLevels.SHADOW,
        live_anchor_eligible: false,
        live_evidence_eligible: false,
        prompt_eligible: false,
        decision_eligible: false,
        shadow_only_reason: "pre_application_evidence_authority_unavailable"
      }));
  ...
  return {
    candidate_pre_application_evidence_snapshot: preApplicationEvidenceSnapshot,
    candidate_observation_snapshot_status: preApplicationEvidenceSnapshot.status || "UNKNOWN",
    candidate_observation_snapshot_reason: preApplicationEvidenceSnapshot.reason_code || null,
