...
function strictProductHierarchyRefinement(currentValue, candidateValue) {
  const current = cleanComparable(currentValue);
  const candidate = cleanComparable(candidateValue);
  return Boolean(current && candidate && candidate !== current && candidate.startsWith(`${current} `));
}
...
function buildConsensusProductHierarchyApplication({
  ...
  candidates.forEach((candidate, index) => {
    ...
    const independentlyAnchored = trace.can_apply_fields?.includes("product")
      && agreed.has("product_hierarchy")
      && agreed.has("subjects")
      && agreed.has("year")
      && !["product", "product_hierarchy", "year", "players", "subjects", "manufacturer"]
