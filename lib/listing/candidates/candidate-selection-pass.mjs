...
// subject + year + (exact code or serial denominator) anchor lock, before a
// non-empty product may be overwritten.
function buildSelectedCandidateProductCorrection({ candidates = [], observedFields = {} } = {}) {
  ...
  const anchored = candidates.filter((candidate) => {
    ...
    return agreed.has("subjects")
      && agreed.has("year")
      && (agreed.has("serial_denominator") || agreement.exact_code_match === true);
  });
