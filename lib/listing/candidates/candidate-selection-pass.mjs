...
export const trustedCatalogCandidateSelectionHeuristicVersion =
  `${candidateSelectionHeuristicVersion}+card-domain-trusted-catalog-v1`;
export const oracleCandidateSelectionHeuristicVersion =
  `${candidateSelectionHeuristicVersion}+card-domain-oracle-v1`;
...
function selectedCandidateDecision(candidates = [], traces = []) {
  ...
  const marginWaiver = top
    ...
    && top.candidate?.reviewed_current_source_identity_match === true
    && candidateUsesReviewedTitleGroundTruth(top.candidate)
...
function oracleCardDomainDecision(baseDecision = {}, reranker = {}, candidates = [], traces = []) {
  const selectedId = cleanText(reranker.top_decision_eligible_candidate_id);
  if (!selectedId) return baseDecision;
  const index = candidates.findIndex((candidate) => candidateId(candidate) === selectedId);
  if (index < 0 || traces[index]?.decision_eligible !== true) return baseDecision;
  const selectedCandidate = candidates[index];
  const selectedTrace = traces[index] || {};
  const selectedRank = (reranker.ranked_candidates || []).find((row) => row.candidate_id === selectedId) || {};
  const eligibleRanks = (reranker.ranked_candidates || []).filter((row) => row.eligible_for_domain_selection === true);
  const nextEligible = eligibleRanks.find((row) => row.candidate_id !== selectedId) || null;
  const margin = Math.max(0, Number((Number(selectedRank.score || 0) - Number(nextEligible?.score || 0)).toFixed(6)));
  const independentlyIdentified = selectedRank.authoritative_observation_correction === true
    || Number(selectedRank.exact_anchor_score || 0) >= 0.5
    || margin >= candidateSelectionHeuristicV1.low_margin_threshold;
  // Oracle mode may expose every sensor and retrieval lane, but it must not
  // turn an unresolved tie into a correct answer by fiat. Otherwise the audit
  // measures an optimistic label lookup instead of the chain's real ceiling.
  if (!independentlyIdentified) return baseDecision;
  const identityId = cleanText(selectedCandidate.candidate_identity_id || selectedCandidate.identity_id);
  const groupIds = candidates.filter((candidate) => (
    identityId && cleanText(candidate.candidate_identity_id || candidate.identity_id) === identityId
  )).map(candidateId).filter(Boolean);
  return {
    ...baseDecision,
    heuristic_version: oracleCandidateSelectionHeuristicVersion,
    selected_candidate_id: selectedId,
    selected_candidate_identity_group_key: identityId || selectedId,
    selected_candidate_group_ids: groupIds.length ? groupIds : [selectedId],
    selected_candidate_source: candidateSourceType(selectedCandidate),
    selected_candidate_source_trust: candidateSourceTrust(selectedCandidate),
    match_level: selectedTrace.match_level || matchLevelForCandidate(selectedCandidate),
    selection_confidence: Number(selectedRank.score || 0),
    selection_margin: margin,
    low_margin_waived_reason: margin < candidateSelectionHeuristicV1.low_margin_threshold
      ? "card_domain_oracle_exact_anchor"
      : "",
    top_candidate_id: selectedId,
    top_candidate_identity_group_key: identityId || selectedId,
    top_candidate_group_ids: groupIds.length ? groupIds : [selectedId],
    second_candidate_id: nextEligible?.candidate_id || "",
    second_candidate_identity_group_key: cleanText(nextEligible?.candidate_identity_id),
    top_candidate_score: Number(selectedRank.score || 0),
    second_candidate_score: Number(nextEligible?.score || 0),
    top_candidate_decision_strength: Number(selectedRank.score || 0),
    second_candidate_decision_strength: Number(nextEligible?.score || 0),
    low_margin_candidate_id: "",
    none_of_the_above_score: 0,
    selected_reason_codes: [
      "card_domain_reranker_oracle_v1",
      ...selectedReasonCodes(
        selectedCandidate,
        margin,
        margin < candidateSelectionHeuristicV1.low_margin_threshold ? "card_domain_oracle_exact_anchor" : ""
      )
    ],
    selection_owner: "card_domain_reranker_oracle_v1"
  };
}

function trustedCatalogCardDomainDecision(baseDecision = {}, reranker = {}, candidates = [], traces = []) {
  const selectedId = cleanText(reranker.top_decision_eligible_candidate_id);
  if (!selectedId) return baseDecision;
  const index = candidates.findIndex((candidate) => candidateId(candidate) === selectedId);
  if (index < 0 || traces[index]?.decision_eligible !== true) return baseDecision;
  ...
  const nextEligible = eligibleRanks.find((row) => row.candidate_id !== selectedId) || null;
  const margin = Math.max(0, Number((Number(selectedRank.score || 0) - Number(nextEligible?.score || 0)).toFixed(6)));
  const trustedCatalog = selectedRank.candidate_lane === "catalog"
    && !candidateIsMarketplace(selectedCandidate)
    && !candidateIsVectorOnly(selectedCandidate)
    && sourceTrustScore(candidateSourceTrust(selectedCandidate)) >= 5;
  const enoughIndependentEvidence = Number(selectedRank.query_coverage || 0) >= 0.3
    || Number(selectedRank.exact_anchor_score || 0) >= 0.5
    || selectedRank.authoritative_observation_correction === true;
  const safelySeparated = Number(selectedRank.score || 0) >= 0.45
    && margin >= candidateSelectionHeuristicV1.low_margin_threshold;
  if (!trustedCatalog
    || selectedRank.eligible_for_domain_selection !== true
    || (selectedRank.conflicting_fields || []).length > 0
    || Number(selectedRank.agreement_score || 0) < 0.8
    || !enoughIndependentEvidence
    || !safelySeparated) return baseDecision;
  ...
  return {
    ...
    selection_confidence: Number(selectedRank.score || 0),
    selection_margin: margin,
    low_margin_waived_reason: "card_domain_trusted_catalog_margin",
    top_candidate_id: selectedId,
    top_candidate_identity_group_key: identityId || selectedId,
    top_candidate_group_ids: groupIds.length ? groupIds : [selectedId],
    second_candidate_id: nextEligible?.candidate_id || "",
    ...
    selected_reason_codes: [
      ...
      ...selectedReasonCodes(selectedCandidate, margin, "card_domain_trusted_catalog_margin")
    ],
...
const safeProductCategoryQualifiers = new Set([
  "baseball", "basketball", "football", "soccer", "tennis", "hockey",
  "golf", "racing", "wrestling", "mma", "ufc"
]);
const productOwnershipTokens = new Set([
  "topps", "bowman", "panini", "donruss", "fanatics", "upper", "deck"
]);
const misplacedCardTypeTokens = new Set([
  "auto", "autograph", "autographs", "autographed", "card", "cards",
  "signature", "signatures"
]);

export function safeConsensusProductCategoryRefinement(currentValue, candidateValue) {
  const currentTokens = cleanComparable(currentValue).split(" ").filter(Boolean);
  const candidateTokens = cleanComparable(candidateValue).split(" ").filter(Boolean);
  if (!currentTokens.length || !candidateTokens.length) return false;
  const currentCore = new Set(currentTokens.filter((token) => (
    !productOwnershipTokens.has(token) && !misplacedCardTypeTokens.has(token)
  )));
  const candidateSet = new Set(candidateTokens);
  if (!currentCore.size || ![...currentCore].every((token) => candidateSet.has(token))) return false;
  const categoryAdditions = candidateTokens.filter((token) => (
    !currentCore.has(token) && !productOwnershipTokens.has(token)
  ));
  return categoryAdditions.length > 0
    && categoryAdditions.every((token) => safeProductCategoryQualifiers.has(token));
}

function buildConsensusProductHierarchyApplication({
  candidates = [],
  traces = [],
  observedFields = {}
} = {}) {
  const currentProduct = cleanText(observedFields.product);
  if (!currentProduct) {
    return {
      status: "not_needed",
      field: "product",
  ...
  candidates.forEach((candidate, index) => {
    ...
    const agreed = new Set(Array.isArray(agreement.agreed) ? agreement.agreed : []);
    const contradicted = new Set(Array.isArray(agreement.contradicted) ? agreement.contradicted : []);
    const directConflicts = new Set(candidatePolicyConflicts(candidate));
    const product = cleanText(candidateFields(candidate).product);
    const catalogTrusted = candidate.__candidate_lane === "catalog"
      && !candidateIsMarketplace(candidate)
      && !candidateIsVectorOnly(candidate)
      && sourceTrustScore(candidateSourceTrust(candidate)) >= 5;
    // Variant conflicts (for example Gold vs Green) must block that variant,
    // but they do not invalidate a release-level product value independently
    // shared by multiple trusted catalog identities.
    const independentlyAnchored = trace.can_apply_fields?.includes("product")
      && agreed.has("subjects")
      && agreed.has("manufacturer")
      && !["product", "players", "subjects", "manufacturer"]
        .some((field) => directConflicts.has(field) || contradicted.has(field));
    if (!catalogTrusted || !independentlyAnchored || !safeConsensusProductCategoryRefinement(currentProduct, product)) return;
    const key = cleanComparable(product);
    const group = groups.get(key) || {
      value: product,
      candidate_ids: new Set(),
      identity_ids: new Set()
    };
    const id = candidateId(candidate);
    const identityId = cleanText(candidate.candidate_identity_id || candidate.identity_id || id);
    if (id) group.candidate_ids.add(id);
    if (identityId) group.identity_ids.add(identityId);
    groups.set(key, group);
  });
  const supported = [...groups.values()].filter((group) => group.identity_ids.size >= 2);
  if (supported.length !== 1) {
    return {
      status: supported.length > 1 ? "blocked_conflicting_consensus" : "blocked_insufficient_consensus",
      field: "product",
      value: "",
      supporting_candidate_ids: [],
      supporting_identity_ids: [],
  ...
  const [winner] = supported;
  return {
    status: "ready_safe_refinement",
    field: "product",
    value: winner.value,
    supporting_candidate_ids: [...winner.candidate_ids],
    supporting_identity_ids: [...winner.identity_ids],
    reason: "trusted_catalog_consensus_product_hierarchy_refinement",
...
function applicationTraceRows({ candidates = [], catalogPromptIds = new Set(), vectorPromptIds = new Set() } = {}) {
  ...
  return candidates.map((candidate) => {
    ...
    const candidateLane = candidate.__candidate_lane || "";
    const retrievalRank = (laneRanks.get(candidateLane) || 0) + 1;
    laneRanks.set(candidateLane, retrievalRank);
    return {
...
export function buildCandidateSelectionPass({
  ...
  cardDomainSelectionMode = "shadow_only"
} = {}) {
  ...
  const heuristicDecision = selectedCandidateDecision(decisionCandidates, decisionTraces);
  const cardDomainRerankerShadow = rankCardDomainCandidates(
    ...
    observedFields,
    {
      baselineSelectedCandidateId: heuristicDecision.selected_candidate_id,
      limit: Math.max(1, Math.min(20, Number(diagnosticCandidateLimit) || 5))
    }
  );
  const decision = result.evaluation_profile === "v4_accuracy_ceiling_oracle_v1"
    ? oracleCardDomainDecision(
      heuristicDecision,
      cardDomainRerankerShadow,
      decisionCandidates,
      decisionTraces
    )
    : cardDomainSelectionMode === "trusted_catalog_margin_v1"
      ? trustedCatalogCardDomainDecision(
        heuristicDecision,
        cardDomainRerankerShadow,
        decisionCandidates,
        decisionTraces
  ...
  const consensusProductHierarchyApplication = buildConsensusProductHierarchyApplication({
    candidates,
    traces,
    observedFields
  });
  ...
  return {
    ...
    consensus_product_hierarchy_application: consensusProductHierarchyApplication,
