...
const referenceFullSerialFields = new Set([
  "serial_number",
  "serial_numerator",
  "print_run_number",
  "print_run_numerator"
]);
const referenceGradeCompanyFields = new Set(["grade_company"]);
const referenceGradeValueFields = new Set(["grade", "card_grade", "auto_grade"]);
const referenceCertFields = new Set(["cert_number"]);

function normalizedContentText(value) {
  if (Array.isArray(value)) return normalizedContentText(value.join(" "));
...
function fullSerialTokens(value) {
  if (typeof value !== "string") return [];
  const tokens = [];
  const pattern = /(?:^|[^0-9])(\d{1,6})\s*\/\s*(\d{1,6})(?![0-9])/g;
  for (const match of value.normalize("NFKC").matchAll(pattern)) {
    tokens.push(`${match[1]}/${match[2]}`);
  }
  return [...new Set(tokens)];
}

function certToken(value) {
  const parts = normalizedContentText(value).split(" ").filter(Boolean);
  while (["cert", "certificate", "certification", "number", "no"].includes(parts[0])) parts.shift();
  const token = parts.join("");
  return token.length >= 4 ? token : "";
}

function containsCompactTokenSequence(value, expectedToken) {
  if (!expectedToken) return false;
  const parts = normalizedContentText(value).split(" ").filter(Boolean);
  for (let start = 0; start < parts.length; start += 1) {
    let token = "";
    for (let end = start; end < parts.length && token.length <= expectedToken.length; end += 1) {
      token += parts[end];
...
function containsNormalizedPhrase(value, phrase) {
  const content = normalizedContentText(value);
  return Boolean(content && phrase && ` ${content} `.includes(` ${phrase} `));
}

export function buildCandidateFieldContentGuard(candidateFieldInventory = []) {
  const pollutionByCandidate = new Map();
  const recordFor = (candidateIdValue) => {
    const candidateIdValueClean = cleanText(candidateIdValue);
    if (!candidateIdValueClean) return null;
    if (!pollutionByCandidate.has(candidateIdValueClean)) {
      pollutionByCandidate.set(candidateIdValueClean, {
        fullSerials: new Set(),
    ...
    return pollutionByCandidate.get(candidateIdValueClean);
  };

  for (const row of Array.isArray(candidateFieldInventory) ? candidateFieldInventory : []) {
    const record = recordFor(row?.candidate_id);
    if (!record) continue;
    const field = cleanText(row?.field_name || row?.field);
    if (referenceFullSerialFields.has(field)) {
      for (const token of fullSerialTokens(row?.value)) record.fullSerials.add(token);
    }
    if (referenceGradeCompanyFields.has(field)) {
      const company = normalizedContentText(row?.value);
      if (company) record.gradeCompanies.add(company);
    }
    if (referenceGradeValueFields.has(field)) {
      const grade = normalizedContentText(row?.value);
      if (grade) record.gradeValues.add(grade);
      if (field === "grade" && /[a-z]/.test(grade) && /\d/.test(grade)) record.gradeTuples.add(grade);
    }
    if (referenceCertFields.has(field)) {
      const token = certToken(row?.value);
      if (token) record.certTokens.add(token);
    }
  }

  for (const record of pollutionByCandidate.values()) {
    for (const company of record.gradeCompanies) {
      for (const grade of record.gradeValues) {
        record.gradeTuples.add(grade === company || grade.startsWith(`${company} `)
          ? grade
          : `${company} ${grade}`);
      }
    }
  ...
  return Object.freeze({
    reasonsFor(row = {}) {
      ...
      const record = pollutionByCandidate.get(cleanText(row?.candidate_id));
      if (!record) return [];
      const reasons = [];
      const valueSerials = new Set(fullSerialTokens(Array.isArray(value) ? value.join(" ") : value));
      if ([...record.fullSerials].some((token) => valueSerials.has(token))) {
        reasons.push("embedded_reference_full_serial");
      }
      if ([...record.gradeTuples].some((tuple) => containsNormalizedPhrase(value, tuple))) {
        reasons.push("embedded_reference_grade_tuple");
      }
      if ([...record.certTokens].some((token) => containsCompactTokenSequence(value, token))) {
        reasons.push("embedded_reference_cert_token");
      }
      return reasons;
    }
  });
...
function contentPollutionReasonsByField({
  candidateIdValue = "",
  fields = [],
  fieldInventory = [],
  contentGuard
  ...
  for (const row of fieldInventory) {
    const field = cleanText(row?.field_name || row?.field);
    if (cleanText(row?.candidate_id) !== cleanText(candidateIdValue) || !allowedFields.has(field)) continue;
    const rowReasons = contentGuard?.reasonsFor(row) || [];
    if (rowReasons.length) reasons[field] = [...new Set([...(reasons[field] || []), ...rowReasons])];
  }
  return reasons;
}
...
function participationLevelForCandidate(candidate = {}, {
  ...
  if (decisionEligible) return participationLevels.EVIDENCE_SUPPORT;
...
function buildLowMarginSafeFieldApplication({
  ...
  const contentPollutionReasons = contentPollutionReasonsByField({
    candidateIdValue: decision.low_margin_candidate_id,
    fields: candidateFieldNames,
    fieldInventory,
    contentGuard
  });
  const contentPollutionFields = new Set(Object.keys(contentPollutionReasons));
  const supported = currentImageSupportedFields(candidate, result)
    .filter((field) => !trace.forbidden_fields?.includes(field) && !contentPollutionFields.has(field));
  const verifierRequired = candidateFieldNames.filter((field) => (
    ...
    && !contentPollutionFields.has(field)
  ));
  return {
    ...
    blocked_fields: [
      ...new Set([
        ...
        ...contentPollutionFields
      ])
    ],
    content_pollution_reasons: contentPollutionReasons,
...
function selectedCandidateFieldApplicationReason(field, agreement = {}, candidate = {}, decision = {}) {
  const agreed = new Set(Array.isArray(agreement.agreed) ? agreement.agreed : []);
  if (agreement.exact_code_match === true && field === "print_finish") {
    return "trusted_exact_code_print_finish_fill";
  }
  if (agreement.exact_code_match === true && selectedCandidateExactIdentityFields.has(field)) {
    return "trusted_exact_code_identity_fill";
  }
  if (field === "product" && reviewedCompositeIdentityEligible(candidate, agreement, decision)) {
    return "trusted_reviewed_product_canonicalization";
  }
...
function buildSelectedCandidateSafeFieldApplication({
  ...
  const contentPollutionReasons = contentPollutionReasonsByField({
    candidateIdValue: selectedId,
    fields: trace.can_apply_fields || [],
    fieldInventory,
    contentGuard
  });
  for (const field of trace.can_apply_fields || []) {
    if (contentPollutionReasons[field]?.length) {
...
function candidateCanSupportFieldEvidence(candidate = {}) {
  const agreement = candidate.anchor_agreement && typeof candidate.anchor_agreement === "object"
    ? candidate.anchor_agreement
    : {};
  // A conflict in one descriptive field must not discard trustworthy support
  // for unrelated fields. Anchor contradictions still fail closed; only the
  // field-specific conflicts are deferred to the per-field application layer.
  const semVerdict = semCatalogTrustVerdict({
    sourceType: candidateSourceType(candidate),
    sourceTrust: candidateSourceTrust(candidate),
    anchorAgreement: agreement,
...
function safeCandidateFieldEvidenceRows(candidate = {}, trace = {}) {
  const conflictedFields = new Set(candidateDirectConflicts(candidate));
  return candidateFieldEvidenceRows(candidate, trace).filter((row) => (
    !conflictedFields.has(row.field_name)
    && ![fieldPermissions.SUGGEST_ONLY, fieldPermissions.FORBIDDEN].includes(row.permission)
  ));
}
...
function applicationTraceRows({ candidates = [], catalogPromptIds = new Set(), vectorPromptIds = new Set() } = {}) {
  return candidates.map((candidate) => {
    ...
    const liveIdentityEligible = candidateCanSupportEvidence(candidate);
    const promptEligible = providerPromptEligible && liveAnchorEligible && liveIdentityEligible;
    // Provider prompt admission and post-observation decision admission are
    // intentionally separate. A reviewed candidate discovered after the model
    // call may support Resolver fields without ever being shown to the model.
    // This keeps prompt anchoring fail-closed while allowing retrieval to make
    // a measurable, auditable contribution after current-image verification.
    const baseTrace = buildCandidateApplicationTrace(candidate, {
      matchLevel: matchLevelForCandidate(candidate)
    });
    const liveFieldEvidenceEligible = candidateCanSupportFieldEvidence(candidate);
    const evidenceRows = liveFieldEvidenceEligible
      ? safeCandidateFieldEvidenceRows(candidate, baseTrace)
      : [];
    const fieldEvidenceEligible = liveFieldEvidenceEligible && evidenceRows.length > 0;
    const identityDecisionEligible = liveIdentityEligible;
    const participationLevel = participationLevelForCandidate(candidate, {
      promptEligible,
      decisionEligible: identityDecisionEligible || fieldEvidenceEligible,
      evidenceRows,
      appliedFields: []
    });
    ...
    return {
      ...
      live_evidence_eligible: fieldEvidenceEligible,
      live_identity_eligible: liveIdentityEligible,
      prompt_eligible: promptEligible,
      decision_eligible: identityDecisionEligible,
      identity_decision_eligible: identityDecisionEligible,
      field_evidence_eligible: fieldEvidenceEligible,
      field_evidence_count: evidenceRows.length,
      field_evidence_fields: evidenceRows.map((row) => row.field_name),
      decision_path: identityDecisionEligible
        ? (promptEligible ? "provider_prompt_and_resolver" : "post_observation_resolver_only")
        : fieldEvidenceEligible
          ? "post_observation_field_evidence_only"
          : "shadow_only",
      provider_prompt_block_reason: providerPromptEligible
        ? (liveAnchorEligible
            ? (liveIdentityEligible ? "" : "post_observation_identity_conflict_blocked")
            : "post_observation_anchor_filter_blocked")
        : "not_in_provider_prompt_safe_candidate_ids",
      shadow_only_reason: identityDecisionEligible || fieldEvidenceEligible
        ? ""
        : !liveAnchorEligible
          ? "post_observation_anchor_filter_blocked"
...
function funnelForLane({
  ...
  return {
    ...
    evidence_support_field_count: evidenceRows.reduce((sum, trace) => sum + Number(trace.field_evidence_count || 0), 0),
...
export function buildCandidateSelectionPass({
  ...
  const decisionCandidateIndexes = traces
    .map((trace, index) => trace.identity_decision_eligible === true ? index : -1)
  ...
  const shadowOnlyCandidateIds = traces
    .filter((trace) => trace.field_evidence_eligible !== true)
    .map((trace) => trace.candidate_id)
    .filter(Boolean);
  const fieldEvidence = traces.flatMap((trace, index) => (
    trace.field_evidence_eligible === true
      ? safeCandidateFieldEvidenceRows(candidates[index], trace)
      : []
  ));
  ...
  const contentGuard = buildCandidateFieldContentGuard(fieldInventory);
  ...
  return {
    ...
    field_evidence_eligible_candidate_count: traces.filter((trace) => trace.field_evidence_eligible === true).length,
    field_evidence_eligible_candidate_ids: traces
      .filter((trace) => trace.field_evidence_eligible === true)
      .map((trace) => trace.candidate_id)
      .filter(Boolean),
