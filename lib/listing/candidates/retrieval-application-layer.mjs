...
import { buildCandidateFieldContentGuard } from "./candidate-selection-pass.mjs";
import { fieldPermissions } from "./candidate-application-policy.mjs";
import { productsProxyCompatible } from "../csm/product-semantics.mjs";
...
const safeResolverAliasApplicationReasons = Object.freeze({
  print_finish: new Set([
...
function normalizedObservationSnapshot(result = {}, resolved = {}) {
  const observed = normalizeFields(
    result.candidate_observation_snapshot && typeof result.candidate_observation_snapshot === "object"
      ? result.candidate_observation_snapshot
      : {}
  );
  return {
    ...observed,
    ...Object.fromEntries(Object.entries(resolved).filter(([, value]) => valuePresent(value)))
  };
}
...
function resolverFieldValue(row = {}) {
  ...
  if (field === "release_variant") return { field: "variation", value: row.value };
  if (field === "print_finish") return { field: "parallel", value: row.value };
...
function valuesEqual(field, left, right) {
  if (field === "product" && productsProxyCompatible(left, right)) return true;
...
function selectedSafeResolverAliasFields(application = {}) {
  if (application.status !== "ready_fill_missing" || application.renderer_application_allowed !== true) {
    return new Set();
  }
  const eligibleFields = Array.isArray(application.eligible_fields) ? application.eligible_fields : [];
  return new Set(eligibleFields.filter((field) => (
    safeResolverAliasApplicationReasons[field]?.has(application.field_reasons?.[field])
  )));
}
...
function decisionForRow(row = {}, {
  ...
  const observedValue = oldValueForRow(observedBefore, row);
  const sameAsCurrent = resolver.field
    ? valuesEqual(resolver.field, observedValue, resolver.value)
    : false;
  ...
  const fieldEvidenceEligible = trace.field_evidence_eligible === true
    || (trace.field_evidence_eligible === undefined && trace.decision_eligible === true);
  ...
  if (contentPollutionReasons.length) {
    return {
      decision: retrievalApplicationDecisions.BLOCK,
      reason: "candidate_field_content_pollution",
      content_pollution_reasons: contentPollutionReasons
    };
  }
  if (!fieldEvidenceEligible) {
    return {
      decision: retrievalApplicationDecisions.REJECT,
      reason: trace.shadow_only_reason || "candidate_not_field_evidence_eligible"
    };
  }
  ...
  const safeMissingResolverAlias = selectedResolverAliasFields.has(field)
    && !valuePresent(observedValue);
  if (isSelected
    && row.permission === fieldPermissions.CAN_APPLY
    && (selectedAppliedFields.has(field) || safeMissingResolverAlias)) {
    return {
      decision: retrievalApplicationDecisions.APPLY,
      reason: "selected_candidate_safe_field_application"
    };
  }
  if (isLowMargin && lowMarginAppliedFields.has(field) && sameAsCurrent) {
    return {
  ...
  if (!isSelected && !selectedGroupMember && !isLowMargin && sameAsCurrent) {
    return {
      decision: retrievalApplicationDecisions.SUPPORT,
      reason: "unselected_trusted_candidate_matches_current_field"
    };
  }
...
function evidenceItemForDecision(row = {}, decision = {}) {
  if (decision.decision !== retrievalApplicationDecisions.APPLY) return null;
  const resolver = resolverFieldValue(row);
  if (!resolver.field || !valuePresent(resolver.value)) return null;
  const confidenceCap = 0.72;
...
export function buildRetrievalApplicationLayer({
  ...
  const observedBefore = normalizedObservationSnapshot(controlledResult, resolvedBefore);
  const inventory = Array.isArray(candidateControl.candidate_field_inventory)
    ? candidateControl.candidate_field_inventory
    : [];
  const contentGuard = buildCandidateFieldContentGuard(inventory);
  ...
  const selectedResolverAliasFields = selectedSafeResolverAliasFields(selectedApplication);
  ...
  const decisions = inventory.map((row) => {
    ...
    const decision = decisionForRow(row, {
      ...
      selectedResolverAliasFields,
      lowMarginAppliedFields,
      selectedEligibleFields,
      lowMarginSupportedFields,
      resolvedBefore,
      observedBefore,
      contentPollutionReasons: contentGuard.reasonsFor(row)
    });
    return {
      ...
      observed_value: oldValueForRow(observedBefore, row),
      ...
      content_pollution_reasons: decision.content_pollution_reasons || []
    };
  });
