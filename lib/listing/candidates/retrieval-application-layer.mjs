...
function decisionForRow(row = {}, {
  ...
  productCorrectionCandidateId = "",
  productCorrectionFields = new Set(),
  ...
  const isProductCorrection = candidateId
    && candidateId === productCorrectionCandidateId
    && productCorrectionFields.has(field);

  if (trace.decision_eligible !== true && !isProductCorrection) {
    return {
      decision: retrievalApplicationDecisions.REJECT,
  ...
  if (!isProductCorrection && ((trace.direct_conflicts || []).includes(field) || (trace.blocked_fields || []).includes(field))) {
    return {
      decision: retrievalApplicationDecisions.BLOCK,
  ...
  if (!resolver.field || !valuePresent(resolver.value)) {
    ...
    // stamp: it could confirm what recognition found and never supply what
    // recognition missed. valuePresent(false) is false, so a catalog row that
    // knows rc=true could not reach a card whose resolver said rc=false --
    // measured on the fixed-20, a selected APPROVED_REFERENCE row whose
    // canonical_title matched the reviewed title word for word applied zero
    // fields. That is the opposite of the intended division of labour, where
    // the catalog answers "which card is this" and recognition answers "what is
    // true of this copy".
    //
    // Filling is allowed only where the catalog is actually authoritative:
    //   - the selected candidate, not any retrieved neighbour
    //   - an official checklist or writer-reviewed row; visual-vector
    //     neighbours stay confirmation-only, since sampling their values showed
    //     just 16% agreement with the reviewed title
    //   - a field that is genuinely empty, so this can fill a hole and can
    //     never overwrite something recognition read off the card
    // Conflict, permission and eligibility checks above still apply first.
    const fillEligibleSource = ["OFFICIAL_CHECKLIST", "INTERNAL_APPROVED_HISTORY"]
      .includes(sourceForRow(row));
    const catalogLane = cleanText(row.candidate_lane).toLowerCase() === "catalog";
    if (isSelected && catalogLane && fillEligibleSource && !valuePresent(oldValue) && valuePresent(row.candidate_value)) {
      return {
        decision: retrievalApplicationDecisions.APPLY,
        reason: "catalog_fills_unresolved_field"
      };
    }
    return {
      decision: retrievalApplicationDecisions.BLOCK,
  ...
  if (isProductCorrection) {
    return {
      decision: retrievalApplicationDecisions.APPLY,
      reason: "trusted_catalog_product_correction"
    };
  }
...
export function buildRetrievalApplicationLayer({
  ...
  const productCorrection = candidateControl.selected_candidate_product_correction || {};
  const productCorrectionCandidateId = cleanId(productCorrection.candidate_id);
  const productCorrectionFields = new Set(Object.keys(productCorrection.product_fields || {}));
