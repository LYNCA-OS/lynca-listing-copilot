...
export { candidateApplicationPolicyVersion } from "../pipeline/decision-owner-versions.mjs";
...
export function candidateFields(candidate = {}) {
  ...
  // Retrieval packets expose checklist people as `subjects`, while candidate
  // application owns the canonical `players` field. Bridge only when the
  // candidate has no canonical player value so an alias cannot override it.
  const playerValues = (value) => Array.isArray(value) ? value : hasValue(value) ? [value] : [];
  const explicitPlayers = [
    ...playerValues(raw.players),
    ...playerValues(raw.player)
  ].filter(hasValue);
  const subjectAlias = [
    ...playerValues(raw.subjects),
    ...playerValues(raw.subject)
  ].filter(hasValue);
  const candidatePlayers = explicitPlayers.length ? explicitPlayers : subjectAlias;
  const normalized = normalizeResolvedFields(stripReferencePrintRunNumerator(candidatePlayers.length
    ? { ...raw, players: candidatePlayers }
    : raw));
  const fields = {
