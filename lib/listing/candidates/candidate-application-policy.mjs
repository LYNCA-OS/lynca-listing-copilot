...
import { catalogDecisionTrustRank } from "../catalog/catalog-source-authority.mjs";
import {
...
export function sourceTrustScore(value = "") {
  return catalogDecisionTrustRank({ sourceTrust: normalizeSourceTrust(value) });
}
...
export function candidateFields(candidate = {}) {
  ...
  const normalized = normalizeResolvedFields(stripReferencePrintRunNumerator(raw));
  const fields = {
...
export function candidateFieldPermissions(candidate = {}) {
  ...
  const lowTrust = marketplace || sourceTrustScore(sourceTrust) <= 0;
  const permissions = {};
