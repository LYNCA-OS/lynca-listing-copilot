...
const MAX_CLIENT_FETCH_TIMEOUT_MS = 300_000;
...
export async function fetchWithBoundedRetry(url, init = {}, {
  ...
  // The direct CSM route owns a 290s browser budget around a 145s authority
  // claim plus a 120s provider deadline. Capping that request at 120s here
  // aborts a healthy provider before its own contract can settle. Keep a hard
  // ceiling at the 300s function boundary so every request remains bounded.
  const perAttemptTimeoutMs = positiveInteger(timeoutMs, 15000, {
    min: 100,
  ...
  for (let attempt = 1; attempt <= attemptsLimit; attempt += 1) {
    ...
    if (response) {
      ...
      // The caller never receives a response that we retry. Release its body
      // before opening the next attempt so an unread 5xx cannot pin a browser
      // connection/stream and turn a transient failure into client-side
      // head-of-line blocking.
      try {
        await response.body?.cancel?.();
      } catch {
        // Cancellation is cleanup only; it must not replace the real retry.
      }
