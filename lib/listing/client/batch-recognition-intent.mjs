...
const BATCH_ASSET_STATUS = Object.freeze({
...
/**
 * One status projection for both the visible card and the full-batch rail.
 *
 * Persistence wins over recognition because a failed recognition can still be
 * resolved by an operator. Explicit rejection wins over the generic `saved`
 * feedback status used by the manual-recovery ledger.
 */
export function batchAssetReviewStatus({
  result = null,
  processing = false,
  active = false
} = {}) {
  ...
  const rejected = String(result.feedbackStatus || "").toLowerCase() === "skipped"
    || String(result.explicitReviewOutcome || "").toUpperCase() === "REJECTED";
