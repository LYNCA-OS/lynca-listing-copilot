...
export function buildSemValidationEvent({
  ...
  if (status === "VALIDATED") {
    ...
    if (!Object.values(sources).some((source) => source.status === "SUPPORTED")) {
  ...
  const base = {
    ...
    parser_version: cleanText(extraction.parser_version) || null,
    sem_standard_version: cleanText(extraction.sem_standard_version) || null,
