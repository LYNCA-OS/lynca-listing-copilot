...
const tcgGrammarPattern = /\b(?:tcg|pokemon|pok[eé]mon|one\s+piece|yu\s*[- ]?gi\s*[- ]?oh|yugioh|dragon\s+ball|digimon|lorcana|magic(?::?\s+the\s+gathering)?|mtg|weiss\s+schwarz|cardfight\s+vanguard|vanguard|union\s+arena|battle\s+spirits|flesh\s+and\s+blood|star\s+wars\s+unlimited|grand\s+archive|altered|shadowverse|final\s+fantasy\s+tcg)\b/i;

function compactSlashNumber(value = "") {
  const match = cleanText(value).match(/(?:^|[^0-9])(\d{1,5})\s*\/\s*(\d{1,5})(?=$|[^0-9])/);
  if (!match) return null;
  return `${Number(match[1])}/${Number(match[2])}`;
}

function compactDenominatorOnly(value = "") {
  const match = cleanText(value).match(/(?:^|[^0-9])#?\s*\/\s*(\d{1,5})(?=$|[^0-9])/);
  return match ? String(Number(match[1])) : null;
}

export function inferSemGrammar(fields = {}) {
  const explicit = cleanUpper(
    fields.sem_grammar
    || fields.grammar
    || fields.card_grammar
    || fields.category
  ...
  if (/\bTCG\b/.test(explicit)) return "TCG";
  ...
  return tcgGrammarPattern.test(corpus) ? "TCG" : "STANDARD";
}

// A TCG checklist fraction (for example 242/204) identifies a card inside a
// set. It is not a physical-copy print run. This normalization owns that
// boundary once so OCR, resolution, and renderers cannot reinterpret it.
export function normalizeSemNumberBoundaries(fields = {}) {
  if (!fields || typeof fields !== "object" || Array.isArray(fields)) return fields;
  const normalized = { ...fields };
  if (inferSemGrammar(normalized) !== "TCG") return normalized;

  const checklistNumber = [
    normalized.tcg_card_number,
    normalized.card_number,
    normalized.collector_number,
    normalized.checklist_code
  ].map(compactSlashNumber).find(Boolean);
  if (!checklistNumber) return normalized;

  const [, checklistDenominator] = checklistNumber.split("/");
  normalized.tcg_card_number = checklistNumber;
  if (!compactSlashNumber(normalized.card_number)) normalized.card_number = checklistNumber;

  const printRunAliases = [
    normalized.print_run_number,
    normalized.serial_number,
    normalized.numerical_rarity
  ];
  const fullPrintRuns = printRunAliases.map(compactSlashNumber).filter(Boolean);
  const denominatorOnlyPrintRuns = printRunAliases.map(compactDenominatorOnly).filter(Boolean);
  const distinctPhysicalPrintRun = fullPrintRuns.some((value) => value !== checklistNumber)
    || denominatorOnlyPrintRuns.some((value) => value !== checklistDenominator);
  const checklistMisclassifiedAsPrintRun = !distinctPhysicalPrintRun && (
    fullPrintRuns.includes(checklistNumber)
    || denominatorOnlyPrintRuns.includes(checklistDenominator)
    || [
      normalized.print_run_denominator,
      normalized.serial_denominator,
      normalized.numbered_to,
      normalized.expected_serial_denominator
    ].map((value) => cleanText(value).replace(/^0+/, "") || "0").includes(checklistDenominator)
  );

  if (!checklistMisclassifiedAsPrintRun) return normalized;
  return {
    ...normalized,
...
export function classifyWriterFeedbackForSemanticLearning({
  ...
  return {
    feedback_layer: SEM_FEEDBACK_LAYER.COMMERCIAL_FEEDBACK,
    semantic_learning_status: stableTrainingSample ? "TRAINING_CANDIDATE_FROM_WRITER_TITLE" : "UNSTABLE_COMMERCIAL_SIGNAL",
    semantic_truth: false,
    training_eligible: stableTrainingSample === true
  };
}
