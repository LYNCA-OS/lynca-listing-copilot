  ...
  try {
    const response = await fetchImpl(`${origin}/api/v4/listing-preingest-worker`, {
      ...
      body: JSON.stringify({
        ...
        // Explicit detail runs use one tenant-wide fair sweep. Per-asset wakes
        // strand detail crops behind each card's anchors and create N separate
        // long tails under a shared stage-capacity lease.
        asset_id: includeDetail ? "" : String(assetId || "").trim(),
        bundle_id: includeDetail ? "" : String(bundleId || "").trim(),
