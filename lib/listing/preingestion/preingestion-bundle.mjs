...
// v3 adds a full-image serial fallback and a resolver rendezvous. The version
// is part of the idempotency key so existing assets are reprocessed exactly
// once under the stronger evidence contract.
export const preingestionOcrJobVersion = "ocr-crop-v3";
...
export function buildPreingestionWorkerJobs({
  ...
  if (enableOcr) {
    for (const crop of cropPlan.filter((item) => [
      "serial_crop",
      "card_code_crop",
      "grade_label_crop"
    ].includes(item.role))) {
