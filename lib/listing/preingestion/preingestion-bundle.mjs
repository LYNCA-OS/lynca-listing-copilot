...
function requiredTenantId(value) {
  ...
  if (!/^[a-zA-Z0-9_-]{1,128}$/.test(tenantId)) throw new Error("Pre-ingestion tenant_id is required.");
  return tenantId;
}
...
function assertBundleScope(bundle = {}) {
  ...
  const assetId = normalizeListingAssetId(bundle.asset_id);
...
export async function readVerifiedImageRecordsByAssetId({
  ...
  return (Array.isArray(rows) ? rows : []).map((row) => {
    if (requiredTenantId(row.tenant_id) !== safeTenantId || normalizeListingAssetId(row.asset_id) !== safeAssetId) {
...
export async function readPreIngestionBundleIdByAsset({
  ...
  if (!response.ok) {
    const message = await response.text();
    throw new Error(`Supabase preingestion bundle id read failed: ${response.status} ${message.slice(0, 180)}`);
  }
  const rows = await readResponseJson(response);
...
export async function enqueuePreIngestionJobs({
  ...
  for (const job of jobs) {
    normalizeListingAssetId(job.asset_id);
