...
export function normalizeOcrWorkerRevision(value = "") {
  const revision = safeString(value).toLowerCase();
  return /^[a-z0-9][a-z0-9._-]{0,119}$/.test(revision) ? revision : "";
}

export function preingestionOcrJobKeyPrefix({
  ocrJobVersion = preingestionOcrJobVersion,
  ocrWorkerRevision = process.env.OCR_WORKER_REVISION
} = {}) {
  const revision = normalizeOcrWorkerRevision(ocrWorkerRevision);
  return revision ? `ocr:${safeString(ocrJobVersion)}:${revision}:` : "";
}
...
export function currentPreingestionEvidencePatches(patches = [], {
  ...
  const currentPrefix = preingestionOcrJobKeyPrefix({ ocrJobVersion, ocrWorkerRevision });
...
export function buildPreingestionQualitySummary({
  ...
  return {
    ...
    client_capture_quality: clientCaptureQuality || null
  };
}
...
export async function readLatestPreIngestionBundleByAsset({
  ...
  try {
    return await readLatestPreIngestionBundleByAssetStrict({
      assetId,
      tenantId,
      bundleVersion,
      env,
    ...
    // Existing evidence is an accelerator on observation paths. The durable
    // post-click commit path uses the strict reader below and owns retries.
    return null;
  }
}

export async function readLatestPreIngestionBundleByAssetStrict({
  assetId,
  tenantId,
  bundleVersion = preingestionBundleVersion,
  env = process.env,
  ...
  if (!response.ok) {
    const message = await response.text();
    throw new Error(`Supabase latest pre-ingestion bundle read failed: ${response.status} ${message.slice(0, 180)}`);
  }
  const rows = await readResponseJson(response);
  const row = Array.isArray(rows) ? rows[0] || null : null;
  if (!row) return null;
...
export function buildPreingestionWorkerJobs({
  ...
  if (enableOcr) {
    const ocrJobKeyPrefix = preingestionOcrJobKeyPrefix({ ocrWorkerRevision });
    if (!ocrJobKeyPrefix) throw new Error("ocr_worker_revision_required");
    ...
    for (const crop of ocrCrops) {
      jobs.push({
        ...
        job_key: `${ocrJobKeyPrefix}${bundleId}:${crop.crop_metadata?.crop_id || crop.source_region}`,
        asset_id: assetId,
        bundle_id: bundleId,
