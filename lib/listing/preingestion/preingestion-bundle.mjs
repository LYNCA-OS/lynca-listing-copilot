...
// v8 keeps one value-aware crop per field on the default path: front serial,
// back card code, and a grade crop only when source geometry looks slab-like.
// This prevents a two-image raw card from producing six competing OCR jobs.
// Detail mode retains the complete crop plan for explicit diagnostics.
// The worker still lets
...
// v21 refreshes the immutable OCR audit record after unavailable provider
// responses became fail-closed. Older text remains available for replay, but
// a release-time 429 can no longer masquerade as a successful empty crop.
// RegionEvidence is an additive observability projection of this same OCR
// result, not a new OCR semantic contract. It therefore must not force every
// existing asset through a fresh paid OCR job.
export const preingestionOcrJobVersion = "ocr-crop-v21";
...
export function imagesFromPreIngestionBundle(bundle = {}) {
  ...
  const tenantId = stringOrNull(bundle.tenant_id || bundle.tenantId);
  const assetId = stringOrNull(bundle.asset_id || bundle.assetId);
  const rawById = new Map(rawImages.map((image) => [image.image_id, image]));
  ...
  const fromDerived = derivedImages.map((image) => {
    const source = rawById.get(image.source_image_id) || {};
    const cropMetadata = {
      crop_id: image.derived_id,
      asset_id: assetId,
      generation_id: assetId,
      source_image_id: image.source_image_id,
      source_object_path: source.object_path || null,
      source_content_sha256: source.content_sha256 || null,
      derived_object_path: image.object_path,
      crop_role: image.role,
      source_region: image.provenance?.source_region || null,
      source_side: source.role || null,
      transform_version: image.provenance?.transform_version || null
    };
    return ({
      id: image.derived_id,
...
// Recognition execution must not trust a browser-supplied bundle id. After a
// queue worker has fenced the durable job and verified its persisted session,
// resolve the newest bundle from that server-owned tenant + asset identity.
export async function readLatestPreIngestionBundleByAsset({
  assetId,
  tenantId,
  bundleVersion = preingestionBundleVersion,
  env = process.env,
  ...
  try {
    ...
    if (
      ...
      throw new Error("Supabase latest pre-ingestion bundle read returned a cross-tenant row.");
    }
    assertBundleScope(row);
    return row;
  } catch {
    // Pre-ingestion is an evidence accelerator, not an availability gate. A
    // bounded read failure leaves the canonical image path available.
    return null;
  }
}
...
function cropSourceSide(crop = {}) {
  return safeString(crop.crop_metadata?.source_side).toLowerCase();
}

function cropSourceLooksLikeSlab(crop = {}) {
  const metadata = crop.crop_metadata || {};
  const width = Number(metadata.source_width);
  const height = Number(metadata.source_height);
  if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) return false;
  return Math.min(width, height) / Math.max(width, height) <= 0.64;
}

function preferredCrop(crops = [], preferredSide = "") {
  return crops.find((crop) => cropSourceSide(crop) === preferredSide) || crops[0] || null;
}

function sparseDefaultOcrCrops(crops = []) {
  const byRole = new Map();
  for (const crop of crops) {
    if (!byRole.has(crop.role)) byRole.set(crop.role, []);
    byRole.get(crop.role).push(crop);
  }
  return [
    preferredCrop(byRole.get("serial_crop"), "front"),
    preferredCrop(byRole.get("card_code_crop"), "back"),
    preferredCrop((byRole.get("grade_label_crop") || []).filter(cropSourceLooksLikeSlab), "front")
  ].filter(Boolean);
}
...
export function buildPreingestionWorkerJobs({
  ...
  if (enableOcr) {
    const merged = mergedOcrCrops(cropPlan);
    const ocrCrops = enableOcrDetail
      ? merged
      : sparseDefaultOcrCrops(merged);
    for (const crop of ocrCrops) {
      const sourceImageId = safeString(crop.source_image_id || crop.crop_metadata?.source_image_id);
      const sourceImage = sourceImages.find((image) => safeString(
        image.image_id || image.derived_id || image.id
      ) === sourceImageId) || {};
      jobs.push({
        tenant_id: tenantId,
        ...
        payload: {
          crop,
          image_generation_id: assetId,
          source_image_sha256: normalizeSha256(
            sourceImage.content_sha256 || sourceImage.contentSha256 || sourceImage.sha256
          ),
          bundle_version: bundle.bundle_version || preingestionBundleVersion,
