...
// Read the current OCR contract alongside the canonical image and bundle
// lookups. A cache decision can then prove every expected idempotency key is
// terminal without repeating a bundle write or a jobs upsert.
export async function readCurrentPreingestionOcrJobsByAsset({
  assetId,
  tenantId,
  env = process.env,
  fetchImpl = globalThis.fetch
  ...
  endpoint.searchParams.set("select", "tenant_id,asset_id,bundle_id,job_key,status");
  endpoint.searchParams.set("tenant_id", `eq.${safeTenantId}`);
  endpoint.searchParams.set("asset_id", `eq.${safeAssetId}`);
  ...
  try {
    ...
    return rows.filter((row) => (
      requiredTenantId(row.tenant_id) === safeTenantId
      && normalizeDurableListingAssetId(row.asset_id) === safeAssetId
      && safeString(row.job_key).startsWith(`ocr:${preingestionOcrJobVersion}:`)
    ));
  } catch {
    // Cache reuse must fail closed. The normal idempotent write path remains
    // available when this optional proof read is unavailable.
    return null;
  }
}
  ...
  if (enableOcr) {
    ...
    for (const crop of ocrCrops) {
      jobs.push({
        ...
        payload: {
          crop,
          // Raw OCR text is audit evidence, not a Resolver field. Persist it
          // only for the full-information Oracle so the writer path stays
          // compact and unchanged.
          persist_raw_ocr_observation: enableOcrDetail === true
        },
        created_at: createdAt,
        updated_at: createdAt
      });
