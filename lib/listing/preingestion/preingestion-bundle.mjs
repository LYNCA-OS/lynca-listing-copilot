...
// v5 adds bounded transient retries and waits for authoritative slab-label
// text before the serial fast-exit. The versioned idempotency key lets assets
// poisoned by exhausted v4 jobs receive one fresh, auditable verification set.
export const preingestionOcrJobVersion = "ocr-crop-v6";
...
const ocrWorkerPriorityByRole = Object.freeze({
  // The first worker wave extracts lookup keys and their disambiguating
  // context. With the default OCR concurrency=3 these run together, allowing
  // catalog lookup to start before the full vision provider returns.
  card_code_crop: 10,
  year_product_crop: 15,
  subject_crop: 20,
  serial_crop: 25,
  grade_label_crop: 30
});
