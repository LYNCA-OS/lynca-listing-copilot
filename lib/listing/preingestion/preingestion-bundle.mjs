...
export function normalizePreingestionImageRole(role = "") {
  ...
  return roleAliases.get(normalized) || normalized || "front_original";
}
...
export function createPreIngestionBundle({
  ...
  const normalizedPatches = evidencePatches
    .map(normalizeEvidencePatch)
    .filter(Boolean);
...
export function buildPreingestionWorkerJobs({
  ...
  if (enableOcr) {
    for (const crop of cropPlan.filter((item) => [
      ...
      jobs.push({
        job_key: `ocr:${bundleId}:${crop.crop_metadata?.crop_id || crop.source_region}`,
        asset_id: assetId,
        bundle_id: bundleId,
...
export async function enqueuePreIngestionJobs({
  ...
  return { enqueued: jobs.length, durable: true };
}
