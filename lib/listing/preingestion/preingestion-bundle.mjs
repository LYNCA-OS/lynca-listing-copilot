...
// v22 moves the sparse serial scan off the brittle bottom-right assumption.
// v23 preserves image side from canonical ids when browser role metadata is
// absent, allowing the slab-reverse selection to actually survive ingestion.
// v24 raises serial OCR signal density by scanning the evidenced upper/middle
// right-side band instead of spending most of the crop on unrelated text.
export const preingestionOcrJobVersion = "ocr-crop-v24";
...
function sparseDefaultOcrCrops(crops = []) {
  ...
  const serialCrops = byRole.get("serial_crop") || [];
  // Raw-card serials are most often on the face. Slabbed cards are the
  // important exception: their front label consumes the top band while the
  // card's limited numbering remains visible on the reverse. Select one side
  // deterministically so the sparse lane keeps one serial request per card.
  const serialPreferredSide = serialCrops.some(cropSourceLooksLikeSlab) ? "back" : "front";
  return [
    preferredCrop(serialCrops, serialPreferredSide),
