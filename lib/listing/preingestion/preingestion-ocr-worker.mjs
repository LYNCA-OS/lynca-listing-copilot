...
export async function readPreingestionOcrState({
  ...
  jobsEndpoint.searchParams.set("select", "job_id,status,attempts,last_error,job_key,updated_at");
  ...
  const jobs = (await readResponseJson(jobsResponse)) || [];

  const bundleEndpoint = new URL(`${url}/rest/v1/preingestion_bundles`);
  bundleEndpoint.searchParams.set("select", "bundle_id,evidence_patches,updated_at");
  ...
  const patches = Array.isArray(bundleRows[0]?.evidence_patches) ? bundleRows[0].evidence_patches : [];
...
export async function processQueuedPreingestionOcrJobs({
  ...
  const concurrency = positiveInteger(env.PREINGESTION_OCR_CONCURRENCY, 3, 6);
  await mapWithConcurrency(jobs, concurrency, async (job) => {
    try {
      ...
      const patches = verified.flatMap((entry) => bundlePatchesFromOcrResult(entry.result, entry.job));
      outcomes.push({ job, patches, ok: true });
    } catch (error) {
      outcomes.push({ job, patches: [], ok: false, error: error?.message || String(error) });
    }
  });

  const patchesByBundle = new Map();
  outcomes.filter((outcome) => outcome.ok && outcome.job.bundle_id && outcome.patches.length).forEach((outcome) => {
    const bucket = patchesByBundle.get(outcome.job.bundle_id) || [];
    bucket.push(...outcome.patches);
    patchesByBundle.set(outcome.job.bundle_id, bucket);
  });
  let patchesAppended = 0;
  const bundleWriteFailures = new Map();
  for (const [jobBundleId, patches] of patchesByBundle) {
    try {
      ...
      bundleWriteFailures.set(jobBundleId, error?.message || String(error));
    }
  }
  ...
  for (const outcome of outcomes) {
    ...
    const bundleWriteError = bundleWriteFailures.get(outcome.job.bundle_id);
    if (outcome.patches.length && bundleWriteError) {
    ...
    await completePreingestionOcrJob({ jobId: outcome.job.job_id, status: "succeeded", env, fetchImpl });
    succeeded += 1;
  }
