...
import { ocrSupportedNormalizedFields } from "../ocr/ocr-contract.mjs";
...
import {
  ...
  releaseListingStageCapacityWithRetry,
...
const persistedOcrNormalizedFields = new Set(ocrSupportedNormalizedFields);

const maxPersistedOcrNormalizedFields = 8;

export function createOcrBatchingClient(client, { windowMs = 4, maxBatch = 8 } = {}) {
  if (!client || typeof client.verifyCrop !== "function" || typeof client.verifyCrops !== "function") return client;
  const pending = [];
  let timer = null;
  let flushing = false;
  ...
  const flush = async () => {
    ...
    timer = null;
    const batch = pending.splice(0, Math.max(1, Math.min(8, Number(maxBatch) || 8)));
    try {
      const results = await client.verifyCrops(batch.map((entry) => entry.input));
      if (!Array.isArray(results) || results.length !== batch.length) {
        throw new Error("ocr_batch_result_count_mismatch");
      }
  ...
  return {
    ...
    verifyCrop(input) {
      return new Promise((resolve, reject) => {
        ...
        if (pending.length >= Math.max(1, Math.min(8, Number(maxBatch) || 8))) {
          queueMicrotask(flush);
        } else if (!timer) {
          timer = setTimeout(flush, Math.max(0, Math.min(20, Number(windowMs) || 4)));
        }
      });
    }
  };
...
function fetchOcrSupabase(endpoint, init = {}, { env, fetchImpl }) {
  const method = normalizeText(init.method || "GET").toUpperCase();
  return fetchPreingestionSupabase(endpoint, init, {
    env,
    fetchImpl,
...
function verifiedSerialEvidenceFromPatches(patches = []) {
  ...
  for (const patch of Array.isArray(patches) ? patches : []) {
    ...
    const current = values.get(parsed.print_run_number) || {
      ...
      consensus_observations: new Set()
    };
    ...
    if (provenance.serial_consensus_verified === true && observationKey) {
      current.consensus_observations.add(observationKey);
    }
  ...
  const candidates = [...values.entries()]
    .map(([value, record]) => ({
      ...
      consensus_observation_count: record.consensus_observations.size,
      verified: record.observations.size >= 2
        || (record.consensus_observations.size >= 1 && record.confidence >= 0.90)
    }))
    .sort((left, right) => right.confidence - left.confidence);
  // Only a dedicated serial-crop read outranks full-image noise. If two or more
  // distinct values were each seen on a serial crop, the crops disagree and the
  // numerator stays a genuine conflict.
  const directCropValues = candidates.filter((candidate) => candidate.direct_crop_observation_count >= 1);
  const directCropWinner = directCropValues.length === 1 && directCropValues[0].confidence >= 0.94
    ? directCropValues[0]
    : null;
  const winner = candidates.length === 1 && candidates[0].verified
    ? candidates[0]
    : directCropWinner;
  return {
    verified: Boolean(winner),
    conflict: candidates.length > 1 && !winner,
...
function ocrLeaseSchemaMissing(value = "") {
  const text = normalizeText(value).toLowerCase();
  return /(?:max_attempts|lease_owner|lease_expires_at)/.test(text)
    && /(?:column|schema cache|does not exist|could not find|unknown)/.test(text);
}

function ocrJobMaxAttempts(job = {}, env = process.env) {
  const configured = positiveInteger(env.PREINGESTION_OCR_MAX_ATTEMPTS, 3, 20);
  return positiveInteger(job.max_attempts, configured, 20);
}

function maxAttemptsForOcrJob(job = {}, env = process.env) {
  return ocrJobMaxAttempts(job, env);
}

function ocrJobHasAttemptsRemaining(job = {}, env = process.env) {
  return Number(job.attempts || 0) < ocrJobMaxAttempts(job, env);
}

export function preingestionOcrJobLeasePlan(env = process.env) {
  const leaseSeconds = Math.max(
    30,
    positiveInteger(env.PREINGESTION_OCR_JOB_LEASE_SECONDS, 120, 900)
  );
  const defaultHeartbeatMs = Math.max(5_000, Math.floor((leaseSeconds * 1000) / 3));
  const heartbeatMs = Math.max(
    5_000,
    Math.min(
      Math.floor(leaseSeconds * 1000 * 0.75),
      positiveInteger(env.PREINGESTION_OCR_JOB_HEARTBEAT_MS, defaultHeartbeatMs, 300_000)
    )
  );
  return {
    lease_seconds: leaseSeconds,
    heartbeat_ms: heartbeatMs,
    max_attempts: positiveInteger(env.PREINGESTION_OCR_MAX_ATTEMPTS, 3, 20)
  };
}

function preingestionOcrLeaseSeconds(env = process.env) {
  return preingestionOcrJobLeasePlan(env).lease_seconds;
}

function leaseExpiryIso(leaseSeconds, now = Date.now()) {
  const timestamp = now instanceof Date ? now.getTime() : Number(now);
  return new Date(timestamp + Math.max(30, Number(leaseSeconds) || 120) * 1000).toISOString();
}
...
async function readPreingestionOcrJobById({
  jobId,
  tenantId = "",
  durableLeaseColumns = true,
  env = process.env,
  fetchImpl = globalThis.fetch
} = {}) {
  if (!preingestionSupabaseConfigured(env) || !normalizeText(jobId)) return null;
  ...
  endpoint.searchParams.set("select", durableLeaseColumns
    ? "tenant_id,job_id,status,attempts,max_attempts,lease_owner,lease_expires_at,last_error"
    : "tenant_id,job_id,status,attempts,last_error");
  endpoint.searchParams.set("tenant_id", `eq.${safeTenantId}`);
  endpoint.searchParams.set("job_id", `eq.${normalizeText(jobId)}`);
  endpoint.searchParams.set("limit", "1");
  const response = await fetchOcrSupabase(endpoint, {
    headers: supabaseHeaders(serviceRoleKey)
  }, { env, fetchImpl });
  if (!response.ok) {
    const message = await response.text();
    if (durableLeaseColumns && ocrLeaseSchemaMissing(message)) {
      return readPreingestionOcrJobById({ jobId, tenantId: safeTenantId, durableLeaseColumns: false, env, fetchImpl });
    }
    return null;
  }
  ...
  if (!row) return null;
  if (
    normalizeText(row.tenant_id) !== safeTenantId
    || normalizeText(row.job_id) !== normalizeText(jobId)
  ) return null;
  return row;
}
...
export async function claimQueuedPreingestionOcrJobs({
  ...
  const currentOcrPrefix = preingestionOcrJobKeyPrefix({
    ocrWorkerRevision: env.OCR_WORKER_REVISION
  });
  // Worker revision is part of the durable OCR identity. Historical rows stay
  // append-only for audit, but must never consume current capacity or trigger a
  // Provider call. Missing revision therefore fails closed before touching the
  // queue instead of falling back to the version-only legacy namespace.
  if (!currentOcrPrefix) return [];
  const { url, serviceRoleKey } = preingestionSupabaseConfig(env);
  const leasePlan = preingestionOcrJobLeasePlan(env);
  ...
  const buildListEndpoint = (durableLeaseColumns = true) => {
    const endpoint = new URL(`${url}/rest/v1/preingestion_jobs`);
    endpoint.searchParams.set(
      "select",
      durableLeaseColumns
        ? "tenant_id,job_id,job_key,asset_id,bundle_id,payload,attempts,max_attempts,priority,created_at,updated_at,lease_owner,lease_expires_at"
        : "tenant_id,job_id,job_key,asset_id,bundle_id,payload,attempts,priority,created_at,updated_at"
    );
    endpoint.searchParams.set("status", "eq.queued");
    endpoint.searchParams.set("job_type", `eq.${preingestionOcrJobType}`);
    endpoint.searchParams.set("job_key", `like.${currentOcrPrefix}*`);
    ...
    if (anchorOnly) endpoint.searchParams.set("priority", "lte.14");
    ...
    endpoint.searchParams.set("order", scopedToAsset ? "priority.asc,created_at.asc" : "created_at.asc,priority.asc");
    endpoint.searchParams.set("limit", String(candidateLimit));
    return endpoint;
  };

  let durableLeaseSupported = true;
  let listEndpoint = buildListEndpoint(true);
  let listResponse = await fetchOcrSupabase(listEndpoint, {
    headers: supabaseHeaders(serviceRoleKey, { prefer: "return=representation" })
  }, { env, fetchImpl });
  let listedRows;
  if (!listResponse.ok) {
    const message = await listResponse.text();
    if (!ocrLeaseSchemaMissing(message)) {
      throw new Error(`Supabase preingestion job list failed: ${listResponse.status} ${message.slice(0, 180)}`);
    }
    // Rolling-deploy compatibility only. Without the migration a claim still
    // works, but it is explicitly marked unleased and cannot claim durable
    // recovery guarantees.
    durableLeaseSupported = false;
    listEndpoint = buildListEndpoint(false);
    listResponse = await fetchOcrSupabase(listEndpoint, {
      headers: supabaseHeaders(serviceRoleKey, { prefer: "return=representation" })
    }, { env, fetchImpl });
    if (!listResponse.ok) {
      const fallbackMessage = await listResponse.text();
      throw new Error(`Supabase preingestion job list failed: ${listResponse.status} ${fallbackMessage.slice(0, 180)}`);
    }
  }
  const listedPayload = (await readResponseJson(listResponse)) || [];
  listedRows = (Array.isArray(listedPayload) ? listedPayload : []).filter((job) => {
    let rowTenantId = "";
    try {
      rowTenantId = requiredOcrTenantId(job?.tenant_id);
    } catch {
      return false;
    }
    if (!normalizeText(job?.job_id)) return false;
    if (!normalizeText(job?.job_key).startsWith(currentOcrPrefix)) return false;
    if (safeTenantId && rowTenantId !== safeTenantId) return false;
    if (normalizeText(assetId) && normalizeText(job?.asset_id) !== normalizeText(assetId)) return false;
    if (normalizeText(bundleId) && normalizeText(job?.bundle_id) !== normalizeText(bundleId)) return false;
    return true;
  });
  ...
  for (const row of rows) {
    if (!ocrJobHasAttemptsRemaining(row, env)) {
      const exhaustedEndpoint = new URL(`${url}/rest/v1/preingestion_jobs`);
      exhaustedEndpoint.searchParams.set("job_id", `eq.${row.job_id}`);
      exhaustedEndpoint.searchParams.set("tenant_id", `eq.${requiredOcrTenantId(row.tenant_id)}`);
      exhaustedEndpoint.searchParams.set("status", "eq.queued");
      exhaustedEndpoint.searchParams.set("job_key", `like.${currentOcrPrefix}*`);
      exhaustedEndpoint.searchParams.set("attempts", `eq.${Number(row.attempts || 0)}`);
      await fetchOcrSupabase(exhaustedEndpoint, {
        method: "PATCH",
        headers: supabaseHeaders(serviceRoleKey, { prefer: "return=minimal" }),
        body: JSON.stringify({
          status: "failed",
          last_error: "ocr_max_attempts_exhausted",
          ...(durableLeaseSupported ? { lease_owner: null, lease_expires_at: null } : {})
        })
      }, { env, fetchImpl }).catch(() => null);
      continue;
    }
    ...
    claimEndpoint.searchParams.set("job_key", `like.${currentOcrPrefix}*`);
    claimEndpoint.searchParams.set("attempts", `eq.${Math.max(0, Number(row.attempts) || 0)}`);
    if (durableLeaseSupported) {
    ...
    const claimBody = {
      ...
      last_error: null,
      ...(durableLeaseSupported ? {
        lease_owner: safeLeaseOwner,
        lease_expires_at: leaseExpiryIso(effectiveLeaseSeconds)
      } : {})
    };
    let claimResponse = await fetchOcrSupabase(claimEndpoint, {
      method: "PATCH",
      headers: supabaseHeaders(serviceRoleKey, { prefer: "return=representation" }),
      body: JSON.stringify(claimBody)
    }, { env, fetchImpl });
    if (!claimResponse.ok && durableLeaseSupported) {
      const message = await claimResponse.text();
      if (ocrLeaseSchemaMissing(message)) {
        durableLeaseSupported = false;
        delete claimBody.lease_owner;
        delete claimBody.lease_expires_at;
        claimResponse = await fetchOcrSupabase(claimEndpoint, {
          method: "PATCH",
          headers: supabaseHeaders(serviceRoleKey, { prefer: "return=representation" }),
          body: JSON.stringify(claimBody)
        }, { env, fetchImpl });
    ...
    if (Array.isArray(updated) && updated.length === 1) {
      claimed.push({
        ...row,
        ...updated[0],
        tenant_id: requiredOcrTenantId(row.tenant_id),
        job_id: normalizeText(row.job_id),
        durable_lease_supported: durableLeaseSupported,
        lease_owner: durableLeaseSupported ? safeLeaseOwner : null,
        lease_expires_at: durableLeaseSupported
          ? (updated[0].lease_expires_at || claimBody.lease_expires_at)
          : null
      });
    } else if (durableLeaseSupported) {
      // A PATCH may commit and then lose its response. The bounded transport
      // retry sees zero queued rows, so reconcile by lease owner before giving
      // up; otherwise a legitimately claimed job would wait for expiry.
      const reconciled = await readPreingestionOcrJobById({
        jobId: row.job_id,
      ...
      if (normalizeText(reconciled?.status).toLowerCase() === "running"
        && normalizeText(reconciled?.lease_owner) === safeLeaseOwner) {
        claimed.push({
          ...row,
          ...reconciled,
          durable_lease_supported: true
        });
      }
    }
  }
...
export async function completePreingestionOcrJob({
  ...
  const updated = (await readResponseJson(response)) || [];
  if (Array.isArray(updated) && updated.length === 1) return true;
  const reconciled = await readPreingestionOcrJobById({ jobId, tenantId: safeTenantId, env, fetchImpl });
  return normalizeText(reconciled?.status).toLowerCase()
...
export async function requeuePreingestionOcrJob({
  ...
  if (Array.isArray(updated) && updated.length === 1) return true;
  const reconciled = await readPreingestionOcrJobById({ jobId, tenantId: safeTenantId, env, fetchImpl });
  return normalizeText(reconciled?.status).toLowerCase() === "queued";
}

export async function deferPreingestionOcrJobForCapacity({
  jobId,
  tenantId = "",
  leaseOwner = "",
  ...
  if (!preingestionSupabaseConfigured(env) || !normalizeText(jobId) || !normalizeText(leaseOwner)) return false;
  const attempts = Math.trunc(Number(expectedAttempts));
  if (!Number.isFinite(attempts) || attempts < 1) return false;
  const safeTenantId = requiredOcrTenantId(tenantId);
  const safeLeaseOwner = requiredOcrLeaseOwner(leaseOwner);
  const refundedAttempts = attempts - 1;
  const { url, serviceRoleKey } = preingestionSupabaseConfig(env);
  const endpoint = new URL(`${url}/rest/v1/preingestion_jobs`);
  endpoint.searchParams.set("job_id", `eq.${normalizeText(jobId)}`);
  endpoint.searchParams.set("tenant_id", `eq.${safeTenantId}`);
  endpoint.searchParams.set("status", "eq.running");
  endpoint.searchParams.set("attempts", `eq.${attempts}`);
  endpoint.searchParams.set("lease_owner", `eq.${safeLeaseOwner}`);
  const response = await fetchOcrSupabase(endpoint, {
    method: "PATCH",
    headers: supabaseHeaders(serviceRoleKey, { prefer: "return=representation" }),
    body: JSON.stringify({
      status: "queued",
      attempts: refundedAttempts,
      last_error: normalizeText(error).slice(0, 500) || "ocr_stage_capacity_busy",
      lease_owner: null,
      lease_expires_at: null
    })
  }, { env, fetchImpl });
  ...
  // A conditional PATCH can commit while its response is lost. Reconcile the
  // refunded attempt before declaring the lease lost; retrying the work here
  // would incorrectly consume provider capacity.
  const reconciled = await readPreingestionOcrJobById({
    jobId,
  ...
  return normalizeText(reconciled?.status).toLowerCase() === "queued"
    && Number(reconciled?.attempts) === refundedAttempts
    && !normalizeText(reconciled?.lease_owner)
    && !normalizeText(reconciled?.lease_expires_at);
}
...
export async function heartbeatPreingestionOcrJob({
  jobId,
  tenantId = "",
  leaseOwner,
  leaseSeconds = null,
  ...
  const owner = normalizeText(leaseOwner);
  if (!preingestionSupabaseConfigured(env) || !normalizeText(jobId) || !owner) {
    return { renewed: false, supported: false, reason: "missing_lease_identity" };
  }
  const safeTenantId = requiredOcrTenantId(tenantId);
  const { url, serviceRoleKey } = preingestionSupabaseConfig(env);
  const leasePlan = preingestionOcrJobLeasePlan(env);
  ...
  endpoint.searchParams.set("tenant_id", `eq.${safeTenantId}`);
  endpoint.searchParams.set("status", "eq.running");
  endpoint.searchParams.set("lease_owner", `eq.${owner}`);
  const response = await fetchOcrSupabase(endpoint, {
    method: "PATCH",
    headers: supabaseHeaders(serviceRoleKey, { prefer: "return=representation" }),
    body: JSON.stringify({
      lease_expires_at: leaseExpiryIso(leaseSeconds || leasePlan.lease_seconds)
    })
  }, { env, fetchImpl });
  if (!response.ok) {
    const message = await response.text();
    return {
      renewed: false,
      supported: !ocrLeaseSchemaMissing(message),
      reason: ocrLeaseSchemaMissing(message) ? "lease_schema_unavailable" : `heartbeat_${response.status}`
    };
  }
  const updated = (await readResponseJson(response)) || [];
  return {
    renewed: Array.isArray(updated) && updated.length === 1,
    supported: true,
    reason: Array.isArray(updated) && updated.length === 1 ? null : "lease_ownership_lost"
  };
}

export function startPreingestionOcrJobHeartbeat({
  jobId,
  tenantId = "",
  leaseOwner,
  leaseSeconds = null,
  ...
  const leasePlan = preingestionOcrJobLeasePlan(env);
  const stats = {
    enabled: Boolean(normalizeText(jobId) && normalizeText(leaseOwner)),
    interval_ms: leasePlan.heartbeat_ms,
    attempts: 0,
    renewed: 0,
    failures: 0,
  ...
  const beat = async () => {
    if (!stats.enabled || inFlight) return;
    stats.attempts += 1;
    inFlight = heartbeatPreingestionOcrJob({
      jobId,
      tenantId,
      leaseOwner,
      leaseSeconds: leaseSeconds || leasePlan.lease_seconds,
      env,
      fetchImpl
    }).then((result) => {
      if (result.renewed) stats.renewed += 1;
        ...
        stats.failures += 1;
        stats.last_error = result.reason || "ocr_job_heartbeat_failed";
        if (result.reason === "lease_ownership_lost") stats.ownership_lost = true;
        if (result.supported === false) stats.enabled = false;
      }
    }).catch((error) => {
      stats.failures += 1;
      stats.last_error = normalizeText(error?.message || error).slice(0, 160) || "ocr_job_heartbeat_failed";
    }).finally(() => {
      inFlight = null;
    });
    await inFlight;
...
export async function recoverStalePreingestionOcrJobs({
  ...
  if (!preingestionSupabaseConfigured(env)) {
    return { configured: false, supported: false, inspected: 0, requeued: 0, finalized: 0 };
  }
  const scopedToAsset = Boolean(normalizeText(assetId) || normalizeText(bundleId));
  const safeTenantId = scopedToAsset
    ? requiredOcrTenantId(tenantId)
  ...
  if (!Number.isFinite(recoveredAt.getTime())) {
    throw new TypeError("invalid_preingestion_ocr_recovery_time");
  }
  ...
  if (normalizeText(assetId)) endpoint.searchParams.set("asset_id", `eq.${normalizeText(assetId)}`);
  if (normalizeText(bundleId)) endpoint.searchParams.set("bundle_id", `eq.${normalizeText(bundleId)}`);
  // Read RUNNING rows oldest-first, then validate expiry again in application
  // code. This also recovers rolling-deployment rows whose newly added lease
  // columns are still null, while preventing a faulty/mocked PostgREST result
  // from reclaiming a fresh lease.
  endpoint.searchParams.set("order", "updated_at.asc");
  endpoint.searchParams.set("limit", String(Math.max(1, Math.min(128, Number(limit) || 64))));
  ...
  if (!response.ok) {
    ...
    if (ocrLeaseSchemaMissing(message)) {
      return {
        ...
        requeued: 0,
        finalized: 0,
        reason: "lease_schema_unavailable"
      };
    }
    return {
      configured: true,
      ...
      requeued: 0,
      finalized: 0,
      reason: `stale_read_${response.status}`
    };
  }
  ...
  for (const row of rows) {
    let rowTenantId = "";
    try {
      rowTenantId = requiredOcrTenantId(row.tenant_id);
    } catch {
      skipped += 1;
      continue;
    }
    if (
      (safeTenantId && rowTenantId !== safeTenantId)
      || normalizeText(row.status).toLowerCase() !== "running"
    ) {
      skipped += 1;
      continue;
    }
    ...
    const canRetry = ocrJobHasAttemptsRemaining(row, env);
    ...
    updateEndpoint.searchParams.set("tenant_id", `eq.${rowTenantId}`);
    updateEndpoint.searchParams.set("status", "eq.running");
    updateEndpoint.searchParams.set("attempts", `eq.${attempts}`);
    if (normalizeText(row.lease_owner)) {
      updateEndpoint.searchParams.set("lease_owner", `eq.${normalizeText(row.lease_owner)}`);
    } else {
      updateEndpoint.searchParams.set("lease_owner", "is.null");
    }
    if (normalizeText(row.lease_expires_at)) {
      updateEndpoint.searchParams.set("lease_expires_at", `eq.${normalizeText(row.lease_expires_at)}`);
    } else {
      updateEndpoint.searchParams.set("lease_expires_at", "is.null");
      if (normalizeText(row.updated_at)) {
        updateEndpoint.searchParams.set("updated_at", `eq.${normalizeText(row.updated_at)}`);
      }
    }
    const update = await fetchOcrSupabase(updateEndpoint, {
      method: "PATCH",
      headers: supabaseHeaders(serviceRoleKey, { prefer: "return=representation" }),
      body: JSON.stringify({
  ...
  return {
    ...
    race_lost: raceLost,
    skipped,
    fresh_ignored: freshIgnored
  };
}

export function retryableOcrFailure(error = "") {
  if (error?.retryable === true || error?.recoverable === true) return true;
  if (error?.retryable === false || error?.recoverable === false) return false;
  ...
  const httpStatus = Number(
    ...
    || text.match(/(?:http\s*|status\s*)(\d{3})/)?.[1]
  );
  if ([408, 409, 425, 429].includes(httpStatus) || httpStatus >= 500) return true;
  if (httpStatus >= 400 && httpStatus < 500) return false;
  if (/timeout|timed out|network|rate.?limit|connection|temporar|cold start|overload|fetch failed/.test(text)) {
    return true;
  }
  if (/source_object_path missing|no text|not visible|empty crop|invalid|unsupported|unauthori[sz]ed|forbidden|not found|content policy/.test(text)) {
    return false;
  }

  // A new worker/SDK failure is more likely transient than a proven bad input.
  // Unknown failures still stop after the job's fixed attempt budget.
  return true;
}

async function transitionOcrJobAfterFailure({
  job,
  error,
  ...
  const message = normalizeText(error?.message || error).slice(0, 500) || "ocr_field_job_failed";
  const leaseOwner = job?.durable_lease_supported === false ? "" : normalizeText(job?.lease_owner);
  const shouldRetry = (forceRetryable || retryableOcrFailure(error))
    && ocrJobHasAttemptsRemaining(job, env);
  const saved = shouldRetry
  ...
  return {
    status: saved ? (shouldRetry ? "requeued" : "failed") : "lease_lost",
...
export async function requeueRetryableFailedPreingestionOcrJobs({
  ...
  const { url, serviceRoleKey } = preingestionSupabaseConfig(env);
  const buildEndpoint = (durable = true) => {
    const endpoint = new URL(`${url}/rest/v1/preingestion_jobs`);
    endpoint.searchParams.set("select", durable
      ? "tenant_id,job_id,job_key,attempts,max_attempts,last_error,lease_owner,lease_expires_at"
      : "tenant_id,job_id,job_key,attempts,last_error");
  ...
  let durableLeaseSupported = true;
  let response = await fetchOcrSupabase(buildEndpoint(true), {
    headers: supabaseHeaders(serviceRoleKey)
  }, { env, fetchImpl });
  if (!response.ok) {
    const message = await response.text();
    if (!ocrLeaseSchemaMissing(message)) {
      return { requeued: 0, configured: true, reason: `failed_job_read_${response.status}` };
    }
    durableLeaseSupported = false;
    response = await fetchOcrSupabase(buildEndpoint(false), {
      headers: supabaseHeaders(serviceRoleKey)
    }, { env, fetchImpl });
  ...
  const retryable = rows.filter((row) => (
    Boolean(currentOcrPrefix)
    &&
    (!safeTenantId || normalizeText(row.tenant_id) === safeTenantId)
    && normalizeText(row.job_key).startsWith(currentOcrPrefix)
    && ocrJobHasAttemptsRemaining({ ...row, max_attempts: row.max_attempts || maxAttempts }, env)
    && (!durableLeaseSupported || (!normalizeText(row.lease_owner) && !normalizeText(row.lease_expires_at)))
...
export function ocrRequestForPreingestionJob(job = {}, { imageUrl = "" } = {}) {
  ...
  return {
    ...
    metadata: {
      ...
      // Only slab labels earn a full-image fallback. Serial evidence must stay
      // tied to the explicit current-image crop; broad scans were both slower
      // and materially less precise in evaluation.
      inline_full_image_fallback: (crop.role || metadata.crop_role) === "grade_label_crop",
...
export function assertAvailablePreingestionOcrResult(result = {}) {
  if (!["UNAVAILABLE", "ERROR", "FAILED"].includes(normalizeText(result.worker_status).toUpperCase())) {
    return result;
  }
  const error = new Error(result.worker_reason || `ocr_worker_${normalizeText(result.worker_status).toLowerCase()}`);
  error.retryable = true;
  error.code = "OCR_WORKER_UNAVAILABLE";
  throw error;
}
...
async function verifyPreingestionJob(client, job, imageUrl, { env = process.env } = {}) {
  ...
  const primary = assertAvailablePreingestionOcrResult(await client.verifyCrop(request));
  ...
  for (const [field, evidenceField] of Object.entries(evidence)) {
    ...
    const patch = normalizeEvidencePatch({
      ...
      provenance: {
        ...
        worker_service_revision: ocrResult.worker_service_revision || null,
        vision_unit_count: Number(ocrResult.vision_unit_count || 0),
        serial_consensus_verified: ocrResult.serial_consensus?.verified === true,
        serial_consensus_conflict: ocrResult.serial_consensus?.conflict === true
      }
    });
...
// A completed OCR request is not the same thing as usable evidence. Keep that
// distinction durable without persisting the crop text itself: otherwise a
// successful provider response with no parsed field is indistinguishable from
// a real card-code patch in later anchor audits.
export function summarizeOcrEvidenceOutcome(verified = [], patches = []) {
  ...
  const rawTextPresentCount = entries.filter((result) => normalizeText(result.raw_text)).length;
  const textCandidateCount = entries.reduce((sum, result) => (
    sum + (Array.isArray(result.text_candidates) ? result.text_candidates.length : 0)
  ), 0);
  const supportedNormalizedFields = [...new Set(entries.flatMap((result) => [
    ...Object.keys(result.normalized_fields || {}),
    ...Object.keys(result.evidence_patch?.evidence || {})
  ])
    .map((field) => normalizeText(field).toLowerCase())
    .filter((field) => persistedOcrNormalizedFields.has(field)))].sort();
  const normalizedFields = supportedNormalizedFields.slice(0, maxPersistedOcrNormalizedFields);
  const workerStatuses = [...new Set(entries
    .map((result) => normalizeText(result.worker_status || result.status).toUpperCase())
    .filter(Boolean))];
  const workerReasons = [...new Set(entries
    .map((result) => normalizeText(result.worker_reason || result.reason).slice(0, 120))
    .filter(Boolean))];
  const providerUnavailable = workerStatuses.includes("UNAVAILABLE")
    || workerReasons.some((reason) => /unavailable|disabled|not[_ -]?configured/i.test(reason));

  let status;
  let reasonCode;
  if (patchCount > 0) {
    status = "EVIDENCE_PATCH_READY";
    reasonCode = "OCR_EVIDENCE_PATCH_READY";
  } else if (supportedNormalizedFields.length > 0) {
    status = "NORMALIZED_FIELD_DROPPED";
    reasonCode = "OCR_NORMALIZED_FIELD_DROPPED_BEFORE_PATCH";
  } else if (providerUnavailable) {
    status = "PROVIDER_UNAVAILABLE";
    reasonCode = "OCR_PROVIDER_UNAVAILABLE";
  } else if (rawTextPresentCount > 0 || textCandidateCount > 0) {
    status = "TEXT_NOT_NORMALIZED";
    reasonCode = "OCR_TEXT_NOT_NORMALIZED_TO_SUPPORTED_FIELD";
  } else {
    status = "NO_TEXT_OBSERVED";
    reasonCode = "OCR_NO_TEXT_OBSERVED";
  }
  ...
  return Object.freeze({
    ...
    evidence_produced: patchCount > 0,
    raw_text_present_count: rawTextPresentCount,
    text_candidate_count: textCandidateCount,
    normalized_field_count: normalizedFields.length,
    normalized_field_observed_count: supportedNormalizedFields.length,
    normalized_field_truncated_count: Math.max(0, supportedNormalizedFields.length - normalizedFields.length),
    normalized_fields: Object.freeze(normalizedFields),
    worker_statuses: Object.freeze(workerStatuses),
    worker_reasons: Object.freeze(workerReasons)
  });
}
...
export async function appendEvidencePatchesToBundle({
  ...
  for (let attempt = 1; attempt <= 4; attempt += 1) {
    ...
    const readResponse = await fetchOcrSupabase(readEndpoint, {
      headers: supabaseHeaders(serviceRoleKey, { prefer: "return=representation" })
    }, { env, fetchImpl });
    if (!readResponse.ok) {
    ...
    const writeResponse = await fetchOcrSupabase(writeEndpoint, {
      method: "PATCH",
      headers: supabaseHeaders(serviceRoleKey, { prefer: "return=representation" }),
      body: JSON.stringify({
...
const maxPersistedOcrEvidenceJobs = 64;

function ocrObservabilityStatusRank(value = "") {
  const status = normalizeText(value).toUpperCase();
  if (["SUCCEEDED", "FAILED", "CANCELLED"].includes(status)) return 3;
  if (status === "RUNNING") return 2;
  if (status === "QUEUED" || status === "RETRYING") return 1;
  return 0;
}

export function mergeOcrExecutionSummary(previous = {}, current = {}) {
  const previousRows = Array.isArray(previous?.evidence_job_observability)
    ? previous.evidence_job_observability
    : [];
  const currentRows = Array.isArray(current?.evidence_job_observability)
    ? current.evidence_job_observability
    : [];
  const byJobId = new Map();
  for (const row of [...previousRows, ...currentRows]) {
    const jobId = normalizeText(row?.job_id);
    if (!jobId) continue;
    const existing = byJobId.get(jobId);
    // Never let a stale queued/running observation erase a durable terminal
    // boundary. At equal/higher status rank the newest dispatch wins.
    if (!existing || ocrObservabilityStatusRank(row.status) >= ocrObservabilityStatusRank(existing.status)) {
      byJobId.set(jobId, { ...row, job_id: jobId });
    }
  }
  const allRows = [...byJobId.values()].sort((left, right) => (
    ocrObservabilityStatusRank(right.status) - ocrObservabilityStatusRank(left.status)
      || left.job_id.localeCompare(right.job_id)
  ));
  const retainedRows = allRows.slice(0, maxPersistedOcrEvidenceJobs)
    .sort((left, right) => left.job_id.localeCompare(right.job_id));
  return {
    ...current,
    aggregate_scope: "MERGED_JOB_HISTORY_BOUNDED",
    evidence_job_observability: retainedRows,
    evidence_job_observability_count: allRows.length,
    evidence_job_observability_retained_count: retainedRows.length,
    evidence_job_observability_truncated_count: Math.max(0, allRows.length - retainedRows.length)
  };
}
...
async function persistOcrExecutionSummary({
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    const current = rows[0];
    const previousExecutionSummary = current.quality_summary?.ocr_stage_execution || {};
    const mergedExecutionSummary = mergeOcrExecutionSummary(previousExecutionSummary, summary);
    const writeEndpoint = new URL(`${url}/rest/v1/preingestion_bundles`);
    writeEndpoint.searchParams.set("bundle_id", `eq.${safeBundleId}`);
    writeEndpoint.searchParams.set("tenant_id", `eq.${safeTenantId}`);
    if (current.updated_at) writeEndpoint.searchParams.set("updated_at", `eq.${current.updated_at}`);
    ...
    const updated = (await readResponseJson(writeResponse)) || [];
    if (!current.updated_at || (Array.isArray(updated) && updated.length === 1)) {
      return { saved: true, attempts: attempt, summary: mergedExecutionSummary };
    }
  }
...
export async function readPreingestionOcrState({
  ...
  // These rows are independent snapshots under the same tenant/bundle scope.
  // Reading them concurrently removes one Supabase RTT from every OCR poll and
  // prevents a slow first read from consuming the second read's timeout budget.
  const [jobsResponse, bundleResponse] = await Promise.all([
    fetchOcrSupabase(jobsEndpoint, {
      headers: supabaseHeaders(serviceRoleKey)
    }, { env, fetchImpl }),
    fetchOcrSupabase(bundleEndpoint, {
      headers: supabaseHeaders(serviceRoleKey)
    }, { env, fetchImpl })
  ]);
  if (!jobsResponse.ok) {
  ...
  const currentVersionPrefix = preingestionOcrJobKeyPrefix({
    ocrWorkerRevision: env.OCR_WORKER_REVISION
  });
  const jobs = currentVersionPrefix
    ? allJobs.filter((job) => normalizeText(job.job_key).startsWith(currentVersionPrefix))
    : [];
  const historicalJobs = allJobs.filter((job) => (
    !currentVersionPrefix || !normalizeText(job.job_key).startsWith(currentVersionPrefix)
  ));
  ...
  return {
    ...
    worker_service_revision: normalizeText(env.OCR_WORKER_REVISION) || null,
...
export async function processQueuedPreingestionOcrJobs({
  ...
  const baseClient = paddleClient || createPaddleOcrClient({ env, fetchImpl });
  const client = createOcrBatchingClient(baseClient, {
    windowMs: Number(env.PREINGESTION_OCR_BATCH_WINDOW_MS || 4),
    maxBatch: Number(env.PREINGESTION_OCR_BATCH_SIZE || 8)
  });
  ...
  const leasePlan = preingestionOcrJobLeasePlan(env);
  const leaseSeconds = leasePlan.lease_seconds;
  ...
  const staleRecovery = await recoverStalePreingestionOcrJobs({
    ...
    requeued: 0,
    finalized: 0,
    reason: normalizeText(error?.message || error).slice(0, 160) || "stale_recovery_failed"
  }));
  ...
  const processJob = async (job) => {
    ...
    if (!capacity.acquired) {
      ...
      // Capacity contention means "not scheduled yet", not "OCR failed".
      // Claiming increments attempts, so refund that increment atomically while
      // returning the leased job to the queue. Provider/worker failures still
      // use transitionOcrJobAfterFailure and consume the bounded retry budget.
      const deferred = await deferPreingestionOcrJobForCapacity({
        jobId: job.job_id,
        tenantId: job.tenant_id,
      ...
      const transition = {
        status: deferred ? "deferred" : "lease_lost",
        retryable: true,
        transition_saved: deferred,
        attempt_refunded: deferred
      };
      return {
        job,
        patches: [],
    ...
    const stageHeartbeat = startListingStageCapacityHeartbeats({
      leases: [
        {
          stageId: stagePlan.stage_id,
          jobId: job.job_id,
          owner: processOwner,
          capacity: stagePlan.global_capacity,
          leaseSeconds: stagePlan.lease_seconds
        },
        ...(capacity.asset_stage_id ? [{
          stageId: capacity.asset_stage_id,
          jobId: job.job_id,
          owner: processOwner,
          capacity: stagePlan.per_asset_capacity,
          leaseSeconds: stagePlan.lease_seconds
        }] : [])
      ],
      env,
    ...
    const jobHeartbeat = startPreingestionOcrJobHeartbeat({
      jobId: job.job_id,
      tenantId: job.tenant_id,
      leaseOwner: job.durable_lease_supported === false ? "" : job.lease_owner,
      leaseSeconds: leasePlan.lease_seconds,
      env,
      fetchImpl
    });
    try {
    ...
    try {
      ...
      const evidenceOutcome = summarizeOcrEvidenceOutcome(verified, patches);
      verificationSummary = {
        request_count: verified.length,
        vision_unit_count: verified.reduce((sum, entry) => (
          sum + Math.max(0, Number(entry.result?.vision_unit_count) || 0)
        ), 0),
        vision_cost_estimate: verified.reduce((sum, entry) => (
          sum + Math.max(0, Number(entry.result?.vision_cost_estimate) || 0)
        ), 0),
        serial_consensus_verified: verified.some((entry) => entry.result?.serial_consensus?.verified === true),
        serial_consensus_conflict: verified.some((entry) => entry.result?.serial_consensus?.conflict === true),
        ...
        evidence_outcome: evidenceOutcome.status,
        evidence_reason_codes: evidenceOutcome.reason_codes,
        evidence_produced: evidenceOutcome.evidence_produced,
        raw_text_present_count: evidenceOutcome.raw_text_present_count,
        normalized_field_count: evidenceOutcome.normalized_field_count,
        normalized_field_observed_count: evidenceOutcome.normalized_field_observed_count,
        normalized_field_truncated_count: evidenceOutcome.normalized_field_truncated_count,
        normalized_fields: evidenceOutcome.normalized_fields,
        worker_statuses: evidenceOutcome.worker_statuses,
        worker_reasons: evidenceOutcome.worker_reasons
      };
    } catch (error) {
      const message = error?.message || String(error);
      const transition = await transitionOcrJobAfterFailure({
        job,
        error: message,
        env,
        fetchImpl
    ...
    return { ...outcome, stage_lane: lane, stage_capacity: capacity };
      ...
      const [stageHeartbeatStats, jobHeartbeatStats] = await Promise.all([
        stageHeartbeat.stop(),
        jobHeartbeat.stop()
      ]);
      Object.assign(capacity, stageHeartbeatStats, {
        job_lease_supported: job.durable_lease_supported !== false,
        job_lease_heartbeat_enabled: jobHeartbeatStats.enabled,
        job_lease_heartbeat_interval_ms: jobHeartbeatStats.interval_ms,
        job_lease_heartbeat_attempts: jobHeartbeatStats.attempts,
        job_lease_heartbeat_renewed: jobHeartbeatStats.renewed,
        job_lease_heartbeat_failures: jobHeartbeatStats.failures,
        job_lease_ownership_lost: jobHeartbeatStats.ownership_lost,
        job_lease_heartbeat_last_error: jobHeartbeatStats.last_error
      });
    }
  };
    ...
    try {
      if (capacity?.acquired && capacity.coordinated) {
        ...
        outcome.stage_capacity = {
          ...
          release_attempts: Math.max(globalRelease.attempts || 0, assetRelease.attempts || 0),
  ...
  if (stagePlan.capacity_control_enabled) {
    ...
    let anchorOutcomes = [];
    let detailOutcomes = [];
    if (localPlan.anchor_concurrency > 0 && localPlan.detail_concurrency > 0) {
      [anchorOutcomes, detailOutcomes] = await Promise.all([
    ...
    } else if (localPlan.anchor_concurrency > 0) {
      // A one-slot per-card plan intentionally gives hard anchors the first
      // wave. Once they finish, every already-claimed detail job must still be
      // processed; dropping a zero-slot lane would strand RUNNING jobs until
      // lease expiry and surface as missing grade/card-code evidence.
      anchorOutcomes = await mapWithConcurrency(anchorJobs, localPlan.anchor_concurrency, runJob);
      detailOutcomes = await mapWithConcurrency(
        detailJobs,
        Math.min(localPlan.capacity || localPlan.per_asset_capacity || 1, stagePlan.detail_concurrency),
        runJob
      );
    } else if (localPlan.detail_concurrency > 0) {
      detailOutcomes = await mapWithConcurrency(detailJobs, localPlan.detail_concurrency, runJob);
      anchorOutcomes = await mapWithConcurrency(
        anchorJobs,
        Math.min(localPlan.capacity || localPlan.per_asset_capacity || 1, stagePlan.anchor_concurrency),
        runJob
      );
    }
  ...
  const claimedJobIds = new Set(jobs.map((job) => normalizeText(job.job_id)).filter(Boolean));
  const outcomeJobIds = new Set(outcomes.map((outcome) => normalizeText(outcome.job?.job_id)).filter(Boolean));
  const firstWaveJobIds = new Set(scheduledFirstWaveJobs.map((job) => normalizeText(job.job_id)).filter(Boolean));
  const unaccountedClaimedJobIds = [...claimedJobIds].filter((jobId) => !outcomeJobIds.has(jobId));
  const duplicateOutcomeCount = Math.max(0, outcomes.length - outcomeJobIds.size);
  const followupOutcomeCount = outcomes.filter((outcome) => (
    !firstWaveJobIds.has(normalizeText(outcome.job?.job_id))
  )).length;
  const zeroSlotLaneRecoveredCount = outcomes.filter((outcome) => (
    (outcome.stage_lane === "anchor" && localPlan.anchor_concurrency === 0)
      || (outcome.stage_lane === "detail" && localPlan.detail_concurrency === 0)
  )).length;
  ...
  const jobObservability = outcomes.map((outcome) => ({
    ...
    raw_text_present_count: Number(outcome.verification_summary?.raw_text_present_count || 0),
    normalized_field_count: Number(outcome.verification_summary?.normalized_field_count || 0),
    normalized_field_observed_count: Number(
      outcome.verification_summary?.normalized_field_observed_count || 0
    ),
    normalized_field_truncated_count: Number(
      outcome.verification_summary?.normalized_field_truncated_count || 0
    ),
    normalized_fields: Array.isArray(outcome.verification_summary?.normalized_fields)
      ? outcome.verification_summary.normalized_fields
      : [],
    evidence_outcome: outcome.verification_summary?.evidence_outcome || null,
    evidence_reason_codes: Array.isArray(outcome.verification_summary?.evidence_reason_codes)
      ? outcome.verification_summary.evidence_reason_codes
      : [],
    evidence_produced: outcome.verification_summary?.evidence_produced === true,
    ...
    stage_capacity_heartbeat_interval_ms: outcome.stage_capacity?.heartbeat_interval_ms ?? null,
    stage_capacity_heartbeat_lease_count: outcome.stage_capacity?.heartbeat_lease_count ?? null,
    stage_capacity_heartbeat_attempts: outcome.stage_capacity?.heartbeat_attempts ?? null,
    stage_capacity_heartbeat_request_count: outcome.stage_capacity?.heartbeat_request_count ?? null,
    stage_capacity_heartbeat_failures: outcome.stage_capacity?.heartbeat_failures ?? null,
    stage_capacity_heartbeat_lost_ownership_count: outcome.stage_capacity?.heartbeat_lost_ownership_count ?? null,
    stage_capacity_heartbeat_last_error: outcome.stage_capacity?.heartbeat_last_error ?? null,
    stage_capacity_release_attempts: outcome.stage_capacity?.release_attempts ?? null,
    stage_capacity_released: outcome.stage_capacity?.released ?? null,
    durable_job_lease_supported: outcome.stage_capacity?.job_lease_supported ?? null,
    durable_job_lease_heartbeat_enabled: outcome.stage_capacity?.job_lease_heartbeat_enabled ?? null,
    durable_job_lease_heartbeat_interval_ms: outcome.stage_capacity?.job_lease_heartbeat_interval_ms ?? null,
    durable_job_lease_heartbeat_attempts: outcome.stage_capacity?.job_lease_heartbeat_attempts ?? null,
    durable_job_lease_heartbeat_renewed: outcome.stage_capacity?.job_lease_heartbeat_renewed ?? null,
    durable_job_lease_heartbeat_failures: outcome.stage_capacity?.job_lease_heartbeat_failures ?? null,
    durable_job_lease_ownership_lost: outcome.stage_capacity?.job_lease_ownership_lost ?? null,
    durable_job_lease_heartbeat_last_error: outcome.stage_capacity?.job_lease_heartbeat_last_error ?? null
  }));
  ...
  const executionSummary = {
    ...
    capacity_heartbeat_job_count: outcomes.filter((outcome) => (
      Number(outcome.stage_capacity?.heartbeat_attempts || 0) > 0
    )).length,
    capacity_heartbeat_attempt_count: outcomes.reduce((sum, outcome) => (
      sum + Number(outcome.stage_capacity?.heartbeat_attempts || 0)
    ), 0),
    capacity_heartbeat_request_count: outcomes.reduce((sum, outcome) => (
      sum + Number(outcome.stage_capacity?.heartbeat_request_count || 0)
    ), 0),
    capacity_heartbeat_failure_count: outcomes.reduce((sum, outcome) => (
      sum + Number(outcome.stage_capacity?.heartbeat_failures || 0)
    ), 0),
    capacity_heartbeat_lost_ownership_count: outcomes.reduce((sum, outcome) => (
      sum + Number(outcome.stage_capacity?.heartbeat_lost_ownership_count || 0)
    ), 0),
    capacity_release_retry_count: outcomes.reduce((sum, outcome) => (
      sum + Math.max(0, Number(outcome.stage_capacity?.release_attempts || 0) - 1)
    ), 0),
    durable_job_lease_supported: jobs.length
      ? jobs.every((job) => job.durable_lease_supported !== false)
      : staleRecovery.supported !== false,
    durable_job_lease_seconds: leasePlan.lease_seconds,
    durable_job_lease_heartbeat_ms: leasePlan.heartbeat_ms,
    durable_job_lease_heartbeat_attempt_count: outcomes.reduce((sum, outcome) => (
      sum + Number(outcome.stage_capacity?.job_lease_heartbeat_attempts || 0)
    ), 0),
    durable_job_lease_heartbeat_failure_count: outcomes.reduce((sum, outcome) => (
      sum + Number(outcome.stage_capacity?.job_lease_heartbeat_failures || 0)
    ), 0),
    durable_job_lease_ownership_lost_count: outcomes.filter((outcome) => (
      outcome.stage_capacity?.job_lease_ownership_lost === true
    )).length,
    stale_lease_inspected_count: Number(staleRecovery.inspected || 0),
    stale_lease_requeued_count: Number(staleRecovery.requeued || 0),
    stale_lease_finalized_count: Number(staleRecovery.finalized || 0),
    stale_lease_race_lost_count: Number(staleRecovery.race_lost || 0),
    retryable_failure_requeued_before_claim_count: Number(recoveredFailures.requeued || 0),
    ...
    followup_job_count: Math.max(0, jobs.length - scheduledFirstWaveJobs.length),
    followup_outcome_count: followupOutcomeCount,
    zero_slot_lane_recovered_count: zeroSlotLaneRecoveredCount,
    outcome_job_count: outcomeJobIds.size,
    duplicate_outcome_count: duplicateOutcomeCount,
    unaccounted_claimed_job_count: unaccountedClaimedJobIds.length,
    all_claimed_jobs_accounted_for: unaccountedClaimedJobIds.length === 0 && duplicateOutcomeCount === 0,
    succeeded,
    failed,
    requeued,
    deferred,
    lease_lost: leaseLost,
    stale_running_recovered: staleRecovery.requeued || staleRecovery.recovered || 0,
    stale_running_failed_final: staleRecovery.finalized || staleRecovery.failed_final || 0,
    ...
    // Persist only the compact evidence boundary, not OCR text or the larger
    // capacity trace. This makes no-patch successes explainable when the
    // pre-L2 probe later reads the immutable bundle snapshot.
    evidence_job_observability: jobObservability.map((row) => ({
      job_id: row.job_id,
      crop_role: row.crop_role,
      status: row.status,
      patch_count: row.patch_count,
      patches_appended: row.patches_appended,
      raw_text_present_count: row.raw_text_present_count,
      text_candidate_count: row.text_candidate_count,
      normalized_field_count: row.normalized_field_count,
      normalized_field_observed_count: row.normalized_field_observed_count,
      normalized_field_truncated_count: row.normalized_field_truncated_count,
  ...
  return {
    ...
    execution_summary: executionSummaryPersistence.summary || executionSummary,
    ...
    deferred,
    lease_lost: leaseLost,
    stale_running_recovered_before_claim: staleRecovery.requeued || staleRecovery.recovered || 0,
    stale_running_failed_final_before_claim: staleRecovery.finalized || staleRecovery.failed_final || 0,
    durable_job_lease_supported: executionSummary.durable_job_lease_supported,
    durable_job_lease_seconds: leasePlan.lease_seconds,
    durable_job_lease_heartbeat_ms: leasePlan.heartbeat_ms,
    stale_lease_recovery: staleRecovery,
