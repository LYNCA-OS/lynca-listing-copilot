...
import { adaptOcrRegionEvidence } from "../evidence/region-evidence-adapter.mjs";
...
import { supabaseRestAdminHeaders as buildSupabaseServiceHeaders } from "../../supabase-service-headers.mjs";
import {
...
function safeOcrErrorCode(error = "") {
  ...
  if (/ocr_worker_unavailable|field_ocr_worker_unavailable|worker_unavailable|vision quota/.test(value)) {
...
function cropLineageForJob(job = {}) {
  const crop = job.payload?.crop && typeof job.payload.crop === "object"
    ? job.payload.crop
    : {};
  const metadata = crop.crop_metadata && typeof crop.crop_metadata === "object"
    ? crop.crop_metadata
    : {};
  return {
    source_image_id: normalizeText(crop.source_image_id || metadata.source_image_id) || null,
    source_side: normalizeText(crop.source_side || metadata.source_side).toLowerCase() || null,
    source_region: normalizeText(crop.source_region || metadata.source_region) || null
  };
}
...
export function bundlePatchesFromOcrResult(ocrResult = {}, job = {}) {
  ...
  const crop = job.payload?.crop || {};
  const shadowEvaluationPayload = job.payload?.evaluation_mode === "SHADOW_EVALUATION_ONLY"
    ? job.payload
    : null;
  const metadata = crop.crop_metadata || {};
  const sourceImageId = crop.source_image_id || metadata.source_image_id || "bundle";
  const cropId = metadata.crop_id || null;
  const regionEvidence = adaptOcrRegionEvidence(ocrResult, {
    imageSha256: job.payload?.source_image_sha256 || metadata.source_content_sha256 || null,
    imageGenerationId: job.payload?.image_generation_id || job.asset_id || null,
    side: crop.source_side || metadata.source_side || null,
    region: crop.source_region || metadata.source_region || null,
    cropId,
    cropRole: crop.role || metadata.crop_role || ocrPatch.crop_type || null,
    cropBox: metadata.pixel_bounds || metadata.normalized_bounds || crop.crop_region || null,
    sensor: "VISION_OCR",
    sensorRevision: "preingestion-ocr-sensor-v1",
    provider: ocrResult.ocr_backend || ocrResult.backend || ocrResult.model_id || "ocr",
    // Transport revision is not the OCR provider/model revision. Preserve an
    // honest UNVERSIONED gap instead of making incomplete lineage look closed.
    providerRevision: ocrResult.provider_revision || null,
    model: ocrResult.model_id || null,
    modelRevision: ocrResult.model_revision || null,
    producer: "preingestion_ocr_worker",
    producerRevision: job.payload?.producer_revision || preingestionOcrJobVersion
  });
  const textCandidates = (ocrResult.text_candidates || []).map((candidate, index) => ({
    value: candidate.text || candidate.normalized_text || "",
    text: candidate.text || candidate.normalized_text || "",
    confidence: candidate.confidence ?? null,
    line_id: candidate.line_id || candidate.lineId || `candidate_${index + 1}`,
    bbox: candidate.bbox || candidate.box || candidate.bounding_box || candidate.boundingBox || null
  })).filter((candidate) => candidate.value);
  const sharedProvenance = {
    generated_by: "preingestion_ocr_worker",
    job_key: job.job_key || null,
    crop_type: ocrPatch.crop_type || crop.role || null,
    source_side: crop.source_side || metadata.source_side || null,
    source_region: crop.source_region || metadata.source_region || null,
    source_object_path: metadata.source_object_path || null,
    image_sha256: job.payload?.source_image_sha256 || metadata.source_content_sha256 || null,
    image_generation_id: job.payload?.image_generation_id || job.asset_id || null,
    ...
    region_evidence_ids: regionEvidence.evidence.map((entry) => entry.evidence_id)
  };
  ...
  for (const [field, evidenceField] of Object.entries(evidence)) {
    ...
    const fieldSource = [
      ...(Array.isArray(evidenceField?.sources) ? evidenceField.sources : []),
      ...(Array.isArray(evidenceField?.candidates)
        ? evidenceField.candidates.flatMap((candidate) => Array.isArray(candidate?.sources) ? candidate.sources : [])
        : [])
    ].find((source) => source && typeof source === "object" && !Array.isArray(source)) || {};
    const sourceTrustTier = Number(fieldSource.trust_tier ?? fieldSource.trustTier);
    const patch = normalizeEvidencePatch({
      ...
      provenance: {
        ...sharedProvenance,
        source_trust_tier: Number.isInteger(sourceTrustTier) && sourceTrustTier >= 1 && sourceTrustTier <= 10
          ? sourceTrustTier
          : null,
        source_inference_method: fieldSource.source_inference_method || fieldSource.sourceInferenceMethod || null,
        ...
        // The evaluation scheduler versions the exact image/crop generation.
        // Only its explicit shadow-only jobs may carry these tokens forward;
        // ordinary OCR patches retain their byte-for-byte production contract.
        ...(shadowEvaluationPayload ? {
          evaluation_mode: shadowEvaluationPayload.evaluation_mode,
          bundle_generation_fingerprint: shadowEvaluationPayload.bundle_generation_fingerprint,
          detail_revision: shadowEvaluationPayload.detail_revision
        } : {})
      }
    });
    if (patch) patches.push(patch);
  }
  const tracePatch = normalizeEvidencePatch({
    field: "region_observation",
    value: regionEvidence.evidence.some((entry) => entry.state === "VALUE") ? "VALUE" : regionEvidence.evidence[0]?.state || "UNKNOWN",
    raw_text: ocrPatch.raw_text || ocrResult.raw_text || null,
    text_candidates: textCandidates,
  ...
  return tracePatch ? [...patches, tracePatch] : patches;
}
