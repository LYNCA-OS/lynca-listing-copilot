...
export function createOcrBatchingClient(client, { windowMs = 4, maxBatch = 8 } = {}) {
  ...
  const flush = async () => {
    ...
    timer = null;
    const requestLimit = Math.max(1, Math.min(8, Number(maxBatch) || 8));
    // The Vision worker expands serial verification to four image units and
    // card-code verification to two. Pack by expanded cost as well as request
    // count so detail-mode Oracle runs cannot cross the 16-image API limit.
    let unitBudget = 16;
    let batchSize = 0;
    for (const entry of pending.slice(0, requestLimit)) {
      const cropType = normalizeText(entry.input?.crop_type).toLowerCase();
      const units = ["serial_number", "serial_crop"].includes(cropType)
        ? 4
        : ["collector_number", "card_code_crop", "collector_number_crop"].includes(cropType)
          ? 2
          : 1;
      if (batchSize > 0 && units > unitBudget) break;
      unitBudget -= units;
      batchSize += 1;
    }
    const batch = pending.splice(0, Math.max(1, batchSize));
    try {
...
export async function claimQueuedPreingestionOcrJobs({
  ...
  listedRows = (Array.isArray(listedPayload) ? listedPayload : []).filter((job) => {
    ...
    if (!normalizeText(job?.job_key).startsWith(`ocr:${preingestionOcrJobVersion}:`)) return false;
...
function sourceSideForCrop(crop = {}) {
  const metadata = crop.crop_metadata || {};
  const explicit = normalizeText(metadata.source_side || metadata.sourceSide).toLowerCase();
  if (["front", "back"].includes(explicit)) return explicit;
  const imageId = normalizeText(crop.source_image_id || metadata.source_image_id).toLowerCase();
  return ["front", "back"].includes(imageId) ? imageId : null;
}
...
export function ocrRequestForPreingestionJob(job = {}, { imageUrl = "" } = {}) {
  ...
  return {
    ...
    metadata: {
      ...
      source_side: sourceSideForCrop(crop),
      asset_id: job.asset_id || metadata.asset_id || null,
      bundle_id: job.bundle_id || null,
      // Slab labels and card codes earn one in-batch full-image pass. Serial
      // evidence stays tied to explicit regions because broad serial scans are
      // materially less precise.
      inline_full_image_fallback: ["grade_label_crop", "card_code_crop"].includes(crop.role || metadata.crop_role),
      source_width: sourceDimensionsFromCrop(job)?.width || null,
      source_height: sourceDimensionsFromCrop(job)?.height || null,
...
export function bundlePatchesFromOcrResult(ocrResult = {}, job = {}) {
  ...
  for (const [field, evidenceField] of Object.entries(evidence)) {
    ...
    const patch = normalizeEvidencePatch({
      ...
      text_candidates: (ocrResult.text_candidates || []).map((candidate) => ({
        ...
        ocr_pass: normalizeText(candidate.ocr_pass || candidate.ocrPass) || null,
        box: candidate.box || candidate.bbox || null
      ...
      provenance: {
        ...
        source_region: crop.source_region || metadata.source_region || null,
        source_side: sourceSideForCrop(crop),
        direct_extraction_method: ocrResult.normalized_field_sources?.[field] || null,
  ...
  if (job.payload?.persist_raw_ocr_observation === true) {
    const rawObservation = normalizeText(ocrPatch.raw_text || ocrResult.raw_text).slice(0, 1_200);
    const rawPatch = rawObservation ? normalizeEvidencePatch({
      field: "ocr_raw_observation",
      value: rawObservation,
      raw_text: rawObservation,
      text_candidates: (ocrResult.text_candidates || []).slice(0, 24).map((candidate) => ({
        value: normalizeText(candidate.text || candidate.normalized_text).slice(0, 240),
        confidence: candidate.confidence ?? null,
        ocr_pass: normalizeText(candidate.ocr_pass || candidate.ocrPass) || null,
      ...
      source_image_id: sourceImageId,
      crop_id: cropId,
      confidence: ocrResult.confidence ?? ocrPatch.confidence ?? null,
      provenance: {
        generated_by: "preingestion_ocr_worker",
        audit_only: true,
        job_key: job.job_key || null,
    ...
    if (rawPatch) patches.push(rawPatch);
  }
  return patches;
}
...
export async function readPreingestionOcrState({
  ...
  const rawObservations = patches
    .filter((patch) => normalizeText(patch.field) === "ocr_raw_observation")
    .map((patch) => ({
      source_image_id: patch.source_image_id || null,
      crop_type: patch.provenance?.crop_type || null,
      source_region: patch.provenance?.source_region || null,
      raw_text: normalizeText(patch.raw_text || patch.value).slice(0, 1_200),
      confidence: Number.isFinite(Number(patch.confidence)) ? Number(patch.confidence) : null,
      model_id: normalizeText(patch.provenance?.model_id) || null,
      model_revision: normalizeText(patch.provenance?.model_revision) || null,
  ...
  return {
    ...
    raw_ocr_observations: rawObservations,
...
export async function waitForPreingestionOcrEvidence({
  ...
  const sweepPromise = triggerSweep
    ? processQueuedPreingestionOcrJobs({
      ...
      assetId,
      bundleId,
      limit: requireTerminal ? 32 : 6,
      anchorOnly: requireTerminal !== true,
      drainAsset: requireTerminal === true,
      targetFields,
      env,
      fetchImpl
    }).catch((error) => ({
  ...
  while (Date.now() - startedAt < Math.max(500, Number(timeoutMs) || 30_000)) {
    ...
    if (requireTerminal !== true && targetedState.target_fields_settled) {
      return {
        ...lastState,
        ...targetedState,
    ...
    if (requireTerminal !== true && lastState.verified_serial_ready && slabAuthoritySettled) {
      return {
        ...lastState,
    ...
    if (requireTerminal !== true && lastState.critical_fields_settled) {
      return {
        ...lastState,
