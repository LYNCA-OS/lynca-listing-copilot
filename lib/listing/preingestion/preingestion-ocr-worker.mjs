...
export async function waitForPreingestionOcrEvidence({
  ...
  while (Date.now() - startedAt < Math.max(500, Number(timeoutMs) || 30_000)) {
    try {
      ...
      stateReadErrorCount += 1;
      lastStateReadError = normalizeText(error?.message || error).slice(0, 180);
      publishState({
        ...(lastState || {}),
        status: "STATE_READ_RETRY",
        terminal: false,
        state_reads: stateReads,
        state_read_error_count: stateReadErrorCount,
        last_state_read_error: lastStateReadError
      });
