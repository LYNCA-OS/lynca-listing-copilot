...
export function fairOcrClaimOrder(jobs = [], { limit = jobs.length, jobsPerAsset = 2 } = {}) {
  ...
  const queues = [...groups.values()].map((rows) => rows.sort((left, right) => (
...
export async function claimQueuedPreingestionOcrJobs({
  ...
  const candidateLimit = scopedToAsset ? requestedLimit : Math.min(128, requestedLimit * 4);
  const listEndpoint = new URL(`${url}/rest/v1/preingestion_jobs`);
  listEndpoint.searchParams.set("select", "job_id,job_key,asset_id,bundle_id,payload,attempts,priority,created_at");
  ...
  // two-jobs-per-card fair order locally. This aligns OCR with provider queue
  // order while preventing one card or one crop role from starving the rest.
  ...
  const rows = fairOcrClaimOrder(listedRows, { limit: requestedLimit, jobsPerAsset: 2 });
...
function resultSuggestsSlabGradeContext(ocrResult = {}) {
  const evidence = ocrResult.evidence_patch?.evidence || {};
  if (evidence.grade_company?.value
    || evidence.card_grade?.value
    || evidence.grade?.value
    || evidence.auto_grade?.value
    || evidence.cert_number?.value) return true;
  const text = normalizeText([
    ocrResult.raw_text,
    ...(Array.isArray(ocrResult.text_candidates)
      ? ocrResult.text_candidates.map((candidate) => candidate?.text || candidate?.normalized_text || candidate?.value)
      : [])
  ].filter(Boolean).join(" "));
  return /\b(?:PSA(?:\s*\/\s*DNA)?|BGS|BECKETT|CGC|CSG|SGC|TAG)\b/i.test(text);
}
...
async function verifyPreingestionJob(client, job, imageUrl) {
  ...
  if (request.crop_type !== "grade_label_crop"
    ...
    || (!gradeCropSourceLooksLikeSlab(job) && !resultSuggestsSlabGradeContext(primary))) {
    return [{ result: primary, job }];
  }

  // Slab labels often sit outside the generic top-band crop after marketplace
  // framing. Only slab-like geometry pays this second scan; raw cards keep the
  // one-request path.
...
export async function processQueuedPreingestionOcrJobs({
  ...
  const localPlan = scopedToAsset
    ...
    : {
      ...
      anchor_concurrency: Math.min(anchorJobCount, stagePlan.anchor_concurrency),
      detail_concurrency: Math.min(detailJobCount, stagePlan.detail_concurrency),
      local_concurrency: Math.min(
        stagePlan.global_capacity,
        Math.min(anchorJobCount, stagePlan.anchor_concurrency)
          + Math.min(detailJobCount, stagePlan.detail_concurrency)
      )
    };
