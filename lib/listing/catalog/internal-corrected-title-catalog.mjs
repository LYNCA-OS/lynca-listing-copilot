...
function splitFields(parsed = {}) {
  ...
  const identityFields = {
    sport: /basketball|nba/i.test([parsed.category, parsed.product, parsed.set].filter(Boolean).join(" ")) ? "basketball" : null,
    league: /nba/i.test([parsed.category, parsed.product, parsed.set].filter(Boolean).join(" ")) ? "NBA" : null,
    season_year: parsed.year || null,
    manufacturer: parsed.manufacturer || null,
    brand: parsed.brand || parsed.manufacturer || null,
    product: parsed.product || null,
    set_or_insert: parsed.set || parsed.insert || null,
