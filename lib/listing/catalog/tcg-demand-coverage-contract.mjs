...
function coverageState(row = {}) {
  ...
  if (row.official_source_count === 0) return "SOURCE_MISSING";
  if (row.official_decision_active_card_count === 0) return "SOURCE_NOT_DECISION_ACTIVE";
  if (row.measurable_card_code_anchor_count === 0) return "NO_MEASURABLE_CARD_CODE_ANCHORS";
  if (row.card_code_match_count === row.measurable_card_code_anchor_count) return "CARD_CODE_ANCHORS_COVERED";
  if (row.card_code_match_count > 0) return "PARTIAL_CARD_CODE_COVERAGE";
  return "CARD_CODE_ANCHORS_UNCOVERED";
}
...
export function buildTcgDemandCoverage({ sources = [], cards = [], generatedAt = new Date().toISOString() } = {}) {
  ...
  const rows = FAMILY_DEFINITIONS.map((definition) => {
    ...
    return { ...row, coverage_state: coverageState(row) };
  });
  ...
  return {
    ...
    summary: {
      ...
      source_active_demanded_family_count: rows.filter((row) => row.reviewed_demand_count > 0 && row.source_operational_stage === "DECISION_ACTIVE").length,
      missing_or_inactive_source_demand_count: rows
