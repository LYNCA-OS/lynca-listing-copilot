 ...
 * product on its own.
 */
// The set field frequently holds a product name rather than a set name.
//
// Counted over the 72 highest-volume set values in production, 2,372 cards:
// 74% of set lookups miss, and the largest misses are "prizm" (190 cards),
// "topps chrome" (177), "chrome" (81), "bowman chrome" (79), "flawless" (70).
// None of these are sets. They are product lines sitting in the set field, so
// harvesting more sets cannot fix them -- the lookup is asking the wrong table.
//
// Reading the set field as a product name recovers 20% of those cards on its
// own. It is deliberately strict: an exact product name, or a product whose
// name ends with this phrase ("flawless" -> "Panini Flawless"), and only when
// exactly one product survives. "prizm" matches three products and stays
// UNKNOWN, because a confident wrong product line is worse than a gap.
function productFromSetField(set, claim, model) {
  const products = Object.keys(model.product_years || {});
  if (!products.length) return result(outcomes.UNKNOWN, { reason: "set_not_in_model" });

  const candidates = products.filter((product) => {
    const name = norm(product);
    return name === set || name.endsWith(` ${set}`);
  });
  if (!candidates.length) return result(outcomes.UNKNOWN, { reason: "set_not_in_model" });
  if (candidates.length === 1) {
    return result(outcomes.VALUE, {
      value: candidates[0],
      candidates,
      reason: "set_field_holds_a_product_name"
    });
  }

  const year = seasonStartYear(claim.year ?? claim.season_year);
  if (year) {
    const forYear = candidates.filter((product) => (model.product_years[product] || []).includes(year));
    if (forYear.length === 1) {
      return result(outcomes.VALUE, {
        value: forYear[0],
        candidates: forYear,
        reason: "set_field_product_narrowed_by_year"
      });
    }
    if (forYear.length > 1) {
      return result(outcomes.UNKNOWN, { candidates: forYear, reason: "several_products_share_that_name" });
    }
  }
  return result(outcomes.UNKNOWN, { candidates, reason: "several_products_share_that_name" });
}
...
export function enumerateProduct(claim = {}, model = null) {
  ...
  if (!entries || !entries.length) return productFromSetField(set, claim, model);
