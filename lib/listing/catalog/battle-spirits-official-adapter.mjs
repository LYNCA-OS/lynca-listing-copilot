export const battleSpiritsParserRevision = "battle-spirits-detail-v2";
...
function selectedCategoryProduct(listingHtml = "", sourceUrl = "") {
  let categoryId = "";
  try {
    categoryId = normalizeText(new URL(sourceUrl).searchParams.get("category"));
  } catch {
    return "";
  }
  if (!/^\d+$/.test(categoryId)) return "";
  for (const match of String(listingHtml || "").matchAll(/<option\b([^>]*)>([\s\S]*?)<\/option>/gi)) {
    const attributes = match[1];
    const value = attributes.match(/\bvalue=["']([^"']*)["']/i)?.[1] || "";
    if (value !== categoryId || !/\bselected\b/i.test(attributes)) continue;
    return normalizeText(decodeHtmlEntities(stripHtml(match[2])));
  }
  return "";
}
...
export function parseBattleSpiritsDetailBundle(rawText = "", { sourceUrl = "" } = {}) {
  ...
  const listingProduct = selectedCategoryProduct(bundle.listing_html, sourceUrl);
  const rows = [];
  ...
  for (const [index, detail] of bundle.detail_pages.entries()) {
    ...
    const detailProduct = normalizeText(decodeHtmlEntities(stripHtml(
    ...
    if (detailProduct && listingProduct && detailProduct !== listingProduct) {
      throw new Error(`battle_spirits_product_mapping_mismatch:${detailCardNumber || index + 1}`);
    }
    const product = detailProduct || listingProduct;
