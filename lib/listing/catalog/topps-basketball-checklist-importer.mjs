...
export const defaultOfficialChecklistIndexUrls = Object.freeze({
  ...
  panini: "https://www.paniniamerica.net/checklists.html",
  upper_deck: "https://www.upperdeck.com/Collectors-Zone/trading-card-checklists.aspx",
...
export function sourceTypeFromOfficialChecklistProvider(provider = "topps") {
  ...
  if (normalized === "leaf" || normalized === "leaf_trading_cards") return catalogSourceTypes.LEAF_OFFICIAL_CHECKLIST;
...
function providerFromSourceType(sourceType = "") {
  ...
  if (normalized === catalogSourceTypes.LEAF_OFFICIAL_CHECKLIST) return "Leaf";
  return "Topps";
}
...
export async function buildOfficialChecklistImport({
  ...
  const resolvedSourceType = sourceType || sourceTypeFromOfficialChecklistProvider(provider);
  if (!officialChecklistCatalogSourceTypes.includes(resolvedSourceType)) {
  ...
  for (const link of links) {
    ...
    const rows = parseOfficialChecklistText(raw, {
