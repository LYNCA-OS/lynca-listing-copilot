...
} from "./catalog-admission-gate.mjs";
...
export const officialChecklistParserVersion = "official-checklist-v5-admission-normalized";
export const upperDeckEpackChecklistParserVersion = "upper-deck-epack-checklist-v1";
export const legacyXlsChecklistParserVersion = `${officialChecklistParserVersion}:legacy-xls-v2`;
...
function delimitedTextToChecklistText(value = "", delimiter = ",") {
  return parseDelimitedText(value, delimiter)
    .map((row) => row.join("\t"))
    .join("\n");
}

function absoluteUrl(href = "", baseUrl = defaultToppsChecklistIndexUrl) {
  const candidate = decodeXml(String(href || "").trim());
...
function normalizedProductLabel(value = "") {
  return normalizeText(value)
    .toLowerCase()
    .replace(/\bformula\s+(?:one|1)\b/g, "f1")
    .replace(/\bchecklist\b/g, " ")
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
}

function isProductDocumentTitleLine(line = "", product = {}) {
  const match = normalizeText(line).match(/^(\d{4})\s+(.+)$/);
  if (!match || match[1] !== normalizeText(product.year)) return false;
  const observedLabel = normalizedProductLabel(match[2]);
  const expectedLabel = normalizedProductLabel(product.product);
  return Boolean(observedLabel && expectedLabel && observedLabel === expectedLabel);
}
...
export function extractXlsxText(buffer) {
  ...
  for (const sheetName of sheetNames) {
    ...
    for (const rowMatch of xml.matchAll(/<row\b[^>]*>([\s\S]*?)<\/row>/gi)) {
      ...
      const normalizedCells = cells.map((value) => normalizeText(value));
      while (normalizedCells.length && !normalizedCells.at(-1)) normalizedCells.pop();
...
function parseTabularOfficialChecklistRows(lines = [], {
  product,
  sourceName = "",
  sourceUrl = ""
} = {}) {
  const structuredHeaderIndex = lines.findIndex((line) => {
    const headers = line.split("\t").map((value) => normalizeText(value).toLowerCase());
    return ["year", "program", "card set", "athlete", "card number"].every((field) => headers.includes(field));
  });
  if (structuredHeaderIndex >= 0) {
    const headers = lines[structuredHeaderIndex].split("\t").map((value) => normalizeText(value).toLowerCase());
    const indexOf = (...names) => names.map((name) => headers.indexOf(name)).find((index) => index >= 0) ?? -1;
    const indexes = {
      sport: indexOf("sport", "category"),
      year: indexOf("year", "season"),
      brand: indexOf("brand", "manufacturer"),
      program: indexOf("program", "product"),
      set: indexOf("card set", "set", "subset"),
      athlete: indexOf("athlete", "player", "name", "subject"),
      team: indexOf("team"),
      cardNumber: indexOf("card number", "number", "checklist"),
      sequence: indexOf("sequence", "print run", "serial denominator")
    };
    const rows = [];
    for (let index = structuredHeaderIndex + 1; index < lines.length; index += 1) {
      const cells = lines[index].split("\t").map(normalizeText);
      const valueAt = (cellIndex) => cellIndex >= 0 ? cells[cellIndex] || "" : "";
      const subject = valueAt(indexes.athlete);
      const cardNumber = valueAt(indexes.cardNumber).replace(/^#\s*/, "").toUpperCase();
      if (!subject || !cardNumber || !/\p{L}/u.test(subject)) continue;
      const brand = valueAt(indexes.brand) || product.manufacturer;
      const program = valueAt(indexes.program) || product.product;
      const productName = normalizeText(new RegExp(`^${brand.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "i").test(program)
        ? program
        : `${brand} ${program}`);
      const setName = valueAt(indexes.set) || null;
      const section = sectionInfo(setName || "");
      const serialDenominator = valueAt(indexes.sequence).match(/^\d{1,4}$/)?.[0] || section.serial_denominator || null;
      const observableComponents = [...new Set([
        ...section.observable_components,
        /\bRC\b|\bRookie\b/i.test(setName || "") ? "rc" : ""
      ].filter(Boolean))];
      const year = valueAt(indexes.year) || product.year;
      const sport = normalizeText(valueAt(indexes.sport)).toLowerCase() || product.category;
      rows.push({
        import_status: catalogImportStatuses.OFFICIAL_CHECKLIST_CANDIDATE,
        source_row_key: `${sourceUrl || sourceName || "official"}:${index + 1}`,
        canonical_title: normalizeText([year, productName, setName, subject, `#${cardNumber}`].filter(Boolean).join(" ")),
        identity_fields: {
          sport,
          category: sport,
          season_year: year || null,
          ...
          players: [subject],
          team: valueAt(indexes.team) || null,
          card_number: cardNumber,
          checklist_code: /[A-Z]/i.test(cardNumber) || cardNumber.includes("-") ? cardNumber : null,
        ...
        field_statuses: {
          ...
          season_year: year ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : catalogFieldStatuses.OFFICIAL_PARSE_REVIEW_REQUIRED,
          manufacturer: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
          product: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
          set_or_insert: setName ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined,
          players: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
          team: valueAt(indexes.team) ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined,
          card_number: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
          checklist_code: /[A-Z]/i.test(cardNumber) || cardNumber.includes("-") ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined,
        ...
        parse_confidence: year && productName && setName ? 0.92 : 0.72,
        review_notes: null
      });
    }
    return rows;
  ...
  const headerIndex = lines.findIndex((line) => {
    const headers = line.split("\t").map((value) => normalizeText(value).toLowerCase());
    return ["set", "subset", "name", "checklist"].every((field) => headers.includes(field));
  });
  if (headerIndex < 0) return [];

  const headers = lines[headerIndex].split("\t").map((value) => normalizeText(value).toLowerCase());
  const indexOf = (name) => headers.indexOf(name);
  const setIndex = indexOf("set");
  const subsetIndex = indexOf("subset");
  const nameIndex = indexOf("name");
  const checklistIndex = indexOf("checklist");
  const foilIndex = indexOf("foil");
  const rows = [];

  for (let index = headerIndex + 1; index < lines.length; index += 1) {
    const cells = lines[index].split("\t").map(normalizeText);
    const setName = cells[setIndex] || "";
    const subset = cells[subsetIndex] || "";
    const subject = cells[nameIndex] || "";
    const checklistCode = (cells[checklistIndex] || "").toUpperCase();
    if (!setName || !subject || !checklistCode || !/\p{L}/u.test(subject)) continue;

    const foilFamily = foilIndex >= 0
      ? normalizeText((cells[foilIndex] || "").replace(/^\d+\s*[-:]\s*/, " "))
      : "";
    const observableComponents = [
      /\bauto(?:graph)?|signature\b/i.test(subset) ? "auto" : "",
      /\bpatch\b/i.test(subset) ? "patch" : "",
      /\b(?:relic|memorabilia|jersey)\b/i.test(subset) ? "relic" : ""
    ].filter(Boolean);
    const officialCardType = /\bauto(?:graph)?|signature\b/i.test(subset)
      ? "Autograph"
      : /\b(?:patch|relic|memorabilia|jersey)\b/i.test(subset)
        ? "Relic"
        : null;
    rows.push({
      import_status: catalogImportStatuses.OFFICIAL_CHECKLIST_CANDIDATE,
      source_row_key: `${sourceUrl || sourceName || "official"}:${index + 1}`,
      canonical_title: normalizeText([
        product.year,
      ...
      identity_fields: {
        ...
        set_or_insert: subset || null,
        set_type: subset ? "insert" : "base",
        players: [subject],
        team: null,
        card_number: checklistCode,
        checklist_code: checklistCode,
        official_card_type: officialCardType,
        // The Foil column names a design family. Color columns describe legal
        // instance variants, so this must not be promoted to exact parallel.
        parallel_name: foilFamily || null,
        parallel_exact: null,
      ...
      field_statuses: {
        sport: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        season_year: product.year
          ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST
          : catalogFieldStatuses.OFFICIAL_PARSE_REVIEW_REQUIRED,
        product: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        set_or_insert: subset ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined,
        players: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        card_number: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        checklist_code: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        official_card_type: officialCardType ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined,
        parallel_name: foilFamily ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined,
        observable_components: observableComponents.length
          ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST
          : undefined
      },
...
function parseToppsUfcSparseWorkbookRows(lines = [], {
  product,
  sourceName = "",
  sourceUrl = ""
} = {}) {
  if (product.category !== "mma" || !/\bUFC\b/i.test(product.product || sourceName)) return [];
  const cardLines = lines.filter((line) => {
    const cells = line.split("\t").map(normalizeText);
    return cells.length >= 2 && /^(?:\d{1,4}|[A-Z0-9]+(?:-[A-Z0-9]+)+)$/i.test(cells[0]) && /\p{L}/u.test(cells[1] || "");
  });
  if (cardLines.length < 50) return [];

  let currentFamily = "BASE";
  let currentSection = "";
  ...
  lines.forEach((line, index) => {
    ...
    if (nonEmpty.length === 1) {
      ...
      if (/^BASE$/i.test(heading)) {
        currentFamily = "BASE";
        currentSection = "";
      } else if (/^AUTOGRAPH RELIC$/i.test(heading)) {
        currentFamily = "AUTOGRAPH RELIC";
        currentSection = "";
      } else if (/^AUTOGRAPH$/i.test(heading)) {
        currentFamily = "AUTOGRAPH";
        currentSection = "";
      } else if (!/^Checklists provided by Topps\b/i.test(heading)) {
        currentSection = heading;
      }
      return;
    }

    const cardNumber = normalizeText(cells[0]).replace(/^#\s*/, "").toUpperCase();
    const subject = normalizeText(cells[1]);
    if (!/^(?:\d{1,4}|[A-Z0-9]+(?:-[A-Z0-9]+)+)$/i.test(cardNumber) || !/\p{L}/u.test(subject)) return;
    const rookie = cells.slice(2).some((value) => /^(?:RC|Rookie)$/i.test(value));
    const isBase = currentFamily === "BASE" && /^BASE CARDS(?:\s+[IVX]+)?$/i.test(currentSection);
    const setOrInsert = isBase ? null : currentSection || null;
    const parsedSection = sectionInfo(currentSection);
    const autographFamily = currentFamily === "AUTOGRAPH" || currentFamily === "AUTOGRAPH RELIC";
    const section = autographFamily
      ? {
          ...parsedSection,
          set_type: "autograph_set",
          official_card_type: "Autograph",
          observable_components: [...new Set([
            ...parsedSection.observable_components,
            "auto",
            currentFamily === "AUTOGRAPH RELIC" ? "relic" : ""
          ].filter(Boolean))]
        }
      : parsedSection;
    const key = `${currentFamily}\u0000${setOrInsert || ""}\u0000${cardNumber}`;
    const existing = grouped.get(key);
    if (existing) {
      if (!existing.players.includes(subject)) existing.players.push(subject);
  ...
  return [...grouped.values()].map((entry) => {
    const checklistCode = /[A-Z]/i.test(entry.cardNumber) || entry.cardNumber.includes("-")
      ? entry.cardNumber
      : null;
    const observableComponents = [...new Set([
      ...entry.section.observable_components,
      entry.rookie ? "rc" : ""
    ].filter(Boolean))];
    return {
      import_status: catalogImportStatuses.OFFICIAL_CHECKLIST_CANDIDATE,
      source_row_key: `${sourceUrl || sourceName || "topps-ufc"}:${entry.firstIndex + 1}${entry.lastIndex === entry.firstIndex ? "" : `-${entry.lastIndex + 1}`}`,
      canonical_title: normalizeText([
        product.year,
        product.product,
        entry.setOrInsert,
      ...
      identity_fields: {
        ...
        set_or_insert: entry.setOrInsert,
        set_type: entry.setOrInsert ? entry.section.set_type : "base",
        players: entry.players,
        team: null,
        card_number: entry.cardNumber,
        checklist_code: checklistCode,
        official_card_type: entry.setOrInsert ? entry.section.official_card_type : "Base",
        parallel_name: entry.section.parallel_name,
        parallel_exact: entry.section.parallel_exact,
        serial_denominator: entry.section.serial_denominator,
        observable_components: observableComponents
      },
      physical_instance_fields: {},
      field_statuses: {
        sport: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        season_year: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        manufacturer: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        product: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        set_or_insert: entry.setOrInsert ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined,
        set_type: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        players: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        card_number: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
        checklist_code: checklistCode ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined,
        official_card_type: (!entry.setOrInsert || entry.section.official_card_type)
          ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST
          : undefined,
        observable_components: observableComponents.length ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined
      },
...
function checklistPayloadType(payload, { sourceUrl = "", contentType = "" } = {}) {
  ...
  const legacyXlsMagic = buffer.subarray(0, 8).toString("hex") === "d0cf11e0a1b11ae1";
  if (magic === "%PDF-") return "pdf";
  if (zipMagic) return "xlsx";
  if (legacyXlsMagic) return "legacy_xls";
  if (type.includes("application/pdf")) return "pdf";
  if (type.includes("application/vnd.ms-excel")) return "legacy_xls";
  if (type.includes("spreadsheet")) return "xlsx";
  if (pathname.endsWith(".pdf")) return "pdf";
  if (pathname.endsWith(".xls")) return "legacy_xls";
  if (pathname.endsWith(".xlsx")) return "xlsx";
  if (type.includes("text/csv") || pathname.endsWith(".csv")) return "csv";
  const preview = buffer.subarray(0, Math.min(buffer.length, 4096)).toString("utf8");
  if (type.includes("application/json") || /^[\s\r\n]*[\[{]/.test(preview)) return "json";
  if (type.includes("html") || /<html|<table|<tr|<li/i.test(preview)) return "html";
  return "plain_text";
}

function upperDeckProductFields(value = "") {
  const sourceProduct = normalizeText(value);
  const seasonYear = sourceProduct.match(/\b(20\d{2}-\d{2}|20\d{2})\b/)?.[1] || null;
  const productName = normalizeText(sourceProduct.replace(/\b20\d{2}(?:-\d{2})?\b/, ""));
  return {
    season_year: seasonYear,
    product: normalizeText(/^Upper Deck\b/i.test(productName) ? productName : `Upper Deck ${productName}`)
  };
}

export function parseUpperDeckEpackChecklistJson(rawText = "", {
  sourceName = "",
  sourceUrl = ""
} = {}) {
  let payload;
  ...
  const rows = [];
  for (const [groupName, entries] of Object.entries(payload)) {
    if (!Array.isArray(entries)) continue;
    for (const entry of entries) {
      const subject = normalizeText(entry?.PlayerName);
      const cardNumber = normalizeText(entry?.CardNumber).replace(/^#\s*/, "").toUpperCase();
      const product = upperDeckProductFields(entry?.EpackProductName || sourceName);
      const insertName = normalizeText(entry?.InsertName || groupName);
      const productWithoutManufacturer = normalizeText(product.product.replace(/^Upper Deck\s+/i, ""));
      const setOrInsert = normalizeText(insertName
        .replace(new RegExp(`^${String(product.season_year || "").replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*`, "i"), "")
        .replace(new RegExp(`^(?:Upper Deck\\s+)?${productWithoutManufacturer.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*`, "i"), "")) || insertName;
      if (!subject || !cardNumber || !product.season_year || !product.product) continue;

      const parallelExact = normalizeText(setOrInsert.match(/^(.+?)\s+(?:Auto\s+)?Parallel\b/i)?.[1] || "") || null;
      const observableComponents = [...new Set([
        /\bauto(?:graph)?|signature\b/i.test(setOrInsert) ? "auto" : "",
        /\bpatch\b/i.test(setOrInsert) ? "patch" : "",
        /\b(?:relic|memorabilia|jersey)\b/i.test(setOrInsert) ? "relic" : "",
        /\brookie|rookies|\bRC\b/i.test(setOrInsert) ? "rc" : ""
      ].filter(Boolean))];
      const templateId = normalizeText(entry?.CardTemplateID);
      const checklistCode = /[A-Z]/i.test(cardNumber) || cardNumber.includes("-") ? cardNumber : null;
      rows.push({
        import_status: catalogImportStatuses.OFFICIAL_CHECKLIST_CANDIDATE,
        source_row_key: templateId || `${sourceUrl || sourceName || "upper-deck"}:${rows.length + 1}`,
        canonical_title: normalizeText([
          product.season_year,
          product.product,
          setOrInsert,
        ...
        identity_fields: {
          ...
          set_type: /\bBase Set\b/i.test(setOrInsert) ? "base" : "insert",
          players: [subject],
          team: null,
          card_number: cardNumber,
          checklist_code: checklistCode,
        ...
        field_statuses: {
          ...
          manufacturer: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
          product: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
          set_or_insert: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
          set_type: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
          players: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
          card_number: catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST,
          checklist_code: checklistCode ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined,
          parallel_exact: parallelExact ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined,
          observable_components: observableComponents.length ? catalogFieldStatuses.AUTO_PARSED_FROM_OFFICIAL_CHECKLIST : undefined
        },
...
export async function extractOfficialChecklistPayload(payload, options = {}) {
  ...
  } else if (extractionMethod === "legacy_xls") {
    const { extractLegacyXlsText } = await import("./legacy-xls-text-extractor.mjs");
    text = extractLegacyXlsText(buffer);
  } else if (extractionMethod === "csv") {
    text = delimitedTextToChecklistText(buffer.toString("utf8"));
  } else {
...
export function parseOfficialChecklistText(rawText = "", {
  ...
  const tabularRows = parseTabularOfficialChecklistRows(lines, {
    product,
    sourceName,
    sourceUrl
  });
  ...
  const ufcWorkbookRows = parseToppsUfcSparseWorkbookRows(lines, {
    product,
    sourceName,
    sourceUrl
  });
  if (ufcWorkbookRows.length) return ufcWorkbookRows;

  const rows = [];
  let currentSection = "";
  lines.forEach((line, index) => {
    if (isProductDocumentTitleLine(line, product)) return;
...
export async function buildOfficialChecklistImport({
  ...
  for (const link of links) {
    ...
    const source = {
      ...
      parser_version: provider === "upper_deck"
        ? upperDeckEpackChecklistParserVersion
        : extracted.extraction_method === "legacy_xls"
    ...
    const parsedRows = provider === "upper_deck" && extracted.extraction_method === "json"
      ? parseUpperDeckEpackChecklistJson(raw, metadata)
      : isOfficialReleaseCatalogSourceType(source.source_type)
      ? parseOfficialReleaseMetadata(raw, metadata)
      : parseOfficialChecklistText(raw, metadata);
    const admissionNormalization = normalizeOfficialCatalogRows(parsedRows);
    const rows = admissionNormalization.rows;
    const admissionValidation = assertOfficialCatalogAdmission(rows, link.href);
    source.source_metadata.catalog_admission = {
      schema_version: admissionValidation.schema_version,
      normalization: admissionNormalization.metrics,
      validation: {
        valid: admissionValidation.valid,
        checked_row_count: admissionValidation.checked_row_count,
        issue_count: admissionValidation.issue_count,
        issue_counts: admissionValidation.issue_counts
      }
    };
    sources.push(source);
  ...
  const officialSourceTypeCounts = sources.reduce((counts, source) => {
    const sourceType = normalizeText(source.source_type || resolvedSourceType);
    if (sourceType) counts[sourceType] = Number(counts[sourceType] || 0) + 1;
    return counts;
  }, {});
  const officialSourceTypes = Object.keys(officialSourceTypeCounts).sort();
  const reportSourceType = officialSourceTypes.length === 1
    ? officialSourceTypes[0]
    : officialSourceTypes.length > 1
      ? "MIXED_OFFICIAL_CATALOG"
      : resolvedSourceType;
  ...
  return {
    ...
    staging,
    metrics: {
      // Explicit manifest entries may override a provider's default source
      // type (for example, a Leaf XLSX checklist rather than a release page).
      // Report the downloaded sources, not the provider-level fallback.
      official_source_type: reportSourceType,
      official_source_types: officialSourceTypes,
      official_source_type_counts: officialSourceTypeCounts,
