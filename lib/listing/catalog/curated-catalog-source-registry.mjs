...
export const curatedCatalogSources = Object.freeze([
  ...
  source({
    provider: "leaf_checklist",
    label: "Leaf Official Checklists",
  ...
  source({
    ...
    kind: "wotc_gatherer_set",
    quality_tier: catalogSourceQualityTiers.OFFICIAL_DISCOVERY,
    import_mode: catalogSourceImportModes.STAGING_IMPORT,
    default_enabled: true,
    prompt_eligible_by_default: true,
    parser_strategy: "wotc_gatherer_next_flight_set_v1",
  ...
  source({
    ...
    allowed_usage: Object.freeze([
      "blind_eval_diagnostics",
      "catalog_gap_discovery"
    ]),
    forbidden_usage: Object.freeze([
      ...sharedForbiddenUsage,
      "candidate_generation",
      "candidate_filter",
      "candidate_ranking",
      "safe_field_support"
...
export function catalogSourceCanEnterRetrieval(providerOrSourceType = "") {
  const entry = curatedCatalogSource(providerOrSourceType);
  return Boolean(
    entry
    && entry.quality_tier !== catalogSourceQualityTiers.REJECTED
    && entry.allowed_usage.includes("candidate_generation")
  );
}
