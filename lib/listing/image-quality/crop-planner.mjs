...
export const fieldCropTransformVersion = "field-crop-v3";
...
export function createFieldCropMetadata({
  ...
  return {
    ...
    // Browser uploads do not always carry a dedicated side property, but the
    // canonical image id and object path do. Preserve that durable identity so
    // sparse OCR can select front/back deterministically after persistence.
    source_side: sourceSideForImage(sourceSide)
      || sourceSideForImage(imageId)
      || sourceSideForImage(sourceObjectPath),
