...
export const imageQualityPolicyVersion = "image-quality-policy-v4-server-metadata-tristate";
export const serverCaptureQualityAuthority = "SERVER_CANONICAL_METADATA";
...
export function buildServerCaptureQualityFromCanonicalImages(images = [], {
  imageGenerationHash = null
} = {}) {
  const imageQualities = (Array.isArray(images) ? images : []).slice(0, 8).map((image) => {
    const width = positiveDimension(image.width || image.originalWidth || image.original_width);
    const height = positiveDimension(image.height || image.originalHeight || image.original_height);
    ...
    const dimensionsObserved = Boolean(width && height);
    const resolutionSufficient = dimensionsObserved
      ? longEdge >= defaultCaptureProfile.min_long_edge
      : null;
    const cropComplete = dimensionsObserved
      ? Math.abs(aspectRatio - defaultCaptureProfile.expected_aspect_ratio)
        / defaultCaptureProfile.expected_aspect_ratio <= defaultCaptureProfile.aspect_ratio_tolerance
      : null;
    return Object.freeze({
      capture_profile_id: defaultCaptureProfileId,
      capture_surface_type: captureSurfaceTypes.UNKNOWN,
      width,
      height,
      resolution_sufficient: resolutionSufficient,
      crop_complete: cropComplete,
      image_quality_degraded: dimensionsObserved ? !resolutionSufficient || !cropComplete : null,
      critical_region_occlusion: Object.freeze({})
    });
  });
  return Object.freeze({
    authority: serverCaptureQualityAuthority,
    policy_version: imageQualityPolicyVersion,
    image_generation_hash: String(imageGenerationHash || "").trim().toLowerCase() || null,
    capture_profile_id: defaultCaptureProfileId,
    capture_surface_type: captureSurfaceTypes.UNKNOWN,
    image_count: imageQualities.length,
    image_quality_degraded: imageQualities.some((quality) => quality.image_quality_degraded === true)
      ? true
      : imageQualities.some((quality) => typeof quality.image_quality_degraded === "boolean")
        ? false
        : null,
    // Width/aspect metadata cannot prove glare or critical-region visibility.
    // Leave the route unknown until a server visual analyzer produces evidence.
    route: null,
    glare_route: null,
    recovered_regions: Object.freeze([]),
    unresolved_regions: Object.freeze([]),
    critical_region_occlusion: Object.freeze({}),
    images: Object.freeze(imageQualities)
  });
}

export function serverCaptureQualityForPayload(payload = {}) {
  const candidate = payload.effective_capture_quality || payload.effectiveCaptureQuality;
  const expectedHash = String(payload.image_set_sha256 || payload.imageSetSha256 || "").trim().toLowerCase();
  const candidateHash = String(candidate?.image_generation_hash || "").trim().toLowerCase();
  if (candidate?.authority === serverCaptureQualityAuthority
    && candidate.policy_version === imageQualityPolicyVersion
    && (!expectedHash || candidateHash === expectedHash)) {
    return candidate;
  }
  return buildServerCaptureQualityFromCanonicalImages(payload.images || [], {
    imageGenerationHash: expectedHash || null
  });
}
