...
const geminiExtractionGuide = [
  ...
  "If you leave any core field empty while readable label/card text exists, add unresolved entries prefixed with visible_text: followed by the exact readable lines.",
  ...
  "Prefer field evidence over title prose. Set title to an empty string. Do not generate an eBay title; deterministic code will render the title from fields.",
  ...
  "For players, return the athlete/person/character names visible on the card or slab. Do not omit a clearly visible subject because serial or year is uncertain.",
  ...
  "Each field_evidence entry should include value, source_type, source_image_id when known, source_region when known, raw_text or visible_text, direct_observation/directly_observed, confidence, and review_required.",
...
export function geminiConfigFromEnv(env = process.env) {
  ...
  return {
    ...
    maxOutputTokens: numberFromEnv(env, "GEMINI_MAX_OUTPUT_TOKENS", 700),
    ...
    formatRepairMaxOutputTokens: numberFromEnv(
      ...
      numberFromEnv(env, "GEMINI_MAX_OUTPUT_TOKENS", 700)
    ),
...
export function geminiProviderResponseSchema() {
  return {
    ...
    properties: {
      ...
      route: {
        type: "string",
        enum: ["FAST_PATH_CANDIDATE", "NEEDS_REVIEW", "MULTI_CARD", "ABSTAIN", ""]
      },
      error_type: {
        type: "string",
      ...
      fields: {
        type: "object",
        additionalProperties: false,
        properties: Object.fromEntries(legacyFieldNames.map((fieldName) => [
          fieldName,
    ...
    required: ["title", "confidence", "recognition_status", "fields", "unresolved"]
  };
}
  ...
  const input = [
    {
      ...
      text: [
        ...
        "Set title to an empty string. Do not generate a listing title or repeat title phrases.",
...
function firstGradeFromText(value) {
  const text = collapseWhitespace(value);
  const match = text.match(/\b(?:10|9\.5|9|8\.5|8|7\.5|7|AUTHENTIC)\b/i);
...
function isInvalidPersonName(value) {
  ...
  return !text
    || /\b(?:copyright|year|season|release|card|label|front|back|text|product|unknown|unreadable|unclear|visible|player|athlete|subject)\b/i.test(text)
    || /^visible[_\s-]*text$/i.test(text)
    || /^(?:Autos?|Autographs?|Certified|Topps\s+Certified|Club\s+Legends|Historic\s+Ties(?:\s+Triple)?|Rookie\s+Ticket|Next\s+Stop\s+Signatures|Canvas\s+Creations(?:\s+Autos?)?|Hoopla|Material\s+Signatures|Variation[-\s]*Autograph|1983\s+Topps|Gem\s*Mt|Mint|Prizm|Topps\s+Chrome|Panini)$/i.test(text)
...
function sanitizeGeminiFields(fields = {}, parsed = {}) {
  ...
  Object.entries(fields).forEach(([fieldName, value]) => {
    ...
    if (fieldName === "checklist_code") {
      normalizedFields.checklist_code = compactFieldValue(value, { maxLength: compactStringFieldMaxLength.checklist_code }) || firstChecklistCodeFromText(value);
      return;
    }
    if (fieldName === "card_number" || fieldName === "collector_number") {
      normalizedFields[fieldName] = compactFieldValue(value, { maxLength: compactStringFieldMaxLength[fieldName] });
      return;
    }
    if (fieldName === "grade" || fieldName === "card_grade" || fieldName === "auto_grade") {
      normalizedFields[fieldName] = compactFieldValue(value, { maxLength: compactStringFieldMaxLength[fieldName] }) || firstGradeFromText(value);
      return;
    }
  ...
  if (!normalizedFields.product) normalizedFields.product = slabFacts.product || productFromText(evidenceText);
...
const topLevelGeminiFields = new Set(Object.keys(geminiProviderResponseSchema().properties));
...
function validateStrictGeminiPayload(payload = {}) {
  ...
  ["title", "confidence", "recognition_status", "fields", "unresolved"].forEach((key) => {
...
function scaffoldGeminiPayload(payload = {}) {
  ...
  const confidence = coerceString(source.confidence).trim().toUpperCase();
  const route = coerceString(source.route).trim().toUpperCase();
  ...
  return {
    ...
    ...(source.model_title_suggestion !== undefined ? { model_title_suggestion: coerceString(source.model_title_suggestion) } : {}),
    ...
    ...(source.route !== undefined ? { route: ["FAST_PATH_CANDIDATE", "NEEDS_REVIEW", "MULTI_CARD", "ABSTAIN", ""].includes(route) ? route : "" } : {}),
    ...
    ...(source.reason !== undefined ? { reason: coerceString(source.reason) } : {}),
...
export function geminiFieldsFromVisibleText(lines = []) {
  ...
  if (!Array.isArray(fields.players) || !fields.players.length) {
    const player = cleanedLines
      .map((line) => cleanPersonCandidate(line))
      .find((line) => line && line.split(/\s+/).length >= 2 && line.split(/\s+/).length <= 4);
    if (player) {
...
export async function transcribeVisibleCardTextWithGemini({
  ...
  const request = buildGeminiVisibleTextRequest({
    ...
    model: config.model,
    temperature: 0,
    maxOutputTokens: Math.min(500, Number(config.maxOutputTokens || 500)),
...
export async function analyzeCardEvidenceWithGemini({
  ...
  try {
    ...
    const interaction = initialCall.interaction;
