...
/**
 * A per-request reasoning effort, for sweeping effort the way models are swept.
 *
 * Effort trades three things against each other -- output tokens (cost), time
 * to first token (latency), and field fidelity (accuracy) -- and which point on
 * that curve is right is an empirical question, not a preference. Answering it
 * needs the efforts interleaved within one run; measuring them in sequence puts
 * each in its own time window and reattributes drift to effort.
 *
 * Like the model override, this is a client algorithm control: the queue strips
 * it unless the caller holds the launch-gate secret.
 */
export function providerReasoningEffortOverrideFromOptions(options = {}) {
  if (!options || typeof options !== "object" || Array.isArray(options)) return "";
  const raw = options.openai_reasoning_effort_override
    || options.openaiReasoningEffortOverride
