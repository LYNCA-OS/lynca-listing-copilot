...
const gpt5MaxOutputTokenCap = gpt5ProviderSafeOutputTokenCap * 10;
const gpt41MaxOutputTokenCap = gpt41ProviderSafeOutputTokenCap * 10;
const defaultOpenAiMaxOutputTokens = 409600000;
const defaultOpenAiTruncationRetryMaxOutputTokens = 819200000;
...
export function openAiEmergencyConfigFromEnv(env = process.env, { shardKey = "", modelOverride = "" } = {}) {
  ...
  const gpt5MaxOutputTokens = positiveNumberFromEnv(
    ...
    Math.max(819200000, defaultMaxOutputTokens)
  );
  const gpt5TruncationRetryMaxOutputTokens = positiveNumberFromEnv(
    env,
    "OPENAI_GPT5_TRUNCATION_RETRY_MAX_OUTPUT_TOKENS",
    Math.max(1280000000, gpt5MaxOutputTokens, defaultTruncationRetryMaxOutputTokens)
  );
