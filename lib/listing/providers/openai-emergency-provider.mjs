...
import { basicSurfaceColorVocabulary } from "../parallel-policy.mjs";
...
const basicSurfaceColorsPrompt = basicSurfaceColorVocabulary.join(", ");
...
function compactResponseProfileEnabled(responseProfile = "") {
  return [compactResponseProfile, ultraCompactResponseProfile].includes(responseProfile);
}
...
export function expandOpenAiCompactProviderPayload(payload = {}) {
  ...
  const conflictingDuplicateFields = new Set();
  let identicalDuplicateCount = 0;
  const compactValueKey = (value) => JSON.stringify(Array.isArray(value)
    ? value.map((item) => String(item).replace(/\s+/g, " ").trim().toLowerCase()).sort()
    : typeof value === "string"
      ? value.replace(/\s+/g, " ").trim().toLowerCase()
      : value);
  const assignEntries = (entries, allowedFields, entryValueKey, validateValue) => {
    for (const entry of entries) {
      const fieldName = String(entry?.field || "").trim();
      const value = entry?.[entryValueKey];
      if (!allowedFields.has(fieldName) || !validateValue(value)) {
        throw providerSchemaError(provider, `Compact provider response contains an invalid ${fieldName || "field"}.`);
      }
      if (conflictingDuplicateFields.has(fieldName)) continue;
      if (seenValues.has(fieldName)) {
        if (seenValues.get(fieldName) === compactValueKey(value)) {
          identicalDuplicateCount += 1;
          continue;
        }
        conflictingDuplicateFields.add(fieldName);
        delete fields[fieldName];
        continue;
      }
      seenValues.set(fieldName, compactValueKey(value));
  ...
  }).filter((entry) => !conflictingDuplicateFields.has(entry.field));

  const unresolved = [...new Set([
    ...(Array.isArray(payload.unresolved) ? payload.unresolved : []),
    ...conflictingDuplicateFields
  ])];
  const providerFieldRejections = [...conflictingDuplicateFields].map((field) => ({
    field,
    value: null,
    reason: "conflicting_duplicate_provider_field_dropped"
  }));
  ...
  return {
    ...
    ...(identicalDuplicateCount || conflictingDuplicateFields.size ? {
      provider_transport_repairs: {
        identical_duplicate_count: identicalDuplicateCount,
        conflicting_duplicate_fields: [...conflictingDuplicateFields]
      }
    } : {}),
    ...(providerFieldRejections.length ? { provider_field_rejections: providerFieldRejections } : {})
  };
}
...
export function expandOpenAiUltraCompactProviderPayload(payload = {}) {
  ...
  return expandOpenAiCompactProviderPayload({
...
export function openAiEmergencyConfigFromEnv(env = process.env, {
  ...
  return {
    ...
    attemptTimeoutMs: Math.min(
      numberFromEnv(env, "OPENAI_LISTING_TIMEOUT_MS", 75000),
      numberFromEnv(env, "OPENAI_LISTING_ATTEMPT_TIMEOUT_MS", 20000)
    ),
    outputTokenCap,
...
function openAiStrictSchemaPrompt(prompt = "", {
  ...
  return [
    ...
    "Year discipline: year means issue year, never a retro design year. Use a slab/product year when printed. On an ungraded card, back text describing season N plus copyright N+1 means issue year N+1; a lone stats or copyright year is not enough. Preserve season strings like 2025-26.",
    ...
    "Autograph discipline: set auto true only for a visible autograph sticker/certification marker, certified autograph wording, Signatures/Auto insert, or slab autograph wording. A printed, facsimile, decorative, signed-image, or unlabeled ink-style signature is not confirmed Auto. Never use Signed Image as card_name for a signature graphic. Keep an official insert name separately in insert.",
    ...
    `Use surface_color for a visible basic color (${basicSurfaceColorsPrompt}). Do not infer exact optical parallels such as Gold Refractor, Wave, Shimmer, Mojo, Prizm, Sparkle, or Holo from appearance alone.`,
    compact
...
function providerErrorAllowsKeyRotation(error) {
  ...
  return ["rate_limited", "upstream_error", "timeout", "provider_timeout", "network_error", "empty_response"]
...
export async function analyzeCardEvidenceWithOpenAiEmergency({
  ...
  try {
    ...
    const callOpenAi = async (maxOutputTokens, requestedOutputTokens = maxOutputTokens) => {
      ...
      const attemptAbort = createRequestAbort({
      ...
      body: JSON.stringify({
        ...
        text: openAiResponsesTextOptions({
          model: config.model,
          name: responseProfile === ultraCompactResponseProfile
          ...
          strict: true,
          schema: responseProfile === ultraCompactResponseProfile
      ...
      signal: attemptAbort.signal
        ...
        if (error?.name === "AbortError") {
          ...
          throw new ProviderError("OpenAI single-provider attempt timed out.", {
            provider,
            ...
            details: {
              ...
              total_timeout_ms: config.timeoutMs
            }
          });
        }
        logOpenAiProviderRequestDiagnostics({
    ...
    const canonicalParsed = responseProfile === ultraCompactResponseProfile
