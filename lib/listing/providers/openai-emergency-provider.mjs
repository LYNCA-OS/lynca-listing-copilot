...
function compactResponseProfileEnabled(responseProfile = "") {
  return [compactResponseProfile, ultraCompactResponseProfile, readOnlyResponseProfile].includes(responseProfile);
}
...
function openAiStrictSchemaPrompt(prompt = "", {
  ...
  const readOnly = responseProfile === readOnlyResponseProfile;
  if (readOnly) {
    return [
      prompt,
...
export async function analyzeCardEvidenceWithOpenAiEmergency({
  ...
  try {
    ...
    const callOpenAi = async (maxOutputTokens, requestedOutputTokens = maxOutputTokens) => {
      ...
      body: JSON.stringify({
        ...
        text: openAiResponsesTextOptions({
          model: config.model,
          name: responseProfile === readOnlyResponseProfile
          ...
          strict: true,
          schema: responseProfile === readOnlyResponseProfile
    ...
    const canonicalParsed = [ultraCompactResponseProfile, readOnlyResponseProfile].includes(responseProfile)
