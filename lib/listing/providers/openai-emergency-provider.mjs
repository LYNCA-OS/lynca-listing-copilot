...
} from "./provider-output-field-contract.mjs";
import { buildOpenAiProviderRequestIdentity } from "./provider-request-identity.mjs";

const provider = visionProviderIds.OPENAI_LEGACY;
const endpoint = "https://api.openai.com/v1/responses";
// Model-published max_output_tokens. Requests above these get a 400 from the
// Responses API, which the output-cap downgrade path then retries — one wasted
// round trip per call. First request must already be within spec.
const gpt5ProviderSafeOutputTokenCap = 128000;
const gpt41ProviderSafeOutputTokenCap = 32768;
const gpt5MaxOutputTokenCap = gpt5ProviderSafeOutputTokenCap;
const gpt41MaxOutputTokenCap = gpt41ProviderSafeOutputTokenCap;
const defaultOpenAiMaxOutputTokens = gpt41ProviderSafeOutputTokenCap;
const defaultOpenAiTruncationRetryMaxOutputTokens = gpt41ProviderSafeOutputTokenCap;
export const openAiProviderFieldNames = providerOutputFieldNames;
const openAiFieldNames = openAiProviderFieldNames;
...
const openAiListFieldNames = new Set(["players", "attributes", "observable_components"]);
const openAiNumberFieldNames = new Set(["card_count"]);
const openAiStringFieldNames = openAiFieldNames.filter((fieldName) => (
  !openAiBooleanFieldNames.has(fieldName)
  && !openAiListFieldNames.has(fieldName)
  && !openAiNumberFieldNames.has(fieldName)
));
const openAiEvidenceFieldNames = Object.freeze([
  ...new Set([...openAiFieldNames, "player", "subject", "grade"])
]);
const compactResponseProfile = "compact_sparse_v1";
const ultraCompactResponseProfile = "compact_sparse_v2";
const disabledMixedWorldKnowledgeResponseProfile = "read_only_world_knowledge_v1";

function compactResponseProfileEnabled(responseProfile = "") {
  return [compactResponseProfile, ultraCompactResponseProfile, readOnlyProviderResponseProfile].includes(responseProfile);
}
...
function compactFieldValueEntrySchema(fieldNames, valueSchema, valueKey = "value") {
  return {
    type: "object",
    additionalProperties: false,
    properties: {
    ...
    required: ["field", valueKey]
  };
}

export function openAiCompactProviderResponseSchema() {
  return {
    type: "object",
    additionalProperties: false,
    properties: {
      ...
      field_values: {
        ...
        properties: {
          strings: {
            type: "array",
            items: compactFieldValueEntrySchema(openAiStringFieldNames, { type: "string" })
          },
          booleans: {
            type: "array",
            items: compactFieldValueEntrySchema([...openAiBooleanFieldNames], { type: "boolean" })
          },
          numbers: {
            type: "array",
            items: compactFieldValueEntrySchema([...openAiNumberFieldNames], { type: "number" })
          },
          lists: {
            type: "array",
            items: compactFieldValueEntrySchema(
              [...openAiListFieldNames],
              { type: "array", items: { type: "string" } },
              "values"
            )
          }
        },
        required: ["strings", "booleans", "numbers", "lists"]
      },
      field_evidence: {
        type: "array",
        items: {
          ...
          additionalProperties: false,
          properties: {
            field: { type: "string", enum: [...openAiEvidenceFieldNames] },
            value: { type: ["string", "number", "boolean", "null"] },
            source_type: { type: "string" },
            source_image_id: { type: "string" },
            source_region: { type: "string" },
      ...
      vector_candidate_decision: openAiProviderResponseSchema().properties.vector_candidate_decision
    },
    required: ["recognition_status", "field_values", "field_evidence", "unresolved", "vector_candidate_decision"]
  };
}

function ultraCompactEntrySchema(fieldNames, valueSchema) {
  return {
    type: "object",
    additionalProperties: false,
    properties: {
      f: { type: "string", enum: [...fieldNames] },
      v: valueSchema
    },
    required: ["f", "v"]
  };
...
export function openAiUltraCompactProviderResponseSchema({ includeVectorDecision = false } = {}) {
  const properties = {
    r: { type: "string", enum: ["CONFIRMED", "RESOLVED", "ABSTAIN"] },
    v: {
      type: "object",
      additionalProperties: false,
      properties: {
        s: { type: "array", items: ultraCompactEntrySchema(openAiStringFieldNames, { type: "string" }) },
        b: { type: "array", items: ultraCompactEntrySchema([...openAiBooleanFieldNames], { type: "boolean" }) },
        n: { type: "array", items: ultraCompactEntrySchema([...openAiNumberFieldNames], { type: "number" }) },
        l: {
          type: "array",
          items: ultraCompactEntrySchema([...openAiListFieldNames], { type: "array", items: { type: "string" } })
        }
      },
      required: ["s", "b", "n", "l"]
    },
    e: {
      ...
      items: {
        ...
        additionalProperties: false,
        properties: {
          f: { type: "string", enum: [...openAiEvidenceFieldNames] },
  ...
  if (includeVectorDecision) {
    properties.c = {
      ...
      properties: {
        i: { type: ["string", "null"] },
        d: { type: "string", enum: ["SELECTED", "PARTIAL_SUPPORT", "REJECTED_ALL", "NOT_AVAILABLE"] },
...
export function openAiReadOnlyProviderResponseSchema() {
  const schema = openAiUltraCompactProviderResponseSchema({ includeVectorDecision: false });
  schema.properties.v.properties.s.items.properties.f.enum = providerReadFieldNamesByType("string");
  schema.properties.v.properties.b.items.properties.f.enum = providerReadFieldNamesByType("boolean");
  schema.properties.v.properties.n.items.properties.f.enum = providerReadFieldNamesByType("number");
  schema.properties.v.properties.l.items.properties.f.enum = providerReadFieldNamesByType("list");
  // The sparse contract has no negative/default rows. Enforce that at the
  // structured-output boundary instead of relying on prompt compliance.
  schema.properties.v.properties.b.items.properties.v = { type: "boolean", const: true };
  schema.properties.v.properties.n.items.properties.v = { type: "number", minimum: 2 };
  schema.properties.e.items.properties.f.enum = [
    ...providerReadFieldNamesByType("string"),
    ...providerReadFieldNamesByType("boolean"),
    ...providerReadFieldNamesByType("number"),
    ...providerReadFieldNamesByType("list"),
    "grade"
  ];
  return schema;
}

function compactEvidenceKind(fieldName = "") {
  const kinds = {
    year: "YEAR_TEXT",
    print_run_number: "PRINTED_LIMITED_NUMBERING",
    print_run_numerator: "PRINTED_LIMITED_NUMBERING",
    print_run_denominator: "PRINTED_LIMITED_NUMBERING",
    numerical_rarity: "PRINTED_LIMITED_NUMBERING",
    serial_number: "LEGACY_SERIAL_ALIAS",
    grade: "GRADE_LABEL",
    ...
    rc: "RC_MARKER",
    auto: "AUTO_EVIDENCE",
    card_name: "PRINTED_CARD_NAME"
  };
  return kinds[fieldName] || "PROVIDER_OBSERVATION";
}

function compactEntries(payload, key) {
  const entries = payload?.field_values?.[key];
  if (!Array.isArray(entries)) {
    throw providerSchemaError(provider, `Compact provider response field_values.${key} must be an array.`);
  }
  return entries;
}

export function expandOpenAiCompactProviderPayload(payload = {}) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    throw providerSchemaError(provider, "Compact provider response must be a JSON object.");
  }

  const fields = {};
  const seen = new Set();
  const assignEntries = (entries, allowedFields, valueKey, validateValue) => {
    for (const entry of entries) {
      const fieldName = String(entry?.field || "").trim();
      const value = entry?.[valueKey];
      if (!allowedFields.has(fieldName) || !validateValue(value)) {
        throw providerSchemaError(provider, `Compact provider response contains an invalid or duplicate ${fieldName || "field"}.`);
      }
      if (seen.has(fieldName)) {
        // Structured-output schemas cannot express uniqueness by the nested
        // `field` property. Treat an exact repeated row as an idempotent
        // transport retry, but keep conflicting duplicates fail-closed.
        if (JSON.stringify(fields[fieldName]) === JSON.stringify(value)) continue;
        throw providerSchemaError(provider, `Compact provider response contains an invalid or duplicate ${fieldName || "field"}.`);
      }
      seen.add(fieldName);
  ...
  assignEntries(
    compactEntries(payload, "strings"),
    new Set(openAiStringFieldNames),
    "value",
    (value) => typeof value === "string"
  );
  assignEntries(
    compactEntries(payload, "booleans"),
    openAiBooleanFieldNames,
    "value",
    (value) => typeof value === "boolean"
  );
  assignEntries(
    compactEntries(payload, "numbers"),
    openAiNumberFieldNames,
    "value",
    (value) => Number.isFinite(value)
  );
  assignEntries(
    compactEntries(payload, "lists"),
    openAiListFieldNames,
    "values",
    (value) => Array.isArray(value) && value.every((item) => typeof item === "string")
  );

  if (!Array.isArray(payload.field_evidence)) {
    throw providerSchemaError(provider, "Compact provider response field_evidence must be an array.");
  }
  const fieldEvidence = payload.field_evidence.map((entry) => {
    const visibleText = String(entry?.visible_text || "");
    const directlyObserved = entry?.directly_observed === true;
    return {
      field: String(entry?.field || ""),
      value: entry?.value ?? null,
      source_type: String(entry?.source_type || "NONE"),
      source_image_id: String(entry?.source_image_id || ""),
      source_region: String(entry?.source_region || ""),
      raw_text: visibleText,
      visible_text: visibleText,
      evidence_kind: compactEvidenceKind(entry?.field),
      confidence: null,
      review_required: entry?.review_required === true,
      directly_observed: directlyObserved,
      direct_observation: directlyObserved
    };
  });
  ...
  return {
    ...
    vector_candidate_decision: payload.vector_candidate_decision
  };
}

function compactEvidenceDirectlyObserved(sourceType = "") {
  const normalized = String(sourceType || "").trim().toUpperCase();
  return Boolean(normalized) && !["NONE", "VISION_ONLY", "VISUAL_GUESS", "AGNES_INFERENCE"].includes(normalized);
}

function compactEvidenceRegion(fieldName = "") {
  if (fieldName === "year") return "year_product";
  if (/^(?:print_run|serial_number|numerical_rarity)/.test(fieldName)) return "print_run_number";
  if (/^(?:grade|card_grade|auto_grade|grade_type|cert_number)/.test(fieldName)) return "grade_label";
  if (/^(?:collector_number|checklist_code|card_number|tcg_card_number)/.test(fieldName)) return "card_code";
  return fieldName;
}

export function expandOpenAiUltraCompactProviderPayload(payload = {}) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    throw providerSchemaError(provider, "Ultra-compact provider response must be a JSON object.");
  }
  const values = payload.v || {};
  const evidence = Array.isArray(payload.e) ? payload.e : [];
  const candidate = payload.c || null;
  const expanded = expandOpenAiCompactProviderPayload({
    recognition_status: payload.r,
    field_values: {
      strings: (values.s || []).map((entry) => ({ field: entry.f, value: entry.v })),
      booleans: (values.b || []).map((entry) => ({ field: entry.f, value: entry.v })),
      numbers: (values.n || []).map((entry) => ({ field: entry.f, value: entry.v })),
      lists: (values.l || []).map((entry) => ({ field: entry.f, values: entry.v }))
    },
    field_evidence: evidence.map((entry) => {
      const sourceType = String(entry.s || "NONE");
      const directlyObserved = compactEvidenceDirectlyObserved(sourceType);
      return {
        field: entry.f,
        value: entry.v,
        source_type: sourceType,
        source_image_id: String(entry.i || ""),
        source_region: compactEvidenceRegion(entry.f),
        visible_text: String(entry.t || ""),
        review_required: !directlyObserved,
        directly_observed: directlyObserved
      };
    }),
    unresolved: Array.isArray(payload.u) ? payload.u : [],
    vector_candidate_decision: candidate
      ? {
        selected_candidate_id: candidate.i ?? null,
        decision: candidate.d,
        supported_fields: candidate.s || [],
        rejected_fields: candidate.r || [],
        conflicts: candidate.c || []
      }
      : {
...
function modelMaxOutputTokenCap(modelId = "", env = process.env) {
  if (isGpt5ResponsesModel(modelId)) {
    return Math.min(
      positiveNumberFromEnv(env, "OPENAI_GPT5_MAX_OUTPUT_TOKEN_CAP", gpt5MaxOutputTokenCap),
      gpt5MaxOutputTokenCap
    );
  }
  if (/^gpt-4\.1(?:$|-)/i.test(String(modelId || "").trim())) {
    return Math.min(
      positiveNumberFromEnv(env, "OPENAI_GPT41_MAX_OUTPUT_TOKEN_CAP", gpt41MaxOutputTokenCap),
      gpt41MaxOutputTokenCap
    );
  }
  return positiveNumberFromEnv(env, "OPENAI_LISTING_MAX_OUTPUT_TOKEN_CAP", defaultOpenAiMaxOutputTokens);
}

function modelSafeOutputTokenFallback(modelId = "", env = process.env) {
  if (isGpt5ResponsesModel(modelId)) {
    return positiveNumberFromEnv(env, "OPENAI_GPT5_SAFE_OUTPUT_TOKEN_CAP", gpt5ProviderSafeOutputTokenCap);
  }
  if (/^gpt-4\.1(?:$|-)/i.test(String(modelId || "").trim())) {
    return positiveNumberFromEnv(env, "OPENAI_GPT41_SAFE_OUTPUT_TOKEN_CAP", gpt41ProviderSafeOutputTokenCap);
  }
  return positiveNumberFromEnv(env, "OPENAI_LISTING_SAFE_OUTPUT_TOKEN_CAP", gpt5ProviderSafeOutputTokenCap);
}
...
export function openAiEmergencyConfigFromEnv(env = process.env, {
  ...
  const keySelection = selectOpenAiApiKey({ env, shardKey, preferredKeySlot });
  const keySelections = orderedOpenAiApiKeySelections({ env, shardKey, preferredKeySlot });
  ...
  const defaultMaxOutputTokens = positiveNumberFromEnv(env, "OPENAI_LISTING_MAX_OUTPUT_TOKENS", outputTokenCap || defaultOpenAiMaxOutputTokens);
  const defaultTruncationRetryMaxOutputTokens = positiveNumberFromEnv(env, "OPENAI_LISTING_TRUNCATION_RETRY_MAX_OUTPUT_TOKENS", Math.max(defaultMaxOutputTokens, outputTokenCap || defaultOpenAiTruncationRetryMaxOutputTokens));
  ...
  const gpt5TruncationRetryMaxOutputTokens = positiveNumberFromEnv(
    ...
    Math.max(gpt5MaxOutputTokens, gpt5MaxOutputTokenCap)
  );
  const requestedMaxOutputTokens = effectiveOutputTokens(
    isGpt5 ? gpt5MaxOutputTokens : defaultMaxOutputTokens,
    outputTokenCap
  );
  const requestedTruncationRetryMaxOutputTokens = effectiveOutputTokens(
    isGpt5 ? gpt5TruncationRetryMaxOutputTokens : defaultTruncationRetryMaxOutputTokens,
    outputTokenCap
  );
...
function validateOpenAiImages(images = [], detail = "high") {
...
function openAiModelProfilePrompt(model = "") {
  ...
  return [
    ...
    "Match the GPT-4.1-mini production extraction semantics: same fields, same evidence discipline, same abstention rules, and same renderer expectations. A sparse transport may omit empty/default values because code expands it locally.",
    ...
    "Never leave product, set, players, card_name, print_run_number, collector/card code, or slab grade empty when visibly shown; preserve printed modifiers such as Platinum. If multiple distinct cards are visible, set multi_card true, card_count, and list every readable subject in players.",
    ...
    "If the slab label shows a certification number (a 7-10 digit number, often near a barcode or labeled CERT/Certification), transcribe it into cert_number exactly as printed, digits only. Never copy cert numbers from reference candidates; only from the current slab label.",
...
function openAiStrictSchemaPrompt(prompt = "", {
  ...
  const compact = compactResponseProfileEnabled(responseProfile);
  const readOnly = responseProfile === readOnlyProviderResponseProfile;
  if (readOnly) {
    return [
      prompt,
      openAiModelProfilePrompt(model),
      "Read-only sparse transport: use only r, v.s/v.b/v.n/v.l rows {f,v}, high-risk evidence rows e={f,v,s,i,t}, and unresolved field names u.",
      "Never emit empty/default rows. Never emit a field outside the schema enum. Do not return c or any candidate decision.",
      "Do not return k or any world-knowledge proposal.",
      "The service expands short keys into the canonical evidence contract; do not add prose, title, reason, confidence, source_region, or duplicate aliases."
    ].join("\n");
  }
  const ultraCompact = responseProfile === ultraCompactResponseProfile;
  const transportRules = ultraCompact
    ? [
      "OpenAI ultra-sparse response note:",
      "Use only the short schema keys. r=recognition status. v.s/v.b/v.n/v.l are sparse field rows {f,v}. e contains only high-risk evidence rows {f,v,s,i,t}, where s=source_type, i=source_image_id, and t=short literal visible_text. u is the unresolved field-name array.",
      includeVectorDecision
        ? "c is the required candidate decision: i=selected id, d=decision, s=supported fields, r=rejected fields, c=conflicts."
        : "No candidate packet was supplied, so do not return c; code deterministically expands it to NOT_AVAILABLE.",
      "Never emit empty/default value rows. Every emitted collector_number/checklist_code/card_number/tcg_card_number needs an e row with literal current-image text.",
      "The service deterministically derives source_region, review_required, directly_observed, evidence_kind, and canonical long keys. Do not spend output on them."
    ]
    : compact
    ? [
      "OpenAI compact sparse response note:",
      "Return only recognition_status, field_values, field_evidence, unresolved, and vector_candidate_decision.",
      "field_values.strings contains only non-empty string facts; booleans contains only true facts; numbers contains only present numeric facts; lists contains only non-empty string arrays. Never emit an empty/default field entry.",
      "Return field_evidence only for non-empty high-risk or review-sensitive facts. Each entry must include field, value, source_type, source_image_id, source_region, visible_text, review_required, and directly_observed.",
      "Every emitted collector_number, checklist_code, card_number, or tcg_card_number must have a directly_observed field_evidence entry with literal visible_text. If the code is not directly readable, omit it or add its field name to unresolved.",
      "The service expands this sparse transport into the canonical fields + field_evidence contract. Sparse means less JSON, not fewer observed facts."
    ]
    : [
  ...
  return [
    ...
    openAiModelProfilePrompt(model),
    "",
    "Apply the same extraction semantics for every OpenAI model. GPT-5 mini is a production main path and must not use looser field or evidence rules than GPT-4.1 mini; only the transport representation may be sparse.",
    ...transportRules,
    ...
    "Autograph discipline: set auto true only for a visible autograph sticker, certified autograph wording, Signatures/Auto insert, slab autograph wording, or clearly distinct live ink. A printed, facsimile, decorative, or signed-style signature alone is not Auto. Keep the official insert name separately in insert.",
    ...
    "Code evidence discipline: every emitted collector_number, checklist_code, card_number, or tcg_card_number must have directly observed field_evidence with literal visible_text. A code without direct evidence must remain unresolved instead of silently entering or leaving identity.",
    ...
    compact
      ? "Omit unknown/default field values from field_values and omit unsupported evidence entries. Do not invent identity values to fill the schema."
...
function outputTokenCount(usage = {}) {
  const raw = usage?.output_tokens ?? usage?.completion_tokens ?? usage?.total_output_tokens;
...
function reasoningTokenCount(usage = {}) {
  const raw = usage?.output_tokens_details?.reasoning_tokens
...
function inputTokenCount(usage = {}) {
  const raw = usage?.input_tokens ?? usage?.prompt_tokens ?? usage?.total_input_tokens;
  if (raw === null || raw === undefined || raw === "") return null;
  const value = Number(raw);
  return Number.isFinite(value) && value >= 0 ? value : null;
}
...
function openAiResponseDiagnostics(data = {}, outputCap = 0, {
  ...
  return {
    ...
    visible_output_tokens: outputTokens !== null
...
function providerErrorAllowsKeyRotation(error) {
  ...
  return ["rate_limited", "upstream_error", "timeout", "network_error", "empty_response"].includes(error.code);
}

function providerErrorLooksOutputCapTooLarge(error) {
  if (!(error instanceof ProviderError) || error.status !== 400) return false;
  return /max[_\s-]*output[_\s-]*tokens?|output[_\s-]*token|maximum.*tokens?|too many.*tokens?|exceeds.*tokens?/i.test(String(error.message || ""));
}
...
function mergeOpenAiUsages(usages = []) {
  ...
  return {
    ...
    reasoning_tokens: sumNullableNumbers(valid.map((usage) => usage.reasoning_tokens)),
    visible_output_tokens: sumNullableNumbers(valid.map((usage) => usage.visible_output_tokens)),
...
export async function analyzeCardEvidenceWithOpenAiEmergency({
  ...
  if (responseProfile === disabledMixedWorldKnowledgeResponseProfile) {
    throw providerInputUnsupported(
      provider,
      "Mixed visual-observation and world-knowledge responses are disabled. World knowledge requires a separate post-observation shadow call."
    );
  }
  const config = openAiEmergencyConfigFromEnv(env, { shardKey, modelOverride, preferredKeySlot });
  if (!config.apiKey) {
  ...
  const providerHttpRequestBudget = Number(requestContext?.provider_http_request_budget);
  const singleHttpRequestOnly = providerHttpRequestBudget === 1;
  const requestedServiceTier = ["auto", "default", "flex", "priority"].includes(String(serviceTier || "").trim().toLowerCase())
    ? String(serviceTier).trim().toLowerCase()
    : null;

  try {
    const effectiveImageDetail = ["low", "high", "auto"].includes(String(imageDetail || "").toLowerCase())
      ? String(imageDetail).toLowerCase()
      : "high";
    const strictPrompt = openAiStrictSchemaPrompt(prompt, {
      model: config.model,
      responseProfile,
      includeVectorDecision
    });
    const validatedImages = validateOpenAiImages(images, effectiveImageDetail);
    const modelControls = openAiResponsesModelControls(config.model, { env });
    const textOptions = openAiResponsesTextOptions({
      model: config.model,
      name: responseProfile === readOnlyProviderResponseProfile
        ? "listing_provider_read_only_sparse"
        : responseProfile === ultraCompactResponseProfile
        ? "listing_provider_evidence_ultra_compact"
        : responseProfile === compactResponseProfile
          ? "listing_provider_evidence_compact"
          : "listing_provider_evidence",
      strict: true,
      schema: responseProfile === readOnlyProviderResponseProfile
        ? openAiReadOnlyProviderResponseSchema()
        : responseProfile === ultraCompactResponseProfile
        ? openAiUltraCompactProviderResponseSchema({ includeVectorDecision })
        : responseProfile === compactResponseProfile
    ...
    const requestIdentityBase = buildOpenAiProviderRequestIdentity({
      model: config.model,
      strictPrompt,
      images,
      imageDetail: effectiveImageDetail,
      responseProfile,
      includeVectorDecision,
    ...
    let providerFirstRequestStartedAt = null;
    let providerLastRequestCompletedAt = null;
    let keyRotationAttempts = 0;
    let outputCapDowngradeAttempts = 0;
    const safeOutputTokenFallback = modelSafeOutputTokenFallback(config.model, env);
    ...
    const rotateOpenAiKey = () => {
      if (singleHttpRequestOnly) return false;
    ...
    const callOpenAi = async (maxOutputTokens, requestedOutputTokens = maxOutputTokens) => {
      ...
      providerFirstRequestStartedAt = providerFirstRequestStartedAt || new Date(requestStartedAt).toISOString();
      ...
      signal: requestAbort.signal
        ...
        providerLastRequestCompletedAt = new Date().toISOString();
        ...
        if (error?.name === "AbortError") {
          logOpenAiProviderRequestDiagnostics({
            ...
            attempt,
            responseStatus: "provider_timeout",
      ...
      const responseStatus = String(text || "").trim()
        ? diagnostics.response_status || "ok"
        : "empty_response";
    ...
    const callOpenAiWithTransientRetry = async (maxOutputTokens, requestedOutputTokens = maxOutputTokens, {
      allowOutputCapDowngrade = !singleHttpRequestOnly
    } = {}) => {
      let attempt = 0;
      for (;;) {
        try {
          const response = await callOpenAi(maxOutputTokens, requestedOutputTokens);
          if (!String(response.text || "").trim()) {
            throw new ProviderError("OpenAI returned HTTP 200 without response content.", {
              provider,
              code: "empty_response",
              retryable: true,
              details: {
                token_diagnostics: response.diagnostics,
                rate_limit_diagnostics: response.rate_limit_diagnostics,
                provider_request_diagnostics: response.request_diagnostics
              }
            });
          }
          return response;
          ...
          if (
            allowOutputCapDowngrade
            && providerErrorLooksOutputCapTooLarge(error)
            && Number.isFinite(Number(safeOutputTokenFallback))
            && Number(safeOutputTokenFallback) > 0
            && Number(maxOutputTokens) > Number(safeOutputTokenFallback)
          ) {
            outputCapDowngradeAttempts += 1;
            return await callOpenAiWithTransientRetry(
              Number(safeOutputTokenFallback),
              requestedOutputTokens,
              { allowOutputCapDowngrade: false }
            );
          }
          ...
          const transientRetryLimit = singleHttpRequestOnly ? 0 : config.transientRetries;
          if (!(error instanceof ProviderError) || error.retryable !== true || attempt >= transientRetryLimit) {
            throw error;
          }
          attempt += 1;
          transientRetryAttempts += 1;
    ...
    if (initial.truncated) {
      ...
      if (singleHttpRequestOnly || retryCap <= Number(config.maxOutputTokens || 0)) {
    ...
    const canonicalParsed = [ultraCompactResponseProfile, readOnlyProviderResponseProfile].includes(responseProfile)
      ? expandOpenAiUltraCompactProviderPayload(parsed)
      : responseProfile === compactResponseProfile
        ? expandOpenAiCompactProviderPayload(parsed)
        : parsed;
    ...
    return {
      ...
      image_detail: effectiveImageDetail,
      ...
      provider_request_identity: {
        ...requestIdentityBase,
        response_model_id: data.model || null,
        provider_http_request_budget: singleHttpRequestOnly ? 1 : null,
        provider_http_request_budget_enforced: singleHttpRequestOnly,
        provider_http_retry_policy: singleHttpRequestOnly ? "FORBIDDEN" : "PRODUCTION_DEFAULT",
        provider_http_request_count: providerRequestCount,
        provider_http_request_started_at: providerFirstRequestStartedAt,
        provider_http_request_completed_at: providerLastRequestCompletedAt
      },
      output_cap_downgrade_attempted: outputCapDowngradeAttempts > 0,
      output_cap_downgrade_attempts: outputCapDowngradeAttempts,
      ...
      content: text,
      tool_calls: [],
      parsed: validateProviderEvidencePayload(provider, canonicalParsed)
    };
  } catch (error) {
    requestAbort.cleanup();
