...
export async function analyzeCardEvidenceWithOpenAiEmergency({
  ...
  try {
        ...
        input: [
          {
            ...
            content: [
              {
                type: "input_text",
                text: openAiStrictSchemaPrompt(prompt, { model: config.model, responseProfile, includeVectorDecision })
              },
              ...validateOpenAiImages(images, imageDetail)
            ]
          }
        ],
        max_output_tokens: maxOutputTokens,
        ...openAiResponsesModelControls(config.model, { env, effortOverride: reasoningEffortOverride }),
    ...
    return {
      ...
      image_detail: ["low", "high", "auto"].includes(String(imageDetail || "").toLowerCase())
        ? String(imageDetail).toLowerCase()
        : "high",
      text_verbosity: textVerbosity || null,
      // The effort that was actually sent, after clamping -- not the one asked
      // for. A sweep has to be able to prove each arm ran at the effort it was
      // labelled with, or it repeats the mistake of attributing a score to a
      // setting that never reached the API.
      reasoning_effort: openAiResponsesModelControls(config.model, {
        env,
