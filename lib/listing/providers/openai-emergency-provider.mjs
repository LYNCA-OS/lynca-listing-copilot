...
const readOnlyResponseProfile = "read_only_sparse_v3";
const worldKnowledgeResponseProfile = "read_only_world_knowledge_v1";

function compactResponseProfileEnabled(responseProfile = "") {
  return [compactResponseProfile, ultraCompactResponseProfile, readOnlyResponseProfile, worldKnowledgeResponseProfile].includes(responseProfile);
}
...
export function openAiWorldKnowledgeProviderResponseSchema() {
  ...
  schema.properties.k = {
    ...
    items: {
      ...
      additionalProperties: false,
      properties: {
        f: { type: "string", enum: ["team", "product"] },
        v: { type: "string" },
        b: { type: "string", enum: ["OBSERVED", "KNOWN"] }
      },
      required: ["f", "v", "b"]
    }
  };
...
export function expandOpenAiUltraCompactProviderPayload(payload = {}) {
  ...
  const proposals = Array.isArray(payload.k) ? payload.k : [];
  return {
    ...expanded,
    world_knowledge_proposals: proposals.map((entry) => ({
      field: String(entry?.f || ""),
      value: String(entry?.v || ""),
      basis: String(entry?.b || "")
    }))
  };
}
...
function openAiStrictSchemaPrompt(prompt = "", {
  ...
  const readOnly = [readOnlyResponseProfile, worldKnowledgeResponseProfile].includes(responseProfile);
  if (readOnly) {
    const worldKnowledge = responseProfile === worldKnowledgeResponseProfile;
    return [
      prompt,
      ...
      worldKnowledge
        ? "k is the only world-knowledge lane: at most one team and one product proposal, each {f,v,b}; b must be OBSERVED or KNOWN. Return k=[] when neither is sufficiently reliable."
...
export async function analyzeCardEvidenceWithOpenAiEmergency({
  ...
  try {
    ...
    const callOpenAi = async (maxOutputTokens, requestedOutputTokens = maxOutputTokens) => {
      ...
      body: JSON.stringify({
        ...
        text: openAiResponsesTextOptions({
          model: config.model,
          name: responseProfile === worldKnowledgeResponseProfile
            ? "listing_provider_read_only_world_knowledge"
            : responseProfile === readOnlyResponseProfile
          ...
          strict: true,
          schema: responseProfile === worldKnowledgeResponseProfile
    ...
    const canonicalParsed = [ultraCompactResponseProfile, readOnlyResponseProfile, worldKnowledgeResponseProfile].includes(responseProfile)
