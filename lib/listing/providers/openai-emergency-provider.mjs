...
export function openAiEmergencyConfigFromEnv(env = process.env, { shardKey = "", modelOverride = "" } = {}) {
  ...
  const defaultMaxOutputTokens = positiveNumberFromEnv(env, "OPENAI_LISTING_MAX_OUTPUT_TOKENS", 4096);
  const defaultTruncationRetryMaxOutputTokens = positiveNumberFromEnv(env, "OPENAI_LISTING_TRUNCATION_RETRY_MAX_OUTPUT_TOKENS", 8192);
  const gpt5MaxOutputTokens = positiveNumberFromEnv(
    env,
    "OPENAI_GPT5_MAX_OUTPUT_TOKENS",
    Math.max(81920, defaultMaxOutputTokens)
  );
  const gpt5TruncationRetryMaxOutputTokens = positiveNumberFromEnv(
    env,
    "OPENAI_GPT5_TRUNCATION_RETRY_MAX_OUTPUT_TOKENS",
    Math.max(gpt5MaxOutputTokenCap, gpt5MaxOutputTokens, defaultTruncationRetryMaxOutputTokens)
  );
  ...
  return {
    ...
    maxOutputTokens: isGpt5 ? Math.min(gpt5MaxOutputTokenCap, gpt5MaxOutputTokens) : defaultMaxOutputTokens,
    truncationRetryMaxOutputTokens: isGpt5 ? Math.min(gpt5MaxOutputTokenCap, gpt5TruncationRetryMaxOutputTokens) : defaultTruncationRetryMaxOutputTokens,
