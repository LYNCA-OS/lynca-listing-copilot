...
function openAiStrictSchemaPrompt(prompt = "", {
  ...
  const compact = responseProfile === compactResponseProfile;
  const transportRules = compact
    ? [
...
export async function analyzeCardEvidenceWithOpenAiEmergency({
  ...
  try {
    ...
    const callOpenAi = async (maxOutputTokens, requestedOutputTokens = maxOutputTokens) => {
      ...
      body: JSON.stringify({
        ...
        input: [
          {
            ...
            content: [
              {
                type: "input_text",
                text: openAiStrictSchemaPrompt(prompt, { model: config.model, responseProfile })
              },
        ...
        text: openAiResponsesTextOptions({
          model: config.model,
          name: responseProfile === compactResponseProfile
            ? "listing_provider_evidence_compact"
            : "listing_provider_evidence",
          strict: true,
          schema: responseProfile === compactResponseProfile
    ...
    const canonicalParsed = responseProfile === compactResponseProfile
