...
export function openAiEmergencyConfigFromEnv(env = process.env, { shardKey = "", modelOverride = "" } = {}) {
  ...
  const gpt5MaxOutputTokens = positiveNumberFromEnv(
    ...
    Math.max(40960, defaultMaxOutputTokens)
  );
  const gpt5TruncationRetryMaxOutputTokens = positiveNumberFromEnv(
    env,
    "OPENAI_GPT5_TRUNCATION_RETRY_MAX_OUTPUT_TOKENS",
    Math.max(81920, gpt5MaxOutputTokens, defaultTruncationRetryMaxOutputTokens)
  );
  ...
  return {
    ...
    maxOutputTokens: isGpt5 ? gpt5MaxOutputTokens : defaultMaxOutputTokens,
    truncationRetryMaxOutputTokens: isGpt5 ? gpt5TruncationRetryMaxOutputTokens : defaultTruncationRetryMaxOutputTokens,
