...
const defaultOpenAiMaxOutputTokens = 40960;
const defaultOpenAiTruncationRetryMaxOutputTokens = 81920;
...
export function openAiEmergencyConfigFromEnv(env = process.env, { shardKey = "", modelOverride = "" } = {}) {
  ...
  return {
    ...
    maxOutputTokens: isGpt5 ? Math.min(outputTokenCap, gpt5MaxOutputTokens) : Math.min(outputTokenCap, defaultMaxOutputTokens),
    truncationRetryMaxOutputTokens: isGpt5 ? Math.min(outputTokenCap, gpt5TruncationRetryMaxOutputTokens) : Math.min(outputTokenCap, defaultTruncationRetryMaxOutputTokens),
