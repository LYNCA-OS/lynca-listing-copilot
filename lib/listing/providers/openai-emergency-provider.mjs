...
export function openAiProviderResponseSchema() {
  return {
    ...
    required: ["recognition_status", "fields", "field_evidence", "unresolved"]
  };
}
...
function openAiStrictSchemaPrompt(prompt = "") {
  return [
    ...
    "Return only recognition_status, fields, field_evidence, and unresolved. Do not return title, model_title_suggestion, route, or reason.",
  ...
  try {
    const callOpenAi = async (maxOutputTokens) => {
      ...
      return {
        data,
        text,
        diagnostics: openAiResponseDiagnostics(data, maxOutputTokens),
        truncated: responseLooksTokenTruncated(data, text, maxOutputTokens)
      };
    };

    const initial = await callOpenAi(config.maxOutputTokens);
    const usages = [
    ...
    if (initial.truncated) {
      ...
      active = await callOpenAi(retryCap);
