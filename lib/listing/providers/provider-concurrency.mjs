...
export function providerServerConcurrencyLimit(providerId, env = process.env) {
  if (providerId === visionProviderIds.GEMINI) {
    return positiveInteger(env.GEMINI_PROVIDER_SERVER_CONCURRENCY, 4);
  }
  if (providerId === visionProviderIds.OPENAI_LEGACY) {
    return positiveInteger(env.OPENAI_PROVIDER_SERVER_CONCURRENCY, 6);
  }
  if (providerId === visionProviderIds.AGNES) {
    return positiveInteger(env.AGNES_PROVIDER_SERVER_CONCURRENCY, 2);
  }
