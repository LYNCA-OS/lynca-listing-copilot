...
import { openAiProviderGlobalConcurrency } from "./openai-key-pool.mjs";
...
export async function runWithProviderConcurrency({
  ...
  if (state.active >= limit) {
    if (failIfBusy === true) {
      throw Object.assign(new Error("Provider local capacity is busy."), {
        code: "PROVIDER_LOCAL_CAPACITY_BUSY",
        retryable: false,
        provider_call_attempted: false,
        local_concurrency_limit: limit,
        active_provider_calls: state.active
      });
    }
  ...
  try {
    ...
    return {
      ...
      execution_ms: Math.max(0, completedAt - startedAt)
    };
  } finally {
