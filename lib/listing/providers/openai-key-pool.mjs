...
export function openAiProviderGlobalConcurrency(env = process.env) {
  ...
  if (cleanText(explicitMax)) {
    return positiveInteger(explicitMax, computed, { min: 1, max: 96 });
  }
  if (poolSize > 1) return computed;
  return positiveInteger(env.OPENAI_PROVIDER_SERVER_CONCURRENCY, computed, { min: 1, max: 96 });
}
...
export function selectOpenAiApiKey({
  ...
  const keyIndex = cleanText(shardKey)
  ...
  return {
    ...
    source: keys.length > 1 ? "pool" : "single"
  };
}
