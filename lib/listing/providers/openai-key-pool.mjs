...
export function openAiPerKeyStableConcurrency(env = process.env) {
  return contractedConcurrency("gpt_provider", positiveInteger(
...
export function openAiProviderGlobalConcurrency(env = process.env) {
  ...
  if (cleanText(explicitMax)) {
    return contractedConcurrency(
      "gpt_provider",
      Math.min(computed, positiveInteger(explicitMax, computed, { min: 1, max: 96 }))
    );
  }
  ...
  return contractedConcurrency("gpt_provider", computed);
}
