...
function configuredGpt5ReasoningEffort(env = process.env) {
  const raw = String(env.OPENAI_GPT5_REASONING_EFFORT || "low").trim().toLowerCase();
  return ["minimal", "low", "medium", "high"].includes(raw) ? raw : "low";
}

function configuredGpt5TextVerbosity(env = process.env) {
  const raw = String(env.OPENAI_GPT5_TEXT_VERBOSITY || "low").trim().toLowerCase();
