...
function configuredGpt5ReasoningEffort(env = process.env, model = "") {
  const supported = supportedReasoningEfforts(model);
  const raw = String(env.OPENAI_GPT5_REASONING_EFFORT || "").trim().toLowerCase();
...
export function openAiResponsesModelControls(model, { env = process.env } = {}) {
  if (isGpt5ResponsesModel(model)) {
    return {
      reasoning: {
        effort: configuredGpt5ReasoningEffort(env, model)
      }
    };
  }
  return {
