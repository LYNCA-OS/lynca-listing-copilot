...
export function providerHttpError(provider, status, message = "") {
  const quotaExhausted = status === 429
    && /(?:exceeded your current quota|insufficient[_\s-]*quota|billing details|billing[_\s-]*hard[_\s-]*limit|quota exhausted)/i.test(String(message || ""));
  ...
  return new ProviderError(
    `${provider} request failed: ${status}${message ? ` ${message}` : ""}`,
    {
      ...
      status,
      code,
      retryable: status === 408 || (status === 429 && !quotaExhausted) || status >= 500
    }
  );
}
...
export function providerSchemaError(provider, reason, details = null) {
  return new ProviderError(reason, {
    ...
    // The schema is deterministic, but the model response is not. One fresh
    // provider attempt can recover a malformed structured response without
    // weakening validation or accepting partial data.
    retryable: true,
    details
  });
}
