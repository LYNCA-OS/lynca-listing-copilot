...
import { openAiKeyPoolSize, openAiProviderGlobalConcurrency } from "./openai-key-pool.mjs";
...
export function providerCatalog(env = process.env) {
  ...
  const openAiEnabled = envFlag(
    env,
    "ENABLE_OPENAI_PROVIDER",
    envFlag(env, "ENABLE_GPT41_PROVIDER", envFlag(env, "ENABLE_GPT41_EMERGENCY_PROVIDER", true))
  );
  const openAiExplicitRetryAllowed = envFlag(
    env,
    "ALLOW_EXPLICIT_OPENAI_RETRY",
    envFlag(env, "ALLOW_EXPLICIT_GPT41_RETRY", true)
  );
  const openAiPoolSize = openAiKeyPoolSize(env);
  const openAiGlobalConcurrency = openAiProviderGlobalConcurrency(env);
  const openAiConcurrency = Math.max(1, Math.min(
    numberFromEnv(env, "OPENAI_PROVIDER_UI_CONCURRENCY", openAiGlobalConcurrency),
    openAiGlobalConcurrency
  ));
  ...
  return {
    [visionProviderIds.OPENAI_LEGACY]: {
      ...
      configured: Boolean(openAiPoolSize > 0 && openAiModel.allowed),
      key_pool_size: openAiPoolSize,
