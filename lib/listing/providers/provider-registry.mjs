...
export function providerCatalog(env = process.env) {
  ...
  return {
    [visionProviderIds.CASCADE_FAST]: {
      ...
      configured: Boolean(agnesEnabled && env.OPENAI_API_KEY && openAiModel.allowed && env.AGNES_API_KEY && agnesModel.allowed),
...
export function selectVisionProvider({
  ...
  if (provider.id === visionProviderIds.OPENAI_LEGACY) {
    if (provider.explicit_retry_allowed === false) {
      throw providerUnavailable(provider.id, "GPT-4.1 emergency retry is disabled by env.");
    }

    if (!normalizedRequested || !explicitEmergency) {
      throw new ProviderError("GPT-4.1 legacy may only be used through an explicit emergency retry.", {
