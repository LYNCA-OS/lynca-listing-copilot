...
function directlyObservedMultiCardEvidence(entry = {}) {
  ...
  return /(?:MULTI[_ -]?CARD|PHYSICAL[_ -]?CARD|CARD[_ -]?LAYOUT|SEPARATE\s+(?:CARDS?|SLABS?)|\bLOT\b|\bX\s*[2-9]\b|\b[2-9]\s+(?:CARDS?|SLABS?)\b)/i.test(signal);
}
...
function withProviderFieldSafetyGuards(payload = {}) {
  return withUnsupportedMultiCardRoutedToReview(withUnsupportedCodesRoutedToReview(payload));
}
...
export function validateProviderEvidencePayload(provider, payload) {
  ...
  const normalizedPayload = withProviderFieldSafetyGuards(Array.isArray(payload.field_evidence)
    ? {
      ...payload,
