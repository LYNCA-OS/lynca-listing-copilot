...
function withUnsupportedParallelRoutedToReview(payload = {}) {
  ...
  // Reject the ungrounded open-set finish, but retain a single basic color as
  // the strictly narrower visual fact. This does not promote Wave/Refractor
  // (or any other optical taxonomy) and cannot manufacture a catalog fact.
  const downgradedSurfaceColor = fields.surface_color
    || safeSurfaceColor(unsupported.map((field) => fields[field]).join(" "))
    || null;
  const migratedInsert = fields.insert || unsupported
    .map((field) => resolveKnowledgeEntry(fields[field]))
    .find((entry) => entry?.type === "insert")?.label
    || null;
  ...
  for (const field of unsupported) {
    ...
    const unresolvedField = field === "parallel" ? "parallel_exact" : field;
    if (!unresolved.includes(unresolvedField)) unresolved.push(unresolvedField);
    rejections.push({
      field,
      value,
  ...
  if (downgradedSurfaceColor && !fields.surface_color) {
    fields.surface_color = downgradedSurfaceColor;
    if (resolved && !resolved.surface_color) resolved.surface_color = downgradedSurfaceColor;
  }
  if (migratedInsert && !fields.insert) {
    fields.insert = migratedInsert;
    if (resolved && !resolved.insert) resolved.insert = migratedInsert;
  }
  return {
    ...payload,
    fields,
