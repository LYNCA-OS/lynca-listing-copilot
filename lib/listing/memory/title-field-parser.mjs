...
function titleCase(value) {
  return normalizeText(value)
    ...
    .replace(/\b(?:Nba|Mlb|Nfl|Mls|Ufc|Uefa|Fifa|Wwe|Rc|Fc|Cf|Usa|Ucla|Ii|Iii|Iv)\b/g, (word) => word.toUpperCase())
...
function serialFromTitle(title) {
  const sanitized = String(title || "")
    .replace(/\b(?:PSA|BGS|SGC|CGC|SCD|Beckett)\s*(?:Gem\s+Mint\s+)?(?:AUTO\s*)?\d+(?:\.\d+)?(?:\s*\/\s*\d+(?:\.\d+)?)?\b/gi, " ")
...
function tcgCardNumberFromTitle(title) {
  ...
  const coded = text.match(/\b(?:OP|EB|ST|BT|EX|FB|UA|SV|SWSH|SM|XY|CORI|TAEV|LOB|MRD|PSV|IOC|CRV)[A-Z0-9-]*-\d{3,4}\b/);
  if (coded) return coded[0];
  if (/\bPOK[EÉ]MON\b/i.test(text)) {
    return text.match(/\b\d{2,3}\s*\/\s*\d{2,3}\b/)?.[0]?.replace(/\s+/g, "") || null;
  }
  return null;
}
...
function gradeFromTitle(title) {
  ...
  const match = normalizeText(title).match(/\b(PSA|BGS|SGC|CGC|SCD|Beckett)\s*(?:Gem\s+Mint\s+)?(AUTO\s*)?(\d+(?:\.\d+)?)\b/i);
  if (!match) return {};
...
const canonicalProductAliases = Object.freeze({
  "Topps Bowman University Chrome": "Bowman University Chrome",
  "Bowman University Chrome": "Bowman University Chrome",
  "Topps NBA Hoops": "Topps NBA Hoops",
  "Donruss Road to FIFA World Cup 26'": "Panini Donruss Road to FIFA World Cup",
  "Donruss Road to FIFA World Cup 26": "Panini Donruss Road to FIFA World Cup",
  "Star Court Kings": "Star Court Kings",
...
function inferPublisherFromProduct(product = "") {
  ...
  if (/^(?:Topps\s+)?Bowman\b/i.test(normalized)) return { manufacturer: "Topps", brand: "Bowman" };
  ...
  if (/^Star\b/i.test(normalized)) return { manufacturer: "Star", brand: "Star" };
...
function productHintsFromTitle(title) {
  ...
  const productPatterns = [
    /\bTopps\s+Bowman\s+University\s+Chrome\b/i,
    /\bBowman\s+University\s+Chrome\b/i,
    /\bTopps\s+NBA\s+Hoops\b/i,
    ...
    /\bTopps\s+Chrome\s+Ben\s+Baller\b/i,
    /\bTopps\s+Chrome\s+Dune\b/i,
    ...
    /\bTopps\s+(?:Three|3)(?:\s+Basketball)?\b/i,
    /\bTopps\s+Chrome\s+UFC\b/i,
    /\bTopps\s+Chrome\s+Formula\s+1\b/i,
    ...
    /\bBowman\s+Chrome\s+Sapphire\b/i,
...
function teamFromTitle(title, product = "") {
  const text = stripKnownPhrase(normalizeText(title), product);
...
function parallelExactFromTitle(title) {
  ...
  const colors = [...colorWords]
    .sort((left, right) => right.length - left.length)
    .map((color) => escapeRegExp(color).replace(/\s+/g, "\\s+"))
    .join("|");
  const suffixes = [...parallelSuffixes]
    .sort((left, right) => right.length - left.length)
    .map((suffix) => escapeRegExp(suffix).replace(/\s+/g, "\\s+"))
  ...
  const explicit = text.match(new RegExp(
    `\\b((?:Super\\s+)?(?:${colors})(?:\\s*&\\s*(?:${colors}))?(?:[-\\s]+(?:${suffixes})))\\b`,
    "i"
  ));
  if (explicit) return titleCase(explicit[1]);
  const superColor = text.match(new RegExp(`\\b(Super\\s+(?:${colors}))\\b`, "i"));
  return superColor ? titleCase(superColor[1]) : null;
}
...
function officialCardTypeFromTitle(title) {
  ...
  const known = officialPhrases.find((phrase) => new RegExp(`\\b${escapeRegExp(phrase).replace(/\s+/g, "\\s+")}\\b`, "i").test(text));
  if (known) return known;
  const draftRound = text.match(/\bRound\s+\d{1,2}\s+Pick\s+\d{1,2}\b/i)?.[0];
  return draftRound ? titleCase(draftRound) : null;
}

function releaseVariationFromTitle(title) {
  const text = normalizeText(title);
  const match = text.match(/\b(Design\s+Variation|Photo\s+Variation|Image\s+Variation|Horizontal\s+Variation|Vertical\s+Variation)\b/i);
...
function stripKnownProductPhrase(text, product) {
  const stripped = stripKnownPhrase(text, product);
  const withoutPublisher = normalizeText(product).replace(/^(?:Panini|Topps|Upper\s+Deck|Leaf)\s+/i, "");
  return withoutPublisher && withoutPublisher !== normalizeText(product)
    ? stripKnownPhrase(stripped, withoutPublisher)
    : stripped;
}
...
function maybeSubjectChunk(chunk = "") {
  ...
  // A reviewed catalog title may list several subjects without punctuation.
  // Preserve the composite phrase instead of deleting all subject evidence.
...
function playersFromTitle(title, {
  ...
  text = text
    ...
    .replace(/\b(?:PSA|BGS|SGC|CGC|SCD|Beckett)\s*(?:Gem\s+Mint\s+)?\d+(?:\.\d+)?\s*\/\s*\d+(?:\.\d+)?\b/gi, " ")
    .replace(/\b(?:PSA|BGS|SGC|CGC|SCD|Beckett)\s*(?:Gem\s+Mint\s+)?(?:AUTO\s*)?\d+(?:\.\d+)?\b/gi, " ")
    .replace(/\b\d+\s*\/\s*\d+\b/g, " ")
    .replace(/#\s*\/\s*\d+\b/g, " ")
    .replace(/#\s*[A-Z0-9][A-Z0-9-]{0,18}\b(?!\s*\/)/gi, " ")
    .replace(/\b[A-Z]{1,5}-[A-Z0-9]{1,8}\b/g, " ")
    .replace(/\bCert(?:ificate)?\s*#?\s*[A-Z0-9-]+\b/gi, " ");
  text = stripKnownProductPhrase(text, product);
  text = stripKnownPhrase(text, officialCardType);
  text = stripKnownPhrase(text, parallelExact);
  text = stripKnownPhrase(text, variation);
...
function rarityFromTitle(title) {
  const text = normalizeText(title);
  const match = text.match(/#\s*(C|U|R|M|E|L|SR|SEC|SCR|SP|SSP)\s+[A-Z0-9][A-Z0-9-]{1,14}\b/i);
  if (match) return match[1].toUpperCase();
  const parts = [];
  if (/\bSpecial\s+Art\s+Rare\b/i.test(text)) parts.push("Special Art Rare");
  const abbreviation = text.match(/\b(SAR|SIR|UR|SR|AR|SSP|SP)\b/i)?.[1]?.toUpperCase();
  if (abbreviation) parts.push(abbreviation);
  return unique(parts).join(" / ") || null;
}

function tcgSetFromTitle(title, product = "") {
  const text = normalizeText(title);
  if (!/\bPok[eé]mon\b/i.test(`${text} ${product}`)) return null;
  const known = [
    "Mega Brave",
    "Pokemon 151",
    "Scarlet & Violet",
    "Lost Origin"
  ].find((phrase) => new RegExp(`\\b${escapeRegExp(phrase).replace(/\s+/g, "\\s+")}\\b`, "i").test(text));
  return known || null;
}

function tcgCardNameFromTitle(title, { product = "", set = "", rarity = "" } = {}) {
  const text = normalizeText(title);
  ...
  working = stripKnownPhrase(working, set);
  for (const rarityPart of normalizeText(rarity).split(/\s*\/\s*/).filter(Boolean)) {
    working = stripKnownPhrase(working, rarityPart);
  }
  working = working.replace(/\b(?:19|20)\d{2}(?:-\d{2})?\b/g, " ");
  ...
  working = working.replace(/\b\d{2,3}\s*\/\s*\d{2,3}\b/g, " ");
  ...
  working = working.replace(/\b(?:Surge\s+)?Foil\b|\b(?:Reverse\s+)?Holo\b/gi, " ");
  working = working.replace(/\b(?:Borderless|Showcase|Extended\s+Art|Alternate\s+Art|Alt\s+Art)\b/gi, " ");
  working = working.replace(/\b(?:PSA|BGS|SGC|CGC|SCD|Beckett)\s*(?:Gem\s+Mint\s+)?(?:AUTO\s*)?\d+(?:\.\d+)?\b/gi, " ");
...
export function parseReviewedTitleFields(title = {}) {
  ...
  const parallelExact = parallelExactFromTitle(stripKnownPhrase(text, productHints.product));
  const variation = releaseVariationFromTitle(text);
  const team = teamFromTitle(text, productHints.product);
  const categoryHints = categoryHintsFromTitle(text, productHints.product);
  const rarity = rarityFromTitle(text);
  const tcgSet = tcgSetFromTitle(text, productHints.product);
  const tcgCardName = tcgCardNameFromTitle(text, {
  ...
  const parsedPlayers = playersFromTitle(text, {
    product: productHints.product,
    officialCardType,
    parallelExact,
    variation,
  ...
  const players = categoryHints.category === "tcg" && tcgCardName
    ? [tcgCardName]
    : parsedPlayers;
  ...
  const printRunText = categoryHints.category === "tcg" && tcgCardNumber
    ? stripKnownPhrase(text, tcgCardNumber)
    : text;
  const printRun = expandPrintRunFields({
    print_run_number: serialFromTitle(printRunText) || (/\bOne\s+of\s+One\b/i.test(printRunText) ? "1/1" : null),
    serial_denominator: serialDenominatorFromTitle(printRunText)
  });
  const catalogCardNumber = collectorNumberFromTitle(text) || tcgCardNumber;
  const fields = {
    ...categoryHints,
    ...productHints,
    ...
    collector_number: catalogCardNumber,
    card_number: catalogCardNumber,
