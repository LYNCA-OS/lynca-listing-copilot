...
function serialDenominatorFromTitle(title) {
  ...
  if (full) return full;
  const denomOnly = normalizeText(title).match(/#\s*\/\s*0*(\d+)\b/i);
...
function productHintsFromTitle(title) {
  ...
  const productPatterns = [
    ...
    /\bTopps\s+Chrome\s+Disney\s*100\b/i,
    /\bTopps\s+Chrome\s+(?:Basketball|Football|Baseball|Soccer|Hockey|Tennis)\b/i,
    ...
    /\bTopps\s+Chrome\b/i,
    /\bTopps\s+Finest\s+(?:Basketball|Football|Baseball|Soccer|Hockey|Tennis)\b/i,
    /\bTopps\s+Finest\b/i,
...
function knownMononymSubjects() {
  return new Set(["pele", "pelé", "neymar"]);
}

const subjectNameJoiners = new Set(["al", "da", "de", "del", "di", "du", "la", "le", "van", "von"]);

function normalizedSubjectToken(value = "") {
  return String(value || "").replace(/^[^A-Za-z0-9]+|[^A-Za-z0-9]+$/g, "");
}

function codeShapedSubjectToken(value = "") {
  const token = normalizedSubjectToken(value);
  if (!/[A-Z]/.test(token) || !/^[A-Z0-9]{3,4}$/.test(token)) return false;
  return !/^[IVXLCDM]+$/.test(token);
}

function plausibleSubjectNeighbor(value = "", stopWords = new Set()) {
  const token = normalizedSubjectToken(value);
  const normalized = token.toLowerCase().replace(/[^a-z]/g, "");
  return Boolean(normalized)
    && !stopWords.has(normalized)
    && /^[A-Za-z][A-Za-z'.-]*$/.test(String(value || ""));
}

function isAdjacentFieldCode(words = [], index = -1, stopWords = new Set()) {
  if (!codeShapedSubjectToken(words[index])) return false;
  if (codeShapedSubjectToken(words[index - 1]) || codeShapedSubjectToken(words[index + 1])) return false;
  const previous = normalizedSubjectToken(words[index - 1]).toLowerCase();
  if (previous.length === 1 || subjectNameJoiners.has(previous)) return false;

  // CSM orders set/card codes beside the subject, so an isolated code is not a name word.
  const leftNames = words.slice(0, index).filter((word) => plausibleSubjectNeighbor(word, stopWords));
  const rightNames = words.slice(index + 1).filter((word) => plausibleSubjectNeighbor(word, stopWords));
  return leftNames.length >= 2 || rightNames.length >= 2;
}
...
function maybeSubjectChunk(chunk = "") {
  ...
  const nameWords = words.filter((word, index) => {
    ...
    if (isAdjacentFieldCode(words, index, stopWords)) return false;
