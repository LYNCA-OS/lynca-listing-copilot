...
export function buildV4CandidateControlPlaneTrace(result = {}) {
  ...
  const shadowReranker = shadow
    ? {
      ...
      ranked_candidates: Array.isArray(shadow.ranked_candidates) ? shadow.ranked_candidates.slice(0, 5) : []
    }
    : null;
  ...
  const cardDomainReranker = cardDomainShadow
    ? {
      shadow_only: true,
      status: cardDomainShadow.mode || null,
      ...
      production_decision_affected: false,
      reason: cardDomainSelectedCandidateId ? "shadow_candidate_available" : "fail_closed_no_eligible_domain_candidate",
      ...
      ranked_candidates: Array.isArray(cardDomainShadow.ranked_candidates)
        ? cardDomainShadow.ranked_candidates.slice(0, 5)
        : []
    }
    : null;
