...
export function scoutHasFinalizeAnchors(queryFields = {}, { allowTcgCodeOnly = false } = {}) {
...
function mergedResolvedForFinalize(scoutResolved = {}, candidateFields = {}) {
  ...
  const candidateSubjects = Array.isArray(candidateFields.subjects)
    ? candidateFields.subjects
    : Array.isArray(candidateFields.players)
      ? candidateFields.players
      : [];
  if (candidateSubjects.length) merged.players = candidateSubjects;
...
export async function maybeFinalizeL1FromExactAnchor({
  ...
  const hasCatalogAnchors = allowCatalogFinalize && scoutHasFinalizeAnchors(queryFields, { allowTcgCodeOnly });
  ...
  const eligible = finalizeEligibleExactAnchorCandidates(candidates, { allowTcgCodeOnly });
  const diagnostics = {
    ...baseDiagnostics,
