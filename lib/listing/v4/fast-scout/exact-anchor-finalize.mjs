...
async function fetchCatalogCandidates({ queryFields, env, fetchImpl, timeoutMs }) {
...
export async function maybeFinalizeL1FromExactAnchor({
  ...
  const notFinalized = (reason) => ({ finalized: false, reason });
  ...
  // the strongest identity source and short-circuits the catalog lane; a
  // cert CONFLICT blocks fast-lane finalize entirely.
  const certLane = await timeoutRace(
    tryCertLookupLane({ scoutResult, scoutResolved, env, fetchImpl, timeoutMs, maxLength }),
    timeoutMs
  );
  if (certLane?.finalized || certLane?.review_required) return certLane;
  const queryFields = exactAnchorQueryFieldsFromScout(scoutResolved);
  if (!scoutHasFinalizeAnchors(queryFields)) return notFinalized("scout_missing_exact_anchors");

  const rows = await timeoutRace(
    fetchCatalogCandidates({ queryFields, env, fetchImpl, timeoutMs }),
    timeoutMs
  );
