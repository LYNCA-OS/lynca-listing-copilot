...
function openAiFastScoutConfig(env = process.env, { shardKey = "", modelOverride = "" } = {}) {
  ...
  const baseMaxOutputTokens = positiveInteger(env.OPENAI_FAST_SCOUT_MAX_OUTPUT_TOKENS, 7000);
  const gpt5MaxOutputTokens = positiveInteger(
    env.OPENAI_GPT5_FAST_SCOUT_MAX_OUTPUT_TOKENS,
    Math.max(70000, baseMaxOutputTokens)
  );
  return {
    ...
    maxOutputTokens: isGpt5ResponsesModel(modelId) ? gpt5MaxOutputTokens : baseMaxOutputTokens,
