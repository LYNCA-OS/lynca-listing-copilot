...
function wholeCardScoutPriority(image = {}, index = 0) {
  const text = imageRoleText(image);
  let score = 100 + index / 1000;
  if (/front/.test(text)) score = 0 + index / 1000;
  else if (/back/.test(text)) score = 30 + index / 1000;
  else if (/slab/.test(text)) score = 45 + index / 1000;
  else if (/card|original|primary/.test(text)) score = 60 + index / 1000;

  if (/model_ready|readability|preingest|normalized/.test(text)) score -= 6;
  if (isLocalCropImage(image)) score += 80;
  return score;
}
...
export function selectFastScoutImages(images = [], { maxImages = 1 } = {}) {
  ...
  const selected = (modelReady.length ? modelReady : primary.length ? primary : input)
    ...
    .sort((left, right) => wholeCardScoutPriority(left.image, left.index) - wholeCardScoutPriority(right.image, right.index))
...
async function signedScoutImages(images = [], { env = process.env, fetchImpl = globalThis.fetch, maxImages = 1 } = {}) {
...
export async function runV4FastScoutObservation({
  ...
  const signedImages = await signedScoutImages(payload.images || [], { env, fetchImpl, maxImages: config.maxImages });
