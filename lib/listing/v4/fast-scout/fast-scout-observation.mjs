...
async function assertVerifiedStorageImage(image = {}, {
  ...
  const durableRecord = await readListingImageVerificationRecord({
    tenantId,
    assetId: metadata.assetId || assetId || null,
    objectPath: metadata.objectPath,
    bucket: metadata.bucket,
    contentType: metadata.contentType,
    size: metadata.size,
...
async function signedScoutImages(images = [], {
  ...
  for (const image of selected) {
    ...
    const metadata = await assertVerifiedStorageImage(image, { tenantId, assetId, env, fetchImpl });
...
function fastScoutPrompt({ payload = {}, imageSummary = [] } = {}) {
  return [
    ...
    `Surface color can be one basic color (${basicSurfaceColorsPrompt}) when visually obvious. Do not upgrade color into Refractor/Wave/Shimmer/Mojo/Prizm/Sparkle/Holo unless the exact word is printed.`,
...
export async function runV4FastScoutObservation({
  ...
  if (typeof onSignedImages === "function") onSignedImages(signedImages);
