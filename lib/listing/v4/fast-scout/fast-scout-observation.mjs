...
const gpt5FastScoutOutputTokenCap = gpt5FastScoutSafeOutputTokenCap * 10;
...
function openAiFastScoutConfig(env = process.env, { shardKey = "", modelOverride = "" } = {}) {
  ...
  const baseMaxOutputTokens = positiveInteger(env.OPENAI_FAST_SCOUT_MAX_OUTPUT_TOKENS, 7000000);
  const gpt5MaxOutputTokens = positiveInteger(
    env.OPENAI_GPT5_FAST_SCOUT_MAX_OUTPUT_TOKENS,
    Math.max(700000000, baseMaxOutputTokens)
  );
  const requestedMaxOutputTokens = isGpt5ResponsesModel(modelId) ? gpt5MaxOutputTokens : baseMaxOutputTokens;
  const outputTokenCap = isGpt5ResponsesModel(modelId) ? gpt5FastScoutOutputTokenCap : requestedMaxOutputTokens;
  return {
