...
const gpt5FastScoutSafeOutputTokenCap = 128000;
const gpt5FastScoutOutputTokenCap = gpt5FastScoutSafeOutputTokenCap;
const gpt41FastScoutOutputTokenCap = 32768;
...
function openAiFastScoutConfig(env = process.env, {
  ...
  const model = providerModelConfig(scoutProvider, modelOverride || env.OPENAI_FAST_SCOUT_MODEL || env.OPENAI_LISTING_MODEL);
  const keySelection = selectOpenAiApiKey({ env, shardKey, preferredKeySlot });
  const modelId = model.model_id || defaultProviderModels[scoutProvider];
  const baseMaxOutputTokens = positiveInteger(env.OPENAI_FAST_SCOUT_MAX_OUTPUT_TOKENS, gpt41FastScoutOutputTokenCap);
  const gpt5MaxOutputTokens = positiveInteger(
    env.OPENAI_GPT5_FAST_SCOUT_MAX_OUTPUT_TOKENS,
    gpt5FastScoutOutputTokenCap
  );
  const outputTokenCap = isGpt5ResponsesModel(modelId)
    ? gpt5FastScoutOutputTokenCap
    : /^gpt-4\.1(?:$|-)/i.test(modelId)
      ? gpt41FastScoutOutputTokenCap
      : baseMaxOutputTokens;
  const requestedMaxOutputTokens = effectiveOutputTokens(
    isGpt5ResponsesModel(modelId) ? gpt5MaxOutputTokens : baseMaxOutputTokens,
    outputTokenCap
  );
  return {
    ...
    safeOutputTokenFallback: positiveInteger(env.OPENAI_GPT5_FAST_SCOUT_SAFE_OUTPUT_TOKEN_CAP, gpt5FastScoutSafeOutputTokenCap),
    imageDetail: cleanText(env.OPENAI_FAST_SCOUT_IMAGE_DETAIL || "low").toLowerCase(),
    maxImages: Math.max(1, Math.min(20, positiveInteger(env.OPENAI_FAST_SCOUT_MAX_IMAGES, 10))),
    requestTimeoutMs: Math.max(5000, Math.min(75000, positiveInteger(env.OPENAI_FAST_SCOUT_TIMEOUT_MS, 45000)))
  };
}

function fastScoutOutputCapTooLarge(status, message = "") {
  return Number(status) === 400
    && /max[_\s-]*output[_\s-]*tokens?|output[_\s-]*token|maximum.*tokens?|too many.*tokens?|exceeds.*tokens?/i.test(String(message || ""));
}
...
function responseDiagnostics(data = {}, outputCap = 0, {
  ...
  const optionalUsageNumber = (...values) => {
    const raw = values.find((value) => value !== null && value !== undefined && value !== "");
    if (raw === undefined) return null;
    const number = Number(raw);
    return Number.isFinite(number) && number >= 0 ? number : null;
  };
  const inputTokens = optionalUsageNumber(usage.input_tokens, usage.prompt_tokens);
  const outputTokens = optionalUsageNumber(usage.output_tokens, usage.completion_tokens);
  const totalTokens = optionalUsageNumber(usage.total_tokens);
...
function responseLooksTokenTruncated(diagnostics = {}) {
  if (!/incomplete/i.test(cleanText(diagnostics.response_status))) return false;
...
async function assertVerifiedStorageImage(image = {}, { tenantId, env = process.env, fetchImpl = globalThis.fetch } = {}) {
...
async function signedScoutImages(images = [], {
  ...
  for (const image of selected) {
    ...
    const metadata = await assertVerifiedStorageImage(image, { tenantId, env, fetchImpl });
    addTiming(timing, "image_verify_ms", verifyStartedAt);
    // Client-supplied signed URLs are never reused: the server signs the
    // tenant-verified object path so a valid metadata token cannot be paired
    // with a URL for another tenant's object.
    const signedStartedAt = Date.now();
    const signedUrl = await createListingImageSignedReadUrl({
      objectPath: metadata.objectPath,
      tenantId,
      bucket: metadata.bucket,
      env,
...
export function buildFastScoutListingResult({
  ...
  return {
    ...
    reason: title
      ? "V4 fast scout produced internal routing evidence; the writer-visible title is generated by L2."
      : "V4 fast scout could not produce internal routing evidence.",
    fields: resolved,
    resolved,
    ...
    provider_key_pool_size: keyPoolSize,
    provider_key_slot: keySlot,
    provider_key_source: keySource,
    provider_latency_ms: latencyMs,
    provider_rate_limit_diagnostics: rateLimitDiagnostics,
    provider_request_diagnostics: requestDiagnostics,
...
function cacheKeyFor({ tenantId, signedImages = [], config = {} } = {}) {
  const imagePart = signedImages
    .map((image) => image.content_sha256 || image.object_path || image.image_id)
    .filter(Boolean)
    .join("|");
  return `${tenantId}|${config.model}|${config.imageDetail}|${config.maxImages}|${imagePart}`;
}
...
async function readPersistentCacheWithBudget({
  ...
  try {
    return await Promise.race([
      readV4FastScoutCache({ cacheId, tenantId, env, fetchImpl }),
      timeout
    ]);
  } finally {
...
export async function runV4FastScoutObservation({
  ...
  if (!tenantId) throw new Error("tenant_id is required for V4 fast scout.");
  const providerOptions = payload.provider_options || payload.providerOptions || {};
  const config = openAiFastScoutConfig(env, {
    shardKey: payload.recognition_session_id || payload.asset_id || payload.assetId || "",
    modelOverride: providerModelOverrideFromOptions(providerOptions),
    preferredKeySlot: payload.openai_preferred_key_slot || payload.provider_key_slot_hint || null
  });
  ...
  const cacheKey = cacheKeyFor({ tenantId, signedImages, config });
  ...
  const persistent = await readPersistentCacheWithBudget({ cacheId, tenantId, env, fetchImpl });
  ...
  if (allowProviderCall === false) {
    const error = new Error("fast_scout_cache_miss_provider_disabled");
    error.code = "FAST_SCOUT_CACHE_MISS_PROVIDER_DISABLED";
    error.cache_id = cacheId;
    error.cache_status = persistent.ok ? "MISS" : "READ_ERROR";
    throw error;
  }

  const startedAt = Date.now();
  let outputCapDowngradeAttempts = 0;
  const callOpenAiFastScout = async (maxOutputTokens, {
    requestedOutputCap = maxOutputTokens,
    ...
    try {
      ...
      logOpenAiProviderRequestDiagnostics({
        ...
        phase: "v4_fast_scout",
        attempt,
        responseStatus: error?.name === "AbortError" ? "timeout" : "network_error",
        ...
        requestContext: {
          provider_call_purpose: "fast_scout",
      ...
      const wrapped = new Error(error?.name === "AbortError"
        ? `OpenAI fast scout timed out after ${config.requestTimeoutMs}ms.`
        : `OpenAI fast scout network error: ${cleanText(error?.message || error).slice(0, 160)}`);
      wrapped.code = error?.name === "AbortError" ? "FAST_SCOUT_TIMEOUT" : "FAST_SCOUT_NETWORK_ERROR";
      throw wrapped;
    } finally {
    ...
    if (!response.ok) {
      ...
      if (
        ...
        && fastScoutOutputCapTooLarge(response.status, message)
        && isGpt5ResponsesModel(config.model)
        && Number(config.safeOutputTokenFallback) > 0
        && Number(maxOutputTokens) > Number(config.safeOutputTokenFallback)
      ) {
        outputCapDowngradeAttempts += 1;
        return await callOpenAiFastScout(Number(config.safeOutputTokenFallback), {
          requestedOutputCap,
    ...
    const tokenDiagnostics = responseDiagnostics(data, maxOutputTokens, {
      requestedOutputCap,
    ...
    const requestDiagnostics = openAiProviderRequestDiagnostics({
      ...
      phase: "v4_fast_scout",
      attempt,
      responseStatus: tokenDiagnostics.response_status || "ok",
      tokenDiagnostics,
      rateLimitDiagnostics,
      providerLatencyMs: requestLatencyMs,
      keyPoolSize: config.keyPoolSize,
  ...
  const fastScoutResponse = await callOpenAiFastScout(config.maxOutputTokens, {
    requestedOutputCap: config.requestedMaxOutputTokens
  });
  const {
    data,
    text,
  ...
  if (responseLooksTokenTruncated(tokenDiagnostics)) {
    throw new Error(`OpenAI fast scout response truncated at output cap ${tokenDiagnostics.output_cap || "unknown"}.`);
  }
  ...
  try {
    ...
    throw new Error(`OpenAI fast scout returned invalid JSON after ${tokenDiagnostics.output_tokens || 0} output tokens: ${String(error?.message || error).slice(0, 120)}`);
  }
