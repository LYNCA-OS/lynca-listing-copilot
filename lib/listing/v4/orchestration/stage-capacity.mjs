...
export function listingStageCapacityPlan(env = process.env) {
  ...
  const ocrPerAssetCapacity = positiveInteger(
    ...
    Math.min(2, ocrGlobalCapacity),
    { max: ocrGlobalCapacity }
  );
  const ocrAnchorConcurrency = positiveInteger(env.PREINGESTION_OCR_ANCHOR_CONCURRENCY, 6, {
    max: ocrGlobalCapacity
  });
...
export function ocrPerAssetConcurrencyPlan(plan = {}, {
  anchorJobCount = 0,
  detailJobCount = 0
} = {}) {
  ...
  const perAssetCapacity = Math.min(
    globalCapacity,
    positiveInteger(plan.per_asset_capacity, Math.min(4, globalCapacity), { max: globalCapacity }),
    Math.max(1, totalJobs)
  );
  if (!totalJobs) {
    return { per_asset_capacity: perAssetCapacity, anchor_concurrency: 0, detail_concurrency: 0, local_concurrency: 0 };
  }

  const configuredAnchor = positiveInteger(plan.anchor_concurrency, perAssetCapacity, { max: perAssetCapacity });
  const configuredDetail = positiveInteger(plan.detail_concurrency, 1, { max: perAssetCapacity });
  let anchorConcurrency = 0;
  let detailConcurrency = 0;

  if (anchorJobs && detailJobs) {
    // One detail lane remains live, while the majority of a card's bounded
    // share goes to hard anchors. No single card can occupy the whole global
    // pool, so later cards get a fair chance to start OCR immediately.
    detailConcurrency = Math.min(detailJobs, configuredDetail, Math.max(1, Math.floor(perAssetCapacity / 4)));
    anchorConcurrency = Math.min(anchorJobs, configuredAnchor, Math.max(1, perAssetCapacity - detailConcurrency));
  } else if (anchorJobs) {
    anchorConcurrency = Math.min(anchorJobs, configuredAnchor, perAssetCapacity);
  } else {
    detailConcurrency = Math.min(detailJobs, configuredDetail, perAssetCapacity);
  }

  let remaining = perAssetCapacity - anchorConcurrency - detailConcurrency;
  if (remaining > 0 && anchorJobs > anchorConcurrency) {
    const extra = Math.min(remaining, configuredAnchor - anchorConcurrency, anchorJobs - anchorConcurrency);
    anchorConcurrency += Math.max(0, extra);
    remaining -= Math.max(0, extra);
  }
  if (remaining > 0 && detailJobs > detailConcurrency) {
    const extra = Math.min(remaining, configuredDetail - detailConcurrency, detailJobs - detailConcurrency);
    detailConcurrency += Math.max(0, extra);
  }

  return {
    per_asset_capacity: perAssetCapacity,
