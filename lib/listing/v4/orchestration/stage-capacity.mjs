...
export function listingStageCapacityPlan(env = process.env) {
  // Cloud Run sweep: 8 was the highest zero-timeout point; 10 timed out.
  // Keep the measured value as the default instead of letting later tuning
  // silently move production past the verified capacity knee.
  const ocrGlobalCapacity = contractedConcurrency(
    "paddle_ocr",
    positiveInteger(
      env.PREINGESTION_OCR_GLOBAL_CAPACITY,
      listingConcurrencyContract.paddle_ocr.concurrency,
      { max: 32 }
    )
  );
  const ocrPerAssetCapacity = positiveInteger(
    env.PREINGESTION_OCR_PER_ASSET_CAPACITY,
    listingConcurrencyContract.paddle_ocr.per_asset_concurrency,
    { max: ocrGlobalCapacity }
  );
  const ocrAnchorConcurrency = positiveInteger(
    env.PREINGESTION_OCR_ANCHOR_CONCURRENCY,
    listingConcurrencyContract.paddle_ocr.anchor_lane_limit,
    {
    max: ocrGlobalCapacity
    }
  );
  // These are lane ceilings, not reserved slots. ocrLaneConcurrencyPlan()
  // performs the actual work-conserving allocation and keeps their combined
  // execution at or below global_capacity.
  const ocrDetailConcurrency = positiveInteger(
    env.PREINGESTION_OCR_DETAIL_CONCURRENCY,
    listingConcurrencyContract.paddle_ocr.detail_lane_limit,
    { max: ocrGlobalCapacity }
  );
  ...
  return {
    ocr: {
      ...
      per_asset_batch_size: positiveInteger(
        env.PREINGESTION_OCR_PER_ASSET_BATCH_SIZE,
        listingConcurrencyContract.paddle_ocr.per_asset_batch_size,
        { max: 6 }
      ),
    ...
    catalog: {
      ...
      global_capacity: contractedConcurrency(
        "catalog_retrieval",
        positiveInteger(env.RETRIEVAL_CATALOG_GLOBAL_CAPACITY, listingConcurrencyContract.catalog_retrieval.concurrency, { max: 16 })
      ),
      query_concurrency: contractedConcurrency(
        "catalog_internal_queries",
        positiveInteger(env.RETRIEVAL_INTERNAL_QUERY_CONCURRENCY, listingConcurrencyContract.catalog_internal_queries.concurrency, { max: 12 })
      ),
    ...
    vector: {
      ...
      global_capacity: contractedConcurrency(
        "vector_query",
        positiveInteger(env.VECTOR_QUERY_GLOBAL_CAPACITY, listingConcurrencyContract.vector_query.concurrency, { max: 16 })
      ),
      index_concurrency: contractedConcurrency(
        "vector_index",
        positiveInteger(env.VISUAL_VECTOR_INDEX_CONCURRENCY, listingConcurrencyContract.vector_index.concurrency, { max: 8 })
      ),
...
export function ocrLaneConcurrencyPlan(plan = {}, {
  ...
  const globalCapacity = positiveInteger(plan.global_capacity, 8, { max: 32 });
...
export function ocrGlobalConcurrencyPlan(plan = {}, counts = {}) {
  const lanePlan = ocrLaneConcurrencyPlan(plan, {
    ...counts,
    capacity: positiveInteger(plan.global_capacity, 8, { max: 32 })
  });
  return {
