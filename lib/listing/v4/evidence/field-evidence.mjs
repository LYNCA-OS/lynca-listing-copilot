...
const gradeCertAtomicFields = Object.freeze([
  "grade",
  "grade_company",
  "card_grade",
  "auto_grade",
...
const gradeCertConflictFields = new Set(gradeCertAtomicFields);
...
function evidenceDocumentConflictFields(result = {}) {
  const fields = new Set();
  const surfaces = [
    result.evidence_document,
    result.normalized_evidence_document,
  ...
  for (const surface of surfaces) {
    ...
    const evidence = surface.evidence && typeof surface.evidence === "object" && !Array.isArray(surface.evidence)
      ? surface.evidence
      : surface;
    for (const [field, entry] of Object.entries(evidence)) {
      if (!entry || typeof entry !== "object" || Array.isArray(entry)) continue;
      if (normalizeText(entry.status || entry.display_status).toUpperCase() !== "CONFLICT") continue;
      const normalized = normalizeText(field).toLowerCase();
      if (gradeCertConflictFields.has(normalized)) fields.add(normalized);
    }
  }
  return fields;
}
...
function explicitConflictFields(result = {}) {
  ...
  evidenceDocumentConflictFields(result).forEach((field) => fields.add(field));
  return fields;
}
...
const conflictFieldGroups = Object.freeze([
  ...
  gradeCertAtomicFields,
  ["card_number", "collector_number", "checklist_code", "tcg_card_number"]
]);

export function hasV4GradeCertConflict(result = {}) {
  return [...displayConflictFields(result)].some((field) => gradeCertConflictFields.has(field));
}
...
function displayStatusFor(field, result = {}) {
  ...
  if (key === "grade" && hasV4GradeCertConflict(result)) return "CONFLICT";
