...
import { safeConsensusProductCategoryRefinement } from "../../candidates/candidate-selection-pass.mjs";
...
function sanitizeStructuralFields(fields = {}) {
  ...
  if (/\b(?:SPLT{1,2}NG\.?\s*IMG|PARALLEL(?:\s+NAME)?)\b\s*[-–—:]/i.test(cardName)
      || /^(?:\(?\s*)?(?:no\s+(?:(?:printed|named)\s+)?insert(?:\s+(?:name|title))?|not\s+visible|none\s+visible)(?:\s+visible)?(?:\s*\)?)$/i.test(cardName)) {
...
export function buildV4ResolvedFields(result = {}) {
  const observedFields = currentImageObservationFloor(
  ...
  const explicitConsensus = result.consensus_product_hierarchy_application;
  const terminalConsensusGroups = new Map();
  const terminalTraceRows = Array.isArray(result.candidate_application_trace)
    ? result.candidate_application_trace
    : Array.isArray(result.l2_candidate_debug?.candidate_application_trace)
      ? result.l2_candidate_debug.candidate_application_trace
      : [];
  const terminalDecisionRows = Array.isArray(result.retrieval_application?.decisions)
    ? result.retrieval_application.decisions
    : Array.isArray(result.l2_candidate_debug?.retrieval_application?.decisions)
      ? result.l2_candidate_debug.retrieval_application.decisions
      : [];
  const traces = new Map(terminalTraceRows.map((trace) => [normalizeText(trace?.candidate_id), trace]));
  for (const decision of terminalDecisionRows) {
    if (normalizeText(decision?.candidate_lane).toLowerCase() !== "catalog"
      || normalizeText(decision?.field).toLowerCase() !== "product"
      || normalizeText(decision?.source_trust).toUpperCase() !== "APPROVED_REFERENCE") continue;
    const candidateId = normalizeText(decision?.candidate_id);
    const trace = traces.get(candidateId) || {};
    const agreed = new Set((trace.anchor_agreement?.agreed || []).map(normalizeText));
    const candidateValue = normalizeText(decision?.candidate_value);
    if (!agreed.has("subjects") || !agreed.has("manufacturer")
      || !safeConsensusProductCategoryRefinement(observedFields.product, candidateValue)) continue;
    const key = candidateValue.toLowerCase();
    const group = terminalConsensusGroups.get(key) || { value: candidateValue, identities: new Set() };
    group.identities.add(normalizeText(decision?.candidate_identity_id) || candidateId);
    terminalConsensusGroups.set(key, group);
  }
  const terminalConsensus = [...terminalConsensusGroups.values()]
    .filter((group) => group.identities.size >= 2);
  const consensusValue = explicitConsensus?.status === "ready_safe_refinement"
    && explicitConsensus.renderer_application_allowed === true
    ? normalizeText(explicitConsensus.value)
    : terminalConsensus.length === 1
      ? terminalConsensus[0].value
      : "";
  // Retrieval application owns candidate fields in V4, so the generic final
  // renderer intentionally does not re-run candidate overlays. Consume the
  // already safety-checked release-category consensus at this sole canonical
  // field boundary; no sibling variant fields are carried by this contract.
  const fields = consensusValue
    ? { ...observedFields, product: consensusValue }
    : observedFields;
  return {
    ...fields,
