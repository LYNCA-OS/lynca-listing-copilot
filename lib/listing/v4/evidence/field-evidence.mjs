...
function currentImageObservationFloor(fields = {}, result = {}) {
  ...
  const ledgerSubjectRows = (result.pipeline_node_ledger?.field_flow?.fields || [])
    .filter((row) => normalizeText(row?.field_group || row?.field).toLowerCase() === "subject"
      && row?.raw_provider_present === true);
  const ledgerSubjects = ledgerSubjectRows
    .flatMap((row) => Array.isArray(row.raw_values) ? row.raw_values : []);
  const observedPlayers = normalizedObservedPlayerValues([
    ...observationSources.flatMap((source) => [source.players, source.player]),
    ledgerSubjects
  ]);
  ...
  if (observedPlayers.length) {
    ...
    if (!currentPlayers.length || currentIsObserved && observedPlayers.length > currentPlayers.length) {
      ...
      } else if (observedPlayers.length > 1 && !hasFieldValue(output.lot_type)) {
        output.lot_type = "MULTI_SUBJECT_REVIEW";
      }
    }
  }
  ...
  // ambiguity routes them to review. The display state remains CONFLICT, so
  // this affects the writer draft only; exact parallels, serial numerators,
  // grades, and other high-risk fields are intentionally absent from the floor.
...
export function buildV4FieldStates(result = {}, payload = {}) {
  ...
  const compact = {
    ...
    // The feedback field graph intentionally offers a coarse product-family
    // view. A persisted V4 field state is different: its field_name is a SEM
    // contract and must match the same canonical resolved_fields key exactly.
    // Never relabel set/subset/team text as product merely to fill this state.
    product: resolvedFields.product ?? null,
...
export function buildV4FieldEvidenceRows({ sessionId, result = {}, payload = {} } = {}) {
  ...
  return Object.values(fieldStates).map((state) => ({
    ...
    provenance: {
      source_fields: state.field === "product"
        ? (state.value ? ["product"] : [])
        : fieldGraph.field_nodes?.[state.field]?.source_fields || [],
      evidence_sources: fieldGraph.field_nodes?.[state.field]?.evidence_sources || [],
      structured_only: true
    }
  }));
}
export const v4FieldStateContractVersion = "v4-field-state-canonical-sem-v2";
