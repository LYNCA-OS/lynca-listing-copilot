...
function selectedCandidateOwnsCurrentSourceCorrection(result = {}, field = "") {
  ...
  return trace?.decision_eligible === true
...
function currentImageObservationFloor(fields = {}, result = {}) {
  ...
  const directSurfaceColor = directObservationSources
    ...
    .find(Boolean);
  const authoritativeCatalogColor = hasFieldValue(output.surface_color)
    && selectedCandidateOwnsCurrentSourceCorrection(result, "surface_color");
  if (directSurfaceColor && !authoritativeCatalogColor) output.surface_color = directSurfaceColor;
