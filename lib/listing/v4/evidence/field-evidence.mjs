...
export function buildV4ResolvedFields(result = {}) {
  ...
  const consensus = result.consensus_product_hierarchy_application;
  ...
  const fields = consensus?.status === "ready_safe_refinement"
    && consensus.renderer_application_allowed === true
    && normalizeText(consensus.value)
    ? { ...observedFields, product: normalizeText(consensus.value) }
    : observedFields;
  return {
    ...fields,
