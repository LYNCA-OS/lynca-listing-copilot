...
import { sanitizeProviderModelOverrides } from "../../providers/provider-contract.mjs";

export const v4TitleStages = Object.freeze({
  L0_INSTANT_SKELETON: "L0_INSTANT_SKELETON",
  L1_INTERNAL_SCOUT: "L1_INTERNAL_SCOUT",
...
export function buildV4ModuleSpeedMetrics({ result = {}, routePlan = {}, stageState = {} } = {}) {
  ...
  const internalScout = stageState.title_stage === v4TitleStages.L1_INTERNAL_SCOUT;
  const l1InternalScoutMs = timingValue(timing, "time_to_l1_internal_scout_ms", "fast_scout_latency_ms", "total_ms") ?? observedTotal;
  ...
  return {
    time_to_l1_internal_scout_ms: l1InternalScoutMs,
    time_to_l1_safe_draft_ms: null,
    time_to_l2_assisted_draft_ms: internalScout ? null : (stageState.assisted_draft ? observedTotal : null),
    ...
    time_to_writer_safe_draft_ms: internalScout ? null : observedTotal,
    time_to_assisted_draft_ms: internalScout ? null : (stageState.assisted_draft ? observedTotal : null),
    ...
    fast_scout_input_image_count: Number(result.fast_scout?.input_image_count || 0) || null,
    fast_scout_image_detail: result.fast_scout?.image_detail || null,
    fast_scout_cache_hit: result.fast_scout?.cache_hit === true,
...
export function buildV4TitleStageState({
  ...
  const gradeCertConflict = hasV4GradeCertConflict(result);
  const reviewFields = highRiskReviewFields(result, resolvedFields, [
    ...unresolvedFieldsFrom(result, fieldStates),
    ...(gradeCertConflict ? ["grade", "cert_number"] : [])
  ]);
  const criticalConflict = criticalConflictPresent(result) || gradeCertConflict;
  ...
  const requestedStage = cleanText(result.title_stage);
  const internalScout = requestedStage === v4TitleStages.L1_INTERNAL_SCOUT;
  const stage = failed
    ? v4TitleStages.L0_INSTANT_SKELETON
    : internalScout
      ? v4TitleStages.L1_INTERNAL_SCOUT
      : v4TitleStages.L2_ASSISTED_DRAFT;
  ...
  const state = {
    title_stage: stage,
    writer_safe_draft: failed || internalScout || writerReviewRequired ? "" : (writerSafeReady || title ? title : ""),
    assisted_draft: failed || internalScout || writerReviewRequired ? null : (assisted ? title : null),
      ...
      : internalScout
        ...
        : writerReviewRequired
          ...
          : assisted
            ? "READY"
            : "PENDING",
    assisted_draft_pending_modules: failed
      ? routePlan.background_modules || []
      : writerReviewRequired
        ? []
        : assisted
    ...
    title_stage_reason: failed
      ...
      : writerReviewRequired
        ? "Recognition completed, but grounded evidence was insufficient for a safe title; writer input is required."
      : internalScout
        ? "Internal fast scout is ready for routing and L2 context; writer-visible title is still pending."
        : writerSafeReady
          ? "Writer-visible one-line title is ready after L2 assisted drafting."
          : "Writer-visible one-line title is ready with unresolved or risky fields omitted or marked for review.",
    l1_return_reason: result.l1_return_reason || (failed ? "fast_scout_failed" : internalScout ? "fast_scout_internal_scout_ready" : "l2_assisted_draft_ready"),
    ...
    readiness: {
      ...
      writer_safe_ready: internalScout ? false : writerSafeReady,
      writer_visible_title_ready: !internalScout && !writerReviewRequired && Boolean(title),
      writer_review_required: writerReviewRequired,
      writer_can_start: !internalScout && (Boolean(title) || writerReviewRequired)
    }
  };
...
export function providerOptionsForV4ProgressiveL1({
  ...
  const original = sanitizeProviderModelOverrides(
    payload.provider_options || payload.providerOptions || {},
    { trustedServerEval }
  );
  return {
    ...(original && typeof original === "object" && !Array.isArray(original) ? original : {}),
    v4_module_speed_architecture: true,
    v4_title_stage_target: v4TitleStages.L1_INTERNAL_SCOUT,
...
export function providerOptionsForV4BackgroundL2({
  ...
  const postObservationCatalogVectorHedgeMs = original.post_observation_catalog_vector_hedge_ms
    ?? original.postObservationCatalogVectorHedgeMs
    ?? 100;
  const postObservationRetrievalCriticalPathBudgetMs = original.post_observation_retrieval_critical_path_budget_ms
    ?? original.postObservationRetrievalCriticalPathBudgetMs
    ?? 250;
  const postObservationRetrievalDeadlineEnabled = original.enable_post_observation_retrieval_deadline
    ?? original.enablePostObservationRetrievalDeadline
    ?? true;
  return {
    ...(original && typeof original === "object" && !Array.isArray(original) ? original : {}),
    v4_module_speed_architecture: true,
    v4_title_stage_target: v4TitleStages.L2_ASSISTED_DRAFT,
    enable_post_observation_retrieval_deadline: postObservationRetrievalDeadlineEnabled,
    post_observation_catalog_vector_hedge_ms: postObservationCatalogVectorHedgeMs,
    post_observation_retrieval_critical_path_budget_ms: postObservationRetrievalCriticalPathBudgetMs,
    deferred_modules: [],
