...
function packetUnavailableReasons(packet = {}) {
  ...
  const rowReasons = rows.map((row) => cleanText(row?.reason)).filter(Boolean);
  if (rowReasons.length) return [...new Set(rowReasons)];
  const statusReason = /UNAVAILABLE|TIMEOUT|ERROR/i.test(`${retrieval.status || ""} ${retrieval.status_code || ""}`)
    ? cleanText(retrieval.reason || retrieval.status_code || retrieval.status)
    : "";
  return statusReason ? [statusReason] : [];
}

function plannedSkip({ funnel = {}, packet = {} } = {}) {
  if (funnel.vector_lazy_skip === true) return true;
  const retrieval = packetRetrieval(packet);
  if (cleanText(retrieval.status).toUpperCase() === "DEFERRED_SHADOW") return true;
  const reasons = packetUnavailableReasons(packet);
  return reasons.length > 0 && reasons.every((reason) => (
    /(?:^|_)DISABLED$|^VECTOR_LAZY_|DEFERRED|DEADLINE/i.test(reason)
  ));
}
...
function sourceState({ source, funnel = {}, packet = {}, selected = false, exactAnchor = false } = {}) {
  ...
  const skipped = plannedSkip({ funnel, packet });
  ...
  const degraded = !skipped && unavailableReasons.length > 0 && candidateCount === 0;
