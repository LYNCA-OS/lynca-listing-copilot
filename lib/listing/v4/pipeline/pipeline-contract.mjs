...
const recognizedProductionCandidateHeuristicVersions = new Set([
  candidateSelectionHeuristicVersion,
  oracleCandidateSelectionHeuristicVersion,
  trustedCatalogCandidateSelectionHeuristicVersion
]);
...
function contractViolations({ payload = {}, routePlan = {}, result = {}, stages = [], retrievalSourceContract = {} } = {}) {
  ...
  if (result.selected_candidate_decision
    && !recognizedProductionCandidateHeuristicVersions.has(result.selected_candidate_decision.heuristic_version)) {
    violations.push({
      severity: "ERROR",
