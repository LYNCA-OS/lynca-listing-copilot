...
function candidateStage(result = {}) {
  ...
  return stage({
    ...
    executionMode: candidateCount > 0 ? "TRANSITIONAL_CORE_BRIDGE" : "NOT_RUN",
    reason: candidateCount > 0 ? "candidate_decision_not_atomic" : "no_candidates_available",
    metrics: { candidate_count: candidateCount }
  });
}

function contractViolations({ payload = {}, routePlan = {}, result = {}, stages = [] } = {}) {
  const violations = [];
...
export function buildV4PipelineContract({
  ...
  const stages = [
    ...
    stage({
      ...
      executionMode: exactAnchor ? "NATIVE_V4_EXACT_ANCHOR" : "TRANSITIONAL_CORE_BRIDGE",
      reason: exactAnchor ? "provider_not_required" : "provider_and_evidence_completion_still_share_transitional_core",
    ...
    stage({
      ...
      executionMode: exactAnchor ? "NATIVE_V4_EXACT_ANCHOR" : hasRetrieval ? "TRANSITIONAL_CORE_BRIDGE" : "NOT_RUN",
    ...
    stage({
      ...
      executionMode: exactAnchor ? "NATIVE_V4_EXACT_ANCHOR" : "TRANSITIONAL_CORE_BRIDGE",
      reason: exactAnchor ? null : "identity_resolution_still_invoked_inside_transitional_core",
    ...
    stage({
      ...
      executionMode: result.title_render_source === "v4_csm_deterministic_renderer"
        || /anchor_catalog_finalized/.test(result.title_render_source || "")
        ? "NATIVE_V4"
  ...
  const violations = contractViolations({ payload, routePlan, result, stages });
  const bridgedStages = stages
    .filter((node) => node.execution_mode === "TRANSITIONAL_CORE_BRIDGE")
