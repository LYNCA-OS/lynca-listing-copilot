...
export async function tryExactIdentityFastFinal({
  ...
  return runImmutableStage({
    ...
    execute: async ({ payload: immutablePayload, preProviderRescanResult: rescan }) => {
      const [approvedMemoryResult, identityCacheLookup] = await Promise.all([
      ...
      const result = approvedMemoryResult || identityCacheResult || rescan || null;
      const route = approvedMemoryResult
        ? "APPROVED_IDENTITY_MEMORY"
        : identityCacheResult
          ? "IDENTITY_RESULT_CACHE"
          : rescan
            ? "PRE_PROVIDER_RESCAN"
            : "FULL_PROVIDER_REQUIRED";
      return {
        result,
