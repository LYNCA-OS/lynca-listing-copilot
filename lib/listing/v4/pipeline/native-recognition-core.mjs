...
import { waitForV4LateProviderCapacity } from "../jobs/late-provider-capacity.mjs";
import {
...
function finalizeListingResultForResponse(result, payload = {}) {
  const terminalIdentityCacheReplay = result?.replay_class === "TERMINAL_L2_IDEMPOTENT"
    && result?.identity_cache?.cache_hit === true;
  // A terminal L2 cache row is the exact writer-visible response snapshot.
  // Re-running Resolver/Renderer here can change deliberately suppressed
  // entity fields (for example #/5 back into 5/5), violating idempotence.
  return terminalIdentityCacheReplay
    ? result
    : finalizeDeterministicPresentation(result, payload);
}

async function buildListingResult(statusCode, result, timingContext, payload = {}) {
  const sidecarEnv = evaluationTelemetryEnvForPayload(payload, process.env);
  if (sidecarEnv !== process.env) {
    addTiming(timingContext, "evaluation_telemetry_write_suppressed_count", 1);
  }
  const finalizedResult = finalizeListingResultForResponse(result, payload);
...
async function withIdentityCacheWrite(result, payload, lookupTelemetry = {}) {
  // The cache is a terminal L2 replay surface. Persist the same deterministic
  // snapshot returned to the writer, not the pre-presentation provider draft.
  // Otherwise a cache hit can legitimately skip the provider while replaying
  // an older title and resolver state.
  const terminalResult = finalizeDeterministicPresentation(result, payload);
  const providerOptions = providerOptionsFromPayload(payload);
  if (identityCacheWriteBypassed(providerOptions)) {
    ...
    return {
      ...
      identity_cache: {
        ...lookupTelemetry,
        ...(terminalResult.identity_cache || {}),
        cache_hit: false,
        read_bypassed: lookupTelemetry.read_bypassed === true,
        miss_reason: lookupTelemetry.miss_reason || null,
  ...
  if (!identityResultCacheWriteEnabled() || !isSupabaseFeedbackConfigured()) {
    return {
      ...
      identity_cache: {
        ...lookupTelemetry,
        ...(terminalResult.identity_cache || {}),
        cache_hit: terminalResult.identity_cache?.cache_hit === true,
        provider_call_skipped: false,
        write_attempted: false,
        write_saved: false,
...
async function prepareCatalogCandidateContext({
  ...
  if (cacheEnabled && cacheKey) {
    ...
    if (cached && cached.expires_at_ms > cacheStartedAt) {
      ...
      addTiming(timingContext, "catalog_cache_hit_count", 1);
      return {
        ...cached.context,
    ...
    addTiming(timingContext, "catalog_cache_ms", Date.now() - cacheStartedAt);
    addTiming(timingContext, "catalog_cache_miss_count", 1);
  } else if (!cacheEnabled) {
    addTiming(timingContext, "catalog_cache_bypass_count", 1);
  }
...
export function evaluationTelemetryEnvForPayload(payload = {}, env = process.env) {
  const options = providerOptionsFromPayload(payload);
  if (options.disable_nonessential_evaluation_writes !== true) return env;
  return {
    ...env,
    VECTOR_QUERY_LOG_ENABLED: "false",
    DATA_LOOP_SIDECARS_ENABLED: "false",
    DATA_LOOP_WORKFLOW_EVENT_LOG_ENABLED: "false",
    DATA_LOOP_INTERNAL_ANNOTATION_QUEUE_ENABLED: "false",
    DATA_LOOP_EXTERNAL_TASK_CREATION_ENABLED: "false"
  };
}
...
async function prepareVectorCandidateContext({
  ...
  if (!hasUsableVisualFeatures(activeVisualFeatures)) {
    ...
    const telemetry = await recordVectorRetrievalTelemetry({
      ...
      env: evaluationTelemetryEnvForPayload(initialPayload, env)
    });
    return {
      mode: config.mode,
      visualFeatures: activeVisualFeatures,
...
async function createOpenAiTitle(payload, selection, {
  ...
  const providerResponseProfile = ultraSparseTransport
    ...
    : ["v4_compact_l2", "v4_ultra_fast_l2"].includes(providerPromptMode)
      ? "compact_sparse_v1"
      : "standard";
  let providerCapacityLateBinding = null;
  if (payload.v4_late_provider_lease_binding === true) {
    const waitingProviderAt = new Date().toISOString();
    providerCapacityLateBinding = await waitForV4LateProviderCapacity({
      ...
      timeoutMs: Number(process.env.V4_LATE_PROVIDER_CAPACITY_WAIT_MS || 30_000),
      pollMs: Number(process.env.V4_LATE_PROVIDER_CAPACITY_POLL_MS || 200),
      signal: requestContext?.signal || null,
      env: process.env,
      fetchImpl: globalThis.fetch
    });
    providerCapacityLateBinding.waiting_provider_at = waitingProviderAt;
    initialPayload.openai_preferred_key_slot = providerCapacityLateBinding.provider_key_slot;
    initialPayload.provider_key_slot_hint = providerCapacityLateBinding.provider_key_slot;
    initialPayload.provider_capacity_slot = providerCapacityLateBinding.provider_capacity_slot;
  }
  ...
  let providerResultWithEvidence = timeSync(timingContext, "renderer_ms", () => ({
    ...
    provider_capacity_late_binding: providerCapacityLateBinding
  }));
...
export const __listingCopilotTitleTestHooks = {
  ...
  finalizeListingResultForResponse,
