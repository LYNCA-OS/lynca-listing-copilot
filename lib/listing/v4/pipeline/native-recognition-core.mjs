...
import { buildPreProviderRescueShadow } from "../risk/pre-provider-rescue-shadow.mjs";
import {
...
function currentSourceFeedbackExclusions(payload = {}, providerOptions = {}) {
  if (optionFlag(providerOptions, "exclude_current_source_feedback", false) !== true) return [];
  return [payload.source_feedback_id, payload.sourceFeedbackId].filter(Boolean);
}
...
function finalizerSafeRawProviderFields(result = {}) {
  const raw = result.raw_provider_fields;
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return raw;
  const rejectedAuto = (Array.isArray(result.provider_field_rejections)
    ? result.provider_field_rejections
    : []).some((entry) => entry?.field === "auto"
      && entry?.reason === "auto_not_directly_supported_by_current_image");
  if (!rejectedAuto) return raw;
  return {
    ...raw,
    auto: false,
    observable_components: Array.isArray(raw.observable_components)
      ? raw.observable_components.filter((value) => !/^auto$/i.test(String(value || "").trim()))
      : raw.observable_components
  };
}
...
function finalResolvedFieldsForPresentation(result = {}, {
  ...
  const safeRawProviderFields = finalizerSafeRawProviderFields(result);
  ...
  const merged = rest.reduce((current, fields) => (
    finalizerMergeCurrentImageFields(current, fields, supportFields, {
      allowExactCodePromotion: fields !== safeRawProviderFields
    })
  ), { ...base });
...
function applyOpenSetAssistShadowPresentationGuard(result = {}, payload = {}) {
  ...
  const oracleEvaluation = result.evaluation_profile === "v4_accuracy_ceiling_oracle_v1"
    || payload.evaluation_profile === "v4_accuracy_ceiling_oracle_v1";
  const applyWriterSuggestionToPresentation = Boolean(writerSuggestion) && !oracleEvaluation;
  ...
  const writerPresentationFields = applyWriterSuggestionToPresentation
    ? {
  ...
  const writerPresentationEvidence = applyWriterSuggestionToPresentation && writerSuggestionEvidence
    ? {
  ...
  const renderedTitle = applyWriterSuggestionToPresentation
    ? presentationTitle
  ...
  return {
    ...
    title_render_source: applyWriterSuggestionToPresentation
    ...
    open_set_presentation_guard: {
      used: true,
      reason: guardReason,
      action: applyWriterSuggestionToPresentation
        ? "kept_visual_parallel_in_writer_presentation_only"
        : writerSuggestion && oracleEvaluation
          ? "kept_visual_parallel_as_review_metadata_only"
    ...
    rendered_fields: {
      ...
      ...(applyWriterSuggestionToPresentation ? { parallel_exact: writerSuggestion.value } : {}),
      title: renderedTitle,
      rendered_title: renderedTitle,
      modules: presentation.modules,
      module_order: presentation.module_order,
      title_render_source: applyWriterSuggestionToPresentation
        ? "open_set_writer_review_suggestion"
        : "open_set_narrow_parallel_guard",
      fields: applyWriterSuggestionToPresentation
...
export function catalogRetrievalFamiliesForFields(fields = {}, {
  ...
  // prompt-expansion exercise. Keep at most two complementary catalog RPCs:
  // one identity/set lane and one serial-variant lane. Running all overlapping
  // families made 5-7 calls compete with the fixed deadline, while using only
  // the denominator lane lost informative inserts such as Rain Drops.
  ...
  if (hasSubject && fieldHasValue(normalized, "set", "subset", "insert")) {
    return [...baseFamilies, retrievalQueryFamilies.CATALOG_SET_SUBJECT];
  }
...
function catalogCandidateContextCacheKey({
  ...
  const currentSourceFeedbackExclusions = [...new Set(
    (Array.isArray(excludeSourceFeedbackIds) ? excludeSourceFeedbackIds : [excludeSourceFeedbackIds])
  ...
  const keyPayload = {
    ...
    current_source_feedback_exclusions: currentSourceFeedbackExclusions
  };
...
function rebindVectorCandidateContextToFields(context = null, queryFields = {}, {
  ...
  const config = vectorRetrievalConfig(env, providerOptions);
  const decisionCandidateLimit = providerOptions.evaluation_profile === "v4_accuracy_ceiling_oracle_v1"
    ? Math.max(20, config.gptCandidateLimit)
    : config.gptCandidateLimit;
  const packet = buildVectorCandidatePacket(context.retrieval, {
    limit: decisionCandidateLimit,
    queryFields: queryFields || {}
  });
  const assistEligibility = vectorCandidatePacketAssistEligibility(packet);
  const assistPacket = buildVectorCandidateAssistPacket(packet, { limit: config.gptCandidateLimit });
  return {
    ...context,
    packet,
    assistPacket,
...
function preingestionOcrPostProviderWaitMs(env = process.env, providerOptions = {}) {
  ...
  const oracle = String(providerOptions.evaluation_profile || "").trim() === "v4_accuracy_ceiling_oracle_v1";
  return Math.min(oracle ? 90_000 : 10_000, Math.trunc(parsed));
}
...
async function prepareVectorCandidateContext({
  ...
  const retrieval = await timeAsync(timingContext, "vector_retrieval_ms", () => runRetrieval({
    ...
    excludeSourceFeedbackIds: currentSourceFeedbackExclusions(initialPayload, providerOptions)
  }));
  const packet = buildVectorCandidatePacket(retrieval, {
    limit: providerOptions.evaluation_profile === "v4_accuracy_ceiling_oracle_v1"
      ? Math.max(20, config.gptCandidateLimit)
      : config.gptCandidateLimit,
    queryFields: resolvedForRetrieval || {}
  });
...
function candidateControlResultPatch(candidateControl = {}) {
  return {
    ...
    consensus_product_hierarchy_application: candidateControl.consensus_product_hierarchy_application,
...
function withCandidateControl(result = {}, context = {}, {
  ...
  const evaluationProfile = String(context.providerOptions?.evaluation_profile || "").trim();
  const controlledInputBase = evaluationProfile && !result.evaluation_profile
    ? { ...result, evaluation_profile: evaluationProfile }
    : result;
  const controlledInput = context.sourceFeedbackId && !controlledInputBase.source_feedback_id
    ? { ...controlledInputBase, source_feedback_id: context.sourceFeedbackId }
    : controlledInputBase;
  const candidateControl = v4ProductionStrategy.candidate_control.select({
    ...
    diagnosticCandidateLimit: evaluationProfile === "v4_accuracy_ceiling_oracle_v1" ? 20 : 5,
    cardDomainSelectionMode: String(context.providerOptions?.card_domain_selection_mode || "shadow_only").trim()
  });
  const controlled = {
    ...controlledInput,
...
async function withEvidenceCompletion(result, payload, {
  ...
  const completion = await timeAsync(timingContext, "evidence_completion_ms", () => completeEvidence({
    ...
    env: retrievalEnv,
    excludeSourceFeedbackIds: currentSourceFeedbackExclusions(payload, providerOptions),
...
async function imagesWithSignedReadUrls(images = [], timingContext = null, tenantId = "") {
  return timeAsync(timingContext, "signed_url_ms", () => mapWithConcurrency(images, signedUrlConcurrency, async (image) => {
    ...
    const reusableSignedUrl = image.signedUrl || image.signed_url || "";

    return {
      ...image,
      signedUrl: reusableSignedUrl || await createListingImageSignedReadUrl({
          tenantId,
          objectPath: metadata.objectPath,
          bucket: metadata.bucket
        }),
...
async function createOpenAiTitle(payload, selection, {
  ...
  const providerOptions = providerOptionsFromPayload(payload);
  const requireTerminalPreingestionOcr = optionFlag(
    providerOptions,
    "require_terminal_preingestion_ocr",
    false
  );
  ...
  const preingestionOcrRendezvousPromise = preingestionBundleId
    ? waitForPreingestionOcrEvidence({
      ...
      timeoutMs: requireTerminalPreingestionOcr
        ? preingestionOcrPostProviderWaitMs(process.env, providerOptions)
        : positiveIntegerFromEnv(process.env, "PREINGESTION_OCR_RENDEZVOUS_TIMEOUT_MS", 22_000),
      pollMs: positiveIntegerFromEnv(process.env, "PREINGESTION_OCR_RENDEZVOUS_POLL_MS", 400),
      env: process.env,
      fetchImpl: globalThis.fetch,
      triggerSweep: true,
      requireTerminal: requireTerminalPreingestionOcr,
      onState: (state) => {
  ...
  const preProviderRescueShadow = timeSync(
    timingContext,
    "pre_provider_rescue_shadow_ms",
    () => buildPreProviderRescueShadow({
      enabled: optionFlag(providerOptions, "enable_pre_provider_rescue_shadow", false),
      resolvedFields: resolvedForRetrieval,
      confirmedPreingestionFields: confirmedPreingestionRetrievalFields(initialPayload),
      catalogContext,
      vectorContext,
      preingestionBundlePresent: Boolean(preingestionBundleId)
    })
  );
  ...
  const requestedProviderServiceTier = ultraFastL2
    ? ultraFastServiceTier(providerOptions)
    : providerServiceTier(providerOptions);
  ...
  const providerResult = await runTimedProviderCall(visionProviderIds.OPENAI_LEGACY, timingContext, () => analyzeCardEvidenceWithOpenAiEmergency({
    ...
    serviceTier: requestedProviderServiceTier,
  ...
  if (deferVectorUntilProviderObservation) {
    let terminalOcrBeforeRetrieval = null;
    if (requireTerminalPreingestionOcr && preingestionBundleId) {
      const oracleOcrBeforeRetrievalStartedAt = nowMs();
      terminalOcrBeforeRetrieval = await preingestionOcrRendezvousPromise;
      addTiming(
        timingContext,
        "oracle_ocr_before_retrieval_wait_ms",
        nowMs() - oracleOcrBeforeRetrievalStartedAt
      );
    }
    ...
    // The OCR rendezvous already owns the terminal, current-contract snapshot.
    // A bundle read immediately after the worker commits can still observe an
    // older row through the REST visibility path. Never let that transient
    // empty refresh erase evidence which this request has already received.
    const terminalOcrPatches = Array.isArray(terminalOcrBeforeRetrieval?.evidence_patches)
      ? terminalOcrBeforeRetrieval.evidence_patches
      : null;
    if (terminalOcrPatches) {
      const rendezvousRefresh = applyPreIngestionEvidencePatchesToPayload(
        initialPayload,
        terminalOcrPatches,
        { source: "terminal_ocr_rendezvous_before_retrieval" }
      );
      preingestionRetrievalRefresh = {
        ...(preingestionRetrievalRefresh || {}),
        ...rendezvousRefresh,
        bundle_refresh_patch_count: preingestionRetrievalRefresh?.patch_count ?? null,
        source: rendezvousRefresh.source
      };
    }
    ...
    const providerObservationRetrievalFields = mergeCurrentFieldsForTitleAssist(
      retrievalFieldsFromProviderObservation(providerResultWithEvidence, resolvedForRetrieval),
      confirmedRetrievalFields
    );
    const retrievalContextText = requireTerminalPreingestionOcr
      ? retrievalOnlyOcrContextFromPayload(initialPayload)
      : "";
    providerResolvedForRetrieval = retrievalContextText
      ? { ...providerObservationRetrievalFields, retrieval_context_text: retrievalContextText }
      : providerObservationRetrievalFields;
  ...
  // Accuracy-ceiling runs must wait for the original all-job rendezvous.
  // A provider-specific targeted rescue (for example missing grade) is a
  // production latency optimization and must never replace the Oracle's
  // requireTerminal contract.
  const ocrPostProviderWaitMs = requireTerminalPreingestionOcr
    ? configuredOcrPostProviderWaitMs
    : criticalOcrWait.wait_budget_ms;
  ...
  const targetedOcrRendezvousPromise = !requireTerminalPreingestionOcr
    && criticalOcrWait.target_fields.length
    && preingestionBundleId
    ? waitForPreingestionOcrEvidence({
      ...preingestionOcrScope,
      bundleId: preingestionBundleId,
  ...
  mergedResult.pre_provider_rescue_shadow = preProviderRescueShadow;
