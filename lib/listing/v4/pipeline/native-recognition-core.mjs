...
export async function completedKnowledgeFirstRouteShadowForEvaluation(
  ...
  try {
    return await completedDecisionPromise;
  } catch {
...
async function createOpenAiTitle(payload, selection, {
  ...
  if (recognitionPreflightPromise) {
    ...
    const recognitionJoinWaitMs = nowMs() - recognitionJoinStartedAt;
  ...
  try {
    ...
    const result = await runWithInFlightIdentityRequest({
      ...
      run: async () => {
        ...
        let knowledgeFirstRouteShadow = initialKnowledgeFirstRouteShadow;
        const knowledgeFirstRouteShadowPromise = Promise.all([
          ...
          knowledgeFirstRouteShadow = decision;
          return decision;
        }).catch(() => initialKnowledgeFirstRouteShadow);
        // Normal production never joins counterfactual telemetry. Cold
        // evaluation may join it after the Provider has completed so the
        // persisted route contains the Worker evidence it is meant to audit.
