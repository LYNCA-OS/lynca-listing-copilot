...
import {
  ...
  providerReasoningEffortOverrideFromOptions,
  visionProviderIds
...
async function createOpenAiTitle(payload, selection, {
  ...
  const runFullProvider = async () => {
    ...
    return analyzeCardEvidenceWithOpenAiEmergency({
      ...
      reasoningEffortOverride: providerReasoningEffortOverrideFromOptions(providerOptions),
  ...
  let providerResultWithEvidence = timeSync(timingContext, "renderer_ms", () => ({
    ...
    provider_reasoning_effort: providerResult.reasoning_effort || null,
  ...
  try {
    ...
    if (exactIdentityResult) {
      const knowledgeFirstRouteShadow = planKnowledgeFirstWithProviderAuxShadow({
        ...
        higherAuthorityRoute: exactIdentityStage.output?.reason_codes?.[0] || "EXACT_IDENTITY_REUSE",
