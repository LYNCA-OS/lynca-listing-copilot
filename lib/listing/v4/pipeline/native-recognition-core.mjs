...
export async function snapshotRecognitionPreflightAfterProvider({
  ...
  const snapshot = await waitForPromiseWithin(recognitionPreflightPromise, 0);
  if (snapshot.settled) {
    return {
    ...
    const result = await runWithInFlightIdentityRequest({
      ...
      run: async () => {
        ...
        const recognitionPreflightPromise = createRecognitionIdentityPreflight(payload, {
          // A late shadow completion must not mutate the already-finalized
          // writer timing ledger. Its own structured timestamps live in the
          // preflight diagnostics snapshot instead.
        ...
        const constraintModelPromise = (typedRouteEvaluationEnabled
          ? timeAsync(
            timingContext,
            "knowledge_first_constraint_model_load_ms",
        ...
        // completed evidence is recorded, while a late Worker remains a
        // structured background trace and cannot delay writer-ready.
