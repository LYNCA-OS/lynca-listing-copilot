...
} from "../../evaluation/evaluation-decision-trace-packet.mjs";
import { runSecondLookEvaluationShadow } from "../../evaluation/second-look-shadow-evaluation.mjs";
import { loadConstraintModelSnapshot } from "../../catalog/constraint-model-store.mjs";
import {
  attachForwardEnumerationCandidates,
  forwardEnumerationExperimentArm,
  forwardEnumerationExperimentArms,
  forwardEnumerationTraceForResult,
  mergeForwardEnumerationApplicationEvidence
} from "../../catalog/forward-enumeration-adapter.mjs";
import {
  attachWorldKnowledgeShadowAssistInput,
  worldKnowledgeShadowAssistRequested
} from "../../knowledge/world-knowledge-layer.mjs";
...
import {
  ...
  fieldNormalizationPolicyVersion,
  normalizeFields,
...
import {
  ...
  targetedAssistEvaluationProfileEnabled
} from "../targeted-assist/targeted-assist-route-executor.mjs";
import { secondLookShadowEvaluationProfileEnabled } from "../targeted-assist/second-look-shadow-executor.mjs";
import {
...
import {
  providerEvidenceNormalizationVersion,
  providerPayloadToEvidenceDocument,
  resolvedFieldsToLegacyFields
} from "../../evidence/provider-evidence-normalizer.mjs";
...
import {
  readWriterFinalReplayRecord,
  writerFinalReplayRecordToListingResult
} from "../../cache/writer-final-replay.mjs";
import {
  identityInFlightCoalescingEnabled,
  runWithInFlightIdentityRequest
} from "../../cache/inflight-identity-request.mjs";
import { attachActiveCatalogSnapshotRevision } from "../../catalog/active-catalog-snapshot.mjs";
import { tryExactIdentityReuseAndAnchorShadow } from "./native-recognition-stages.mjs";
import {
  planKnowledgeFirstRecognition,
  publishableForwardEnumerationClaim
} from "../route-planner/knowledge-first-route-policy.mjs";
import { planProviderAuxRouteShadow } from "../route-planner/provider-aux-route-shadow.mjs";
import {
...
export function verifiedSerialNumeratorFromPreingestion(payload = {}) {
  ...
  // A busy card back yields spurious serial-shaped tokens (copyright years,
  // cert numbers, "13/10" component codes). When several distinct values
  // appear, a value read on the dedicated high-confidence serial crop is
  // authoritative over full-image noise, so resolve the conflict in its favor
  // instead of discarding a genuine numerator observation.
  let only;
  if (fullValues.size === 1) {
    only = [...fullValues.values()][0];
  } else {
    // Only a dedicated serial-crop read outranks full-image noise. If two or
    // more distinct values were each seen on a serial crop, the crops disagree
    // and the numerator stays a genuine conflict.
    const directCropValues = [...fullValues.values()].filter((entry) => (
      entry.direct_crop_observation_keys.size >= 1
    ));
    if (directCropValues.length !== 1 || directCropValues[0].confidence < singleCropOcrSerialConfidence) {
      return {
        verified: false,
        value: null,
        confidence: 0,
...
function finalizeDeterministicPresentation(result = {}, payload = {}) {
  ...
  const effectiveSerialNumeratorVerified = result.serial_numerator_verified
    ?? result.serialNumeratorVerified
    ?? serialNumeratorVerificationFromPayload(payload);
  const effectiveTerminalRendererInputs = {
    max_title_length: payload.maxTitleLength || maxFallbackTitleLength,
    serial_numerator_verified: effectiveSerialNumeratorVerified,
  ...
  const presentation = renderListingPresentation({
    ...
    maxLength: effectiveTerminalRendererInputs.max_title_length,
    ...
    serialNumeratorVerified: effectiveSerialNumeratorVerified
  });
  ...
  return {
    ...
    effective_terminal_renderer_inputs: effectiveTerminalRendererInputs,
...
function withoutEvaluationDecisionTracePacket(result = {}) {
  const {
    evaluation_decision_trace_packet: _staleEvaluationDecisionTracePacket,
    ...cleanResult
...
async function buildListingResult(statusCode, result, timingContext, payload = {}) {
  if (!recognitionBoundaryRecorded(timingContext, "decision_ready")) {
    markRecognitionBoundary(timingContext, "decision_ready", {
      metadata: { route: "result_boundary_fallback" }
    });
  }
  ...
  const timedResult = withTiming({
    ...
    recognition_benchmark_profile: providerOptionsFromPayload(payload).recognition_benchmark_profile || null,
    recognition_benchmark_phase: providerOptionsFromPayload(payload).recognition_benchmark_phase || null
  }, timingContext);
  ...
  const finalResult = {
    ...withoutEvaluationDecisionTracePacket(workflowResult),
    // Evaluation replay must be self-describing on every terminal route. Some
    // full-provider paths already carry these versions from normalization,
    // while downstream sidecars and alternate finalizers do not. Stamp the
    // canonical owner versions at the single result boundary before tracing.
    normalization_version: workflowResult.normalization_version || fieldNormalizationPolicyVersion,
    candidate_policy_version: workflowResult.candidate_policy_version
      || v4ProductionStrategy.profile.candidate_control_policy_version,
    resolver_version: workflowResult.resolver_version
      || workflowResult.identity_resolution_version
      || identityResolverPolicyVersion,
    timing: finalizeTiming(timingContext, workflowResult)
  };
  markRecognitionBoundary(timingContext, "core_terminal_ready");
  finalResult.pipeline_node_ledger = buildPipelineNodeLedger({
    result: finalResult,
    timingContext,
    payload
  });
  const evaluationDecisionTracePacket = buildEvaluationDecisionTracePacket(finalResult, payload);
  if (evaluationDecisionTracePacket) {
    finalResult.evaluation_decision_trace_packet = evaluationDecisionTracePacket;
  }
  const providerFailureCode = finalResult.provider_error_type || finalResult.provider_error_code || null;
  setRecognitionPathOutcome(timingContext, {
    status: providerFailureCode ? "FAILED" : "COMPLETED",
    reason: providerFailureCode
  });
  markRecognitionBoundary(timingContext, "response_built");
  const recognitionCriticalPath = snapshotRecognitionCriticalPath(timingContext);
  finalResult.recognition_critical_path = evaluationTraceEnabled(payload)
    ? recognitionCriticalPath
    : compactRecognitionCriticalPath(recognitionCriticalPath);
...
async function createApprovedMemoryTitle(payload) {
  ...
  try {
    lookup = await lookupApprovedIdentityMemory({
      ...
      verifyImages: payload.exact_identity_storage_verified === true
        ? async () => ({ ok: true })
        : verifyApprovedMemoryImages
    });
  } catch {
    return null;
  }
...
async function createWriterFinalReplayTitle(payload) {
  const options = providerOptionsFromPayload(payload);
  if (optionFlag(options, "disable_writer_final_replay", false)) return null;
  if (!isSupabaseFeedbackConfigured()) return null;
  const key = buildIdentityResultCacheKey(payload);
  if (!key.ok || !key.tenant_id) return null;
  const startedAt = Date.now();
  try {
    const lookup = await readWriterFinalReplayRecord({
      tenantId: key.tenant_id,
      imageGenerationHash: key.image_generation_hash
    });
    if (!lookup.hit) return null;
    if (payload.exact_identity_storage_verified !== true) {
      const verified = await verifyIdentityCacheImages(payload);
      if (!verified.ok) return null;
    }
    return writerFinalReplayRecordToListingResult({
      record: lookup.record,
      payload,
      latencyMs: Date.now() - startedAt
    });
...
function exactAnchorFastFinalShadowFromPayload(payload = {}) {
  const probe = payload.v4_anchor_probe;
  if (!probe || typeof probe !== "object") {
    return {
      evaluated: false,
      eligible: false,
      applied: false,
      would_skip_full_provider: false,
      reason: "pre_l2_anchor_probe_unavailable"
    };
  }
  const finalize = probe.shadow_finalize && typeof probe.shadow_finalize === "object"
    ? probe.shadow_finalize
    : null;
  const eligible = probe.finalized === true && finalize?.finalized === true;
  return {
    evaluated: true,
    eligible,
    applied: false,
    would_skip_full_provider: eligible,
    reason: finalize?.reason || probe.reason || "anchor_probe_not_finalized",
    route: probe.plan?.route || null,
    candidate: finalize?.candidate || null,
    query_fields: finalize?.query_fields || null,
    shadow_title: finalize?.title || "",
    shadow_resolved_fields: finalize?.resolved_fields || {},
    catalog_candidate_count: Number(finalize?.catalog_candidate_count || probe.metrics?.catalog_candidate_count || 0),
    trusted_candidate_count: Number(finalize?.trusted_candidate_count || probe.metrics?.trusted_candidate_count || 0),
    eligible_candidate_count: Number(finalize?.eligible_candidate_count || probe.metrics?.eligible_candidate_count || 0),
    direct_conflict_count: Array.isArray(finalize?.candidate?.anchor_agreement?.contradicted)
      ? finalize.candidate.anchor_agreement.contradicted.length
      : null,
    policy: "shadow_only_full_provider_still_required"
  };
}

function identityCacheReadBypassed(options = {}) {
  return optionFlag(options, "disable_identity_result_cache", false)
    || optionFlag(options, "disable_identity_result_cache_read", false);
}

function identityCacheWriteBypassed(options = {}) {
  return optionFlag(options, "disable_identity_result_cache", false)
    || optionFlag(options, "disable_identity_result_cache_write", false);
}

function identityCacheLookupOutcome({
  result = null,
  key = {},
  missReason = null,
  readBypassed = false,
  ...
  return {
    ...
    telemetry: {
      cache_hit: hit,
      read_bypassed: readBypassed === true,
      miss_reason: hit ? null : missReason,
      provider_call_skipped: hit,
      cached_result_version_match: hit ? true : versionMatch,
      cache_key: key.cache_key || null,
      cache_scope: "global_verified_content",
      image_generation_hash: key.image_generation_hash || null,
      version_fingerprint: key.version_fingerprint || null,
      recognition_pipeline_fingerprint: key.recognition_pipeline_fingerprint || key.version_fingerprint || null,
      result_version: key.result_version || null,
      cached_version_fingerprint: cachedVersionFingerprint || null
    }
  };
}
...
async function createIdentityCacheTitle(payload) {
  const providerOptions = providerOptionsFromPayload(payload);
  if (identityCacheReadBypassed(providerOptions)) {
    return identityCacheLookupOutcome({
      missReason: "identity_cache_bypassed_by_request",
      readBypassed: true
    });
  }
  if (!identityResultCacheReadEnabled()) {
    return identityCacheLookupOutcome({ missReason: "identity_cache_read_disabled", readBypassed: true });
  }
  if (!isSupabaseFeedbackConfigured()) {
    return identityCacheLookupOutcome({ missReason: "supabase_not_configured", readBypassed: true });
  }

  const startedAt = Date.now();
  const key = buildIdentityResultCacheKey(payload);
  if (!key.ok) return identityCacheLookupOutcome({ key, missReason: key.reason });

  try {
    if (payload.exact_identity_storage_verified !== true) {
      return identityCacheLookupOutcome({ key, missReason: "verified_storage_required" });
    }

    const lookup = await readIdentityResultCacheRecord({
      cacheKey: key.cache_key,
      imageGenerationHash: key.image_generation_hash,
      expectedVersion: { fingerprint: key.version_fingerprint }
    });
    if (!lookup.hit) {
      return identityCacheLookupOutcome({
        key,
        missReason: lookup.reason,
        versionMatch: lookup.cached_result_version_match ?? null,
        cachedVersionFingerprint: lookup.record?.version_fingerprint || null
      });
    }

    return identityCacheLookupOutcome({
      key,
      result: identityResultCacheRecordToListingResult({
        record: lookup.record,
        payload,
        expectedVersion: { fingerprint: key.version_fingerprint },
        latencyMs: Date.now() - startedAt
    ...
    return identityCacheLookupOutcome({ key, missReason: "identity_cache_read_failed" });
  }
}

async function withIdentityCacheWrite(result, payload, lookupTelemetry = {}) {
  const providerOptions = providerOptionsFromPayload(payload);
  if (identityCacheWriteBypassed(providerOptions)) {
    const key = buildIdentityResultCacheKey(payload);
    return {
      ...result,
      identity_cache: {
        ...
        cache_hit: false,
        read_bypassed: true,
        miss_reason: "identity_cache_bypassed_by_request",
        provider_call_skipped: false,
        write_attempted: false,
        write_saved: false,
        write_reason: "identity_cache_bypassed_by_request",
        cache_key: key.ok ? key.cache_key : null,
        image_generation_hash: key.ok ? key.image_generation_hash : null,
        version_fingerprint: key.ok ? key.version_fingerprint : null,
        recognition_pipeline_fingerprint: key.ok ? key.recognition_pipeline_fingerprint : null,
        result_version: key.ok ? key.result_version : null
      }
    };
  }
  ...
  try {
    ...
    return {
      ...
      identity_cache: {
        ...
        recognition_pipeline_fingerprint: key.recognition_pipeline_fingerprint,
...
async function createIdentityInFlightKey(payload) {
  ...
  if (optionFlag(providerOptions, "disable_identity_result_cache", false)
    || optionFlag(providerOptions, "disable_identity_inflight_replay", false)) return null;
  if (!identityInFlightCoalescingEnabled()) return null;
  if (!isSupabaseFeedbackConfigured()) return null;

  const key = buildIdentityResultCacheKey(payload);
  const tenantScopedKey = buildTenantScopedIdentityInFlightKey(payload);
  if (!key.ok || !tenantScopedKey) return null;

  try {
    if (payload.exact_identity_storage_verified === true) return tenantScopedKey;
    const verified = await verifyIdentityCacheImages(payload);
    return verified.ok ? tenantScopedKey : null;
  } catch {
    return null;
  }
}
...
function withEvidenceCompatibility(result, providerPayload, payload) {
  ...
  return {
    ...
    normalization_version: fieldNormalizationPolicyVersion,
    provider_evidence_normalization_version: providerEvidenceNormalizationVersion,
    ...
    raw_provider_field_evidence: Array.isArray(providerPayload.field_evidence)
      ...
      : [],
    raw_provider_unresolved: Array.isArray(providerPayload.unresolved)
      ? providerPayload.unresolved
      : [],
    raw_provider_recognition_status: providerPayload.recognition_status || null
  };
}
...
export function catalogRetrievalFamiliesForFields(fields = {}, {
  ...
  // year and serial denominator but no printed
  ...
  const subjectAnchorLane = (hasSubject && !hasPrintedCode && hasSerialDenominator && hasYear)
    ? [retrievalQueryFamilies.CATALOG_SUBJECT_ANCHOR]
    : [];
...
function catalogCandidateContextCacheKey({
  ...
  const sourceExclusions = [...new Set((Array.isArray(excludeSourceFeedbackIds)
    ? excludeSourceFeedbackIds
    : [excludeSourceFeedbackIds])
  ...
  const keyPayload = {
    ...
    source_exclusion_sha256: sourceExclusions.length
      ? crypto.createHash("sha256").update(sourceExclusions.join("\0")).digest("hex")
      : "",
    fields: {
...
function canOverlapProviderCapacityHandoffAfterInitialCall({
  assistShadowOnly = false,
  additionalProviderCallPending = false
} = {}) {
  ...
  return assistShadowOnly === true && additionalProviderCallPending !== true;
}
...
function candidateControlResultPatch(candidateControl = {}) {
  return {
    ...
    candidate_pre_application_evidence_snapshot:
      candidateControl.candidate_pre_application_evidence_snapshot,
    candidate_observation_snapshot_status: candidateControl.candidate_observation_snapshot_status,
    candidate_observation_snapshot_reason: candidateControl.candidate_observation_snapshot_reason,
...
function withCandidateControl(result = {}, context = {}, {
  ...
  const controlledInput = context.sourceFeedbackId && !result.source_feedback_id
    ...
    : result;
  const persistedObservationSnapshot = controlledInput.candidate_pre_application_evidence_snapshot;
  const candidateInput = Object.isFrozen(persistedObservationSnapshot)
    && persistedObservationSnapshot?.source === "LIVE_PRE_APPLICATION_CURRENT_IMAGE_EVIDENCE"
    ? controlledInput
    : {
        ...controlledInput,
        candidate_pre_application_evidence_snapshot:
          v4ProductionStrategy.candidate_control.build_pre_application_evidence_snapshot(controlledInput)
      };
  ...
  const application = v4ProductionStrategy.candidate_control.build_retrieval_application({
    result: controlled,
    candidateControl,
    enabled,
  ...
  return {
    ...controlled,
    retrieval_application: mergeForwardEnumerationApplicationEvidence(controlled, application)
  };
}
...
export function recognitionIdentityPreflightDecision(payload = {}, env = process.env) {
  ...
  const preingestionBundleReady = Boolean(
    bundleId
    && payload.preingestion_bundle_used === true
    && bundleStatus === "READY"
  );
  // READY proves only that the immutable image bundle can be loaded. Existing
  // pre-ingestion patches cover a narrow hard-text subset and do not carry a
  // versioned, exhaustive recognition authority snapshot. Keep the production
  // skip stable; cold evaluation may measure the missing evidence in shadow.
  const hasPublishablePreingestionEvidence = preingestionBundleReady
    && hasRecognitionEvidence(preingestionEvidenceDocumentFromPayload(payload));
  const providerOptions = providerOptionsFromPayload(payload, env);
  const evaluationShadowOnly = evaluationTraceEnabled(payload)
    && optionFlag(providerOptions, "recognition_worker_preflight_shadow_only", false) === true;
  const explicitlyEnabled = envFlag(env, "ENABLE_RECOGNITION_PREFLIGHT_WITH_PREINGESTION", false);

  // The combined Worker is not writer-critical production infrastructure. A
  // READY bundle may be re-observed only in the cold evaluation shadow (or by
  // the existing emergency switch) until latency, cost and title parity are
  // independently proven.
  if (preingestionBundleReady && !evaluationShadowOnly && !explicitlyEnabled) {
    return {
      run: false,
      shadow_only: false,
  ...
  return {
    run: true,
    shadow_only: evaluationShadowOnly,
    reason: preingestionBundleReady
      ? hasPublishablePreingestionEvidence
        ? "preingestion_bundle_evidence_is_partial"
        : "preingestion_bundle_has_no_publishable_evidence"
...
async function createRecognitionIdentityPreflight(payload, {
  ...
  const decision = decisionSnapshot || recognitionIdentityPreflightDecision(payload, process.env);
  if (!decision.run) {
    return {
      decision,
      result: null,
  ...
  try {
    ...
    if (!hasRecognitionEvidence(evidenceDocument)) {
      return {
        ...
        response,
        signedImages,
        worker_call_attempted: workerCallAttempted,
        worker_started_at: workerStartedAt,
        worker_completed_at: workerCompletedAt
      };
    }
    ...
    if (workerCallAttempted && !workerCompletedAt) workerCompletedAt = new Date().toISOString();
    return {
      decision,
      result: null,
      evidenceDocument: null,
...
export function recognitionPreflightEvidenceApplication(
  recognitionEvidenceDocument = null,
  recognitionPreflight = null
) {
  const workerEvidenceDocument = recognitionPreflight?.evidenceDocument || null;
  const shadowOnly = recognitionPreflight?.decision?.shadow_only === true;
  return {
    production_evidence_document: shadowOnly
      ? recognitionEvidenceDocument
      : workerEvidenceDocument || recognitionEvidenceDocument,
    shadow_evidence_document: workerEvidenceDocument,
    shadow_only: shadowOnly
  };
}

export function buildRecognitionPreflightDiagnostics({
  recognitionPreflight = null,
  shadowEvidenceDocument = null,
  joinWaitMs = 0,
  providerCompletedAt = null,
  completionStatus = null,
  writerCriticalPathJoined = null
} = {}) {
  const workerStartedAt = Date.parse(recognitionPreflight?.worker_started_at || "");
  const workerCompletedAt = Date.parse(recognitionPreflight?.worker_completed_at || "");
  const providerCompletedMs = Date.parse(providerCompletedAt || "");
  return {
    schema_version: "recognition-preflight-diagnostics-v2",
    run: recognitionPreflight?.decision?.run === true,
    decision_reason: recognitionPreflight?.decision?.reason || null,
    outcome_reason: recognitionPreflight?.reason || null,
    shadow_only: recognitionPreflight?.decision?.shadow_only === true,
    skipped: recognitionPreflight?.skipped === true,
    worker_call_count: recognitionPreflight?.worker_call_attempted === true ? 1 : 0,
    evidence_field_count: Object.keys(shadowEvidenceDocument?.evidence || {}).length,
    worker_execution_ms: Number.isFinite(workerStartedAt) && Number.isFinite(workerCompletedAt)
      ? Math.max(0, workerCompletedAt - workerStartedAt)
      : null,
    join_wait_ms: Number.isFinite(Number(joinWaitMs)) ? Math.max(0, Number(joinWaitMs)) : null,
    completion_status: completionStatus
      || (recognitionPreflight?.deferred_after_writer_ready === true
        ? "DEFERRED_AFTER_WRITER_READY"
        : recognitionPreflight?.skipped === true
          ? "SKIPPED"
          : recognitionPreflight?.error
            ? "FAILED"
            : "COMPLETED"),
    writer_critical_path_joined: writerCriticalPathJoined === null
      ? recognitionPreflight?.decision?.shadow_only !== true
      : writerCriticalPathJoined === true,
    late_result_policy: recognitionPreflight?.deferred_after_writer_ready === true
      ? "NO_BUSINESS_STATE_BACKPATCH"
      : null,
    worker_finished_before_provider: Number.isFinite(workerCompletedAt)
      && Number.isFinite(providerCompletedMs)
      ? workerCompletedAt <= providerCompletedMs
      : null,
    worker_error_code: recognitionPreflight?.error?.code || null
  };
}

export async function snapshotRecognitionPreflightAfterProvider({
  recognitionPreflightPromise = null,
  decision = null
} = {}) {
  if (!recognitionPreflightPromise || typeof recognitionPreflightPromise.then !== "function") {
    return {
      recognition_preflight: null,
      completion_status: "NOT_REQUESTED",
      writer_join_wait_ms: 0,
      writer_critical_path_joined: false
    };
  }

  if (decision?.shadow_only !== true) {
    const joinStartedAt = nowMs();
    return {
      recognition_preflight: await recognitionPreflightPromise,
      completion_status: "REQUIRED_JOIN_COMPLETED",
      writer_join_wait_ms: nowMs() - joinStartedAt,
      writer_critical_path_joined: true
    };
  }

  let snapshot;
  try {
    snapshot = await waitForPromiseWithin(recognitionPreflightPromise, 0);
  } catch (error) {
    return {
      recognition_preflight: {
        ...
        response: null,
        skipped: false,
        reason: "recognition_preflight_shadow_failed_before_writer_ready",
        failed_before_writer_ready: true,
        error: safeRecognitionError(error)
      },
      completion_status: "FAILED_BEFORE_WRITER_READY",
      writer_join_wait_ms: 0,
      writer_critical_path_joined: false
    };
  }
  if (snapshot.settled) {
    return {
      recognition_preflight: snapshot.value,
      completion_status: "COMPLETED_BEFORE_WRITER_FINALIZE",
      writer_join_wait_ms: 0,
      writer_critical_path_joined: false
    };
  }

  scheduleBackgroundCompletion(recognitionPreflightPromise);
  return {
    recognition_preflight: {
      decision,
      result: null,
      ...
      response: null,
      skipped: false,
      reason: "recognition_preflight_shadow_deferred_after_writer_ready",
      deferred_after_writer_ready: true
    },
    completion_status: "DEFERRED_AFTER_WRITER_READY",
    writer_join_wait_ms: 0,
    writer_critical_path_joined: false
  };
}

export async function snapshotKnowledgeFirstRouteShadowForEvaluation(
  currentDecision = null,
  completedDecisionPromise = null
) {
  if (!completedDecisionPromise || typeof completedDecisionPromise.then !== "function") return currentDecision;
  let snapshot;
  try {
    snapshot = await waitForPromiseWithin(completedDecisionPromise, 0);
  } catch {
    return {
      ...(currentDecision || {}),
      shadow_trace_completion: {
        status: "FAILED_BEFORE_WRITER_READY",
        writer_critical_path_joined: false
      }
    };
  }
  if (snapshot.settled) {
    return {
      ...(currentDecision || {}),
      // The initial route is the only route that is guaranteed to have been
      // frozen before the deployed Provider started. A Worker/constraint
      // result that arrives while Provider is already running remains useful
      // diagnosis, but must never replace that activation denominator.
      post_initial_diagnostic_decision: snapshot.value || null,
      shadow_trace_completion: {
  ...
  scheduleBackgroundCompletion(completedDecisionPromise);
  return {
    ...(currentDecision || {}),
      shadow_trace_completion: {
        status: "DEFERRED_AFTER_WRITER_READY",
        writer_critical_path_joined: false,
...
function planKnowledgeFirstWithProviderAuxShadow({
  evidenceDocument = {},
  ...
  const decisionEvidenceCutoffAt = includeReplayInput === true
    ? new Date().toISOString()
    : null;
  const knowledgeFirstDecision = planKnowledgeFirstRecognition({
    evidenceDocument,
    forwardEnumerationTrace,
    usableImageCount,
    exactAnchorShadow,
  ...
  // The auxiliary-route contract is evaluation-only. Normal production keeps
  // the existing lightweight detailed Shadow decision and pays no snapshot
  // clone/hash/manifest overhead for a counterfactual executor.
  if (includeReplayInput !== true) return knowledgeFirstDecision;
  const routeDecidedAt = new Date().toISOString();
  return Object.freeze({
    ...knowledgeFirstDecision,
    provider_aux_route_shadow: planProviderAuxRouteShadow({
      knowledgeFirstDecision,
      evidenceDocument,
      forwardEnumerationTrace,
      usableImageCount,
...
function knowledgeFirstEvidenceDocumentBeforeProvider(payload = {}, recognitionEvidenceDocument = null) {
  const preingestionEvidenceDocument = preingestionEvidenceDocumentFromPayload(payload);
  const documents = [recognitionEvidenceDocument, preingestionEvidenceDocument]
    .filter((document) => hasRecognitionEvidence(document));
  if (!documents.length) return {};
  return {
    schema_version: "knowledge-first-pre-provider-evidence-v1",
    evidence: mergeEvidenceMaps(...documents.map((document) => document.evidence)),
    // Pre-ingestion is appended last and therefore remains the deterministic
    // tie-breaker for direct, verified current-image observations. Conflicting
    // values are still surfaced by mergeEvidenceMaps and block model-free
    // finalization in the route policy.
    resolved: mergeResolvedFields(...documents.map((document) => document.resolved)),
    unresolved: [...new Set(documents.flatMap((document) => (
      Array.isArray(document.unresolved) ? document.unresolved : []
    )))]
  };
}

function knowledgeFirstRouteFromPreProviderEvidence({
  evidenceDocument = {},
  ...
  const claim = publishableForwardEnumerationClaim(evidenceDocument);
  const forwardEnumerationTrace = constraintModel
    ? forwardEnumerationTraceForResult({
      raw_observed_fields: claim,
      normalized_evidence: evidenceDocument.evidence || {},
      evidence_schema_version: evidenceDocument.schema_version || null
    }, constraintModel, { observationContext })
    : [];
  return planKnowledgeFirstWithProviderAuxShadow({
    evidenceDocument,
    forwardEnumerationTrace,
    usableImageCount,
    exactAnchorShadow,
...
async function createOpenAiTitle(payload, selection, {
  ...
  recognitionPreflightPromise = null,
  recognitionPreflightDecision: recognitionPreflightDecisionSnapshot = null,
  signedImages: reusableSignedImages = null,
  timingContext = null,
  visualFeatures = {},
  requestContext = null,
  providerAuxRouteDecision = null
} = {}) {
  let recognitionEvidenceDocument = initialRecognitionEvidenceDocument;
  let recognitionPreflightError = null;
  let recognitionPreflightDiagnostics = null;
  ...
  const readOnlyProviderContract = readOnlyProviderContractEnabled(initialPayload, process.env);
  const worldKnowledgeShadowAssist = readOnlyProviderContract
    && worldKnowledgeShadowAssistRequested(initialPayload, process.env);
  const providerPromptMode = readOnlyProviderContract
    ? "v4_read_only_surface"
    : ultraFastL2
    ? "v4_ultra_fast_l2"
  ...
  const providerResponseProfile = readOnlyProviderContract
    ...
    : ["v4_compact_l2", "v4_ultra_fast_l2"].includes(providerPromptMode)
      ? "compact_sparse_v1"
      : "standard";
  const targetedCandidate = targetedAssistCandidateEnabled({
    providerOptions,
    traceLevel: providerOptions.trace_level,
    routeDecision: providerAuxRouteDecision
  });
  const targetedEvaluationProfile = targetedAssistEvaluationProfileEnabled({
    providerOptions,
    traceLevel: providerOptions.trace_level
  });
  const secondLookShadowProfileEnabled = secondLookShadowEvaluationProfileEnabled({
    providerOptions,
    traceLevel: providerOptions.trace_level
  });
  let prompt = targetedCandidate ? null : await buildInitialProviderPrompt(initialPayload, maxTitleLength);
  const fullProviderRequestContext = openAiRequestContextFromPayload(initialPayload, {
    providerCallPurpose: "full_l2",
    titleStage: providerOptions.v4_title_stage_target || initialPayload.v4_title_stage_target || ""
  });
  const providerExecutionSignal = requestContext?.signal || null;
  const runFullProvider = async () => {
    prompt = prompt || await buildInitialProviderPrompt(initialPayload, maxTitleLength);
    return analyzeCardEvidenceWithOpenAiEmergency({
      ...
      requestContext: fullProviderRequestContext,
      signal: providerExecutionSignal
    });
  };
  const observationExecution = targetedEvaluationProfile
    ? await executeTargetedAssistObservationRoute({
        ...
        requestContext: fullProviderRequestContext,
        signal: providerExecutionSignal,
        runProviderStage: (work) => runTimedProviderCall(
    ...
    : {
        result: await runTimedProviderCall(
          visionProviderIds.OPENAI_LEGACY,
          timingContext,
          runFullProvider
        ),
  ...
  const providerResult = observationExecution.result;
  const effectiveProviderPromptMode = observationExecution.execution?.final_observation_owner === "TARGETED_VISUAL_OBSERVATION"
    ? "v4_targeted_visual_read_only"
    : providerPromptMode;
  const effectiveProviderPromptChars = observationExecution.execution?.final_observation_owner === "TARGETED_VISUAL_OBSERVATION"
    ? Number(providerResult.targeted_visual_observation?.prompt_chars || 0)
    : String(prompt || "").length;
  const effectiveProviderInputImageCount = observationExecution.execution?.final_observation_owner === "TARGETED_VISUAL_OBSERVATION"
    ? Number(providerResult.targeted_visual_observation?.input_image_count || 0)
    : Array.isArray(initialPayload.images) ? initialPayload.images.length : 0;
  let providerResultWithEvidence = timeSync(timingContext, "renderer_ms", () => ({
    ...
    provider_prompt_mode: effectiveProviderPromptMode,
    provider_prompt_chars: effectiveProviderPromptChars,
    provider_input_image_count: effectiveProviderInputImageCount,
    ...
    ...(observationExecution.execution
      ? {
          targeted_assist_execution: observationExecution.execution,
          provider_call_ledger: observationExecution.execution.provider_call_ledger
        }
      : {})
  }));
  // The provider lease protects only the scarce GPT call. A production-
  // authoritative recognition preflight may still join before finalization;
  // a shadow-only preflight gets a zero-budget snapshot and can never delay
  // resolution, rendering, cache write, or writer-ready commit.
  let providerCapacityStageHandoffPromise = null;
  let providerCapacityHandoffOverlapStartedAt = null;
  if (canOverlapProviderCapacityHandoffAfterInitialCall({
    assistShadowOnly,
    additionalProviderCallPending: secondLookShadowProfileEnabled
  })
  ...
  if (recognitionPreflightPromise) {
    const recognitionSnapshot = await snapshotRecognitionPreflightAfterProvider({
      ...
      decision: recognitionPreflightDecisionSnapshot
    });
    const recognitionPreflight = recognitionSnapshot.recognition_preflight;
    const recognitionJoinWaitMs = recognitionSnapshot.writer_join_wait_ms;
    addTiming(timingContext, "recognition_preflight_join_wait_ms", recognitionJoinWaitMs);
    const evidenceApplication = recognitionPreflightEvidenceApplication(
      recognitionEvidenceDocument,
      recognitionPreflight
    );
    recognitionEvidenceDocument = evidenceApplication.production_evidence_document;
    recognitionPreflightError = recognitionPreflight?.error || null;
    recognitionPreflightDiagnostics = buildRecognitionPreflightDiagnostics({
      recognitionPreflight,
      shadowEvidenceDocument: evidenceApplication.shadow_evidence_document,
      joinWaitMs: recognitionJoinWaitMs,
      providerCompletedAt: providerResult.provider_slot_timing?.completed_at || null,
      completionStatus: recognitionSnapshot.completion_status,
      writerCriticalPathJoined: recognitionSnapshot.writer_critical_path_joined
    });
  }
  ...
  // Capture the provider/OCR/current-image state before Forward Enumeration,
  // retrieval, selection, or application can add candidate-derived evidence.
  // `withCandidateControl` preserves only this runtime-owned frozen packet.
  providerResultWithEvidence = {
    ...providerResultWithEvidence,
    candidate_pre_application_evidence_snapshot:
      v4ProductionStrategy.candidate_control.build_pre_application_evidence_snapshot(
        providerResultWithEvidence,
        initialPayload
      )
  };
  providerResultWithEvidence.current_image_context =
    providerResultWithEvidence.candidate_pre_application_evidence_snapshot.current_image_context || null;
  const forwardEnumerationConsumesRetrieval = evaluationTraceEnabled(initialPayload)
    && forwardEnumerationExperimentArm(providerOptions) === forwardEnumerationExperimentArms.CONSUME;
  if (evaluationTraceEnabled(initialPayload) || readOnlyProviderContract) {
    providerResultWithEvidence = await timeAsync(timingContext, "forward_enumeration_shadow_ms", async () => (
      attachForwardEnumerationCandidates(
        providerResultWithEvidence,
        await loadConstraintModelSnapshot(),
        {
          shadow: !forwardEnumerationConsumesRetrieval,
          observationContext: initialPayload
        }
      )
    ));
  }
  if (worldKnowledgeShadowAssist) {
    providerResultWithEvidence = timeSync(timingContext, "world_knowledge_shadow_input_ms", () => (
      attachWorldKnowledgeShadowAssistInput(
        providerResultWithEvidence,
        { requested: true }
      )
  ...
  if (recognitionPreflightDiagnostics && evaluationTraceEnabled(initialPayload)) {
    providerResultWithEvidence = {
      ...providerResultWithEvidence,
      recognition_preflight_diagnostics: recognitionPreflightDiagnostics
    };
  }
  ...
  if (deferVectorUntilProviderObservation) {
    ...
    providerResolvedForRetrieval = mergeCurrentFieldsForTitleAssist(
      providerResultWithEvidence.retrieval_query_expansion_fields,
  ...
  const finalizeProviderResult = async (result, terminalPath = "unknown") => {
    let handoff = null;
    let handoffJoinWaitMs = 0;
    const joinProviderCapacityHandoff = async () => {
      const handoffJoinStartedAt = nowMs();
      handoff = await (providerCapacityStageHandoffPromise || handoffProviderCapacityAfterStage(
        initialPayload,
        requestContext,
        timingContext
      ));
      handoffJoinWaitMs = nowMs() - handoffJoinStartedAt;
    ...
    // The evaluation-only second look is an additional bounded Provider call.
    // It retains the database capacity lease, but its local Provider slot is
    // fail-if-busy so the database lease can never sit in the local semaphore.
    // Every other profile preserves the original handoff ordering exactly.
    if (!secondLookShadowProfileEnabled) await joinProviderCapacityHandoff();
    ...
    let secondLookShadow = null;
    if (secondLookShadowProfileEnabled) {
      try {
        secondLookShadow = await timeAsync(timingContext, "second_look_shadow_ms", () => runSecondLookEvaluationShadow({
          baselineResult: finalizedWithTerminalRecognition,
          stageOneResult: mergedResult,
          images: initialPayload.images,
          providerOptions,
          traceLevel: providerOptions.trace_level,
          fieldStates: mergedResult.field_states || mergedResult.fieldStates || {},
          unresolved: [
            ...(Array.isArray(mergedResult.unresolved) ? mergedResult.unresolved : []),
            ...(Array.isArray(mergedResult.raw_provider_unresolved) ? mergedResult.raw_provider_unresolved : [])
          ],
          identityCriticalReason: providerOptions.second_look_identity_critical_reason
            || providerOptions.secondLookIdentityCriticalReason
            || "",
          executorContext: {
            shardKey: initialPayload.recognition_session_id || initialPayload.asset_id || initialPayload.assetId || "",
            preferredKeySlot: initialPayload.openai_preferred_key_slot || initialPayload.provider_key_slot_hint || null,
          ...
          onObservationComplete: joinProviderCapacityHandoff,
          resolveCandidate: (candidateInput) => withEvidenceCompletion(candidateInput, initialPayload, {
            timingContext: null,
        ...
        secondLookShadow = {
          ...
          reason_code: "SECOND_LOOK_SHADOW_INTERNAL_ERROR",
          error: {
            code: String(error?.code || "SECOND_LOOK_SHADOW_INTERNAL_ERROR").slice(0, 120),
          ...
          baseline_title: String(finalizedWithTerminalRecognition.final_title || finalizedWithTerminalRecognition.title || ""),
    ...
    const baselineProviderCalls = Number.isFinite(Number(finalizedWithoutReplayTrace.usage?.provider_calls))
      ? Math.max(0, Math.trunc(Number(finalizedWithoutReplayTrace.usage.provider_calls)))
      : null;
    const shadowProviderCalls = typeof secondLookShadow?.paid_provider_calls === "number"
      && Number.isFinite(secondLookShadow.paid_provider_calls)
      ? Math.max(0, Math.trunc(secondLookShadow.paid_provider_calls))
      : null;
    const secondLookLedger = Array.isArray(secondLookShadow?.provider_call_ledger)
      ? secondLookShadow.provider_call_ledger
      : [];
    const secondLookLedgerRow = secondLookLedger[0] || null;
    const finalized = {
      ...finalizedWithoutReplayTrace,
      ...(secondLookShadow ? {
        second_look_shadow: secondLookShadow,
        baseline_provider_calls: baselineProviderCalls,
        shadow_provider_calls: shadowProviderCalls,
        total_provider_calls: baselineProviderCalls !== null && shadowProviderCalls !== null
          ? baselineProviderCalls + shadowProviderCalls
          : null,
        provider_accounting_complete: secondLookLedgerRow?.accounting_complete === true,
        second_look_shadow_usage: {
          provider_calls: shadowProviderCalls,
          input_tokens: secondLookLedgerRow?.input_tokens ?? null,
          output_tokens: secondLookLedgerRow?.output_tokens ?? null,
          total_tokens: secondLookLedgerRow?.total_tokens ?? null,
          estimated_cost_usd: secondLookLedgerRow?.estimated_cost_usd ?? null,
          cost_configured: secondLookLedgerRow?.cost_configured ?? null,
          model_id: secondLookLedgerRow?.model_id || secondLookShadow.model_id || null
        }
      } : {}),
...
export const __listingCopilotTitleTestHooks = {
  ...
  exactAnchorFastFinalShadowFromPayload,
  ...
  snapshotRecognitionPreflightAfterProvider,
  snapshotKnowledgeFirstRouteShadowForEvaluation,
  knowledgeFirstEvidenceDocumentBeforeProvider,
  knowledgeFirstRouteFromPreProviderEvidence,
  withoutEvaluationDecisionTracePacket,
  providerOptionsFromPayload,
...
async function createProviderTitle(payload, {
  recognitionEvidenceDocument = null,
  recognitionPreflightPromise = null,
  recognitionPreflightDecision = null,
  signedImages = null,
  timingContext = null,
  visualFeatures = {},
  requestContext = null,
  ...
  return createOpenAiTitle(payload, selection, {
    recognitionEvidenceDocument,
    recognitionPreflightPromise,
    recognitionPreflightDecision,
    signedImages,
    timingContext,
    visualFeatures,
    requestContext,
    providerAuxRouteDecision
  });
}
...
export async function runNativeV4Recognition({
  ...
  const runtimeOptions = providerOptionsFromPayload(payload);
  const identityCacheRuntimeEnabled = (
    !identityCacheReadBypassed(runtimeOptions) && identityResultCacheReadEnabled()
  ) || (
    !identityCacheWriteBypassed(runtimeOptions) && identityResultCacheWriteEnabled()
  );
  // Catalog revision is evaluation evidence as well as a cache-key input.
  // Cold benchmarks deliberately disable cache read/write, but their replay
  // packets still need the exact catalog snapshot to make repeated runs
  // comparable. Keep this read independent of cache runtime enablement.
  const evaluationCatalogEvidenceRequired = runtimeOptions.trace_level === "evaluation";
  if (identityCacheRuntimeEnabled || evaluationCatalogEvidenceRequired) {
    await attachActiveCatalogSnapshotRevision(payload).catch(() => null);
  }
  payload.exact_identity_storage_verified = false;
  const exactReplayLookupEnabled = isSupabaseFeedbackConfigured()
    && ((!optionFlag(runtimeOptions, "disable_approved_identity_memory", false) && listingApprovedMemoryEnabled())
      || (!identityCacheReadBypassed(runtimeOptions) && identityResultCacheReadEnabled()));
  if (exactReplayLookupEnabled) {
    const verified = await verifyIdentityCacheImages(payload).catch(() => ({ ok: false }));
    payload.exact_identity_storage_verified = verified.ok === true;
  }

  try {
    const preProviderRescanResult = timeSync(timingContext, "image_quality_check_ms", () => createPreProviderRescanResult(payload));
    const exactIdentityStage = await tryExactIdentityReuseAndAnchorShadow({
      payload,
      preProviderRescanResult,
      lookupWriterFinalReplay: (immutablePayload) => timeAsync(
        timingContext,
        "writer_final_replay_lookup_ms",
        () => createWriterFinalReplayTitle(immutablePayload)
      ),
      lookupApprovedMemory: (immutablePayload) => timeAsync(
        timingContext,
        "approved_memory_lookup_ms",
        () => createApprovedMemoryTitle(immutablePayload)
      ),
      lookupIdentityCache: (immutablePayload) => timeAsync(
        timingContext,
        "identity_cache_lookup_ms",
        () => createIdentityCacheTitle(immutablePayload)
      ),
      lookupExactAnchorShadow: (immutablePayload) => exactAnchorFastFinalShadowFromPayload(immutablePayload)
    });
    if (exactIdentityStage.error) throw exactIdentityStage.error;
    const exactIdentityResult = exactIdentityStage.output?.result || null;
    const exactAnchorFastFinalShadow = exactIdentityStage.output?.exact_anchor_fast_final_shadow || null;
    const identityCacheTelemetry = exactIdentityStage.output?.identity_cache_lookup?.telemetry || {};
    if (exactIdentityResult) {
      const exactIdentityRoute = exactIdentityStage.output?.reason_codes?.[0] || "EXACT_IDENTITY_REUSE";
      setRecognitionPathIntent(timingContext, {
        pathKind: exactIdentityRoute === "PRE_PROVIDER_RESCAN"
          ? "PRE_PROVIDER_FAST_FINAL"
          : "EXACT_REPLAY",
        providerExpected: false,
        identityCacheStageExpected: false
      });
      const knowledgeFirstRouteShadow = planKnowledgeFirstWithProviderAuxShadow({
        usableImageCount: primaryImagesFromImages(payload.images || []).length,
        exactAnchorShadow: exactAnchorFastFinalShadow,
        higherAuthorityRoute: exactIdentityRoute,
        includeReplayInput: evaluationTraceEnabled(payload)
      });
      markRecognitionBoundary(timingContext, "decision_ready", {
        metadata: { route: exactIdentityRoute }
      });
      return buildListingResult(200, {
        ...exactIdentityResult,
        exact_anchor_fast_final_shadow: exactAnchorFastFinalShadow,
        knowledge_first_route_shadow: knowledgeFirstRouteShadow,
        native_core_stage_trace: [exactIdentityStage.trace]
    ...
    markRecognitionBoundary(timingContext, "full_path_started");
    ...
    const result = await runWithInFlightIdentityRequest({
      ...
      run: async () => {
        ...
        // Preflight can decide to skip before awaiting this shared promise.
        // Observe rejection immediately so a transient verification timeout
        // cannot terminate the function before the provider path awaits it.
        void sharedSignedImagesPromise.catch(() => null);
        const recognitionPreflightDecision = recognitionIdentityPreflightDecision(payload, process.env);
        const recognitionPreflightPromise = createRecognitionIdentityPreflight(payload, {
          // A late shadow completion must not mutate the writer timing ledger.
          // The writer path persists only a DEFERRED/FAILED marker; complete
          // late evidence belongs to a separate auxiliary-audit profile.
          timingContext: recognitionPreflightDecision.shadow_only ? null : timingContext,
          providerOptions,
          signedImagesPromise: sharedSignedImagesPromise,
          decision: recognitionPreflightDecision
        });
        // Only evaluation needs the 590 KB local constraint snapshot. Normal
        // production requests skip its read/parse entirely; evaluation starts
        // it beside image preflight and adds no new await/join to the Provider
        // path (its CPU cost remains visible in benchmark wall time).
        const typedRouteEvaluationEnabled = evaluationTraceEnabled(payload);
        const constraintModelState = { settled: !typedRouteEvaluationEnabled, value: null };
        const constraintModelPromise = (typedRouteEvaluationEnabled
          // This is counterfactual telemetry. It must not mutate the writer
          // timing/node ledger if the load finishes after writer-ready.
          ? loadConstraintModelSnapshot()
          : Promise.resolve(null))
          .then((value) => {
            constraintModelState.settled = true;
            constraintModelState.value = value;
            return value;
          })
          .catch(() => {
            constraintModelState.settled = true;
        ...
        const recognitionFastFinalDisabled = optionFlag(
          providerOptions,
          "disable_recognition_worker_fast_final",
          false
        );
        const recognitionFastLaneRace = recognitionFastFinalDisabled
        ...
        if (recognitionFastLaneRace.settled && recognitionFastLaneRace.value?.result) {
          ...
          const knowledgeFirstRouteShadow = planKnowledgeFirstWithProviderAuxShadow({
            evidenceDocument: recognitionFastLaneRace.value.evidenceDocument || {},
            usableImageCount: primaryImagesFromImages(payload.images || []).length,
            exactAnchorShadow: exactAnchorFastFinalShadow,
            higherAuthorityRoute: "RECOGNITION_WORKER_DETERMINISTIC_FINAL",
            includeReplayInput: typedRouteEvaluationEnabled
          });
          const fastFinalResult = {
          ...
          markRecognitionBoundary(timingContext, "decision_ready", {
            metadata: { route: "RECOGNITION_WORKER_DETERMINISTIC_FINAL" }
          });
          const cacheStageResult = await timeAsync(timingContext, "identity_cache_write_ms", () => withIdentityCacheWrite(
            fastFinalResult,
            payload,
            identityCacheTelemetry
          ));
          markRecognitionBoundary(timingContext, "identity_cache_stage_completed", {
            metadata: {
              write_attempted: cacheStageResult.identity_cache?.write_attempted === true,
              write_saved: cacheStageResult.identity_cache?.write_saved === true,
              write_reason: cacheStageResult.identity_cache?.write_reason || "identity_cache_stage_completed"
            }
          });
        ...
        // Freeze an initial decision for observability, then refresh it from
        // the same preflight promise that the current provider path already
        // joins. The Provider starts immediately; this does not add a new wait
        // to its critical path, but it prevents a 50 ms diagnostic race from
        // permanently under-reporting available pre-provider evidence.
        const initialKnowledgeFirstEvidence = knowledgeFirstEvidenceDocumentBeforeProvider(
          payload,
          recognitionFastLaneRace.value?.evidenceDocument || null
        );
        const initialKnowledgeFirstRouteShadow = knowledgeFirstRouteFromPreProviderEvidence({
          evidenceDocument: initialKnowledgeFirstEvidence,
          constraintModel: constraintModelState.settled ? constraintModelState.value : null,
          usableImageCount: primaryImagesFromImages(payload.images || []).length,
          exactAnchorShadow: exactAnchorFastFinalShadow,
          includeReplayInput: typedRouteEvaluationEnabled,
          observationContext: payload
        });
        const knowledgeFirstRouteShadowPromise = Promise.all([
          recognitionPreflightPromise,
          constraintModelPromise
        ]).then(([recognitionPreflight, constraintModel]) => (
          knowledgeFirstRouteFromPreProviderEvidence({
            evidenceDocument: knowledgeFirstEvidenceDocumentBeforeProvider(
              payload,
              recognitionPreflight?.evidenceDocument || null
            ),
            constraintModel,
            usableImageCount: primaryImagesFromImages(payload.images || []).length,
            exactAnchorShadow: exactAnchorFastFinalShadow,
        ...
        )).catch(() => initialKnowledgeFirstRouteShadow);
        // Normal production never joins counterfactual telemetry. Cold
        // evaluation also takes only a zero-budget post-Provider snapshot:
        // completed evidence is recorded, while a late Worker leaves only a
        // DEFERRED marker and cannot delay or backpatch writer-ready.
        if (!typedRouteEvaluationEnabled) void knowledgeFirstRouteShadowPromise;
        ...
        const providerResult = await createProviderTitle(payload, {
          ...
          requestContext,
          providerAuxRouteDecision: initialKnowledgeFirstRouteShadow.provider_aux_route_shadow || null
        });
        const persistedKnowledgeFirstRouteShadow = typedRouteEvaluationEnabled
          ? await snapshotKnowledgeFirstRouteShadowForEvaluation(
            initialKnowledgeFirstRouteShadow,
            knowledgeFirstRouteShadowPromise
          )
          : initialKnowledgeFirstRouteShadow;
        const decisionResult = {
          ...providerResult,
          exact_anchor_fast_final_shadow: exactAnchorFastFinalShadow,
          knowledge_first_route_shadow: persistedKnowledgeFirstRouteShadow,
          native_core_stage_trace: [exactIdentityStage.trace]
        };
        markRecognitionBoundary(timingContext, "decision_ready", {
          metadata: { route: "FULL_PROVIDER" }
        });
        const cacheStageResult = await timeAsync(timingContext, "identity_cache_write_ms", () => withIdentityCacheWrite(
          decisionResult,
          payload,
    ...
    if (result.identity_inflight?.coalesced === true) {
      setRecognitionPathIntent(timingContext, {
        pathKind: "INFLIGHT_REPLAY",
        providerExpected: false,
        identityCacheStageExpected: false
      });
    }
    ...
    return buildListingResult(200, {
      ...
      ...(error.targeted_assist_execution
        ? {
            targeted_assist_execution: error.targeted_assist_execution,
            provider_call_ledger: Array.isArray(error.provider_call_ledger)
              ? error.provider_call_ledger
              : error.targeted_assist_execution.provider_call_ledger,
