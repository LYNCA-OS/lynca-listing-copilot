...
import { attachForwardEnumerationCandidates } from "../../catalog/forward-enumeration-adapter.mjs";
import {
...
import { planKnowledgeFirstRecognition } from "../route-planner/knowledge-first-route-policy.mjs";
import {
  ...
  try {
    ...
    const result = await runWithInFlightIdentityRequest({
      ...
      run: async () => {
        ...
        // The replacement architecture is evaluated from evidence that exists
        // before the paid full-card Provider call. It never consumes the
        // Provider result it is meant to replace, and it remains shadow-only
        // until paired accuracy proves the targeted routes.
        const knowledgeFirstRouteShadow = planKnowledgeFirstRecognition({
          evidenceDocument: recognitionFastLaneRace.value?.evidenceDocument
            || preingestionEvidenceDocumentFromPayload(payload)
            || {},
          usableImageCount: primaryImagesFromImages(payload.images || []).length,
          exactAnchorShadow: exactAnchorFastFinalShadow
        });
