  ...
  try {
    ...
    const result = await runWithInFlightIdentityRequest({
      ...
      run: async () => {
        ...
        // Deliberately detach: this is counterfactual telemetry, never a new
        // production critical-path join. Full Provider remains the observable
        // compatibility fallback while the new route is Shadow-only.
