...
import { buildEvaluationDecisionTracePacket } from "../../evaluation/evaluation-decision-trace-packet.mjs";
...
import {
  ...
  buildVerifiedImageGenerationIdentity,
...
import {
  attachActiveCatalogSnapshotRevision,
  readActiveCatalogSnapshotRevision
} from "../../catalog/active-catalog-snapshot.mjs";
import { buildRecognitionEffectiveConfiguration } from "../application/recognition-profile-adapter.mjs";
import {
  anchorRouteLateShadowEnabled,
  buildPostRefreshAnchorRouteShadow,
  postRendezvousAnchorExecutionSummary
} from "../anchors/anchor-route-shadow.mjs";
...
function redactedRawTextTrace(value = "") {
  const rawText = String(value || "");
  return {
    raw_text_present: rawText.length > 0,
    raw_text_length: rawText.length,
    raw_text_sha256: rawText
...
function withVerifiedPreingestionPrintRun(result = {}, payload = {}) {
  ...
  const renderedFields = evidenceResult.rendered_fields && typeof evidenceResult.rendered_fields === "object"
    ...
    : {};
  const verificationRawTextTrace = redactedRawTextTrace(verification.raw_text);
  const next = {
    ...evidenceResult,
  ...
  const next = {
    ...
    resolution_trace: [
      ...(Array.isArray(evidenceResult.resolution_trace) ? evidenceResult.resolution_trace : []),
      {
        ...
        input: { provider_value: previous || null, ...verificationRawTextTrace },
...
async function buildListingResult(statusCode, result, timingContext, payload = {}) {
  ...
  const effectiveOptions = providerOptionsFromPayload(payload, process.env);
  const benchmarkProfile = String(effectiveOptions.recognition_benchmark_profile || "").trim() || null;
  const recognitionEffectiveConfiguration = buildRecognitionEffectiveConfiguration(payload, process.env);
  const timedResult = withTiming({
    ...
    recognition_benchmark_phase: effectiveOptions.recognition_benchmark_phase || null,
    ...(recognitionEffectiveConfiguration
      ? { recognition_effective_configuration: recognitionEffectiveConfiguration }
      : {})
  }, timingContext);
...
async function createWriterFinalReplayTitle(payload) {
  ...
  // Writer-final authority is keyed only by verified image identity. It must
  // remain available even when an algorithm owner revision is temporarily
  // unknown and AI cache/in-flight replay correctly fail closed.
  const key = buildVerifiedImageGenerationIdentity(payload);
...
function exactAnchorFastFinalShadowFromPayload(payload = {}) {
  ...
  return {
    ...
    ...(probe.input_trace ? { input_trace: probe.input_trace } : {}),
    ...(Array.isArray(probe.reason_codes) && probe.reason_codes.length
...
async function withIdentityCacheWrite(result, payload, lookupTelemetry = {}) {
  ...
  try {
    const expectedCatalogRevision = String(
      payload.recognition_cache_catalog_revision_at_start
      || payload.active_catalog_snapshot_revision
      || ""
    ).trim();
    const currentCatalogRevision = await readActiveCatalogSnapshotRevision().catch(() => ({
      ok: false,
      revision: "",
      reason: "catalog_revision_read_exception"
    }));
    if (!currentCatalogRevision.ok || currentCatalogRevision.revision !== expectedCatalogRevision) {
      return {
        ...result,
        identity_cache: {
          ...lookupTelemetry,
          ...
          write_attempted: false,
          write_saved: false,
          write_reason: !currentCatalogRevision.ok
            ? currentCatalogRevision.reason || "catalog_revision_revalidation_failed"
            : "catalog_revision_changed_during_request"
        }
      };
    }
    const verified = await verifyIdentityCacheImages(payload);
...
async function createOpenAiTitle(payload, selection, {
  ...
  if (anchorRouteLateShadowEnabled({ payload: initialPayload, env: process.env })) {
    mergedResult.pre_l2_anchor_late_route_shadow = buildPostRefreshAnchorRouteShadow({
      payload: initialPayload,
      result: providerResultWithEvidence,
      // The payload may still contain the bundle summary captured before the
      // Provider ran. Late-route diagnostics must use the state observed at
      // the post-provider rendezvous, including terminal job/evidence status.
      // An explicit null intentionally suppresses stale payload fallback.
      executionSummary: postRendezvousAnchorExecutionSummary(preingestionOcrRendezvous)
    });
  }
...
export async function runNativeV4Recognition({
  ...
  const identityInFlightRuntimeEnabled = !optionFlag(runtimeOptions, "disable_identity_result_cache", false)
    && !optionFlag(runtimeOptions, "disable_identity_inflight_replay", false)
    && identityInFlightCoalescingEnabled()
    && isSupabaseFeedbackConfigured();
  if (identityCacheRuntimeEnabled || identityInFlightRuntimeEnabled) {
    // Queue admission binds one authoritative Catalog revision to every job in
    // the batch. Honor that immutable snapshot here; direct/non-queue calls
    // still resolve it once when absent. Per-job force refresh would add a
    // serial database round trip and could split one batch across revisions.
    const attached = await attachActiveCatalogSnapshotRevision(payload).catch(() => ({
      resolution: { ok: false, revision: "", reason: "catalog_revision_read_exception" }
    }));
    payload.recognition_cache_revision_gate = {
      ok: attached.resolution?.ok === true,
      reason: attached.resolution?.reason || null
    };
    payload.recognition_cache_catalog_revision_at_start = attached.resolution?.ok === true
      ? attached.resolution.revision
      : null;
  }
