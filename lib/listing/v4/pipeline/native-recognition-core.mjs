...
import {
  ...
  worldKnowledgeProposalsEnabled
...
function normalizeAiResult(result, maxTitleLength, source = "openai") {
  ...
  return {
    ...
    world_knowledge_proposals: Array.isArray(result.world_knowledge_proposals)
      ? result.world_knowledge_proposals
      : [],
...
export async function snapshotKnowledgeFirstRouteShadowForEvaluation(
  ...
  if (snapshot.settled) {
    return {
      ...(snapshot.value || currentDecision || {}),
      shadow_trace_completion: {
...
function knowledgeFirstRouteFromPreProviderEvidence({
  ...
  const forwardEnumerationTrace = constraintModel
    ...
    : [];
  return planKnowledgeFirstRecognition({
    evidenceDocument,
...
async function createOpenAiTitle(payload, selection, {
  ...
  const worldKnowledgeProposals = readOnlyProviderContract
    && worldKnowledgeProposalsEnabled(initialPayload, process.env);
  const providerPromptMode = readOnlyProviderContract
    ? worldKnowledgeProposals
      ? "v4_read_only_world_knowledge"
  ...
  if (worldKnowledgeProposals) {
    providerResultWithEvidence = await timeAsync(timingContext, "world_knowledge_validation_ms", async () => (
      attachWorldKnowledgeProposals(
        providerResultWithEvidence,
...
export async function runNativeV4Recognition({
  ...
  try {
    ...
    if (exactIdentityResult) {
      const knowledgeFirstRouteShadow = planKnowledgeFirstRecognition({
