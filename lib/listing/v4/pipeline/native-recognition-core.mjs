...
async function buildListingResult(statusCode, result, timingContext, payload = {}) {
  ...
  const evaluationDecisionTracePacket = buildEvaluationDecisionTracePacket(finalizedResult, payload);
  const preingestionSummary = payload.preingestion_summary || null;
  const timedResult = withTiming({
    ...finalizedResult,
    ...(evaluationDecisionTracePacket ? { evaluation_decision_trace_packet: evaluationDecisionTracePacket } : {}),
