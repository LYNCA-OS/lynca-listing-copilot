...
const writerAssistedProviderOverrides = Object.freeze({
  ...
  card_domain_selection_mode: "trusted_catalog_margin_v1",
  exclude_current_source_feedback: false,
...
// Evaluation owns cache isolation on the server. Client provider options are
// intentionally stripped at the request boundary, so formal replays must use
// a profile that preserves the production writer strategy while preventing a
// historical result or approved memory from satisfying the current run.
const writerAssistedEvaluationProviderOverrides = Object.freeze({
  ...writerAssistedProviderOverrides,
  evaluation_profile: "v4_writer_assisted_evaluation_v1",
  exclude_current_source_feedback: true,
...
const accuracyCeilingOracleProviderOverrides = Object.freeze({
  ...
  // Oracle collection measures the chain ceiling, not writer latency. Wait
  // for both retrieval lanes so a production critical-path deadline cannot
  // turn a late Top-20 candidate into an artificial miss.
  enable_post_observation_retrieval_deadline: false,
  // Oracle runs are intentionally outside the writer latency contract. They
  // must consume the complete OCR evidence set instead of measuring whatever
  // happened to finish inside the production critical-path budget.
  require_terminal_preingestion_ocr: true,
...
export function resolveRecognitionProfile(profileId = defaultRecognitionProfileId, env = process.env) {
  ...
  if (normalized === recognitionProfileIds.WRITER_ASSISTED_EVALUATION) {
    return {
      profile_id: normalized,
      contract_version: recognitionRequestContractVersion,
      execution: { ...writerAssistedExecution },
      provider_options: {
        ...defaultProviderOptionsFromEnv(env),
        ...writerAssistedEvaluationProviderOverrides
      }
    };
  }
