...
import {
  ...
  normalizeFeedbackDatasetDisposition
} from "../../feedback/feedback-capture.mjs";
import {
  TITLE_DIFF_ALGORITHM_VERSION,
  TITLE_DIFF_MAX_INPUT_CHARACTERS,
  assertTitleDiffInputWithinLimits,
  buildTitleDiff
} from "../../feedback/title-diff.mjs";
...
function maxTitleLengthFromPayload(payload = {}) {
  ...
  return Math.min(TITLE_DIFF_MAX_INPUT_CHARACTERS, Math.max(1, Math.trunc(value || 80)));
}
...
export function buildV4FeedbackArtifacts({
  ...
  datasetDisposition = FEEDBACK_DATASET_DISPOSITION
} = {}) {
  const requestedAction = String(action || "").trim().toUpperCase();
  if (!["ACCEPT", "EDIT", "REJECT"].includes(requestedAction)) {
    throw new Error("invalid_v4_feedback_action");
  }
  const normalizedAction = normalizeV4FeedbackAction(action);
  const normalizedDatasetDisposition = normalizeFeedbackDatasetDisposition(datasetDisposition);
  const semanticReviewAllowed = reviewedSemanticFields === true
    && normalizedDatasetDisposition !== ADMIN_TEST_DATASET_DISPOSITION;
  ...
  const titleMaxLength = maxTitleLengthFromPayload(resultPayload);
  const rawSubmittedWriterTitle = String(writerTitle || "");
  assertTitleDiffInputWithinLimits(rawSubmittedWriterTitle, {
    label: "feedback_writer_title",
    maxCharacters: titleMaxLength
  });
  const submittedWriterTitle = normalizeTitle(rawSubmittedWriterTitle);
  if (normalizedAction === "ACCEPT" && !generated) {
    throw new Error("feedback_ai_title_required_for_accept");
  }
  if (normalizedAction === "ACCEPT" && submittedWriterTitle && submittedWriterTitle !== generated) {
    throw new Error("feedback_accept_title_must_match_ai_title");
  }
  if (normalizedAction === "EDIT" && !submittedWriterTitle) {
    throw new Error("feedback_writer_title_required_for_edit");
  }
  if (normalizedAction === "EDIT" && submittedWriterTitle === generated) {
    throw new Error("feedback_edit_title_must_change_ai_title");
  }
  ...
  assertTitleDiffInputWithinLimits(writerFinalTitle, {
    label: "feedback_writer_title",
    maxCharacters: titleMaxLength
  });
  ...
  const feedbackTrainingEvent = buildFeedbackLoopEvent({
    ...
    reviewOutcome,
    stableTrainingSample: semanticReviewAllowed,
    reviewedSemanticFields: semanticReviewAllowed
  });
  ...
  feedbackTrainingEvent.dataset_disposition = normalizedDatasetDisposition;
  ...
  const writerFeedback = {
    ...
    dataset_disposition: normalizedDatasetDisposition,
  ...
  const feedbackEvent = {
    ...
    // The production RPC keeps immutable feedback rows OBSERVE_ONLY. The
    // narrower admin-test use survives inside writer_feedback instead.
    dataset_disposition: FEEDBACK_DATASET_DISPOSITION
  };
  ...
  const learningEvent = {
    ...
    training_eligible: feedbackTrainingEvent.training_ready === true && semanticReviewAllowed
  };
  ...
  return {
    ...
    writerFeedback,
    semExtraction,
    feedbackDataUse: normalizedDatasetDisposition,
