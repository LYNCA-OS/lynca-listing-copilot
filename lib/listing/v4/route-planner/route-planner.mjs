...
export function planV4RecognitionRoute(payload = {}, env = process.env) {
  ...
  if (preingestionBundleId && exactAnchor && catalogAssist && approvedCandidateSupport) {
    return routePlan({
      ...
      route_reason: "preingestion bundle and exact anchor can constrain the background assist lane; L1 still returns from direct fast scout",
      blocking_modules: [
      ...
      pending_modules: ["fast_scout_observation", "writer_review"],
      skipped_modules: [
  ...
  if ((catalogAssist || (vectorAssist && vectorReady)) && approvedCandidateSupport) {
    return routePlan({
      route: "ASSISTED_FULL",
      route_reason: "catalog/vector assist is available for the background L2 lane; L1 returns from direct fast scout first",
      blocking_modules: [
        "signed_read_url_preparation",
        "fast_scout_observation",
        "resolver_safety_check",
  ...
  if (count > 0 || preingestionBundleId) {
    return routePlan({
      ...
      route_reason: marketplaceBlind
        ? "marketplace/blind input has no approved catalog identity; return fast safe draft and continue assist in background"
        : "no approved candidate assist is available; generate safe one-line draft from direct evidence",
      blocking_modules: [
        "signed_read_url_preparation",
        "fast_scout_observation",
        "resolver_safety_check",
