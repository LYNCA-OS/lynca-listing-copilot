...
export const recognitionRoutePlannerPolicyVersion = "v4-recognition-route-planner-v2-provider-options-owner";
...
function hasExactAnchor(payload = {}) {
  const formalRoute = String(payload.v4_anchor_probe?.plan?.route || "");
  return ["TCG_EXACT_LOOKUP", "SPORTS_COMPOSITE_LOOKUP"].includes(formalRoute);
}
...
export function planV4RecognitionRoute(payload = {}, env = process.env) {
  ...
  // Execution already resolves provider controls from `provider_options` plus
  // environment defaults. Route planning must consume that same contract or a
  // benchmark can report one route while running a different assist policy.
  const effectiveProviderOptions = providerOptionsFromPayload(payload, env);
  const catalogAssist = optionFlag(effectiveProviderOptions, "enable_catalog_assist", true);
  const vectorAssist = optionFlag(effectiveProviderOptions, "enable_vector_assist", true)
    && optionFlag(effectiveProviderOptions, "enable_vector_retrieval", true)
    && String(effectiveProviderOptions.vector_retrieval_mode || "assist").trim().toLowerCase() !== "off";
  ...
  const formalAnchorRoute = String(payload.v4_anchor_probe?.plan?.route || "");
  const formalAnchorFinalized = payload.v4_anchor_probe?.finalized === true;

  if (preingestionBundleId && exactAnchor && catalogAssist && (approvedCandidateSupport || formalAnchorFinalized)) {
    return routePlan({
  ...
  if (preingestionBundleId && catalogAssist
    && ["TCG_EXACT_LOOKUP", "SPORTS_COMPOSITE_LOOKUP"].includes(formalAnchorRoute)) {
    return routePlan({
      route: "ANCHOR_CONSTRAINED_L2",
      route_reason: "a direct natural key was extracted before L2, but no unique trusted identity finalized; use it to constrain catalog retrieval and keep full L2 as fallback",
      blocking_modules: [
        "full_card_gpt_observation",
        "resolver_safety_check",
        "deterministic_renderer"
      ],
      conditionally_blocking_modules: ["catalog_exact_anchor_lookup", "ocr_crop_verifier"],
      background_modules: [
        "post_observation_catalog_lookup",
        "candidate_reranker",
        "sidecar_workflow_dispatch"
      ],
      pending_modules: ["l2_assisted_draft", "writer_review"],
      skipped_modules: vectorReady ? [] : ["visual_vector_online_query"],
      risk_flags: ["ANCHOR_NOT_UNIQUELY_RESOLVED"],
      expected_risk: "MEDIUM"
    });
  }
