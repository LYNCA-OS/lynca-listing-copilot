...
function targetFields(decision = {}) {
  return [...new Set([
    ...array(decision.visual_field_targets),
...
function assistSequence(route, decision = {}) {
  const visualTargets = array(decision.visual_field_targets).map(cleanText).filter(Boolean);
  ...
  if (visualTargets.length) {
    stages.push(Object.freeze({
      ...
      image_access: "RELEVANT_CROPS_ONLY",
      max_paid_calls: 1,
...
export function planProviderAuxRouteShadow({
  ...
  return Object.freeze({
    ...
    visual_field_targets: Object.freeze(array(knowledgeFirstDecision.visual_field_targets).map(cleanText).filter(Boolean)),
    ...
    activation_blockers: Object.freeze([
      "SHADOW_ONLY",
      ...(route === providerAuxRoutes.TARGETED_MODEL_ASSIST ? ["TARGETED_EXECUTOR_NOT_IMPLEMENTED"] : []),
...
export function withObservedProviderAuxRoute(shadow = null, {
  ...
  return Object.freeze({
    ...shadow,
    observed_production_action: calls === null
      ? "UNKNOWN"
      : calls > 0
        ? "RUN_FULL_PROVIDER"
