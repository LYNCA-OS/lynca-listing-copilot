...
function canonicalEvidenceValueKey(value) {
...
function canonicalFieldState(evidenceDocument = {}, aliases = []) {
  ...
  const publishableValueKeys = new Set(publishable.map(({ entry }) => (
    canonicalEvidenceValueKey(entry.normalized_value ?? entry.value)
  )));
  if (publishableValueKeys.size > 1) {
    return {
      value: null,
      state: "CONFLICT",
      source_field: publishable.map(({ field }) => field).join("|") || null,
      evidence_statuses: statuses
    };
  }

  const confirmed = publishable[0];
  if (confirmed) {
    return {
...
function canonicalSnapshot(evidenceDocument = {}) {
  const requiredIdentityFields = new Set(["year", "manufacturer", "players", "product"]);
  const states = {
    ...
    ...Object.fromEntries(literalIdentityFields.map((field) => [
      field,
      canonicalFieldState(evidenceDocument, [field])
    ]))
  };
  const literalIdentityEntries = literalIdentityFields
    .map((field) => ({ field, state: states[field] }))
    .filter(({ state }) => state.state === "PUBLISHABLE" && present(state.value));
  const literalIdentityValues = Object.freeze(Object.fromEntries(
    literalIdentityEntries.map(({ field, state }) => [field, state.value])
  ));
  const primaryLiteralIdentity = literalIdentityEntries[0] || null;
  return {
    ...Object.fromEntries(Object.entries(states).map(([field, state]) => [field, state.value])),
    // These fields are complementary identity facts, not aliases. Keep every
    // value and state independent so a legitimate set + card-code pair cannot
    // be misclassified as a conflict merely because their strings differ.
    literal_identity: primaryLiteralIdentity?.state.value ?? null,
    literal_identity_source_field: primaryLiteralIdentity?.field ?? null,
    literal_identity_fields: literalIdentityValues,
...
function validForwardEnumerationRow(row = {}, field = "", status = "") {
  ...
  const provenanceComplete = String(provenance.source || "").trim() === "CATALOG_CONSTRAINT_SNAPSHOT"
    ...
    && present(provenance.source_sha256)
