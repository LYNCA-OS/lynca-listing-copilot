...
const routeFieldExpansions = Object.freeze({
...
export function expandTargetedVisualFields(targetFields = []) {
  const requested = (Array.isArray(targetFields) ? targetFields : [])
    .map(cleanText)
    .filter(Boolean)
    .flatMap((field) => routeFieldExpansions[field] || [field]);
...
export function selectTargetedVisualImages(images = [], targetFields = [], {
  ...
  const requirementsCoveredByCrops = unique(requiredTargets.map(cleanText)).every((requirement) => {
    ...
    const fields = routeFieldExpansions[requirement] || [requirement];
...
function requirementText(target) {
  return target === "card_name_or_insert_or_code"
    ? `any one of [${literalIdentityFields.join(", ")}]`
    : target;
}
...
export function targetedVisualObservationSafety(parsed = {}, requestedTargets = [], { knownFields = {} } = {}) {
  ...
  for (const target of unique((Array.isArray(requestedTargets) ? requestedTargets : []).map(cleanText))) {
    if (target === "card_name_or_insert_or_code") {
      if (!literalIdentityFields.some((field) => valuePresent(fields[field]))) missing.push(target);
...
export async function runTargetedVisualObservation({
  ...
  const parsed = expandTargetedVisualPacket(result.parsed, targetFields, { selectedImages });
  const safety = targetedVisualObservationSafety(parsed, requiredTargets, { knownFields });
  return {
    ...result,
