...
// The default ExcelJS Node entry recursively loads hundreds of modules and can
// dominate a cold serverless invocation. The package's bundled build exposes
// the same Workbook API while loading as one deterministic artifact.
...
function buildExportObjectPath(batchId, now = new Date()) {
  const yyyyMm = now.toISOString().slice(0, 7).replace("-", "/");
  return `exports/writer-batches/${yyyyMm}/${batchId}.xlsx`;
}
...
function buildItemRows({ batchId, rows }) {
  return rows.map((row) => ({
...
export async function createWriterBatchExport({
  ...
  const objectPath = buildExportObjectPath(batchId, now);
  ...
  const itemRows = buildItemRows({ batchId, rows: normalizedRows });
  const manifest = {
