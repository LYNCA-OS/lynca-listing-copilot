...
const terminalFieldAliases = Object.freeze({
  year: ["year"],
  ...
  numerical_rarity: ["print_run_number", "numerical_rarity", "serial_number", "serial_denominator", "numbered_to"],
...
function collectorNumberMigratedToNumericalRarity(row = {}, resolved = {}) {
  if (row.field_group !== "collector_number") return false;
  const sourceValues = [
    ...(Array.isArray(row.resolved_values) ? row.resolved_values : []),
    ...(Array.isArray(row.raw_values) ? row.raw_values : [])
  ].map(comparableFieldValue).filter(Boolean);
  const destinationValues = [
    resolved.print_run_number,
    resolved.numerical_rarity,
    resolved.serial_number
  ].map(comparableFieldValue).filter(Boolean);
  return sourceValues.some((value) => /^\d+\/\d+$/.test(value) && destinationValues.includes(value));
}

function terminalFieldFlow(baseFieldFlow, session = {}) {
  const hasTerminalResolvedFields = Boolean(
    session.resolved_fields
    && typeof session.resolved_fields === "object"
    && !Array.isArray(session.resolved_fields)
  );
  if (!baseFieldFlow || !hasTerminalResolvedFields) return baseFieldFlow || null;

  const resolved = session.resolved_fields;
  const rows = (Array.isArray(baseFieldFlow.fields) ? baseFieldFlow.fields : []).map((row) => {
    const aliases = terminalFieldAliases[row.field_group] || [row.field_group];
    const terminalResolvedPresent = aliases.some((alias) => hasValue(resolved[alias]));
    const migratedToNumericalRarity = !terminalResolvedPresent
      && collectorNumberMigratedToNumericalRarity(row, resolved);
    // Once the upstream resolver has retained a value, REVIEW means "show it
    // with caution", not "silently erase it at the V4 persistence boundary".
    // This also covers catalog/OCR-added values that were not in raw provider
    // output. Intentional omission must happen before resolved_present=true.
    const unexplainedTerminalDrop = row.resolved_present === true
      && terminalResolvedPresent === false
      && migratedToNumericalRarity === false;
    return {
      ...row,
      pipeline_disposition: row.disposition,
      terminal_resolved_present: terminalResolvedPresent,
      terminal_expected_present: row.resolved_present === true && migratedToNumericalRarity === false,
      terminal_semantic_migration: migratedToNumericalRarity,
      terminal_destination_fields: migratedToNumericalRarity ? ["numerical_rarity"] : [],
      terminal_drop_reason: unexplainedTerminalDrop
        ? "upstream_resolved_value_missing_from_v4_session"
        : null,
      disposition: unexplainedTerminalDrop
        ? "UNEXPLAINED_TERMINAL_DROP"
        : migratedToNumericalRarity
          ? "MIGRATED_TO_NUMERICAL_RARITY"
          : row.disposition
    };
  });
  const terminalDrops = rows.filter((row) => row.disposition === "UNEXPLAINED_TERMINAL_DROP");
  return {
    ...baseFieldFlow,
    terminal_resolved_field_count: Object.entries(terminalFieldAliases)
      .filter(([, aliases]) => aliases.some((alias) => hasValue(resolved[alias])))
      .length,
    unexplained_terminal_drop_count: terminalDrops.length,
    unexplained_terminal_drop_fields: terminalDrops.map((row) => row.field_group),
    grade_atomic: {
      ...(baseFieldFlow.grade_atomic || {}),
      terminal: {
        grade_company: hasValue(resolved.grade_company),
        card_grade: hasValue(resolved.card_grade || resolved.grade),
        auto_grade: hasValue(resolved.auto_grade)
      }
    },
    fields: rows
  };
...
export function buildEndToEndNodeLedger({
  ...
  const writerReviewRequired = display.display_status === "WRITER_REVIEW"
  ...
  const deterministicTitleSource = /^(?:v4_csm_deterministic_renderer|deterministic_renderer_finalizer|exact_anchor_catalog_finalized|pre_l2_anchor_catalog_finalized)$/.test(titleRenderSource);
  ...
  const finalFieldFlow = terminalFieldFlow(base?.field_flow, session || {});
  const lifecycleNodes = [
    ...
    },
    {
      ...
      metrics: {
        ...
        patch_count: optionalNumber(v4L2Timing.pre_l2_anchor_patch_count),
        anchor_count: optionalNumber(v4L2Timing.pre_l2_anchor_candidate_count),
        direct_anchor_count: optionalNumber(v4L2Timing.pre_l2_anchor_direct_candidate_count),
        anchor_type_breakdown: v4L2Timing.pre_l2_anchor_type_breakdown || {},
        context_dimensions: optionalNumber(v4L2Timing.pre_l2_anchor_context_dimensions),
        direct_context_dimensions: optionalNumber(v4L2Timing.pre_l2_anchor_direct_context_dimensions),
        lookup_attempted: v4L2Timing.pre_l2_anchor_lookup_attempted === true,
        catalog_candidate_count: optionalNumber(v4L2Timing.pre_l2_anchor_catalog_candidate_count),
        trusted_candidate_count: optionalNumber(v4L2Timing.pre_l2_anchor_trusted_candidate_count),
        eligible_candidate_count: optionalNumber(v4L2Timing.pre_l2_anchor_eligible_candidate_count),
        input_reason_codes: Array.isArray(v4L2Timing.pre_l2_anchor_input_reason_codes)
          ? v4L2Timing.pre_l2_anchor_input_reason_codes
          : [],
        reason_codes: Array.isArray(v4L2Timing.pre_l2_anchor_reason_codes)
          ? v4L2Timing.pre_l2_anchor_reason_codes
          : [],
        current_code_patch_count: optionalNumber(v4L2Timing.pre_l2_anchor_current_code_patch_count),
        typed_direct_code_anchor_count: optionalNumber(v4L2Timing.pre_l2_anchor_typed_direct_code_anchor_count),
        sports_pre_provider_reachability: v4L2Timing.pre_l2_anchor_sports_pre_provider_reachability || null
      }
    },
    {
      node_id: "full_l2_provider_decision",
      category: "orchestration",
      status: preL2FullL2Skipped
        ? "SKIPPED"
      ...
      duration_ms: optionalNumber(v4L2Timing.recognition_core_ms ?? v4L2Timing.v2_call_ms),
      attempts: preL2FullL2Skipped ? 0 : 1,
      input_count: 1,
      output_count: preL2FullL2Skipped ? 0 : (workerStatus === "COMPLETED" ? 1 : 0),
      error_code: workerStatus === "FAILED" ? (job.error?.code || "V4_JOB_FAILED") : null,
    ...
    },
    {
      ...
      category: "output",
      status: writerReviewRequired
        ? "SKIPPED"
        : writerReady
          ? (deterministicTitleSource ? "COMPLETED" : "FAILED")
          : workerStatus === "FAILED"
            ? "FAILED"
            : "RUNNING",
      expected: !writerReviewRequired,
      duration_ms: null,
      attempts: writerReady ? 1 : 0,
      input_count: writerReady && !writerReviewRequired ? 1 : 0,
      output_count: deterministicTitleSource ? 1 : 0,
      error_code: writerReady && !writerReviewRequired && !deterministicTitleSource ? "NON_DETERMINISTIC_WRITER_TITLE" : null,
      skip_reason: writerReviewRequired ? "grounded_identity_abstained_without_safe_title" : null,
  ...
  const persistedWriterTerminal = Boolean(
    session?.l2_status === "READY"
    && (
      session?.l2_title
      || session?.final_title
      || session?.status === "WRITER_REVIEW"
      || session?.provider_result_summary?.writer_review_required === true
      || session?.provider_result_summary?.assisted_draft_status === "REVIEW_REQUIRED"
    )
  );
  const writerStateMatches = writerReady === persistedWriterTerminal;
  const checks = [
    ...
    },
    {
      ...
      actual: persistedWriterTerminal,
      detail: null
    },
    {
    ...
    ...(finalFieldFlow ? [{
      ...
      status: Number(finalFieldFlow.unexplained_terminal_drop_count || 0) === 0 ? "PASS" : "FAIL",
      severity: "ERROR",
      expected: 0,
      actual: Number(finalFieldFlow.unexplained_terminal_drop_count || 0),
      detail: (finalFieldFlow.unexplained_terminal_drop_fields || []).join(",") || null
    }] : []),
    ...(writerReady && !writerReviewRequired ? [{
      check_id: "writer_title_uses_deterministic_csm_renderer",
      status: deterministicTitleSource ? "PASS" : "FAIL",
      severity: "ERROR",
      expected: "v4_csm_deterministic_renderer, deterministic_renderer_finalizer, exact_anchor_catalog_finalized, or pre_l2_anchor_catalog_finalized",
