...
export function statusPollQueueSelfHealPlan(jobs = [], {
  ...
  const runningCount = rows.filter((job) => (
    job.status === v4JobStatuses.RUNNING
    && job.queue_tags?.provider_capacity_released !== true
    && !String(job.queue_tags?.provider_capacity_released_at || "").trim()
  )).length;
