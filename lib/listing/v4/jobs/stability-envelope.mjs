...
function waveMetrics(report = {}, waveIndex = 0) {
  ...
  return {
    ...
    ocr_timeout_count: Number(summary.preingestion_ocr?.timeout_count || 0) + Number(summary.preingestion_ocr?.worker_timeout_count || 0),
...
export function analyzeV4StabilityEnvelope(reports = [], {
  ...
  if (waves.length < minimumWaves) rejectionReasons.push("STABILITY_WAVE_COUNT_TOO_SMALL");
  if (attemptedCount < minimumCards) rejectionReasons.push("STABILITY_SAMPLE_TOO_SMALL");
  if (tenants.length < minimumTenants) rejectionReasons.push("MULTI_TENANT_EVIDENCE_MISSING");
  if (requireFreshSamples && waves.some((wave) => !wave.fresh_sample)) rejectionReasons.push("FRESH_SAMPLE_POLICY_NOT_PROVEN");
  if (assetIds.length !== uniqueAssetIds.size) rejectionReasons.push("CROSS_WAVE_SAMPLE_REUSE");
  if (technicalAvailability === null || technicalAvailability < minimumTechnicalAvailability) rejectionReasons.push("TECHNICAL_AVAILABILITY_BELOW_TARGET");
  if (sumField(waves, "residual_backlog_count") > 0) rejectionReasons.push("QUEUE_DID_NOT_DRAIN");
  if (queueWaitMax !== null && queueWaitMax > maximumQueueWaitMs) rejectionReasons.push("QUEUE_STARVATION_DETECTED");
  if (writerP95 === null || writerP95 > maximumWriterP95Ms) rejectionReasons.push("WRITER_P95_ABOVE_BUDGET");
  if (tenantCompletionFairness === null || tenantCompletionFairness < minimumTenantCompletionFairness) rejectionReasons.push("TENANT_COMPLETION_FAIRNESS_BELOW_TARGET");
  if (recoveredRetryRate !== null && recoveredRetryRate > maximumRecoveredRetryRate) rejectionReasons.push("RECOVERED_RETRY_RATE_ABOVE_TARGET");
  if (leaseRecoveryRate !== null && leaseRecoveryRate > maximumLeaseRecoveryRate) rejectionReasons.push("LEASE_RECOVERY_RATE_ABOVE_TARGET");
  if (throughputCv !== null && throughputCv > maximumThroughputCv) rejectionReasons.push("THROUGHPUT_UNSTABLE_ACROSS_WAVES");
  if (writerP95Cv !== null && writerP95Cv > maximumWriterP95Cv) rejectionReasons.push("WRITER_TAIL_UNSTABLE_ACROSS_WAVES");
  if (lastWaveP95Regression !== null && lastWaveP95Regression > maximumLastWaveP95Regression) rejectionReasons.push("TAIL_LATENCY_DEGRADES_OVER_TIME");
  ...
  const strictZeroFields = [
    ...
    ["ocr_timeout_count", "OCR_TIMEOUT_PRESENT"],
  ...
  for (const [field, reason] of strictZeroFields) {
    if (sumField(waves, field) > 0) rejectionReasons.push(reason);
  }
  if (waves.some((wave) => wave.batch_status_fatal_error)) rejectionReasons.push("STATUS_CONTROL_PLANE_FATAL_ERROR");
  ...
  const tenantQueueSpreadMs = tenantQueueP95Values.length
    ...
    : null;
  if (tenantQueueSpreadMs !== null && tenantQueueSpreadMs > maximumTenantQueueP95SpreadMs) rejectionReasons.push("TENANT_QUEUE_WAIT_SPREAD_ABOVE_TARGET");
