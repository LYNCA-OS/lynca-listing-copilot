...
export function isClientStatusNotFound(error = null) {
  ...
  return status === 404
    || code === "V4_JOB_STATUS_NOT_FOUND"
    || /(?:status_poll_)?job_missing|recognition jobs? not found/i.test(message);
}

export function shouldDeclareClientStatusOrphan({
  jobNotFoundCount = 0,
  sessionNotFoundCount = 0,
  elapsedMs = 0,
  minimumFailures = 3,
  minimumElapsedMs = 20_000
} = {}) {
  return Math.floor(nonNegativeNumber(jobNotFoundCount)) >= Math.max(1, Math.floor(nonNegativeNumber(minimumFailures)))
    && Math.floor(nonNegativeNumber(sessionNotFoundCount)) >= Math.max(1, Math.floor(nonNegativeNumber(minimumFailures)))
    && nonNegativeNumber(elapsedMs) >= Math.max(1_000, nonNegativeNumber(minimumElapsedMs));
}
