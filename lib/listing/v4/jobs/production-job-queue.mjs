...
function withCanonicalRows(result = {}) {
  return {
    ...result,
    rows: Array.isArray(result.rows)
      ? result.rows.map(withCanonicalV4JobState)
      : []
  };
}

async function readTenantRowsByIds({
  tableName,
  ids = [],
  tenantId,
  select = "*",
  ...
  if (!normalizedIds.length || !tenantId) return { ok: true, rows: [], error: null };
  const quoted = normalizedIds.map((id) => `"${id.replaceAll('"', '\\"')}"`).join(",");
  return readV4Rows({
    table: tableName,
    select,
    search: {
      id: `in.(${quoted})`,
      tenant_id: `eq.${String(tenantId).slice(0, 120)}`,
...
export function createV4DeterministicJobId({
  ...
  if (idempotencyKey) return deterministicV4Id("v4job", ["explicit", tenantId, idempotencyKey, jobType]);
...
export function createV4DeterministicSessionId({
  ...
  if (idempotencyKey) return deterministicV4Id("v4sess", ["explicit", tenantId, idempotencyKey]);
  if (!batchId || !assetId) return null;
...
export function normalizeV4JobInput({
  ...
  const createdByUserId = job.created_by_user_id || payload.created_by_user_id || effectiveOperatorId || null;
  const assignedToUserId = job.assigned_to_user_id || payload.assigned_to_user_id || createdByUserId;
  ...
  return compact({
    ...
    max_attempts: positiveInteger(job.max_attempts, v4JobRetryPolicy.maxAttempts, { min: 1, max: 10 }),
...
export function expandV4RecognitionStageJobs({
  ...
  for (const input of Array.isArray(jobs) ? jobs : []) {
    ...
    if (effectiveTenantId) basePayload.tenant_id = effectiveTenantId;
    const baseJob = {
      ...input,
      payload: basePayload,
      batch_id: effectiveBatchId,
...
export async function enqueueV4RecognitionJob({
  ...
  return {
    ...saved,
    row: withCanonicalV4JobState(saved.row || row),
    rows: Array.isArray(saved.rows) ? saved.rows.map(withCanonicalV4JobState) : []
  };
}
...
export async function enqueueV4RecognitionJobs({
  ...
  const effectiveTenantId = String(tenantId || "").trim().slice(0, 120);
  if (!effectiveTenantId) throw new TypeError("tenant_id_required");
  const prepared = (Array.isArray(jobs) ? jobs : [])
    .filter(Boolean)
    .map((job) => normalizeV4JobInput({ job, batchId, operatorId, tenantId: effectiveTenantId, priority }));
  const batchCreatorId = prepared.find((row) => row.created_by_user_id)?.created_by_user_id || operatorId || null;
  const batchWrite = await writeV4Row({
    table: "v4_recognition_batches",
    row: {
      id: batchId,
      tenant_id: effectiveTenantId,
      created_by_user_id: batchCreatorId,
      status: "QUEUED",
  ...
  if (!batchWrite.saved) {
    return {
      ...
      jobs: prepared.map((row) => ({
        saved: false,
        row: withCanonicalV4JobState(row),
        error: `recognition_batch_create_failed:${batchWrite.error || "unknown_error"}`
      })),
      queued_count: 0,
      inserted_count: 0,
      deduplicated_count: 0,
      persistence_mode: "batch_failed",
      batch_saved: false,
  ...
  if (preparedJobIds.length) {
    const existingJobs = await readTenantRowsByIds({
      tableName: table,
      ids: preparedJobIds,
      tenantId: effectiveTenantId,
      env,
  ...
  if (sessionIds.length) {
    const existing = await readTenantRowsByIds({
      tableName: "v4_recognition_sessions",
      ids: sessionIds,
      tenantId: effectiveTenantId,
      select: "id",
      env,
  ...
  if (missingSessionRows.length) {
    ...
    if (sessionBatch.saved) {
      const confirmedSessionIds = new Set((sessionBatch.rows || []).map((row) => row?.id).filter(Boolean));
      const unresolvedSessionIds = missingSessionRows
        .map((row) => row.id)
        .filter((id) => !confirmedSessionIds.has(id));
      if (unresolvedSessionIds.length) {
        const confirmed = await readTenantRowsByIds({
          tableName: "v4_recognition_sessions",
          ids: unresolvedSessionIds,
          tenantId: effectiveTenantId,
          select: "id",
          env,
          fetchImpl
        ...
        if (confirmed.ok) {
          confirmed.rows.forEach((row) => {
            if (row?.id) confirmedSessionIds.add(row.id);
          });
        }
      }
      for (const sessionRow of missingSessionRows) {
        const confirmed = confirmedSessionIds.has(sessionRow.id);
        sessionWrites.set(sessionRow.id, {
          saved: confirmed,
          error: confirmed ? null : "tenant_scoped_session_conflict"
        });
        if (confirmed) existingSessions.add(sessionRow.id);
      }
    } else {
      ...
      for (const sessionRow of missingSessionRows) {
        ...
        let confirmed = saved.saved && saved.row?.id === sessionRow.id;
        if (saved.saved && !confirmed) {
          const reread = await readTenantRowsByIds({
            tableName: "v4_recognition_sessions",
            ids: [sessionRow.id],
            tenantId: effectiveTenantId,
            select: "id",
          ...
          confirmed = reread.ok && reread.rows.some((row) => row?.id === sessionRow.id);
        }
        sessionWrites.set(sessionRow.id, {
          saved: confirmed,
          error: confirmed ? null : saved.error || "tenant_scoped_session_conflict"
        });
        if (confirmed) existingSessions.add(sessionRow.id);
      }
    }
  ...
  for (const row of prepared) {
    ...
    if (existingJob) {
      results.push({ saved: true, row: withCanonicalV4JobState(existingJob), error: null, deduplicated: true });
      continue;
    }
  ...
  if (eligibleRows.length) {
    ...
    if (jobBatch.saved) {
      ...
      const unresolvedJobIds = eligibleRows
        .map((row) => row.id)
        .filter((id) => !savedById.has(id));
      const confirmedAfterConflict = unresolvedJobIds.length
        ? await readTenantRowsByIds({
          tableName: table,
          ids: unresolvedJobIds,
          tenantId: effectiveTenantId,
          env,
          fetchImpl
        })
        : { ok: true, rows: [] };
      const confirmedById = new Map((confirmedAfterConflict.rows || []).map((row) => [row.id, row]));
      for (const row of eligibleRows) {
        const savedRow = savedById.get(row.id) || confirmedById.get(row.id) || null;
        results.push({
          saved: Boolean(savedRow),
          row: withCanonicalV4JobState(savedRow || row),
          error: savedRow ? null : "tenant_scoped_job_conflict",
          deduplicated: !savedById.has(row.id) && Boolean(savedRow)
        });
      }
    } else {
      persistenceMode = "bulk_with_row_fallback";
      for (const row of eligibleRows) {
        ...
        let savedRow = saved.row || null;
        if (saved.saved && !savedRow) {
          const reread = await readTenantRowsByIds({
            tableName: table,
            ids: [row.id],
            tenantId: effectiveTenantId,
          ...
          savedRow = reread.rows?.find((entry) => entry?.id === row.id) || null;
        }
        results.push({
          ...saved,
          saved: saved.saved && Boolean(savedRow),
          row: withCanonicalV4JobState(savedRow || row),
          error: saved.saved && !savedRow ? "tenant_scoped_job_conflict" : saved.error || null,
          deduplicated: saved.saved && !saved.row && Boolean(savedRow)
        });
      }
    }
  }
...
export async function claimV4RecognitionJobs({
  ...
  if (primary.ok || !capacityControl || !isMissingBalancedCapacityRpc(primary.error)) {
    return {
      ...withCanonicalRows(primary),
      rpc_mode: capacityControl ? "balanced_capacity" : "legacy"
    };
  }
...
export async function heartbeatV4RecognitionJob({
  ...
  if (!required && !v4JobLeaseHeartbeatEnabled(env)) {
...
export async function completeV4RecognitionJob({
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (completion.saved) {
      return {
        ...completion,
        row: withCanonicalV4JobState(completion.row),
...
export async function failV4RecognitionJob({
  ...
  if (!job.id) return { saved: false, row: null, error: "missing_job_id" };
  const attemptCount = positiveInteger(job.attempt_count, 0, { min: 0, max: 100 });
  const maxAttempts = positiveInteger(job.max_attempts, v4JobRetryPolicy.maxAttempts, { min: 1, max: 100 });
  const retryPlan = planV4JobRetry({
    attemptCount,
    maxAttempts,
    error,
    forceFinalFailure
  ...
  const currentFailure = {
    ...
    code: retryPlan.classification.code,
    http_status: retryPlan.classification.httpStatus,
    retryable: retryPlan.classification.retryable,
    will_retry: retryPlan.shouldRetry,
    retry_delay_seconds: retryPlan.retryDelaySeconds,
    classification: retryPlan.classification.category,
    failed_at: nowIso()
  };

  const failed = await callV4Rpc({
  ...
  if (!failed.ok) {
    return {
      saved: false,
      row: null,
      error: failed.error || "job_failure_rpc_failed",
      retry_plan: retryPlan
    };
  }
  const row = withCanonicalV4JobState(failed.rows?.[0] || null);
  if (!row) {
    return {
      saved: false,
      row: null,
...
export async function readV4RecognitionJobs({
  ...
  if (tenantId) search.tenant_id = `eq.${String(tenantId).slice(0, 120)}`;
  ...
  const result = await readV4Rows({ table, select: "*", search, env, fetchImpl });
  return withCanonicalRows(result);
}
...
function queueTagsForManualRetry(queueTags = {}, requestedAt = nowIso(), requestedByUserId = "") {
  ...
  return {
    ...
    manual_retry_requested_by_user_id: String(requestedByUserId || "").trim() || null,
...
export async function retryV4RecognitionJob({
  ...
  if (!normalizedJobId || !normalizedOperatorId || !normalizedTenantId) {
    return {
      saved: false,
      retryable: false,
      error_code: !normalizedJobId
        ? "V4_JOB_RETRY_JOB_ID_REQUIRED"
        : !normalizedTenantId
          ? "V4_JOB_RETRY_TENANT_REQUIRED"
          : "V4_JOB_RETRY_OPERATOR_REQUIRED",
  ...
  // RETRY_JOB is a tenant-scoped Manager/Owner capability. The API supplies
  // the actor only for audit metadata; the durable ownership boundary here is
  // tenant_id, never the job's original operator_id.
  const job = existing.rows.find((row) =>
    String(row?.tenant_id || "") === normalizedTenantId
  ) || null;
  if (!job) {
    return {
      saved: false,
  ...
  const maxAttempts = Math.max(
    positiveInteger(job.max_attempts, v4JobRetryPolicy.maxAttempts, { min: 1, max: 1000 }),
    attemptCount + 2
  );
    ...
    patch: {
      ...
      error: {
        ...
        manual_retry: {
          ...
          requested_by_user_id: normalizedOperatorId,
      ...
      queue_tags: queueTagsForManualRetry(job.queue_tags, requestedAt, normalizedOperatorId),
  ...
  return {
    ...
    row: withCanonicalV4JobState(retried.row || {
      ...job,
      status: v4JobStatuses.RETRYING,
      lane: v4JobLanes.INTERACTIVE,
      priority: 0,
