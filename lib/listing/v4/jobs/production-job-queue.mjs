...
export async function claimV4RecognitionJobs({
  ...
  lane = null,
  tenantId = null,
  lateProviderBinding = false,
  env = process.env,
  fetchImpl = globalThis.fetch
} = {}) {
  const normalizedLane = lane ? normalizeLane(lane, "") : null;
  const capacityControl = v4ProviderCapacityControlEnabled(env) && lateProviderBinding !== true;
  const deploymentAffinity = v4QueueDeploymentAffinity(env);
  ...
  if (primary.ok || deploymentAffinity || !capacityControl || !isMissingBalancedCapacityRpc(primary.error)) {
    return {
      ...primary,
      rpc_mode: capacityControl ? "balanced_capacity" : lateProviderBinding ? "late_provider_unbound" : "legacy"
    };
  }
