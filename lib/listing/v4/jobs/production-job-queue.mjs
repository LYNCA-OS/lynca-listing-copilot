...
import { callV4Rpc, isV4SupabaseConfigured, patchV4Row, readV4Rows, writeV4Row, writeV4Rows } from "../session/supabase-rest.mjs";
...
export async function enqueueV4RecognitionJobs({
  ...
  if (eligibleRows.length) {
    ...
    if (jobBatch.saved) {
      ...
      persistenceMode = "bulk_with_row_fallback";
      for (const row of eligibleRows) {
        const saved = await writeV4Row({ table, row, upsert: true, env, fetchImpl });
