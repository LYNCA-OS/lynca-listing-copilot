...
function immutableRecognitionPayload(payload = {}) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) return {};
  const immutable = { ...payload };
  // Browser timing is observability, not recognition intent. A response-lost
  // replay may measure a different duration while still referring to the exact
  // same immutable intake position, asset generation and server-owned policy.
  // Keeping telemetry in the identity would turn a safe replay into a conflict.
  delete immutable.clientTiming;
  delete immutable.client_timing;
  return immutable;
}
...
function immutableV4JobIdentity(job = {}) {
  return canonicalJson({
    ...
    payload: immutableRecognitionPayload(job.payload)
  });
}
...
export function normalizeV4JobInput({
  ...
  const benchmarkProfile = String(
    payload.provider_options?.recognition_benchmark_profile
    || payload.providerOptions?.recognition_benchmark_profile
    || ""
  ).trim();
  const noRetryEvaluationJob = benchmarkProfile === "cold_second_look_shadow_benchmark";
  return compact({
    ...
    max_attempts: noRetryEvaluationJob
      ? 1
      : positiveInteger(
          job.max_attempts,
