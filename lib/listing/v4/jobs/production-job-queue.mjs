...
export function v4QueueDeploymentAffinity(env = process.env) {
  ...
  // Preview work belongs to a chain-compatible execution profile, not to one
  // immutable Vercel deployment. Pinning to VERCEL_DEPLOYMENT_ID orphans every
  // in-flight durable job as soon as a patched Preview is deployed.
  const configured = String(env.V4_QUEUE_PREVIEW_AFFINITY || "").trim();
  if (/^preview:[A-Za-z0-9._:-]{1,100}$/.test(configured)) return configured;
...
const ephemeralQueuePayloadIdentityKeys = new Set([
  "storageVerificationToken",
  "storage_verification_token"
]);

function stableQueuePayloadIdentity(value) {
  if (Array.isArray(value)) return value.map(stableQueuePayloadIdentity);
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value)
      .filter(([key, child]) => child !== undefined && !ephemeralQueuePayloadIdentityKeys.has(key))
      .map(([key, child]) => [key, stableQueuePayloadIdentity(child)]));
  }
  return value;
}
...
function immutableV4JobIdentity(job = {}) {
  return canonicalJson({
    ...
    payload: job.payload && typeof job.payload === "object" && !Array.isArray(job.payload)
      ? stableQueuePayloadIdentity(job.payload)
      : {}
  });
}
...
export async function enqueueV4RecognitionJobs({
  ...
  if (!rpc.ok) {
    if (/queue_job_identity_conflict|queue_batch_identity_conflict/i.test(String(rpc.error || ""))) {
      const recovered = await recoverV4AtomicEnqueueIdentityConflict({
        prepared,
        batchId: normalizedBatchId,
        env,
        fetchImpl
...
export async function recoverV4AtomicEnqueueIdentityConflict({
  prepared = [],
  batchId = "",
  env = process.env,
  fetchImpl = globalThis.fetch,
  ...
  const expected = (Array.isArray(prepared) ? prepared : []).filter((row) => row?.id);
  if (!expected.length) return null;
  const quoted = expected.map((row) => `"${String(row.id).replaceAll('"', '\\"')}"`).join(",");
  const existing = await readRows({
    table,
    select: "*",
    search: { id: `in.(${quoted})`, limit: String(expected.length) },
    env,
    fetchImpl
  });
  if (!existing?.ok) return null;
  const byId = new Map((existing.rows || []).map((row) => [String(row?.id || ""), row]));
  if (expected.some((row) => !byId.has(row.id) || !v4JobIdentityMatches(byId.get(row.id), row))) return null;
  ...
  const jobs = expected.map((row) => {
    const persisted = byId.get(row.id);
    const terminalFailure = [v4JobStatuses.FAILED, v4JobStatuses.CANCELLED]
      .includes(String(persisted.status || "").toUpperCase());
    return terminalFailure ? {
      ...
      error: null,
      deduplicated: true,
      recovered_after_atomic_identity_conflict: true,
      canonical_payload_sha256: v4CanonicalJobIdentitySha256(persisted)
    };
  });
  const acceptedCount = jobs.filter((job) => job.saved).length;
  return {
    batchId: String(batchId || expected[0]?.batch_id || ""),
    jobs,
    accepted_count: acceptedCount,
    queued_count: jobs.filter((job) => job.saved && runnableStatuses.has(String(job.row?.status || "").toUpperCase())).length,
    inserted_count: 0,
    deduplicated_count: jobs.length,
    persistence_mode: "atomic_rpc_identity_conflict_recovered",
    batch_saved: true,
    session_rows_written: 0,
    job_rows_written: 0
  };
