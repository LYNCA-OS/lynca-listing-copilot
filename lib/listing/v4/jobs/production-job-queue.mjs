...
function makeLegacyQueueRpcProbePayload() {
  return {
    p_tenant_id: "probe_tenant",
    p_operator_id: "probe_operator",
    p_batch: {
...
export async function checkV4QueueRpcReady({
  ...
  if (!isV4SupabaseConfigured(env)) {
    return { ready: false, reason: "supabase_not_configured", probe_rows: null };
  }

  const payloads = [
    makeQueueRpcProbePayload(),
    makeLegacyQueueRpcProbePayload()
  ];

  let probe = { ok: false, rows: [], error: "rpc_not_attempted" };
  for (const payload of payloads) {
    probe = await callV4Rpc({
      fn: "enqueue_v4_recognition_batch_atomic",
      payload,
      env,
      fetchImpl
    });
    if (probe.ok || !isAtomicV4RpcSignatureError(probe.error)) {
      break;
    }
  }
  ...
  if (probe.ok) {
    return {
      ready: true,
      reason: null,
      probe_rows: Array.isArray(probe.rows) ? probe.rows : []
    };
  }

  const reason = String(probe.error || "unknown_error");
  if (isAtomicV4RpcSignatureError(reason)) {
    return {
      ready: false,
      reason: "signature_or_privilege_mismatch",
      probe_rows: [],
      error: reason
    };
  }
