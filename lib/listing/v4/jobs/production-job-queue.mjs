...
export async function readV4RecognitionJobs({
  ...
  const normalizedTenantId = String(tenantId || "").trim();
  if (!normalizedTenantId) return { ok: false, rows: [], error: "tenant_id_required" };
  ...
  search.tenant_id = `eq.${normalizedTenantId}`;
...
export async function retryV4RecognitionJob({
  ...
  // RETRY_JOB is a tenant-scoped Manager/Owner capability. The actor is
  // retained for audit metadata, but the durable ownership boundary is the
  // signed tenant, not the job's original operator.
  const job = existing.rows.find((row) => String(row?.tenant_id || "") === normalizedTenantId) || null;
  if (!job) {
    return {
      saved: false,
      retryable: false,
