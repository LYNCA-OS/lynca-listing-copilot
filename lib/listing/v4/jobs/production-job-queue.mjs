...
export const v4QueueEnqueueContractVersion = "v4-queue-identity-outbox-v1";

export async function checkV4QueueEnqueueContractReady({
  env = process.env,
  fetchImpl = globalThis.fetch
} = {}) {
  const result = await callV4Rpc({
    fn: "v4_queue_enqueue_contract_version",
    payload: {},
    env,
    fetchImpl
  });
  const observed = String(result.rows?.[0] || "").trim();
  return {
    ready: result.ok === true && observed === v4QueueEnqueueContractVersion,
    expected: v4QueueEnqueueContractVersion,
    observed: observed || null,
    reason: result.ok !== true
      ? `queue_enqueue_contract_probe_failed:${String(result.error || "unknown_error")}`
      : observed !== v4QueueEnqueueContractVersion
        ? "queue_enqueue_contract_version_mismatch"
        : null
  };
}
...
export function createV4DeterministicBatchId({
  ...
  // Queue sessions are writer-owned workflow records, so the signed actor is
  // part of their identity. Provider work is deduplicated independently by the
  // cross-writer exact-result cache and in-flight identity coalescer. Keeping
  // those two identities separate preserves Accept/Edit ownership without
  // allowing a browser nonce to purchase the same work twice for one writer.
...
const nonDecisionPayloadIdentityKeys = new Set([
  "client_timing",
  "clientTiming",
  "client_capture_quality",
  "capture_quality",
  "captureQuality",
  "category",
  "idempotency_key",
...
function immutableDecisionPayload(value = {}) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};
  return Object.fromEntries(Object.entries(value)
    .filter(([key]) => !nonDecisionPayloadIdentityKeys.has(key))
...
function immutableV4JobIdentity(job = {}) {
  return canonicalJson({
    ...
    operator_id: job.operator_id || job.created_by_user_id || null,
    ...
    payload: immutableDecisionPayload(job.payload)
  });
}
...
export function createV4DeterministicJobId({
  ...
  return deterministicV4Id("v4job", [tenantId, batchId, assetId, jobType]);
}
...
export function createV4DeterministicSessionId({
  ...
  if (!batchId || !assetId) return null;
  return deterministicV4Id("v4sess", [tenantId, batchId, assetId]);
}
...
export async function checkV4QueueRpcReady({
  ...
  const enqueueContract = await checkV4QueueEnqueueContractReady({ env, fetchImpl });
  ...
  if (!signatureProbe.ok) {
    ...
    if (isAtomicV4RpcDependencyError(reason)) {
      return {
        ready: false,
        enqueue_contract_ready: enqueueContract.ready,
        signature_ready: false,
        dependencies_ready: false,
        legacy_principal_ready: false,
  ...
  return {
    ready: signatureProbe.ok && dependencyReady && legacyPrincipal.ok && enqueueContract.ready,
    enqueue_contract_ready: enqueueContract.ready,
    signature_ready: true,
    dependencies_ready: dependencyReady,
    legacy_principal_ready: legacyPrincipal.ok,
    reason: !enqueueContract.ready
      ? enqueueContract.reason
      : legacyPrincipal.ok ? null : "queue_legacy_principal_not_ready",
    signature_error: null,
    dependency_error: null,
...
export async function enqueueV4RecognitionJobs({
  ...
  const semanticJobIdentities = prepared.map((row) => [
    row.tenant_id,
    row.operator_id,
    row.recognition_session_id,
    row.asset_id,
  ...
  if (new Set(semanticJobIdentities).size !== semanticJobIdentities.length) {
    return failure("atomic_enqueue_duplicate_job_identity", "atomic_validation_failed");
  }

  // The application never writes against the legacy RPC: without this
  // preflight a code-first rolling deploy could accept recognition while no
  // atomic OCR intent is recorded. The expand migration is dual-compatible,
  // so database-first remains zero-downtime for old instances.
  const enqueueContract = await checkV4QueueEnqueueContractReady({ env, fetchImpl });
  if (!enqueueContract.ready) {
    return failure(enqueueContract.reason || "queue_enqueue_contract_not_ready", "atomic_contract_not_ready");
  }
  ...
  if (transaction.queue_enqueue_contract_version !== v4QueueEnqueueContractVersion) {
    return failure("atomic_enqueue_contract_response_mismatch", "atomic_contract_mismatch");
  }
...
export async function completeV4RecognitionJob({
  ...
  // A few seconds of bounded idempotent retries is much cheaper than putting a
  // completed provider call back through the 120s durable-job retry cycle.
  const maxAttempts = positiveInteger(env.V4_JOB_COMPLETION_WRITE_ATTEMPTS, 5, { min: 1, max: 5 });
  const retryBaseMs = positiveInteger(env.V4_JOB_COMPLETION_RETRY_BASE_MS, 200, { min: 1, max: 2000 });
