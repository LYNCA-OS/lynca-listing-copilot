...
export function buildProviderCapacityTimeline({
  ...
  providerCapacityLateBinding = null,
  ...
  const lateBinding = providerCapacityLateBinding && typeof providerCapacityLateBinding === "object"
    ? providerCapacityLateBinding
    : {};
  const waitingProvider = iso(lateBinding.waiting_provider_at || slot.queued_at);
  const providerStarted = iso(slot.started_at);
  const providerCompleted = iso(slot.completed_at);
  const capacityAcquired = iso(lateBinding.acquired_at || job.queue_tags?.provider_capacity_leased_at);
  const capacityReleased = iso(providerCapacityReleasedAt);
  return Object.freeze({
    schema_version: providerCapacityTimelineSchemaVersion,
    late_binding_design_enabled: Boolean(lateBinding.binding_mode === "late_provider_lease_v1"),
