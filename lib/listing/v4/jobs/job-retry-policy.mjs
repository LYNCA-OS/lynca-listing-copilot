...
export function classifyV4JobError(error = {}) {
  ...
  const lifecycle = classifyAssetLifecycleFailure(input);
  ...
  // Input identity failures are not execution failures. Retrying the same
  // immutable job would only replay stale paths or a broken image generation.
  if (lifecycle.recovery_action === assetRecoveryActions.INPUT_REBIND) {
    return {
      retryable: false,
      code: lifecycle.code || code || "INPUT_STATE_INVALID",
      category: "input_rebind_required",
  ...
  if (input.retryable === true || input.recoverable === true) {
    return {
      retryable: true,
      code: code || "EXPLICIT_RETRYABLE",
      category: "explicit",
      recovery_action: assetRecoveryActions.EXECUTION_RETRY,
      httpStatus
    };
  }
  if (input.retryable === false || input.recoverable === false) {
    return {
      retryable: false,
      code: code || "EXPLICIT_NON_RETRYABLE",
      category: "explicit",
      recovery_action: assetRecoveryActions.NONE,
      httpStatus
    };
  }
  if (/exceeded your current quota|insufficient[_\s-]*quota|billing details|billing[_\s-]*hard[_\s-]*limit|quota exhausted/.test(message)) {
    return {
      retryable: false,
      code: code || "QUOTA_EXHAUSTED",
      category: "permanent_quota",
      recovery_action: assetRecoveryActions.NONE,
      httpStatus
    };
  }
  if (nonRetryableCodes.has(code)) {
    return { retryable: false, code, category: "permanent_code", recovery_action: assetRecoveryActions.NONE, httpStatus };
  }
  if (retryableCodes.has(code)) {
    return { retryable: true, code, category: "transient_code", recovery_action: assetRecoveryActions.EXECUTION_RETRY, httpStatus };
  }
  if (httpStatus !== null) {
  ...
  if (/invalid payload|unsupported image|unsupported input|permission denied|unauthori[sz]ed|forbidden/.test(message)) {
    return {
      retryable: false,
      code: code || "PERMANENT_INPUT_ERROR",
      category: "permanent_message",
      recovery_action: assetRecoveryActions.NONE,
      httpStatus
    };
  }
  if (/timeout|timed out|rate.?limit|too many requests|temporar|overload|connection reset|fetch failed|network/.test(message)) {
    return {
      retryable: true,
      code: code || "TRANSIENT_PROVIDER_ERROR",
      category: "transient_message",
      recovery_action: assetRecoveryActions.EXECUTION_RETRY,
      httpStatus
    };
  }
