...
function deploymentProtectionHeaders(env = process.env) {
  // Same-deployment fetches to VERCEL_URL still cross Deployment Protection.
...
export function scheduleTrustedV4QueuePump({
  ...
  const completion = (async () => {
    ...
    if (typeof acquireKick === "function" && dedupScope) {
      ...
      if (requireDedupAcquisition && (!acquired.ok || acquired.acquired !== true)) {
        return {
          ok: false,
          invoked: false,
          deduplicated: false,
          reason: "wake_dedup_backend_unavailable",
          error: acquired.error || "wake_dedup_acquisition_failed"
        };
      }
    }
