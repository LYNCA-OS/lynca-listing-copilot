...
export function configuredWorkerSecret(env = process.env) {
  // Queue execution and deployment-protection bypass are separate authority
  // domains. Never let a browser/deployment bypass token authorize paid work.
  return String(env.V4_JOB_WORKER_SECRET || "").trim();
}
