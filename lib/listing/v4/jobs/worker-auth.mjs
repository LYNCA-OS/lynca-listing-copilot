...
export function configuredWorkerSecret(env = process.env) {
  // Deployment-protection bypass credentials are browser/Preview authority,
  // never queue execution authority. Keep only the explicit worker secret and
  // the named legacy worker secret during migration.
  return String(env.V4_JOB_WORKER_SECRET || env.LYNCA_WORKER_SECRET || "").trim();
}
