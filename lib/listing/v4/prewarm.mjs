...
export function v4DeploymentInfo(env = process.env) {
  return {
    // Vercel system metadata belongs to the current immutable deployment.
    // Project-level Git variables can outlive many releases, so they are only
    // fallbacks for non-Vercel runtimes and must never mask the deployed SHA.
    git_commit_sha: deploymentValue(env, "VERCEL_GIT_COMMIT_SHA")
      || deploymentValue(env, "LYNCA_RELEASE_GIT_SHA")
      || deploymentValue(env, "GIT_COMMIT_SHA"),
    git_commit_ref: deploymentValue(env, "VERCEL_GIT_COMMIT_REF")
      || deploymentValue(env, "LYNCA_RELEASE_GIT_REF")
      || deploymentValue(env, "GIT_BRANCH"),
    ...
    deployment_url: deploymentValue(env, "VERCEL_URL")
  };
}
