...
export function v4DeploymentInfo(env = process.env) {
  return {
    // Manual Vercel deployments are not Git deployments and may retain stale
    // VERCEL_GIT_* metadata. Release automation supplies these explicit values
    // so health/evaluation reports remain tied to the deployed source.
    git_commit_sha: deploymentValue(env, "LYNCA_RELEASE_GIT_SHA")
      || deploymentValue(env, "GIT_COMMIT_SHA")
      || deploymentValue(env, "VERCEL_GIT_COMMIT_SHA"),
    git_commit_ref: deploymentValue(env, "LYNCA_RELEASE_GIT_REF")
      || deploymentValue(env, "GIT_BRANCH")
      || deploymentValue(env, "VERCEL_GIT_COMMIT_REF"),
