...
export { preL2AnchorInputTrace } from "./anchor-input-trace.mjs";
...
export async function probePreL2Anchors({
  ...
  const inputTrace = preL2AnchorInputTrace(payload, dossier, plan);
  ...
  const baseMetrics = {
    ...
    card_code_crop_available_count: inputTrace.card_code_crop_available_count,
    observable_card_code_job_count: inputTrace.observable_card_code_job_count,
    current_code_patch_count: inputTrace.current_code_patch_count,
    typed_direct_code_anchor_count: inputTrace.typed_direct_code_anchor_count,
    threshold_eligible_code_anchor_count: inputTrace.threshold_eligible_code_anchor_count
  };
  const result = {
    ...
    finalized: false,
    reason: plan.reason,
    reason_codes: Object.freeze([...inputTrace.reason_codes, "ANCHOR_LOOKUP_NOT_ATTEMPTED"]),
    metrics: baseMetrics,
    timing: {
  ...
  return {
    ...
    reason_codes: Object.freeze([...new Set([
      ...inputTrace.reason_codes,
      "ANCHOR_LOOKUP_ATTEMPTED",
      Number(finalize?.eligible_candidate_count || 0) > 0
        ? "ANCHOR_LOOKUP_ELIGIBLE_CANDIDATE_PRESENT"
        : "ANCHOR_LOOKUP_NO_ELIGIBLE_CANDIDATE",
      finalize?.finalized === true
        ? "ANCHOR_LOOKUP_FINALIZED"
        : "ANCHOR_LOOKUP_NOT_FINALIZED"
    ])]),
    finalize,
    metrics: {
      ...baseMetrics,
