...
const certNumberPattern = /^[0-9]{7,14}$/;

// Retail/product identifiers narrow a release family; they never identify a
// loose card by themselves. UPC-A/EAN-13/GTIN-14 are kept separate from slab
// certs so a bare barcode cannot accidentally enter the cert registry lane.
const barcodePattern = /^(?:[0-9]{8}|[0-9]{12,14})$/;

function explicitCertContext({ graderHint = "", fieldHint = "", cropHint = "" } = {}) {
  return Boolean(normalizeGrader(graderHint)
    || /(?:^|_)(?:cert|cert_number|slab_cert)(?:$|_)/i.test(cleanText(fieldHint))
    || /(?:grade_label|slab_label|slab_cert)/i.test(cleanText(cropHint)));
}

function explicitProductCodeContext({ fieldHint = "", cropHint = "" } = {}) {
  return /(?:barcode|upc|ean|gtin|product_code)/i.test(`${cleanText(fieldHint)} ${cleanText(cropHint)}`);
}

export function classifyAnchorText(raw, {
  graderHint = "",
  fieldHint = "",
  cropHint = ""
} = {}) {
  ...
  if (barcodePattern.test(compact) && explicitProductCodeContext({ fieldHint, cropHint })) {
    return {
      anchor_type: "product_code",
  ...
  if (certNumberPattern.test(compact) && explicitCertContext({ graderHint, fieldHint, cropHint })) {
  ...
  if (barcodePattern.test(compact)) {
    return {
      anchor_type: "barcode_candidate",
      anchor_class: anchorClasses.CATALOG,
      normalized: compact,
      lookup_key: false,
      lookup_target: null
...
export function collectAnchors({ resolved = {}, evidence = {} } = {}) {
  ...
  const anchors = [];
  const push = (value, sourceField, cropHint = "") => {
    const anchor = classifyAnchorText(value, { graderHint, fieldHint: sourceField, cropHint });
  ...
  for (const [field, entry] of Object.entries(evidence || {})) {
    ...
    push(
      entry?.value ?? entry,
      `evidence:${field}`,
      entry?.provenance?.crop_type || entry?.source_crop_id || entry?.crop_id || ""
    );
  }
...
export function strongestIdentityAnchor(anchors = []) {
  return anchors.find((anchor) => anchor.anchor_type === "tcg_card_code")
    || null;
}

export function strongestInstanceAnchor(anchors = []) {
