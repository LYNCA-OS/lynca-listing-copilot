...
export function planAnchorRoute(dossier = {}) {
  ...
  // Two of {year, product, subject} must be directly read before the catalog is
  // consulted -- and `product` is one of the three. That is circular: the
  // product is what the lookup exists to establish, so a card whose product is
  // not already known cannot use the lookup that would tell us. It is why
  // pre_l2_anchor_lookup_attempted is false on all sixty benchmark cards, with
  // reason anchor_missing_sufficient_direct_context, while the catalog holds
  // 8,957 sets for the very product those cards came from.
  //
  // A card number plus a directly-read player name is enough on its own.
  // Measured over the 2.26M harvested cards: keyed on (player, card_number) the
  // median key resolves to ONE distinct product line and the 90th percentile to
  // three. That is not a guess about the card, it is a shortlist -- which turns
  // the provider's job from open generation into selection, and is exactly what
  // an unseen product needs, since the shortlist comes from the manufacturer's
  // checklist rather than from how often the model has seen the product.
  const subjectAnchoredLookup = Boolean(sportsAnchor)
    && anchorIsDirectEnough(sportsAnchor, 0.82)
    && (dossier.context || {}).subject_direct === true;
  if (sportsAnchor
    && anchorIsDirectEnough(sportsAnchor, 0.82)
    && ((contextDimensions >= 2 && directContextDimensions >= 2) || subjectAnchoredLookup)) {
    return {
      route: anchorRoutes.SPORTS_COMPOSITE_LOOKUP,
      reason: directContextDimensions >= 2
        ? "direct_card_code_with_composite_context"
        : "direct_card_code_with_direct_subject",
