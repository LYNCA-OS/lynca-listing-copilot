...
function recoverTitleFromResolvedFields({
  ...
  if (!resolvedFields || typeof resolvedFields !== "object" || Array.isArray(resolvedFields)) {
    return { result, finalTitle: existingTitle, recovered: false, reconciled: false, reconciliationReasons: [] };
  }
  ...
  if (existingTitle) {
    const reconciliationReasons = authoritativeTitleRequirements(result, resolvedFields)
      .filter((requirement) => (
    ...
    return {
      result: {
        ...
        title_reconciled_from_v4_field_graph: true,
        title_reconciliation_reasons: reconciliationReasons,
        title_reconciliation_source: "authoritative_evidence_csm_render"
      },
