...
function providerUsageSummary(result = {}) {
  const providerUsage = plainObject(result.provider_usage);
  const aggregateUsage = plainObject(result.usage);
  const tokenDiagnostics = plainObject(result.provider_token_diagnostics);
  if (!providerUsage && !aggregateUsage && !tokenDiagnostics) return null;
  const source = {
    ...(providerUsage || {}),
    ...(aggregateUsage || {})
  };

  const providerCallValue = firstUsageNumber(source, ["provider_calls", "provider_call_count"]);
  const providerCalls = providerCallValue === null ? null : Math.trunc(providerCallValue);
  const inputTokens = firstUsageNumber(source, ["input_tokens", "prompt_tokens", "total_input_tokens"])
    ?? firstUsageNumber(tokenDiagnostics, ["input_tokens", "prompt_tokens", "total_input_tokens"]);
  const outputTokens = firstUsageNumber(source, ["output_tokens", "completion_tokens", "total_output_tokens"])
    ?? firstUsageNumber(tokenDiagnostics, ["output_tokens", "completion_tokens", "total_output_tokens"]);
  const totalTokens = firstUsageNumber(source, ["total_tokens", "total_token_count"])
    ?? firstUsageNumber(tokenDiagnostics, ["total_tokens", "total_token_count"])
    ?? (inputTokens !== null || outputTokens !== null
      ? (inputTokens || 0) + (outputTokens || 0)
      : null);
  const rawCost = firstUsageNumber(source, ["estimated_cost_usd", "cost_usd"]);
  const explicitCostConfigured = typeof source.cost_configured === "boolean"
    ? source.cost_configured
    : null;

  // Historical provider normalization emitted numeric zero even when no price
  // table was configured. Keep those calls and tokens, but do not turn an
  // unknown price into a measured $0. A configured/observed zero remains valid.
  const estimatedCostUsd = explicitCostConfigured === true && rawCost !== null
    ? rawCost
    : explicitCostConfigured === null && rawCost !== null && rawCost > 0
      ? rawCost
      : null;
  const costConfigured = explicitCostConfigured
    ?? (estimatedCostUsd !== null ? true : null);
  ...
  return {
    ...
    cost_configured: costConfigured
  };
}

function serialNumeratorVerification(result = {}) {
  const hasCanonical = Object.prototype.hasOwnProperty.call(result, "serial_numerator_verified");
  const hasLegacyAlias = Object.prototype.hasOwnProperty.call(result, "serialNumeratorVerified");
  const value = hasCanonical
    ? result.serial_numerator_verified
    : hasLegacyAlias
...
const serialFieldStateNames = new Set([
  "serial",
  "serial_number",
  "serial_numerator",
  "serial_denominator",
...
const serialStateValueKeys = new Set([
  "value",
  "normalized_value",
  "normalizedValue",
  "resolved_value",
...
function serialStateValue(fieldName, resolvedFields = {}) {
  if (/numerator/i.test(fieldName)) return null;
  if (/denominator|numbered_to/i.test(fieldName)) return resolvedFields.print_run_denominator || null;
  return resolvedFields.print_run_number || resolvedFields.serial_number || resolvedFields.numerical_rarity || null;
}
...
function guardSerialStateNode(value, fieldName, resolvedFields, key = "") {
  if (/numerator/i.test(key)) return null;
  if (serialStateValueKeys.has(key)) return serialStateValue(fieldName, resolvedFields);
  if (Array.isArray(value)) {
    return value.map((entry) => guardSerialStateNode(entry, fieldName, resolvedFields));
  }
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value).map(([entryKey, entry]) => [
      entryKey,
      guardSerialStateNode(entry, fieldName, resolvedFields, entryKey)
    ]));
  }
  return typeof value === "string" ? redactSerialNumeratorText(value) : value;
}

function guardSerialFieldStates(fieldStates, resolvedFields = {}, result = {}) {
  if (serialNumeratorVerification(result) !== false || !fieldStates) return fieldStates;
  const guardState = (state, fallbackField = "") => {
    ...
    if (!serialFieldStateNames.has(fieldName)) return state;
    return guardSerialStateNode(state, fieldName, resolvedFields);
  };
...
function explicitPresentationResolvedFields(result = {}) {
  const fields = result.presentation_resolved_fields;
...
function withCanonicalResolvedFields(result = {}, resolvedFields = {}) {
  ...
  return {
    ...
    presentation_resolved_fields: resolvedFields,
    rendered_fields: {
      ...renderedFields,
      ...resolvedFields,
      ...(serialNumeratorVerification(result) === false ? { serial_numerator: null } : {}),
      fields: resolvedFields
    },
    field_states: guardSerialFieldStates(result.field_states, resolvedFields, result)
  };
}
...
function recoverTitleFromResolvedFields({
  ...
  if (!resolvedFields || typeof resolvedFields !== "object" || Array.isArray(resolvedFields)) {
    const canonicalResult = withCanonicalResolvedFields(result, {});
  ...
  const presentation = renderListingPresentation({
    ...
    serialNumeratorVerified: serialNumeratorVerification(result)
  });
  const presentationResolvedFields = presentation.presentation_resolved_fields || resolvedFields;
  const canonicalResult = withCanonicalResolvedFields(result, presentationResolvedFields);
  const safeExistingTitle = serialNumeratorVerification(result) === false
    ? redactSerialNumeratorText(existingTitle)
    : existingTitle;
  const canonicalResultWithSafeTitle = safeExistingTitle === existingTitle
    ? canonicalResult
    : {
      ...canonicalResult,
      title: safeExistingTitle,
      final_title: safeExistingTitle,
      rendered_title: safeExistingTitle
    };
  if (!Object.values(presentationResolvedFields).some(hasResolvedValue)) {
    return {
      result: canonicalResultWithSafeTitle,
      resolvedFields: presentationResolvedFields,
      finalTitle: safeExistingTitle,
      recovered: false,
      reconciled: false,
  ...
  const reconciliationReasons = existingTitle
    ? authoritativeTitleRequirements(result, presentationResolvedFields)
      .filter((requirement) => (
...
export function prepareV4PresentationResult({ result = {}, payload = {} } = {}) {
  const resolvedFields = explicitPresentationResolvedFields(result) || buildV4ResolvedFields(result);
  const recovery = recoverTitleFromResolvedFields({ result, payload, resolvedFields });
  return {
    result: recovery.result,
    resolvedFields: recovery.resolvedFields || resolvedFields,
...
export function adaptV2ResultToV4({
  ...
  const providerUsage = providerUsageSummary(adaptedResult);
  ...
  return withV4Version({
    ...
    provider_result: {
      ...
      usage: providerUsage,
      provider_calls: providerUsage?.provider_calls ?? null,
      input_tokens: providerUsage?.input_tokens ?? null,
      output_tokens: providerUsage?.output_tokens ?? null,
      total_tokens: providerUsage?.total_tokens ?? null,
      estimated_cost_usd: providerUsage?.estimated_cost_usd ?? null,
      cost_configured: providerUsage?.cost_configured ?? null,
      token_diagnostics: adaptedResult.provider_token_diagnostics || adaptedResult.provider_usage || providerUsage || null,
      ...
      serial_numerator_verified: serialNumeratorVerification(adaptedResult),
...
export function buildV4PersistenceRows({ sessionId, result = {}, payload = {} } = {}) {
  const prepared = prepareV4PresentationResult({ result, payload });
  return {
    fieldEvidenceRows: buildV4FieldEvidenceRows({ sessionId, result: prepared.result, payload }),
    candidateTrace: buildV4CandidateControlPlaneTrace(prepared.result)
  };
}
