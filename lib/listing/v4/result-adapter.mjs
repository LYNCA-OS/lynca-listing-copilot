...
import { compactRecognitionCriticalPath } from "../pipeline/timing.mjs";

export const V4_TERMINAL_TITLE_MAX_LENGTH = 80;
...
export function v4TerminalTitleMaxLength({ payload = {}, result = {} } = {}) {
  const requested = [
    payload.maxTitleLength,
    payload.max_title_length,
    result.max_title_length
  ].map(positiveIntegerOrNull).filter((value) => value !== null);
  return Math.min(...requested, V4_TERMINAL_TITLE_MAX_LENGTH);
}

export function withV4ServerTitleLimit(payload = {}) {
  const maxTitleLength = v4TerminalTitleMaxLength({ payload });
  return {
    ...payload,
    maxTitleLength,
...
export function assertV4TerminalTitleWithinLimit(result = {}, payload = {}) {
  const maxTitleLength = v4TerminalTitleMaxLength({ payload, result });
  const terminalTitles = ["final_title", "rendered_title", "title"]
    .map((field) => ({ field, title: normalizeTitle(result[field]) }))
    .filter(({ title }) => Boolean(title));
  const violation = terminalTitles.find(({ title }) => title.length > maxTitleLength);
  if (violation) {
    throw Object.assign(new Error(`v4_terminal_title_length_exceeded:${violation.title.length}>${maxTitleLength}`), {
      code: "V4_TERMINAL_TITLE_LENGTH_EXCEEDED",
      retryable: false,
      title_field: violation.field,
      title_length: violation.title.length,
      max_title_length: maxTitleLength
    });
  }
  const title = terminalTitles[0]?.title || "";
  return { title, title_length: title.length, max_title_length: maxTitleLength };
}
...
function recoverTitleFromResolvedFields({
  ...
  const effectiveSerialNumeratorVerified = result.serial_numerator_verified
    ...
    // This PR is production-inert: keep the deployed conservative fallback
    // while persisting the exact effective input for evaluation replay.
    ?? false;
  const effectiveTerminalRendererInputs = {
    max_title_length: v4TerminalTitleMaxLength({ payload, result }),
  ...
  const canonicalResult = {
    ...withCanonicalResolvedFields(result, resolvedFields),
...
export function prepareV4PresentationResult({ result = {}, payload = {} } = {}) {
  // Queue/session adaptation is the last serialization boundary. Normalize
  // the field graph here as well so deterministic catalog taxonomy and
  // canonical aliases cannot be lost after the recognition core completed.
  const resolvedFields = normalizeFields(buildV4ResolvedFields(result));
  const recovery = recoverTitleFromResolvedFields({ result, payload, resolvedFields });
  assertV4TerminalTitleWithinLimit(recovery.result, payload);
  return {
...
function normalizedProviderUsage(result = {}) {
  const source = result.usage && typeof result.usage === "object" && !Array.isArray(result.usage)
    ? result.usage
    : result.provider_usage && typeof result.provider_usage === "object" && !Array.isArray(result.provider_usage)
      ? result.provider_usage
      : null;
  if (!source) return null;

  const providerCalls = nonNegativeNumberOrNull(source.provider_calls ?? source.provider_call_count);
  const inputTokens = nonNegativeNumberOrNull(source.input_tokens ?? source.prompt_tokens ?? source.total_input_tokens);
  const outputTokens = nonNegativeNumberOrNull(source.output_tokens ?? source.completion_tokens ?? source.total_output_tokens);
  const reportedTotal = nonNegativeNumberOrNull(source.total_tokens);
  const totalTokens = reportedTotal ?? (
    inputTokens !== null || outputTokens !== null
      ? (inputTokens || 0) + (outputTokens || 0)
      : null
  );
  const costConfigured = typeof source.cost_configured === "boolean"
    ? source.cost_configured
    : null;
  const reportedCost = nonNegativeNumberOrNull(source.estimated_cost_usd ?? source.cost_usd);
  const estimatedCost = costConfigured === false
    ? null
    : reportedCost;

  return {
    ...source,
    provider_calls: providerCalls === null ? null : Math.trunc(providerCalls),
...
export function adaptRecognitionResultToV4({
  sessionId,
  result = {},
  payload = {},
  routePlan = {},
  ...
  const providerUsage = normalizedProviderUsage(adaptedResult);
  ...
  return withV4Version({
    ...
    v4_pipeline_contract: pipelineContract,
    strategy_replay_trace: adaptedResult.strategy_replay_trace || null,
    ...
    provider_result: {
      ...
      baseline_provider_calls: adaptedResult.baseline_provider_calls ?? null,
      shadow_provider_calls: adaptedResult.shadow_provider_calls ?? null,
      total_provider_calls: adaptedResult.total_provider_calls ?? providerUsage?.provider_calls ?? null,
      provider_accounting_complete: adaptedResult.provider_accounting_complete ?? null,
      provider_usage_scope: adaptedResult.second_look_shadow ? "BASELINE_ONLY" : "COMPLETE",
      second_look_shadow_usage: adaptedResult.second_look_shadow_usage || null,
      second_look_shadow: adaptedResult.second_look_shadow || null,
      provider_slot_timing: adaptedResult.provider_slot_timing || null,
      recognition_critical_path: compactRecognitionCriticalPath(adaptedResult.recognition_critical_path),
      ...
      title_render_source: adaptedResult.title_render_source || null,
      model_title_suggestion: adaptedResult.model_title_suggestion || null,
      ...
      effective_terminal_renderer_inputs: adaptedResult.effective_terminal_renderer_inputs || null,
      ...(adaptedResult.targeted_assist_execution
        ? { targeted_assist_execution: adaptedResult.targeted_assist_execution }
        : {}),
      ...(Array.isArray(adaptedResult.provider_call_ledger) && adaptedResult.provider_call_ledger.length > 0
        ? { provider_call_ledger: adaptedResult.provider_call_ledger }
        : {}),
