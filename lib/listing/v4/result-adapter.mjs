...
function structuredIdentityReady(fields = {}) {
  // TCG observations often put the collectible identity in card_name while
  // sports cards use players/subject. Both are valid CSM identity anchors.
  const subjectReady = [fields.players, fields.subject, fields.character, fields.card_name].some(hasResolvedValue);
  const productReady = [fields.product, fields.set, fields.brand, fields.manufacturer, fields.ip].some(hasResolvedValue);
  return subjectReady && productReady;
}
...
function recoverTitleFromResolvedFields({
  ...
  // A model-written title is diagnostic text, not the V4 serialization layer.
  // Keep it only when the structured observation is too sparse to render a
  // useful identity; otherwise one deterministic CSM renderer owns ordering,
  // de-duplication and the 80-character policy for every provider.
  if (existingTitle && !structuredIdentityReady(resolvedFields)) {
    return {
      result: {
        ...canonicalResult,
        model_title_suggestion: existingTitle,
        title_render_source: "provider_model_fallback_structured_identity_incomplete",
