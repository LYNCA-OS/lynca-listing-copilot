...
function structuredIdentityReady(fields = {}) {
  const subject = fields.players || fields.subject || fields.character;
  const product = fields.product || fields.set || fields.brand || fields.manufacturer || fields.ip;
  return hasResolvedValue(subject) && hasResolvedValue(product);
}
