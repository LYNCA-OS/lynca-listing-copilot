...
export async function resolveV4WorkerSessionIdentity({
  ...
  if (!normalizedSessionId || !normalizedTenantId || !normalizedOperatorId) {
...
export function scopeV4RecognitionPayloadFromFencedJob(job = {}) {
  ...
  for (const key of [
    ...
    // preingestion_bundles has no tenant ownership column today. Until the
    // schema can bind bundle -> tenant + asset fingerprint, a browser-selected
    // bundle id must never cross the service-role worker boundary.
  ...
  return {
    ...
    tenant_id: tenantId,
    operator_id: operatorId,
    created_by_user_id: cleanText(job.created_by_user_id || operatorId),
    assigned_to_user_id: cleanText(job.assigned_to_user_id || job.created_by_user_id || operatorId),
