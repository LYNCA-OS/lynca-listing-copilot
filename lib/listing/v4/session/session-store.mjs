...
export async function persistV4PreingestionBundle({
  ...
  if (!normalizedBundleId) return { saved: false, error: "missing_bundle_id" };
  if (!normalizedTenantId) return { saved: false, error: "missing_tenant_id" };
  return writeV4Row({
...
export async function persistV4FastScoutCache({
  ...
  const tenantId = String(cache.tenant_id || cache.tenantId || "").trim();
...
export async function readV4FastScoutCache({
  ...
  const normalizedTenantId = String(tenantId || "").trim();
  if (!normalizedTenantId) return { ok: false, row: null, error: "missing_tenant_id" };
  const result = await readV4Rows({
    table: "v4_fast_scout_cache",
    select: "*",
    search: { id: `eq.${cacheId}`, tenant_id: `eq.${normalizedTenantId}`, limit: "1" },
    env,
    fetchImpl
  });
