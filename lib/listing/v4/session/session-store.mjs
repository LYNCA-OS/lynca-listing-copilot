...
export function buildV4RecognitionSessionRow({
  ...
  const trustedTenantId = tenantId || payload.tenant_id || payload.tenantId || null;
  const creatorId = createdByUserId || operatorId || null;
  return compact({
    id: sessionId,
    ...
    assigned_to_user_id: assignedToUserId || payload.assigned_to_user_id || payload.assignedToUserId || creatorId,
...
export async function persistV4PreingestionBundle({
  ...
  if (!tenantId) return { saved: false, error: "missing_tenant_id" };
  return writeV4Row({
...
export async function persistV4FastScoutCache({
  ...
  if (!cache.tenant_id) return { saved: false, error: "missing_tenant_id" };
  const now = nowIso();
  const row = compact({
    id: cache.id,
...
export async function readV4FastScoutCache({
  ...
  if (!tenantId) return { ok: false, row: null, error: "missing_tenant_id" };
  const result = await readV4Rows({
    table: "v4_fast_scout_cache",
    select: "*",
    search: { id: `eq.${cacheId}`, tenant_id: `eq.${tenantId}`, limit: "1" },
    env,
    fetchImpl
  });
...
export async function checkV4Tables({
  ...
  for (const table of tableNames) {
    const select = table === "v4_production_quality_ledger"
      ? "id,provider_diagnostics,pipeline_node_ledger"
      : "id";
