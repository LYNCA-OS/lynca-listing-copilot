...
export async function verifyV4AdminTestFeedbackIsolation({
  sessionId,
  tenantId,
  feedbackEventId,
  env = process.env,
  ...
  if (!sessionId || !tenantId || !feedbackEventId) {
    return { verified: false, proof: null, error: "missing_admin_test_feedback_proof_identity" };
  }
  const result = await callV4Rpc({
    fn: "verify_v4_admin_test_feedback_isolation",
    payload: {
      p_session_id: String(sessionId),
      p_tenant_id: String(tenantId),
      p_feedback_event_id: String(feedbackEventId)
    },
    env,
    fetchImpl
  });
  const proof = result.rows?.[0] || null;
  return {
    verified: result.ok && proof?.verified === true,
    proof,
    error: result.error || (proof?.verified === true ? null : "admin_test_feedback_isolation_not_proven")
  };
}
...
export async function checkV4Tables({
  ...
  const tableSpecs = [
    ...
    ["listing_identity_resolution_cache", "cache_key,recognition_pipeline_fingerprint"],
    ["listing_active_catalog_snapshot", "singleton,content_revision"],
    ["listing_writer_final_replay", "tenant_id,image_generation_hash,replay_status"],
