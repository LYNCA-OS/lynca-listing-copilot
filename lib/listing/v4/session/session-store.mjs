...
export async function persistV4PreingestionBundle({
  ...
  if (reuseExisting) {
    const existing = await readV4Rows({
      ...
      select: "id,tenant_id,asset_id,status",
      search: {
        id: `eq.${bundleId}`,
        tenant_id: `eq.${safeTenantId}`,
        asset_id: safeAssetId ? `eq.${safeAssetId}` : undefined,
        limit: "1"
      },
      env,
      fetchImpl
    });
    if (existing.ok && existing.rows.length === 1) {
      return {
        saved: true,
        row: existing.rows[0],
        rows: existing.rows,
