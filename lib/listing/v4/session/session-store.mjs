...
export async function createV4RecognitionSession({
  ...
  const row = buildV4RecognitionSessionRow({ sessionId, payload, routePlan, operatorId });
...
export async function persistV4WriterFeedbackTransaction({
  ...
  if (!sessionId || !operatorId || !status) {
  ...
  const result = await callV4Rpc({
    ...
    payload: {
      ...
      p_feedback_event: buildV4FeedbackEventRow(feedbackEvent),
      p_learning_event: buildV4LearningEventRow(learningEvent)
    },
    env,
    fetchImpl
  });
