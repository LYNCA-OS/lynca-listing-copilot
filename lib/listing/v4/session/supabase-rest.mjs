...
function headers(key, prefer = "return=representation") {
  ...
  // Legacy service-role keys are JWTs and still use Bearer authentication.
  // New sb_secret_* keys are opaque API keys and must be sent only as apikey.
  if (String(key).split(".").length === 3) result.authorization = `Bearer ${key}`;
  return result;
}
