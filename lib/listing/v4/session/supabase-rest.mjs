...
async function fetchReadWithRetry(fetchImpl, url, init = {}, {
  timeoutMs = 8000,
  attempts = 2,
  baseDelayMs = 120
} = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    if (init.signal?.aborted) throw init.signal.reason || new Error("v4_supabase_read_aborted");
    try {
  ...
  throw lastError || new Error("v4_supabase_read_failed");
}
...
export async function readV4Rows({
  ...
  try {
    ...
    const response = await fetchReadWithRetry(fetchImpl, endpoint, {
      headers: headers(key, count ? `count=${count}` : undefined)
    }, {
      timeoutMs: boundedTimeoutMs(env.V4_SUPABASE_READ_TIMEOUT_MS, 8000),
      attempts: env.V4_SUPABASE_READ_ATTEMPTS || 2
    });
    if (!response.ok) {
      const message = await response.text();
      throw new Error(`${response.status} ${message.slice(0, 180)}`);
