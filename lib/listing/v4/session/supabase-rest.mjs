  ...
  try {
    ...
    const response = await fetchWithTimeout(fetchImpl, endpoint, {
      ...
      headers: headers(key, upsert
        ? `resolution=merge-duplicates,return=${returnRepresentation ? "representation" : "minimal"}`
...
export async function writeV4Rows({
  ...
  const result = await writeV4Row({ table, row: safeRows, upsert, onConflict, returnRepresentation, env, fetchImpl });
  return {
    saved: result.saved,
