...
function normalizeCompany(value) {
  ...
  if (/\bCSG\b/.test(text)) return "CGC";
  return gradeCompanies.find((company) => text.includes(company)) || null;
}
...
function declaredGradeTypeForInput(resolved = {}, legacyFields = {}) {
  if (Object.hasOwn(legacyFields, "grade_type")) {
    return normalizeGradeType(legacyFields.grade_type);
  }

  const hasLegacyGradeValue = ["grade", "card_grade", "auto_grade"]
    .some((fieldName) => Object.hasOwn(legacyFields, fieldName));
  return hasLegacyGradeValue ? "UNKNOWN" : normalizeGradeType(resolved.grade_type);
}
...
export function resolveGradeFields({
  ...
  const declaredGradeType = declaredGradeTypeForInput(resolved, legacyFields);
  const explicitLegacyCardGrade = normalizeGradeToken(legacyFields.card_grade || legacyFields.grade);
  const explicitLegacyAutoGrade = normalizeAutoGradeToken(legacyFields.auto_grade);
  const inferredDualGradeType = declaredGradeType === "UNKNOWN"
    && Boolean(explicitLegacyCardGrade)
    && Boolean(explicitLegacyAutoGrade);
  const suppliedCardGrade = normalizeGradeToken(
    legacyFields.card_grade || legacyFields.grade || resolved.card_grade || resolved.grade
  );
  const labeledAutoGrade = parseAutoGradeFromText(legacyFields.auto_grade || resolved.auto_grade || "");
  const suppliedAutoGradeAllowed = declaredGradeType === "CARD_AND_AUTO"
    || (declaredGradeType === "AUTO_ONLY" && !suppliedCardGrade)
    || Boolean(labeledAutoGrade)
    || inferredDualGradeType;
  const rejectedSuppliedAutoGrade = suppliedAutoGradeAllowed
    ? null
    : normalizeAutoGradeToken(legacyFields.auto_grade || resolved.auto_grade);
  const next = sanitizeGradeFields(suppliedAutoGradeAllowed
    ? resolved
    : { ...resolved, auto_grade: null });
  const notes = [];
  if (inferredDualGradeType) {
    notes.push({
      field: "grade_type",
      action: "infer_card_and_auto_grade_type",
      to: "CARD_AND_AUTO"
    });
  }
  const parsed = [
  ...
  if (!next.auto_grade && suppliedAutoGradeAllowed && legacyFields.auto_grade) {
  ...
  if (!next.auto_grade && rejectedSuppliedAutoGrade) {
    notes.push({
      field: "auto_grade",
      action: "discard_unsupported_auto_grade",
      from: rejectedSuppliedAutoGrade,
      to: null
    });
  }

  next.grade_type = gradeTypeForValues(next.card_grade, next.auto_grade);
  const atomic = gradeAtomicCompleteness(next);
  const atomicResolved = enforceAtomicGradeFields(next);
  if (atomic.incomplete_score_without_company) {
    notes.push({
      field: "grade",
      action: "discard_incomplete_grade_without_company",
      from: {
