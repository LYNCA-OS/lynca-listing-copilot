...
export function classifyNumberToken(value) {
  ...
  if (normalizeSlashNumber(text).match(/^\/\d{1,4}$/)) {
...
export function resolveNumberFields({
  resolved = {},
  legacyFields = {},
  allowLegacyCardNumberAsPrintRun = false
} = {}) {
  const next = { ...resolved };
  const notes = [];
  ...
  if (legacyFields.card_number) {
    ...
    if (split.serial_number && allowLegacyCardNumberAsPrintRun !== true) {
      split.collector_number = cleanNumberText(legacyFields.card_number) || null;
    ...
    for (const [field, value] of Object.entries(split)) {
      if (value && !next[field]) {
        ...
        notes.push({
          field,
          action: field === "collector_number" && classifyNumberToken(legacyFields.card_number) === "serial_number"
            ? "preserve_ambiguous_slash_card_number"
            : "split_legacy_card_number",
