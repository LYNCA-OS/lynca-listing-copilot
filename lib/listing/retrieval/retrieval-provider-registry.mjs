...
import { catalogProvider } from "./catalog-provider.mjs";
...
import { postgresHybridProvider } from "./postgres-hybrid-provider.mjs";
...
export function createRetrievalProviderRegistry({
  ...
  const providers = {
    ...
    [retrievalProviderIds.CATALOG]: catalogProvider({ env, fetchImpl }),
    [retrievalProviderIds.VISUAL_VECTOR]: visualVectorProvider({ env, fetchImpl }),
    [retrievalProviderIds.POSTGRES_HYBRID]: postgresHybridProvider({ env, fetchImpl }),
