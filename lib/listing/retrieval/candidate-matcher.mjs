...
const retrievalContextStopTokens = new Set([
  "the", "and", "for", "with", "from", "this", "that", "were", "was",
  "his", "her", "their", "card", "cards", "topps", "panini", "upper", "deck"
]);

function addRetrievalContextSignal(result, candidate = {}, resolved = {}) {
  const context = normalizeText(resolved?.retrieval_context_text || "");
  if (!context) return;
  const structuredText = normalizeText(Object.entries(resolved || {})
    .filter(([field]) => field !== "retrieval_context_text")
    .map(([, value]) => Array.isArray(value) ? value.join(" ") : value)
    .filter(Boolean)
    .join(" "));
  const structuredTokens = new Set(structuredText.split(" ").filter(Boolean));
  const informative = [...new Set(context.split(" "))]
    .filter((token) => token && !structuredTokens.has(token))
    .filter((token) => /^\d{1,4}$/.test(token) || (token.length >= 4 && !retrievalContextStopTokens.has(token)));
  if (!informative.length) return;

  const candidateTokens = new Set(normalizeText(candidateText(candidate)).split(" ").filter(Boolean));
  const matches = informative.filter((token) => candidateTokens.has(token));
  if (!matches.length) return;
  const numericMatches = matches.filter((token) => /^\d{1,4}$/.test(token)).length;
  const boost = Math.min(0.08, numericMatches * 0.05 + Math.min(0.03, (matches.length - numericMatches) * 0.01));
  if (boost <= 0) return;
  result.score += boost;
  if (!result.matched_fields.includes("retrieval_context")) result.matched_fields.push("retrieval_context");
}
...
export function scoreRetrievalCandidate(candidate = {}, resolved = {}) {
  ...
  addRetrievalContextSignal(result, candidate, resolved);
