...
function fieldValuesCompatible(field, left, right) {
  ...
  if (leftValue === rightValue) return true;
  if (["product", "set", "subset", "card_type", "insert", "parallel", "variation"].includes(field)) {
...
export function scoreRetrievalCandidate(candidate = {}, resolved = {}) {
  ...
  ["brand", "manufacturer", "product", "set", "card_type", "insert", "parallel"].forEach((field) => {
    if (!fields[field]) return;
    if (textIncludes(text, fields[field]) || textIncludes(candidateFields[field], fields[field])) {
      addMatch(result, field, field === "parallel" ? 0.08 : 0.1);
    }
  });
