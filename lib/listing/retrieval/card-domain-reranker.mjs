...
function hasAuthoritativeCurrentSourceIdentity(candidate = {}) {
  if (candidate.reviewed_current_source_identity_match === true) return true;
  const overrides = candidate.anchor_agreement?.authoritative_overrides;
  return Array.isArray(overrides) && overrides.includes("reviewed_current_source_identity_match");
}
...
export function rankCardDomainCandidates(candidates = [], observedFields = {}, {
  ...
  const ranked = candidates.map((candidate) => {
    ...
    const authoritativeIdentityMatch = hasAuthoritativeCurrentSourceIdentity(candidate);
    const authoritativeObservationCorrection = authoritativeIdentityMatch && directConflicts.length === 0;
    ...
    const score = Math.max(0, Math.min(1,
      ...
      + agreement * 0.12
      + visual * 0.08
      + (authoritativeIdentityMatch ? 0.2 : 0)
      - marketplacePenalty
      - conflictPenalty
    ));
    return {
      ...
      authoritative_identity_match: authoritativeIdentityMatch,
      authoritative_observation_correction: authoritativeObservationCorrection,
      conflicting_fields: conflicts,
      eligible_for_domain_selection: candidate.__decision_eligible === true
        && (conflicts.length === 0 || authoritativeObservationCorrection)
        && (score >= 0.3 || authoritativeObservationCorrection)
        && (
          queryCoverage >= 0.3
          || exactAnchor >= 0.5
          || (agreement >= 0.8 && trust >= 5 / 6)
          || authoritativeIdentityMatch
        )
    };
  }).sort((left, right) => (
    Number(right.authoritative_identity_match) - Number(left.authoritative_identity_match)
    || right.score - left.score
    || right.source_trust_score - left.source_trust_score
  ));
