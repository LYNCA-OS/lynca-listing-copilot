...
function sanitizedCandidateFields(fields = {}) {
  ...
  const output = {
    ...
    expected_serial_denominator: normalized.expected_serial_denominator || serialDenominator(normalized.serial_number)
  };
...
function sourceTrust(candidate = {}) {
  ...
  if (neverApprovedPromptSources.has(sourceType) || candidateLooksMarketplaceWeak(candidate)) {
...
function candidateLooksMarketplaceWeak(candidate = {}) {
  const sourceType = candidateSourceType(candidate);
  if (sourceType === "MARKETPLACE_REFERENCE") return true;
  const text = [
    candidate.provider_id,
...
function candidateHasAnyPromptConflict(candidate = {}) {
  return candidateHasDirectConflict(candidate)
    || candidateSoftConflictFields(candidate).length > 0
...
function openSetPromptBlockReason(retrieval = {}, promptCandidates = []) {
  ...
  const topCandidate = promptCandidates[0] || null;
  if (decision === "LOW_MARGIN_MATCH" && candidateHasHardConstraintAnchor(topCandidate)) {
    return "";
  }
...
function productHierarchyCompatible(left, right) {
  const leftTokens = productSignificantTokens(left);
  const rightTokens = productSignificantTokens(right);
  return productCompatible(left, right) || tokenSubsetCompatible(leftTokens, rightTokens);
}
...
function candidateOriginalSerial(candidate = {}) {
  return cleanText(candidate.fields?.serial_number || candidate.serial_number || candidate.reference_metadata?.serial_number);
}
...
function candidateHasExactAnchor(candidate = {}, queryFields = {}, candidateFields = {}) {
  ...
  const candidateDenominator = cleanText(candidateFields.expected_serial_denominator || serialDenominator(candidateOriginalSerial(candidate)));
  return support.has("collector_number")
    || support.has("card_number")
    || support.has("checklist_code")
    || support.has("serial_denominator")
    || support.has("serial_number")
    || Boolean(denominator && candidateDenominator && denominator === candidateDenominator)
    || Boolean(queryFields.collector_number && candidateFields.collector_number && valueCompatible(queryFields.collector_number, candidateFields.collector_number))
    || Boolean(queryFields.checklist_code && candidateFields.checklist_code && valueCompatible(queryFields.checklist_code, candidateFields.checklist_code));
}
...
function yearConflictIsSoftCatalogAnchor(candidate = {}, queryFields = {}, candidateFields = {}) {
  ...
  if (!yearsCompatible(queryFields.year, candidateFields.year)) return false;
  return sourceTrust(candidate) === "APPROVED_REFERENCE"
...
function candidateAnchorAgreement(candidate = {}, queryFields = {}, candidateFields = {}) {
  ...
  if (hasValue(queryFields.year) && hasValue(candidateFields.year)) {
    (yearsCompatible(queryFields.year, candidateFields.year) ? agreed : contradicted).add("year");
  }
  ...
  if (queryHierarchy && candidateHierarchy) {
    (productHierarchyCompatible(queryHierarchy, candidateHierarchy) ? agreed : contradicted).add("product_hierarchy");
  }

  let exactCodeMatch = false;
  for (const field of ["collector_number", "checklist_code"]) {
    if (!hasValue(queryFields[field]) || !hasValue(candidateFields[field])) continue;
    if (valueCompatible(queryFields[field], candidateFields[field])) {
      agreed.add(field);
      exactCodeMatch = true;
    } else {
...
function buildFieldSupportRows(candidates = [], queryFields = {}, { limit = 24 } = {}) {
  ...
  for (const candidate of candidates) {
    if (sourceTrust(candidate) !== "APPROVED_REFERENCE") continue;
    if (candidateLooksMarketplaceWeak(candidate)) continue;
    if (!isPromptAssistCandidate(candidate)) continue;
    const fields = sanitizedCandidateFields(candidate.fields || {});
    ...
    if (hardConflicts.size || softConflicts.length || candidateAnchorContradictions(candidate).length) continue;
    for (const field of fieldSupportFields) {
      if (fieldSupportConflictBlocked(field, hardConflicts)) continue;
      if (!candidateFieldCanSupport(candidate, field, queryFields, fields)) continue;
...
export function buildVectorCandidatePacket(retrieval = {}, {
  ...
  const fieldSupportSourceCandidates = openSetPromptBlockReason(retrieval, promptSafeCandidates)
    ? []
    : promptSafeCandidates;
...
export function vectorCandidatePacketAssistEligibility(packet = {}) {
  ...
  const promptCandidates = uniquePromptCandidates(approved.filter(isPromptAssistCandidate));
