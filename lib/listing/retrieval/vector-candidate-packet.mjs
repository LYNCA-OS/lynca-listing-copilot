...
function buildHybridCandidateRows(retrieval = {}, { limit = 5, queryFields = {} } = {}) {
  ...
  return sources.slice(0, Math.max(1, Number(limit) || 5)).map((candidate, index) => {
    ...
    const anchorAgreement = candidateAnchorAgreement(candidate, queryFields, fields);
    const printRunCopyViolation = referencePrintRunCopyViolation(candidate);
    const referenceMetadata = candidate.reference_metadata && typeof candidate.reference_metadata === "object"
      ? candidate.reference_metadata
      : {};
    return {
    rank: index + 1,
    ...
    provider_id: candidate.provider_id || candidate.retrieval_provider_id || null,
    source_type: candidate.source_type || referenceMetadata.source_type || null,
    source_url: candidate.source_url || referenceMetadata.source_url || null,
