...
function candidateIsProviderCatalogLike(candidate = {}) {
  const provider = cleanText(candidate.provider_id || candidate.retrieval_provider_id || candidate.source_provider).toLowerCase();
  return catalogProviderIds.has(provider) || catalogSourceTypes.has(candidateSourceType(candidate));
}
...
function candidateHasProviderPromptIdentityAnchor(candidate = {}) {
  if (!candidateIsProviderCatalogLike(candidate)) return candidateHasPromptIdentityAnchor(candidate);
  ...
  return agreement.exact_code_match === true || authoritativeCurrentSource;
}
