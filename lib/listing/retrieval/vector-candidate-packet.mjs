...
function uniquePromptCandidates(candidates = []) {
  ...
  return candidates.filter((candidate, index) => {
    const key = promptCandidateId(candidate) || `prompt_candidate_${index + 1}`;
...
function candidateHasPromptIdentityAnchor(candidate = {}) {
  ...
  return hasSubject
    || hasPrintedCode
    || (hasProduct && hasSerialDenominator)
    || (hasProduct && hasObservableComponent);
}
...
function serialConflictIsOnlyReferenceDenominator(candidate = {}, queryFields = {}, candidateFields = {}) {
  ...
  const originalSerial = candidateOriginalSerial(candidate);
  return !/^\s*\d{1,4}\s*\/\s*\d{1,4}\s*$/.test(originalSerial);
}
...
const identityDependentFieldSupport = new Set([
  "product",
  "set",
  "subset",
  "insert",
...
function candidateFieldCanSupport(candidate = {}, field = "", queryFields = {}, candidateFields = {}) {
  ...
  if (field === "product" || field === "set" || field === "subset" || field === "insert") {
...
function fieldSupportConflictBlocked(field = "", hardConflicts = new Set()) {
  ...
  if (identityDependentFieldSupport.has(field) && (
    hardConflicts.has("players")
    || hardConflicts.has("collector_number")
    || hardConflicts.has("checklist_code")
  )) return true;
  return false;
}
...
export function buildVectorCandidatePacket(retrieval = {}, {
  ...
  const fieldSupport = buildFieldSupportRows(packetCandidates, queryCandidateFields);
