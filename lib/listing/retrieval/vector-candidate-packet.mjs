...
function subjectList(fields = {}) {
  // Candidate packets store the canonical subject list under `subjects`,
  // while recognition and catalog rows use `players`. Rebinding a packet to
  // the final observation normalizes the fields a second time; without this
  // alias bridge that second pass silently erased every subject anchor.
  ...
  const normalized = normalizeResolvedFields(explicitSubjects.length
    ? { ...fields, players: explicitSubjects }
    : fields);
...
function compoundBrandProduct(fields = {}, normalized = {}) {
  if (hasValue(normalized.product)) return normalized.product;
  if (hasValue(normalized.set)) return normalized.set;

  const brand = cleanText(fields.brand);
  const manufacturer = cleanText(fields.manufacturer);
  if (!brand || !manufacturer) return null;

  const brandTokens = compareTokens(brand);
  const manufacturerTokens = compareTokens(manufacturer);
  if (brandTokens.length <= manufacturerTokens.length) return null;
  if (!manufacturerTokens.every((token) => brandTokens.includes(token))) return null;

  // Some provider observations put the full product family in `brand` while
  // preserving the actual manufacturer separately (Topps + Topps Heritage).
  // Treat only that strict superset as product hierarchy; a plain brand name
  // must never manufacture a product anchor by itself.
  return brand;
}

function sanitizedCandidateFields(fields = {}) {
  const numeratorSafeFields = stripReferencePrintRunNumerator(fields || {});
  const normalized = normalizeResolvedFields(numeratorSafeFields);
  const output = {
    year: normalized.year,
    manufacturer: normalized.manufacturer || normalized.brand,
    brand: normalized.brand || normalized.manufacturer,
    product: compoundBrandProduct(numeratorSafeFields, normalized),
    set: normalized.set,
    ...
    subjects: subjectList(numeratorSafeFields),
...
function candidateIsProviderRetrievedReference(candidate = {}) {
  const provider = cleanText(candidate.provider_id || candidate.retrieval_provider_id || candidate.source_provider).toLowerCase();
  const sourceType = candidateSourceType(candidate);
  return catalogProviderIds.has(provider)
    || catalogSourceTypes.has(sourceType)
    || provider === "visual_vector"
    || sourceType === "VISUAL_VECTOR";
}

export function candidateHasPromptIdentityAnchor(candidate = {}) {
  ...
  // Catalog rows fail closed until their anchors have been compared with the
  // current image. Merely carrying plausible identity fields is not proof that
  // a visually similar reference is this card.
  return agreement.prompt_hard_filter_applicable === true
    && agreement.prompt_hard_filter_pass === true;
}

function candidateHasProviderPromptIdentityAnchor(candidate = {}) {
  if (!candidateIsProviderRetrievedReference(candidate)) return candidateHasPromptIdentityAnchor(candidate);
  if (!candidateHasPromptIdentityAnchor(candidate)) return false;
  const agreement = candidate.anchor_agreement || {};
  const authoritativeCurrentSource = Array.isArray(agreement.authoritative_overrides)
    && agreement.authoritative_overrides.includes("reviewed_current_source_identity_match");
  return agreement.exact_code_match === true
    || authoritativeCurrentSource
    || candidate.hard_constraint_eligible === true;
}
...
function isPromptAssistCandidate(candidate = {}) {
  return isAssistApprovedCandidate(candidate)
    ...
    && candidateHasProviderPromptIdentityAnchor(candidate);
}
...
const productTokenAliases = new Map([
  ["hoopers", "hoops"],
  ["rowman", "bowman"]
]);
...
function productSignificantTokens(value) {
  return compareTokens(value)
    ...
    .map((token) => productTokenAliases.get(token) || token);
}
...
function productFamilyTokens(value) {
  return productSignificantTokens(value)
    .filter((token) => productFamilyOnlyTokens.has(token));
}

function productFamilyKey(token) {
  return ["topps", "bowman"].includes(token) ? "topps_bowman" : token;
}

function productFamiliesOverlap(leftFamilies, rightFamilies) {
  const leftKeys = new Set([...leftFamilies].map(productFamilyKey));
  const rightKeys = new Set([...rightFamilies].map(productFamilyKey));
  return [...leftKeys].some((token) => rightKeys.has(token));
}

function productFamiliesExactOverlap(leftFamilies, rightFamilies) {
  return [...leftFamilies].some((token) => rightFamilies.has(token));
}

const publisherProductFamilyTokens = new Set([
  "bandai",
  "bowman",
  "donruss",
  "fleer",
...
function productManufacturerCoherent(fields = {}) {
  // Compare publisher families only. IP tokens such as `one piece` describe
  // the licensed game, not its manufacturer, so Bandai + One Piece is valid.
  const manufacturerFamilies = new Set(productFamilyTokens(fields.manufacturer || fields.brand)
    .filter((token) => publisherProductFamilyTokens.has(token)));
  const productFamilies = new Set(productFamilyTokens(fields.product)
    .filter((token) => publisherProductFamilyTokens.has(token)));
  return !manufacturerFamilies.size
    || !productFamilies.size
    || productFamiliesOverlap(manufacturerFamilies, productFamilies);
}
...
function productHierarchyCompatible(left, right) {
  const leftTokens = productSignificantTokens(left);
  const rightTokens = productSignificantTokens(right);
  const leftFamilies = new Set(productFamilyTokens(left));
  const rightFamilies = new Set(productFamilyTokens(right));
  if (leftFamilies.size && rightFamilies.size
    && !productFamiliesExactOverlap(leftFamilies, rightFamilies)) {
    return false;
  }
...
function hierarchyComponentWithoutCommentary(value) {
...
function meaningfulSetComponent(fields = {}) {
  const setValue = hierarchyComponentWithoutCommentary(fields.set || fields.insert || fields.subset);
  if (!setValue) return "";
  const productValue = hierarchyComponentWithoutCommentary(fields.product);
  if (!productValue) return setValue;
  const productTokens = new Set(productSignificantTokens(productValue));
  const setOnlyTokens = productSignificantTokens(setValue).filter((token) => !productTokens.has(token));
  return setOnlyTokens.length ? setValue : "";
}

function productHierarchyComponentsCompatible(leftFields = {}, rightFields = {}) {
  const rawLeftProduct = hierarchyComponentWithoutCommentary(leftFields.product);
  const rawRightProduct = hierarchyComponentWithoutCommentary(rightFields.product);
  const leftSet = meaningfulSetComponent(leftFields);
  const rightSet = meaningfulSetComponent(rightFields);
  // A provider occasionally emits only the manufacturer in `product` and the
  // actual observed product in `set` (Topps + Rowman Chrome). Compare the most
  // specific observed component; a family-only placeholder must not veto it.
  const leftProduct = productSpecificTokens(rawLeftProduct).length ? rawLeftProduct : leftSet || rawLeftProduct;
  const rightProduct = productSpecificTokens(rawRightProduct).length ? rawRightProduct : rightSet || rawRightProduct;
  if (leftProduct && rightProduct && !productHierarchyCompatible(leftProduct, rightProduct)) return false;

  if (leftSet && rightSet && !productHierarchyCompatible(leftSet, rightSet)) return false;
  if ((leftProduct && rightProduct) || (leftSet && rightSet)) return true;

  const leftHierarchy = [leftProduct, leftSet].filter(Boolean).join(" ");
  const rightHierarchy = [rightProduct, rightSet].filter(Boolean).join(" ");
  return Boolean(leftHierarchy && rightHierarchy)
    && productHierarchyCompatible(leftHierarchy, rightHierarchy);
}
...
function queryCandidateConflictFields(queryFields = {}, candidateFields = {}) {
  ...
  if (!productManufacturerCoherent(candidateFields)) {
...
function productConflictIsOnlyHierarchySplit(queryFields = {}, candidateFields = {}) {
  return productHierarchyComponentsCompatible(queryFields, candidateFields);
}
...
function candidateHasIdentityAnchor(candidate = {}, queryFields = {}, candidateFields = {}) {
  ...
  const hierarchySupport = productHierarchyComponentsCompatible(queryFields, candidateFields);
...
function conflictSoftenedByIdentityGranularity(field, candidate = {}, queryFields = {}, candidateFields = {}) {
  ...
  if (normalizedField === "product" || normalizedField === "set") {
    if (!productManufacturerCoherent(candidateFields)) return false;
...
// anchors and either an exact printed-code agreement, or product hierarchy
// plus another independent identity anchor. Year + subject alone is not
// enough: the same player commonly appears in many products in the same year.
function candidateAnchorAgreement(candidate = {}, queryFields = {}, candidateFields = {}) {
  ...
  const queryHasProductHierarchy = hasValue(queryFields.product)
    || hasValue(queryFields.set)
    || hasValue(queryFields.insert);
  const candidateHasProductHierarchy = hasValue(candidateFields.product)
    || hasValue(candidateFields.set)
    || hasValue(candidateFields.insert);
  if (queryHasProductHierarchy && candidateHasProductHierarchy) {
    (productManufacturerCoherent(candidateFields)
      && productHierarchyComponentsCompatible(queryFields, candidateFields)
      ? agreed
      : contradicted).add("product_hierarchy");
  }
  ...
  const queryAnchorDimensions = [
    ...
    queryHasProductHierarchy,
    hasValue(queryFields.collector_number) || hasValue(queryFields.checklist_code),
    Boolean(queryDenominator)
  ].filter(Boolean).length;
  const productAnchored = agreed.has("product_hierarchy");
  const productCompanionAnchored = [
    "subjects",
    "year",
    "serial_denominator",
    "collector_number",
    "checklist_code",
    "card_number"
  ].some((field) => agreed.has(field));
  // Once the current images expose a subject, a catalog row without a
  // subject cannot become an identity candidate from year + product alone.
  // The only safe exception is a natural-key lookup: an exact printed code
  // together with a compatible product hierarchy. This keeps sparse official
  // checklist rows usable without allowing same-product neighbours to anchor
  // the vision model to a different person.
  const subjectCoverageRequired = querySubjects.length > 0;
  const subjectCoveragePass = !subjectCoverageRequired
    || agreed.has("subjects")
    || (exactCodeMatch && productAnchored);
  ...
  return {
    ...
    subject_coverage_required: subjectCoverageRequired,
    subject_coverage_pass: subjectCoveragePass,
    ...
    prompt_hard_filter_pass: queryAnchorDimensions > 0
      && contradicted.size === 0
      && subjectCoveragePass
      && (exactCodeMatch || (productAnchored && productCompanionAnchored))
  };
}
...
export function rebindCandidateToObservedFields(candidate = {}, observedFields = {}) {
  ...
  const fields = sanitizedCandidateFields(candidate.fields || {});
  let anchorAgreement = candidateAnchorAgreement(candidate, queryFields, fields);
  if (candidate.reviewed_current_source_identity_match === true) {
    const reviewedIdentityFields = new Set(["year", "subjects", "manufacturer", "product_hierarchy"]);
    anchorAgreement = {
      ...anchorAgreement,
      agreed: [...new Set([...(anchorAgreement.agreed || []), ...reviewedIdentityFields])],
      contradicted: (anchorAgreement.contradicted || []).filter((field) => !reviewedIdentityFields.has(field)),
      observed_conflicts: (anchorAgreement.observed_conflicts || []).filter((field) => !reviewedIdentityFields.has(field)),
      authoritative_overrides: [...new Set([
        ...(anchorAgreement.authoritative_overrides || []),
        "reviewed_current_source_identity_match"
      ])],
      subject_coverage_pass: true,
  ...
  return {
    ...
    conflicting_fields: [...new Set(candidateConflictsForPrompt({ ...candidate, anchor_agreement: anchorAgreement }, queryFields, fields)
      .filter((field) => candidate.reviewed_current_source_identity_match !== true
        || !["year", "subjects", "manufacturer", "product", "product_hierarchy"].includes(field)))],
...
function catalogPromptScore(candidate = {}, queryFields = {}) {
  ...
  if (productHierarchyComponentsCompatible(queryFields, fields)) score += 0.06;
...
function buildCatalogCandidateRows(retrieval = {}, { limit = 5, queryFields = {} } = {}) {
  ...
  return sources.filter(isCatalogLikeCandidate)
    ...
    .filter(({ candidate }) => {
      const fields = sanitizedCandidateFields(candidate.fields || {});
      const identityKey = [
        cleanText(candidate.candidate_identity_id || candidate.identity_id),
        cleanCompare(sanitizedReferenceTitle(candidate)),
        JSON.stringify(fields).toLowerCase()
      ].join(":");
      if (!identityKey || seenIdentities.has(identityKey)) return false;
...
export function buildVectorCandidateAssistPacket(packet = {}, { limit = Number.POSITIVE_INFINITY } = {}) {
  const retrieval = packet.vector_retrieval || {};
  const candidates = Array.isArray(retrieval.candidates) ? retrieval.candidates : [];
  ...
  const fieldSupport = Array.isArray(retrieval.field_support) ? retrieval.field_support : [];
  const promptLimit = Number.isFinite(Number(limit))
  ...
  ))).slice(0, promptLimit).map((candidate, index) => ({
    ...candidate,
    rank: index + 1
  }));
  ...
  return {
    vector_retrieval: {
      ...
      prompt_candidate_limit: Number.isFinite(promptLimit) ? promptLimit : null,
