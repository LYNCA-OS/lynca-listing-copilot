...
function candidateConflictFields(candidate = {}) {
  ...
  return [...new Set(explicit.map((field) => cleanText(
    typeof field === "string" ? field : field?.field || field?.name || field?.conflicting_field
  )).filter(Boolean))];
}
...
function buildHybridCandidateRows(retrieval = {}, { limit = 5 } = {}) {
  ...
  return sources.slice(0, Math.max(1, Number(limit) || 5)).map((candidate, index) => ({
    ...
    fields: sanitizedCandidateFields(candidate.fields || {})
  }));
}
...
export function buildVectorCandidatePacket(retrieval = {}, {
  ...
  const hybridRows = buildHybridCandidateRows(retrieval, { limit });
...
export function vectorCandidatePacketAssistEligibility(packet = {}) {
  ...
  const approved = candidates.filter((candidate) => sourceTrust(candidate) === "APPROVED_REFERENCE");
  const eligible = approved.filter((candidate) => !candidateHasDirectConflict(candidate));
...
export function vectorCandidatePacketHasAssistEligibleCandidates(packet = {}) {
  return vectorCandidatePacketAssistEligibility(packet).eligible;
}
