...
function productHierarchyCompatible(left, right) {
  ...
  if (leftFamilies.size && rightFamilies.size
    && ![...leftFamilies].some((token) => rightFamilies.has(token))) {
    return false;
  }
...
function productHierarchyComponentsCompatible(leftFields = {}, rightFields = {}) {
  const leftProduct = hierarchyComponentWithoutCommentary(leftFields.product);
  const rightProduct = hierarchyComponentWithoutCommentary(rightFields.product);
...
function candidateAnchorAgreement(candidate = {}, queryFields = {}, candidateFields = {}) {
  ...
  if (queryHasProductHierarchy && candidateHasProductHierarchy) {
    (productHierarchyComponentsCompatible(queryFields, candidateFields) ? agreed : contradicted).add("product_hierarchy");
  }
