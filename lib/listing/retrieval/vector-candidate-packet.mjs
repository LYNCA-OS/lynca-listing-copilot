...
function candidateHasPromptIdentityAnchor(candidate = {}) {
  ...
  if (agreement && typeof agreement === "object") {
    // A packet built before the current image has supplied any observable
    // anchors cannot prove that a catalog row is this card. Keep those rows in
    // raw/shadow telemetry and re-evaluate them after provider observation;
    // otherwise an approved but merely similar row can anchor the vision model
    // before conflicts are knowable.
