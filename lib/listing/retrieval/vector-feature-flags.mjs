...
export function vectorRetrievalConfig(env = process.env, options = {}) {
  ...
  return {
    ...
    queryTimeoutMs: optionInteger(options, ["vector_query_timeout_ms", "vectorQueryTimeoutMs"], positiveInteger(env.VECTOR_QUERY_TIMEOUT_MS || env.VISUAL_VECTOR_RETRIEVAL_TIMEOUT_MS, 20000)),
    workerMaxAttempts: optionInteger(options, ["vector_worker_max_attempts", "vectorWorkerMaxAttempts"], positiveInteger(env.VECTOR_WORKER_MAX_ATTEMPTS, 2)),
    workerRetryBaseMs: optionInteger(options, ["vector_worker_retry_base_ms", "vectorWorkerRetryBaseMs"], positiveInteger(env.VECTOR_WORKER_RETRY_BASE_MS, 200)),
...
// The blocking online-embed lane only pays off once the vector index has real
// coverage. eBay C10 (138 stored embeddings): nearest neighbours were noise
// while the critical-path embed cost 4.8s p50 / 83s p95 per card. Until the
// index is seeded past readiness, assist requests degrade to a skipped
// context. Flip with VECTOR_INDEX_READY=true (or vector_index_ready option)
// after seeding.
export function vectorIndexReady(env = process.env, options = {}) {
  if (options.vector_index_ready === true || options.vectorIndexReady === true) return true;
  if (options.vector_index_ready === false || options.vectorIndexReady === false) return false;
  return String(env.VECTOR_INDEX_READY || "").toLowerCase() === "true";
}
