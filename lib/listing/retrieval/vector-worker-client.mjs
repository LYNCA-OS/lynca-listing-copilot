...
function retryableWorkerStatus(status) {
...
// A kill switch, because the measurement says this call is currently a pure
// cost. Over 3,659 production calls it averaged 3,556ms and returned zero
// candidates 92.7% of the time -- 217 minutes spent, 201 of them for nothing.
//
// The reason is not the model. The index it searches, card_image_embeddings,
// holds 587 rows and was last written on 2026-07-06, so almost every query is
// a lookup against a table that does not contain the card.
//
// The 266 calls that did return candidates are not the hard cards, which was
// the thing worth checking before disabling anything: they read a card number
// 58.3% of the time against 33.7% for the rest, and retrieval keys on the card
// number, so having one is the entry condition rather than the payoff. On team
// and parallel -- the fields this was supposed to help with -- the two groups
// are indistinguishable, 26.7% against 26.5% and 9.4% against 9.3%.
//
// This returns the same VECTOR_RETRIEVAL_UNAVAILABLE contract every caller
// already handles for an unconfigured worker, so nothing downstream changes
// except that it stops waiting 3.5 seconds to be told nothing.
//
// Set V4_VECTOR_RETRIEVAL_DISABLED=false to restore it, which is the right
// thing to do once the index is repopulated.
export function vectorRetrievalDisabled(env = process.env) {
  const raw = String(env.V4_VECTOR_RETRIEVAL_DISABLED ?? "").trim().toLowerCase();
...
export async function embedImagesWithVectorWorker({
  ...
  if (vectorRetrievalDisabled(env)) {
    return unavailable("vector_retrieval_disabled_empty_index");
  }
  ...
  const stagePlan = listingStageCapacityPlan(env).vector;
  const stageOwner = `vector-query-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  const execution = await runWithListingStageCapacity({
    plan: stagePlan,
    ...
    fetchImpl,
    task: async () => {
      const maxAttempts = Math.max(1, Math.min(4, Number(config.workerMaxAttempts) || 2));
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
          ...
          if (!response.ok) {
            if (attempt < maxAttempts && retryableWorkerStatus(response.status)) {
              await delay(Math.min(1_500, Number(config.workerRetryBaseMs || 200) * attempt));
              continue;
            }
            return {
      ...
      return unavailable("vector_worker_attempts_exhausted");
    }
  });

  if (!execution.executed) {
    return unavailable(execution.stage_capacity?.configured === false
      ? "vector_stage_capacity_unavailable"
      : "vector_stage_capacity_busy", {
      stage_capacity: execution.stage_capacity
    });
  }
  return {
    ...execution.value,
