...
function rowSourceFeedbackId(row = {}) {
  const fields = safeJson(row.fields) || {};
  const referenceMetadata = safeJson(row.reference_metadata) || {};
  const explicit = cleanText(
    row.source_feedback_id
    || referenceMetadata.source_feedback_id
    || fields.source_feedback_id
  );
  if (explicit) return explicit;
  const match = cleanText(row.identity_key).match(/^supabase_feedback:(.+)$/i);
  return cleanText(match?.[1]);
}

function candidatePassesSourceExclusion(candidate = {}, excluded = new Set()) {
  ...
  if (sourceFeedbackId) return !excluded.has(sourceFeedbackId);
  // Approved/reviewed history is not independent evidence unless its source
  // identity is observable. Registry rows can legitimately have no feedback
  // origin and remain available.
  return candidate.source_type !== retrievalSourceTypes.INTERNAL_APPROVED_HISTORY;
}
  ...
  return {
    ...
    async search({ query = {}, resolved = {} } = {}) {
      ...
      try {
        ...
        const candidates = candidatesFromRows(rows, query);
        const excluded = excludedSourceFeedbackIds(query);
        const filteredCandidates = candidates.filter((candidate) => (
          candidatePassesSourceExclusion(candidate, excluded)
        ));
        return {
          provider_id: retrievalProviderIds.POSTGRES_HYBRID,
          ...
          metadata: {
            ...
            source_feedback_self_excluded_count: candidates.length - filteredCandidates.length
          }
        };
      } catch (error) {
