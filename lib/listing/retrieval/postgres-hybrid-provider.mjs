...
function readBody(fields = {}, query = {}) {
  return {
    search_text: cleanText(query.search_text || query.query || [subjectText(fields), productText(fields)].filter(Boolean).join(" ")),
    ...
    exact_subject: cleanText(query.exact_subject || subjectText(fields)),
    exact_year: cleanText(query.exact_year || fields.year),
    exact_product: cleanText(query.exact_product || fields.product || fields.set),
