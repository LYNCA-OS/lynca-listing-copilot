...
function providerConfig(env = process.env) {
  return {
    ...
    liveCuratedFallbackTimeoutMs: positiveInteger(env.CATALOG_LIVE_CURATED_TIMEOUT_MS, 4000),
...
async function liveCuratedCatalogRows({
  ...
  const providerRows = await Promise.all(providers.map(async (provider) => {
    const controller = new AbortController();
    const timeoutMs = Math.max(250, Number(config.liveCuratedFallbackTimeoutMs) || 4000);
    let timer = null;
    try {
      const deadline = new Promise((_, reject) => {
        timer = setTimeout(() => {
          controller.abort();
          const error = new Error(`live_curated_catalog_timeout:${provider}`);
      ...
      for (const [index, entry] of stagingRows.entries()) {
        ...
        if (rows.length >= config.liveCuratedFallbackMatchCount) break;
      }
      return rows;
    } catch {
      return [];
  ...
  return providerRows.flat().slice(0, config.liveCuratedFallbackMatchCount);
}
