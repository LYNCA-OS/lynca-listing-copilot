...
function lotFieldsFromTitle(title = "") {
  ...
  // Reading, not writing. Our own marker is `Lot*N` (founder, 2026-08-08), but
  // historical titles and the reviewed writer corpus spell it `lotx3`, so both
  // are matched here. Tolerating `x` on the way IN is not permission to emit
  // it: the composer emits `*` only.
  const lotMatch = text.match(/\blot\s*(?:x|\*|of)?\s*(\d{1,3})\b|\blot[x*](\d{1,3})\b/i);
  if (!lotMatch) return {};
  return {
    lot_type: "LOT",
