...
function applyInternalWriterTitleSpecificity(fields = {}, titleFields = {}) {
  ...
  for (const key of ["manufacturer", "brand", "product", "set", "card_name", "collector_number", "checklist_code", "language", "rarity"]) {
  ...
  if (Array.isArray(titleFields.players) && titleFields.players.length) {
...
function readBody(fields = {}, query = {}) {
  ...
  return {
    ...
    exact_subject: cleanText(query.exact_subject || (queryOnly ? "" : subjectText(normalized))),
...
function candidatesFromRows(rows = [], query = {}, config = {}) {
  return (Array.isArray(rows) ? rows : []).map((row, index) => {
    ...
    const enrichedFields = internalWriterTitle
      ? applyInternalWriterTitleSpecificity(mergeMissingFields(rawFields, titleFields), titleFields)
      : mergeMissingFields(rawFields, titleFields);
    const fields = normalizeResolvedFields({
      ...enrichedFields,
      serial_number: expectedSerialDenominator ? `/${expectedSerialDenominator}` : enrichedFields.serial_number
    });
