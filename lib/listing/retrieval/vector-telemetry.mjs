...
function featureRows({
  ...
  return features
    .map((feature) => {
      ...
      return {
        tenant_id: cleanText(context.tenantId || context.tenant_id),
...
export async function recordVectorRetrievalTelemetry({
  ...
  const tenantId = cleanText(context.tenantId || context.tenant_id);
  if (!tenantId) return { saved: false, reason: "tenant_id_required" };
  ...
  try {
    ...
    const candidates = retrievalRunId ? candidateRows({ retrievalRunId, tenantId, packet }) : [];
