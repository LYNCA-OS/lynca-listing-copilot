...
function candidatesFromRows(rows = [], query = {}) {
  ...
  return normalizedRows.map((row, index) => {
    ...
    return {
      ...
      reference_metadata: {
        ...referenceMetadata,
        source_feedback_id: rowSourceFeedbackId(row),
...
function rowSourceFeedbackId(row = {}) {
  const explicit = metadataValue(row, "source_feedback_id");
  if (explicit) return explicit;

  const identityKey = cleanText(row.identity_key);
  const match = identityKey.match(/^supabase_feedback:(.+)$/i);
...
function rowIsSelfExcluded(row = {}, query = {}) {
  ...
  const excludedSourceFeedbackIds = [
    query.source_feedback_id,
    ...(Array.isArray(query.exclude_source_feedback_ids)
      ? query.exclude_source_feedback_ids
      : [query.exclude_source_feedback_ids])
  ].map(cleanText).filter(Boolean);
  if (sourceFeedbackId && excludedSourceFeedbackIds.includes(sourceFeedbackId)) return true;
...
async function fetchVectorRows({
  ...
  const response = await fetchWithTimeout(fetchImpl, endpoint, {
    ...
    body: JSON.stringify({
      ...
      include_candidate_identities: includeCandidateIdentities
    })
  }, config.timeoutMs);
  ...
  if (!response.ok) {
    ...
    return {
      ok: false,
      reason: `supabase_visual_vector_rpc_${response.status}:${message.slice(0, 80)}`,
      rows: []
    };
  }
...
export function visualVectorProvider({
  ...
  return {
    ...
    async search({ query = {}, resolved = {} } = {}) {
      ...
      const excludedSourceFeedbackIds = [...new Set([
        query.source_feedback_id,
        ...(Array.isArray(query.exclude_source_feedback_ids)
          ? query.exclude_source_feedback_ids
          : [query.exclude_source_feedback_ids])
      ...
      try {
        let queryResult = await fetchVectorRows({
          config,
          fetchImpl,
          endpoint,
          embedding,
          ...
          embeddingRole,
          category,
          includeCandidateIdentities: config.includeCandidateIdentities
        });
        if (!queryResult.ok) return unavailable(queryResult.reason);

        let fallbackUsed = false;
        let fallbackReason = "";
        let rows = queryResult.rows;
        if (!rows.length && embeddingRole) {
          const fallbackResult = await fetchVectorRows({
            config,
            fetchImpl,
            endpoint,
            embedding,
          ...
          if (fallbackResult.ok && fallbackResult.rows.length) {
            rows = fallbackResult.rows;
            fallbackUsed = true;
            fallbackReason = "embedding_role_zero_hit";
          }
        }

        const filteredRows = rows.filter((row) => !rowIsSelfExcluded(row, query));
        return {
          provider_id: retrievalProviderIds.VISUAL_VECTOR,
          metadata: {
            query_embedding_role: embeddingRole || "",
            role_agnostic_fallback_used: fallbackUsed,
            role_agnostic_fallback_reason: fallbackReason,
            returned_row_count: rows.length,
            self_excluded_count: rows.length - filteredRows.length,
            source_feedback_exclusion_filter_active: excludedSourceFeedbackIds.length > 0,
            source_feedback_exclusion_count: excludedSourceFeedbackIds.length
          },
