...
function providerRuntimeFieldsForLane(lane, context = {}, metadata = null) {
  ...
  return {
    ...
    self_excluded_count: Number.isFinite(Number(providerMetadata.self_excluded_count))
      ...
      : null,
    self_exclusion_query_attempted: Boolean(context?.retrieval),
    self_exclusion_filter_active: context?.self_retrieval_exclusion?.query_planner_filter_active === true,
    self_exclusion_requested_source_count: Number.isFinite(Number(
      context?.self_retrieval_exclusion?.requested_source_feedback_id_count
    ))
      ? Number(context.self_retrieval_exclusion.requested_source_feedback_id_count)
      : null,
    self_exclusion_source_ids_sha256: cleanText(
      context?.self_retrieval_exclusion?.source_feedback_ids_sha256
    ) || null
  };
}
