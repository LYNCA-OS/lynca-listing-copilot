...
function packetSignal(packet = {}) {
  ...
  const deferred = Array.isArray(retrieval.deferred) ? retrieval.deferred : [];
  return {
    ...
    deferred_reasons: deferred.map((item) => cleanText(item.reason)).filter(Boolean)
  };
}
...
function providerRuntimeFieldsForLane(lane, context = {}, metadata = null) {
  ...
  const output = {
    ...
    runtime_deferred_reasons: signal.deferred_reasons || []
  };
  ...
  return {
    ...
    self_excluded_count: Number.isFinite(Number(providerMetadata.self_excluded_count))
      ...
      : null,
    self_exclusion_query_attempted: context?.self_exclusion_query_attempted === true
      || providerMetadata.source_feedback_exclusion_filter_active === true,
    self_exclusion_filter_active: context?.self_exclusion_filter_active === true
      || providerMetadata.source_feedback_exclusion_filter_active === true,
    self_exclusion_requested_source_count: Number.isFinite(Number(context?.self_exclusion_requested_source_count))
      ? Number(context.self_exclusion_requested_source_count)
      : Number.isFinite(Number(providerMetadata.source_feedback_exclusion_count))
        ? Number(providerMetadata.source_feedback_exclusion_count)
        : null,
    self_exclusion_source_ids_sha256: context?.self_exclusion_source_ids_sha256 || null
  };
}
...
function summarizeLane({
  ...
  const trustBlockedCount = numeric(
    eligibility.trust_blocked_count,
...
export function buildCandidateContextSummary({
  ...
  return {
    ...
    trust_blocked_count: numeric(openSetReadiness.trust_blocked_count, catalog.trust_blocked_count + vector.trust_blocked_count),
