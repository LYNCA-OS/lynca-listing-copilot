...
function canonicalCacheInput(value) {
  if (Array.isArray(value)) {
    return value
      .map(canonicalCacheInput)
  ...
  return Object.fromEntries(
    Object.entries(value)
      ...
      .map(([key, item]) => [key, canonicalCacheInput(item)])
  );
}

function keyFor(query = {}) {
  // Retrieval constraints are decision inputs, not trace metadata. In
  // particular, source exclusions must never reuse an unfiltered cache entry.
  // Key the complete query contract so newly added exact constraints fail safe
  // without requiring developers to remember another manual cache bump.
  return JSON.stringify(canonicalCacheInput(query));
}
