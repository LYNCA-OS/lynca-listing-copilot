...
function finalizePlannedQueries(planned = [], excludeSourceFeedbackIds = []) {
  ...
  return planned
    ...
    .map((query) => {
      if (!exclusions.length) return query;
      if (![retrievalProviderIds.CATALOG, retrievalProviderIds.VISUAL_VECTOR].includes(query.provider_id)) {
        return query;
      }
      return {
        ...query,
        exclude_source_feedback_ids: normalizedExclusionIds([
          ...(Array.isArray(query.exclude_source_feedback_ids) ? query.exclude_source_feedback_ids : []),
          ...exclusions
        ]),
