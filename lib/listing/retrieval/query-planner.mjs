...
export function planRetrievalQueries({
  ...
  planned.push(makeQuery(retrievalQueryFamilies.INTERNAL_REGISTRY, planned.length, [
    ...
    fields: ["checklist_code", "insert", "parallel", "product"],
  ...
  if (subject && (fields.product || fields.brand) && serialDenom) {
    planned.push(makeQuery(retrievalQueryFamilies.PRODUCT_AND_SERIAL_DENOMINATOR, planned.length, `${quotePhrase(subject)} ${quotePhrase(fields.product || fields.brand)} ${quotePhrase(serialDenom)}`, {
      fields: ["players", "character", "product", "serial_number"],
      reason: "serial denominator can disambiguate scarce parallels"
    }));
  }
