...
function finalizePlannedQueries(planned = [], excludeSourceFeedbackIds = []) {
  ...
  return planned
    ...
    .map((query) => {
      // Visual self-matches would leak the current image back into its own
      // recognition path, so vectors stay source-excluded. Official and
      // writer-reviewed catalogs are durable identity memory: a previously
      // seen card is allowed to match those directories on a later run.
      if (!exclusions.length || query.provider_id !== retrievalProviderIds.VISUAL_VECTOR) {
        return query;
      }
      return {
        ...query,
