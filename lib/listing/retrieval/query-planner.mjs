...
const sourceFeedbackExclusionProviders = new Set([
...
function finalizePlannedQueries(planned = [], excludeSourceFeedbackIds = []) {
  ...
  return planned
    ...
    .map((query) => {
      // A reviewed benchmark row may exist in multiple internal retrieval
      // lanes. Carry the same source exclusion through every provider capable
      // of returning that row; official-only and external providers remain
      // unchanged. Providers still decide locally which candidates match.
      if (!exclusions.length || !sourceFeedbackExclusionProviders.has(query.provider_id)) {
        return query;
      }
      return {
        ...query,
...
function addCatalogFirstQueries(planned, fields, {
  ...
  // Product-independent subject anchor. When the model mis-identifies the
  // product ("Chrome Black" for a Smugglers card), every product-scoped lane
  // queries the wrong product and returns nothing (catalog_raw_candidate_count=0).
  // Anchoring on subject + serial denominator + year with the observed product
  // ignored keeps catalog recall alive so a trusted candidate can still correct
  // the product downstream. All three anchors are required: subject + year alone
  // is highly collision-prone for star players, while set/insert already has its
  // own dedicated catalog lane. Exact printed codes also have a stronger lane.
  if (subject && fields.year && serialDenom && !fields.checklist_code && !collectorNumber) {
    planned.push(makeQuery(retrievalQueryFamilies.CATALOG_SUBJECT_ANCHOR, planned.length, [
      subject,
      serialDenom,
      fields.year
    ].filter(Boolean).map(quotePhrase).join(" "), {
      fields: ["players", "serial_denominator", "year"],
      reason: "catalog subject anchor recalls trusted candidates independent of a possibly mis-identified product",
      extra: {
        ...categoryExtra,
        exact_subject: subject,
        exact_year: fields.year || "",
...
export function planRetrievalQueries({
  ...
  addCatalogFirstQueries(planned, fields, { subject, product, serialDenom, categories });
