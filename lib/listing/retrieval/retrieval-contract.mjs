...
export const allowedOpenAiWebSearchModels = Object.freeze([
  // Legacy OWS compatibility only. This list does not make OWS reachable
  // unless the deployment explicitly enables the fallback.
  "gpt-4.1-mini",
  "gpt-4.1"
]);

export const defaultOpenAiWebSearchFallbackEnabled = false;

export function openAiWebSearchFallbackEnabled(env = {}) {
  const raw = env.ENABLE_OPENAI_WEB_SEARCH_FALLBACK;
  if (raw === undefined || raw === null || raw === "") {
    return defaultOpenAiWebSearchFallbackEnabled;
  }
