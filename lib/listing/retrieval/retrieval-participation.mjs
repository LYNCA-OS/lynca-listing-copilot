...
function appliedFieldsForSource({ funnel = {}, traceRows = [], decisionStage = {} } = {}) {
...
function supportedFieldsForSource({ funnel = {}, traceRows = [] } = {}) {
  return cleanTextList([
...
export function classifyRetrievalParticipation({
  ...
  const evidenceSupportFieldCount = finiteCount(funnel.evidence_support_field_count);
  const appliedFields = appliedFieldsForSource({ funnel, traceRows: rows, decisionStage: candidateDecisionStage });
  const supportedFields = supportedFieldsForSource({ funnel, traceRows: rows });
  const selectedId = selectedCandidateId(funnel, candidateDecisionStage);
  const appliedFieldCount = Math.max(finiteCount(funnel.applied_field_count), appliedFields.length);
