...
import { additiveCohorts, rankWithAdditiveAdmission } from "./additive-source-admission.mjs";
...
import { contractedConcurrency, listingConcurrencyContract } from "../v4/orchestration/concurrency-contract.mjs";
...
function normalizedSourceExclusions(values = []) {
  return new Set((Array.isArray(values) ? values : [values])
    .map((value) => String(value || "").trim())
    .filter(Boolean));
}

function candidateSourceFeedbackId(candidate = {}) {
  return String(
...
function excludeSelfReferenceCandidates(candidates = [], exclusions = new Set()) {
  if (!exclusions.size) return Array.isArray(candidates) ? candidates : [];
  return (Array.isArray(candidates) ? candidates : []).filter((candidate) => {
    const sourceFeedbackId = candidateSourceFeedbackId(candidate);
    return !sourceFeedbackId || !exclusions.has(sourceFeedbackId);
  });
}

export function internalQueryConcurrency(env = process.env, queryCount = 0) {
  if (!envFlag(env.ENABLE_INTERNAL_RETRIEVAL_QUERY_CONCURRENCY, true)) return 1;
  const contracted = contractedConcurrency(
    "catalog_internal_queries",
    positiveInteger(
      env.RETRIEVAL_INTERNAL_QUERY_CONCURRENCY,
      listingConcurrencyContract.catalog_internal_queries.concurrency
    )
  );
  return Math.max(1, Math.min(contracted, queryCount || 1));
}
...
export async function runRetrieval({
  ...
  // Provider-local filters are useful, but the Retrieval Engine is the unique
  // owner of the candidate set. Enforce self-exclusion again at this boundary
  // so cache, fallback, and future provider paths cannot reintroduce a row from
  // the card currently under evaluation.
  const sourceExclusions = normalizedSourceExclusions(excludeSourceFeedbackIds);
  const admissibleCandidates = excludeSelfReferenceCandidates(allCandidates, sourceExclusions);
  const selfExcludedCandidateCount = allCandidates.length - admissibleCandidates.length;

  // A cohort under evaluation is ranked separately so it cannot move the
  // selection or the margin of everything that was already there.
  const evaluationCohorts = additiveCohorts(env);
  const rankCandidates = hybridEnabled
    ? (rows, resolvedFields, options = {}) => rankHybridRetrievalCandidates(rows, resolvedFields, {
      rrfK,
      lowMarginThreshold,
      hardNegatives,
      ...options
    })
    : (rows, resolvedFields, options = {}) => rankRetrievalCandidates(rows, resolvedFields, options);
  const ranked = rankWithAdditiveAdmission(admissibleCandidates, resolved, {
    rank: rankCandidates,
  ...
  return {
    ...
    self_retrieval_exclusion: {
      requested_source_count: sourceExclusions.size,
      excluded_candidate_count: selfExcludedCandidateCount,
      active: sourceExclusions.size > 0
    },
