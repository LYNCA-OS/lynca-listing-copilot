...
const tenantScopedQueryFamilies = new Set([
...
function withTenantCacheScope(query = {}, tenantId = "") {
  if (!tenantScopedQueryFamilies.has(query.family)) return query;
  ...
  return {
    ...query,
    cache_tenant_id: normalizedTenantId
  };
}
...
export async function runRetrieval({
  ...
  const registry = providerRegistry || createRetrievalProviderRegistry({ env, tenantId });
  ...
  const queries = planRetrievalQueries({
    ...
    allowOwsFallback,
    excludeSourceFeedbackIds
  }).map((query) => withTenantCacheScope(query, tenantId));
  const providersUsed = [];
  const unavailable = [];
  const trace = [];
  const allCandidates = [];
