...
export const identityCacheResolverSnapshotVersion = "identity-cache-resolver-snapshot-v1";
...
const privateCacheKeys = new Set([
  "tenant_id", "tenantid", "user_id", "userid", "asset_id", "assetid",
  "object_path", "objectpath", "storage_path", "storagepath", "signed_url",
  "signedurl", "bucket", "request_id", "requestid", "job_id", "jobid",
  "session_id", "sessionid", "analysis_run_id", "analysisrunid"
]);

function reusableCacheValue(value) {
  if (Array.isArray(value)) return value.map(reusableCacheValue).filter((item) => item !== undefined);
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value)
      .filter(([key]) => !privateCacheKeys.has(String(key).replace(/-/g, "_").toLowerCase()))
      .map(([key, item]) => [key, reusableCacheValue(item)])
      .filter(([, item]) => item !== undefined));
  }
  if (typeof value === "string" && /(?:tenants\/[^/]+\/listing-assets\/|\/storage\/v1\/object\/sign\/|[?&]token=)/i.test(value)) {
    return undefined;
  }
  return value;
}

export function buildAuthoritativeResolverReplaySnapshot(result = {}) {
  return reusableCacheValue({
    snapshot_version: identityCacheResolverSnapshotVersion,
    ...
    resolved: result.resolved && typeof result.resolved === "object" ? result.resolved : {},
    field_states: result.field_states ?? null,
    unresolved: result.unresolved ?? null,
    conflict_map: result.conflict_map ?? result.conflicts ?? null,
    confidence_report: result.confidence_report ?? null,
    assisted_draft_status: result.assisted_draft_status ?? null,
    writer_review_required: result.writer_review_required ?? null,
    writer_review_reason: result.writer_review_reason ?? null
  });
}

function resolverReplayEnvelope(result = {}) {
  return {
    cache_envelope_version: identityCacheResolverSnapshotVersion,
    original_identity_resolution: reusableCacheValue(result.identity_resolution ?? null),
    resolver_snapshot: buildAuthoritativeResolverReplaySnapshot(result)
  };
}

function resolverReplaySnapshotFromRecord(record = {}) {
  const envelope = record.identity_resolution;
  if (envelope?.cache_envelope_version === identityCacheResolverSnapshotVersion
    && envelope.resolver_snapshot?.snapshot_version === identityCacheResolverSnapshotVersion) {
    return envelope.resolver_snapshot;
  }
  return null;
}

function reusableImageFingerprints(fingerprints = []) {
...
export function buildVerifiedImageGenerationIdentity(payload = {}) {
...
export function buildIdentityResultCacheKey(payload = {}, env = process.env) {
  const imageIdentity = buildVerifiedImageGenerationIdentity(payload);
  if (!imageIdentity.ok) return imageIdentity;
  const readiness = recognitionCacheRevisionReadiness(payload, env);
  if (!readiness.ok) return { ...imageIdentity, ok: false, reason: readiness.reason, cache_key: "" };
  const resultVersion = buildIdentityCacheVersionVector(payload, env);
  const cacheKey = crypto.createHash("sha256")
    .update(JSON.stringify({
      version: identityCacheContractVersion,
      image_generation_hash: imageIdentity.image_generation_hash,
  ...
  return {
    ok: true,
    cache_key: cacheKey,
    tenant_id: imageIdentity.tenant_id,
    image_generation_hash: imageIdentity.image_generation_hash,
    version_fingerprint: resultVersion.fingerprint,
    recognition_pipeline_fingerprint: resultVersion.recognition_pipeline_fingerprint,
    result_version: resultVersion.vector,
    image_fingerprints: imageIdentity.image_fingerprints,
    image_count: imageIdentity.image_count
  };
}

export function buildTenantScopedIdentityInFlightKey(payload = {}, env = process.env) {
  const globalKey = buildIdentityResultCacheKey(payload, env);
  const tenantId = tenantIdForPayload(payload);
  if (!globalKey.ok || !tenantId) return null;
  return crypto.createHash("sha256")
    .update(JSON.stringify({ tenant_id: tenantId, global_cache_key: globalKey.cache_key }))
    .digest("hex");
}
...
export function identityResultToCacheRow({
  ...
  if (!key.tenant_id) return { ok: false, reason: "tenant_id_missing", cache_key: key.cache_key };
  ...
  return {
    ...
    row: {
      ...
      recognition_pipeline_fingerprint: key.recognition_pipeline_fingerprint || key.version_fingerprint,
      result_version: key.result_version,
      image_fingerprints: reusableImageFingerprints(key.image_fingerprints),
      image_count: key.image_count || key.image_fingerprints.length,
      identity_status: result.identity_resolution_status,
      ambiguity_status: result.ambiguity_status || null,
      final_title: finalTitle,
      resolved_fields: reusableCacheValue(resolved),
      legacy_fields: reusableCacheValue(result.fields || resolvedFieldsToLegacyFields(resolved)),
      evidence_snapshot: {},
      identity_resolution: resolverReplayEnvelope(result),
      field_states: reusableCacheValue(result.field_states || {}),
      conflict_map: reusableCacheValue(result.conflict_map || result.conflicts || []),
      resolution_trace: reusableCacheValue(
        Array.isArray(result.resolution_trace) ? result.resolution_trace.slice(0, 128) : []
      ),
      confidence_report: reusableCacheValue(result.confidence_report || null),
...
export async function readIdentityResultCacheRecord({
  ...
  mismatchEndpoint.searchParams.set("select", "cache_key,version_fingerprint,recognition_pipeline_fingerprint,result_version,updated_at");
...
export function identityResultCacheRecordToListingResult({
  ...
  const resolverReplaySnapshot = resolverReplaySnapshotFromRecord(record);
  const cachedResultVersionMatch = expectedVersion
    ? identityCacheVersionMatches(record, expectedVersion)
    : Boolean(record.recognition_pipeline_fingerprint || record.version_fingerprint);
  return {
    title,
    final_title: title,
    ...
    identity_resolution: resolverReplaySnapshot
      ? resolverReplaySnapshot.identity_resolution
      : record.identity_resolution || null,
    field_states: resolverReplaySnapshot?.field_states ?? jsonContainer(record.field_states, []),
    conflict_map: resolverReplaySnapshot?.conflict_map ?? record.conflict_map ?? null,
    confidence_report: resolverReplaySnapshot?.confidence_report ?? record.confidence_report ?? null,
    unresolved: resolverReplaySnapshot?.unresolved ?? null,
    assisted_draft_status: resolverReplaySnapshot?.assisted_draft_status ?? null,
    writer_review_required: resolverReplaySnapshot?.writer_review_required ?? null,
    writer_review_reason: resolverReplaySnapshot?.writer_review_reason ?? null,
    resolver_replay_snapshot: resolverReplaySnapshot,
    ...
    identity_cache: {
      ...
      recognition_pipeline_fingerprint: record.recognition_pipeline_fingerprint || record.version_fingerprint || null,
