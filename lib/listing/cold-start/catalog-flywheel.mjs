...
function previousPromotionTitleForGap(gapRow = {}) {
  return cleanText(
    gapRow.promoted_title
    || gapRow.metadata?.promoted_title
    || gapRow.metadata?.promotion_title
    || gapRow.metadata?.last_promoted_title
    || gapRow.metadata?.writer_final_title_at_promotion
    || gapRow.metadata?.ai_draft_title_at_promotion
  );
}

function promotedStatusForGap(gapRow = {}) {
  return ["APPROVED", "PROMOTED", "MERGED"].includes(cleanText(gapRow.status).toUpperCase())
    || ["APPROVED", "PROMOTED", "MERGED"].includes(cleanText(gapRow.promotion_status).toUpperCase());
}

function writerTitleSupersedesGap({ gapRow = {}, writerFinalTitle = "" } = {}) {
  const previousTitle = previousPromotionTitleForGap(gapRow);
  const finalTitle = cleanText(writerFinalTitle);
  return promotedStatusForGap(gapRow)
    && Boolean(previousTitle)
    && Boolean(finalTitle)
...
function reviewedCatalogStaging({ gapRow = {}, writerFinalTitle = "", writerConfirmedFields = {}, action = "" } = {}) {
  ...
  const supersedesPreviousPromotion = writerTitleSupersedesGap({ gapRow, writerFinalTitle });
  ...
  staging.source.source_metadata = {
    ...
    writer_title_supersedes_previous_promotion: supersedesPreviousPromotion,
    previous_promotion_title: previousPromotionTitleForGap(gapRow) || null,
    ai_draft_title: cleanText(gapRow.ai_draft_title) || null
  };
...
export function applyWriterConfirmationToFlywheel({
  ...
  const writerSupersedesPreviousPromotion = writerTitleSupersedesGap({ gapRow, writerFinalTitle });
  const previousPromotionTitle = previousPromotionTitleForGap(gapRow);
  ...
  const updatedGapRow = {
    ...
    metadata: {
      ...
      external_candidate_used_as_truth: false,
      promotion_requires_writer_review: true,
      writer_final_title_overrides_ai_draft: Boolean(writerFinalTitle),
      writer_title_supersedes_previous_promotion: writerSupersedesPreviousPromotion,
      previous_promotion_title: previousPromotionTitle || null,
      ai_draft_title_at_review: cleanText(gapRow.ai_draft_title) || null
    }
  };
