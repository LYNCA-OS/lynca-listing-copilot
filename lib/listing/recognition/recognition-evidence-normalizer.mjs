...
function multiCardDetectionSummary(response = {}) {
  ...
  const imageDetections = Array.isArray(detection.images) ? detection.images : [];
  const independentlySingle = imageDetections.length >= 2
    && imageDetections.every((item) => (
      item?.status === "OK"
      && item?.multi_card !== true
      && Number(item?.card_count_estimate ?? item?.card_count) === 1
      && Array.isArray(item?.candidates)
      && item.candidates.length === 1
      && item?.detectors?.central_two_by_two_grid?.status !== "CONFIRMED"
    ))
    && imageDetections.some((item) => Number(item?.confidence) >= 0.9)
    && detected === false
    && validCount === 1
    && confidenceForItem(detection, 0) >= 0.9;
  return {
    ...
    single_card_independently_supported: detection.status === "OK" && independentlySingle,
    single_card_supporting_image_count: independentlySingle ? imageDetections.length : 0,
