...
function entriesFromMultiCardDetection(response = {}) {
  ...
  const hasEstimatedCount = Number.isInteger(cardCount) && cardCount >= 1;
  const hasConfirmedCount = detection.card_count_confirmed === true && hasEstimatedCount;
  ...
  const source = sourceForEntry({
    ...
    observed_text: multiCard
      ? `independent detector observed ${hasEstimatedCount ? `at least ${cardCount}` : "multiple"} separate cards`
      : "",
    region: {
