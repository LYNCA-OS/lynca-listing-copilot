...
function entriesFromMultiCardDetection(response = {}) {
  ...
  // The rectangle detector intentionally reports an unconfirmed lower bound.
  // Slab borders, labels and card frames can look like separate cards, so that
  // diagnostic signal must never become identity evidence on its own.
