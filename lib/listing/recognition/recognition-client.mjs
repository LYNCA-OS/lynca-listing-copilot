...
export async function analyzeCardImagesWithRecognitionWorker({
  ...
  try {
    ...
    const expectedRevision = String(env.RECOGNITION_WORKER_REVISION || "").trim();
    const actualRevision = String(payload.processing?.service_revision || "").trim();
    if (expectedRevision && actualRevision !== expectedRevision) {
      throw new RecognitionWorkerError("Recognition worker revision does not match the expected immutable release.", {
        code: "recognition_worker_revision_mismatch",
        status: 503,
        retryable: true,
        details: {
