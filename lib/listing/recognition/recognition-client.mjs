...
function serviceContractProbeCacheKey(config = {}) {
...
async function runRecognitionWorkerServiceContractProbe({
  config,
  fetchImpl,
  timeoutMs
}) {
  ...
  try {
    ...
    // This intentionally invalid request proves only service identity, bearer
    // authorization and ownership of the analysis route. It does not claim
    // that image download, OCR, model preload or evidence production is ready.
    // No image reaches execution: the full Worker authenticates first and then
    // returns its contract validator's 422; a wrong token returns 403 and the
    // field-only OCR service returns 404.
    stage = "analysis";
    ...
    await consumeResponseBody(analysisResponse);
    return {
    ...
    return {
      ...
      reason: error?.name === "AbortError"
        ? `recognition_worker_${stage}_timeout`
        : `recognition_worker_${stage}_error`
    };
  } finally {
...
export async function probeRecognitionWorkerServiceContract({
  env = process.env,
  ...
  const key = serviceContractProbeCacheKey(config);
  const now = Date.now();
  if (serviceContractProbeCache.key === key && serviceContractProbeCache.value && serviceContractProbeCache.expires_at > now) {
    return serviceContractProbeCache.value;
  }
  if (serviceContractProbeCache.key === key && serviceContractProbeCache.promise) return serviceContractProbeCache.promise;

  const promise = runRecognitionWorkerServiceContractProbe({ config, fetchImpl, timeoutMs })
    .then((value) => {
      serviceContractProbeCache = {
        key,
  ...
  serviceContractProbeCache = { key, expires_at: 0, value: null, promise };
  return promise;
}
