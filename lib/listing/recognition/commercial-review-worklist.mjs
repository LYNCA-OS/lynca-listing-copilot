...
function worklistItem(task = {}, index = 0) {
  ...
  return {
    ...
    corrected_title_is_reviewed_title_ground_truth: task.corrected_title_is_reviewed_title_ground_truth === true || Boolean(task.corrected_title_hint || task.source_titles?.corrected_title),
    corrected_title_used_as_field_ground_truth: task.corrected_title_used_as_field_ground_truth === true,
...
export function createCommercialReviewWorklist(reviewPacket = {}, {
  ...
  const badPolicyCount = allItems.filter((item) => item.corrected_title_used_as_field_ground_truth || item.corrected_title_used_as_ground_truth || item.suggestions_are_ground_truth).length;
  ...
  return {
    ...
    summary: {
      ...
      corrected_title_reviewed_ground_truth_count: items.filter((item) => item.corrected_title_is_reviewed_title_ground_truth).length,
      corrected_title_used_as_field_ground_truth_count: items.filter((item) => item.corrected_title_used_as_field_ground_truth).length,
