...
export function createCommercialReviewPacket(candidateManifest = {}, {
  ...
  const tasks = selectedItems.map((item) => {
    ...
    return {
      ...
      corrected_title_is_reviewed_title_ground_truth: Boolean(sourceTitles.corrected_title),
      title_ground_truth_scope: sourceTitles.corrected_title ? "writer_reviewed_marketplace_title" : null,
      ...
      review_notes: "corrected_title is reviewed title ground truth. Fill field-level reviewed_ground_truth from image/card/official evidence; suggested_fields are title-derived hints only."
    };
  });
  ...
  return {
    ...
    summary: {
      ...
      corrected_title_reviewed_ground_truth_count: tasks.filter((task) => task.corrected_title_is_reviewed_title_ground_truth).length,
      corrected_title_is_reviewed_title_ground_truth: tasks.some((task) => task.corrected_title_is_reviewed_title_ground_truth),
      corrected_title_used_as_ground_truth: false,
      corrected_title_used_as_field_ground_truth: false,
...
function validateReviewedTask(task = {}, index = 0, {
  ...
  if (task.corrected_title_used_as_field_ground_truth === true) {
    reasons.push("corrected_title cannot be used as field-level ground truth");
  }
