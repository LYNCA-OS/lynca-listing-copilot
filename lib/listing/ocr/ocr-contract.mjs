...
const evidenceFieldsByNormalizedField = Object.freeze({
  ...
  tcg_card_number: "collector_number",
...
function extractSerials(text = "") {
  ...
  const full = normalized.matchAll(/\b(\d{1,5})\s*\/\s*(\d{1,5})\b/g);
  for (const match of full) {
    const numerator = Number(match[1]);
    const denominator = Number(match[2]);
    if (Number.isFinite(numerator) && Number.isFinite(denominator) && denominator >= 1 && numerator >= 0 && numerator <= denominator) {
      serials.push({
        serial_number: `${numerator}/${denominator}`,
        serial_denominator: String(denominator)
      });
    }
  }
  const denominatorOnly = normalized.match(/\B\/\s*(\d{1,5})\b/);
  if (!serials.length && denominatorOnly) {
    serials.push({
      serial_number: null,
      serial_denominator: denominatorOnly[1]
    });
  }
  return serials;
}
...
function extractChecklistCodes(text = "") {
  ...
  ].map((match) => match[0]).filter((token) => !/^(?:PSA|BGS|SGC|CGC|TAG)/.test(token)));
}

function extractCollectorNumbers(text = "") {
  const normalized = normalizeOcrText(text).toUpperCase();
  const tokens = [
    ...normalized.matchAll(/(?:CARD\s*(?:NO\.?|NUMBER|#)|NO\.?|#)\s*([A-Z]{0,4}\d{1,4}[A-Z]?)/gi)
  ].map((match) => match[1]);
  if (!tokens.length) {
    tokens.push(...[...normalized.matchAll(/\b([A-Z]{0,3}\d{1,4}[A-Z]?)\b/g)]
      .map((match) => match[1])
      .filter((token) => {
        if (/^(?:19|20)\d{2}$/.test(token)) return false;
        if (/^\d{1,5}$/.test(token) && normalized.includes(`/${token}`)) return false;
        return true;
      }));
  }
...
function extractGradeFields(text = "") {
  ...
  const gradeText = normalized.match(/\b(?:GEM\s+MT|GEM\s+MINT|MINT|NM-MT|NM|EX-MT|EX)?\s*(AUTHENTIC|AUTH|ALTERED|\d+(?:\.\d+)?)\b/)?.[1] || null;
...
function normalizedFieldsFromText(text = "", cropType = "") {
  const fields = {};
  const normalizedCropType = normalizeOcrCropType(cropType);
  const serials = extractSerials(text);
  if (["serial_number", "serial_denominator"].includes(normalizedCropType) || serials.length) {
    if (serials[0]?.serial_number) fields.serial_number = serials[0].serial_number;
    if (serials[0]?.serial_denominator) fields.serial_denominator = serials[0].serial_denominator;
  }

  const gradeAllowed = ["grade_label", "slab_cert"].includes(normalizedCropType)
    || /\b(?:PSA\/?DNA|PSA|BGS|CGC|SGC|TAG|CERT|GEM\s+MT|GEM\s+MINT)\b/i.test(text);
  if (gradeAllowed) {
    const grade = extractGradeFields(text);
    Object.entries(grade).forEach(([field, value]) => {
...
export function normalizePaddleOcrResponse(payload = {}, requestInput = {}, {
  ...
  const normalizedFields = mergeExplicitFields(
    payload.normalized_fields || payload.normalizedFields || payload.fields,
    normalizedFieldsFromText(rawText, request.crop_type)
  );
...
export function ocrResultToEvidencePatch(ocrResult = {}, {
  ...
  Object.entries(normalizedFields).forEach(([normalizedField, value]) => {
    ...
    const confidence = clamp01(ocrResult.confidence, 0.5);
...
function mergeEvidenceField(existing = null, incoming = null, fieldName = "") {
  ...
  if (conflict) {
    conflicts.push({
      ...
      severity: ["serial_number", "grade_company", "card_grade", "collector_number", "checklist_code"].includes(fieldName) ? "HIGH" : "MEDIUM",
...
export function taskIdForOcrCropType(cropType = "") {
  ...
  if (["serial_number", "serial_denominator"].includes(normalized)) return "ocr_serial_verifier";
