...
function candidateGradeFields(textCandidates = [], rawText = "", cropType = "") {
  if (normalizeOcrCropType(cropType) !== "grade_label") return {};
  ...
  const candidates = textCandidates.map((candidate, index) => {
    const text = normalizeOcrText(candidate.text).toUpperCase();
    const previous = normalizeOcrText(textCandidates[index - 1]?.text || "").toUpperCase();
    const next = normalizeOcrText(textCandidates[index + 1]?.text || "").toUpperCase();
    ...
    const autoContext = /\b(?:AUTO|AUTOGRAPH)\b/.test(text)
      || (/^\s*(?:10(?:\.0)?|[1-9](?:\.\d)?)\s*$/.test(text)
        && /\b(?:AUTO|AUTOGRAPH)\b/.test(`${previous} ${next}`));
    const normalizedGrade = explicitCardGradeContext ? normalizeGradeValue(text) : null;
...
function extractPrintRuns(text = "", {
  ...
  const denominatorOnly = normalized.match(/#?\s*\/\s*(\d{1,5})\b/);
...
export function normalizePaddleOcrResponse(payload = {}, requestInput = {}, {
  ...
  const normalizedFields = sanitizeGradeFields(mergeExplicitFields(
