...
function fullImageCollectorNumber(textCandidates = [], metadata = {}) {
  ...
  if (alphanumeric[0]) return alphanumeric[0].token;
  ...
  if (sourceSide === "back" && numericTokens.length === 1) {
    ...
    if (Math.max(...unique.map((candidate) => candidate.confidence)) >= 0.92) return numericTokens[0];
  }
  ...
  if (positioned[0]) return positioned[0].token;
  return null;
}
