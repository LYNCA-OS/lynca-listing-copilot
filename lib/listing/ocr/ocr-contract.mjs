...
function fullImageCollectorNumberDecision(textCandidates = [], metadata = {}) {
  const fullImageVertices = textCandidates
    .filter((candidate) => candidate.ocr_pass === "full_image_fallback")
    .flatMap((candidate) => boxVertices(candidate.box))
    .filter((point) => Number.isFinite(point.x) && Number.isFinite(point.y));
  const inferredWidth = Math.max(0, ...fullImageVertices.map((point) => point.x));
  const inferredHeight = Math.max(0, ...fullImageVertices.map((point) => point.y));
  const sourceWidth = Number(metadata.source_width || metadata.sourceWidth) || inferredWidth;
  const sourceHeight = Number(metadata.source_height || metadata.sourceHeight) || inferredHeight;
  ...
  const candidates = textCandidates.flatMap((candidate) => {
    ...
    const positionalEligible = !numeric || (top <= 0.40 && (horizontalCenter <= 0.35 || horizontalCenter >= 0.65));
    return [{ token, area, confidence: Number(candidate.confidence), numeric, positionalEligible }];
  });
  const alphanumeric = candidates.filter((candidate) => !candidate.numeric);
  alphanumeric.sort((left, right) => right.area - left.area || right.confidence - left.confidence);
  if (alphanumeric[0]) return { value: alphanumeric[0].token, method: "full_image_alphanumeric" };
  const numericTokens = [...new Set(candidates.filter((candidate) => candidate.numeric).map((candidate) => candidate.token))];
  const sourceSide = normalizeText(metadata.source_side || metadata.sourceSide).toLowerCase();
  if (sourceSide === "back" && numericTokens.length === 1) {
    const unique = candidates.filter((candidate) => candidate.token === numericTokens[0]);
    if (Math.max(...unique.map((candidate) => candidate.confidence)) >= 0.92) {
      return { value: numericTokens[0], method: "unique_back_numeric" };
    }
  }
  const positioned = candidates.filter((candidate) => candidate.positionalEligible);
  positioned.sort((left, right) => right.area - left.area || right.confidence - left.confidence);
  if (positioned[0]) return { value: positioned[0].token, method: "positioned_full_image_numeric" };
  return null;
}
...
export function normalizePaddleOcrResponse(payload = {}, requestInput = {}, {
  ...
  if (["collector_number", "card_code_crop", "collector_number_crop"].includes(normalizeOcrCropType(request.crop_type))
    ...
    const fallbackCollectorNumber = fullImageCollectorNumberDecision(textCandidates, request.metadata || {});
    if (fallbackCollectorNumber) {
      extractedFields.collector_number = fallbackCollectorNumber.value;
      normalizedFieldSources.collector_number = fallbackCollectorNumber.method;
    }
  }
  ...
  return {
    ...
    normalized_field_sources: normalizedFieldSources,
