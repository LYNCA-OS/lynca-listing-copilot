...
// Canonical allowlist for downstream diagnostics and persistence. Consumers
// must derive field-name telemetry from this contract instead of accepting
// arbitrary worker-provided object keys.
export const ocrSupportedNormalizedFields = Object.freeze([
  ...new Set(Object.values(normalizedFieldsByCropType).flatMap((fields) => [...fields]))
].sort());
