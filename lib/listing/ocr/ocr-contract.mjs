...
const normalizedFieldsByCropType = Object.freeze({
  ...
  product_text: new Set(["product_text"]),
...
function candidateGradeFields(textCandidates = [], rawText = "", cropType = "") {
  if (normalizeOcrCropType(cropType) !== "grade_label") return { fields: {}, confidence_by_field: {} };
  ...
  const company = extractGradeFields(joined).grade_company
    || inferredPsaCompanyFromGradeLabel(joined, textCandidates);
  ...
  const companyConfidence = Math.max(0, ...textCandidates
    .filter((candidate) => {
