    ...
    for (let attempt = 1; attempt <= config.request_max_attempts; attempt += 1) {
      ...
      try {
        ...
        const batchTelemetry = {
          batch_request_count: optionalFiniteNumber(payload.request_count),
          batch_unique_image_download_count: optionalFiniteNumber(payload.unique_image_download_count),
          batch_decode_count: optionalFiniteNumber(payload.decode_count),
          batch_vision_unit_count: optionalFiniteNumber(payload.vision_unit_count),
          batch_vision_cost_estimate: optionalFiniteNumber(payload.vision_cost_estimate),
          batch_latency_ms: optionalFiniteNumber(payload.latency_ms),
          batch_vision_http_attempt_count: optionalFiniteNumber(payload.vision_http_attempt_count),
          batch_google_annotate_request_count: optionalFiniteNumber(payload.google_annotate_request_count),
          batch_attempted_vision_unit_count: optionalFiniteNumber(payload.attempted_vision_unit_count),
          batch_confirmed_vision_unit_count: optionalFiniteNumber(payload.confirmed_vision_unit_count),
          batch_billing_unknown: optionalBoolean(payload.billing_unknown),
          batch_auth_mode: payload.auth_mode ? String(payload.auth_mode) : null
        };
