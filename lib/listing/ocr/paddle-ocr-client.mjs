...
export function paddleOcrConfig(env = process.env) {
  // The unified Recognition Worker owns production OCR. Legacy Paddle/OCR
  // aliases remain fallbacks for isolated A/B workers, but cannot silently
  // override the canonical chain target when both are configured.
  const urls = splitUrls(env.RECOGNITION_WORKER_URLS || env.PADDLE_OCR_WORKER_URLS || env.PADDLEOCR_WORKER_URLS || env.OCR_WORKER_URLS);
  const url = normalizeText(env.RECOGNITION_WORKER_URL || env.PADDLE_OCR_WORKER_URL || env.PADDLEOCR_WORKER_URL || env.OCR_WORKER_URL);
    ...
    for (let attempt = 1; attempt <= config.request_max_attempts; attempt += 1) {
      ...
      try {
        ...
        return payload.results.map((item, index) => {
          ...
          return {
            ...normalized,
            ocr_backend: normalizeText(item.ocr_backend || item.backend || item.vision_provider || item.provider) || null,
            inline_full_image_fallback_evaluated: item.inline_full_image_fallback_evaluated === true,
            inline_full_image_fallback_used: item.inline_full_image_fallback_used === true,
            inline_full_image_fallback_target_found: item.inline_full_image_fallback_target_found === true,
            inline_full_image_fallback_status: normalizeText(item.inline_full_image_fallback_status) || null,
