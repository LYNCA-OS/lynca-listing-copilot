...
export function paddleOcrConfig(env = process.env) {
  const urls = splitUrls(env.PADDLE_OCR_WORKER_URLS || env.PADDLEOCR_WORKER_URLS || env.OCR_WORKER_URLS);
  const url = normalizeText(env.PADDLE_OCR_WORKER_URL || env.PADDLEOCR_WORKER_URL || env.OCR_WORKER_URL);
  const workerUrls = urls.length ? urls : (url ? [url] : []);
  const token = normalizeText(env.PADDLE_OCR_WORKER_TOKEN || env.PADDLE_OCR_API_KEY || env.PADDLEOCR_API_KEY || env.OCR_WORKER_TOKEN || env.OCR_WORKER_API_KEY);
