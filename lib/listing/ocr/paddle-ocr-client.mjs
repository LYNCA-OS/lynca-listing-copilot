...
export function paddleOcrConfig(env = process.env) {
  ...
  return {
    ...
    worker_revision: normalizeText(env.OCR_WORKER_REVISION),
    reason: !enabled ? "feature_disabled" : !workerUrls.length ? "paddle_ocr_worker_url_missing" : ""
  };
}

function verifiedWorkerRevision(payload = {}, config = {}) {
  const actualRevision = normalizeText(payload.service_revision || payload.worker_revision);
  if (config.worker_revision && actualRevision !== config.worker_revision) {
    throw new PaddleOcrClientError("OCR worker revision does not match the expected immutable Cloud Run release.", {
      code: "ocr_worker_revision_mismatch",
      status: 503,
      retryable: true,
      details: {
        expected_revision: config.worker_revision,
        actual_revision: actualRevision || null
      }
    });
  }
  return actualRevision || null;
}
  ...
  async function verifyCrop(input = {}) {
    ...
    for (let attempt = 1; attempt <= config.request_max_attempts; attempt += 1) {
      ...
      try {
        ...
        const workerServiceRevision = verifiedWorkerRevision(payload, config);
        ...
        return {
          ...normalized,
          worker_service_revision: workerServiceRevision,
