    ...
    for (let attempt = 1; attempt <= config.request_max_attempts; attempt += 1) {
      ...
      try {
        ...
        return {
          ...
          primary_ocr_latency_ms: Number.isFinite(Number(payload.primary_ocr_latency_ms))
            ? Number(payload.primary_ocr_latency_ms)
            : null,
          fallback_ocr_latency_ms: Number.isFinite(Number(payload.fallback_ocr_latency_ms))
            ? Number(payload.fallback_ocr_latency_ms)
            : null,
