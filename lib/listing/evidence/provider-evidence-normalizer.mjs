...
import { sourceIdentityForVerifiedImage } from "./current-image-manifest.mjs";

export const providerEvidenceNormalizationVersion = "provider-evidence-normalization-v2-current-image-source-binding";
...
function structuredSource(entry = {}, {
  ...
  const currentImageIdentity = sourceIdentityForVerifiedImage(images, image);
...
function expandStructuredFieldEvidence(fieldEvidence = {}) {
  ...
  if (Array.isArray(fieldEvidence)) {
    for (const entry of fieldEvidence) {
      if (!entry || typeof entry !== "object" || Array.isArray(entry)) continue;
      const fieldName = normalizeText(entry.field || entry.field_name || entry.f);
      if (!fieldName) continue;
      const previous = expanded[fieldName];
      if (!previous || clampConfidence(entry.confidence, 0) >= clampConfidence(previous.confidence, 0)) {
  ...
  Object.entries(Array.isArray(fieldEvidence) ? expanded : fieldEvidence).forEach(([fieldName, entry]) => {
...
const reviewOnlyCodeProposalFields = Object.freeze([
  "card_number",
  "tcg_card_number"
]);

// The production normalizer intentionally removes model-only printed codes
// from resolved truth. A bounded evaluation second look still needs to replay
// that rejected value through Resolver as REVIEW evidence so we can measure
// whether the extra observation is useful. This opt-in projection never marks
// the model claim direct and is disabled for every production/default caller.
function reviewOnlyRejectedCodeProposals(fieldEvidence = {}, {
  images = [],
  fallbackConfidence = 0.5,
  resolved = {}
} = {}) {
  ...
  const evidence = {};
  const proposals = {};
  for (const fieldName of reviewOnlyCodeProposalFields) {
    const entry = expanded[fieldName];
    const value = structuredEntryValue(fieldName, entry);
    if (value === undefined || value === null || value === "") continue;
    const field = structuredEvidenceField(fieldName, value, {
      ...entry,
      review_required: true,
      directly_observed: false,
      direct_observation: false,
      unresolved_reason: entry?.unresolved_reason || "model_only_code_proposal_requires_resolver_review"
    }, {
      images,
      fallbackConfidence,
      resolved: { ...resolved, [fieldName]: value }
    });
    if (!field) continue;
    evidence[fieldName] = field;
...
export function providerPayloadToEvidenceDocument(payload = {}, {
  images = [],
  preserveReviewOnlyCodeProposals = false
} = {}) {
  ...
  const rejectedCodeProposals = preserveReviewOnlyCodeProposals === true
    ? reviewOnlyRejectedCodeProposals(payload.field_evidence, {
        resolved,
        images,
        fallbackConfidence: confidenceForPayload(payload)
      })
  ...
  Object.entries(rejectedCodeProposals.evidence).forEach(([fieldName, field]) => {
    if (!evidence[fieldName]) evidence[fieldName] = field;
  });
  ...
  const document = {
    ...
    resolved: {
      ...resolved,
      ...rejectedCodeProposals.proposals
    },
    unresolved,
