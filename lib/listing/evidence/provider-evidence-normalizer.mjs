...
function structuredEntryValue(fieldName, entry = {}) {
  ...
  if (["grade_company", "card_grade", "auto_grade", "grade_type"].includes(fieldName)) {
    ...
    if (fieldName === "card_grade") return nonEmptyEvidenceValue(entry.card_grade) ?? nonEmptyEvidenceValue(entry.grade) ?? inferredGrade.card_grade ?? explicitValue;
    if (fieldName === "auto_grade") return nonEmptyEvidenceValue(entry.auto_grade) ?? nonEmptyEvidenceValue(entry.autograph_grade) ?? inferredGrade.auto_grade ?? explicitValue;
    if (fieldName === "grade_type") return nonEmptyEvidenceValue(entry.grade_type) ?? nonEmptyEvidenceValue(entry.type) ?? inferredGrade.grade_type ?? explicitValue;
  }
