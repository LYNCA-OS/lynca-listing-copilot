...
function structuredEvidenceField(fieldName, value, entry = {}, {
  ...
  const confidence = clampConfidence(entry.confidence, fallbackConfidence);
  const source = structuredSource(entry, { fieldName, images });
  return createEvidenceField({
    value,
    normalizedValue: value,
    status: structuredStatusFor(fieldName, entry, confidence),
    confidence,
    candidates: [{
      value,
      confidence,
    ...
    unresolvedReason: entry.unresolved_reason || (entry.review_required === true ? "structured_field_requires_review" : null)
  });
}
