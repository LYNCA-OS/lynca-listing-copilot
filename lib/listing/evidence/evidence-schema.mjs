...
} from "../pipeline/text.mjs";
import { collapseDescriptorExtendedSubjectIdentities } from "../pipeline/subject-identity.mjs";

export const evidenceSchemaVersion = "evidence-fields-v1";
// Public EvidenceDocument remains v1. This owner version tracks changes to
// the schema owner's normalization semantics so result-cache fingerprints
// cannot replay fields produced before a newly supported input alias existed.
export const evidenceSchemaOwnerVersion = "evidence-schema-owner-v3-current-image-source-binding";
...
function samePersonWithOptionalMiddleName(left, right) {
  ...
  if (leftTokens.length === rightTokens.length
    && leftTokens.slice(0, -1).every((token, index) => token === rightTokens[index])) {
    const leftLast = leftTokens.at(-1);
    const rightLast = rightTokens.at(-1);
    const shorterLast = leftLast.length <= rightLast.length ? leftLast : rightLast;
    const longerLast = shorterLast === leftLast ? rightLast : leftLast;
    if (shorterLast.length >= 4
      && longerLast.length - shorterLast.length <= 2
      && longerLast.startsWith(shorterLast)) return true;
  }
...
function normalizePlayerArray(value) {
  ...
  return collapseDescriptorExtendedSubjectIdentities(normalized);
}
...
export function createVisionSource({
  ...
  // A caller may describe where an observation came from, but only the
  // server-side Evidence Normalizer may attach the verified current-image
  // identity envelope. Consumers must require this envelope before allowing
  // visual evidence to affect candidate application.
  const identity = currentImageIdentity
    && typeof currentImageIdentity === "object"
    && !Array.isArray(currentImageIdentity)
    ? currentImageIdentity
    : {};
  return {
    source_type: evidenceSourceTypes.includes(sourceType) ? sourceType : "VISION_MODEL",
    image_id: identity.derived_image_id || identity.source_image_id || imageId,
    source_image_id: identity.source_image_id || null,
    tenant_id: identity.tenant_id || null,
    asset_id: identity.asset_id || null,
    image_generation_id: identity.image_generation_id || null,
    side,
    capture_role: captureRole,
    region,
    source_crop_id: identity.source_crop_id || sourceCropId,
    source_inference_method: sourceInferenceMethod,
    source_object_path: identity.source_object_path || sourceObjectPath,
    source_content_sha256: identity.source_content_sha256 || null,
    derived_image_id: identity.derived_image_id || null,
    derived_object_path: identity.derived_object_path || derivedObjectPath,
    derived_content_sha256: identity.derived_content_sha256 || null,
    crop_lineage: identity.crop_lineage || null,
    current_image_manifest_fingerprint: identity.current_image_manifest_fingerprint || null,
...
const serialNumeratorDirectSourceTypes = Object.freeze(new Set([
  ...
  "INTERNAL_APPROVED_HISTORY",
  "OFFICIAL_GRADING_DATA",
  // The vision provider reading the serial off the current images IS a
  // current-image observation. Across two sealed reviewed-50 runs every
  // CONFIRMED vision-read numerator matched the reviewed ground truth, while
  // the previous vision-excluded gate stripped 17 of 19 correct serials to a
  // bare #/denominator. Hallucination protection remains layered: CONFIRMED
  // status, the tri-state OCR conflict veto (an OCR observation of a
  // DIFFERENT numerator still suppresses), suspicious_print_run, and the
  // unverified 1/1 special case are all unchanged.
  "VISION_MODEL"
]));
...
export function normalizeResolvedFields(input = {}) {
  ...
  // Retrieval packets use `subjects`, while provider/SEM payloads use
  // `players`. Both names describe the same canonical identity field; losing
  // the retrieval alias here silently removed every catalog subject before
  // Candidate Application and forced exact identities to fall through.
  const players = normalizePlayerArray(raw.players ?? raw.player ?? raw.subjects ?? raw.subject);
  ...
  const rawLotType = normalizeStringOrNull(raw.lot_type ?? raw.lotType);
  const multiCard = cardCount === 1
    ? false
    : normalizeBoolean(raw.multi_card ?? raw.multiCard) || Number(cardCount || 0) > 1;
  const officialCardType = normalizeStringOrNull(raw.official_card_type || raw.officialCardType);
  const normalizedSet = sanitizeIdentitySetValue(raw.set || raw.set_or_insert);
  const normalizedCardName = sanitizeIdentityCardNameValue(raw.card_name || raw.cardName || raw.name);
  const normalizedRarity = normalizeStringOrNull(raw.rarity);
  const normalizedAutoGrade = normalizeAutoGradeValue(raw.auto_grade);
  const normalizedGradeType = gradeTypeForValues(raw.card_grade || raw.grade, raw.auto_grade, raw.grade_type);
  const explicitRookieCardName = /^(?:Rookie|Rookie Card)$/i.test(normalizedCardName || "");
  const autographGradeSemantics = Boolean(normalizedAutoGrade)
    || normalizedGradeType === "AUTO_ONLY"
    || normalizedGradeType === "CARD_AND_AUTO";
  const firstBowmanLiteral = [normalizedSet, normalizedCardName, raw.subset]
    .some((value) => /\b1st\s+Bowman\b/i.test(String(value || "")));

  const explicitProduct = normalizeProductName(raw.product, raw.manufacturer || raw.brand, raw.brand);
  const narratedProduct = narratedProductIdentityFromSet(raw.set || raw.set_or_insert);
  const product = narratedProduct && /^(?:Topps|Bowman|Panini|Upper\s+Deck|Donruss|Leaf)$/i.test(explicitProduct || "")
    ? normalizeProductName(narratedProduct, raw.manufacturer || raw.brand, raw.brand)
    : explicitProduct;
  const normalized = {
    ...
    attributes,
    rc: normalizeBoolean(raw.rc) || observableComponents.includes("rc") || explicitRookieCardName,
    first_bowman: normalizeBoolean(raw.first_bowman) || firstBowmanLiteral,
    ssp: normalizeBoolean(raw.ssp) || /^SSP$/i.test(normalizedRarity || ""),
    case_hit: normalizeBoolean(raw.case_hit),
    auto: normalizeBoolean(raw.auto) || observableComponents.includes("auto") || autographGradeSemantics,
    ...
    grade_type: normalizedGradeType
  };
  return normalized;
}
