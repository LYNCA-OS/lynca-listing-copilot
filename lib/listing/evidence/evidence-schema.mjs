...
import { normalizeSemNumberBoundaries } from "../csm/sem-definition.mjs";
...
function normalizeGradeCompany(value) {
  ...
  if (/\b(?:ccic|gtbc|bgn)\b/i.test(normalized)) return normalized.toUpperCase();
...
export function normalizeResolvedFields(input = {}) {
  ...
  const normalized = {
    ...
    year: normalizeStringOrNull(raw.year || raw.season_year || raw.product_year),
  ...
  return normalizeSemNumberBoundaries(normalized);
}
