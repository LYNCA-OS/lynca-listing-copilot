...
const serialNumeratorDirectSourceTypes = Object.freeze(new Set([
  ...
  "VISION_MODEL",
  // Provider prompts use VISION_ONLY for literal current-card reads that do
  // not have a narrower OCR/slab source. It is the same current instance, not
  // a vector/catalog reference, so a CONFIRMED numerator is safe to render.
  "VISION_ONLY"
]));
