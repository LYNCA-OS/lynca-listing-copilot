...
export function renderListingPresentation({
  ...
  const normalized = normalizeResolvedFields(resolved);
  // The numerator identifies the physical copy, so it only renders when the
  // current serial evidence has direct current-instance provenance (or the
  // caller proved verification, e.g. a focused-crop re-read in the gate).
