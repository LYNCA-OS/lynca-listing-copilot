...
function renderLotTitle(resolved = {}, {
  ...
  const items = [
    ...
    // MULTI_SUBJECT_REVIEW is still one physical card, so its current-image
    // print run belongs to the whole item. A true lot intentionally keeps this
    // field suppressed because a serial can belong to only one card in it.
    !includeQuantity
      ? {
        key: "numerical_rarity",
        text: resolved.numerical_rarity || resolved.print_run_number,
        priority: 5,
        required: Boolean(resolved.numerical_rarity || resolved.print_run_number),
        compactable: false
      }
      : null,
...
export function renderListingPresentation({
  ...
  const safeUnverifiedDenominator = serialNumeratorVerified === false
    ...
    : "";
  // Absence of print-run evidence is not evidence against the print run. The
  // provider states the numbering in fields and, on the fixed-20 benchmark,
  // supplied no field_evidence entry for it on every card that carried one --
  // so the "no confirmed evidence" branch suppressed the whole field and the
  // serial hit rate was 0/41 even though 9 of 14 numbered cards already held
  // the correct value (81% exact across repeats, the rest numerator-off-by-one
  // with the right denominator).
  //
  // Distinguish the two cases: an evidence entry that exists but was not
  // confirmed is real doubt and still suppresses, while a missing entry falls
  // back to the resolved value. The writer confirms every title before it is
  // listed, so a numbering they can check against the card in hand is worth
  // more than a blank they must type from scratch.
  const resolvedPrintRunUncontradicted = !printRunEvidence
    && printRunFields.print_run_number
    && printRunFields.suspicious_print_run !== true
    && serialNumeratorVerified !== false;
  const printRunDisplay = !unverifiedOneOfOne
    && ((!hasEvidence && trustResolvedPrintRunWithoutEvidence)
      || resolvedPrintRunUncontradicted
      || (confirmedPrintRunEvidence && (sourcedPrintRunEvidence || directPrintRunProvenance)))
    ? printRunTitleText(printRunFields, {
      directCurrentInstance: directPrintRunProvenance
        || resolvedPrintRunUncontradicted
        || (!hasEvidence && trustResolvedPrintRunWithoutEvidence)
    })
    : safeUnverifiedDenominator;
  const printRunSuppressedResolved = (unverifiedOneOfOne || (!resolvedPrintRunUncontradicted && (hasEvidence || !trustResolvedPrintRunWithoutEvidence) && (!confirmedPrintRunEvidence || (!sourcedPrintRunEvidence && !directPrintRunProvenance)))) && (printRunFields.print_run_number || printRunFields.print_run_denominator)
    ? {
      ...grammarAdjusted,
