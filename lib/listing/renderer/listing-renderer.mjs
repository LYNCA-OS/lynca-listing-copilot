...
export function renderListingPresentation({
  ...
  // REVIEW), backfill the denominator-only print run (#/D). Deterministic and
  // numerator-free, so a model that forgets the field cannot silently drop
  // the print limit from the title.
  const serialEvidence = evidence?.serial_number;
  const backfilledNumericalRarity = !hasPresentationValue(normalized.numerical_rarity)
    && hasPresentationValue(normalized.serial_number)
    && serialNumeratorDirectProvenance(serialEvidence)
    ? serialDenominatorOnlyText(normalized.serial_number)
    : "";
  const presentationResolved = backfilledNumericalRarity
    ? { ...normalized, numerical_rarity: backfilledNumericalRarity }
    : normalized;
