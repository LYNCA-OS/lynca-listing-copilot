...
function autographSemanticsAdjusted(resolved = {}) {
  ...
  return {
    ...
    ...(genericSignedOnly ? {
      observable_components: components.filter((item) => item !== "auto")
    } : {}),
    // "Signed image" is descriptive narration for a visible signature shape,
    // not an official autograph identity. A printed facsimile can cause the
    // model to also emit observable_components:auto, so the narration must
    // win the fail-closed check unless official Auto/Autograph wording exists.
    auto: genericSignedOnly
      ? false
      : observableAuto || officialAutoName
        ? true
        : resolved.auto,
    card_name: genericSignedOnly ? null : resolved.card_name
  };
}
