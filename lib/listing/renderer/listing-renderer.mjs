...
export function renderListingPresentation({
  ...
  const printRunFields = expandPrintRunFields(gradeSanitized);
  const printRunDisplay = (!hasEvidence && trustResolvedPrintRunWithoutEvidence) || (confirmedPrintRunEvidence && (sourcedPrintRunEvidence || directPrintRunProvenance))
    ...
    : "";
  const printRunSuppressedResolved = (hasEvidence || !trustResolvedPrintRunWithoutEvidence) && (!confirmedPrintRunEvidence || (!sourcedPrintRunEvidence && !directPrintRunProvenance)) && (printRunFields.print_run_number || printRunFields.print_run_denominator)
    ? {
      ...gradeSanitized,
