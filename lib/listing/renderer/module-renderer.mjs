...
function releaseVariantTokenParts(resolved = {}) {
  const variation = semReleaseVariantText(stripProductConfigurationTerms(resolved.variation));
  return variation ? [{ text: variation, fields: ["variation"] }] : [];
}

function printFinishTokenParts(resolved = {}) {
  const exact = stripProductConfigurationTerms(resolved.parallel_exact);
  // parallel_exact normally wins, because it is the manufacturer's own wording.
  // But it is a naming claim, while surface_color is something read off the
  // card, and when the two disagree the read wins.
  //
  // Measured on 2,090 production cards: of the 430 that lost their colour from
  // the title, 25 had a parallel_exact naming a different colour than the one
  // observed -- "Blue Cracked Ice" on a card read as Green -- and every one of
  // those lost the colour. Among the 1,660 that kept it, that contradiction
  // occurs zero times. Emitting neither is the worst of the three options: the
  // colour was actually seen.
  const observedColor = safeSurfaceColor(resolved.surface_color);
  const exactContradictsObservation = Boolean(exact) && Boolean(observedColor)
    && !new RegExp(`\\b${observedColor.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "i").test(exact);
  if (exact && !exactContradictsObservation) return [{ text: exact, fields: ["parallel_exact"] }];
  const parts = [];
...
function releaseVariantText(resolved) {
  ...
  [
    ...
    resolved.subset && !/^(?:RC|Rookie|Rookie Card|Rated Rookie)$/i.test(resolved.subset) ? resolved.subset : null
...
function releaseVariantModuleTokenParts(resolved) {
  ...
  return [
    ...
    {
      text: resolved.subset && !/^(?:RC|Rookie|Rookie Card|Rated Rookie)$/i.test(resolved.subset || "") ? resolved.subset : null,
      fields: ["subset"]
    }
  ];
}
...
function cardNumberText(resolved) {
  return [
    resolved.collector_number ? `#${displayCardNumber(resolved.collector_number, resolved)}` : null,
...
function cardNumberTokenParts(resolved) {
  return [
    {
      text: resolved.collector_number ? `#${displayCardNumber(resolved.collector_number, resolved)}` : null,
...
const semText = (value) => String(value ?? "").toLowerCase().replace(/[^a-z0-9]+/g, " ").replace(/\s+/g, " ").trim();

// SEM orders the modules, and that order is also the answer to where a value
// belongs when the provider puts it in more than one. A card that is
// "2025 Panini Donruss Rated Throwback Travis Hunter" rendered as
// "2025 Panini Donruss Throwback Travis Hunter" because "Throwback" arrived in
// both `set` and `card_name`: `set` renders inside product_identity, which
// precedes the subject, so the card name overtook the player. Reviewed titles
// put the duplicated value after the subject on 4 of 5 cards where this occurs.
//
// The exception is the fifth, and it is why the rule cannot simply be "the
// later module wins": in "2018-19 Panini Prizm Jalen Brunson RC Prizm Mojo",
// "Prizm" genuinely is part of the product hierarchy AND names the finish. What
// distinguishes the two is whether the product line already carries the word.
//
// So: a value duplicated across modules is dropped from card_name only when the
// product hierarchy already states it; otherwise it is dropped from `set`,
// which is where it did not belong. Either way it appears once, at its SEM
// position.
export function resolveSemModuleOwnership(fields = {}) {
  const set = semText(fields.set);
  const cardName = semText(fields.card_name);
  if (!set || !cardName || set !== cardName) return fields;

  const hierarchy = [fields.manufacturer, fields.brand, fields.product]
    .map(semText).filter(Boolean).join(" ");
  const statedByProduct = hierarchy
    ? new RegExp(`(^|\\s)${set.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(\\s|$)`).test(hierarchy)
    : false;

  return statedByProduct
...
export function renderListingModules({
  ...
  const normalized = resolveSemModuleOwnership(normalizeResolvedFields(resolved));
  const modules = {
    year: buildModule({
      key: "year",
