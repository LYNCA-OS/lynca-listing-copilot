...
function tcgIpText(resolved = {}) {
  ...
  if (/\blorcana\b/.test(text)) return "Disney Lorcana";
  if (/\bdigimon\b/.test(text)) return "Digimon";
  if (/\bmagic\b|\bmtg\b/.test(text)) return "Magic: The Gathering";
  if (/\bweiss schwarz\b/.test(text)) return "Weiss Schwarz";
  if (/\bunion arena\b/.test(text)) return "Union Arena";
  if (/\bbattle spirits\b/.test(text)) return "Battle Spirits";
...
function tcgCardNumberText(resolved = {}) {
  const tcgCardNumber = normalizeText(resolved.tcg_card_number).replace(/^#/, "");
  const cardNumber = normalizeText(resolved.card_number).replace(/^#/, "");
  ...
  return tcgCardNumber || cardNumber || collector || checklist || "";
}
...
function pokemonItems(resolved) {
  ...
  return [
    ...
    { key: "language", text: language, priority: 8, required: Boolean(language), compactable: false },
    ...
    { key: "subject", text: subject, priority: 4, required: Boolean(subject), compactable: true },
    { key: "card_name", text: resolved.card_name, priority: 7, compactable: true },
    { key: "additional_info", text: cardNumber, priority: 7, required: Boolean(cardNumber), compactable: true },
