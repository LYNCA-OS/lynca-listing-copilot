...
function normalizeCardTypeText(value) {
  ...
  const namedConstruction = text.match(/^(.+?)\s+(?:Triple\s+|Dual\s+|Quad\s+)?(?:Auto|Autograph|Autographs|Autographed|Signature|Signatures|Relic|Jersey|Patch|Memorabilia)(?:\s+(?:Auto|Autograph|Autographs|Autographed|Signature|Signatures|Relic|Jersey|Patch|Memorabilia|Card))*$/i);
...
function titleItems(resolved) {
  ...
  const delayedAttributeCardTypes = cardTypes.filter((item) => /^(?:Auto|Patch|Relic|Auto Relic)$/i.test(normalizeText(item.text)));
  const leadingCardTypes = cardTypes.filter((item) => !delayedAttributeCardTypes.includes(item));
