...
import { semReleaseVariantText } from "../csm/sem-definition.mjs";
...
function stripProductConfigurationTerms(value = "") {
  return titleCleanup(normalizeText(value)
    ...
    .replace(/\b(?:Edition|Ed\.?)[\s]*[-:][\s]*(?=(?:Aqua|Gold|Green|Red|Blue|Purple|Orange|Black|Silver)\b)/gi, " ")
    .replace(productConfigurationPattern, " "));
}

function stripExplicitUncertainty(value = "") {
  let text = normalizeText(value)
    .replace(/\([^)]*\b(?:visible\s+partial|partial\s+wording|uncertain|unclear|illegible|possibly|likely)\b[^)]*\)/gi, " ");
  // A token ending in `?` is a model uncertainty marker, not publishable card
  // identity. Preserve the remaining directly observed words and let V4 mark
  // the source field for review.
  text = text.replace(/\b[A-Za-z0-9][A-Za-z0-9'&./-]*\?(?=\s|$)/g, " ");
  return titleCleanup(text);
}
...
function textHasNamedCardComponent(value = "") {
  return /\b(?:Auto|Autograph|Autographs|Signature|Signatures|Signed|Patch|Relic|Jersey|Memorabilia|Material|Materials|Ticket|Dynastic|Deed|Spotlight|Downtown|Kaboom|Booklet|Logoman|Shield|Laundry\s+Tag|Rookie|College|Survivor|Series|Chrome|Refractor)\b/i.test(normalizeText(value));
}

function stripSubjectPrefixFromCardName(cardName = "", subject = "") {
  let text = titleCleanup(cardName);
  const subjectText = titleCleanup(subject);
  if (!text || !subjectText) return text;

  const escapedSubject = subjectText.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const fullSubjectPrefix = text.match(new RegExp(`^${escapedSubject}\\b\\s*(.+)$`, "i"));
  if (fullSubjectPrefix && textHasNamedCardComponent(fullSubjectPrefix[1])) {
    return titleCleanup(fullSubjectPrefix[1]);
  }

  const subjectTokens = subjectText.split(/\s+/).filter(Boolean);
  const lastSubjectToken = subjectTokens.length > 1 ? subjectTokens.at(-1) : "";
  if (!lastSubjectToken || lastSubjectToken.length < 4) return text;

  const escapedLast = lastSubjectToken.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const lastNamePrefix = text.match(new RegExp(`^${escapedLast}\\b\\s*(.+)$`, "i"));
  if (lastNamePrefix && textHasNamedCardComponent(lastNamePrefix[1])) {
    return titleCleanup(lastNamePrefix[1]);
  }

  return text;
}

function normalizeCodeLeakedCardName(value = "") {
  let text = titleCleanup(value)
    .replace(/-(?=(?:Auto|Autograph|Autographs|Signature|Signatures|Patch|Relic|Jersey|Memorabilia)\b)/gi, " ")
    .replace(/\bBWM\s+PROS\s+MEGA\b/gi, " ")
    .replace(/\bPROS\s+MEGA\b/gi, " ")
    .replace(/\bAU[-\s]*(Black|Blue|Gold|Green|Orange|Pink|Purple|Red|Silver|White|Yellow)\b/gi, "Auto $1")
    .replace(/\bSIG[-\s]*(Black|Blue|Gold|Green|Orange|Pink|Purple|Red|Silver|White|Yellow)\b/gi, "Signatures $1");
  text = titleCleanup(text);
  return /^(?:BWM|PROS|MEGA|AU|SIG)$/i.test(text) ? "" : text;
}

function releaseVariantText(resolved = {}) {
  return semReleaseVariantText(stripProductConfigurationTerms(resolved.variation));
}

function productFinishText(resolved = {}) {
  const exact = stripProductConfigurationTerms(resolved.parallel_exact);
  if (exact) {
    // Topps calls the release pattern Mega Chrome; Bowman Mega marketplace
    // titles conventionally call the same finish Mojo Refractor. Keep the
    // catalog value canonical and translate only at presentation time.
    if (/^Mega Chrome$/i.test(exact) && /Bowman Mega Box/i.test(normalizeText(resolved.product))) {
      return "Mojo Refractor";
    }
    return exact;
  }
  ...
  if (legacyParallel
    && !semReleaseVariantText(legacyParallel)
    && !looksLikeOpticalParallel(legacyParallel)) pushUniquePhrase(parts, legacyParallel);
...
function printFinishText(resolved = {}) {
  ...
  const family = normalizeText(resolved.parallel_family);
  if (finish && family) {
    const finishTokens = new Set(finish.toLowerCase().split(/\s+/).filter(Boolean));
    const missingFamilyTokens = family.split(/\s+/).filter((token) => !finishTokens.has(token.toLowerCase()));
    if (missingFamilyTokens.length) return titleCleanup(`${finish} ${missingFamilyTokens.join(" ")}`);
  }
...
function variantItems(resolved, identityText = "") {
  const seen = [];
  const rawInsertText = stripExplicitUncertainty(resolved.insert) || legacyNamedInsertFromCardType(resolved.card_type);
  const subsetText = stripExplicitUncertainty(resolved.subset);
  ...
  const displayReleaseText = releaseText;
  ...
  const parts = [
    ...
    {
      field: "subset",
      text: subsetText && !/^(?:RC|Rookie|Rookie Card|1st Bowman)$/i.test(subsetText)
        ? subsetText
        : null,
      priority: 30
    }
...
function searchOptimizationItems(resolved, existingText = "") {
  ...
  const existingHasPatchLike = /\b(?:Patch|Swatch|Material|Materials|Memorabilia|Relic)\b/i.test(existing);
  ...
  // "Signatures" is often the official insert name while "Auto" remains a
  // separate marketplace attribute. Preserve both; only an explicit Auto or
  // Autograph token already satisfies the attribute.
  if ((resolved.auto || materialSignature) && !/\b(?:Auto|Autograph)s?\b/i.test(existing)) parts.push("Auto");
  if ((resolved.patch || materialSignature) && !existingHasPatchLike) parts.push("Patch");
...
function cardNameText(resolved = {}, subject = "") {
  let text = normalizeCodeLeakedCardName(stripExplicitUncertainty(resolved.card_name));
  text = stripSubjectPrefixFromCardName(text, subject);
  ...
  if (/^(?:Certified\s+)?Auto(?:\s+(?:Issue|Card))?$/i.test(text)) return "Auto";
  const color = safeSurfaceColor(resolved.surface_color);
  if (color) {
...
function compactCardNameAgainstProduct(cardName = "", {
  productIdentity = "",
  setText = ""
} = {}) {
  let text = titleCleanup(cardName);
  ...
  const removeOverlappingPrefix = (prefix = "") => {
    const prefixWords = normalizeText(prefix).split(/\s+/).filter(Boolean);
    let words = normalizeText(text).split(/\s+/).filter(Boolean);
    if (!prefixWords.length || !words.length) return;
    while (prefixWords.length && words.length) {
      const max = Math.min(prefixWords.length, words.length);
      let removed = false;
      for (let size = max; size >= 2; size -= 1) {
        const prefixTail = prefixWords.slice(prefixWords.length - size).join(" ");
        const textHead = words.slice(0, size).join(" ");
        if (normalizeText(prefixTail).toLowerCase() !== normalizeText(textHead).toLowerCase()) continue;
    ...
    text = titleCleanup(words.join(" ")) || text;
  };

  removeOverlappingPrefix(productIdentity);
  removeOverlappingPrefix(setText);
  text = titleCleanup(text
    .replace(/\bProspects?\s+Auto\b/gi, "Auto")
    .replace(/\bProspects?\s+Autographs?\b/gi, "Auto")
    .replace(/\bPrizms\b/gi, "Prizm"));
...
function productCoreText(resolved = {}) {
  ...
  let product = titleCleanup(normalizeText(resolved.product)
    .replace(new RegExp(`^${escapedYear}\\s+`, "i"), "")
    .replace(new RegExp(`\\s+${escapedYear}$`, "i"), ""));
  ...
  const manufacturerComparable = normalizeText(manufacturer).toLowerCase();
  ...
  if (manufacturerComparable && (productComparable === manufacturerComparable || productComparable.startsWith(`${manufacturerComparable} `))) {
    return compactOutputProductIdentity(product);
  }
  ...
  if ((brandText && phraseIncludes(product, brandText)) || (manufacturer && phraseIncludes(product, manufacturer))) {
    return compactOutputProductIdentity(product);
  }
...
function compactSetAgainstCardDesign(setText = "", designParts = []) {
  const set = titleCleanup(setText);
  if (!set) return "";

  const coveredTokens = new Set(designParts
    .flatMap((part) => comparableTokens(part))
    .filter((token) => token.length > 1));
  if (!coveredTokens.size) return set;

  const residual = normalizeText(set)
    .replace(/\s+[-–—]\s+/g, " ")
    .split(/\s+/)
    .filter((token) => {
      const comparable = comparableTokens(token)[0] || "";
      return !comparable || !coveredTokens.has(comparable);
    })
    .join(" ");

  return titleCleanup(residual);
}
...
function collectorNumberText(resolved = {}) {
  ...
  // a sports-card title field — reviewed titles omit it and it inflated titles
  // with spurious "#154"/"#306" (sometimes duplicated). Alphabetic insert/
  // parallel codes carry set identity and are kept. Serial numbers render on
  // the dedicated serial_limit path and are unaffected.
  if (/^#?\d+$/.test(collector)) return "";
...
function titleItems(resolved) {
  let productIdentity = productCoreText(resolved);
  const sapphireObserved = [resolved.product, resolved.set, resolved.parallel_exact, resolved.parallel]
    .some((value) => /\bSapphire\b/i.test(normalizeText(value)));
  if (sapphireObserved && !/\bSapphire\b/i.test(productIdentity)) {
    productIdentity = titleCleanup(`${productIdentity} Sapphire`);
  }
  const rawSetText = setTitleText(resolved, productIdentity);
  const subject = subjectText(resolved);
  const team = normalizeText(resolved.team);
  const rawCardName = cardNameText(resolved, subject);
  let cardName = compactCardNameAgainstProduct(rawCardName, { productIdentity, setText: rawSetText });
  if (rawSetText && cardName
    && normalizeText(rawSetText).toLowerCase() === normalizeText(cardName).toLowerCase()) {
    cardName = "";
  }
  const identityText = titleCleanup(productIdentity);
  let variants = variantItems(resolved, titleCleanup(`${identityText} ${cardName}`));
  if (rawSetText) {
    variants = variants.filter((item) => (
      normalizeText(item.text).toLowerCase() !== normalizeText(rawSetText).toLowerCase()
    ));
  }
  const existingVariantText = variants.map((item) => item.text).join(" ");
  let printFinish = printFinishText(resolved);
  if (rawSetText && normalizeText(printFinish).toLowerCase() === normalizeText(rawSetText).toLowerCase()) {
    printFinish = "";
  }
  const parallelFamily = normalizeText(resolved.parallel_family);
  if (
    printFinish
    && cardName
    && parallelFamily
    && phraseIncludes(cardName, parallelFamily)
    && !phraseIncludes(cardName, printFinish)
  ) {
    // Writer/catalog card names such as "Lucky Hyper" are the more specific
    // identity. Do not append an independently observed generic finish such as
    // "Blue Hyper" and manufacture two mutually competing parallel names.
    printFinish = "";
  }
  // Storage keeps the full SEM fields. The output layer removes only tokens
  // already represented by Card Name / Release Variant / Print Finish so a
  // composite Set cannot swallow those higher-value modules during fitting.
  const setText = compactSetAgainstCardDesign(rawSetText, [
    cardName,
    existingVariantText,
    printFinish
  ]);
  ...
  return [
    ...
    { key: "print_finish", text: printFinish, priority: 8, compactable: true },
