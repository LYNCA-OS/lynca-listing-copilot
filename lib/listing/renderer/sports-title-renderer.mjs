...
function searchOptimizationItems(resolved, existingText = "") {
  ...
  if (resolved.rc || materialSignature && /\bRookie\b/i.test(cardName) || /^(?:RC|Rookie|Rookie Card|Rated Rookie)$/i.test(subset) || /Chrome\s+Rookie\s+Auto/i.test(existingText)) parts.push("RC");
...
function titleItems(resolved) {
  ...
  return [
    ...
    { key: "set", text: setText, priority: pureSportSetText(setText) ? 96 : 16, compactable: true },
    ...
    ...rarity,
    { key: "card_number", text: collectorNumber, priority: 98, compactable: false },
    ...searchOptimization,
