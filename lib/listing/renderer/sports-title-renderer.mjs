...
function cardNameText(resolved = {}, subject = "") {
  ...
  if (insert && phraseIncludes(text, insert) && text.toLowerCase().startsWith(insert.toLowerCase())) {
    const suffix = titleCleanup(text.slice(insert.length)
      .replace(/\b(?:Auto|Autograph|Autographs|Autographed|Signature|Signatures|Signed)\b/gi, resolved.auto ? " " : "$&")
      .replace(finishWordPattern, " "));
    const releaseVariant = suffix.match(releaseVariantPattern)?.[0] || "";
    // The structured insert is the Card Name owner. Free-form pose wording
    // emitted beside it is not promoted into Card Name unless it is a defined
    // Release Variant such as Horizontal, Vertical, or Variation.
    text = titleCleanup(`${insert} ${releaseVariant}`);
  }
  const color = safeSurfaceColor(resolved.surface_color);
  if (color) {
...
function titleItems(resolved) {
  ...
  const cardName = compactCardNameAgainstProduct(rawCardName, { productIdentity, setText });
  const identityText = titleCleanup(productIdentity);
  const variants = variantItems(resolved, titleCleanup(`${identityText} ${cardName}`));
