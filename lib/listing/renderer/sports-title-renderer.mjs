...
function compactOutputProductIdentity(value = "") {
  const text = titleCleanup(normalizeText(value)
    .replace(/\bUEFA\s+Club\s+Competitions\b/gi, "UCC")
...
function productFinishText(resolved = {}) {
  ...
  if (legacyParallel
    ...
    && !looksLikeOpticalParallel(legacyParallel, { product: resolved.product })) pushUniquePhrase(parts, legacyParallel);
...
function teamTitleText(value) {
  // Reviewed titles write the franchise plainly ("... 17/50 Spurs"), never
  // parenthesized, and module-renderer already emits the bare value. The
  // parenthesized form was the odd one out.
  return normalizeText(value);
}
...
function titleItems(resolved) {
  ...
  const searchOptimization = searchOptimizationItems(resolved, [
    cardName,
    existingVariantText,
    printFinish,
    ...cardTypes.map((item) => item.text)
  ].filter(Boolean).join(" "));
