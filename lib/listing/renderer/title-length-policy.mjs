...
function compactProduct(item) {
  return {
    ...item,
    text: titleCleanup(item.text)
      .replace(/\bSpecial\s+PR\s+Pack\b/gi, "Special PR")
      .replace(/\bVol\.?\s*(\d+)\s*[-:]\s*/gi, "Vol.$1 ")
...
function compactTcgCardNumberItems(items, maxLength, trace) {
  if (renderItems(items).length <= maxLength) return items;
  return items.map((item) => {
    if (item.key !== "additional_info" || !item.compactable) return item;
    const match = String(item.text || "").match(/^#?(\d{1,5})\s*\/\s*\d{1,5}$/);
    if (!match) return item;
    const compacted = { ...item, text: `#${Number(match[1])}` };
...
export function fitTitleItems(items, {
  ...
  current = compactTcgCardNumberItems(current, maxLength, trace);
  ...
  if (title.length > maxLength) {
    ...
    title = requiredFit.title;
    current = requiredFit.items;
  }
