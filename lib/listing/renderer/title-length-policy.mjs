  ...
  items.forEach((item) => {
    ...
    const existingIndex = deduped.findIndex((existing) => {
      ...
      const gradingAttributePair = [existing.key, item.key].includes("grading")
        && [existing.key, item.key].some((key) => key === "attributes" || key === "rookie_marker" || key === "card_type" || key === "variant_parallel_rarity");
      ...
      const productVariantPair = [existing.key, item.key].includes("variant_parallel_rarity")
        && [existing.key, item.key].some((key) => key === "product_identity" || key === "product_set" || key === "franchise_brand");
      ...
      if (brandProductSetPair) {
        ...
        if (brandComparable
          ...
          && !productComparable.startsWith(`${brandComparable} `)) return false;
      }
    ...
    const keepNew = item.required && !existing.required
      || (existing.key === "card_type" && item.key === "variant_parallel_rarity" && item.text.length > existing.text.length)
...
function compactVariantDescriptor(item) {
  const color = "(Black|Blue|Bronze|Gold|Green|Orange|Pink|Purple|Red|Silver|White|Yellow)";
  const compacted = titleCleanup(item.text)
    .replace(new RegExp(`\\b${color}\\s+(?:Refractor|Prizm|Foil)\\b`, "gi"), "$1")
    .replace(new RegExp(`\\b${color}\\s+(Wave|Shimmer|Sparkle|Speckle|Mojo|Ice)\\s+(?:Refractor|Prizm)\\b`, "gi"), "$1 $2");
  return {
    ...item,
    text: compacted
  };
...
function compactVariantItems(items, maxLength, trace) {
  ...
  current = current.map((item) => {
    if (!["card_variant", "variant_parallel_rarity"].includes(item.key) || !item.compactable) return item;
