...
export function serialLimitText(value, {
  ...
  // A numerator that survives the resolver comes from current-image evidence
  // (REVIEW-status serials are downgraded to denominator-only upstream), so
  // render the complete printed serial per LYNCA CSM Standard Card Grammar.
