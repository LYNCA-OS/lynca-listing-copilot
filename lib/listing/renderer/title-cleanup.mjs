...
export function titleCleanup(value) {
  return normalizeText(value)
    // Provider/catalog placeholders are absence markers, never buyer-facing
    // title text. Handle them at the shared renderer boundary so every title
    // family receives the same deterministic guard.
    .replace(/^(?:none|null|undefined|unknown|n\/?a)$/gi, " ")
    .replace(/[\[(]\s*(?:none|null|undefined|unknown|n\/?a)\s*[\])]/gi, " ")
    .replace(/\bPokémon\b/giu, "Pokemon")
    // Brand compounds like "Fleer/SkyBox" tokenize as one word and stop the
    // buyer-searchable brand tokens from matching; space the slash between
    // alphabetic words (serials 025/150 and codes TG22/TG30 are untouched).
    .replace(/\b([A-Za-z]{4,})\/([A-Za-z]{4,})\b/g, "$1 / $2")
    // Provider prose glues descriptor suffixes ("Passing The Torch-Auto",
    // "Meta-Rare"); the reviewed standard writes them as separate words.
    .replace(/\b([A-Za-z]{3,})-(Auto|Rare)\b/g, "$1 $2")
    .replace(/\bRain\s+Drops\s+Signatures\b/gi, "Raindrops Signatures")
    .replace(/\bRookie\s+Card\b/gi, "RC")
    // "Rated Rookie" is Donruss/Optic's on-card subset branding and the
    // reviewed title standard keeps it alongside RC — never collapse it.
    .replace(/\bRookie\s+RC\b/gi, "RC")
    .replace(/\bRC\s+RC\b/gi, "RC")
    .replace(/\bAutograph\b/gi, "Auto")
    .replace(/\bAutos\b/gi, "Auto")
    .replace(/\bBrasil\b/gi, "Brazil")
    .replace(/\bChr\.(?=\s|$)/gi, "Chrome")
    .replace(/\bRef\.(?=\s|$|[,;:])/gi, "Refractor")
...
export function productHierarchyText(resolved = {}) {
  let brand = brandIdentityText(resolved);
  const product = productSetText(resolved);
  let brandComparable = normalizeComparable(brand);
  const productComparable = normalizeComparable(product);

  if (brandComparable && productComparable && brandComparable !== productComparable) {
    const brandWords = brand.split(/\s+/).filter(Boolean);
    const productWordCount = productComparable.split(/\s+/).filter(Boolean).length;
    if (brandComparable.startsWith(`${productComparable} `)) {
      brand = titleCleanup(brandWords.slice(productWordCount).join(" "));
      brandComparable = normalizeComparable(brand);
    } else if (brandComparable.endsWith(` ${productComparable}`)) {
      brand = titleCleanup(brandWords.slice(0, -productWordCount).join(" "));
      brandComparable = normalizeComparable(brand);
    }
  }
  ...
  if (/^topps$/i.test(normalizeText(resolved.manufacturer))
    && /^bowman$/i.test(normalizeText(resolved.brand))) {
    return titleCleanup(`Bowman ${product}`);
  }
  if (/^topps$/i.test(brand) && /^bowman\b/i.test(product)) return titleCleanup(product);
  if (/^topps\s+bowman$/i.test(brand) && /^bowman\b/i.test(product)) return titleCleanup(product);
