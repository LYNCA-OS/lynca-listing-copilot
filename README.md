...
Provider routing is wired with Agnes as the intended primary provider and GPT-4.1 as the legacy emergency provider. The browser renders server-whitelisted provider buttons and sends explicit provider metadata with each request. Agnes is the default selected provider. GPT-4.1 is exposed as an emergency button when enabled, but it is not selected by default and can be used only through an explicit emergency action; the backend intentionally does not fall through from any Agnes unavailable state to GPT, including missing Agnes credentials, unsupported Agnes inputs, or an env-selected OpenAI default. Model IDs are allowlisted server-side: Agnes accepts `agnes-2.0-flash`; the emergency OpenAI path accepts `gpt-4.1-mini` or `gpt-4.1`. A fully disabled GPT emergency provider is hidden from the ordinary frontend provider list; if explicit emergency retry is disabled while the provider remains enabled, the UI can show the GPT button as unavailable.
...
The eval output also includes `final_approved_publish_accuracy`, `glare_impact`, `retrieval_provider_gains`, `vision_provider_comparison`, and `agnes_vs_openai_legacy`. These are diagnostics for final approved rows, glare samples, Brave recovery, eBay market-reference help, OWS fallback contribution, and Agnes/GPT-4.1 emergency differences; they do not replace the overall commercial denominator.
...
- [docs/architecture/phase-1-provider-routing-2026-06-22.md](docs/architecture/phase-1-provider-routing-2026-06-22.md) — Agnes primary and GPT-4.1 emergency routing boundary
