...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --set-env-vars "RECOGNITION_ALLOWED_IMAGE_HOSTS=${ALLOWED_HOSTS},ENABLE_IMAGE_DOWNLOAD=true,ENABLE_TESSERACT_OCR=false,ENABLE_OPENCV_RECTIFICATION=true,ENABLE_VISUAL_EMBEDDINGS=true,VISUAL_EMBEDDING_PRELOAD=true,VISUAL_EMBEDDING_MODEL_ID=google/siglip2-base-patch16-384,VISUAL_EMBEDDING_MODEL_REVISION=f775b65a79762255128c981547af89addcfe0f88,VISUAL_EMBEDDING_PREPROCESSING_VERSION=card-rectification-v1,VISUAL_EMBEDDING_DIMENSIONS=768,VISUAL_EMBEDDING_DEVICE=cpu,ENABLE_CANDIDATE_VERIFICATION=false,ENABLE_PADDLEOCR=false,PADDLEOCR_PRELOAD=false,WORKER_PROCESSES=1,RECOGNITION_REQUEST_TIMEOUT_SECONDS=${TIMEOUT}" \
