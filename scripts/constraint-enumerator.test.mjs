...
test("year can narrow product while same-year ambiguity remains UNKNOWN", () => {
  assert.equal(enumerateProduct({ set: "Refractor", year: "2024" }, model).value, "topps chrome");
  assert.equal(enumerateProduct({ set: "Tomorrow Set", year: "2027" }, model).status, outcomes.UNKNOWN);
});
...
test("shadow adapter never mutates resolver input; active adapter only adds evidence", () => {
  const input = {
    raw_observed_fields: { subject: "Victor Wembanyama", set: "Fade To Black", year: "2025", sport: "basketball" },
