...
assert.match(
  js,
  /本地预览已显示；正在校验并上传原图，点击开始识别后整批会持续入队/,
  "status copy should separate durable upload from writer-authorized recognition"
);
...
assert.match(
  js,
  /本地预览已显示；正在校验并上传原图，点击开始识别后整批会持续入队。/,
  "local preview preparation should announce the writer-authorized batch handoff"
);
assert.match(js, /卡片已进入识别队列；后续图片准备完成后会自动加入。/, "recognition intent should cover the still-arriving batch");
assert.match(
  js,
  /setStatus\("本地预览已显示；正在校验并上传原图，点击开始识别后整批会持续入队。",\s*\{\s*busy:\s*true\s*\}\)/,
  "preview preparation should render as an active waiting state without authorizing recognition"
);
