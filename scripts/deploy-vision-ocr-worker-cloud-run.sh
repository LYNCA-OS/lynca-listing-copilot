...
deploy_vision_revision() {
  gcloud run deploy "$SERVICE_NAME" \
    ...
    --set-env-vars "RECOGNITION_ALLOWED_IMAGE_HOSTS=${RECOGNITION_ALLOWED_IMAGE_HOSTS:?set to the storage host for the target project},RECOGNITION_MAX_IMAGE_BYTES=26214400,RECOGNITION_MAX_TOTAL_PIXELS=50000000,ENABLE_IMAGE_DOWNLOAD=true,OCR_BACKEND=google_vision,VISION_USE_ADC=true,VISION_FEATURE_TYPE=DOCUMENT_TEXT_DETECTION,VISION_TIMEOUT_SECONDS=30,RECOGNITION_REQUEST_TIMEOUT_SECONDS=30,WORKER_PROCESSES=1" \
