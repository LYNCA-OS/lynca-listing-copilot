...
export function metricRow(report = {}, path = "", concurrencyOverride = null) {
  ...
  const row = {
    ...
    submission_concurrency: numberOrNull(report.submission_concurrency ?? report.configured_submission_concurrency ?? report.concurrency),
    provider_concurrency: numberOrNull(report.provider_concurrency ?? report.execution_control_snapshot?.global_provider_concurrency),
    ...
    writer_visible_recognition_p50_ms: numberOrNull(summary.writer_visible_recognition_p50_ms),
    writer_visible_recognition_p95_ms: numberOrNull(summary.writer_visible_recognition_p95_ms),
    writer_visible_recognition_measurement_rate: numberOrNull(summary.writer_visible_recognition_measurement_rate),
