...
export function payloadForItem(item = {}, index = 0, images = itemImages(item), {
  ...
  if (worldKnowledgeProposals) providerOptions.v4_world_knowledge_proposals = true;
...
function providerDiagnosticsFromSummary(summary = {}) {
  ...
  const output = {
    ...
    reasoning_tokens: numberOrNull(token.reasoning_tokens ?? request.reasoning_tokens),
    visible_output_tokens: numberOrNull(token.visible_output_tokens ?? request.visible_output_tokens),
...
function sessionL2Summary(statusPayload = {}) {
  ...
  return {
    ...
    reasoning_tokens: providerDiagnostics.reasoning_tokens,
    visible_output_tokens: providerDiagnostics.visible_output_tokens,
...
export function mergeJobDiagnosticsIntoResult(row = {}, statusPayload = {}) {
  ...
  return compactObject({
    ...
    reasoning_tokens: providerDiagnostics.reasoning_tokens ?? row.reasoning_tokens ?? null,
    visible_output_tokens: providerDiagnostics.visible_output_tokens ?? row.visible_output_tokens ?? null,
...
async function runOne({
  ...
  if (queueMode && speculative) {
    ...
    return compactObject({
      ...
      reasoning_tokens: finalProviderDiagnostics.reasoning_tokens,
      visible_output_tokens: finalProviderDiagnostics.visible_output_tokens,
...
export function summarize(results = [], { runWallMs = null } = {}) {
  ...
  return {
    ...
    identity_cache_hit_count: results.filter((item) => item.identity_cache_hit === true).length,
    identity_cache_provider_call_skipped_count: results.filter((item) => item.provider_call_skipped === true).length,
    identity_cache_version_match_count: results.filter((item) => item.cached_result_version_match === true).length,
    identity_cache_miss_reason_breakdown: countBy("identity_cache_miss_reason"),
...
export async function runV4EbaySmoke({
  ...
  const report = {
    ...
    world_knowledge_proposals_enabled: worldKnowledgeProposals,
...
export async function main(argv = process.argv, env = process.env) {
  ...
  const report = await runV4EbaySmoke({
    ...
    worldKnowledgeProposals: hasFlag(argv, "--world-knowledge-proposals"),
