...
async function preingestItem({
  ...
  return {
    ...
    cache_hit: response.data?.preingestion_cache_hit === true,
    cache_reason: response.data?.preingestion_cache_reason || null,
...
async function runOne({
  ...
  if (queueMode && speculative) {
    ...
    return compactObject({
      ...
      preingestion_cache_hit: preingestionResult?.cache_hit ?? null,
      preingestion_cache_reason: preingestionResult?.cache_reason || null,
...
export function resultFromBatchJob(prepared = {}, batchPoll = {}, thinkMs = 0) {
  ...
  return compactObject({
    ...
    preingestion_cache_hit: preingestion.cache_hit ?? null,
    preingestion_cache_reason: preingestion.cache_reason || null,
...
export function summarize(results = [], { runWallMs = null } = {}) {
  ...
  return {
    ...
    preingestion_cache_hit_count: results.filter((item) => item.preingestion_cache_hit === true).length,
    preingestion_cache_reason_breakdown: countBy("preingestion_cache_reason"),
...
export async function main(argv = process.argv, env = process.env) {
  ...
  process.stdout.write([
    ...
    `preingestion_cache_hits: ${report.summary.preingestion_cache_hit_count}`,
    `preingestion_cache_reasons: ${JSON.stringify(report.summary.preingestion_cache_reason_breakdown)}`,
