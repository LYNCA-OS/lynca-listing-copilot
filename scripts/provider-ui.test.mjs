...
assert.doesNotMatch(js, /body\.provider = provider/, "title intent must not select an algorithm provider");
assert.doesNotMatch(js, /defaultProviderOptions/, "frontend must not own algorithm defaults");
assert.match(js, /withRecognitionRequestIntent/, "frontend should submit a stable recognition intent contract");
assert.match(js, /defaultRecognitionProfileId/, "frontend should select a product profile instead of algorithm switches");
...
assert.doesNotMatch(js, /provider_options:\s*{/, "title intent must not include provider options");
assert.doesNotMatch(js, /body\.explicitEmergency = Boolean/, "frontend must not select an emergency algorithm path");
...
assert.doesNotMatch(js, /create_l1_job:\s*false/, "frontend must not select internal recognition stages");
assert.doesNotMatch(js, /create_l2_job:\s*true/, "frontend must not select internal recognition stages");
assert.match(recognitionProfileAdapter, /create_l1_job:\s*false/, "server profile should preserve the L2-only production plan");
assert.match(recognitionProfileAdapter, /create_l2_job:\s*true/, "server profile should preserve the writer-visible final L2 plan");
