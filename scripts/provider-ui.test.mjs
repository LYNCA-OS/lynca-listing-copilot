...
assert.match(js, /Promise\.allSettled\(batches\.map/, "one failed status shard must not discard successful status shards");
...
assert.match(queuedBatchPollSource, /if \(shouldRender\) renderResults\(\)/, "queued polling should rebuild the batch only when a title reaches a terminal state");
