...
assert.equal(sealedSelfContamination.metrics.retrieval_recall_at_20.rate, 1);
assert.equal(sealedSelfContamination.metrics.selection_accuracy_given_retrieved_at_20.denominator, 0);
...
assert.equal(sealedSelfContamination.cards[0].correct_candidate_id, "independent");
assert.equal(sealedSelfContamination.cards[0].selected_candidate_correct, null);
...
const recordContract = evaluateV4ChainOracleAudit({
  dataset: {
    ...dataset,
    items: [{
      ...dataset.items[0],
      reviewed_ground_truth: {
        fields: {
          year: { reviewed_status: "CONFIRMED", reviewed_value: "2024" },
          subject: { reviewed_status: "UNKNOWN", reviewed_value: "Ignored Player" }
        }
      },
      retrieval_ground_truth: { retrieval_evaluable: true, identity_fields: { year: "2024" } }
    }, {
      item_id: "excluded-card",
      reviewed_ground_truth: { fields: { year: { reviewed_status: "CONFIRMED", reviewed_value: "2025" } } },
      retrieval_ground_truth: { retrieval_evaluable: false }
    }]
  },
  trace,
  independentIdentityOnly: true
});
assert.equal(recordContract.data_quality.source_dataset_card_count, 2);
assert.equal(recordContract.data_quality.dataset_card_count, 1);
assert.equal(recordContract.data_quality.excluded_non_independent_identity_card_count, 1);
assert.equal(recordContract.data_quality.reviewed_field_count, 1);
assert.equal(recordContract.metrics.evidence_oracle_recall.rate, 1);
