...
function activeV4Rows({ tenantId, recognitionSessionId, fields }) {
  const founderBetaWebReceipt = {
    ...noSearchDurableReceipts(fields).founderBetaWebReceipt,
    schema_version: "founder-beta-web-receipt-v2",
    outcome: "NOT_USED",
    semantic_state_sha256: sha256Json(fields)
  };
  const setCardNameRelationReceipt = noSearchDurableReceipts(fields)
    .setCardNameRelationReceipt;
  const identitySha256 = sha256Json({ tenantId, recognitionSessionId });
  const tcgFieldSourceAuthorityReceipt = buildTcgFieldSourceAuthorityReceipt({
    fieldSources: ["set", "card_number"].map((field) => ({
    ...
    founderBetaWebReceipt,
    sourceExecution: {
      operationPayloadSha256: identitySha256,
      originalImageFingerprints: [`sha256:${identitySha256}`],
      recognitionImageFingerprints: [`sha256:${identitySha256}`],
  ...
  const resolvedFields = {
    ...fields,
    grammar: tcgGrammarContextClaimReceipt.resolved_grammar
  };
  const composed = composeLyncaStandardName(resolvedFields, {
...
const recordFields = parseCanonicalFields(JSON.parse(payload)).fields;
const recordRows = activeV4Rows({
  tenantId: "t1",
  recognitionSessionId: "sess-1",
  fields: recordFields
});
const record = {
  asset_id: "asset-1", recognition_session_id: "sess-1",
  resolution_id: recordRows.resolution.id,
  output_id: recordRows.output.id, canonical_payload: payload,
  output_title: recordRows.output.title,
  resolver_version: "thin-path-observation-only-v1",
  replay_rows: recordRows,
...
const futureReviewRows = activeV4Rows({
  tenantId: "t1",
...
const futureReviewRecord = {
  ...
  output_title: futureReviewRows.output.title,
...
// The v4 writer no longer guesses the meaning of receipt-less flat payloads.
// Rebuild the archived v2 rows instead and prove that their public bytes stay
// identical after the active writer moves forward.
...
for (const [grammar, canonicalPayload] of legacyFlatCases) {
  const legacyFields = parseCanonicalFields(canonicalPayload).fields;
  const legacyComposed = composeFromCanonicalFields(legacyFields);
  const legacyRows = buildCsmStageRows({
    tenantId: "t1",
    recognitionSessionId: `session-${grammar}`,
    fields: legacyFields,
  ...
  const legacyRecord = {
    ...
    output_title: legacyComposed.title,
    resolver_version: legacyRows.resolution.resolver_version,
    replay_rows: legacyRows,
    owner_execution_receipt: {
      version: CSM_OWNER_EXECUTION_RECEIPT_VERSION,
      sha256: OWNER_RECEIPT_SHA256
    }
...
}
{
  ...
  const activeRows = activeV4Rows({
    tenantId: "t1",
    recognitionSessionId: "session-standard-active",
    fields: recordFields
  });
  const activeRecord = {
    asset_id: "asset-standard-active",
    recognition_session_id: "session-standard-active",
    resolution_id: activeRows.resolution.id,
    output_id: activeRows.output.id,
    canonical_payload: payload,
    output_title: activeRows.output.title,
    composer_version: activeStandard.composer_version,
    marketplace_profile_version: activeStandard.marketplace_profile_version,
    resolver_version: activeRows.resolution.resolver_version,
    replay_rows: activeRows,
    tcg_grammar_context_source_execution_verified: true,
    owner_execution_receipt: {
      version: CSM_OWNER_EXECUTION_RECEIPT_VERSION,
      sha256: OWNER_RECEIPT_SHA256
  ...
  }), (error) => error?.message === "tcg_field_source_authority_receipt_invalid");
}
const activeStandard = activeWriterProjectionContract().standard;
for (const marketplaceProfileVersion of [
  ...
  assert.throws(
    ...
    (error) => error?.message === "tcg_field_source_authority_receipt_invalid",
    `a receipt-less ${marketplaceProfileVersion} flat record must fail closed`
  );
}
...
// --- a v4 stored title that disagrees with its sealed replay packet is rejected -
{
