...
assert.equal(cascade.label, "GPT-4.1 优先 · Agnes 辅助");
assert.equal(cascade.primary_provider_id, "openai_legacy");
assert.equal(cascade.secondary_provider_id, "agnes");
assert.equal(cascade.secondary_role, "secondary_verifier");
assert.equal(cascade.secondary_configured, true);
assert.equal(cascade.secondary_disabled_reason, null);
assert.equal(agnes.selectable, true);
assert.equal(agnes.role, "secondary_verifier");
...
openai = response.body.providers.find((provider) => provider.id === "openai_legacy");
assert.equal(cascade.selectable, true);
assert.equal(cascade.disabled_reason, null);
assert.equal(cascade.secondary_configured, false);
assert.equal(cascade.secondary_disabled_reason, "agnes_disabled_by_env");
