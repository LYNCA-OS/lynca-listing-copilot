...
assert.equal(vectorReadiness.details.online_retrieval_default_enabled, false);
...
assert.equal(response.body.execution_control.concurrency_contract.schema_version, "listing-concurrency-contract-v1");
assert.equal(response.body.execution_control.concurrency_contract.stages.catalog_retrieval.status, "FROZEN");
assert.equal(response.body.execution_control.concurrency_contract.stages.vector_query.concurrency, 3);
