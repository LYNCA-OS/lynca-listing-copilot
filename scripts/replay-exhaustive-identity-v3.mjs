...
// Zero-cost, evaluation-only replay of the v3 semantic identity veto on the
// paired canonical/exhaustive 150-card checkpoint.
...
const outPath = arg("--out", "artifacts/extreme-observation-2026-08-02/identity-replay-exhaustive-v3-150.json");
const canonicalByAsset = new Map(readRows(canonicalPath).filter((row) => row.arm === "thin_canonical_high" && row.fields).map((row) => [row.asset_id, row]));
const exhaustiveByAsset = new Map(readRows(exhaustivePath).filter((row) => row.arm === "exhaustive_observation_high").map((row) => [row.asset_id, row]));
const cards = [...canonicalByAsset.entries()].flatMap(([assetId, row]) => {
  ...
  const replay = replayCandidateIdentityV3(row.fields, facts);
  ...
  return [{ asset_id: row.asset_id, reference: row.reference, baseline_title: baseline.title, replay_title: candidate.title,
    baseline_score: before, replay_score: after, delta_f1: after.f1 - before.f1, source_logo_facts: facts,
    changes: replay.changes, rejected_facts: replay.rejected_facts }];
});
const result = {
  schema_version: "candidate-identity-replay-v3-exhaustive-150", resolver: "candidate-identity-replay-v3",
  authority: "evaluation_only", production_promoted: false, source: { canonical: canonicalPath, exhaustive: exhaustivePath },
  cards: cards.length, changed_cards: cards.filter((card) => card.changes.length).length,
  baseline_macro_f1: mean(cards.map((card) => card.baseline_score.f1)), replay_macro_f1: mean(cards.map((card) => card.replay_score.f1)),
  delta_macro_f1: mean(cards.map((card) => card.delta_f1)), wins: cards.filter((card) => card.delta_f1 > 1e-12).length,
  losses: cards.filter((card) => card.delta_f1 < -1e-12).length, ties: cards.filter((card) => Math.abs(card.delta_f1) <= 1e-12).length,
  cards_detail: cards
};
writeFileSync(outPath, `${JSON.stringify(result, null, 2)}\n`);
console.log(JSON.stringify({ resolver: result.resolver, cards: result.cards, changed_cards: result.changed_cards,
  baseline_macro_f1: result.baseline_macro_f1, replay_macro_f1: result.replay_macro_f1, delta_macro_f1: result.delta_macro_f1,
  wins: result.wins, losses: result.losses, ties: result.ties, out: outPath }, null, 2));
