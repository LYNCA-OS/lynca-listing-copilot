...
export const PRODUCTION_PARITY_READBACK_RECEIPT_SCHEMA =
  "production-parity-persisted-readback-receipt-v1";

const CANONICAL_PRODUCTION_ORIGIN = "https://listing.lyncafei.team";
const WRITER_JOURNEY_EVIDENCE_SCHEMA = "production-writer-journey-evidence-v5";
...
function parityEvidence(evidence, { deploymentUrl, gitSha }) {
  ...
  if (!/^csmsess_[0-9a-f]{40}$/.test(recognitionSessionId)
      ...
      || !exactKeys(versions, ["resolution_view_schema", "csm_contract", "resolver", "composer"])
...
async function main(argv) {
  ...
  if (mode === "asset-id") {
    process.stdout.write(`${productionParityAssetId(input)}\n`);
    return;
  }
  ...
  await writePrivate(values.get("--out"), verifyProductionParityReadback({
    ...input,
    resolutionView
  }));
}
