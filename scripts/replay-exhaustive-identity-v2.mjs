...
// Evaluation-only replay: reuse the already-paid exhaustive observation arm to
// test the guarded identity/affiliation admission rule against its paired
// canonical arm. This script never calls a provider and never mutates CSM.
...
const outPath = arg("--out", "artifacts/extreme-observation-2026-08-02/identity-replay-exhaustive-v2-150.json");

const canonicalByAsset = new Map(readRows(canonicalPath)
  .filter((row) => row.arm === "thin_canonical_high" && row.fields)
  .map((row) => [row.asset_id, row]));
const exhaustiveByAsset = new Map(readRows(exhaustivePath)
...
const cards = rows.map(({ row, observation }) => {
  ...
  const replay = replayCandidateIdentityV2(row.fields, facts);
...
const result = {
  schema_version: "candidate-identity-replay-v2-exhaustive-150",
