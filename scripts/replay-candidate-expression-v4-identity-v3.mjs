...
const candidates = new Map(rows(candidatePath).map((row) => [row.asset_id, row.candidate_facts || []]));
const cards = [...canonical.values()].filter((row) => candidates.has(row.asset_id)).map((row) => {
  ...
  return { asset_id: row.asset_id, reference: row.reference, baseline_title: baseline.title, replay_title: candidate.title,
    baseline_score: before, replay_score: after, delta_f1: after.f1 - before.f1, changes: replay.changes,
...
const result = {
  ...
  source: { canonical: canonicalPath, candidates: candidatePath, candidate_arm: "candidate_expression_v4_high" },
  cards: cards.length,
