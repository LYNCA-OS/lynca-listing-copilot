...
const internalGuardPatterns = Object.freeze({
  ...
  "api/v4/listing-preingest-worker.js": /\b(?:isV4WorkerRequest|isV4CronRequest)\s*\(/,
...
for (const file of [
  ...
  const routeSource = source(file);
  assert.match(routeSource, /readJsonPayload\(req,\s*\{\s*maxBytes:/, `${file} must use the shared bounded JSON reader`);
  assert.doesNotMatch(routeSource, /req\.on\(["']data["']/, `${file} must not maintain an unbounded local body accumulator`);
}

const browserPreingestSource = source("api/listing-preingest.js");
assert.doesNotMatch(
  browserPreingestSource,
  /buildPreingestionWorkerJobs|enqueuePreIngestionJobs|scheduleTrustedPreingestionOcrWake/,
  "pre-click browser preparation must not enqueue paid OCR work"
);
const v4PreingestSource = source("api/v4/listing-preingest.js");
assert.doesNotMatch(
  v4PreingestSource,
  /body\.bundle_id\s*\|\|\s*payload\.(?:preingestion_bundle_id|preingestionBundleId)/,
  "the V4 wrapper must never fall back to browser bundle identity"
);
const preingestWorkerSource = source("api/v4/listing-preingest-worker.js");
assert.doesNotMatch(preingestWorkerSource, /requireTenantAccess\s*\(/, "ordinary tenant sessions must not execute internal OCR work");

assert.equal(actualApiFiles.length, 54);
assert.equal(publicRoutes.length, 4);
assert.equal(internalSecretRoutes.length, 20);
