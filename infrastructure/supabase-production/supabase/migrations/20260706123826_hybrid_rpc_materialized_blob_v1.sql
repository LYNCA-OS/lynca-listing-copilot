...
as $function$
  ...
  candidate_text as (
    ...
    select
      ...
      concat_ws(':', cc.sport, cc.season_year, cc.product, public.catalog_players_text(cc.players), cc.card_number, cc.checklist_code) as identity_key,
      cc.canonical_title,
    ...
    where cc.review_status in ('REVIEW_REQUIRED', 'REVIEWED_INTERNAL')
      ...
      -- Pure-text queries pre-filter through the trigram index; anchored
      -- queries are already narrowed by the identity filters below.
      and (
        i.q = ''
        or i.subject <> ''
        or i.year_text <> ''
