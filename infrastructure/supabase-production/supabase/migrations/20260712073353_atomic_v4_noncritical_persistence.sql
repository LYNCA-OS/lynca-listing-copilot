...
begin
  if coalesce(p_session_id, '') = '' then raise exception 'missing_session_id'; end if;
  ...
  if jsonb_typeof(coalesce(p_field_evidence, '[]'::jsonb)) = 'array' then
    insert into public.v4_field_evidence (
      id, recognition_session_id, field_name, field_value, display_status,
      source_type, provenance, confidence, created_at
    )
    select
      row.id, p_session_id, row.field_name, row.field_value,
      coalesce(nullif(row.display_status, ''), 'NORMAL'),
      coalesce(nullif(row.source_type, ''), 'V4_RESULT_ADAPTER'),
      coalesce(row.provenance, '{}'::jsonb), row.confidence,
      coalesce(row.created_at, now())
    from jsonb_to_recordset(coalesce(p_field_evidence, '[]'::jsonb)) as row(
      id text, recognition_session_id text, field_name text, field_value text,
      display_status text, source_type text, provenance jsonb,
      confidence double precision, created_at timestamptz
    )
    where coalesce(row.id, '') <> '' and coalesce(row.field_name, '') <> ''
  ...
  if p_candidate_trace is not null and jsonb_typeof(p_candidate_trace) = 'object' then
    insert into public.v4_candidate_traces (
      id, recognition_session_id, schema_version, trace, created_at
    ) values (
    ...
    on conflict (id) do update set
      schema_version = excluded.schema_version, trace = excluded.trace;
  ...
  if p_catalog_gap is not null and jsonb_typeof(p_catalog_gap) = 'object' then
    insert into public.v4_catalog_gap_queue (
      id, recognition_session_id, asset_id, gap_type, observed_fields,
      candidate_snapshot, draft_title, status, created_at, updated_at
    ) values (
      p_catalog_gap ->> 'id', p_session_id, nullif(p_catalog_gap ->> 'asset_id', ''),
  ...
  if p_quality_ledger is not null and jsonb_typeof(p_quality_ledger) = 'object' then
    insert into public.v4_production_quality_ledger (
      id, recognition_session_id, schema_version, route, provider, model, status,
      confidence, latency_ms, input_tokens, output_tokens, total_tokens,
      provider_error_type, token_diagnostics, timing, route_plan, warnings,
      persistence_summary, provider_diagnostics, pipeline_node_ledger, created_at
    ) values (
    ...
    on conflict (id) do update set
      route = excluded.route, provider = excluded.provider, model = excluded.model,
      ...
      token_diagnostics = excluded.token_diagnostics, timing = excluded.timing,
      route_plan = excluded.route_plan, warnings = excluded.warnings,
  ...
  return jsonb_build_object(
    'saved', true, 'recognition_session_id', p_session_id,
