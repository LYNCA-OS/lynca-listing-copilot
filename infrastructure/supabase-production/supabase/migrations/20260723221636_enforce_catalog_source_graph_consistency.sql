-- Catalog provenance is a graph invariant.
...
begin
  if tg_table_name = 'catalog_products' then
    if new.source_id is distinct from old.source_id and (
      exists (
        select 1 from public.catalog_sets catalog_set
        where catalog_set.product_id = old.id
          and catalog_set.source_id is distinct from new.source_id
      )
      or exists (
        select 1 from public.catalog_cards card
        where card.product_id = old.id
          and card.source_id is distinct from new.source_id
      )
    ) then
      raise exception using errcode = '23514', message = 'catalog_product_source_graph_conflict';
    end if;
    return new;
  end if;

  select product.source_id into v_product_source_id
  from public.catalog_products product
  where product.id = new.product_id;

  if v_product_source_id is null or v_product_source_id is distinct from new.source_id then
    raise exception using errcode = '23514', message = 'catalog_product_source_mismatch';
  end if;
  ...
  if tg_table_name = 'catalog_sets' then
    if tg_op = 'UPDATE'
      ...
      raise exception using errcode = '23514', message = 'catalog_set_source_graph_conflict';
    end if;
    return new;
  end if;
  ...
  if new.set_id is not null then
    ...
    if v_set_source_id is null
      ...
      raise exception using errcode = '23514', message = 'catalog_set_source_mismatch';
    end if;
  end if;
