...
const LEAK = resolve(HERE, "excluded.json");
...
// this cohort.
const CSM_LANGUAGE = await readFile(resolve(HERE, "canonical-language.txt"), "utf8");
...
// Both arms put their text in `instructions`, so POSITION is held constant and
// only CONTENT differs. The user message carries images alone.
//
// Last attempt buried the stable text in the same message as the images, so the
// message changed every card and no prefix could be shared: 0/49 cache hits on
// a 2,946-token prompt. `instructions` is a separate block ahead of the input.
const ARMS = {
  control: { key: "control", instructions: CAPTURED_E1AE_CANONICAL_FIELDS_PROMPT },
  treatment: { key: "treatment", instructions: `${CSM_LANGUAGE}\n\n${CAPTURED_E1AE_CANONICAL_FIELDS_PROMPT}` }
};
...
async function callProvider({ apiKey, instructions, imageDataUrls }) {
...
async function main() {
  ...
  await writeFile(resolve(OUT, "run-manifest.json"), JSON.stringify({
    schema: "field-semantics-instructions-paired-v1",
    model: MODEL,
    effort: EFFORT,
    cards: cohort.length,
    ...
    csm_language_sha256: sha256(CSM_LANGUAGE),
