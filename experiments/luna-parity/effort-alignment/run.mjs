...
// The variable under test. Production pins "low". Measured spend at that
// setting: median 245 reasoning tokens per card, 34 cards in 100 under 200,
// one card at zero -- for a task that must read a slab label, both faces, an
// optical finish and a small foil serial, then apply eight naming rules.
const CONTROL_EFFORT = "low";
const TREATMENT_EFFORT = "high";
...
// Same prompt, schema, images and composer. Only the reasoning budget moves.
// The prompt-context arm was already measured and produced no signal
// (n=50, +0.0178, p=0.50), so holding the prompt fixed is what keeps this a
// single-variable test.
const ARMS = {
  control: { key: "control", prompt: CAPTURED_E1AE_CANONICAL_FIELDS_PROMPT, effort: CONTROL_EFFORT },
  treatment: { key: "treatment", prompt: CAPTURED_E1AE_CANONICAL_FIELDS_PROMPT, effort: TREATMENT_EFFORT }
};
...
async function callProvider({ apiKey, prompt, imageDataUrls, effort }) {
...
async function main() {
  ...
  console.log(`prompt sha256 ${sha256(ARMS.control.prompt).slice(0, 16)} (identical in both arms)`);
  console.log(`control effort=${CONTROL_EFFORT}   treatment effort=${TREATMENT_EFFORT}`);
    ...
    for (const armKey of order) {
      ...
      try {
        const { body, latencyMs } = await callProvider({
          apiKey, prompt: ARMS[armKey].prompt, imageDataUrls: dataUrls, effort: ARMS[armKey].effort
        });
  ...
  await writeFile(resolve(OUT, "run-manifest.json"), JSON.stringify({
    schema: "effort-alignment-paired-v1",
    model: MODEL,
    control_effort: CONTROL_EFFORT,
    treatment_effort: TREATMENT_EFFORT,
    cards: cohort.length,
    excluded_leaking_cards: [...LEAKING],
    prompt_sha256: sha256(ARMS.control.prompt)
  }, null, 1));
  console.log("done");
}
